...
$set = array(
  "banknaam"        => "Quinet bank",           //  volledige banknaam
  "depot"           => "QUI",                   //  depotbankcode v/d bank
  "filePrefix"      => "quintet",               //  fileprefix
  ...
  "transactieCodes" => "quintetTransactieCodes",    //  tabelnaam van de transactiecodes
  "bankCode"        => "Quintetcode"                //  veldnaam bankcode in de Fondsentabel
);
...
while ($data = fgetcsv($handle, 4096, $set["fileDelimit"]))
{
  ...
  if($data[0] == "Filename" and $data[1] == "Referencenumber")
  {
    continue; // sla header regel over
  }
  ...
  if(strtoupper(trim($data["transactieCode"])) == "DCC-FOREX")
  {
    $tRegels[$data['transactieId']][] = $data;
    continue;
  }
...
foreach($tRegels as $tId => $rows)
{
  ...
  if(count($rows)>2)
  {
    ...
    {
      $meldArray[] = "<span style='color:maroon; font-weight: bold;'>regel {$data['regelnr']}: meer dan twee regels. FX handmatig boeken</span>";
      $error[]     = "{$data['regelnr']}: meer dan twee regels. FX handmatig boeken";
    }
    }
