<?
/*
    AE-ICT sourcemodule created 28 apr. 2021
    Author              : Chris van Santen
    Filename            : airsRM_import.php
...
while ($data = fgetcsv($handle, 4096, $set["fileDelimit"]))
{
  ...
  if ($row == 1)
  {
	...
	if (checkVoorDubbelInRM(array(
                                  "bankTransactieId" => $data[0],
                                  "Rekening"         => $data[10],
                                  "Boekdatum"        => $data[1]
  )))
  {
  ...
  $query = "
    ...
    , `Fonds`             = '{$data[2]}'
    , `Omschrijving`      = '{$data[3]}'
    , `Grootboekrekening` = '{$data[4]}'
    , `Memoriaalboeking`  = '{$data[5]}'
    , `Valuta`            = '{$data[6]}'
    , `Valutakoers`       = '{$data[7]}'
    , `Aantal`            = '{$data[8]}'
    , `Fondskoers`        = '{$data[9]}'
    ...
    , `Credit`            = '{$data[12]}'
    , `Bedrag`            = '{$data[13]}'
    , `Transactietype`    = '{$data[14]}'
    , `settlementDatum`   = '{$data[17]}'
    , `orderId`           = '{$data[18]}'
    ";
...
aangemaakte mutatieregels : <?=$totalAdded?><BR>
<?=$skipped?>
<hr>
