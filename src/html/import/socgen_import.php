...
$set = array(
  "banknaam"        => "Societe Generale",       //  volledige banknaam
  "depot"           => "SOC",                    //  depotbankcode v/d bank
  "filePrefix"      => "socgen",                 //  fileprefix
  ...
  "transactieCodes" => "socgenTransactieCodes",  //  tabelnaam van de transactiecodes
  "bankCode"        => "SOCcode"                 //  veldnaam bankcode in de Fondsentabel
);
...
<?
  ...
  $x          = 0;
	...
	{
		$parts        = explode(":", $errorm);
    $foutregels[] = trim($parts[0]);
?>
  	<tr>
...
while ($data = fgetcsv($handle, 4096, $set["fileDelimit"]))
{
  ...
  if ($data["isin"] != "" AND $data["bankCode"] != "" AND strtoupper($data["isin"]) != "NULL")
  {
    getFonds();
