...
/*
 		Author  						: $Author: cvs $
 		Laatste aanpassing	: $Date: 2018/05/23 13:25:44 $
 		File Versie					: $Revision: 1.6 $

 		$Log: lynx_import.php,v $
 		Revision 1.6  2018/05/23 13:25:44  cvs
 		no message
...
//  6 => 'TransactionCode *',
//  7 => 'Number of Sec',
//  8 => 'Bought/Sold Interest Instr Ccy',
//  9 => 'Price',
//  10 => 'Account currency',
//  11 => 'Exch Rate (Instr to account)',
//  12 => 'GrossAmount InstrCCY',
//  13 => 'Commission Acc Ccy',
//  14 => 'Commission InstrCcy',
//  15 => 'External Cost Acc Ccy',
//  16 => 'External Cost InstrCcy',
//  17 => 'Taxes Acc ccy',
//  18 => 'Taxes InstrCcy',
//  19 => 'NetAmount AccountCCY',
//  20 => 'TradeDate',
//  21 => 'SettlementDate',
//  22 => 'TransReference',
//  23 => 'FreeText',
...
//  27 => 'ExpDate',
//  28 => 'Call/Put',
//  29 => 'Secutity Type',  //toegevoegde kolommen 23-5-2018
//  30 => 'ComCCY FX',
//  31 => 'Value',
//  32 => 'Proceeds',
//  33 => 'ProfitLoss',
...
include_once("lynx_functies.php");
...
$csvRegels = 1;
include("lynx_validate.php");

$volgnr = 1;
$prb->setLabelValue('txt1','Validatie van Lynx CSV bestand');
...
$prb->setLabelValue('txt1','Converteren records ('.$csvRegels.' records)');
 ...
 $query = "SELECT LYNXcode,doActie FROM lynxTransactieCodes";
 $DB->executeQuery($query);
 while ($row = $DB->nextRecord())
 {
   $transactieMapping[$row["LYNXcode"]] = $row["doActie"];
 }
...
//   exit();
  ...
  if($LynxCodeNotFound AND trim($data[4]) != "N/A" )
  {
   ...
   $num = count($data);
   $val = $transactieMapping[trim($data[1])."-".trim($data[6])];
   ...
   {
     do_error(trim($data[1])."-".trim($data[6]));
   }
...
Records in LYNX CSV bestand :<?=$row?><br>
