...
$set = array(
  "banknaam"        => "KNOX bank",         //  volledige banknaam
  "depot"           => "KNOX",                   //  depotbankcode v/d bank
  "filePrefix"      => "knox",                    //  fileprefix
  "fileDelimit"     => "\t",                     //  CSV delimter
  "fileEnclosure"   => '"',              // enclosure char csv file
  "decimalSign"     => ".",                     //  decimaalteken in getallen
  "thousandSign"    => "",                      //  duizend scheidingsteken
  "headerRow"       => true,                    //  is de eerste regel een header?
  "transactieCodes" => "knoxTransactieCodes",     //  tabelnaam van de transactiecodes
  "bankCode"        => "KNOXcode",                 //  veldnaam bankcode in de Fondsentabel
);
...
?>
    ...
    <input type="hidden" name="reversedStornoTrans" value="<?=implode(",", $stornoTrans)?>">
...
while ($data = fgetcsv($handle, 4096, $set["fileDelimit"]))
{
  ...
  $array1 = array('SALE-CURS', 'BUY-CURP', 'DEP-INTR', 'WDRL-INTC', 'WDRL-INTR', 'WDRL-FEES', 'DEP-CASD', 'WDRL-CASW', 'WDRL-OTHR', 'DEP-OTHR', 'DEL-WIRO', 'REC-WIRI');

  if (
    ($data["isin"] != "" OR $data["bankCode"] != "" ) &&
    !in_array($data["transactieCode"], $array1) || // FX
    (in_array($data["transactieCode"], array('DEP-INTR', 'WDRL-INTC', 'WDRL-INTR')) && $data['bankCode'] != $data['bankCodeDiv']) // Rente
  )
  {
    getFonds();
  }
