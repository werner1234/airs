<?
/*
    AE-ICT sourcemodule created 22 mrt. 2021
    Author              : Chris van Santen
    Filename            : dil_import.php
...
$csvRegels = 1;
include("dil_validate.php");

$volgnr = 1;
$prb->setLabelValue('txt1','Validatie van DIL CSV bestand');
...
$prb->setLabelValue('txt1','Converteren records ('.$csvRegels.' records)');
 ...
 $query = "SELECT bankCode,doActie FROM dilTransactieCodes";
 $DB->executeQuery($query);
...
while ($data = fgetcsv($handle, 4096, "\t"))
{
  ...
  if (!$fileType = checkRowType($data, $row))
  {
    //$meldArray[] = "{$row}: checkRowType() voldoet niet";
    continue;
  }
  $data["rowType"] = $fileType;
...
Records in DIL CSV bestand :<?=$row?><br>
