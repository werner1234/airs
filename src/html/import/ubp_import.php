...
/*
 		...
 		$Log: ubp_import.php,v $
 		Revision 1.7  2020/06/08 12:21:48  cvs
 		call 8668
 		
 		Revision 1.6  2020/01/22 11:30:46  cvs
 		call 8196
 		
 		Revision 1.5  2019/10/25 15:12:21  cvs
 		call 8196
 		
 		Revision 1.4  2018/10/08 14:49:31  cvs
 		call 7160
 		
 		Revision 1.3  2017/09/20 06:17:48  cvs
 		megaupdate 2722
...
include_once("ubp_functies.php");

include_once "../../classes/AIRS_import_zoekVervang.php";
$zv = new AIRS_import_zoekVervang("UBP");

include_once "../../classes/AIRS_import_afwijkingen.php";
$afw = new AIRS_import_afwijkingen("UBP");
...
$csvRegels = 1;
include("ubp_validate.php");

$volgnr = 1;
$prb->setLabelValue('txt1','Validatie van UBP CSV bestand');
...
$prb->setLabelValue('txt1','Converteren records ('.$csvRegels.' records)');
 ...
 $query = "SELECT bankCode,doActie FROM ubpTransactieCodes";
 $DB->executeQuery($query);
...
//  $comRows[$data[18]] = array($data[3],$data);


  $data["row"] = $row;
  $comRows[$data[18]][trim($data[3])."-".trim($data[4])][$data[5]][] = $data;
}
...
Records in UBP CSV bestand :<?=$row?><br>
