...
include_once("cacaeis_functies.php");

include_once "../../classes/AIRS_import_afwijkingen.php";
$afw = new AIRS_import_afwijkingen("KAS");

$skipFoutregels = array();
$meldArray = array();

$bankTransactieTable   = "caceisTransactieCodes";
$depotBank      = "KAS";
$bankNaam       = "Caceis";
$rowDelimiter   = ",";
...
if ($handle = @fopen($file, "r"))
{
  ...
  $delimit = substr($dataRaw,16,1);
  if ($delimit == "," OR $delimit == ";")
  {
    $rowDelimiter = $delimit;
  }
}
...
$csvRegels = 1;
include("cacaeis_validate.php");

$volgnr = 1;
$prb->setLabelValue('txt1',"Validatie van {$bankNaam} CSV bestand");
...
//  }
  ...
  if ($data["saldoRegel"] )
  {
    continue;  // saldoregels overslaan bij CASH
  }
  if ($data["unsettled"] )
  {
    $meldArray[] = "regel ".$row.": stukkenmutatie unsettled";
    continue;  // saldoregels overslaan bij CASH
  }
...
//  debug($data);
  ...
  {
    $data["row"] = $row;
    $fxArray[$data["transactieId"]][] = $data;
    continue;
  }
...
Records in <?=$bankNaam?> CSV bestand :<?=$row?><br>
