...
/*
 		Author  						: $Author: cvs $
 		Laatste aanpassing	: $Date: 2019/06/05 12:53:36 $
 		File Versie					: $Revision: 1.3 $

 		$Log: lanschot_import.php,v $
 		Revision 1.3  2019/06/05 12:53:36  cvs
 		call 7844
...
include_once("lanschot_functies.php");

include_once "../../classes/AIRS_import_afwijkingen.php";
$afw = new AIRS_import_afwijkingen("FVL");
...
$csvRegels = 1;
include("lanschot_validate.php");

$volgnr = 1;
$prb->setLabelValue('txt1','Validatie van FVL CSV bestand');
...
<?
    ...
    {
      echo "<br/><a href='wwwFoutenBestand.php?bank=FVL' ><button id='btnDownload'>Download FOUTEN bestand</button></a><br/>";
    }
?>
...
$prb->setLabelValue('txt1','Converteren records ('.$csvRegels.' records)');
 ...
 $query = "SELECT FVLCode,doActie FROM lanschotTransactieCodes";
 $DB->executeQuery($query);
 while ($row = $DB->nextRecord())
 {
   $transactieMapping[$row["FVLCode"]] = $row["doActie"];
 }
...
//   exit();
   ...
   if ($data[22] <> "" AND $data[3] <> "")
   {
     ...
     $FLVCode = substr("00000000".trim($data[22]),-7);
     $query = "SELECT * FROM Fondsen WHERE FVLCode = '".$FLVCode."' ";
     $DB->SQL($query);
