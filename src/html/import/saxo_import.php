...
$set = array(
  "banknaam"        => "SAXO bank",              //  volledige banknaam
  "depot"           => "SAXO",                   //  depotbankcode v/d bank
  "filePrefix"      => "saxo",             //  fileprefix
  ...
  "transactieCodes" => "saxoTransactieCodes",    //  tabelnaam van de transactiecodes
  "bankCode"        => "SAXOcode"                //  veldnaam bankcode in de Fondsentabel
);
...
$afw = new AIRS_import_afwijkingen("SAXO");
...
<?php
    ...
    {
      echo "<br/><a href='wwwFoutenBestand.php?bank=SAXO' ><button id='btnDownload'>Download FOUTEN bestand</button></a><br/>";
    }
?>
	<br>
	<br>
...
while ($data = fgetcsv($handle, 4096, $set["fileDelimit"]))
{
  ...
  $idx = ($data["transactieId"] == "")?$data["rekening"].$data["afrekenValuta"].$data["row"]:$data["transactieId"];
  $idx = $idx."-".$data["rekening"]."-".$data["transactieCode"];
//  debug($data,$idx);
  if ($data["transactieCodeDetail"] == "")
  {
    $dataSet[$idx][] = $data;
  }
  else
  {
    $dataSet[$idx][$data["transactieCodeDetail"]] = $data;
  }
...
foreach ($dataSet as $transId => $data)
{
  ...
  $val        = $transactieMapping[$tcode];
  $transcode  = $tcode;  // tbv  errormelding
  $do_func    = "do_$val";
//debug($data, $do_func);
