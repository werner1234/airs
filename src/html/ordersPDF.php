...
/*
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2016/02/10 13:02:03 $
 		File Versie					: $Revision: 1.25 $

 		$Log: ordersPDF.php,v $
 		Revision 1.25  2016/02/10 13:02:03  rvv
 		*** empty log message ***
 		
 		Revision 1.24  2015/12/13 17:57:40  rvv
 		*** empty log message ***
...
if (!$_GET["orderid"] )
{
  ...
  {
   if($ordermoduleAccess==2)
     $query= "SELECT OrdersV2.id, OrdersV2.giraleOrder, OrdersV2.id as orderid, OrdersV2.fondsOmschrijving, OrdersV2.ISINcode as fondsCode, 
    ...
    LEFT Join Fondsen ON OrdersV2.fonds = Fondsen.Fonds
     WHERE OrderRegelsV2.orderid = OrdersV2.id AND OrdersV2.batchId='".$orderRec['batchId']."' ORDER BY OrderRegelsV2.add_date";
   else
    ...
    OrderRegels.client,OrderRegels.portefeuille,OrderRegels.rekeningnr,OrderRegels.valuta, Orders.Memo, Fondsen.Valuta as fondsValuta,Orders.koersLimiet
...
function AlteredHeader()
{
  ...
  {
    if($beheerderRec["Vermogensbeheerder"]=='AUR')
      $pdf->Image('rapport/logo/'.$beheerderRec["Logo"],8,8,60);
    else
      $pdf->Image('rapport/logo/'.$beheerderRec["Logo"],125,4,88);
  }
  $pdf->SetX(10);
  $pdf->SetY(17);
...
function AlteredFooter()
{
  ...
  $pdf->SetTableWidths(array("10",'20',"58","30","14","18","20","35"));
...
$pdf->SetTableWidths(array("10","25","5","30","40","35","23","23"));
...
if(count($uitvoering) > 0)
{
 ...
 if($beheerderRec['OrderLoggingOpNota']==1)
 {
  ...
  $pdf->SetTableWidths(array(10+25+5,60+30,"35"));
