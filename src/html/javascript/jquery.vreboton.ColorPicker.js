...
/* Handle keystrokes. */
	...
	_connectColorPicker: function(target, inst) {
		...
		// Create sample span
		inst._sampleSpan = $('<span class="ColorPickerDivSample" style="margin: 0px 0px 0px 4px; border: solid 1px #000; padding: 0px 10px; position: relative;cursor: pointer;background-color:' + inst._input[0].value + ';height:' + inst._input[0].offsetHeight + ';">&nbsp;</span>');
...
$.extend(ColorPickerInstance.prototype, {
	...
	_setValue: function (sel) {
		...
		{
		    this._input[0].value = $.attr(sel,'title').substring(1,7);
