(function()
{
  ...
  w._dynarch_menu_shadow.src = w._dynarch_menu_url + "img/shadow.png";
            ...
            )
    {
      ...
      if (tmp)
        try
        {
          ...
          DynarchMenu._addEvent(tmp, "mousedown", documentMouseDown);
          DynarchMenu._addEvent(tmp, "mouseup", documentMouseUp);
          DynarchMenu._addEvent(tmp, "mouseover", documentMouseOver);
        }
        catch (e)
        {
        }
...
{
  for (var i = DynarchMenu._menus.length;
          --i >= 0 && !callback(DynarchMenu._menus[i]);
          )
    ;
};
...
{
  var tmp, ctx = null, cfg = this.config, licl = li.className, icon = null, label, html_popup = true, tooltip, action = null, item, info, self = this, key = null, disabled = DynarchMenu._RE_DS.test(licl), nohover = DynarchMenu.getCA(li, "nohover");
  if (cfg.d_profile)
  ...
  info = new DynarchMenu.MenuItem(
          {
            html_popup: html_popup, separator: html_popup || !/\S/.test(label) && !icon, icon: icon, label: label, parent: parent, submenu: null, tooltip: tooltip, action: action, menu: this, disabled: disabled, nohover: nohover
          }
  );
  if (li.id)
  ...
  else
  {
    ...
    else
    {
      ...
      tmp = DynarchMenu._createElement("td", item);
      tmp.className = "test";
      tmp.setAttribute("url",info.action.url);
      tmp.setAttribute("location",'content');
  ...
  {
    if (html_popup)
      item.onmouseover = DynarchMenu.EventHandlers.popup_resetActive;
    if (key)
...
function documentMouseDown(ev) {ev || (ev = window.event);var el = is_ie ? ev.srcElement : ev.target, j;for (j = el; j && !j.__msh_info; j = j.parentNode);if (!j || j.__msh_info.base) {DynarchMenu._closeOtherMenus(j && j.__msh_info.menu);}};
...
DynarchMenu.prototype.createMenuTree = function(ul, horiz)
{
  ...
  for (i = tmp.length;
          --i >= 0;
          (div = DynarchMenu._createElement("div", div)).className = tmp[i])
    ;
...
DynarchMenu._addEvent = function(el, evname, func)
{
  ...
  if (el.addEventListener) {
    ...
    {
      if ( evname == 'mouseover') {
        el.addEventListener(evname, function (e){var t=DynarchMenu._C,n,r;if(t&&t.config.electric){e||(e=window.event);n=is_ie?e.srcElement:e.target;for(;n&&!n.__msh_info;n=n.parentNode);if(!n){r=t.config.electric;if(r==true)r=1;if(!DynarchMenu._T)DynarchMenu._T=setTimeout("DynarchMenu._closeOtherMenus(null);DynarchMenu._T = null;",r)}else DynarchMenu._clearTimeout()}}, true);
      } 
      else if(evname == 'mousedown') {
        el.addEventListener(evname, function (ev) {ev || (ev = window.event);var el = is_ie ? ev.srcElement : ev.target, j;for (j = el; j && !j.__msh_info; j = j.parentNode);if (!j || j.__msh_info.base) {DynarchMenu._closeOtherMenus(j && j.__msh_info.menu);}}, true);
      } else if(evname == 'mouseup') {
        el.addEventListener(evname, function (e){e||(e=window.event);if(DynarchMenu._msupTimeout)return false;var t=DynarchMenu._C,n,r;if(t){n=is_ie?e.srcElement:e.target;for(;n&&!(r=n.__msh_info);n=n.parentNode);if(!n)DynarchMenu._closeOtherMenus(null);else if(r&&r.exec)r.exec()}DynarchMenu._C=null;DynarchMenu._activeItem=null}, true);
      }
    }
  }
...
DynarchMenu._nfo =
        {
          product: "hmenu-2.4.1", licensee: "Dynarch.com user: aecvs", license_key: "trial-701441", purchase_date: "Sun Mar 20 09:43:46 2005 GMT", license_type: "trial"
        };
