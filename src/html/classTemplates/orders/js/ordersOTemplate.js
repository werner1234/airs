...
var Oform = 'form#NominaalMeervoudigForm';
...
function preSave (event, element) {
  ...
  $.when(validateOform()).done(function (a1) {
...
function preSubmit (event) {
  showLoading('Opslaan');
  submitOform().done(redirect); // submit and redirect
  removeLoading();
}
...
function validateOform() {
  ...
  $(Oform + ' .help-block.with-errors').remove();
  $(Oform).find(':input').removeClass('input_error');
      ...
      if (data.saved == false) {
        $.each(data.error, function (field, fieldData) {
          ...
          if ( field == 'orderStatus' ) {
            ...
            // $(Oform + ' [name=' + field + ']').toggleClass('input_error');
            // $(Oform + ' [name=' + field + ']').parent().append('<span class="help-block with-errors"><br />' + fieldData.message + '</span>');
...
function submitOform() {
  ...
  $(Oform + ' .help-block.with-errors').remove();
  $(Oform).find(':input').css('border-color', ''); //reset inputfield style
  
  var postData = $(document).find(Oform).serializeArray();
  var formURL = $(Oform).attr("action");
...
$(function() {
  ...
  // $('#bedrag').inputmask({ "mask": "9", "repeat": '*', "greedy": false, rightAlign: true });
