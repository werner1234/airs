...
$RenHoogwidgetHelp = new AE_cls_WidgetsHelper($columnDataRenHoog, $var_columns);

//$out = $tmpl->parseBlock("kop",array("header" => "Top $rows, Hoogste rendement ($showPort) <!--ttl-->", "btnSetup" => "btn_".$dialogName));
$parse = array(
  "header" => vtb("Top %s, Hoogste rendement (%s)", array($rows, vt($showPort))),
...
';
    ...
    foreach ($RenHoogwidgetHelp->columnData as $k=> $v)
    {
      if ($v["show"] != 1) {continue;}
      $out .= "<div class='rTableHead' ".$RenHoogwidgetHelp->getWidth($v["width"]) ." btr-title='".$v['btrTitle']."' title='".$v["title"]."' data-toggle='tooltip'> ".$v["koptxt"]."</div>\n";
    }
...
';
    foreach ($RenHoogwidgetHelp->columnData as $k=> $v)
    {
      ...
      $out .= "<div btr-title='".$v['btrTitle']."' title='".$v["title"]."' class='rTableCell ".$v["class"]." $negren' ".$RenHoogwidgetHelp->getWidth($v["width"]) ."> ".$value."</div>\n";
    }
...
<!-- Dialoog '.$dialogName.' -->
<div id="setupWidget_'.$dialogName.'" title="' . vt('Instellen hoogste rendement') . '" class="setupWidget">
  <div class="formblock">
    <div class="formlinks">' . vt('Portefeuilleselectie') . '</div>
    <div class="formrechts">
      <select name="port_'.$dialogName.'" id="port_'.$dialogName.'">
        '.$RenHoogwidgetHelp->makeAccessOptions($showPort).'
      </select>
    </div>
  </div>
  <div class="formblock">
...
<p></p>
  <p>
    '.$RenHoogwidgetHelp->makeHtmlInput().'
  </p>
    ...
    var setup'.$dialogName.'Dialog = $("#setupWidget_'.$dialogName.'").dialog(
    {
      ...
      buttons:
      {
        ...
        "' . vt('Opslaan') . '": function()
        {
          ...
          $(".kolCheck'.$RenHoogwidgetHelp->uid.'").each(function()
          {
