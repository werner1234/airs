...
$var_portSelect = $USR."_widget_var_".$dialogName."_portefeuilleSelectie";
...
$portSel = $cfg->getData($var_portSelect);
if ($rows == 0) $rows = 10;
...
//$out = $tmpl->parseBlock("kop",array("header" => "Top $rows, Vermogen ($showPort) <!--ttl-->", "btnSetup" => "btn_".$dialogName));

$parse = array(
  "header" => vtb("Top %s, Vermogen (%s) ", array($rows, $showPort)),
...
';
    foreach ($widgetHelpVermogen->columnData as $k=> $v)
    {
      ...
      $out .= "<div title='".$v["title"]."' btr-title='".$v['btrTitle']."' class='rTableCell ".$v["class"]." $negren' ".$widgetHelpVermogen->getWidth($v["width"]) ."> ".$value."</div>\n";
    }
$out .= '
  </div>
';
...
$out .= '
  ...
  <!-- Dialoog '.$dialogName.' -->
  <div id="setupWidget_'.$dialogName.'" title="' . vt('Instellen vermogen') . '" class="setupWidget">
    <div class="formblock">
