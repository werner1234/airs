<?php
/*
    AE-ICT sourcemodule created 21 sep. 2022
    Author              : Chris van Santen
    Filename            : CRM_naw_zipUload.php
...
//exit;
  ...
  if ($error == "")
  {
    ...
    else
    {
      ...
      {
        $filesize   = round(filesize($dirName."/".$item)/1024,1);
        if ($indeling == "map")
        {
          $p = explode("/", $item);
          if ($stdOmschrijving != "")
        ...
        $portefeuille = mysql_real_escape_string($p[0]);
...
?>
  ...
  <div class="pageContainer">
    <h2>&nbsp;&nbsp;&nbsp; Uploaden documenten in .ZIP file</h2>
    <?
    if ($error != "")
    {
      ?>
...
?>
      ...
      <fieldset>
        ...
        <br/>
          ...
          <div id="uitlegContent">
            <b>Per map:</b> de mapnaam bestaat uit het portefeuillenummer. Alle bestanden daaronder worden aan dit portefeuillenummer gekoppeld<br>
            <b>Per bestand:</b> het portefeuillenummer moet in de bestandsnaam zitten. Alle bestanden worden bekeken tot 1 map diep.
          </div>
        ...
        <div>
          <div class="lDiv">Zipbestand indeling:</div>
            ...
            <option value="file" >Per bestand</option>
          </select>
          <div id="fileSelect">
            <div class="lDiv">Scheidingsteken na portefeuille:</div>
            <select name="delimiter" id="delimiter">
              <option value="#"># (hekje)</option>
              <option value="-">- (min teken)</option>
...
//      debug($files);
      foreach ($files as $index=>$docs)
      {
        ...
        if ($exists)
        {
          ...
          {
            $chk = "<input type='checkbox' name='check#{$port}||{$index}' class='vink'>";
            $chkP = "<input type='checkbox' name='portaal#{$port}||{$index}' class='vinkP'>";
            $fsColor  = "DarkGreen";
          }
          echo "
            <tr>
            ...
            <td style='color:{$fsColor}; text-align: right'>{$filesize} Kb</td>
            <td> {$chkP}</td>
            </tr>
      ";
