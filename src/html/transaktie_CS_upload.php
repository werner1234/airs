...
<style>
  ...
  <div>
    <article id="addDialog">
      <p>
        Selecteer <?=strtoupper($depot)?> bestanden:
      </p>
      <form action="transaktie_fileUpload.php?depot=<?=$depot?>" class="dropzone" id="inpDropzone"></form>
    </article>
  </div>
    ...
    $(document).ready(function ()
    {
      ...
      init: function() {
        this.on("totaluploadprogress", function(totaluploadprogress)
        {
          ...
          {
            parent.frames['listframe'].location.href = "transaktie_CS_fileList.php?depot=<?=$depot?>&random=" + Math.floor((Math.random() * 10000) + 1);
          }
        });
