...
/*
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2015/12/23 16:04:30 $
 		File Versie					: $Revision: 1.5 $

 		$Log: ordersPDF_L40.php,v $
 		Revision 1.5  2015/12/23 16:04:30  rvv
 		*** empty log message ***
 		
 		Revision 1.4  2015/09/30 07:54:15  rvv
 		*** empty log message ***
 		
 		Revision 1.3  2014/07/27 11:29:40  rvv
 		*** empty log message ***
 		
 		Revision 1.2  2012/12/02 11:04:26  rvv
 		*** empty log message ***
 		
 		Revision 1.1  2012/11/28 17:02:33  rvv
 		*** empty log message ***
 		
 		Revision 1.1  2011/12/27 09:36:24  rvv
 		*** empty log message ***
...
if (!$_GET["orderid"] )
{
  ...
  $query = "SELECT * FROM Gebruikers WHERE Gebruiker='".$USR."' limit 1 ";
  $db->SQL($query);
  ...
  $query= "SELECT OrderRegels.brutoBedrag,Orders.giraleOrder, Orders.id, Orders.orderid, Orders.fondsOmschrijving, Orders.fondsCode, OrderRegels.aantal, Orders.transactieSoort, Orders.transactieType,
  Depotbanken.Omschrijving as depotbanknaam, OrderRegels.client,OrderRegels.portefeuille,OrderRegels.rekeningnr,OrderRegels.valuta, Orders.Memo, Fondsen.Valuta as fondsValuta,Orders.koersLimiet
    FROM (Orders, OrderRegels)
    LEFT Join Fondsen ON Orders.fonds = Fondsen.Fonds
    LEFT Join Depotbanken ON Orders.depotbank = Depotbanken.depotbank
  ...
  $kop[] = array("field" =>"Huidige datum"    ,"value" =>date("d-m-Y"));
  $kop[] = array("field" =>"Medewerker"    ,"value" =>$gebruikersnaam);
  $kop[] = array("field" =>"Depotbank"    ,"value" =>$depotbank);
  ...
  $pdf->SetTableWidths(array("10",'20',"60","30","20","25","30","30","40"));
  $pdf->SetTableAligns(array("R","L","L","R","L","R","L","L","R"));
  $pdf->ln();


  $pdf->AddTableRow(array("Pos",'Kenmerk',"Fonds omschrijving","ISIN-code","Valuta","Aantal","Transactiesoort","Transactietype","Geschat orderbedrag"));
...
//listarray($orderRegels);
 ...
 foreach ($orderRegels as $row)
 {
   ...
   if($row['transactieType']=='Limiet')
    $row['transactieType'].=": ".formatAantal($row['koersLimiet'],2);
...
//  
  ...
  if($fondsenRec['Fondseenheid'] <> 0)
   ...
   $pdf->AddTableRow(array($positie,$row['orderid'],$row['fondsOmschrijving'],$row['fondsCode'],$row['fondsValuta'],$aantal,$row['transactieSoort'],$row['transactieType'],$bedrag));
   if($row['Memo'] <> '')
   {
     $backupW=$pdf->widths;
     $backupA=$pdf->aligns;
  ...
  $query = "SELECT Orders.*, Depotbanken.Omschrijving as depotbanknaam FROM Orders 
  LEFT Join Depotbanken ON Orders.depotbank = Depotbanken.depotbank
  WHERE orderid='".$_GET["orderid"]."' ";
  $db->SQL($query); 
  $orderRec = $db->lookupRecord();

      $depotbank=$orderRec['depotbanknaam'];
    if($client == '')
  ...
  $kop[] = array("Order kenmerk",$orderRec["orderid"],
  "Laatste koers",date("d-m-Y",db2jul($fondsKoersRec["Datum"]))." ".formatGetal($fondsKoersRec["Koers"],3));
  $kop[] = array("Fonds",$orderRec["fondsOmschrijving"],
  "Looptijd",$__ORDERvar["tijdsSoort"][$orderRec["tijdsSoort"]].$looptijd);
  $kop[] = array("ISIN",$orderRec["fondsCode"],
  "Huidige datum",date("d-m-Y"));
  $kop[] = array("TransactieSoort",$__ORDERvar["transactieSoort"][$orderRec["transactieSoort"]],
  "Medewerker",$gebruikersnaam);
  $aantal = formatAantal($orderRec["aantal"],0,true);
  $kop[] = array("Aantal",$aantal.$verschilTxt,
  "Depotbank",$depotbank);
  $kop[] = array("Fondsvaluta",$fondsenRec['Valuta'],
   "Memo",$orderRec['memo']);
  $kop[] = array("TransactieType",$__ORDERvar["transactieType"][$orderRec["transactieType"]].$trType);
...
$pdf->SetTableWidths(array(45,80,45,120));
$pdf->SetTableAligns(array("L","L","L","L"));

for ($x=0;$x < count($kop);$x++)
{
   $pdf->AddTableRow(array($kop[$x][0],$kop[$x][1],$kop[$x][2],$kop[$x][3]));
}
...
while ($row = $db->nextRecord())
{
  ...
  if($gemiddeldePrijsValuta <> 0)
    ...
    if(strlen($row['client']) > 21)
      $row['client']=substr($row['client'],0,19).'...';
...
function AlteredHeader()
{
  ...
  if (file_exists('rapport/logo/'.$beheerderRec["Logo"]) && filetype('rapport/logo/'.$beheerderRec["Logo"]) != 'dir' )
    $pdf->Image('rapport/logo/'.$beheerderRec["Logo"],240,4,50);

  $pdf->SetX(10);
  $pdf->SetY(10);
  //$pdf->SetFont("Arial","",10);
  //$pdf->MultiCell(190,4, $adres,0,'R');
  $pdf->ln();
...
function AlteredFooter()
{
  ...
  $pdf->SetX(10);
  $pdf->SetY(275);
  $pdf->Cell(190,4, "Printinformatie: ".$USR." ".date('d-m-Y h:i'),0,0,"L");
