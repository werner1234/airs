...
/*
    ...
    Author  						: $Author: cvs $
 		...
 		$Log: controlePortefeuilles_readSNSCVSFile.php,v $
...
function readSNSCvsFile($fileSAL,$filePOS)
{
...
";
  ...
  while ($data = fgetcsv($handle, 1000, ";"))
  {
    ...
    $aantal = str_replace(",",".",($data[4]));


    $bankOutput[$ndx][rekeningnr] = trim(intval($data[1])).trim($data[2]);
  ...
  while ($tmpdata = $tmpDB->nextRecord())
  {
    ...
    //$outputArray[$portefeuilleInCsv][B][$idx][match] = 0;
      ...
      $outputArray[$portefeuille][B][$ndx][isin] = $_isin;
  	 if ( $_isin <> "")
     {
   		 $query = "SELECT * FROM Fondsen WHERE snsCode = '".$_isin."' LIMIT 1 ";
    	 $DB->SQL($query);
    	 $DB->Query();
    	 if (!$fonds = $DB->nextRecord())
    	   $outputArray[$portefeuille][B][$ndx][fonds] = "SNS code komt niet voor fonds tabel ($_isin)";
    	 else
    	 {
      ...
      {
    	 $outputArray[$portefeuille][B][$ndx][fonds] = "Geen SNS code bij ".$portefeuille;
      }
    }
    $ndx++;
  }
