...
include_once("jb_reconV3Functies.php");
...
<style>
  ...
  <ul>
    ...
    if (!jb_validateFile($_GET["file"]))
    {
      listError($error);
      exit;
    }

    $recon = new AE_cls_reconV3("JUL BAER", $_SESSION["vbArray"],$_GET["manualBoekdatum"]);
    $batch = "JB_" . date("ymd_His");
    $recon->batch = $batch;
    ...
    ob_flush();
    flush();
    $bankRecords = jb_recon_readBank($_GET["file"]);
