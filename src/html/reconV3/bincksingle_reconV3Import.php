<?php
/*
    AE-ICT sourcemodule created 11 okt. 2021
    Author              : Chris van Santen
    Filename            : bincksingle_reconV3Import.php
...
if ($_REQUEST["soort"] == "cash")
{
  $_SESSION["cashRecon"] = $_REQUEST;
  header("location:bincksingle_cashRecon.php");
  exit;
}


include_once("bincksingle_reconV3Functies.php");
...
<style>
  ...
  <ul>
    ...
    $recon = new AE_cls_reconV3("BINS", $_SESSION["vbArray"],$_GET["manualBoekdatum"]);
    if ($_GET["modus"] == DBreconRead)
    {
      $recon->dbReadserver = DBreconRead;
      echo " <li> Gebruik de Shadowserver " . date("H:i:s") . "</li>";
      ob_flush();
      flush();
    }
    $batch = "BINCKS_" . date("ymd_His");
    $recon->batch = $batch;
    $recon->addToReconLog("Binck recon gestart, ".implode(", ",$_SESSION["vbArray"]));
    ob_flush();
    flush();
    ...
    ob_flush();
    flush();
    $recon->addToReconLog("binck_recon_readBank");
...
$recon->trPileToDb();
$recon->addToReconLog("BinckSingle recon klaar..");
?>
