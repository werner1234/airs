...
/*
    ...
    Author  						: $Author: cvs $
 		...
 		$Log: rabobank_reconV3Import.php,v $
...
include_once("rabobank_reconV3Functies.php");
...
if (!rabo_validateFile($_GET["file"],$_GET["file2"]))
{
  listError($error);
  exit;
}

$recon = new AE_cls_reconV3("RABO", $_SESSION["vbArray"],$_GET["manualBoekdatum"]);
$batch = "RABO_".date("ymd_His");
$recon->batch = $batch;
...
$cRecords .= rabo_recon_readBank($_GET["file"]);
echo " <li> bankbestand bevatte ".$cRecords." dataregels</li>";
ob_flush();flush();
