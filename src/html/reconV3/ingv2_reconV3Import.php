...
include_once("ingv2_reconV3Functies.php");
...
<style>
  ...
  <ul>
    ...
    if (!ingv2_validateFile($_GET["file"]))
    {
      listError($error);
      exit;
    }

    $recon = new AE_cls_reconV3("ING", $_SESSION["vbArray"],$_GET["manualBoekdatum"]);
    $batch = "ING_" . date("ymd_His");
    $recon->batch = $batch;
    ...
    ob_flush();
    flush();
    $bankRecords = ingv2_recon_readBank($_GET["file"]);
