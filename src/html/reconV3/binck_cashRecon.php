...
include_once("binck_cashReconV3Functies.php");
...
<style>
  ...
  <ul>
    ...
    $recon = new AE_cls_reconV3("BIN", $_SESSION["vbArray"],$_GET["manualBoekdatum"]);
    $batch = "BIN_" . date("ymd_His");
    $recon->batch = $batch;
    $starttijd = mktime();
    ...
    $recon->addToReconLog("binck_cashRecon_readBank");
    echo " <li>Bank rekeningen gestart om " . date("H:i:s") . "</li>";
    ob_flush();
    flush();
    $bankRecords = binck_cashRecon_readBank($_GET["file"]);
...
$recon->trPileToDb();
$recon->addToReconLog("Binck recon klaar..");
?>
