...
/*
    ...
    Filename            : degirov2_reconV3Import.php
...
if ($_REQUEST["soort"] == "cash")
{
  ...
  header("location:abnv2_cashRecon.php");
  exit;
}

include_once("abnv2_reconV3Functies.php");
...
<style>
  ...
  <ul>
    ...
    $recon->batch = $batch;
    $recon->addToReconLog("AAB recon gestart, ".implode(", ",$_SESSION["vbArray"]));
    ob_flush();
    flush();
    ...
    ob_flush();
    flush();
    $recon->addToReconLog("abnv2_recon_readBank");
    $bankRecords = abnv2_recon_readBank($_GET["file"]);
...
$recon->trPileToDb();
$recon->addToReconLog("AAB recon klaar..");
?>
