...
include_once("fundshare_reconV3Functies.php");
...
<style>
  ...
  <ul>
    ...
    if (!fundshare_validateFile($_GET["file"]))
    {
      listError($error);
      exit;
    }

    $recon = new AE_cls_reconV3("FSE", $_SESSION["vbArray"],$_GET["manualBoekdatum"]);
    $batch = "FSE_" . date("ymd_His");
    $recon->batch = $batch;
    ...
    ob_flush();
    flush();
    $bankRecords = fundshare_recon_readBank($_GET["file"]);
