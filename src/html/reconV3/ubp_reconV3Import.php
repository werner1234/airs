...
include_once("ubp_reconV3Functies.php");
...
<style>
  ...
  <ul>
    ...
    if (!ubp_validateFile($_GET["file"]))
    {
      listError($error);
      exit;
    }

    $recon = new AE_cls_reconV3("ubp", $_SESSION["vbArray"],$_GET["manualBoekdatum"]);
    $batch = "ubp_" . date("ymd_His");
    $recon->batch = $batch;
    ...
    ob_flush();
    flush();
    $bankRecords = ubp_recon_readBank($_GET["file"]);
