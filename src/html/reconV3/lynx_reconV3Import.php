...
include_once("lynx_reconV3Functies.php");
...
<style>
  ...
  <ul>
    ...
    if (!lynx_validateFile($_GET["file"]))
    {
      listError($error);
      exit;
    }

    $recon = new AE_cls_reconV3("LYNX", $_SESSION["vbArray"],$_GET["manualBoekdatum"]);
    $batch = "LYNX_" . date("ymd_His");
    $recon->batch = $batch;
    ...
    ob_flush();
    flush();
    $bankRecords = lynx_recon_readBank($_GET["file"]);
