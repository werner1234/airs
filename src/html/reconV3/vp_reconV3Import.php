...
include_once("vp_reconV3Functies.php");
...
<style>
  ...
  <ul>
    ...
    if (!vp_validateFile($_GET["file"]))
    {
      listError($error);
      exit;
    }

    $recon = new AE_cls_reconV3("VP", $_SESSION["vbArray"],$_GET["manualBoekdatum"]);
    $batch = "VP_" . date("ymd_His");
    $recon->batch = $batch;
    ...
    ob_flush();
    flush();
    $bankRecords = vp_recon_readBank($_GET["file"]);
