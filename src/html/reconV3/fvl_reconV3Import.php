...
include_once("fvl_reconV3Functies.php");
...
<style>
  ...
  <ul>
    ...
    if (!fvl_validateFile($_GET["file"]))
    {
      listError($error);
      exit;
    }

    $recon = new AE_cls_reconV3("FVL", $_SESSION["vbArray"],$_GET["manualBoekdatum"]);
    $batch = "FVL_" . date("ymd_His");
    $recon->batch = $batch;
    ...
    ob_flush();
    flush();
    $bankRecords = fvl_recon_readBank($_GET["file"]);
