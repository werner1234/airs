...
include_once("optimix_reconV3Functies.php");
...
<style>
  ...
  <ul>
    ...
    if (!optimix_validateFile($_GET["file"]))
    {
      listError($error);
      exit;
    }

    $recon = new AE_cls_reconV3("OPT", $_SESSION["vbArray"],$_GET["manualBoekdatum"]);
    $batch = "OPT_" . date("ymd_His");
    $recon->batch = $batch;
    ...
    ob_flush();
    flush();
    $bankRecords = optimix_recon_readBank($_GET["file"]);
...
<li>airs rekeningnrs: ".$recon->getAirsReknrs(false)."</li>
    ...
    flush();
    $recon->findUnmatched();
    debug($recon->unmatchArray,"unmatch Array");
