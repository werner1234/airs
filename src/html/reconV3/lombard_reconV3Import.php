...
include_once("lombard_reconV3Functies.php");
...
<style>
  ...
  <ul>
    ...
    if (!lombard_validateFile($_GET["file"]))
    {
      listError($error);
      exit;
    }

    $recon = new AE_cls_reconV3("LOM", $_SESSION["vbArray"],$_GET["manualBoekdatum"]);
    $batch = "LOMBARD_" . date("ymd_His");
    $recon->batch = $batch;
    ...
    ob_flush();
    flush();
    $bankRecords = lombard_recon_readBank($_GET["file"]);
