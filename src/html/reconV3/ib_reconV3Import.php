...
include_once("ib_reconV3Functies.php");
...
<style>
  ...
  <ul>
    ...
    if (!ib_validateFile($_GET["file"]))
    {
      listError($error);
      exit;
    }

    $recon = new AE_cls_reconV3("IB", $_SESSION["vbArray"],$_GET["manualBoekdatum"]);
    $batch = "IB_" . date("ymd_His");
    $recon->batch = $batch;
    ...
    ob_flush();
    flush();
    $bankRecords = ib_recon_readBank($_GET["file"]);
