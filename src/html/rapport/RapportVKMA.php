...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2020/05/24 14:40:56 $
File Versie					: $Revision: 1.8 $

$Log: RapportVKMA.php,v $
Revision 1.8  2020/05/24 14:40:56  rvv
*** empty log message ***

Revision 1.7  2020/05/24 14:32:04  rvv
*** empty log message ***

Revision 1.6  2020/05/24 12:41:14  rvv
*** empty log message ***

Revision 1.5  2020/05/23 16:38:02  rvv
*** empty log message ***

Revision 1.4  2020/05/20 17:10:34  rvv
*** empty log message ***

Revision 1.3  2020/04/19 06:32:30  rvv
*** empty log message ***

Revision 1.2  2020/04/18 17:05:42  rvv
*** empty log message ***

Revision 1.1  2020/04/15 16:10:25  rvv
*** empty log message ***
...
class RapportVKMA
{
	function RapportVKMA($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
    ...
    if($this->pdfVullen==true)
		{
	  	...
	  	$this->pdf->row(array('Doorlopende kosten', $this->formatGetal($totaalDoorlopendekosten*$this->scaleFactor, 0) . ' '.$this->pdf->rapportageValuta ));
			$this->pdf->excelData[]=array('','Doorlopende kosten', '','',round($totaalDoorlopendekosten*$this->scaleFactor, 0), $this->pdf->rapportageValuta );
		...
		if($this->pdfVullen==true)
		{
			...
			$this->pdf->ln();
			$this->pdf->excelData[]=array();
			$this->pdf->row(array('Eindvermogen', $this->formatGetal($eindWaarde*$this->scaleFactor, 0) . ' '.$this->pdf->rapportageValuta ));
    ...
    $grootboekKosten=array();
     ...
     // listarray($data);
      $kostenProcent=($data['totaal']-$data['vasteKosten'])/$eindWaarde*100;
		  //echo "$kostenProcent=(".$data['totaal']."-".$data['vasteKosten'].")/$eindWaarde*100;<br>\n";
      $kostenHerrekend=(($eindWaarde*$kostenProcent/100)*$this->scaleFactor)+$data['vasteKosten'];
      //echo "$kostenHerrekend=(($eindWaarde*$kostenProcent/100)*$this->scaleFactor)+".$data['vasteKosten'].";<br>\n";
      $kostenProcent=$kostenHerrekend/($eindWaarde*$this->scaleFactor)*100;
      //echo "$kostenProcent=$kostenHerrekend/($eindWaarde*$this->scaleFactor)*100;<br>\n";
      //echo "$kostenProcent=".$data['totaal']."/$eindWaarde*100;<br>\n";
      
      $data['totaal']=$kostenHerrekend/$this->scaleFactor;
     // echo $data['totaal']."=$kostenHerrekend/".$this->scaleFactor.";<br>\n";exit;
      
     // echo "$kostenHerrekend=$eindWaarde*$kostenProcent/100;<br>\n";
     // echo "$kostenHerrekend*".$this->scaleFactor."=".($kostenHerrekend*$this->scaleFactor)." <br>\n";exit;
			if($this->pdfVullen==true)
			{
				$this->pdf->row(array($data['Omschrijving'],$this->formatGetal($kostenHerrekend,0),$this->formatGetal($kostenProcent,2). ' %' ));
				$this->pdf->excelData[]=array('',$data['Omschrijving'],'',round($kostenHerrekend,0),round($kostenProcent,2) );
...
$this->totalen['gemiddeldeWaarde']=$perfTotaal['eindwaarde'];
    $totaalSom=array();
    $sub=array();
    ...
    foreach ($categorieData as $categorie=>$fondsData)
    {
      ...
      foreach ($fondsData['fondsen'] as $id=>$fonds)
      {
          ...
          if($sub['aantal']>1 )
          {
						...
						if($this->pdfVullen==true)
						{
							...
							$this->pdf->row(array('', '        subtotaal ' . $laatsteFonds,
																...
																$this->formatGetal($sub['eindwaarde'] * $kostenPercentage['percentage'] / 100, 0),
        ...
        $bijdrageVKM=$sub['weging']*$totaalKostenPercentage;
				$dlkostenAbsoluut=$sub['eindwaarde']*$totaalKostenPercentage/100;
				if($DB->records()>0)
			...
			if($this->pdfVullen==true)
			{
				if ($lastRegio <> '')
				{
					...
					$this->pdf->row(array('', '  subtotaal ' . $perRegio[$hoofdcategorie][$categorie][$lastRegio]['omschrijving'],
														...
														$this->formatGetal($subregio['eindwaarde']*$this->scaleFactor, 0),
														$this->formatGetal($subregio['resultaat'] / $subregio['eindwaarde'] * 100, 2),
