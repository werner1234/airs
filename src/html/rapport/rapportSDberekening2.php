...
$Log: rapportSDberekening2.php,v $
Revision 1.3  2019/11/06 15:53:41  rvv
*** empty log message ***

Revision 1.2  2019/01/05 09:19:01  rvv
*** empty log message ***

Revision 1.1  2018/12/29 13:56:27  rvv
*** empty log message ***
...
class rapportSDberekening2
{

	function rapportSDberekening2($portefeuille,$rapportageDatum,$gebruikStartdatumMeerjarenrendement=1)
	{
	  global $__appvar;
	  $this->db = new DB();
...
//$this->settings['Startdatum']='2014-06-01';
    ...
    if($this->settings['SdFrequentie']=='j')
      $this->settings['aantalPerJaar']=1;
  ...
  function addReeks($verdeling='totaal',$fonds='',$verdelingOpDatum=false,$uitsplitsingFonds=false)
  {
    $this->getReeksen($verdeling,$fonds,$verdelingOpDatum,$uitsplitsingFonds);
 ...
 function rollingPeriodSD($reeksen)
 {
    ...
    foreach($reeksen as $cat=>$catData)
    {
      ...
      else
      {
        ...
        foreach($catData as $index=>$waarden)
        {
          ...
          //echo $cat." ".$waarden['datum']." ".count($buffer)."<br>\n";
  ...
  function getFondsVerdeling($fonds,$tot)
  {
    $DB=new DB();
    ...
    {
      $query="SELECT fonds,percentage,vanaf FROM benchmarkverdelingVanaf WHERE benchmark='$fonds' AND vanaf < '$tot' ORDER BY vanaf desc limit 1";
      $DB->SQL($query);
      $DB->Query();
      if($DB->records()>0)
      {
        $datum = $DB->nextRecord();
        $query = "SELECT fonds,percentage,vanaf FROM benchmarkverdelingVanaf WHERE benchmark='$fonds' AND vanaf = '" . $datum['vanaf'] . "'";
  ...
  function fondsPerf($fonds,$van,$tot,$uitsplitsingFonds=false)
  {
    $DB=new DB();
  
    if($uitsplitsingFonds==true)
      $verdeling=$this->getFondsVerdeling($fonds,$tot);
    else
      $verdeling=array($fonds=>100);
   // echo "$fonds,$van,$tot<br>\n";
   //  listarray($verdeling);
    $totalPerf=0;
    foreach($verdeling as $fonds=>$percentage)
    {
      if($uitsplitsingFonds==true)
        $verdeling2=$this->getFondsVerdeling($fonds,$tot);
      else
        $verdeling2=array($fonds=>100);

   //   listarray($verdeling2);
      foreach($verdeling2 as $fonds2=>$percentage2)
      {

        $query = "SELECT Fonds, Datum, Koers FROM Fondskoersen WHERE datum  <= '$van' AND Fonds='" . $fonds2 . "' ORDER BY Datum DESC LIMIT 1";
        $DB->SQL($query);
        $startKoers = $DB->lookupRecord();

        $query = "SELECT Fonds, Datum, Koers FROM Fondskoersen WHERE datum  <= '$tot' AND Fonds='" . $fonds2 . "' ORDER BY Datum DESC LIMIT 1";
        $DB->SQL($query);
        $eindKoers = $DB->lookupRecord();
        $perfPeriode = ($eindKoers['Koers'] - $startKoers['Koers']) / ($startKoers['Koers']);
      //  echo "$fonds => $fonds2 |  $perfPeriode=(" . $eindKoers['Koers'] . " - " . $startKoers['Koers'] . ") / (" . $startKoers['Koers'] . "); <br>\n";

        $totalPerf += ($perfPeriode * $percentage * $percentage2/100);
       // echo "$totalPerf += ($perfPeriode * $percentage * $percentage2/100);<br>\n";
      }
    }
  //  listarray($verdeling);
  ...
  function getReeksen($verdeling='totaal',$fonds='',$verdelingOpDatum=false,$uitsplitsingFonds=false)
  {
     global $__appvar;
     $db=new DB();
     ...
     if($verdeling=='benchmark'||$verdeling=='benchmarkTot')
     {
       $fondsArray=array();
        ...
        if(count($fondsArray)>0 || $fonds != '' || $verdelingOpDatum==true)
	        ...
	        foreach($perioden as $periode)
          {
            if(count($fondsArray) > 0 || ($verdelingOpDatum==true && count($this->consolidatiePortefeuilles)>0))
            {
              if($verdelingOpDatum==true)
              {
                ...
                foreach($benchmarkFonds as $fondsDeel=>$aandeel)
                {
                  ...
                  $verdeling=$this->getFondsVerdeling($fonds,$periode['stop']);
                  foreach($verdeling as $fonds=>$percentage)
                  {
                    if (isset($fondsVerdeling[$data['fonds']]))
                    {
                      $fondsVerdeling[$fonds] += $percentage * $aandeel / 100;
                    }
                    else
                    {
                      $fondsVerdeling[$fonds] = $percentage * $aandeel / 100;
            ...
            elseif($verdeling=='benchmarkTot')
            {
              ...
              //echo $periode['stop']." $fonds <br>\n";ob_flush();
            }
           //echo " $fonds,".$periode['start'].",".$periode['stop']." |$uitsplitsingFonds| <br>\n";
            $this->reeksen[$verdeling][$periode['stop']]['perf']=$this->fondsPerf($fonds,$periode['start'],$periode['stop'],$uitsplitsingFonds);
           // listarray($this->reeksen[$verdeling][$periode['stop']]['perf']);
  ...
  function riskAnalyze($eerste='totaal',$tweede='benchmark',$verloop=false)
  {
   ...
   // echo "<br>\ndatum;perf;benchmark<br>\n";
    ...
    {
      if(isset($this->reeksen[$tweede][$datum]))
      {
        ...
        //echo "$datum;".$waarden['perf'].";$benchmarkPerf <br>\n";
      }
...
//    listarray($maxDrawdownArray);
  ...
  function getPeriodeInstellingen()
  {
    ...
    elseif($this->settings['SdFrequentie']=='w')
      $periode=$this->indexberekening->getWeken($this->settings['julStartdatum'],$this->settings['julRapportageDatum'],true);
    ...
    if($this->settings['SdFrequentie']=='j')
      $periode=$this->indexberekening->getJaren($this->settings['julStartdatum'],$this->settings['julRapportageDatum']);
...
Rekeningmutaties.Boekdatum >= '".$this->settings['Startdatum']."' AND  Rekeningmutaties.Boekdatum <= '".$this->settings['RapportageDatum']."'
...
foreach($afmVerdelingCategorien as $filterCategorie=>$afmVerdeling)
{
  ...
  foreach ($perioden as $periode)
  {
    ...
    if(substr($datumBegin,0,4) <> substr($datumEind,0,4))
      continue;
