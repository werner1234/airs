     ...
     $this->pdf->SetY(75);
      ...
      foreach ( $client['rows'] as $fonds => $values ) {
        ...
        if ( ! is_numeric ($fonds) ) {
          ...
          /** loop records **/
          $this->pdf->SetWidths(array(55, 30, 20, 30, 20, 30));
          ...
          foreach ($values as $type => $overviewData) {
            if ( is_numeric ($type) ){ //numeric are rows
              ...
              $requeredItems = array (
                'transactietype' => $overviewData['transactietype'],
                'aantal' => $overviewData['aantal'],
                EURO,
                'koers' => $overviewData['koers'],
                EURO,
                'waarde' => $overviewData['waarde']
              );
              ...
              if ( in_array ($type, array('head', 'foot')) ) {
                ...
                $this->pdf->Row((array) array(strip_tags ($overviewData['datum']), $overviewData['aantal'], EURO, $overviewData['koers'], EURO, $overviewData['waarde']) );
...
//                $this->pdf->Row((array) array('',  '', strip_tags ($overviewData['transactietype']), $overviewData['aantal']) );
    ...
    if ( $marginLeftOnPage < $totalingHeight ) {
      $this->pdf->AddPage();
      $this->pdf->SetWidths(array(55, 30, 30,30,30,40));
      $this->pdf->SetAligns(array('L','R','R','R','R','R'));
    }
      $this->pdf->SetWidths(array(55, 30, 30,30,30,90));
      $this->pdf->Row((array) array('', '', '') );
      $this->pdf->Row((array) array('', '', '') );
      $this->pdf->MultiCell(230,10,'Waarde in EUR',0,"C");
//          $this->pdf->Row((array) array('', 'Waarde in EUR') );
      ...
      foreach ( $footer as $fonds => $footerItems ) {
        ...
        $this->pdf->Row((array) array($fonds, EURO, $startValue['waarde'], EURO, $endValue['waarde'] ));
      }
