...
foreach ( $participants as $client ) {
  ...
  foreach ( $partitipateFonds as $fonds ) {
    ...
    if ( $transactions ) {
      $rows[$client['crm_id']]['fonds'][$fonds] = $fondsen->parseBySearch(
        array('fonds' => $fonds)
      );
...
//                $requeredItems['waarde'] = $this->AENumbers->viewFormat2Decimals($overviewData['waarde']);
//                $requeredItems['aantal'] = $this->AENumbers->viewFormat2Decimals($overviewData['aantal']);
//                $requeredItems['koers'] = $this->AENumbers->viewFormatMinMaxDecimals($overviewData['koers']);
                //end format fields
