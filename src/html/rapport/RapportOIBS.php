...
$Log: RapportOIBS.php,v $
Revision 1.29  2011/09/14 09:26:56  rvv
*** empty log message ***
...
class RapportOIBS
{
	function RapportOIBS($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
*/
	...
	function writeRapport()
	{
		...
		while($categorien = $DB->NextRecord())
		{
			...
			$DB2 = new DB();
			$DB2->SQL($subquery);// echo "$subquery <br>\n";exit;
			$DB2->Query();
//listarray($categorien);
			...
			while($subdata = $DB2->NextRecord())
			{
				...
				$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
//echo $this->formatGetal(($subdata['actuelePortefeuilleWaardeEuro']/$categorien['subtotaalactueel'])*100,$afronding)." ".$subdata[fondsOmschrijving]."<br>\n";
