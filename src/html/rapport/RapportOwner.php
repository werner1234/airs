...
/*
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2012/04/25 15:19:32 $
 		File Versie					: $Revision: 1.2 $

 		$Log: RapportOwner.php,v $
...
class RapportOwner
{
	...
	*/
	...
	function RapportOwner(  $selectData )
	{
  ...
  $portefeuilles = $selectie->getSelectie();
	...
	if($records <= 0)
	{
	  ...
	  $db = new DB();
		$query="SELECT Owner, (percentage/100) as percentage,Portefeuille FROM OwnersPerPortefeuille WHERE Portefeuille IN('".implode("','",$portefeuilleNummers)."') ORDER BY Owner";
    $db->SQL($query);
    $db->Query();
    while($data=$db->nextRecord())
    {
		  $owners[$data['Owner']]=$data['Owner'];
		  $percentage[$data['Portefeuille']][$data['Owner']]=$data['percentage'];
    }
...
//Hierin komt per owner een regel met portefeuille.portefeuille, crm_naam, portefeuille.accountmanager, portefeuille.depotbank, %-tage ownership, totaal vermogen portefeuille, te betalen beheerfee (voor BTW!), "geownde vermogen", "geownde inkomsten".
    ...
    {
      array_push($header,"$owner %-tage ownership");
      array_push($header,"$owner geownde vermogen");
      array_push($header,"$owner geownde inkomsten");
    }
...
*/
    	...
    	foreach ($owners as $owner)
      {
        array_push($tmp,$percentage[$pdata['Portefeuille']][$owner]);
        array_push($tmp,$percentage[$pdata['Portefeuille']][$owner] * $waarden['basisRekenvermogen']);
        array_push($tmp,$percentage[$pdata['Portefeuille']][$owner] * 0);
      }
