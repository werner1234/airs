...
class RapportMODEL
{
	...
	*/
	...
	function RapportMODEL($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	  $this->pdf = &$pdf;
		$this->selectData =
	...
	function writeRapport()
	{
		...
		$DB = new DB();
		$this->pdf->excelData[] = array("Fonds",'Beleggingscategorie','ISINcode','Valuta',"Model percentage","Werkelijk Percentage","Afwijkings Percentage","Afwijking in EUR","Kopen","Verkopen","Waarde volgens percentage Model","Koers in locale valuta","Huidige Waarde","Geschat orderbedrag"); //rvv huidige waarde toegevoegd.
      ...
      TijdelijkeRapportage.".$this->selectData['modelcontrole_level']." as RegelOmschrijving,
          ...
          if($aankoopStuks > 0)
            ...
            $aankoopStuks=ceil($aankoopStuks/100)*100;
				...
				$this->pdf->excelData[] = array($fdata['fondsOmschrijving'],$fdata['beleggingscategorieOmschrijving'],$fdata['ISINCode'],$fdata['valuta'],
        ...
        array_push($totaalRekeningen,array("Liquiditeiten","Model waarde","Huidige waarde"));
			...
			while($fdata = $DB2->nextRecord())
			{
				...
				if ($fdata['portefeuilleWaarde'] != 0)
				{
				   ...
				   $this->pdf->excelData[] = array("Effectenrekening ".$fdata['fondsOmschrijving'],$fdata['beleggingscategorieOmschrijving'],'',$fdata['valuta'],
