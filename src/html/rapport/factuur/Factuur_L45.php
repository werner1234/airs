...
//	$this->pdf->row(array("  Gemiddeld belegd vermogen","€",$this->formatGetal($this->waarden['gemiddeldeVermogen'],2)));
  ...
  //rvv
	...
	{
  	$this->pdf->row(array("  Managementfee",'','',"€",$this->formatGetal($this->waarden['beheerfeePerPeriodeNor'],2)."  "));
    $this->waarden['beheerfeePerPeriode']=$this->waarden['beheerfeePerPeriodeNor'];
	}
  $this->pdf->ln(); 
  if($this->waarden['transactiefee'] <> 0)
  {
     $this->pdf->row(array("  Mutatiefee",'','',"€",$this->formatGetal($this->waarden['transactiefee'],2)."  "));
  }
  else
  {
    $this->pdf->ln();
  ...
  $this->pdf->CellBorders = array('','','','','T');
		$this->pdf->row(array("  Managementfee exclusief BTW",'','',"€",$this->formatGetal($this->waarden['beheerfeeBetalen'],2)."  "));
	  $this->pdf->ln();
  $this->pdf->SetFont($font,"",12);
  unset($this->pdf->CellBorders);
    ...
    $this->pdf->SetFont($font,"B",12);
    $this->pdf->CellBorders = array('','','','','T');
    $this->pdf->row(array("  Managementfee inclusief BTW",'','',"€",$this->formatGetal($this->waarden['beheerfeeBetalenIncl'],2)."  "));
	  $this->pdf->ln();
    $this->pdf->SetFont($font,"",12);
    unset($this->pdf->CellBorders);
