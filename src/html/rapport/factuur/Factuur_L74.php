...
//$this->pdf->row(array("", $vermData['Woonplaats'].", ". date("j")." ".$this->__appvar["Maanden"][date("n")]." ".date("Y")));
$this->pdf->ln();
if($this->waarden['BeheerfeeAantalFacturen']==4)
{
  $factuurPrefix=date("Y", $totjul).'Q'.$this->waarden['kwartaal'];
  $periode='per kwartaal';
}
elseif($this->waarden['BeheerfeeAantalFacturen']==12)
{
  $factuurPrefix=date("YM", $totjul);
  $periode='per maand';
}
$this->pdf->row(array('', "Factuurnummer: " . $factuurPrefix . sprintf("%04d", $this->waarden['factuurNummer'])));
$this->pdf->ln(6);
...
if(in_array($this->waarden['BeheerfeeBasisberekening'],array(0,4,5,6,7)))
{
  $text='Uw gemiddeld vermogen bedraagt ';
}
elseif($this->waarden['BeheerfeeBasisberekening']==2)
{
  $text='Uw vermogen bedraagd per '.$totDatum.' ';
}
$this->pdf->row(array('',$text.' € '.$this->formatGetal($this->waarden['basisRekenvermogen'],2)));
if($this->waarden['portefeuilledata']['valutaUitsluiten']==1)
{
  $this->pdf->SetFont($font, "", $fontsize-1);
  $this->pdf->row(array('', 'Exclusief liquiditeiten'));
  $this->pdf->SetFont($font, "", $fontsize);
}

if($this->waarden['BeheerfeeBedragBuitenFeePortefeuille']<>0)
{
  $this->pdf->row(array('','Bedrag buiten fee € '.$this->formatGetal($this->waarden['BeheerfeeBedragBuitenFeePortefeuille'],2)));
}



//$this->pdf->row(array('','Vermogensbeheer ' . $kwartaal . ' kwartaal ' . $jaar,'€', $this->formatGetal($this->waarden['beheerfeePerPeriode'] , 2)));

$this->pdf->ln(8);
if($this->waarden['BeheerfeeMethode']==1)
{
  $this->pdf->row(array('', 'Fee '. $periode. ' conform staffel', '€',
                    $this->formatGetal($this->waarden['beheerfeePerPeriode'] - $this->waarden['administratieBedrag'], 2)));
}
else
{
  $this->pdf->row(array('', ($this->waarden['BeheerfeePercentageVermogenDeelVanJaar'] <> 0?$this->formatGetal($this->waarden['BeheerfeePercentageVermogenDeelVanJaar'], 3) . '%':'') . ' ' . $periode, '€',
                    $this->formatGetal($this->waarden['beheerfeePerPeriode'] - $this->waarden['administratieBedrag'], 2)));
}
$this->pdf->ln();
