...
/*
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2019/03/09 18:47:30 $
 		File Versie					: $Revision: 1.4 $

 		$Log: ouderdomsAnalyse.php,v $
 		Revision 1.4  2019/03/09 18:47:30  rvv
 		*** empty log message ***
...
class ouderdomsAnalyse
{
	...
	*/
	...
	function ouderdomsAnalyse( $selectData )
	{
      ...
      FROM Fondsen WHERE Fonds='".mysql_real_escape_string($fonds)."' ".$this->FondsEinddatumFilter." ".$this->koersControleCheck;
			...
			if(isset($fondsInfo['Fonds']))
			{
				...
				if($this->selectData['ouderdomDagen'] <> '')
				{
					...
					if(($now -db2jul($koersen[0]['add_date'])) >  ($this->selectData['ouderdomDagen'] * 86400))
					{
					  echo "Laatste koers van $fonds is ".round(($this->selectData['datumTm'] -db2jul($koersen[0]['datum']))/86400)." dagen oud. Fonds overgeslagen.<br>\n";
            continue;
					}
					else
					{
...
*/
				...
				{
					if(abs($procentVerschil)<$this->selectData['ouderdomPercentage'])
						continue;
				}
...
ORDER BY Rekeningmutaties.Fonds";
    ...
    {
      $this->wherePortefeuilles = " AND Portefeuilles.consolidatie=0 ";
    }
