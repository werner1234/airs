...
class RapportVHO_L39
{
	function RapportVHO_L39($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
IF(TijdelijkeRapportage.type = 'fondsen',TijdelijkeRapportage.historischeWaarde,0) as historischeWaarde,
IF(TijdelijkeRapportage.type = 'fondsen',(TijdelijkeRapportage.totaalAantal * TijdelijkeRapportage.historischeWaarde * TijdelijkeRapportage.fondsEenheid * TijdelijkeRapportage.historischeValutakoers),0) AS historischeWaardeEuro,
IF(TijdelijkeRapportage.type = 'fondsen',(TijdelijkeRapportage.totaalAantal * TijdelijkeRapportage.historischeWaarde * TijdelijkeRapportage.fondsEenheid ),0 ) AS historischeWaardeValuta,
     ...
     if($lossingsJul > 0 && $fonds['type']=='fondsen')
      ...
      // echo $fonds['fonds']."$ytm = p $p,r $r,b $b,y $y <br>\n";
        ...
        $ytm='';
			...
			if($lastCategorieKey <> $fonds['beleggingscategorie'] && !empty($lastCategorieKey) )
			{
        ...
        $totaalactueelRente =0;
			...
			if($lastSectorKey <> $fonds['beleggingssector'] && $fonds['beleggingssectorOmschrijving'] <> '')
			{
				...
				$fondsResultaat = ($fonds['actuelePortefeuilleWaardeInValuta'] - $fonds['historischeWaardeValuta']) * $fonds['actueleValuta'] / $this->pdf->ValutaKoersEind;
				$fondsResultaatprocent = ($fondsResultaat / $fonds['historischeWaardeEuro']) * 100;
				$valutaResultaat = $fonds['actuelePortefeuilleWaardeEuro'] - $fonds['historischeWaardeEuro'] - $fondsResultaat;
...
if($fonds['type']=='rekening' || $fonds['type']=='rente')
{
  ...
  $fonds['beginwaardeLopendeJaar']=0;
      ...
      if($fonds['rekening']<>'')
      {
        $fonds['historischeWaardeEuro']=$this->getRekeningBeginWaarde($fonds['rekening']);
      }
      
if($fonds['type']=='rente')
{
  $fonds['fondsOmschrijving']='Opgelopen rente';
  $percentageVanTotaaltxt='';
  $beginWaardeEur=  $this->formatGetal($fonds['historischeWaardeEuro'],$this->pdf->rapport_VOLK_decimaal);
}
else
  $beginWaardeEur=  $this->formatGetal($fonds['historischeWaardeEuro'],$this->pdf->rapport_VOLK_decimaal,true);
        ...
        $hcatTotaal['percentageVanTotaal'] +=$percentageVanTotaal;
				...
				$hcatTotaal['totaalbegin'] +=$fonds['historischeWaardeEuro'];
