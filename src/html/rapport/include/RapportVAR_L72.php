...
class RapportVAR_L72
{
	function RapportVAR_L72($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	  ...
	  $this->db=new DB();
    $this->pdf->excelData[]=array('Beleggingscategorie','Naam','Rating instr.','Valuta','Nominaal','Koers','Marktwaarde','Coupon Yield','Yield to Maturity','Modified duration','Resterende looptijd','% port.');
...
ORDER BY TijdelijkeRapportage.beleggingscategorieVolgorde,TijdelijkeRapportage.fondspaar,TijdelijkeRapportage.fondsOmschrijving,TijdelijkeRapportage.rekening";
		$DB->SQL($query);
		$DB->Query();
...
//	        $this->cashfow
    ...
    //  '',$this->formatGetal($data['actuelePortefeuilleWaardeEuro'],0),$this->formatGetal($data['beginPortefeuilleWaardeEuro'],0),$this->formatGetal($ongerealiseerdResultaat,0),$this->formatGetal($aandeel,1).'%',$this->formatGetal($data['rente'],2)
      ...
      $this->pdf->excelData[]=array($data['categorieOmschrijving'],$data['fondsOmschrijving'],$data['fondsRating'],$data['valuta'],$data['totaalAantal'],
        round($data['actueleFonds'],2),round($data['actuelePortefeuilleWaardeEuro'],0),round($koers['Rentepercentage'],3),round($ytm,2),round($modifiedDuration,2),
