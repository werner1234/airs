...
class RapportAFM_L70
{
	function RapportAFM_L70($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
*/
  ...
  //  listarray($waarden);
    ...
    $this->waarden['Historie']=$att->bereken($rapportageStartJaar,  $this->rapportageDatum,'EUR','hoofdcategorie');//Hoofdcategorie

    
   // listarray($att->categorien);exit;
...
//listarray($this->jaarTotalen);
    ...
    $this->pdf->ln();
    $att->normData['totaal']=100;
   foreach ($this->jaarTotalen as $categorie=>$jaarWaarden)
    {

      $waarden=$jaarWaarden[$startJaar];
      //listarray($waarden);      
      $this->pdf->row(array(vertaalTekst($att->categorien[$categorie], $this->pdf->rapport_taal),
     ...
     // listarray($this->maandTotalen);
      ...
      $colors=array('allocate effect'=>$this->pdf->rapport_grafiek_afm,'selectie effect'=>$this->pdf->rapport_grafiek_icolor);//,'Totaal'=>array(0, 52, 121)); //'attributie effect'=>,array(87,165,25)
      $xval=25;$yval=185;
      foreach($colors as $effect=>$color)
      {
         $this->pdf->Rect($xval, $yval, 3, 3, 'DF','L',$color);
    ...
    //  $colors=array('allocate effect'=>$this->pdf->rapport_grafiek_pcolor,'selectie effect'=>array(87,165,25),'attributie effect'=>$this->pdf->rapport_grafiek_afm); //
     $colors=array('allocate effect'=>$this->pdf->rapport_grafiek_afm,'selectie effect'=>$this->pdf->rapport_grafiek_icolor,'attributie effect'=>$this->pdf->rapport_grafiek_pcolor); //
...
function LineDiagram($x,$y,$w, $h, $data, $title,$color=null, $maxVal=0, $minVal=0, $horDiv=4, $verDiv=4,$jaar=0)
  {
   ...
   // $color=array(200,0,0);
       ...
       $colors=array('allocateEffect'=>$this->pdf->rapport_grafiek_afm,'selectieEffect'=>$this->pdf->rapport_grafiek_icolor,'attributieEffect'=>$this->pdf->rapport_grafiek_pcolor); //
  ...
  function VBarDiagram2($w, $h, $data, $format, $color=null,$nbDiv=4,$numBars=0)
  {
      ...
      if($numBars > 0)
        $this->pdf->NbVal=$numBars;
         ...
         $colors=array('allocateEffect'=>$this->pdf->rapport_grafiek_afm,'selectieEffect'=>$this->pdf->rapport_grafiek_icolor);//,'totaalEffect'=>array(0, 52, 121)); //
    ...
    $vBar = ($bGrafiek / ($this->pdf->NbVal ))/3; //4
      ...
      foreach($data as $maand=>$maandData)
      {
        ...
        foreach($maandData as $type=>$val)
        {
          ...
          $hval = ($val * $unit);
          $this->pdf->Rect($xval, $yval, $lval, $hval, 'DF','L',$color);
...
$this->pdf->Rect($XstartGrafiek, $YstartGrafiek-$hGrafiek, $w- $margin, $hGrafiek,'FD','',array(245,245,245));
      ...
      if($numBars > 0)
        ...
        $colors=array(array(87,165,25),array(255,0,59),$this->pdf->rapport_grafiek_pcolor);
