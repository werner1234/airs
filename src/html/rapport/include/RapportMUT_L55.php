...
Revision 1.2  2016/12/17 18:57:35  rvv
*** empty log message ***

Revision 1.1  2016/03/09 17:24:31  rvv
*** empty log message ***
...
class RapportMUT_L55
{
	function RapportMUT_L55($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	...
	function writeRapport()
	{
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
		...
		$DB = new DB();
		// voor data
		$this->pdf->widthA = array(25,25,90,25,30,15,25,25,25);
		$this->pdf->alignA = array('L','R','L','R','R','R','R','R');

		// voor kopjes
		$this->pdf->widthB = array(25,25,90,25,30,15,25,25,25);
		$this->pdf->alignB = array('L','R','L','R','R','R','R','R');
      ...
      else
      {
        ...
        if($omschrijving==$mutaties['Omschrijving'])
        {
          ...
          $omschrijving=implode(" ",$omschrijvingDelen);
			...
			$maand=vertaalTekst($this->pdf->__appvar["Maanden"][date("n",$boekJul)],$this->pdf->rapport_taal);
      $regel=fillLine($this->pdf,$regel);
			$this->pdf->row(array($maand.' '.date("Y",$boekJul),
