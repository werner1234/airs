...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: ATTberekening_L22.php,v $
 		Revision 1.7  2017/05/26 16:45:07  rvv
 		*** empty log message ***
 		
 		Revision 1.6  2015/12/19 09:03:50  rvv
 		*** empty log message ***
...
class ATTberekening_L22
{

	function ATTberekening_L22($rapportData,$periode='maanden')
	{
	...
	function bereken($van,$tot,$verdeling='Hoofdcategorie',$gebruikHPI=false)
	{
	  global $__appvar;
    ...
    if(!in_array(substr($van,5,5),$beginMaanden) && substr($van,8,2) <> '01' && ($van <> $this->rapport->pdf->PortefeuilleStartdatum))
      $gebruikHPI=false;
    //if(!in_array(substr($tot,5,5),$eindMaanden))
    //  $gebruikHPI=false;
    //$eindMaanden
...
if($stapeling == true)
{
 ...
 $query = "SELECT  Grootboekrekeningen.Opbrengst,Grootboekrekeningen.Kosten, Grootboekrekeningen.Grootboekrekening,".
    ...
    if($categorie <> 'totaal' && $rekeningRekeningenWhere <>'')
    {
