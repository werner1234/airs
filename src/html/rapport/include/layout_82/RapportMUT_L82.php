...
class RapportMUT_L82
{
	function RapportMUT_L82($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//		$this->pdf->Line($totaal2+2,$this->pdf->GetY(),$totaal2 + $this->pdf->widthA[7],$this->pdf->GetY());
	...
	function writeRapport()
	{
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
		...
		$DB = new DB();
		// voor data
		$this->pdf->widthA = array(20,25,25,90,30,20,25,25,25);
		$this->pdf->alignA = array('R','R','L','L','R','R','R','R');

		// voor kopjes
		$this->pdf->widthB = array(20,25,25,90,30,20,25,25,25);
		$this->pdf->alignB = array('R','R','L','L','R','R','R','R');
    ...
    $filter=false;
	   ...
	   $extraquery .= "AND Rekeningmutaties.Grootboekrekening IN('$grootboekSelectie')  ";
		...
		$query = "SELECT ".
			...
			"ORDER BY Grootboekrekeningen.Afdrukvolgorde, Rekeningmutaties.Boekdatum, ABS(Rekeningmutaties.bedrag), Rekeningmutaties.id ";
    ...
    $maanden = array("", "Jan", "Feb", "Mrt", "Apr", "Mei", "Jun", "Jul", "Aug", "Sep", "Okt", "Nov", "Dec");
        ...
        $this->pdf->ln();
				...
				if($totaal < 0)
        {
          $this->printTotaal(vertaalTekst("Totaal", $this->pdf->rapport_taal) . " " . vertaalTekst($lastCategorie, $this->pdf->rapport_taal), abs($totaal), 0);
        }
				else
        {
          $this->printTotaal(vertaalTekst("Totaal", $this->pdf->rapport_taal) . " " . vertaalTekst($lastCategorie, $this->pdf->rapport_taal), 0, abs($totaal));
        }
				$subdebet = 0;
				$subcredit = 0;
			}
      ...
      $maand=vertaalTekst($maanden[date("n",db2jul($mutaties['Boekdatum']))],$this->pdf->rapport_taal);
