...
$Log: RapportPERF_L82.php,v $
Revision 1.4  2020/04/30 05:33:19  rvv
*** empty log message ***

Revision 1.3  2020/04/29 09:44:54  rvv
*** empty log message ***

Revision 1.2  2020/04/18 17:07:28  rvv
*** empty log message ***
...
class RapportPERF_L82
{

	function RapportPERF_L82($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function getOPKO($portefeuille, $van,$tot ,$valuta = 'EUR')
  {
    if ($valuta != "EUR")
    ...
    $query = "SELECT ".
      ...
      "Rekeningmutaties.Grootboekrekening = 'OPKO'
	 ";
    $DB = new DB();
    $DB->SQL($query);
    $DB->Query();
    ...
    $opko               = $this->getOPKO($this->portefeuille,$this->rapportageDatumVanaf,$this->rapportageDatum,$this->pdf->rapportageValuta);
		$onttrekkingen 		 	= getOnttrekkingen($this->portefeuille,$this->rapportageDatumVanaf,$this->rapportageDatum,$this->pdf->rapportageValuta)-$opko;
		
		
		$resultaatVerslagperiode = $waardeMutatie - $stortingen + $onttrekkingen + $opko;
...
//rvv end
		...
		$this->pdf->ln();
			...
			if(round($opko,2) <> 0.0)
        $this->pdf->row(array("",vertaalTekst("Openingskosten",$this->pdf->rapport_taal),$this->formatGetal($opko,2),""));
    ...
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
     ...
     // $this->pdf->Line($posSubtotaal+$extraLengte  ,$this->pdf->GetY() ,$posSubtotaalEnd ,$this->pdf->GetY());
      ...
      {
        $this->pdf->ln(1);
        $this->pdf->row(array("", vertaalTekst("Rendement lopend jaar", $this->pdf->rapport_taal), $this->formatGetal($rendementProcentJaar, 2), "%", ""));
     //   $this->pdf->Line($posSubtotaal + $extraLengte, $this->pdf->GetY(), $posSubtotaalEnd, $this->pdf->GetY());
      }
			//$this->pdf->Line($posSubtotaal+$extraLengte  ,$this->pdf->GetY()+1 ,$posSubtotaalEnd ,$this->pdf->GetY()+1);
...
/*
		$vkm=new RapportHulpVKM_L82($this->portefeuille, $this->rapportageDatumVanaf, $this->rapportageDatum);
		$this->pdf->setY(50);
		$this->pdf->setAligns(array('L','L','R','R','R'));
		$this->pdf->SetWidths(array(170,50+30+20));
		$this->pdf->row(array("","Berekening vergelijkende kostenmaatstaf"));
		$this->pdf->SetWidths(array(170,50,30,20));

		$this->pdf->row(array("","","EUR",'Percentage'));
		foreach($vkm->perfData as $omschrijving=>$waarden)
		{

			//	  $this->pdf->row(array("","","","Resultaat over verslagperiode",$this->formatGetal($totaalOpbrengst - $totaalKosten,2)));
			$this->pdf->row(array("",$omschrijving,$this->formatGetal($waarden['eur'],0),$this->formatGetal($waarden['percentage'],1)."%"));
		}
	//	listarray($vkm->perfData);
*/
	}
}
?>