...
$Log: RapportVOLK_L82.php,v $
Revision 1.3  2020/04/29 09:44:54  rvv
*** empty log message ***
...
class RapportVOLK_L82
{
	function RapportVOLK_L82($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
TijdelijkeRapportage.Lossingsdatum, TijdelijkeRapportage.fondsOmschrijving asc";
    ...
    $lastCategorieOmschrijving=$data['categorieOmschrijving'];
		...
		$query = "SELECT TijdelijkeRapportage.fondsOmschrijving, ".
			...
			" TijdelijkeRapportage.actuelePortefeuilleWaardeEuro / ".$this->pdf->ValutaKoersEind. " as actuelePortefeuilleWaardeEuro, ".
			"
			if(TijdelijkeRapportage.type = 'rekening',
			 ((SELECT actuelePortefeuilleWaardeEuro FROM TijdelijkeRapportage WHERE portefeuille = '".$this->portefeuille."' AND rapportageDatum = '".$this->rapportageDatumVanaf."' AND TijdelijkeRapportage.valuta = zoekValuta AND type='rekening'  LIMIT 1)  / ".$this->pdf->ValutaKoersBegin."),
			 (TijdelijkeRapportage.beginPortefeuilleWaardeEuro / ".$this->pdf->ValutaKoersBegin. ")
			 ) as beginPortefeuilleWaardeEuro,".
			" TijdelijkeRapportage.valuta, TijdelijkeRapportage.portefeuille ".
			" FROM TijdelijkeRapportage
			
			WHERE ".
			" TijdelijkeRapportage.portefeuille = '".$this->portefeuille."'  AND ".
			" (TijdelijkeRapportage.type = 'rekening' OR TijdelijkeRapportage.beleggingscategorie = 'Liquiditeiten') ".
		      ...
		      else
		      {
		        $this->pdf->fillCell=array();
		         $fill=true;
					...
					$percentageVanTotaal = ($data['actuelePortefeuilleWaardeEuro']) / ($totaalWaarde/100);
					$this->pdf->row(array("  ".$data['fondsOmschrijving'],'',$data['Valutateken'],'',
                        $this->formatGetal($data['beginPortefeuilleWaardeEuro'],$this->pdf->rapport_VOLK_decimaal),
												'','',
											  $this->formatGetal($data['actuelePortefeuilleWaardeEuro'],$this->pdf->rapport_VOLK_decimaal),
												$this->formatGetal($percentageVanTotaal,$this->pdf->rapport_VOLK_decimaal_proc)
...
FROM Indices, Fondsen WHERE Indices.Beursindex = Fondsen.Fonds AND Vermogensbeheerder = '".$this->pdf->portefeuilledata['Vermogensbeheerder']."' ORDER BY Afdrukvolgorde";
    $border=0;
		$DB  = new DB();
		$DB2 = new DB();
    ...
    $widthDesc=54;
		$this->pdf->ln();
		$this->pdf->SetFillColor(255,255,255);
		$this->pdf->Rect($this->pdf->marge,$this->pdf->getY(),3*$w+$extraX+$widthDesc,$hoogte,'F');
		$this->pdf->SetFillColor(0);
		$this->pdf->Rect($this->pdf->marge,$this->pdf->getY(),3*$w+$extraX+$widthDesc,$hoogte);
		$this->pdf->SetX($this->pdf->marge);
		...
		$this->pdf->SetTextColor($this->pdf->rapport_kop_fontcolor['r'],$this->pdf->rapport_kop_fontcolor['g'],$this->pdf->rapport_kop_fontcolor['b']);
		if($this->pdf->rapport_perfIndexJanuari == true)
			$this->pdf->Cell($w,4, date("d-m-Y",db2jul($januariDatum)), $border,0, "R");
		$this->pdf->Cell($w,4, date("d-m-Y",db2jul($rapportageDatumVanaf)), $border,0, "R");
		$this->pdf->Cell($w,4, date("d-m-Y",db2jul($rapportageDatum)), $border,0, "R");
		$y=$this->pdf->getY();
    $x=$this->pdf->getX();
		$this->pdf->MultiCell($w,4, vertaalTekst("Perf. Verslagperiode in %",$this->pdf->rapport_taal), $border, "R");//$perfVal

		if($this->pdf->rapport_printAEXVergelijkingEur == 1)
		  $this->pdf->Cell($w,4, vertaalTekst("Perf in % in EUR",$this->pdf->rapport_taal), $border,$perfEur, "R");
		if($this->pdf->rapport_perfIndexJanuari == true)
    {
      $this->pdf->setXY($x+$w,$y);
      $this->pdf->MultiCell($w, 4, vertaalTekst("Perf. Lopend jaar \nin %", $this->pdf->rapport_taal), $border, "R");//$perfJan
    }
//MultiCell($w,$h,$txt,$border=0,$align='J',$fill=0)
      ...
      //echo $perf[Omschrijving]." $performanceEur = (.".$koers2['Koers']."*".$valutaKoersStop['Koers']." - ".$koers1['Koers']."*".$valutaKoersStart['Koers'].") / (".$koers1['Koers']."*".$valutaKoersStart['Koers']."/100 );<br>";
		  ...
		  if($this->pdf->rapport_perfIndexJanuari == true)
		     $this->pdf->Cell($w,4, $this->pdf->formatGetal($koers0[Koers],2), $border,0, "R");
			$this->pdf->Cell($w,4, $this->pdf->formatGetal($koers1[Koers],2), $border,0, "R");
			$this->pdf->Cell($w,4, $this->pdf->formatGetal($koers2[Koers],2), $border,0, "R");
		  $this->pdf->Cell($w,4, $this->pdf->formatGetal($performance,2).$teken, $border,$perfVal, "R");
		  if($this->pdf->rapport_printAEXVergelijkingEur == 1)
		    $this->pdf->Cell($w,4, $this->pdf->formatGetal($performanceEur,2).$teken, $border,$perfEur, "R");
		  if($this->pdf->rapport_perfIndexJanuari == true)
		    $this->pdf->Cell($w,4, $this->pdf->formatGetal($performanceJaar,2).$teken, $border,$perfJan, "R");
		}
