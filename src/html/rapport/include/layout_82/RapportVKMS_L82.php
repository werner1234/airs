...
class RapportVKMS_L82
{
	function RapportVKMS_L82($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum,$laatstejaar=true)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
...
if(Rekeningmutaties.Grootboekrekening='OPKO',100,Grootboekrekeningen.Afdrukvolgorde)+1 as volg
FROM Rekeningmutaties
JOIN Rekeningen ON Rekeningmutaties.Rekening=Rekeningen.Rekening
JOIN Grootboekrekeningen ON Rekeningmutaties.Grootboekrekening=Grootboekrekeningen.Grootboekrekening
WHERE Rekeningen.Portefeuille='".$this->portefeuille."' AND Rekeningmutaties.Boekdatum>'".$this->vanafDatum."'
AND Rekeningmutaties.Boekdatum<='".$this->rapportageDatum."' AND (Grootboekrekeningen.Kosten=1 OR Rekeningmutaties.Grootboekrekening='OPKO')
GROUP BY Rekeningmutaties.Grootboekrekening
ORDER BY volg";
		$DB=new DB();
		$DB->SQL($query);
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    if($this->pdfVullen==true)
      ...
      $this->pdf->SetTextColor($this->pdf->rapport_default_fontcolor['r'], $this->pdf->rapport_default_fontcolor['g'], $this->pdf->rapport_default_fontcolor['b']);
	  	...
	  	$this->pdf->row(array(vertaalTekst('Indirecte (fonds)kosten',$this->pdf->rapport_taal), $this->formatGetal($totaalDoorlopendekosten, 0) . ' EUR'));
      ...
      else
      {
        $kostenTxt=vertaalTekst('Directe kosten vanaf',$this->pdf->rapport_taal).' ' . date('d-m-Y', db2jul($this->vanafDatum));
			...
			$this->pdf->row(array($kostenTxt, 'EUR', vertaalTekst('Percentage',$this->pdf->rapport_taal)));
