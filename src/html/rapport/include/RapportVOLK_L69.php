...
class RapportVOLK_L69
{
	function RapportVOLK_L69($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//echo $subquery."<br><br>";exit;
      ...
      $grandtotaaldividendCorrected  += $subtotaal['totaalDividendCorrected'];
		...
		$query = "SELECT TijdelijkeRapportage.fondsOmschrijving, ".
			...
			" FROM Rekeningen 
LEFT JOIN TijdelijkeRapportage on Rekeningen.rekening = TijdelijkeRapportage.rekening AND Rekeningen.Portefeuille = TijdelijkeRapportage.portefeuille AND".
			" TijdelijkeRapportage.portefeuille = '".$this->portefeuille."' AND ".
			" TijdelijkeRapportage.type = 'rekening'  ".
			" AND TijdelijkeRapportage.rapportageDatum = '".$this->rapportageDatum."' "
			.$__appvar['TijdelijkeRapportageMaakUniek']."
			WHERE Rekeningen.portefeuille = '".$this->portefeuille."' AND Rekeningen.Inactief=0 AND Rekeningen.Memoriaal=0 
			ORDER BY TijdelijkeRapportage.fondsOmschrijving, TijdelijkeRapportage.valuta asc";
		debugSpecial($query,__FILE__,__LINE__);

		$DB1 = new DB();
		$DB1->SQL($query); //echo $query;exit;
		$DB1->Query();
