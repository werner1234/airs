...
class RapportTRANSFEE_L25
{
	function RapportTRANSFEE_L25($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  //  listarray($waarden);
    ...
    $att=new ATTberekening_L25($this);
      ...
      */
      ...
      { //listarray($waarden);
        ...
        if($categorie!='totaal')
        {
          ...
          // indexBijdrageWaarde naar 0 geforceerd voor vergelijkbare waarden als in de ATT_L35
          $waarden['indexBijdrageWaarde']=0;
         // echo "$datum| ".$waarden['weging']."-".$waarden['indexBijdrageWaarde']." <br>\n";
   ...
   foreach ($this->jaarTotalen as $categorie=>$jaarWaarden)
    {
     ...
     // listarray($this->maandTotalen);
      ...
      $this->VBarDiagram2(130,77,$barData,'');
      $colors=array('allocate effect'=>$this->pdf->rapport_grafiek_icolor,'selectie effect'=>$this->pdf->rapport_grafiek_afm);//,'Totaal'=>array(0, 52, 121)); //'attributie effect'=>,array(87,165,25)
      $xval=25;$yval=175;
    ...
    //  $colors=array('allocate effect'=>array(0,52,121),'selectie effect'=>array(87,165,25),'attributie effect'=>array(108,31,128)); //
     $colors=array('allocate effect'=>$this->pdf->rapport_grafiek_icolor,'selectie effect'=>$this->pdf->rapport_grafiek_afm,'attributie effect'=>$this->pdf->rapport_grafiek_pcolor); //
    
    

    
    $this->LineDiagram(160,50+50,120,60,$this->maandTotalenCumulatief,'');
      $xval=165;$yval=175;
      foreach($colors as $effect=>$color)
      {
         $this->pdf->Rect($xval, $yval, 3, 3, 'DF',null,$color);
...
function LineDiagram($x,$y,$w, $h, $data, $title,$color=null, $maxVal=0, $minVal=0, $horDiv=4, $verDiv=4,$jaar=0)
  {
   ...
   // $color=array(200,0,0);
    ...
    $colors=array('allocateEffect'=>$this->pdf->rapport_grafiek_icolor,'selectieEffect'=>$this->pdf->rapport_grafiek_afm,'attributieEffect'=>$this->pdf->rapport_grafiek_pcolor); //
  ...
  function VBarDiagram2($w, $h, $data, $format, $color=null,$nbDiv=4,$numBars=0)
  {
      ...
      if($numBars > 0)
        $this->pdf->NbVal=$numBars;
         ...
         $colors=array('allocateEffect'=>$this->pdf->rapport_grafiek_icolor,'selectieEffect'=>$this->pdf->rapport_grafiek_afm);//,'totaalEffect'=>array(0, 52, 121)); //
...
$this->pdf->Rect($XstartGrafiek, $YstartGrafiek-$hGrafiek, $w- $margin, $hGrafiek,'FD','',array(245,245,245));
      ...
      if($numBars > 0)
        ...
        $colors=array($this->pdf->rapport_grafiek_icolor,$this->pdf->rapport_grafiek_afm,$this->pdf->rapport_grafiek_pcolor);
