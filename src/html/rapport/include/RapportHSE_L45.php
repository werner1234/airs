...
class RapportHSE_L45
{
	function RapportHSE_L45($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
   ...
   // 	$this->cashfow = new Cashflow($this->portefeuille,$this->pdf->rapport_datumvanaf,$this->pdf->rapport_datum,$this->pdf->debug);
	...
	function writeRapport()
	{
		...
		// voor kopjes
			$this->pdf->widthA = array(70,25,15,20,25,0,0,20,25,15,20,25,0,15);
 ...
 //$this->pdf->SetFillColor($this->pdf->rapport_kop_bgcolor['r']*1.2,$this->pdf->rapport_kop_bgcolor['g']*1.2,$this->pdf->rapport_kop_bgcolor['b']*1.2);
    ...
    $ytm='';        
        ...
        if($totaalactueelRente <> 0)
        {
          if($fill==true)
		      {
		        $this->pdf->fillCell = array(1,1,1,1,1,0,0,1,1,1,1,1,0,1,1);
            $fill=false;
		      }
		      else
		      {
