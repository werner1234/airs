...
$Log: RapportTRANS_L25.php,v $
Revision 1.6  2019/06/21 14:52:45  rm
7880

Revision 1.5  2018/11/03 18:45:31  rvv
*** empty log message ***

Revision 1.4  2016/01/24 09:52:26  rvv
*** empty log message ***

Revision 1.3  2016/01/03 09:16:56  rvv
*** empty log message ***

Revision 1.2  2015/12/19 08:29:17  rvv
*** empty log message ***

Revision 1.1  2012/04/08 08:13:35  rvv
*** empty log message ***
...
class RapportTRANSFEE_L13
{
	function RapportTRANSFEE_L13($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		$this->pdf->rapport_type = "TRANSFEE";
		$this->pdf->rapport_datum = db2jul($rapportageDatum);
		$this->pdf->rapport_datumvanaf = db2jul($rapportageDatumVanaf);
		$this->pdf->rapport_titel = "Overzicht effectentransacties";
	...
	function getAankopen($fonds,$datumTot)
  {
    if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
    ...
    $query = "SELECT Fondsen.Omschrijving, ".
     ...
     Rekeningmutaties.omschrijving as rekeningOmschrijving,
      ...
      "Rekeningmutaties.Transactietype = 'A' AND ".
      "Rekeningmutaties.Fonds = '".mysql_real_escape_string($fonds)."' AND ".
      "Grootboekrekeningen.FondsAanVerkoop = '1' AND ".
      "Rekeningmutaties.Boekdatum <= '".$datumTot."' ".
    ...
    while($mutaties = $DB->nextRecord())
    {
      ...
      //if(substr($this->rapportageDatum,0,4)==date('Y',$boekJul))
      //  $dag=date("d-m-y", $boekJul);
     // else
      $dag=date("d-m-Y", $boekJul);
      ...
      $rowData[]=array('',
                        ...
                        rclip($mutaties['Omschrijving'], 40),
                        $aankoop_koers,
                        $dag,//$aankoop_waardeinValuta,
                        $aankoop_waarde,
                      );
	...
	function writeRapport()
	{
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
		...
		$this->pdf->widthB = array(13,12,15,0,16,22,22,16,22,22,22,22,22,0);
    $this->pdf->widthB[3]=297-array_sum($this->pdf->widthB)-$this->pdf->marge*2;
...
//	echo "lopende -> ".$resultaatlopende." <-  voorgaande ".$resultaatvoorgaande. " -  <br>" ;
      ...
      //  unset($this->pdf->fillCell);
			if($mutaties['Transactietype']=='V')
      {

        $aanKoopRows=$this->getAankopen($mutaties['Fonds'],$mutaties['Boekdatum']);
        if(count($aanKoopRows)==1)
        {
          $aankoop_koers=$aanKoopRows[0][4];
          $aankoop_waardeinValuta=$aanKoopRows[0][5];
          $aankoop_waarde=$aanKoopRows[0][6];
        }
        else
        {
          $aantal=count($aanKoopRows);
          foreach ($aanKoopRows as $i=>$row)
          {
            if($i+1==$aantal)
            {
              $aankoop_koers=$aanKoopRows[$i][4];
              $aankoop_waardeinValuta=$aanKoopRows[$i][5];
              $aankoop_waarde=$aanKoopRows[$i][6];
            }
            else
              $this->pdf->row($row);
          }
