...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2018/09/27 06:53:41 $
File Versie					: $Revision: 1.3 $

$Log: RapportATT_L7.php,v $
Revision 1.3  2018/09/27 06:53:41  rvv
*** empty log message ***

Revision 1.2  2018/09/26 15:53:28  rvv
*** empty log message ***

Revision 1.1  2018/09/22 17:12:17  rvv
*** empty log message ***

Revision 1.5  2014/11/08 18:37:31  rvv
*** empty log message ***
...
include_once($__appvar["basedir"]."/html/indexHerberekening_L7.php");


class RapportATT_L7
{
	function RapportATT_L7($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
ORDER BY Beleggingscategorien.Afdrukvolgorde"; *///WHERE Beleggingscategorie IN('LIQ','ZAK','VAR','Liquiditeiten')
    ...
    $indexData = $index->getWaarden($this->rapportageDatumVanaf ,$this->rapportageDatum ,$this->portefeuille,'','maanden','EUR',true);
...
function formatGetalLength ($getal,$decimaal,$gewensteLengte)
{
 ...
 return number_format($getal,$decimaal,',','');
  ...
  $gemiddelde = $totaalWaarde['begin'] + $weging['totaal1'];
    ...
    //echo " $opgelopenRente = ".$totaalWaarde['renteEind']."-".$totaalWaarde['renteBegin']." $beginDatum - $eindDatum<br>\n";
...
*/
  ...
  function VBarDiagram($w, $h, $data)
  {
      ...
      foreach (($this->categorieVolgorde) as $categorie)//array_reverse
      {
        ...
        {
          $this->pdf->Rect($XstartGrafiek+$w+3 , $YstartGrafiek-$hGrafiek+$n*6+2, 2, 2, 'DF',null,$colors[$categorie]);
          $this->pdf->SetXY($XstartGrafiek+$w+6 ,$YstartGrafiek-$hGrafiek+$n*6+1.5 );
