...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2020/04/02 06:20:23 $
File Versie					: $Revision: 1.3 $

$Log: RapportATT_L65.php,v $
Revision 1.3  2020/04/02 06:20:23  rvv
*** empty log message ***

Revision 1.2  2020/04/01 16:54:10  rvv
*** empty log message ***
...
class RapportATT_L65
{
	function RapportATT_L65($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		$this->pdf->rapport_type = "ATT";
		$this->pdf->rapport_datum = db2jul($rapportageDatum);
		$this->pdf->rapport_datumvanaf = db2jul($rapportageDatumVanaf);
		$this->pdf->rapport_titel = "Rendement lopend kalenderjaar";
...
foreach ($indexData as $index=>$data)
{
  ...
  $barGraph['Index'][$data['datum']]['leeg']=0;
    ...
    {
      if($categorie=='LIQ' || $categorie=='Liquiditeiten')
        $categorie='G-LIQ';
...
Beleggingscategorien.Beleggingscategorie as Hoofdcategorie,
Beleggingscategorien.Omschrijving
FROM
KeuzePerVermogensbeheerder
INNER JOIN CategorienPerHoofdcategorie ON KeuzePerVermogensbeheerder.vermogensbeheerder = CategorienPerHoofdcategorie.Vermogensbeheerder AND KeuzePerVermogensbeheerder.waarde = CategorienPerHoofdcategorie.Beleggingscategorie
INNER JOIN Beleggingscategorien ON CategorienPerHoofdcategorie.Hoofdcategorie = Beleggingscategorien.Beleggingscategorie
WHERE
	KeuzePerVermogensbeheerder.vermogensbeheerder = '$beheerder'
...
//listarray($barGraph);
/*
		$q="SELECT hoofdcategorie as Beleggingscategorie,hoofdcategorieOmschrijving as Omschrijving,hoofdcategorieVolgorde 
    FROM TijdelijkeRapportage 
    WHERE Portefeuille='".$this->portefeuille."' AND hoofdcategorie <>'' 
    GROUP BY hoofdcategorie  ORDER BY hoofdcategorieVolgorde asc"; //WHERE Beleggingscategorie IN('LIQ','ZAK','VAR','Liquiditeiten')
*/
		$DB->SQL($q);
		$DB->Query();
   ...
   if(count($rendamentWaarden) > 0)
   {
     ...
     //   $this->pdf->CellBorders = array('','US','US','US','US','US','US','US','US','US','US','US');
       ...
       //$this->pdf->SetFillColor(230,230,230);
        ...
        $this->pdf->SetFillColor($this->pdf->rapport_kop_bgcolor['r']*1.5,$this->pdf->rapport_kop_bgcolor['g']*1.5,$this->pdf->rapport_kop_bgcolor['b']*1.5);
    ...
    $this->pdf->SetXY(8, 191);
    $this->pdf->MultiCell(170,4,vertaalTekst('*Koersresultaat (totaal) is het ongerealiseerde en gerealiseerde koersresultaat.',$this->pdf->rapport_taal),0,'L');
