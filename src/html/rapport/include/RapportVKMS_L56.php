...
class RapportVKMS_L56
{
	function RapportVKMS_L56($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    if($this->pdfVullen==true)
      ...
      $this->pdf->row(array(vertaalTekst('Indirecte kosten vanaf',$this->pdf->rapport_taal).' ' . date('d-m-Y', db2jul($this->vanafDatum)), 'EUR', vertaalTekst('Percentage',$this->pdf->rapport_taal)));
      $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
      $this->pdf->row(array(vertaalTekst('Indirecte (fonds)kosten',$this->pdf->rapport_taal), $this->formatGetal($totaalDoorlopendekosten, 0), $this->formatGetal($doorlopendeKostenPercentage * 100, 2) . ' %'));
      $this->pdf->ln();
      
      
      $this->pdf->excelData[]=array('','Indirecte (fonds)kosten', '','',round($totaalDoorlopendekosten, 0), 'EUR');
			$this->pdf->excelData[]=array('','Indirecte (fonds)kosten ten opzichte van onderliggend vermogen','','', round($doorlopendeKostenPercentage * 100, 2) ,'%');
			$this->pdf->excelData[]=array();
      ...
      $this->pdf->setWidths(array(108, 30, 30));
		...
		if($this->pdfVullen==true)
		{
			...
			$this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
			$this->pdf->row(array(vertaalTekst('Indirecte (fonds)kosten factor van de portefeuille',$this->pdf->rapport_taal),$this->formatGetal($vkmPercentagePortefeuille, 2) . ' %'));// $this->formatGetal($vkmPercentagePortefeuille*$gemiddelde/100, 0)
			$barData=array();
			$barData['Indirecte (fonds)kosten']=$vkmPercentagePortefeuille;
			$this->pdf->excelData[]=array('','Indirecte (fonds)kosten factor van de portefeuille','','',round($vkmPercentagePortefeuille, 2),'%');
			$this->pdf->ln();
			$this->pdf->excelData[]=array();
...
foreach ($perCategorie as $hoofdcategorie=>$categorieData)
{
 ...
 $perfTotaal['percentageIndirectVermogenMetKostenfactor']=$percentageIndirectVermogenMetKostenfactor;
      ...
      $this->grootboekKleuren=$grootboekKleuren;
			...
			if ($this->pdfVullen == true)
			{
				...
				$txt='De indirecte kosten van de financiële instrumenten zijn zo goed mogelijk ingeschat. Voor de betrokken derde partijen is de regelgeving nog in ontwikkeling. Er is dan ook sprake van voortschrijdend inzicht. Wij sluiten niet uit dat de indirecte kosten te hoog zijn ingeschat.';
 ...
 WHERE Portefeuilles.portefeuille='".$this->portefeuille."'";
    ...
    $waardeZonderKostenKleur=array(35,45,105);//array( 0,112,192 );
    $waardeNaKostenKleur=array(189,149,91);//array(12,173,71);
    $cumulatieveKostenKleur=array(150,150,150);//array(250,6,87);
    
    if($this->pdf->getY()+70>$this->pdf->pagebreak)
      $this->pdf->addPage();
    $ystart=$this->pdf->getY()+10;
    $this->pdf->setXY(30,$ystart);
    ...
    if(db2jul($this->pdf->portefeuilledata['Startdatum']) < db2jul('2021-01-01'))
      $start='2021-01-01';
    else
      $start=substr($this->pdf->portefeuilledata['Startdatum'],0,10);

    
    $index=new indexHerberekening();
    $jaren=$index->getjaren(db2jul($start),db2jul($this->rapportageDatum));
    ...
    foreach($jaren as $periode)
    {
      ...
      $beginvermogen = 0;
      $vkm = new RapportVKM(null, $this->portefeuille, $periode['start'], $periode['stop'], false);
      $vkm->writeRapport();
      $rendement = performanceMeting($this->portefeuille, $periode['start'], $periode['stop'], $this->pdf->portefeuilledata['PerformanceBerekening'], $this->pdf->rapportageValuta);
      $regels = berekenPortefeuilleWaarde($this->portefeuille, $periode['start'], (substr($periode['start'], 5, 5) == '01-01')?true:false);
      foreach ($regels as $regel)
      {
        $beginvermogen += $regel['actuelePortefeuilleWaardeEuro'];
      }
      if ($beginvermogen == 0)
      {
        $beginvermogen = getStortingen($this->portefeuille, $periode['start'], $periode['stop']);
      }
  
  
      $jaarDeel = (db2jul($periode['stop']) - db2jul($periode['start'])) / 365.25 / 86400;
      $nieuweWaarde = $beginvermogen * pow(1 + ($rendement / 100), $jaarDeel);
      $kosten += $nieuweWaarde * ($vkm->vkmWaarde['vkmWaarde'] / 100);
  
      $grafiekWaardenVorigjaar['waardeNaKosten'][] = $nieuweWaarde;
      $grafiekWaardenVorigjaar['cumulatieveKosten'][] = $kosten;
      $grafiekWaardenVorigjaar['waardeZonderKosten'][] = $nieuweWaarde + $kosten;
      $grafiekWaardenVorigjaar['datum'][] = substr($periode['stop'], 0, 4);
  

    }
      /*
          
          foreach($periode as $p=>$datum)
          {
            
           
            
        
            if(!isset($waardeOpDatum[$datum]))
            {
              $regels = berekenPortefeuilleWaarde($this->portefeuille, $datum, (substr($datum, 5, 5) == '01-01')?true:false);
              foreach ($regels as $regel)
              {
                $waardeOpDatum[$datum] += $regel['actuelePortefeuilleWaardeEuro'];
              }
            }
          }
      ...
      if($waardeOpDatum[$periode['start']]==0)
      {
        $waardeOpDatum[$periode['start']]=getStortingen($this->portefeuille,$periode['start'],$periode['stop']);
      }
    ...
    foreach($waardeOpDatum as $datum=>$waarde)
    {
      if(substr($datum,5,5)=='01-01'||substr($datum,5,5)=='12-31')
      {
        $jaren=(db2jul($datum)-db2jul($start))/365.25/86400;
        $nieuweWaarde=$beginvermogen*pow(1+($rendement/100),$jaren);
        $kosten+=$nieuweWaarde*($this->vkmWaarde['vkmWaarde']/100);
  
        $grafiekWaardenVorigjaar['waardeNaKosten'][]=$nieuweWaarde;
        $grafiekWaardenVorigjaar['cumulatieveKosten'][]=$kosten;
        $grafiekWaardenVorigjaar['waardeZonderKosten'][]=$nieuweWaarde+$kosten;
        $grafiekWaardenVorigjaar['datum'][]=round(substr($start,0,4)+$jaren);
        
      
        $grafiekWaardenVorigjaar['waardeNaKosten'][]=$waarde;
        $kosten+=$this->vkmWaarde['totaalDoorlopendekosten']+$this->vkmWaarde['totaalDirecteKosten'];
        $grafiekWaardenVorigjaar['cumulatieveKosten'][]=$kosten;
        $grafiekWaardenVorigjaar['waardeZonderKosten'][]=$waarde+$kosten;
        $grafiekWaardenVorigjaar['datum'][]=substr($datum,0,4);
        $grafiekWaardenVorigjaar['datumVolledig'][]=$datum;
   
      }
    }
             */
//listarray($grafiekWaardenVorigjaar);
    /*
    ...
    */
    ...
    $grafiekWaardenVorigjaar['titel']="Impact van kosten op het rendement vanaf ".date('d-m-Y',db2jul($start));
    
    $this->pdf->setXY(175,$ystart);
    $this->LineDiagram(110, 55, $grafiekWaardenVorigjaar,array($waardeZonderKostenKleur,$waardeNaKostenKleur,$cumulatieveKostenKleur),0,0,4,4,true);//50
...
//		echo "$doelJaar $rendement $beginwaarde";
	...
	//	$maxVal=round($maxVal,floor(log10($maxVal))*-1+1);
		...
		$unit = $lDiag / (count($data)-($vanafBegin?1:0));
      ...
      $xCorrectie=0;
		...
		for ($i=0; $i<count($data); $i++)
		{
			...
			if($i%$jaren==0)
				$this->pdf->TextWithRotation($XDiag+($i)*$unit-5+($vanafBegin?0:$unit),$YDiag+$hDiag+8,$legendDatum[$i],25);
		...
		foreach ($legendaItems as $index=>$item)
		{
			...
			$this->pdf->Rect($XPage+$step-20 , $YPage+$h+10, 3, 3, 'DF','',$kleur);
			$this->pdf->SetXY($XPage+$step+3-20,$YPage+$h+10);
