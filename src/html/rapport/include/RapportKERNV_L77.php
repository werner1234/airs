...
$Log: RapportKERNV_L77.php,v $
Revision 1.2  2020/06/06 15:48:23  rvv
*** empty log message ***
...
class RapportKERNV_L77
{
  function RapportKERNV_L77($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
  {
    $this->pdf = &$pdf;
  ...
  function writeRapport()
  {
    ...
    if($consolidatie)
    {
      ...
      foreach($this->pdf->portefeuilles as $portefeuille)
      {
        ...
        {
         // listarray($waarde);exit;
          if($waarde['beleggingscategorie']=='')
          {
            $waarde['beleggingscategorie']='GeenCategorie';
          ...
          $categorieVolgorde[$waarde['beleggingscategorie']]=$waarde['beleggingscategorieVolgorde'];
    ...
    else
    {
        ...
        while($categorien = $DB->NextRecord())
        {
          ...
          $categorieVolgorde[$categorien['beleggingscategorie']]=$categorien['beleggingscategorieVolgorde'];
...
//listarray($categorieVolgorde);exit;
    /*
      ...
      */
      ...
      if(count($this->pdf->portefeuilles) < 60)
      {
        ...
        foreach($this->clientVermogensbeheerder as $portefeuilleNaam)
        {
          ...
          //  echo "$portefeuilleNaam $width <br>\n";
          if ($width > $eurw+$portw+$pw)
          {
            $tweeRegels = true;
          }
        }
        ...
        {
          // if ($width==$pw || $width==$portw || $width==$eurw)
          // {
          if($width==$eurw)
            $extraX=2;
          else
          ...
          //    }
          $lastX += $width;
          if($i==count($this->clientVermogensbeheerder)*3+3)
            break;
        }
        */
        $this->pdf->Rect($this->pdf->marge, $this->pdf->getY()-.5, 297-$this->pdf->marge*2, 5+$tweeRegels*4, 'F');
...
//S        logScherm('aantal:'.count($this->clientVermogensbeheerder));
      ...
      foreach($categorieVolgorde as $categorie=>$volgorde)
      {
        ...
        foreach($portefeuilleWaarden as $portefeuille=>$belCatData)
        {
          ...
          if($regelData[$portefeuille][$categorieOmschrijving[$categorie]]['percentage']>=100)
            $dec=0;
          else
            $dec=1;
          array_push($regel, $this->formatGetal($regelData[$portefeuille][$categorieOmschrijving[$categorie]]['percentage'],$dec).'%');
      ...
      if($consolidatie==true)
      {
        ...
        if($regelDataTotaal['Totaal']['percentage']>=100)
          $dec=0;
        else
          $dec=1;
        array_push($regel, $this->formatGetal($regelDataTotaal['Totaal']['percentage'],$dec).'%');
      ...
      foreach($portefeuilleWaarden as $portefeuille=>$belCatData)
      {
        ...
        if($regelDataTotaal[$portefeuille]['percentage']>=100)
          $dec=0;
        else
          $dec=1;
        array_push($regel, $this->formatGetal($regelDataTotaal[$portefeuille]['percentage'],$dec).'%');
      ...
      if(true)
      {
        ...
        if($consolidatie==true)
        {
          ...
          {
            $this->pdf->setXY($x - 5, $y + (count($categorieVolgorde) + 1) * $this->pdf->rowHeight + 8);
            $this->BarDiagram(35, 35, $totaalGrafiek[0], '', $totaalGrafiek[1]);
          }
          ...
          if($aandeelOpTotaal>=1)
            $dec=0;
          else
            $dec=1;
          $this->pdf->Cell($headerWidth, 4, $this->formatGetal($aandeelOpTotaal * 100, $dec) . ' %', 0, 0, 'C', 0);
        }
      }
      ...
      foreach($portefeuilleWaarden as $portefeuille=>$belCatData)
      {
        ...
        {
          $this->pdf->setXY($x-5, $beginY+5);
          $this->BarDiagram(35, 35, $portefeuilleWaardenGrafiek[$portefeuille]['waarde'],'', $portefeuilleWaardenGrafiek[$portefeuille]['kleur']);
        }
        ...
        $this->pdf->Cell(35-$extraW,4,$this->formatGetal($aandeelOpTotaal*100,$dec).' %',0,0,'C',0);
