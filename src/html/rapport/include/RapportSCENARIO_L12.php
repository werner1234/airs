...
$Log: RapportSCENARIO_L12.php,v $
Revision 1.4  2020/02/29 16:24:09  rvv
*** empty log message ***

Revision 1.3  2018/03/03 17:14:34  rvv
*** empty log message ***

Revision 1.2  2017/10/18 15:30:04  rvv
*** empty log message ***

Revision 1.1  2017/10/14 17:27:54  rvv
*** empty log message ***
...
class RapportSCENARIO_L12
{
	function RapportSCENARIO_L12($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//    {
      ...
      $this->pdf->widthA = array(175,30);
		  $this->pdf->widthB = array(150,29,25,26,25,25);
      ...
      $this->pdf->row(array('',
                            ...
                            vertaalTekst('Zeer pessimistisch',$this->pdf->rapport_taal),
                            vertaalTekst('Verwacht',$this->pdf->rapport_taal),
                            vertaalTekst('Zeer optimistisch',$this->pdf->rapport_taal)));
      $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
      $negatiefAdvies=true;
      $maxKansTmp=0;
      ...
      foreach($kansData['risicoklassen'] as $risicoklasse=>$klasseData)
      {
        ...
        $this->pdf->row(array('',vertaalTekst($risicoklasse,$this->pdf->rapport_taal),
                            ...
                            $this->formatGetalNegatief($klasseData['uitkomstKans']['scenarioEindwaarden']['Verwacht']),
    ...
    if(!$sc->loadMatrix())
      $sc->createNewMatix(true);
//
   	$this->pdf->widthA = array(40,5,25,20);
		$this->pdf->alignA = array('L','C','R','R');
    $this->pdf->SetWidths($this->pdf->widthA);
    $this->pdf->SetAligns($this->pdf->alignA);
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
    $this->pdf->setY(50);
    ...
    $this->pdf->SetFont($this->pdf->rapport_font,'b',$this->pdf->rapport_fontsize);
    $this->pdf->setY(50);
    $this->pdf->widthB = array(150,40,5,25,20);
    ...
    $verwachtVermogen=$kansData['risicoklassen'][$sc->CRMdata['gewenstRisicoprofiel']]['uitkomstKans']['scenarioEindwaarden']['Verwacht'];
    ...
    $this->pdf->row(array('',vertaalTekst('Verwacht scenario',$this->pdf->rapport_taal),"€",$this->formatGetalNegatief($verwachtVermogen,0)));
    if($this->pdf->lastPOST['scenario_inflatie']==1)
    {
      $verwachtVermogenMetInflatie = $verwachtVermogen / $sc->inflatieCorrectie;
      $this->pdf->row(array('', vertaalTekst('Met inflatiecorrectie', $this->pdf->rapport_taal), "€", $this->formatGetalNegatief($verwachtVermogenMetInflatie, 0)));
    }
    unset($this->pdf->CellFontStyle);
    ...
    if($sc->vrhMethode >0)
    {
      ...
      if($sc->vrhMethode==1)
        $this->pdf->row(array('',vertaalTekst('Volledig',$this->pdf->rapport_taal),$this->formatGetal($rendementsheffingWaarden['percentage1'],1),'0'));
      elseif($sc->vrhMethode==2)
        $this->pdf->row(array('',vertaalTekst('Vrijstelling 1P',$this->pdf->rapport_taal),$this->formatGetal($rendementsheffingWaarden['percentage1'],1),$this->formatGetal($rendementsheffingWaarden['vrijstellingEenP'],0)));
      elseif($sc->vrhMethode==3)
        $this->pdf->row(array('',vertaalTekst('Vrijstelling 2P',$this->pdf->rapport_taal),$this->formatGetal($rendementsheffingWaarden['percentage1'],1),$this->formatGetal($rendementsheffingWaarden['vrijstellingTweeP'],0)));
    }
    ...
    $this->pdf->setY(90);
		$this->pdf->widthB = array(5,35,30,45);
		$this->pdf->alignB = array('L','L','R','R');
    $this->pdf->SetWidths($this->pdf->widthB);
    $this->pdf->SetAligns($this->pdf->alignB);
    $this->pdf->SetFont($this->pdf->rapport_font,'b',$this->pdf->rapport_fontsize);
    $this->pdf->row(array('',vertaalTekst('Scenario',$this->pdf->rapport_taal).' '.vertaalTekst($sc->CRMdata['gewenstRisicoprofiel'],$this->pdf->rapport_taal),
                             vertaalTekst('Kans van ongeveer',$this->pdf->rapport_taal),
                             vertaalTekst('Op minimaal eindvermogen van',$this->pdf->rapport_taal)));
...
*/
   ...
   // listarray($polly);
    ...
    $rr=1.5;
    $this->pdf->Circle($xval,$yval, $rr, 0,360, $style = 'DF', $circleStyle, array(255,255,255));
    $this->pdf->Circle($xval,$yval, $r, 0,360, $style = 'DF', $circleStyle, array(0,0,0));

    $this->pdf->Circle($XDiag,$YDiag+$h+10, $rr, 0,360, $style = 'DF', $circleStyle, array(255,255,255));
    $this->pdf->Circle($XDiag,$YDiag+$h+10, $r, 0,360, $style = 'DF', $circleStyle, array(0,0,0));
    $this->pdf->TextWithRotation($XDiag+3,$YDiag+$h+10+1,vertaalTekst('Doelvermogen',$this->pdf->rapport_taal),0);
