...
Revision 1.1  2013/08/18 12:23:35  rvv
*** empty log message ***
...
class RapportMUT_L42
{
	function RapportMUT_L42($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		$this->pdf->rapport_type = "MUT";
		$this->pdf->rapport_datum = db2jul($rapportageDatum);
		$this->pdf->rapport_datumvanaf = db2jul($rapportageDatumVanaf);
		$this->pdf->rapport_titel = "Mutatie overzicht ".date("j",$this->pdf->rapport_datumvanaf)." ".
...
//		$this->pdf->Line($totaal2+2,$this->pdf->GetY(),$totaal2 + $this->pdf->widthA[7],$this->pdf->GetY());
    ...
    $this->pdf->templateVarsOmschrijving['MUTPaginas']=$this->pdf->rapport_titel;
	   ...
	   $extraquery .= "AND Rekeningmutaties.Grootboekrekening IN('$grootboekSelectie')  ";
		...
		while($mutaties = $DB->nextRecord())
		{
			...
			if($lastCategorie <> $mutaties['gbOmschrijving'] && !empty($lastCategorie) )
			{
				...
				if($totaal < 0)
					$actueleWaardePortefeuille += $this->printTotaal(vertaalTekst("Totaal",$this->pdf->rapport_taal)." ".strtolower(vertaalTekst($lastCategorie,$this->pdf->rapport_taal)), abs($totaal), 0);
				else
				 	$actueleWaardePortefeuille += $this->printTotaal(vertaalTekst("Totaal",$this->pdf->rapport_taal)." ".strtolower(vertaalTekst($lastCategorie,$this->pdf->rapport_taal)), 0, abs($totaal));
				$subdebet = 0;
				$subcredit = 0;
			}
      ...
      $mutaties['Omschrijving']=substr($mutaties['Omschrijving'],0,1).strtolower(substr($mutaties['Omschrijving'],1));
