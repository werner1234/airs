...
$Log: RapportSCENARIO_L48.php,v $
Revision 1.3  2016/10/23 11:32:33  rvv
*** empty log message ***

Revision 1.2  2015/11/14 13:24:32  rvv
*** empty log message ***

Revision 1.1  2015/11/11 17:28:10  rvv
*** empty log message ***
...
class RapportSCENARIO_L48
{
	function RapportSCENARIO_L48($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//
    ...
    $this->pdf->row(array(vertaalTekst('Gekozen beleggingsprofiel',$this->pdf->rapport_taal),vertaalTekst($sc->CRMdata['gewenstRisicoprofiel'],$this->pdf->rapport_taal)));
    $this->pdf->row(array(vertaalTekst('Prognose rendement',$this->pdf->rapport_taal),$this->formatGetal(($sc->profieldata['verwachtRendement']-1)*100,1).'%'));
    ...
    $this->pdf->row(array('',vertaalTekst('Gemiddeld eindvermogen',$this->pdf->rapport_taal),"€ ".$this->formatGetalNegatief($sc->verwachteWaarden['Normaal'])));
    ...
    else
    {
      ...
      foreach($sc->cashflowText as $bedragData)
      {
          $this->pdf->row(array('',$bedragData[0],$this->formatGetal($bedragData[1],0),$this->formatGetal($bedragData[2],0)));
      }
    }
    
  
    $this->pdf->setY(90);
		$this->pdf->widthB = array(5,50,30,30);
		$this->pdf->alignB = array('L','L','R','R');
    $this->pdf->SetWidths($this->pdf->widthB);
    $this->pdf->SetAligns($this->pdf->alignB);
    $this->pdf->SetFont($this->pdf->rapport_font,'b',$this->pdf->rapport_fontsize);
...
//    {
      ...
      $this->pdf->SetFont($this->pdf->rapport_font,'b',$this->pdf->rapport_fontsize);
      $this->pdf->row(array('',
                            vertaalTekst('Beleggingsprofiel',$this->pdf->rapport_taal),
