...
class RapportVKMS_L12
{
	function RapportVKMS_L12($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
    ...
    if($this->pdf->rapportageValuta==''|| $this->pdf->rapportageValuta=='EUR')
      $this->valutaTeken='€';
    elseif($this->pdf->rapportageValuta=='USD')
      $this->valutaTeken='$';
    elseif($this->pdf->rapportageValuta=='GBP')
      $this->valutaTeken='£';
    else
      $this->valutaTeken='';
	}
  ...
  function formatGetalKoers($waarde, $dec ,$procent=false,$toonNul=false, $start=false)
  {
    if($waarde==0 && $toonNul==false)
      return;
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
	...
	//		$this->pdf->row(array(vertaalTekst('Doorlopende kosten ten opzichte van onderliggend vermogen',$this->pdf->rapport_taal), $this->formatGetal($doorlopendeKostenPercentage * 100, 2) . ' %'));
		...
		//echo "$herrekendeKosten $aandeelIndirect";exit;
    ...
    $grootboekRegels=array();
    while($data = $DB->nextRecord())
    {
      $grootboekRegels[$data['Grootboekrekening']]=$data;
    }
    if($spreadKostenEUR <> 0)
    {
      $grootboekRegels['KOST']['totaal'] += $spreadKostenEUR;
      if (!isset($grootboekRegels['KOST']['Omschrijving']))
      {
        $grootboekRegels['KOST']['Omschrijving'] = 'Transactiekosten';
        $grootboekRegels['KOST']['Grootboekrekening'] = 'KOST';
      }
    }
  
		foreach($grootboekRegels as $data)
		{
			$voetNoot='';
			if($this->pdfVullen==true)
			{
...
//					$voetNoot='3';
        ...
        if($data['Grootboekrekening']=='BEH')
        {
          ...
          $directeKostenRegels[]=array('Vergoeding Waterland',$zonderBTWBedrag/$gemiddelde*100,$zonderBTWBedrag,$voetNoot);//,'1');
          if($risicoklasse['BeheerfeeBTW']<>0)
             $directeKostenRegels[]=array('BTW over vergoeding Waterland',($data['totaal']-$zonderBTWBedrag)/$gemiddelde*100, $data['totaal']-$zonderBTWBedrag,$voetNoot);//,'4');
        }
        else
        {
    ...
    //$directeKostenRegels[]=array('Spread-kosten',$spreadKostenEUR/$gemiddelde*100,$spreadKostenEUR,'');
    //$totaal+= $spreadKostenEUR;
      ...
      $this->pdf->multicell(100,6,vertaalTekst('Overzicht kosten in de periode',$this->pdf->rapport_taal).' '.
      ...
      $this->pdf->ln($this->lnHoogte);
      $this->pdf->Cell(60,$this->cellHoogte,vertaalTekst('Kosten van de beleggingsdienstverlening',$this->pdf->rapport_taal),0,0,'L',0);
      ...
      $this->pdf->Cell(4,$this->cellHoogte,$this->valutaTeken,0,0,'L',1);
      $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
      $this->pdf->Cell(18,$this->cellHoogte,$this->formatGetalKoers($totaal, 0),0,0,'R',1);
      $this->pdf->Cell(3,$this->cellHoogte,'',0,0,'R',0);
      $this->pdf->Cell(15,$this->cellHoogte, $this->formatGetal($kostenPercentage, 2).' %',0,0,'R',1);
      $this->pdf->ln($this->lnHoogte+3);
      $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
      $this->pdf->MultiCell(60,4,vertaalTekst("Kosten van de financiële instrumenten van de beleggingsportefeuille",$this->pdf->rapport_taal),0,'L',0);
      ...
      $this->pdf->Cell(4,$this->cellHoogte,$this->valutaTeken,0,0,'L',1);
      $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
      $this->pdf->Cell(18,$this->cellHoogte,$this->formatGetalKoers($vkmPercentagePortefeuille*$gemiddelde/100, 0),0,0,'R',1);
      $this->pdf->Cell(3,$this->cellHoogte,'',0,0,'R',0);
      $this->pdf->Cell(15,$this->cellHoogte, $this->formatGetal($vkmPercentagePortefeuille, 2).' %',0,0,'R',1);
      /*
  	$this->pdf->Cell(22,$this->cellHoogte,'€ '.$this->formatGetal($euroBedrag, 0),0,0,'R',1);
		
		$this->pdf->Cell(15,$this->cellHoogte,$this->formatGetal($percentage, 2) . ' %',0,0,'R',1);
       */
      $this->pdf->ln($this->lnHoogte+4);
      ...
      $this->pdf->ln(3);
      $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
      $this->pdf->Cell(60,$this->cellHoogte,vertaalTekst('Totaal',$this->pdf->rapport_taal),0,0,'L',0);
      $this->pdf->SetFillColor($this->pdf->rapport_donker_kleur[0],$this->pdf->rapport_donker_kleur[1],$this->pdf->rapport_donker_kleur[2]);
      
      $this->pdf->SetFont($this->pdf->rapport_fontEur,'B',$this->pdf->rapport_fontsize);
      $this->pdf->Cell(4,$this->cellHoogte,$this->valutaTeken,0,0,'L',0);
      $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
      $this->pdf->Cell(18,$this->cellHoogte,$this->formatGetalKoers($vkmWaarde*$gemiddelde/100, 0),0,0,'R',0);
      $this->pdf->Cell(3,$this->cellHoogte,'',0,0,'R',0);
      $this->pdf->Cell(15,$this->cellHoogte, $this->formatGetal($vkmWaarde, 2).' %',0,0,'R',0);
      

      $this->pdf->setY(85);
      $this->pdf->Cell(60,$this->cellHoogte,'Kosten van de beleggingsdienstverlening',0,0,'L',0);
      
      $this->pdf->SetFont($this->pdf->rapport_fontEur,'B',$this->pdf->rapport_fontsize);
      $this->pdf->Cell(4,$this->cellHoogte,$this->valutaTeken,0,0,'L',0);
      $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
      $this->pdf->Cell(18,$this->cellHoogte,$this->formatGetalKoers($totaal,0),0,0,'R',0);
      $this->pdf->Cell(3,$this->cellHoogte,'',0,0,'R',0);
      $this->pdf->Cell(15,$this->cellHoogte,$this->formatGetal($totaal/$gemiddelde*100,2).' %',0,0,'R',0);
      $this->pdf->ln(8);
			$this->pdf->SetFillColor($this->pdf->rapport_donker_kleur[0],$this->pdf->rapport_donker_kleur[1],$this->pdf->rapport_donker_kleur[2]);
      foreach($directeKostenRegels as $regel)
      {
			...
			$this->pdf->line($this->pdf->marge,$this->pdf->getY(),$this->pdf->marge+103,$this->pdf->getY(),array('color'=>array($this->pdf->rapport_totaalLijnenColor[0],$this->pdf->rapport_totaalLijnenColor[1],$this->pdf->rapport_totaalLijnenColor[2])));
      ...
      $this->pdf->SetFillColor($this->pdf->rapport_licht_kleur[0],$this->pdf->rapport_licht_kleur[1],$this->pdf->rapport_licht_kleur[2]);
			...
			$this->addKostenRegel('Kosten beleggingsfondsen','',$vkmPercentagePortefeuille*$gemiddelde*$aandeel/100,$vkmPercentagePortefeuille*$aandeel);
  		$aandeel=$totaalDoorlopendekostenGesplitst['FundTransCost']/$totaalKosten;
			$this->addKostenRegel('Transactiekosten beleggingsfondsen','',$vkmPercentagePortefeuille*$gemiddelde*$aandeel/100,$vkmPercentagePortefeuille*$aandeel);
			$aandeel=$totaalDoorlopendekostenGesplitst['FundPerfFee']/$totaalKosten;
			$this->addKostenRegel('Performance fee beleggingsfondsen','',$vkmPercentagePortefeuille*$gemiddelde*$aandeel/100,$vkmPercentagePortefeuille*$aandeel);

			$this->pdf->setX($this->pdf->marge);
			/*
      $this->pdf->Cell(60,$this->cellHoogte,'',0,0,'L',0);
      $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
      $this->pdf->Cell(40,$this->cellHoogte,$this->formatGetal($vkmPercentagePortefeuille,2).' %',0,0,'C',0);
      */
      $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
      $this->pdf->MultiCell(60,4,vertaalTekst("Kosten van de financiële instrumenten van de beleggingsportefeuille  2)",$this->pdf->rapport_taal),0,'L',0);
      $this->pdf->setXY($this->pdf->marge+60,$this->pdf->getY()-7.5);
      
      $this->pdf->SetFont($this->pdf->rapport_fontEur,'B',$this->pdf->rapport_fontsize);
      $this->pdf->Cell(4,$this->cellHoogte,$this->valutaTeken,0,0,'L',0);
      $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
      $this->pdf->Cell(18,$this->cellHoogte,$this->formatGetalKoers($vkmPercentagePortefeuille*$gemiddelde/100,0),0,0,'R',0);
      

      $this->pdf->Cell(3,$this->cellHoogte,'',0,0,'R',0);
      $this->pdf->Cell(15,$this->cellHoogte,$this->formatGetal($vkmPercentagePortefeuille,2).' %',0,0,'R',0);
      $this->pdf->ln($this->lnHoogte+3);
      $this->pdf->SetFillColor($this->pdf->rapport_donker_kleur[0],$this->pdf->rapport_donker_kleur[1],$this->pdf->rapport_donker_kleur[2]);
   ...
   //   rapport_kop_kleur
	...
	{
		$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
		$this->pdf->Cell(57,$this->cellHoogte,vertaalTekst($omschrijving,$this->pdf->rapport_taal),0,0,'L',0);
		$this->pdf->setTextColor($this->voetnootGrijs[0],$this->voetnootGrijs[1],$this->voetnootGrijs[2]);
		$this->pdf->Cell(3,$this->cellHoogte,$getal,0,0,'R',0);
		$this->pdf->setTextColor(0);
		$this->pdf->SetFont($this->pdf->rapport_fontEur,'',$this->pdf->rapport_fontsize);
    $this->pdf->Cell(4,$this->cellHoogte,$this->valutaTeken,0,0,'L',1);
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
		$this->pdf->Cell(18,$this->cellHoogte,$this->formatGetalKoers($euroBedrag, 0),0,0,'R',1);
	...
	function voetNoot()
	{
		$this->pdf->setWidths(array(140,$this->cellHoogte,130));
		$this->pdf->setAligns(array('L','L','L'));
		$this->pdf->setXY($this->pdf->marge,140);
		$this->pdf->line($this->pdf->getX()+140,$this->pdf->getY()-4,$this->pdf->getX()+140+105,$this->pdf->getY()-4,
										 array('color'=>array($this->pdf->rapport_totaalLijnenColor[0],$this->pdf->rapport_totaalLijnenColor[1],$this->pdf->rapport_totaalLijnenColor[2])));
    $this->pdf->setTextColor($this->voetnootGrijs[0],$this->voetnootGrijs[1],$this->voetnootGrijs[2]);
    $this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
...
foreach ($perCategorie as $hoofdcategorie=>$categorieData)
{
 ...
 WHERE Portefeuilles.portefeuille='".$this->portefeuille."'";
    ...
    for($i=$this->pdf->rapport_jaar; $i<=$doelJaar; $i++)
		...
		$grafiekWaarden['titel']="Invloed van kosten op de waarde van de portefeuille";
		$waardeZonderKostenKleur= $this->pdf->rapport_kop_kleur;//  array(100,100,200);
		$waardeNaKostenKleur=$this->pdf->rapport_donker_kleur;array(100,200,100);
		$cumulatieveKostenKleur=$this->pdf->rapport_licht_kleur;array(200,100,100);
...
//		echo "$doelJaar $rendement $beginwaarde";
	...
	function LineDiagram($w, $h, $data, $color=null, $maxVal=0, $minVal=0, $horDiv=4, $verDiv=4,$vanafBegin=false)
	{
		...
		$this->pdf->setXY($XPage,$this->pdf->getY()-10);
		$this->pdf->SetFillColor($this->pdf->rapport_kop_kleur[0],$this->pdf->rapport_kop_kleur[1],$this->pdf->rapport_kop_kleur[2]);
		$this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
		$this->pdf->SetTextColor(255);
		$this->pdf->Cell($w,6,vertaalTekst($titel,$this->pdf->rapport_taal),0,0,'L',1);
		$this->pdf->SetTextColor(0);


		$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
		$this->pdf->SetLineStyle(array('width' => 0.3, 'cap' => 'round', 'join' => 'miter', 'dash' => 0, 'color' => array($this->pdf->rapport_licht_kleur[0],$this->pdf->rapport_licht_kleur[1],$this->pdf->rapport_licht_kleur[2])));

		//$this->pdf->Rect($XDiag, $YDiag, $w-$margin, $h,'D','',array(245,245,245));
    $this->pdf->Line($XDiag, $YDiag, $XDiag,$YDiag+$h,array('width' => 0.2,'color'=>array($this->pdf->rapport_licht_kleur[0],$this->pdf->rapport_licht_kleur[1],$this->pdf->rapport_licht_kleur[2])));
    $this->pdf->Line($XDiag, $YDiag+$h, $XDiag + $w  ,$YDiag+$h,array('width' => 0.2,'color'=>array($this->pdf->rapport_licht_kleur[0],$this->pdf->rapport_licht_kleur[1],$this->pdf->rapport_licht_kleur[2])));
    ...
    $this->pdf->SetFont($this->pdf->rapport_fontEur,'',$this->pdf->rapport_fontsize);
    $this->pdf->setXY($XDiag-4, $YPage+$hDiag-4);
    $this->pdf->Cell(4,$this->cellHoogte,$this->valutaTeken,0,0,'L',0);
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
		...
		{
			$skipNull = true;
			$this->pdf->Line($XDiag, $i, $XPage+$w ,$i,array('color'=>array($this->pdf->rapport_licht_kleur[0],$this->pdf->rapport_licht_kleur[1],$this->pdf->rapport_licht_kleur[2])));
		...
		for ($i=0; $i<count($data); $i++)
		{
			if($i%$jaren==0)
			{
			//	$this->pdf->Line($XDiag + ($i) * $unit , $YDiag, $XDiag + ($i) * $unit  ,$YDiag+$h,array('width' => 0.2,'color'=>array($this->pdf->rapport_licht_kleur[0],$this->pdf->rapport_licht_kleur[1],$this->pdf->rapport_licht_kleur[2])));
      //  $this->pdf->Line($XDiag + ($i) * $unit , $YDiag+$h, $XDiag + ($i) * $unit  ,$YDiag+$h+1,array('width' => 0.2,'color'=>array($this->pdf->rapport_licht_kleur[0],$this->pdf->rapport_licht_kleur[1],$this->pdf->rapport_licht_kleur[2])));
			...
			elseif($index==1)
      {
        ...
        $step +=5;
			...
			$this->pdf->Rect($XPage+$step , $YPage+$h+11, 3, 1, 'DF','',$kleur);
			$this->pdf->SetXY($XPage+$step+3,$YPage+$h+10);
