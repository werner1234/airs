...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: ATTberekening_L67.php,v $
 		Revision 1.6  2020/06/17 15:38:53  rvv
 		*** empty log message ***
 		
 		Revision 1.5  2020/06/14 10:04:17  rvv
 		*** empty log message ***
 		
 		Revision 1.4  2020/06/14 06:41:39  rvv
 		*** empty log message ***
 		
 		Revision 1.3  2018/07/07 17:35:19  rvv
 		*** empty log message ***
...
class ATTberekening_L67
{

	function ATTberekening_L67($rapportData,$periode='maanden')
	{
...
ORDER BY HoofdBeleggingscategorien.Afdrukvolgorde, Regios.Afdrukvolgorde,Beleggingscategorien.Afdrukvolgorde";
    ...
    if($categorie=='totaal')
    {
      $fondsData=array('Fonds'=>$this->rapport->pdf->portefeuilledata['SpecifiekeIndex'],'Percentage'=>1);
...
INNER JOIN ZorgplichtPerBeleggingscategorie ON ZorgplichtPerRisicoklasse.Zorgplicht = ZorgplichtPerBeleggingscategorie.Zorgplicht AND ZorgplichtPerRisicoklasse.Vermogensbeheerder = ZorgplichtPerBeleggingscategorie.Vermogensbeheerder
INNER JOIN Portefeuilles ON ZorgplichtPerRisicoklasse.Risicoklasse = Portefeuilles.Risicoklasse AND Portefeuilles.Vermogensbeheerder = ZorgplichtPerRisicoklasse.Vermogensbeheerder
INNER JOIN IndexPerBeleggingscategorie ON IndexPerBeleggingscategorie.Vermogensbeheerder = ZorgplichtPerBeleggingscategorie.Vermogensbeheerder AND IndexPerBeleggingscategorie.Beleggingscategorie = ZorgplichtPerBeleggingscategorie.Beleggingscategorie
WHERE Portefeuilles.Portefeuille='" . $this->rapport->portefeuille . "' AND ZorgplichtPerBeleggingscategorie.Beleggingscategorie='$categorie' ORDER BY IndexPerBeleggingscategorie.Fonds";
        $DB->SQL($query);
        $fondsData = $DB->lookupRecord();
      ...
      $queryKostenOpbrengsten = "SELECT
       ...
       if(count($fondsData['rekeningen']) > 0 && $fondsData['rekeningen'][0] <> 'geen')
       {
         ...
         // $RekeningDirecteKostenOpbrengsten['kostenTotaal']+= $nietToegerekendeKosten['kostenTotaal'];
          ...
          $AttributieStortingenOntrekkingen['nietToegerekendeKosten']+= $nietToegerekendeKosten['kostenTotaal'];
...
//listarray($RekeningDirecteKostenOpbrengsten); ob_flush();
  ...
  'onttrekking'=>$AttributieStortingenOntrekkingen['onttrekking'],
   'nietToegerekendeKosten'=>$AttributieStortingenOntrekkingen['nietToegerekendeKosten'],
...
$stapelItems=array('procent','bijdrage','indexPerf'.'indexBijdrage');
$avgItems=array('weging','gemWaarde');
$somItems=array('resultaat','storting','onttrekking','kosten','opbrengst','stort','nietToegerekendeKosten');
