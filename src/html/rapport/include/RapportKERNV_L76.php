...
class RapportKERNV_L76
{
	function RapportKERNV_L76($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    if(!isset($this->pdf->excelData) || count($this->pdf->excelData)<1)
      $this->pdf->excelData[] = array('ISIN','Fonds','Beleggingscategorie','Rating','TypeInstrument','Valuta','Valutakoers','Positie','Historische koers','Historisch Waarde EUR','Ytd-kostprijs','Actuele koers','Waarde in EUR','Opgelopen rente in EUR','Totale waarde in EUR','Actueel rentepercentage','Mod. Duration','Lossingsdatum','Portefeuille','Koersdatum','DoGood');
...
(TijdelijkeRapportage.totaalAantal * TijdelijkeRapportage.historischeWaarde * TijdelijkeRapportage.historischeValutakoers * TijdelijkeRapportage.fondsEenheid / TijdelijkeRapportage.historischeRapportageValutakoers) AS historischeWaardeEUR    
    ...
    $fondsExtraInformatie=new FondsExtraInformatie();
...
WHERE emittentPerFonds.vermogensbeheerder='".$this->pdf->portefeuilledata['Vermogensbeheerder']."' AND  emittentPerFonds.fonds='$fonds'"; 
    ...
    foreach($fondsen as $fonds=>$fondsData)
    {
      $fondsExtraInformatie->getByField('fonds',$fonds);
     ...
     if($fondsData['Lossingsdatum']=='0000-00-00')
      ...
      $this->pdf->excelData[] = array($fondsData['ISINCode'],
                                      ...
                                      $fondsData['Valuta'],
                                      $fondsData[$this->rapportageDatum]['actueleValuta'],
                                      ...
                                      $fondsExtraInformatie->get('DoGood'));
