...
/*
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2016/11/30 16:48:42 $
 		File Versie					: $Revision: 1.8 $

 		$Log: RapportCASHY_L66.php,v $
 		Revision 1.8  2016/11/30 16:48:42  rvv
 		*** empty log message ***
 		
 		Revision 1.7  2016/11/23 16:55:38  rvv
 		*** empty log message ***
 		
 		Revision 1.6  2016/11/16 16:51:17  rvv
 		*** empty log message ***
 		
 		Revision 1.5  2016/11/13 10:11:20  rvv
 		*** empty log message ***
 		
 		Revision 1.4  2016/11/12 20:21:18  rvv
 		*** empty log message ***
 		
 		Revision 1.3  2016/10/26 16:13:40  rvv
 		*** empty log message ***
 		
 		Revision 1.2  2016/10/24 10:00:10  rvv
 		*** empty log message ***
...
class RapportCASHY_L66
{
	function RapportCASHY_L66($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	 //
		$this->pdf = &$pdf;
	...
	{
		global $__appvar;
    $cashStart=db2jul($this->rapportageDatum);
  	...
  	$cashflowJaar=array();
		$cashflowTotaal=0;
/*
    $janJul=mktime(0,0,0,1,1,date("Y",$this->pdf->rapport_datumvanaf));
    ...
    if($startJul>$janJul)
      ...
      $cashStart=$janJul;
*/
    $cashfow = new Cashflow($this->portefeuille,$cashStart,$cashStart,$this->pdf->debug);
    ...
    $cashflowJaarGrafiek=array();
      ...
      if($jaar>=$eersteJaar && $jaar<($eersteJaar+12) && $jaar <> 'Overig')
      {
        if(!isset($cashflowJaarGrafiek[$jaar]))
          $cashflowJaarGrafiek[$jaar]=array('lossing'=>0,'rente'=>0);

        $cashflowJaarGrafiek[$jaar][$regel[2]] += $regel[3];
      }
...
$this->pdf->CellBorders = array();
    ...
    $this->VBarDiagram(160,60,$cashflowHuidigjaar,"Lopend jaar",100);
		$this->pdf->setXY(160,110);
    $this->VBarDiagram(160,60,$cashflowJaarGrafiek,"Langere termijn",1000);
	}


	function VBarDiagram($w, $h, $data,$titel,$yVerdeling)
  {
      global $__appvar;
      $legendaWidth = 50;
      $grafiekPunt = array();
      ...
      $this->pdf->Multicell(20,5,'X '.$this->formatGetal($yVerdeling,0),'','L');
      ...
      $colors=array('rente'=>array(0,54,103),
      'lossing'=>array(149,160,169));
      ...
      for($i=$nulpunt; $i > $top; $i-= $absUnit*$stapgrootte)
      {
        ...
        $this->pdf->SetFont($this->pdf->rapport_font, 'B', 6);
        $this->pdf->Cell(10, 3, $this->formatGetal($n*$stapgrootte/$yVerdeling)."",0,0,'L');
        $n++;
      }
