...
$Log: RapportVKMS_L42.php,v $
Revision 1.2  2019/02/03 15:50:24  rvv
*** empty log message ***
...
class RapportVKMS_L42
{
	function RapportVKMS_L42($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
    ...
    while($data = $DB->nextRecord())
    {
      ...
      $kostenData[$data['Grootboekrekening']]['totaal']+=$data['totaal'];
        ...
        if($data['Grootboekrekening']=='BEH')
        {
          ...
          $kostenRow[] = array(vertaalTekst('Vergoeding Ritzer + Rouw', $this->pdf->rapport_taal), $this->formatGetal($bedrag, 0), $this->formatGetal($bedrag/$gemiddelde*100, 2) . ' %');
          if($risicoklasse['BeheerfeeBTW'] <> 0)
          {
            $kostenRow[] = array(vertaalTekst("BTW over vergoeding Ritzer + Rouw", $this->pdf->rapport_taal), $this->formatGetal($btw, 0), $this->formatGetal($btw/$gemiddelde*100, 2) . ' %');
          }
        }
        else
        {
