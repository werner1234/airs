...
include_once("rapport/include/ATTberekening_L44.php");

class RapportPERF_L44
{

	function RapportPERF_L44($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    $this->att=new ATTberekening_L44($this);
	}
...
function risicoBox()
{
  ...
  if(1==0)
  {
    ...
    if($yearCount <> 0)
    {
      $indexWaarden = $index->getWaarden($this->pdf->PortefeuilleStartdatum,$this->rapportageDatum,$this->portefeuille,'',$perioden);
      ...
      echo "standaarddeviatie= $standaarddeviatie= $standaarddeviatiePeriode*sqrt($yearCount)<br>\n";
      listarray($afm);
      */
    }
  }
 ...
 // $this->pdf->Row(array('','Standaarddeviatie volgens portefeuilleprofiel',$this->formatGetal($verwachtRendement['klasseStd'],2).' %'));
  ...
  function plotZorgBar($width,$height,$data)
  {
    ...
    $this->pdf->Cell(5,5,"Mandaatcontrole",0,0,'C');
  ...
  function plotZorgBar2($barWidth,$height,$zorgdata)
  {
    ...
    $this->pdf->setXY(240,45);
    $this->pdf->Cell(5,5,"Mandaat controle",0,0,'C');
    $this->pdf->Rect(205,50,70,95);
    $this->pdf->setXY(215,64);
