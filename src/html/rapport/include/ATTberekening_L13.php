...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: ATTberekening_L13.php,v $
 		Revision 1.3  2019/01/12 17:08:31  rvv
 		*** empty log message ***
 		
 		Revision 1.2  2018/12/24 08:20:14  rvv
 		*** empty log message ***
...
class ATTberekening_L13
{

	function ATTberekening_L13($rapportData)
	{
...
ORDER BY HoofdBeleggingscategorien.Afdrukvolgorde, Regios.Afdrukvolgorde,Beleggingscategorien.Afdrukvolgorde";
    ...
    $perfData['totaal'] = $this->fondsPerformance($alleData,$van,$tot,false,'totaal');
    
    foreach ($categorien as $categorie=>$categorieData)
    { 
        $this->huidigeCategorie=$categorie;
		    $perfData[$categorie] = $this->fondsPerformance($categorieData,$van,$tot,false,$categorie);
        ...
        WHERE
         ...
         if($categorie=='Liquiditeiten')
         {
          ...
          // $FondsDirecteKostenOpbrengsten['kostenTotaal']+= $nietToegerekendeKosten['kostenTotaal'];
           ...
           $nietToegerekendeKostenExtra=$nietToegerekendeKosten['kostenTotaal'];
         }
         else
         {
...
//echo "<br>$nietToegerekendeKostenExtra<br><br>";
...
if($stapeling == true || 1)
{
 ...
 $query = "SELECT  Grootboekrekeningen.Opbrengst,Grootboekrekeningen.Kosten, Grootboekrekeningen.Grootboekrekening,".
  ...
  $kostenPerGrootboek=array();
      ...
      if($categorie=='Liquiditeiten')
        $perfData['totaal']['kosten']= $perfData['totaal']['extraKosten'];
      else
        $perfData['totaal']['kosten']=$totaalKosten+ $perfData['totaal']['extraKosten'];
      
      //echo "$categorie ".$perfData['totaal']['kosten'] ." = $totaalKosten+". $perfData['totaal']['extraKosten']." <br>\n";
