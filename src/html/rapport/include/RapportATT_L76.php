...
$Log: RapportATT_L76.php,v $
Revision 1.7  2018/08/18 12:40:14  rvv
php 5.6 & consolidatie

Revision 1.6  2018/07/16 05:24:11  rvv
*** empty log message ***

Revision 1.5  2018/07/14 14:04:37  rvv
*** empty log message ***

Revision 1.4  2018/04/29 09:49:14  rvv
*** empty log message ***
...
class RapportATT_L76
{
	function RapportATT_L76($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
$this->pdf->rapport_titel ="Vergelijkingsmaatstaven, beleggingsresultaat en kosten";
$this->toonIndex();
...
$this->toonKosten();
    ...
    $data['periode']=$this->getResultaat($this->realRapportageDatumVanaf,$this->rapportageDatum);
	  $data['ytm']=$this->getResultaat($this->rapportageDatumVanaf,$this->rapportageDatum);
    ...
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
  	...
  	$this->pdf->row(array(vertaalTekst("Beleggingsresultaat",$this->pdf->rapport_taal),"Verslagperiode","Lopend jaar"));
		//$this->pdf->row(array(vertaalTekst("Vanaf",$this->pdf->rapport_taal),$this->realRapportageDatumVanaf,$this->rapportageDatumVanaf));
		$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
		unset($this->pdf->CellBorders);
		$this->pdf->fillCell=array();
		$this->pdf->SetTextColor($this->pdf->rapport_fontcolor['r'],$this->pdf->rapport_fontcolor['g'],$this->pdf->rapport_fontcolor['b']);
		...
		$this->pdf->row(array(vertaalTekst('Ongerealiseerde resultaten',$this->pdf->rapport_taal),$this->formatGetal($data['periode']['ongerealiseerdeKoersResultaat'],2),$this->formatGetal($data['ytm']['ongerealiseerdeKoersResultaat'],2),""));
		$this->pdf->row(array(vertaalTekst('Gerealiseerde resultaten',$this->pdf->rapport_taal),$this->formatGetal($data['periode']['gerealiseerdKoersresultaat'],2),$this->formatGetal($data['ytm']['gerealiseerdKoersresultaat'],2),""));
		$this->pdf->row(array(vertaalTekst('Valutaresultaat geldrekeningen',$this->pdf->rapport_taal),$this->formatGetal($data['periode']['resulaatValutas'],2),$this->formatGetal($data['ytm']['resulaatValutas'],2),""));
		$this->pdf->row(array(vertaalTekst('Mutatie opgelopen rente',$this->pdf->rapport_taal),$this->formatGetal($data['periode']['renteResultaat'],2),$this->formatGetal($data['ytm']['renteResultaat'],2),""));

		$keys=array();
 		foreach ($data['ytm']['opbrengstPerGrootboek'] as $key=>$val)
			$keys[$key]=$key;
		foreach ($keys as $key)
		{
			if(round($data['periode']['opbrengstPerGrootboek'][$key],2) != 0.00 || round($data['ytm']['opbrengstPerGrootboek'][$key],2) != 0.00)
				$this->pdf->row(array(vertaalTekst($key,$this->pdf->rapport_taal),$this->formatGetal($data['periode']['opbrengstPerGrootboek'][$key],2),$this->formatGetal($data['ytm']['opbrengstPerGrootboek'][$key],2),""));
		}
		$this->pdf->CellBorders=array('','TS','TS');
		$this->pdf->row(array(vertaalTekst('',$this->pdf->rapport_taal),$this->formatGetal($data['periode']['totaalOpbrengst'],2),$this->formatGetal($data['ytm']['totaalOpbrengst'],2),""));
		unset($this->pdf->CellBorders);
		$this->pdf->ln();
    ...
    unset($this->pdf->CellBorders);
		...
		$this->pdf->ln();
		$this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
		$this->pdf->row(array("Totaal",$this->formatGetal($data['periode']['totaalOpbrengst']-$data['periode']['totaalKosten'],2),$this->formatGetal($data['ytm']['totaalOpbrengst']-$data['ytm']['totaalKosten'],2)));
		$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);

  }
  
  function getResultaat($vanafDatum,$totDatum)
	{
		global $__appvar;
		$waarden=array();
		$totalen=array();
		...
		$fondswaarden =  berekenPortefeuilleWaarde($this->portefeuille,$vanafDatum,$startJaar,$this->pdf->rapportageValuta,$vanafDatum);
    foreach($fondswaarden as  $fondsData)
		{
			$totalen[$vanafDatum]['waardeEurTotaal']+=$fondsData['actuelePortefeuilleWaardeEuro'];
			if($fondsData['type']=='rente')
			  $totalen[$vanafDatum]['rente']+=$fondsData['actuelePortefeuilleWaardeEuro'];
			if($fondsData['type']=='fondsen')
			{
				$totalen[$vanafDatum]['fondsWaardeEind'] += $fondsData['actuelePortefeuilleWaardeEuro'];
				$totalen[$vanafDatum]['fondsWaardeBegin']+=$fondsData['beginPortefeuilleWaardeEuro'];
			}
		}


		if(substr($totDatum,5,5)=='01-01')
			$startJaar=true;
		else
			$startJaar=false;

		$fondswaarden =  berekenPortefeuilleWaarde($this->portefeuille,$totDatum,$startJaar,$this->pdf->rapportageValuta,$vanafDatum);
		foreach($fondswaarden as  $fondsData)
		{
			$totalen[$totDatum]['waardeEurTotaal']+=$fondsData['actuelePortefeuilleWaardeEuro'];
			if($fondsData['type']=='rente')
				$totalen[$totDatum]['rente']+=$fondsData['actuelePortefeuilleWaardeEuro'];
			if($fondsData['type']=='fondsen')
			{
				$totalen[$totDatum]['fondsWaardeEind'] += $fondsData['actuelePortefeuilleWaardeEuro'];
				$totalen[$totDatum]['fondsWaardeBegin']+=$fondsData['beginPortefeuilleWaardeEuro'];
			}
		}
		...
		$waardeMutatie=$totalen[$totDatum]['waardeEurTotaal']-$totalen[$vanafDatum]['waardeEurTotaal'];
		$resultaatVerslagperiode = $waardeMutatie - $stortingen + $onttrekkingen;


		$waarden['ongerealiseerdeKoersResultaat'] = $totalen[$totDatum]['fondsWaardeEind'] - $totalen[$totDatum]['fondsWaardeBegin'];
		$waarden['gerealiseerdKoersresultaat'] = gerealiseerdKoersresultaat($this->portefeuille, $vanafDatum, $totDatum,$this->pdf->rapportageValuta,true);
		$waarden['renteResultaat'] = $totalen[$totDatum]['rente']-$totalen[$vanafDatum]['rente'];

		$totaalOpbrengst=$waarden['ongerealiseerdeKoersResultaat']+$waarden['gerealiseerdKoersresultaat']+$waarden['renteResultaat'];

		$kosten=$this->getKosten($vanafDatum,$totDatum);
		$opbrengst=$this->getOpbrengsten($vanafDatum,$totDatum);

		foreach($opbrengst['opbrengstPerGrootboek'] as $omschrijving=>$waarde)
		{
			$totaalOpbrengst+=$waarde;
		}

		$waarden['resulaatValutas'] = $resultaatVerslagperiode - ($totaalOpbrengst  -  $kosten['totaalKosten']);
		$totaalOpbrengst += $waarden['resulaatValutas'];

		$waarden['totaalOpbrengst']=$totaalOpbrengst;
		$waarden['opbrengstPerGrootboek']=$opbrengst['opbrengstPerGrootboek'];
		$waarden['kostenPerGrootboek']=$kosten['kostenPerGrootboek'];
		$waarden['totaalKosten']=$kosten['totaalKosten'];
	...
	function getOpbrengsten($vanafDatum,$totDatum)
	{
		$waarden=array();
		$DB=new DB();
...
"Rekeningmutaties.Boekdatum > '".$vanafDatum."' AND Grootboekrekeningen.Opbrengst = '1' AND ".
"Rekeningmutaties.Boekdatum <= '".$totDatum."' GROUP BY  Grootboekrekeningen.Omschrijving ";
		$DB->SQL($query);
		$DB->Query();
...
while($opbrengst = $DB->nextRecord())
{
	...
	$totaalOpbrengst += ($opbrengst['totaalcredit'] - $opbrengst['totaaldebet']);
		...
		$waarden['opbrengstPerGrootboek']=$opbrengstenPerGrootboek;
		$waarden['totaalOpbrengst']=$totaalOpbrengst;
		return $waarden;
...
function toonIndex()
    ...
    $this->pdf->Ln(7);
	 // $this->pdf->templateVars['INDEXPaginas'] = $this->pdf->customPageNo;
		$this->pdf->templateVars['ATT2Paginas']=$this->pdf->customPageNo;
		$this->pdf->templateVarsOmschrijving['ATT2Paginas'] = $this->pdf->rapport_titel;
...
function toonATT()
    ...
    $startJaarDatumTekst=date("j",db2jul($rapportageStartJaar))." ".vertaalTekst($this->pdf->__appvar["Maanden"][date("n",db2jul($rapportageStartJaar))],$this->pdf->rapport_taal)." ".date("Y",db2jul($rapportageStartJaar));
		...
		$this->pdf->templateVars[$this->pdf->rapport_type.'Paginas'] = $this->pdf->customPageNo;//+$this->pdf->extraPage
		$this->pdf->templateVarsOmschrijving[$this->pdf->rapport_type.'Paginas'] = $this->pdf->rapport_titel;
