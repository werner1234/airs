...
class RapportKERNZ_L32
{
	function RapportKERNZ_L32($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		$this->pdf->rapport_type = "KERNZ";
		$this->pdf->rapport_datum = db2jul($rapportageDatum);
		$this->pdf->rapport_titel = "Resultaat per investering";//Onderverdeling in beleggingscategorie";
	...
	function portefeuilleWaarden($portefeuille)
  {
    ...
    $interneboeking     = $stortingen-$onttrekkingen;
	...
	function header($categorieVolgorde,$extraW)
  {
    ...
    if(is_array($this->pdf->portefeuilles))
    {
      $query="SELECT Portefeuille,ClientVermogensbeheerder,Selectieveld1, if(Selectieveld1<>'',Selectieveld1,Portefeuille) as omschrijving FROM Portefeuilles WHERE Portefeuille IN('".implode("','",$this->pdf->portefeuilles)."') order by ClientVermogensbeheerder, omschrijving ";
      $DB->SQL($query);
      $DB->Query();
      while($portefeuille = $DB->NextRecord())
      {
        ...
        $portefeuilleVolgorde[$portefeuille['Portefeuille']]=$portefeuille['omschrijving'];
      }
    }
    else
    {
      $query="SELECT Portefeuille,ClientVermogensbeheerder,Selectieveld1 FROM Portefeuilles WHERE Portefeuille = '".$this->portefeuille."'";
      $DB->SQL($query);
      $DB->Query();
  ...
  //  asort($portefeuilleVolgorde);
    //listarray($portefeuilleVolgorde);
    $portefeuilleWaardenSort=array();
    foreach($portefeuilleVolgorde as $portefeuille=>$omschrijving)
    {
      $portefeuilleWaardenSort[$portefeuille]=$portefeuilleWaarden[$portefeuille];
    }
    $portefeuilleWaarden=$portefeuilleWaardenSort;
  ...
  //2+35+extraw
   ...
   // echo $portefeuilleAantal."<br>\n";listarray($portefeuilleWaarden);exit;
        ...
        foreach($portefeuilleWaarden as $portefeuille=>$pData)
        {
          ...
          $query = "SELECT
                ...
                Depotbanken.Omschrijving as depotbankOmschrijving
		          ...
		          WHERE
		            Portefeuilles.Portefeuille IN('".$portefeuille."')
          ...
          if($portefeuilledata[$portefeuille]['SoortOvereenkomst'] != 'Liquiditeiten')
          {
    ...
    unset($this->pdf->CellFontStyle);
		...
		if(count($categorieVerdeling['percentage'])>0)
    {
...
//listarray($colorArray);listarray($data);
    ...
    foreach($data as $key=>$val)
    {
      ...
      $filteredVal=$val;
      if($filteredVal<$minVal)
        $filteredVal=$minVal;
      if($filteredVal>$maxVal)
        $filteredVal=$maxVal;
