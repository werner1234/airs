...
function Header_basis_L117($object)
{
  global $__appvar;
    $pdfObject = &$object;
    ...
    $pdfObject->ln();
    $pdfObject->Line($pdfObject->marge,$pdfObject->GetY(),$pdfObject->w-$pdfObject->marge-8,$pdfObject->GetY());
    $pdfObject->Line($pdfObject->w-$pdfObject->marge-8,$pdfObject->GetY(),$pdfObject->w-$pdfObject->marge-2,$pdfObject->GetY()-6);
     ...
     $rapportagePeriode = date("d/m/Y",$pdfObject->rapport_datumvanaf).' - '.date("d/m/Y",$pdfObject->rapport_datum);
      ...
      $poly=array($pdfObject->w-$pdfObject->marge,$pdfObject->h-20,
        $pdfObject->w-$pdfObject->marge,$pdfObject->h-15,
        $pdfObject->w-$pdfObject->marge-5,$pdfObject->h-15);
      $pdfObject->Polygon($poly,'F',null,$pdfObject->rapport_donkergroen);
	...
	function HeaderVKM_L117($object)
	{
		$pdfObject = &$object;
		$pdfObject->HeaderVKM();
	}
	
	  function HeaderFRONT_L117($object)
	  {
	    $pdfObject = &$object;
	    //$pdfObject->headerSCENARIO();

	  }
	  function HeaderSCENARIO_L117($object)
	  {
	    $pdfObject = &$object;
	    //$pdfObject->headerSCENARIO();

	  }
function HeaderVKMS_L117($object)
{
  $pdfObject = &$object;
  //$pdfObject->headerSCENARIO();
    ...
    function HeaderRISK_L117($object)
    {
	    $pdfObject = &$object;
			$pdfObject->Ln();
    ...
    function HeaderGRAFIEK_L117($object)
    {
	    $pdfObject = &$object;
    }

    function HeaderEND_L117($object)
    {
     	$pdfObject = &$object;
		}

    function HeaderPERFD_L117($object)
    {
      $pdfObject = &$object;
      HeaderPERF_L117($object);
      $pdfObject->SetTextColor($pdfObject->rapport_fontcolor['r'],$pdfObject->rapport_fontcolor['g'],$pdfObject->rapport_fontcolor['b']);
    }
  
    
    function HeaderTRANS_L117($object)
	  {
	    $pdfObject = &$object;

	  }
    
    function HeaderMUT_L117($object)
	  {
	    $pdfObject = &$object;

    }
    
	  function HeaderAFM_L117($object)
	  {
	    $pdfObject = &$object;
...
$pdfObject->ln(2);
	  ...
	  function HeaderINHOUD_L117($object)
	  {
	    $pdfObject = &$object;
	    //$pdfObject->headerSCENARIO();

	  }	  
 	  function HeaderPERF_L117($object)
	  {
	    $pdfObject = &$object;

	  }
    
    function HeaderINDEX_L117($object)
	  {
	    $pdfObject = &$object;
	  }
    function HeaderOIB_L117($object)
	  {
	    $pdfObject = &$object;
	  }

function HeaderOIS_L117($object)
{
  $pdfObject = &$object;
}
function HeaderCASHY_L117($object)
{
  $pdfObject = &$object;
}
function HeaderOIR_L117($object)
{
  $pdfObject = &$object;
}
function HeaderOIH_L117($object)
{
  $pdfObject = &$object;
}
function HeaderHSE_L117($object)
{
  $pdfObject = &$object;
}
  function HeaderATT_L117($object)
	{
    $pdfObject = &$object;
	}
   function HeaderPERFG_L117($object)
	{
    $pdfObject = &$object;
...
*/
  ...
  function HeaderVOLK_L117($object)
	{
    $pdfObject = &$object;
  }
  
function HeaderVHO_L117($object)
	{
    $pdfObject = &$object;
   	$pdfObject->ln();
		$pdfObject->SetFont($pdfObject->rapport_font,$pdfObject->rapport_kop_fontstyle,$pdfObject->rapport_fontsize);
...
function PieChart_L117($pdfObject,$w,$h,$data, $format, $colors=null,$titel='',$legendaStart='')
{
  ...
  $pdfObject->NbVal=count($data);
    ...
    {
      $l = vertaalTekst($l, $pdfObject->rapport_taal);
      $p=sprintf('%.2f',$val).'%';
