...
class RapportCASHY_L117
{
	function RapportCASHY_L117($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	 //
		$this->pdf = &$pdf;
  ...
  function addRegel($waarden)
  {
    ...
    $cashflowJaar[vertaalTekst('> 10 Jaar', $this->pdf->rapport_taal)]['rente']=0;
    
		foreach ($cashfow->regelsRaw as $regel)
		{
		  $jaar=substr($regel['0'],6,4);
 		  if($jaar > ($rapJaar+9))
	      $jaar=vertaalTekst('> 10 Jaar', $this->pdf->rapport_taal);
	...
	function VBarDiagram($w, $h, $data,$titel)
  {
      ...
      $colors=array('rente'=>array(0,76,76),
      'lossing'=>array(0,146,134));
