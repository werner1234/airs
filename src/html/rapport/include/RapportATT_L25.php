...
$Log: RapportATT_L25.php,v $
Revision 1.6  2020/02/19 15:02:02  rvv
*** empty log message ***

Revision 1.5  2017/01/15 08:01:57  rvv
*** empty log message ***
...
Revision 1.2  2013/12/11 17:13:16  rvv
*** empty log message ***

Revision 1.1  2013/12/07 17:51:24  rvv
*** empty log message ***
...
class RapportATT_L25
{
	function RapportATT_L25($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
     ...
     //   $this->pdf->CellBorders = array('','US','US','US','US','US','US','US','US','US','US','US');
       ...
       //$this->pdf->SetFillColor(230,230,230);
        ...
        $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
		  ...
		  {
		    $yCorrectie=-15;
        $this->pdf->SetXY(8,112+$yCorrectie);//104
        ...
        $this->pdf->SetTextColor($this->pdf->rapport_titel_fontcolor[0],$this->pdf->rapport_titel_fontcolor[1],$this->pdf->rapport_titel_fontcolor[2]);
  		  ...
  		  $this->pdf->SetXY(15,117+$yCorrectie)		;//112
        $valX = $this->pdf->GetX();
        $valY = $this->pdf->GetY();
        //function LineDiagram($w, $h, $data, $color=null, $maxVal=0, $minVal=0, $horDiv=4, $verDiv=4,$jaar=0)
        $this->LineDiagram(270, 30+35, $grafiekData,$this->pdf->rapport_grafiek_color,0,0,6,5,1);//50
...
function LineDiagram($w, $h, $data, $color=null, $maxVal=0, $minVal=0, $horDiv=4, $verDiv=4,$jaar=0)
  {
    ...
    if($color == null)
      $color=$this->pdf->rapport_titel_fontcolor;//array(184,134,17);
    ...
    {
      $skipNull = true;
      $this->pdf->Line($XDiag, $i, $XPage+$w+$margin ,$i,array('dash' => 1,'color'=>array(0,0,0)));
    ...
    for ($i=0; $i<count($data); $i++)
    {
      ...
      if($i <> 0)
        $extrax1=($unit*0.1*-1);
          ...
          $this->pdf->TextWithRotation($XDiag+($i)*$unit-10+$unit,$YDiag+$hDiag+8,$maanden[date("n",db2jul($legendDatum[$i]))].'-'.date("Y",db2jul($legendDatum[$i])),45);
