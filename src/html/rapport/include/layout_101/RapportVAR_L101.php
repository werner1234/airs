...
class RapportVAR_L101
{
	function RapportVAR_L101($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function formatGetalKoers($waarde, $dec , $start = false)
    ...
    foreach ($kleuren['Rating'] as $rating=>$waarde)
    {
      ...
      if($waarde['R']['value']<>0||$waarde['G']['value']<>0||$waarde['B']['value']<>0)
      {
        $this->standaardKleurenKort[$i] = array($waarde['R']['value'], $waarde['G']['value'], $waarde['B']['value']);
        $i++;
      }
    }
...
ORDER BY  Afdrukvolgorde";
  ...
  //  arsort ($ratingData);
    ...
    PieChart_L101($this->pdf,50,50,$this->ratingGrafiek,'%l',$ratingGrafiekKleuren,vertaalTekst('Rating',$this->pdf->rapport_taal),$legendaStart);
...
ORDER BY TijdelijkeRapportage.beleggingscategorieVolgorde,TijdelijkeRapportage.fondsOmschrijving,TijdelijkeRapportage.rekening";
  ...
  //  arsort ($durationChart);
    ...
    */
    ...
    PieChart_L101($this->pdf,50,50,$durationChartFiltered,'%l',$durationChartKleuren,vertaalTekst('Duration',$this->pdf->rapport_taal),$legendaStart);
