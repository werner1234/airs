...
class RapportKERNZ_L101
{
	function RapportKERNZ_L101($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	 // $this->perfg = new RapportPERFG_L101($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum);
		...
		$this->pdf->rapport_titel = "Verloop van totaal vermogen";//Onderverdeling in beleggingscategorie";
	...
	function portefeuilleWaarden($portefeuille)
  {
    ...
    foreach($gegevens as $waarde)
    {
      ...
      $portefeuilleWaarden['valutaverdeling'][$waarde['valuta']]+=$waarde['actuelePortefeuilleWaardeEuro'];
    }
   ...
   // echo $portefeuilleAantal."<br>\n";listarray($portefeuilleWaarden);exit;
    ...
    foreach($portefeuilleWaarden as $portefeuille=>$belCatData)
    {
      ...
      if(1)//count($this->pdf->portefeuilles) < 60)
      {
        ...
        $this->pdf->Cell(37+$extraW, 6, vertaalTekst("Beheerder", $this->pdf->rapport_taal), 0, 0, "L",1);
        ...
        foreach ($portefeuilleWaarden as $portefeuille=>$pdata)
        {
          ...
          $this->pdf->SetX($this->pdf->marge);
          $this->pdf->Cell(37+$extraW, 4, $this->naamPerPortefeuille[$portefeuille], 0, 0, "L",0);
          foreach($categorieVolgorde as $categorie=>$veld)
          {
...
//      $huidigeJaarGrafiek[$data['datum']]['performance']=$data['performance'];
    ...
    */
    ...
    if(is_array($this->pdf->portefeuilles))
    {
      ...
      while($portefeuille = $DB->NextRecord())
      {
        ...
        $portefeuilleWaarden[$portefeuille['Portefeuille']]=$this->portefeuilleWaarden($portefeuille['Portefeuille']);
      ...
      $this->pdf->setXY(150,150);
      //$this->VBarDiagram2(120, 60, $jaarGrafiek, true,true,$kleurPerPortefeuille);
      $this->VBarDiagram2(120,60,$huidigeJaarGrafiek,false,false,$kleurPerPortefeuille);
    }
  ...
  function VBarDiagram2($w, $h, $data,$metLijn=false,$jaren=false,$colors)
  {
    ...
    foreach($data as $maand=>$maandData)
    {
      ...
      {
        if($waarde>0)
          $maandMaxTotaal+=$waarde;
        else
          $maandMinTotaal+=$waarde;
        if($maandMaxTotaal > $maxVal)
          $maxVal = $maandMaxTotaal;
        if($maandMinTotaal < $minVal)
          $minVal = $maandMinTotaal;
      }
    }
    if($metLijn==true)
  ...
  function clientTotaal($data,$client,$extraW)
  {
    ...
    $this->pdf->SetFont($this->pdf->rapport_font,'B', $this->pdf->rapport_fontsize);
    $this->pdf->Cell(37+$extraW, 6,vertaalTekst('Totaal',$this->pdf->rapport_taal).' '.$client, 0, 0, "L",0);
...
//listarray($colorArray);listarray($data);
  ...
  function PieChart( $w, $h, $data, $format, $colors = null, $legendaLocatie=null)
  {
