...
class RapportVOLK_L101
{
	function RapportVOLK_L101($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//echo $subquery."<br><br>";exit;
			...
			while ($subdata = $DB2->NextRecord())
			{
				...
				//$this->pdf->Cell($this->pdf->widthB[1],4,,null,null,null,null,null);
				//$this->pdf->setX($this->pdf->marge);
				$this->pdf->SetTextColor($this->pdf->rapport_fontcolor['r'], $this->pdf->rapport_fontcolor['g'], $this->pdf->rapport_fontcolor['b']);
				$this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
				
				$this->pdf->row(array("",
													$this->cutString($subdata['fondsOmschrijving'],$this->pdf->widths[1]),
...
//rvv
  ...
  */
				...
				$this->pdf->row(array("",
													...
													$this->cutString($omschrijving,$this->pdf->widths[1]),
													"",
													"",
													"",
													"",
	...
	function cutString($omschrijving, $width)
	{
		  $stringWidth=$this->pdf->GetStringWidth($omschrijving);
      if($stringWidth >= $width-2)
      {
        $newString='';
        $omschrijvingRuimte=$width-$this->pdf->GetStringWidth('...')-2;
