...
$Log: RapportPERFG_L101.php,v $
Revision 1.10  2019/11/06 16:11:20  rvv
*** empty log message ***

Revision 1.9  2017/10/25 15:59:31  rvv
*** empty log message ***

Revision 1.8  2017/07/22 18:22:05  rvv
*** empty log message ***

Revision 1.7  2017/07/05 16:06:40  rvv
*** empty log message ***

Revision 1.6  2017/06/18 09:18:24  rvv
*** empty log message ***

Revision 1.5  2017/06/10 18:09:58  rvv
*** empty log message ***

Revision 1.4  2017/05/25 14:35:58  rvv
*** empty log message ***

Revision 1.3  2017/05/17 15:57:50  rvv
*** empty log message ***
...
include_once($__appvar["basedir"]."/html/rapport/include/layout_101/RapportPERF_L101.php");

class RapportPERFG_L101
{
	function RapportPERFG_L101($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
    $this->kernz=new RapportKERNZ_L101($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum);
    $this->perf=new RapportPERF_L101($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum);
    ...
    $this->pdf->templateVarsOmschrijving[$this->pdf->rapport_type.'Paginas']=$this->pdf->rapport_titel;
	  ...
	  $this->pdf->setXY(240,80);
	 // $this->benchmarkVergelijking();return;
    $this->perfwaarden['Periode']=$this->perf->att->bereken($this->rapportageDatumVanaf,$this->rapportageDatum,'','categorie');
    ...
    foreach($allekleuren['DEP'] as $depotbank=>$kleur)
      $this->depotbankKleuren[$depotbank]=array($kleur['R']['value'],$kleur['G']['value'],$kleur['B']['value']);
  ...
  foreach ($indexData as $i=>$data)
  {
    ...
    {
      $huidigeJaarRendementen[$portefeuille] = $index->getWaarden($this->pdf->portefeuilledata['Startdatum']  ,$this->rapportageDatum ,$portefeuille,'','jaar');
      $laatsteDatum='';
      foreach ($huidigeJaarRendementen[$portefeuille] as $i=>$data)
      {
        $aandeel=$data['waardeHuidige']/$cumulatieveWaarde[$data['datum']];
  
        $jaarGrafiek[$data['datum']][$portefeuille]=$data['performance']*$aandeel;
        $jaarGrafiek[$data['datum']][$portefeuille.'Cumu']=((1+$jaarGrafiek[$laatsteDatum][$portefeuille.'Cumu']/100) * (1+$data['performance']*$aandeel/100)-1) * 100;
        $laatsteDatum=$data['datum'];
    ...
    if(is_array($this->pdf->portefeuilles))
    {
      ...
      while($portefeuille = $DB->NextRecord())
      {
        if($portefeuille['selectieveld1']<>'')
          ...
          $this->clientVermogensbeheerder[$portefeuille['Portefeuille']]=$portefeuille['Depotbank']." - ".$portefeuille['Portefeuille'];//$portefeuille['depotbankOmschrijving'];
        $portefeuilleWaarden[$portefeuille['Portefeuille']]=$this->kernz->portefeuilleWaarden($portefeuille['Portefeuille']);
        ...
        $vertalingen[$portefeuille['depotbankOmschrijving']]=$portefeuille['Depotbank'];
      }
    }
    else
    {
      $query="SELECT Portefeuilles.Portefeuille,Portefeuilles.ClientVermogensbeheerder,Portefeuilles.selectieveld1,Portefeuilles.Depotbank, Depotbanken.Omschrijving as depotbankOmschrijving FROM Portefeuilles JOIN Depotbanken on Portefeuilles.Depotbank=Depotbanken.Depotbank WHERE Portefeuilles.Portefeuille = '".$this->portefeuille."'";
      $DB->SQL($query);
      $DB->Query();
      $portefeuille = $DB->NextRecord();
      if($portefeuille['selectieveld1']<>'')
        $this->clientVermogensbeheerder[$this->portefeuille]=$portefeuille['selectieveld1'];
      else
      {
        $this->clientVermogensbeheerder[$this->portefeuille] = $portefeuille['depotbankOmschrijving'];
        $vertalingen[$portefeuille['depotbankOmschrijving']]=$portefeuille['Depotbank'];
      }
      $portefeuilleWaarden[$this->portefeuille]=$this->kernz->portefeuilleWaarden($this->portefeuille);
    }
    ...
    {
      /*
      foreach($pData['valutaverdeling'] as $valuta=>$waarde)
      {
        $valutaVerdeling[$valuta]+=$waarde/$totaalWaarde;
      }
      */
      
      if($this->clientVermogensbeheerder[$portefeuille]<>'')
      {
        $naam=$this->clientVermogensbeheerder[$portefeuille];
      }
      else
      {
        $naam=$portefeuille;
      ...
      $percentage=$pData['rendementProcent'];
      if($percentage<0)
      {
        $barGraph = true;
      }
      ...
      if($kleur[0]==0 && $kleur[1]==0 && $kleur[2]==0)
        $kleur = $this->depotbankKleuren[$vertalingen[$naam]];
      ...
      $aandeel=$pData['totaleWaarde']/$cumulatieveWaarde[$this->rapportageDatum];
      
      $categorieVerdeling['percentage'][$naam]=$percentage;//*$aandeel;
      $categorieVerdeling['kleur'][]=$kleur;
      $categorieVerdeling['kleurBar'][$naam]=$kleur;
      $lastMonth[$portefeuille]['kleur']=$kleur;
      $kleurPerPortefeuille[$portefeuille]=$kleur;
    ...
    $this->pdf->SetWidths(array(93,93,93));
    $this->pdf->SetFont($this->pdf->rapport_font,'b',$this->pdf->rapport_fontsize+4);
    $this->pdf->row(array(vertaalTekst("Rendementsverdeling",$this->pdf->rapport_taal),vertaalTekst("Rendementsverdeling",$this->pdf->rapport_taal),vertaalTekst("Rendementsverdeling vergelijking",$this->pdf->rapport_taal)));
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
    ...
    if($barGraph==false)
    {
      $this->pdf->setXY(100,$grafiekY);
      $this->kernz->PieChart(50, 50, $categorieVerdeling['percentage'], '%l (%p)',$categorieVerdeling['kleur']);
    }
    else
    {
      $this->pdf->setXY(130,$grafiekY);
      $this->kernz->BarDiagram(50, $grafiekH, $categorieVerdeling['percentage'], '%l (%p)',$categorieVerdeling['kleurBar'],'');//"Portefeuillewaarde € ".$this->formatGetal($this->portTotaal[$this->rapportageDatum],2)
    }
    
    
    $i=0;
    $barGraph=false;
    foreach($this->perfwaarden['Periode'] as $cat=>$rendementWaarden)
    {
      if($cat=='totaal')
        continue;
     // $rendementen[$cat]=;
      ...
      if($kleur[0]==0 && $kleur[1]==0 && $kleur[2]==0)
        $kleur = array(rand(0, 255), rand(0, 255), rand(0, 255));
      if($rendementWaarden['procent']<0)
        $barGraph=true;

      $rendementVerdeling['percentage'][$this->perf->att->categorien[$cat]]=$rendementWaarden['procent'];
      $rendementVerdeling['kleur'][]=array($this->categorieKleuren[$cat]['R']['value'],$this->categorieKleuren[$cat]['G']['value'],$this->categorieKleuren[$cat]['B']['value']);
      $rendementVerdeling['kleurBar'][$this->perf->att->categorien[$cat]]=array($this->categorieKleuren[$cat]['R']['value'],$this->categorieKleuren[$cat]['G']['value'],$this->categorieKleuren[$cat]['B']['value']);
      $i++;
    }
    ...
    if($barGraph==false)
    {
      $this->pdf->setXY(15,$grafiekY);
      $this->kernz->PieChart(50, 50, $rendementVerdeling['percentage'], '%l (%p)',$rendementVerdeling['kleur']);
    }
    else
    {
      $this->pdf->setXY(30,$grafiekY);
      $this->kernz->BarDiagram(60, $grafiekH, $rendementVerdeling['percentage'], '%l (%p)',$rendementVerdeling['kleurBar'],'');//"Portefeuillewaarde € ".$this->formatGetal($this->portTotaal[$this->rapportageDatum],2)
    }
    
    $this->pdf->setXY(240,$grafiekY+35);
    $this->benchmarkVergelijking($lastMonth);
    ...
    if (count($barGraph) > 0)
        ...
        if($meerjaren==true)
	      ...
	      $this->pdf->SetXY(155,180)		;//112
        if($meerjaren==true)
        {
          $this->VBarDiagram2(120, 60, $jaarGrafiek, true,true,$kleurPerPortefeuille);
        }
        else
        {
    ...
    unset($this->pdf->skipHeader);
   //$this->paginaTwee($rendamentWaarden);

	}
	
	function benchmarkVergelijking($lastMonth)
  {
  //  listarray($lastMonth);exit;
   // $lastMonth=array('benchmark'=>1.3,'521076247575'=>8.29,'676-4752001-34'=>6.85);
    $plotWaarden=array();
    foreach ($lastMonth as $key=>$value)
      $plotWaarden[$key]=$value['performance']-$lastMonth['benchmark']['performance'];
    $max=max(abs(max(array_values($plotWaarden))),abs(min(array_values($plotWaarden))))*2;
  
    $this->pdf->setLineStyle(array('width'=>0.1,'color'=>array(0,0,0)));
    
    $xcenter=$this->pdf->getX();
    $ycenter=$this->pdf->getY();
    $radius=30;
    $this->pdf->Sector($this->pdf->getX(), $this->pdf->getY(), $radius, 0, 180,'D', false, 0);
    $this->pdf->circle($this->pdf->getX(),$this->pdf->getY(),1,0,360,'DF',array('color'=>array(0,0,0)));
  
  
    $percentages=array('start'=>0,'stop'=>$max*2);
    foreach($plotWaarden as $key=>$value)
      $percentages[$key]=$max+$value;
    $steps=$max*2;
    $step=M_PI/$steps;
    foreach($percentages as $key=>$percentage)
    {
      $pstep=$percentage/$steps*$max*2;
      $this->pdf->setXY($xcenter+cos($step*$pstep+M_PI)*1.15*$radius,
                        $ycenter+sin($step*$pstep+M_PI)*1.15*$radius);
      $this->pdf->Cell(1,0,$this->formatGetal($percentage-$max,1).'%',0,0,'C');
  
      if($key<>'start' && $key <> 'stop')
      {
        if(isset($lastMonth[$key]['kleur']))
          $kleur=$lastMonth[$key]['kleur'];
        else
          $kleur=array(50,50,100);
      // listarray($lastMonth);
        $this->pdf->Line($xcenter,
                         $ycenter,
                         $xcenter + cos($step * $pstep + M_PI) * 1 * $radius,
                         $ycenter + sin($step * $pstep + M_PI) * 1 * $radius, array('color' => $kleur, 'width' => 1, 'cap' => 'round'));
      }
      $this->pdf->setLineStyle(array('width'=>0.1,'color'=>array(0,0,0)));
    }
    
   // listarray($plotWaarden);
   // echo $max;
   // exit;
  }

  function VBarDiagram2($w, $h, $data,$metLijn=false,$jaren=false,$colors)
  {
    global $__appvar;
    ...
    foreach($data as $periode=>$maandData)
    {
      ...
      foreach($maandData as $type=>$val)
      {
        ...
        $hval = ($val * $unit);
        /*
        if(substr($type,0,11)!='performance' && substr($type,0,9)!='benchmark' )
        {
         
          if($val>0)
          {
            
            $this->pdf->Rect($xval, $yvalPos, $lval, $hval, 'DF',null, $color);
            $yvalPos = $yvalPos + $hval;
          }
          else
          {
            $this->pdf->Rect($xval, $yvalNeg, $lval, $hval, 'DF',null, $color);
            $yvalNeg = $yvalNeg + $hval;
          }
        }
        */
        /*
    ...
    foreach($legenda as $type=>$kleur)
    {
      ...
      $this->pdf->Rect($XstartGrafiek+10+$extraX, $extraY+$YstartGrafiek+8+($n*4), 2, 2, 'F',null,$kleur);
...
*/
	...
	function paginaTwee($rendamentWaarden)
  {
    if(count($rendamentWaarden) > 0)
    {
      ...
      foreach ($rendamentWaarden as $row)
      {
        ...
        $totalen['totaalWaarde'] = $row['waardeHuidige'];
        $totalen['totaalResultaat'] += $row['resultaatVerslagperiode'];
        $totalen['totaalGerealiseerd'] += $row['gerealiseerd'];
        $totalen['totaalOngerealiseerd'] += $row['ongerealiseerd'];
        $totalen['totaalOpbrengsten'] += $row['opbrengsten'];
        $totalen['totaalKosten'] += $row['kosten'];
        $totalen['totaalRente'] += $row['rente'];
        $totalen['totaalStortingenOntrekkingen'] += $row['stortingen']-$row['onttrekkingen'];
        $totalen['totaalRendament'] = $row['index'];
      ...
      $this->pdf->row(array(vertaalTekst('Totaal',$this->pdf->rapport_taal),
                        ...
                        $this->formatGetal($totalen['totaalStortingenOntrekkingen'],0),
                        $this->formatGetal($totalen['totaalGerealiseerd']+$totalen['totaalOngerealiseerd'],0),
                        $this->formatGetal($totalen['totaalOpbrengsten'] ,0),
                        $this->formatGetal($totalen['totaalKosten'],0),
                        $this->formatGetal($totalen['totaalRente'] ,0),
                        $this->formatGetal($totalen['totaalResultaat'],0),
                        $this->formatGetal($totalen['totaalWaarde'],0),
                        '',
