...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2019/05/15 15:32:37 $
File Versie					: $Revision: 1.5 $

$Log: RapportVKMD_L39.php,v $
Revision 1.5  2019/05/15 15:32:37  rvv
*** empty log message ***

Revision 1.4  2019/05/11 16:48:39  rvv
*** empty log message ***
...
class RapportVKMD_L39
{
	function RapportVKMD_L39($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
			...
			$this->pdf->rapport_titel = vertaalTekst("Vergelijkende kostenmaatstaf",$this->pdf->rapport_taal).' (euro)';
			$this->pdfVullen=true;
    ...
    if($this->pdfVullen==true)
		{
	  	...
	  	$this->pdf->row(array(vertaalTekst('Doorlopende kosten',$this->pdf->rapport_taal), 'euro '.$this->formatGetal($totaalDoorlopendekosten, 0) ));
		...
		if($this->pdfVullen==true)
		{
			...
			$this->pdf->ln();
			$this->pdf->excelData[]=array();
			$this->pdf->row(array(vertaalTekst('Gemiddeld vermogen',$this->pdf->rapport_taal), 'euro '.$this->formatGetal($gemiddelde, 0)));
			...
			$this->pdf->setWidths(array(28+50+20-2,20,20+2,20,20,18,18,18,18,18,15));
			$this->pdf->row(array(vertaalTekst('Directe kosten vanaf',$this->pdf->rapport_taal).' ' . date('d-m-Y', db2jul($this->vanafDatum)), 'euro', vertaalTekst('percentage',$this->pdf->rapport_taal)));
			$this->pdf->excelData[]=array('','Directe kosten vanaf ' . date('d-m-Y', db2jul($this->vanafDatum)),'', 'euro', 'percentage');
			$this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
		}
		$totaal=0;
