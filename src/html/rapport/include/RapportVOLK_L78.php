...
$Log: RapportVOLK_L78.php,v $
Revision 1.5  2018/10/20 18:05:20  rvv
*** empty log message ***

Revision 1.4  2018/07/07 17:35:19  rvv
*** empty log message ***

Revision 1.3  2018/06/24 11:13:16  rvv
*** empty log message ***
...
class RapportVOLK_L78
{
	function RapportVOLK_L78($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  //F $procentResultaat);
			...
			if($totaalA <>0)
				$this->printCol(6,$this->formatGetal($totaalA,$this->pdf->rapport_VOLK_decimaal,true),"subtotaal");
			if($totaalC <>0)
	...
	function printKop($title, $type="default")
	{
		...
		if($this->pdf->getY()+30>$this->pdf->PageBreakTrigger)
		  $this->pdf->addPage();
  ...
  function getDividend($fonds)
  {
	...
	function checkPage($aantalRegels)
	{
		$nodigeRuimte=($aantalRegels+3)*$this->pdf->rowHeight;
		$beschikbareRuimte=$this->pdf->PageBreakTrigger-$this->pdf->getY();
		if($aantalRegels < 3 &&  $nodigeRuimte>$beschikbareRuimte )
		{
			$this->pdf->addPage();
		}
		return $aantalRegels-1;
	}
	...
	function writeRapport()
	{
		...
		$this->pdf->widthB = array(5,55,18,12,18,20,21,17,21,21,13,17,17,17,11);
		$this->pdf->widthA = array(  60,18,12,18,20,21,17,21,21,13,17,17,17,11);
...
//echo $subquery."<br><br>";exit;
      $n=0;
      $kopPrinted=false;
			$aantalRegels=$DB2->records();
			while($subdata = $DB2->NextRecord())
			{
				$aantalRegels=$this->checkPage($aantalRegels);
...
//rvv
		...
		" (TijdelijkeRapportage.actuelePortefeuilleWaardeInValuta) subtotaalValuta, ".
		" (TijdelijkeRapportage.beginPortefeuilleWaardeEuro) / ".$this->pdf->ValutaKoersBegin. " as subtotaalbegin, ".
		" (TijdelijkeRapportage.actuelePortefeuilleWaardeEuro) / ".$this->pdf->ValutaKoersEind. " as subtotaalactueel FROM ".
		" TijdelijkeRapportage ".
    ...
    " TijdelijkeRapportage.beleggingscategorie =  '".$categorien['beleggingscategorie']."' AND ".
		...
		if($DB2->records() > 0)
		{
			...
			while($rente = $DB2->NextRecord())
			{
				...
				$percentageVanTotaaltxt=$this->formatGetal($rente['subtotaalactueel']/$totaalWaarde*100,2)." %";
				$this->pdf->SetFont($this->pdf->rapport_font,$this->pdf->rapport_fontstyle,$this->pdf->rapport_fontsize);
				$this->pdf->SetTextColor($this->pdf->rapport_fonds_fontcolor[r],$this->pdf->rapport_fonds_fontcolor[g],$this->pdf->rapport_fonds_fontcolor[b]);
				$this->pdf->setX($this->pdf->marge);
				$this->pdf->Cell($this->pdf->widthB[0],4,"");
				$this->pdf->Cell($this->pdf->widthB[1],4,'    '.$rente['fondsOmschrijving'],null,null,null,null,null);
				$this->pdf->setX($this->pdf->marge);
				$this->pdf->SetTextColor($this->pdf->rapport_fontcolor[r],$this->pdf->rapport_fontcolor[g],$this->pdf->rapport_fontcolor[b]);
				$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
				$this->pdf->row(array("",
													...
													'',
													'',
													$this->formatGetal($rente['subtotaalactueel'],$this->pdf->rapport_VOLK_decimaal),
													$percentageVanTotaaltxt));
			}
...
//rvv
      ...
      $grandtotaaldividendCorrected  += $subtotaal['totaalDividendCorrected'];
        ...
        $this->pdf->AddPage();
			...
			$aantalRegels=count($liqiteitenBuffer);
