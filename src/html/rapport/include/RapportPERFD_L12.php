...
class RapportPERFD_L12
{

  function RapportPERFD_L12($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	  global $__appvar;
		$this->pdf = &$pdf;
...
$datumStop  = db2jul($eind);
    ...
    $this->tmp=array();
    $this->waarden=array();
    $this->tmp['rapportagePeriode']=$this->berekening->bereken($start,$this->rapportageDatum,'attributie');
...
$this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
    $toonenVanaf=0;
   // echo "$toonenVanaf=$laatsteJaar-12;"exit;
...
//listarray($indexWaardenGrafiek);
 ...
 if(($laatsteJaar-$eersteJaar) > 11 )
 {
   $this->pdf->addPage();
   $grafiekYCorrectie=-70;
 }
 else
  $grafiekYCorrectie=0;


$this->LineDiagram(18,120+$grafiekYCorrectie,110,50,$indexWaardenGrafiek,'Totaal resultaat');


$this->VBarDiagram(160,170+$grafiekYCorrectie,123,50,$barData,'Ontwikkeling vermogen');
