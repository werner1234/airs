...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: RapportRISK_L57.php,v $
 		Revision 1.3  2020/05/20 17:13:48  rvv
 		*** empty log message ***
 		...
 		*** empty log message ***
 		...
 		Revision 1.6  2019/08/21 15:32:46  rvv
 		*** empty log message ***
...
class RapportRISK_L57
{
	function RapportRISK_L57($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
FROM HistorischePortefeuilleIndex WHERE HistorischePortefeuilleIndex.Portefeuille='$cPortefeuille' AND HistorischePortefeuilleIndex.Datum='$dag' AND HistorischePortefeuilleIndex.periode='m'";
   ...
   // listarray($stdev);
    ...
    $this->rechtsOnder($riskData,$riskBenchmark,$stdev);
...
function rechtsOnder($riskData,$riskBenchmark,$stdev)
{
  global $__appvar;
  $DB=new DB();
  ...
  if(count($riskData)==0)
    $riskData['standaarddeviatieAFM']=$stdev->uitvoer['afm'];
...
function addStdevGrafieken($stdev)
{
  ...
  if(count($stdev->standaardDeviatieReeksen['totaal'])>0)
  {
  ...
  {
    foreach ($stdev->standaardDeviatieReeksen['afm'] as $datum => $devData)
    {
      $benchmarkData = $stdev->standaardDeviatieReeksen['benchmarkTot'][$datum];
      $afmData = $stdev->standaardDeviatieReeksen['afm'][$datum];
