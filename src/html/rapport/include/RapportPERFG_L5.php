...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2014/11/01 22:05:57 $
File Versie					: $Revision: 1.4 $

$Log: RapportPERFG_L5.php,v $
...
class RapportPERFG_L5
{
	function RapportPERFG_L5($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function VBarDiagram($w, $h, $data)
  {
      ...
      foreach (array_reverse($this->categorieVolgorde) as $categorie)
      {
        if(is_array($grafiekCategorie[$categorie]))
        {
          ...
          $this->pdf->SetXY($XstartGrafiek+$bGrafiek+8 ,$YstartGrafiek-$hGrafiek+$n*8+1.5 );
