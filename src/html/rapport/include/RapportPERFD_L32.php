...
class RapportPERFD_L32
{
	function RapportPERFD_L32($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
   ...
   if(count($rendamentWaarden) > 0)
   {
     ...
     //   $this->pdf->CellBorders = array('','US','US','US','US','US','US','US','US','US','US','US');
       ...
       // $this->pdf->SetFillColor($this->pdf->rapport_kop_bgcolor['r']*1.2,$this->pdf->rapport_kop_bgcolor['g']*1.2,$this->pdf->rapport_kop_bgcolor['b']*1.2);
        ...
        $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
        $this->pdf->Cell(146, 5, vertaalTekst('Vermogensverdeling in EUR',$this->pdf->rapport_taal), 0, 0);
	    	$this->pdf->Cell(100, 5, vertaalTekst('Vermogensverdeling in %',$this->pdf->rapport_taal), 0, 1);
  		    $this->pdf->Line($this->pdf->marge, $this->pdf->GetY(),$this->pdf->marge+277,$this->pdf->GetY());
	      $this->pdf->SetXY(163,180)		;//112
	      $this->VBarDiagram(80, 60, $barGraph['Index']);
		      $this->pdf->SetXY(23,180)		;//112
