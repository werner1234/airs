...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2015/03/18 16:29:56 $
File Versie					: $Revision: 1.8 $

$Log: RapportVOLK_L21.php,v $
Revision 1.8  2015/03/18 16:29:56  rvv
*** empty log message ***

Revision 1.7  2015/03/14 17:01:49  rvv
*** empty log message ***

Revision 1.6  2015/03/08 09:47:28  rvv
*** empty log message ***

Revision 1.5  2015/02/04 16:08:06  rvv
*** empty log message ***

Revision 1.4  2014/10/11 16:24:58  rvv
*** empty log message ***

Revision 1.3  2014/09/17 15:16:31  rvv
*** empty log message ***

Revision 1.2  2014/06/08 15:27:58  rvv
*** empty log message ***

Revision 1.1  2014/06/08 07:56:13  rvv
*** empty log message ***
...
class RapportVOLK_L21
{
	function RapportVOLK_L21($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	...
	function writeRapport()
	{
      ...
      if($this->pdf->rapport_layout == 8) //rvv
		...
		// voor kopjes
		//	$this->pdf->widthA = array(60,18,8,15,21,21,1,15,21,21,1,15,1,1,15);
...
//echo $subquery."<br><br>";exit;
		...
		if($DB1->records() > 0)
		{
			...
			foreach($liqiteitenBuffer as $data)
			{
				...
				else if($this->pdf->rapport_layout == 7 || $this->pdf->rapport_layout == 24 || $this->pdf->rapport_layout == 30 || $this->pdf->rapport_layout == 31 || $this->pdf->rapport_layout == 29)
				{
				...
				{
					if($this->pdf->rapport_layout == 7 || $this->pdf->rapport_layout == 14 || $this->pdf->rapport_layout == 24 || $this->pdf->rapport_layout == 30 || $this->pdf->rapport_layout == 31 || $this->pdf->rapport_layout == 29)
					{
						$this->pdf->row(array("",
												"",
												"",
    ...
    $begin=db2jul($this->pdf->PortefeuilleStartdatum);

    $query="SELECT min(Datum) as eerste FROM HistorischePortefeuilleIndex WHERE portefeuille='".$this->portefeuille."' AND periode='m'";
		$DB->SQL($query); 
  	$index=$DB->lookupRecord();

    if($index['eerste'] <> '')
    {
      $index['eerste']=substr($index['eerste'],0,7).'-01';
      $indexBegin=db2jul($index['eerste']);
      if($indexBegin < $begin)
        $begin=$indexBegin;
    }
    
  $this->pdf->SetY($stopY-18);  
  $index=new indexHerberekening();
  $dagen=(db2jul($this->rapportageDatum)-$begin)/86400;
  if($dagen < 365)
  {
    $perioden='geenData';
    $yearCount=0;
...
//  }
  ...
  if($yearCount >0)
  {
    $indexData = $index->getWaarden(date('Y-m-d',$begin) ,$this->rapportageDatum ,$this->portefeuille,'',$perioden);
    $buffer=array();
    $bufferIndex=array();
    $bufferStartIndex=1;
    $indexCounter=0;
    $this->pdf->excelData[0]=array('Datum','Perf','Standaarddeviatie');  
    foreach($indexData as $index=>$perf)
    {
        $buffer[$index]=100+$perf['performance'];
        $perfCumArray[]=$perf['index'];
        $this->pdf->excelData[$indexCounter+1][0] = $perf['datum'];  
        $this->pdf->excelData[$indexCounter+1][1] = 100+$perf['performance'];     
        if(count($buffer)==12)
        {
          $stdev=standard_deviation($buffer)*sqrt($yearCount);
          $standaardDeviatieReeksen[]=$stdev;
          $this->pdf->excelData[$indexCounter+1][2] = $stdev;     
    ...
    foreach($perfCumArray as $index=>$waarde)
    {
      ...
      $maxDrawdownArray[$index]=$max-$min;
    }
    ...
    $stdev=array_sum($standaardDeviatieReeksen)/count($standaardDeviatieReeksen);
    $this->pdf->excelData[]=array('','gemiddelde',$stdev);   
    $this->pdf->Rect($this->pdf->marge+140,$this->pdf->getY()+2,80,12);
    $this->pdf->SetWidths(array(140,65,15));
    $this->pdf->SetAligns(array('L','L','R'));
    $this->pdf->Ln(4);
    $this->pdf->Row(array('','Standaarddeviatie gecorrigeerd voor de trend',$this->formatGetal($stdev,1).'%'));
    $this->pdf->Row(array('','Drawdown',$this->formatGetal($maxDrawdown,1).'%'));

  }
  else
  {
     $this->pdf->Rect($this->pdf->marge+140,$this->pdf->getY()+2,85,32);
    $this->pdf->SetWidths(array(140,65,20));
    $this->pdf->SetAligns(array('L','L','R'));
    $this->pdf->Ln(4);
    $this->pdf->Row(array('','Standaarddeviatie gecorrigeerd voor de trend','NB')); 
    $this->pdf->Row(array('','Drawdown','NB')); 
    $this->pdf->Ln(4);  
    $this->pdf->SetWidths(array(140,65+20));
    $this->pdf->Row(array('','Voor het berekenen van de standaarddeviatie en drawdown van uw portefeuille is minimaal een periode van 12 maanden noodzakelijk. Zodra die periode is bereikt treft u de standaarddeviatie en drawdown op uw kwartaalrapportages aan.')); 
  }
  ...
  function printRendement($portefeuille, $rapportageDatum, $rapportageDatumVanaf)
  {
		...
		if(($this->pdf->GetY() + 26 ) >= $this->pdf->pagebreak) {
			$this->pdf->AddPage();
			$this->pdf->ln();
		}
		...
		$this->pdf->Rect($this->pdf->marge,$this->pdf->getY(),110,(16),'F');
		$this->pdf->SetFillColor(0);
		$this->pdf->Rect($this->pdf->marge,$this->pdf->getY(),110,(16));
		$this->pdf->ln(2);
		//$this->pdf->SetX($this->pdf->marge);
		$this->pdf->SetX($this->pdf->marge);
      ...
      if ($this->pdf->rapport_resultaatText)
		    $this->pdf->Cell(80,4, vertaalTekst($this->pdf->rapport_resultaatText,$this->pdf->rapport_taal), 0,0, "L");
		  else
