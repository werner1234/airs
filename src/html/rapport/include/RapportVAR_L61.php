...
$Log: RapportVAR_L61.php,v $
Revision 1.2  2019/05/05 13:04:29  rvv
*** empty log message ***

Revision 1.1  2019/05/04 18:22:49  rvv
*** empty log message ***
...
class RapportVAR_L61
{
	function RapportVAR_L61($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function formatGetalKoers($waarde, $dec , $start = false)
	...
	function writeRapport()
	{
		  ...
		  $this->vastWhere="AND (TijdelijkeRapportage.beleggingscategorie='OBL' OR Fondsen.Lossingsdatum <> '0000-00-00') AND TijdelijkeRapportage.Type <> 'rekening'  AND Fondsen.Fondssoort='OBL'";// ";
...
//echo "</table>";
    ...
    if($aantalRegels>1)
    {
      ...
      {
        $this->pdf->addPage();
        $this->toonRating(12, 40);
        $this->toonDuration(102, 40);
        $this->toonRegio(192, 40);
      }
      else
      {
        $this->toonRating(12, 120);
        $this->toonDuration(102, 120);
        $this->toonRegio(192, 120);
      }
    }
  }
...
ORDER BY  Afdrukvolgorde";
  ...
  //  arsort ($ratingData);
    ...
    PieChart_L61($this->pdf,50,50,$this->ratingGrafiek,'%l',$ratingGrafiekKleuren,vertaalTekst('Rating',$this->pdf->rapport_taal),$legendaStart);
...
ORDER BY TijdelijkeRapportage.beleggingscategorieVolgorde,TijdelijkeRapportage.fondsOmschrijving,TijdelijkeRapportage.rekening";
    ...
    */
    ...
    PieChart_L61($this->pdf,50,50,$durationChartFiltered,'%l',$durationChartKleuren,vertaalTekst('Duration',$this->pdf->rapport_taal),$legendaStart);
  
  
  }
  
  
  function toonRegio($x,$y)
  {
    $this->pdf->setXY($x,$y);
    global $__appvar;
    $DB = new DB();
    ...
    $DB->Query();
    $totaalWaarde = $DB->nextRecord();
    $regioTotaalWaarde=$totaalWaarde['totaal'];
    
    
    $query = "SELECT
SUM(TijdelijkeRapportage.actuelePortefeuilleWaardeEuro) as waardeEur ,
TijdelijkeRapportage.regio,
TijdelijkeRapportage.regioOmschrijving AS regioOmschrijving,
TijdelijkeRapportage.regiovolgorde as Afdrukvolgorde
FROM
TijdelijkeRapportage
Left Join Fondsen ON Fondsen.Fonds = TijdelijkeRapportage.Fonds
WHERE (TijdelijkeRapportage.portefeuille = '".$this->portefeuille."' ) AND TijdelijkeRapportage.rapportageDatum = '".$this->rapportageDatum."' ".$__appvar['TijdelijkeRapportageMaakUniek']. $this->vastWhere."
AND TijdelijkeRapportage.`type` NOT IN ('rekening')
...
ORDER BY  Afdrukvolgorde";
    ...
    while($rating = $DB->NextRecord())
    {
      if($rating['regioOmschrijving']=='')
        $rating['regioOmschrijving']='Geen regio';
  
      $regioData[$rating['regioOmschrijving']] +=$rating['waardeEur'];
      $ratingTotaalWaarde +=$rating['waardeEur'];
    ...
    //listarray($ratingData);
    //  arsort ($ratingData);
    foreach ($regioData as  $rating=>$waarde)
    {
      // $waarden=$regioData[$rating];
      $procent=$waarde/$regioTotaalWaarde;
      $this->regioData[$rating.' ('.$this->formatGetal($procent*100,1).' %)']=$procent*100;
      $ratingGrafiekKleuren[]=$kleuren[$i];
      $i++;
    }
    ...
    $legendaStart=$this->correctLegentHeight(count($this->regioData));
    PieChart_L61($this->pdf,50,50,$this->regioData,'%l',$ratingGrafiekKleuren,vertaalTekst('Regio',$this->pdf->rapport_taal),$legendaStart);
  ...
  {
    return array($this->pdf->GetX()+57,$this->pdf->GetY()+ 35 -($regels*4)/2);
  }
