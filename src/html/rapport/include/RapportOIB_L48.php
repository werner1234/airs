...
$Log: RapportOIB_L48.php,v $
Revision 1.9  2020/02/05 17:12:48  rvv
*** empty log message ***

Revision 1.8  2020/01/29 17:36:42  rvv
*** empty log message ***
...
class RapportOIB_L48
{
	function RapportOIB_L48($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//listarray($zpwaarde['conclusie']);
    ...
    $beginY=$this->pdf->getY();
  	...
  	$this->pdf->SetWidths(array(55,20,20,20,20));
  	$this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
    ...
    foreach ($categorieWaarden as $cat=>$percentage)
    {
     ...
     //   $max='';  
      ...
      if($tmp[$cat][5]=='Voldoet')
        $this->pdf->MemImage($this->checkImg,133,$this->pdf->getY(),3.9,3.9);
      else
        $this->pdf->MemImage($this->deleteImg,133,$this->pdf->getY(),3.9,3.9);
