...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2020/06/25 05:34:17 $
File Versie					: $Revision: 1.6 $

$Log: RapportRISK_L67.php,v $
Revision 1.6  2020/06/25 05:34:17  rvv
*** empty log message ***

Revision 1.5  2020/06/25 05:25:27  rvv
*** empty log message ***

Revision 1.4  2020/06/24 13:02:42  rvv
*** empty log message ***
...
class RapportRISK_L67
{

	function RapportRISK_L67($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
function addStdevGrafieken($stdev)
{
  ...
  function LineDiagram($w, $h, $data, $color=null, $maxVal=0, $minVal=0, $horDiv=4, $verDiv=4)
  {
    ...
    if($titel=='Portefeuille rendement')
      $vanafNul=true;
    else
      $vanafNul=false;
    ...
    if(is_array($data2))
    {
      ...
      for ($i=0; $i<count($data2); $i++)
      {
        ...
        if (($i>0||$vanafNul==true) && $data2[$i] <> 'F' && $lastValue <> 'F')
        {
          $this->pdf->line($XDiag+$i*$unit, $yval, $XDiag+($i+1)*$unit, $yval2,$lineStyle );
        }
        $yval = $yval2;
