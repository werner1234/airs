...
$Log: RapportVOLK_L52.php,v $
Revision 1.10  2019/04/10 15:50:36  rvv
*** empty log message ***

Revision 1.9  2019/04/07 11:07:21  rvv
*** empty log message ***
...
class RapportVOLK_L52
{
	function RapportVOLK_L52($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    //  ".
		...
		while($categorien = $DB->NextRecord())
		{
           ...
           $procentResultaat = (($totaalactueel - $totaalbegin + $totaaldividendCorrected) / ($totaalbegin /100));
			...
			if($categorien['beleggingscategorie']=='ZAK')
				$extraOrder="TijdelijkeRapportage.regioVolgorde,";
        ...
        " TijdelijkeRapportage.RegioOmschrijving, ".
			//	" TijdelijkeRapportage.beginPortefeuilleWaardeEuro / $beginQuery as beginPortefeuilleWaardeEuro, ".
				...
				.$__appvar['TijdelijkeRapportageMaakUniek'].
				" ORDER BY TijdelijkeRapportage.Lossingsdatum, $extraOrder TijdelijkeRapportage.fondsOmschrijving asc";//exit;
...
//echo $subquery."<br><br>";exit;
      ...
      $kopPrinted=false;
        ...
        if($lastRegio <> $subdata['Regio'] && $categorien['beleggingscategorie']=='ZAK')
				{
          $this->pdf->SetWidths($this->pdf->widthA);
          $this->pdf->SetAligns($this->pdf->alignA);
          
          $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
          $this->pdf->row(array("  ".$subdata['RegioOmschrijving']));
