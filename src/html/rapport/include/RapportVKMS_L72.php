...
$Log: RapportVKMS_L72.php,v $
Revision 1.11  2019/02/03 15:50:24  rvv
*** empty log message ***

Revision 1.10  2019/01/23 16:27:16  rvv
*** empty log message ***

Revision 1.9  2019/01/19 18:05:31  rvv
*** empty log message ***

Revision 1.8  2019/01/19 13:54:10  rvv
*** empty log message ***

Revision 1.7  2019/01/16 08:41:45  rvv
*** empty log message ***

Revision 1.6  2019/01/12 17:08:31  rvv
*** empty log message ***

Revision 1.5  2019/01/05 18:38:35  rvv
*** empty log message ***

Revision 1.4  2018/12/15 17:49:14  rvv
*** empty log message ***

Revision 1.3  2018/02/28 16:48:22  rvv
*** empty log message ***
...
class RapportVKMS_L72
{
	function RapportVKMS_L72($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
		...
		$this->pdf->excelData[]=array('Naam Fonds','',date('d-m-Y',$this->pdf->rapport_datumvanaf),
   ...
   // $this->pdf->SetLineStyle(array('color'=>array($this->pdf->rapport_kop2_fontcolor[r],$this->pdf->rapport_kop2_fontcolor[g],$this->pdf->rapport_kop2_fontcolor[b])));
    $this->pdf->SetDrawColor($this->pdf->rapport_kop2_fontcolor[r],$this->pdf->rapport_kop2_fontcolor[g],$this->pdf->rapport_kop2_fontcolor[b]);
    $this->pdf->Rect($this->pdf->marge, $this->pdf->getY(), 297-$this->pdf->marge*2, 60 , 'D');
    $this->pdf->Rect($this->pdf->marge, $this->pdf->getY()+63, 297-$this->pdf->marge*2, 100 , 'D');
    $this->pdf->Line(297/2, $this->pdf->getY()+63, 297/2, $this->pdf->getY()+63+100 , 'D');
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    if($this->pdfVullen==true)
		{
			...
			$this->pdf->setWidths(array(3,100, 30, 30));
		...
		if($this->pdfVullen==true)
		{
			...
			$this->pdf->setWidths(array(150,28+50,20,20,20,20,18,18,18,18,18,15));
    ...
    if($spreadKostenEUR <> 0)
      ...
      $huidigeY=$this->pdf->getY();
			...
			$this->pdf->setXY(170,170);
			arsort($barData);
			//echo $startYGrafiek-$startY-10;exit;
			
			$this->VBarVerdeling(50,70,$barData);
...
foreach ($perCategorie as $hoofdcategorie=>$categorieData)
{
	if($this->pdfVullen==true)
  {
   // $this->printKop($perHoofdcategorie[$hoofdcategorie]['omschrijving'],'B');
    ...
    foreach ($categorieData as $categorie=>$fondsData)
    {
          ...
          if($this->pdf->getY() > 180)
            $this->pdf->addPage();
					$this->addHeader($perCategorie[$hoofdcategorie][$categorie]['omschrijving'], '');
				}
 ...
 $perfTotaal['percentageIndirectVermogenMetKostenfactor']=$percentageIndirectVermogenMetKostenfactor;
  ...
  function addHeader($categorie,$belcat='')
  {
    ...
    $this->pdf->Row(array(vertaalTekst("Naam Fonds", $this->pdf->rapport_taal),
                 ...
                 vertaalTekst("Gem.verm.", $this->pdf->rapport_taal),
                 vertaalTekst("Doorl. %", $this->pdf->rapport_taal),
                 vertaalTekst("Trans. %", $this->pdf->rapport_taal),
                 vertaalTekst("Perf Fee %", $this->pdf->rapport_taal),
                 vertaalTekst("Kost. abs.", $this->pdf->rapport_taal),
                 "" . vertaalTekst("Weging", $this->pdf->rapport_taal),
                 vertaalTekst("VKM", $this->pdf->rapport_taal)));
...
//		echo "$doelJaar $rendement $beginwaarde";
	...
	function LineDiagram($w, $h, $data, $color=null, $maxVal=0, $minVal=0, $horDiv=4, $verDiv=4,$vanafBegin=false)
	{
		...
		$this->pdf->setXY($XPage-18,$YPage-3);
		$this->pdf->SetFont($this->pdf->rapport_font,'b',$this->pdf->rapport_fontsize);
    $this->pdf->Cell($w,0,'X 1.000',0,0,'L');
	...
	//	$maxVal=round($maxVal,floor(log10($maxVal))*-1+1);
		...
		for($i=$nulpunt; $i<= $bodem; $i+= $absUnit*$stapgrootte)
		{
			...
			$this->pdf->Cell(7 , 4 , "€ ". $this->formatGetal(0-($n*$stapgrootte)/1000,0) , 0, 1, "R");
		...
		for($i=$nulpunt; $i >= $top; $i-= $absUnit*$stapgrootte)
		{
			...
			{
				$this->pdf->setXY($XDiag-7, $i);
				$this->pdf->Cell(7 , 4 , "€ " .$this->formatGetal(($n * $stapgrootte)/1000 + 0,0) , 0, 1, "R");
		...
		foreach ($legendaItems as $index=>$item)
		{
			...
			$this->pdf->Rect($XPage+1 , $YPage+$h+$step+6, 3, 3, 'DF','',$kleur);
			$this->pdf->SetXY($XPage+3+1,$YPage+$h+$step+6);
