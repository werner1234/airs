...
class RapportTRANS_L95
{
	function RapportTRANS_L95($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function formatAantal($waarde, $dec, $VierDecimalenZonderNullen=false)
  {
    ...
    $this->pdf->SetFont($this->pdf->rapport_font,$this->pdf->rapport_fontstyle,$this->pdf->rapport_fontsize);
		...
		$query = "SELECT Fondsen.id as fondsId, Fondsen.Omschrijving, ".
...
//	echo "lopende -> ".$resultaatlopende." <-  voorgaande ".$resultaatvoorgaande. " -  <br>" ;
    ...
    foreach($cashFondsTotaal as $groep=>$groepData)
    {
      foreach($groepData as $cashRegelFonds=>$cashData)
      {
        ...
        $this->pdf->row(array('',
                          $transactieType,
                          $this->formatAantal(round($cashData['Aantal'],4), 0, true),
                          '','',
                          $cashData['DebetTxt'],
                          $cashData['DebetEURTxt'],
                          '',
