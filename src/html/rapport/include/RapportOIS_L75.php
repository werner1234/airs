...
$Log: RapportOIS_L75.php,v $
Revision 1.6  2020/05/27 16:13:36  rvv
*** empty log message ***

Revision 1.5  2020/04/22 15:40:47  rvv
*** empty log message ***

Revision 1.4  2019/07/10 15:37:54  rvv
*** empty log message ***

Revision 1.3  2019/07/07 12:24:34  rvv
*** empty log message ***

Revision 1.2  2019/07/06 15:43:47  rvv
*** empty log message ***

Revision 1.1  2019/05/22 16:06:45  rvv
*** empty log message ***
...
class RapportOIS_L75
{
  function RapportOIS_L75($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
  {
    $this->pdf = &$pdf;
  ...
  function portefeuillePerf($ystart)
  {
    ...
    $this->pdf->SetWidths(array(35,40,40,40,40,40,40));
    $this->pdf->SetAligns(array('L','R','R','R','R','R'));
    unset($this->pdf->CellBorders);
    $this->pdf->setX($this->pdf->marge);
    $this->pdf->row(array(vertaalTekst("Portefeuille",$this->pdf->rapport_taal),
                      vertaalTekst("Vermogen",$this->pdf->rapport_taal), vertaalTekst('Liquide beleggingen',$this->pdf->rapport_taal),
                      vertaalTekst('Illiquide beleggingen',$this->pdf->rapport_taal),vertaalTekst("Resultaat",$this->pdf->rapport_taal).' €',
    ...
    foreach($portefeuilleWaarden as $portefeuille=>$data)
    {
      if(round($data['eindLiquide'])<>0 || round($data['eindIlLiquide'],2)<>0 || round($data['resultaat'],2)<>0 || round($data['rendement'],2)<>0)
      {
        ...
        $this->pdf->row(array(vertaalTekst($data['portefeuilleNaam'], $this->pdf->rapport_taal),
    ...
    $this->pdf->ln($witruimte);
    $this->pdf->row(array(vertaalTekst("Totaal",$this->pdf->rapport_taal),$this->formatGetal($jaarTotalen['portefeuilleWaarde'],2,false,$max)
                    ,$this->formatGetal($jaarTotalen['eindLiquide'],2,false,$max),$this->formatGetal($jaarTotalen['eindIlLiquide'],2,false,$max)
