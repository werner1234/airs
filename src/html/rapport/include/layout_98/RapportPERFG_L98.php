...
class RapportPERFG_L98
{
	function RapportPERFG_L98($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  if(count($rendamentWaarden) > 0)
   {
     ...
     $totaal=array();
        ...
        $kleuren=array(array(70,130,180),array(122,139,139));
        ...
        foreach($kleuren as $index=>$kleur)
        {
          ...
          if($index==1)
            $this->pdf->Cell(50, 4, 'Benchmark', 0, 0, "L");//$this->pdf->portefeuilledata['SpecifiekeIndex']
        }
...
*/
   ...
   if(count($data)>36)
    ...
    for ($i=0; $i<count($data); $i++)
    {
      ...
      if($legendaTonen)
        $this->pdf->TextWithRotation($XDiag+($i)*$unit-10+$unit,$YDiag+$hDiag+8,jul2form($datumJul),25);
