...
class RapportATT_L98
{
	function RapportATT_L98($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
$grafiekData['Datum'][]="$RapStartJaar-12-01";
   ...
   if(count($rendamentWaarden) > 0)
   {
     ...
     //   $this->pdf->ln(3);
        ...
        $this->pdf->CellBorders = array();
		    $this->pdf->row(array(vertaalTekst('Totaal periode',$this->pdf->rapport_taal),
...
*/
        ...
        $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
  		  ...
  		  $this->pdf->SetXY(15,117)		;//112
        $this->LineDiagram(273, 60, $grafiekData,$this->pdf->rapport_grafiek_color,0,0,6,5,1);//50
  ...
  function LineDiagram($w, $h, $data, $color=null, $maxVal=0, $minVal=0, $horDiv=4, $verDiv=4,$jaar=0)
  {
    ...
    $color1=array(122,139,139);
    if($color == null)
      $color=array(70,130,180);
    ...
    $bodem = $YDiag+$hDiag;
    $absUnit =abs($unith);
    $lijnenKleur=array(100,100,100);
    $nulpunt = $YDiag + (($maxVal) * $waardeCorrectie);
    $n=0;
    for($i=$nulpunt; $i<= $bodem; $i+= $absUnit*$stapgrootte)
    {
      $skipNull = true;
      $this->pdf->Line($XDiag, $i, $XPage+$w ,$i,array('dash' => 1,'color'=>$lijnenKleur));
...
*/
    ...
    if(is_array($data))
    {
      ...
      $this->pdf->line($XPage, $YDiag + $hDiag + 11, $XPage+2, $YDiag + $hDiag + 11, $lineStyle);
      $this->pdf->TextWithRotation($XPage+3, $YDiag + $hDiag + 12, 'Portefeuille', 0);
    }
    if(is_array($data1))
    {
      // listarray($data1);
      ...
      $xtraX=30;
      $this->pdf->SetFont($this->pdf->rapport_font, '', 8);
      $this->pdf->line($XPage+$xtraX, $YDiag + $hDiag + 11, $XPage+$xtraX+2, $YDiag + $hDiag + 11, $lineStyle);
      $this->pdf->TextWithRotation($XPage+3+$xtraX, $YDiag + $hDiag + 12, 'Benchmark', 0);
    }
