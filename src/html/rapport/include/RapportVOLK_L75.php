...
$Log: RapportVOLK_L75.php,v $
Revision 1.8  2020/07/22 16:35:31  rvv
*** empty log message ***

Revision 1.7  2018/06/17 15:51:53  rvv
*** empty log message ***

Revision 1.6  2018/06/16 17:42:56  rvv
*** empty log message ***

Revision 1.5  2018/06/10 11:45:56  rvv
*** empty log message ***
...
class RapportVOLK_L75
{
	function RapportVOLK_L75($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		$this->pdf->rapport_type = "VOLK";
		$this->pdf->rapport_datum = db2jul($rapportageDatum);
		$this->pdf->rapport_datumvanaf = db2jul($rapportageDatumVanaf);
		$this->pdf->rapport_titel = "Waarde-ontwikkeling lopend jaar";

		if(substr(jul2form($this->pdf->rapport_datumvanaf),0,5) != '01-01')
			$this->pdf->rapport_titel = "Waarde-ontwikkeling rapportage periode";
  ...
  function getDividend($fonds)
  {
	...
	function writeRapport()
	{
		...
		{
			$gegevens = bepaaldFondsWaardenVerdiept_L75($this->portefeuille, $this->rapportageDatum, $this->pdf,$this->rapportageDatumVanaf);
...
//echo $subquery."<br><br>";exit;
     ...
     //   }
		...
		//		echo "$procentResultaat = ((".$subdata['actuelePortefeuilleWaardeEuro']." - ".$subdata['beginPortefeuilleWaardeEuro']." + ".$dividend['corrected'].") / (".$subdata['beginPortefeuilleWaardeEuro']." /100)); <br>\n";
...
//rvv
		...
		" SUM(TijdelijkeRapportage.actuelePortefeuilleWaardeEuro) / ".$this->pdf->ValutaKoersEind. " as subtotaalactueel FROM ".
		" TijdelijkeRapportage ".
		" WHERE TijdelijkeRapportage.portefeuille = '".$this->portefeuille."' ".$this->categorieFilter." AND ".
