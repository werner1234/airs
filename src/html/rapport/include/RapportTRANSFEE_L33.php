...
$Log: RapportTRANSFEE_L33.php,v $
Revision 1.11  2020/01/12 14:01:32  rvv
*** empty log message ***

Revision 1.10  2020/01/04 18:55:50  rvv
*** empty log message ***

Revision 1.9  2019/01/26 19:33:28  rvv
*** empty log message ***

Revision 1.8  2019/01/16 16:28:10  rvv
*** empty log message ***

Revision 1.7  2019/01/16 08:41:44  rvv
*** empty log message ***

Revision 1.6  2019/01/14 17:47:15  rvv
*** empty log message ***

Revision 1.5  2019/01/12 17:08:31  rvv
*** empty log message ***

Revision 1.4  2019/01/09 15:52:19  rvv
*** empty log message ***

Revision 1.3  2018/12/22 16:15:52  rvv
*** empty log message ***

Revision 1.2  2018/12/16 08:31:57  rvv
*** empty log message ***
...
class RapportTRANSFEE_L33
{
	function RapportTRANSFEE_L33($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
		...
		$this->skipLangeTermijn=false;//true;
	}
	...
	function kostenKader($totaalDoorlopendekosten,$perfTotaal,$toekomst=false,$totaalDoorlopendekostenEind=0)
	{
    if ($this->pdfVullen==true && $this->pdf->rapportageValuta <> 'EUR')
    {
      $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
  
     // $koersQuery =	" / if(Rekeningmutaties.Valuta='". $this->pdf->rapportageValuta."',Rekeningmutaties.Valutakoers,
    //    (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 )) ";
    }
    else
    {
      $koersQuery = "";
...
GROUP BY Rekeningmutaties.Grootboekrekening";
	 ...
	 $query="SELECT
SUM((ABS(Rekeningmutaties.Credit) * Rekeningmutaties.Valutakoers ) - (ABS(Rekeningmutaties.Debet) * Rekeningmutaties.Valutakoers )*-1 $koersQuery)  AS totaal,
IF(Rekeningmutaties.Grootboekrekening <> 'BEH',	'KNBA',	Rekeningmutaties.Grootboekrekening) AS Grootboekrekening,
IF(Grootboekrekeningen.Omschrijving <> 'Beheervergoeding',	'Bankkosten',	Grootboekrekeningen.Omschrijving) AS Omschrijving
FROM Rekeningmutaties
JOIN Rekeningen ON Rekeningmutaties.Rekening=Rekeningen.Rekening
...
ORDER BY Grootboekrekeningen.Afdrukvolgorde";
    ...
    $eindwaarde=$this->verdelingTotaal['totaal']['eindwaarde'];
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    if($this->pdfVullen==true)
      ...
      if ($toekomst == false)
      {
        $this->pdf->row(array(vertaalTekst('Overzicht totale kosten over het afgelopen jaar', $this->pdf->rapport_taal)));
      }
      else
      {
        $this->pdf->row(array(vertaalTekst('Overzicht verwachte kosten voor het komende jaar', $this->pdf->rapport_taal)));
      }
      $this->pdf->ln();
      $this->pdf->setWidths(array(28,50,20,20,20,20,20,18,18,18,18,18,15));
      $this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
			$this->pdf->setWidths(array(28+30,20,20,20,20,20,20,20,20,20));
			$this->pdf->row(array(vertaalTekst('Directe kosten',$this->pdf->rapport_taal), vertaalTekst($this->pdf->rapportageValuta,$this->pdf->rapport_taal), vertaalTekst('Percentage' ,$this->pdf->rapport_taal)));
			$this->pdf->excelData[]=array('','Directe kosten vanaf ' . date('d-m-Y', db2jul($this->vanafDatum)),'', vertaalTekst($this->pdf->rapportageValuta ,$this->pdf->rapport_taal), vertaalTekst('Percentage' ,$this->pdf->rapport_taal) );
			$this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
		}
		$totaal=0;
    ...
    {
      $buffer[$data['Grootboekrekening']]=$data;
    }
    $buffer['KNBA']['totaal']+=$spreadKostenEUR;

    foreach($buffer as $grootboek=>$data)
    {
			$data['Omschrijving']=$this->grootboekOmschrijvingen[$data['Grootboekrekening']];
			if($this->pdfVullen==true)
			{
        ...
        if($toekomst==true)
        {
          $data['totaal'] = $kostenProcent * $eindwaarde / 100;
          $rekengemiddelde = $eindwaarde;
        }
        else
        {
          $rekengemiddelde = $gemiddelde;
        }
        if($data['Grootboekrekening']=='BEH')
        {
          $bedrag=$data['totaal']/1.21;
          $btw=$data['totaal']-$bedrag;
          

          $this->pdf->row(array(vertaalTekst($data['Omschrijving'],$this->pdf->rapport_taal), $this->formatGetal($bedrag, 0), $this->formatGetal($bedrag/$rekengemiddelde*100, 2) . ' %'));
          $this->pdf->row(array("21% ".vertaalTekst("B.T.W. over",$this->pdf->rapport_taal)." ".vertaalTekst($data['Omschrijving'],$this->pdf->rapport_taal), $this->formatGetal($btw, 0), $this->formatGetal($btw/$rekengemiddelde*100, 2) . ' %'));
        }
        else
        {
          $this->pdf->row(array(vertaalTekst($data['Omschrijving'],$this->pdf->rapport_taal), $this->formatGetal($data['totaal'], 0), $this->formatGetal($kostenProcent, 2) . ' %'));
				...
				$this->pdf->excelData[]=array('',vertaalTekst($data['Omschrijving'],$this->pdf->rapport_taal),'',round($data['totaal'],0),round($kostenProcent,2) );
				$barData[$data['Grootboekrekening']]=$kostenProcent;
			}
			$totaal+=$data['totaal'];
		}
/*
...
//  echo "transfee $vkmPercentagePortefeuille=$herrekendeKosten*$aandeelIndirect*100;";
...
//exit;
		...
		if($this->pdfVullen==true)
    {
      ...
      $this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
      if (round($totaalDoorlopendekosten, 2) <> 0 || round($vkmPercentagePortefeuille, 2) <> 0)
      {
        if($toekomst==true)
        {
          $kostenBedrag=$totaalDoorlopendekostenEind;
        }
        else
        {
          $kostenBedrag=$totaalDoorlopendekosten;
        }
        $vkmPercentagePortefeuille=$kostenBedrag/$rekengemiddelde*100;
        $this->pdf->row(array(vertaalTekst('Indirecte (fonds)kosten *', $this->pdf->rapport_taal), $this->formatGetal($kostenBedrag, 0), $this->formatGetal($vkmPercentagePortefeuille, 2) . ' %'));
        $this->pdf->ln();
      }
  
      $kostenPercentage=$totaal/$rekengemiddelde*100;
      ...
      if ($toekomst == false)
        $this->pdf->row(array(vertaalTekst('Totale kosten afgelopen jaar', $this->pdf->rapport_taal), $this->formatGetal($vkmWaarde * $rekengemiddelde / 100, 0), $this->formatGetal($vkmWaarde, 2) . ' %'));
      else
        $this->pdf->row(array(vertaalTekst('Totale verwachte kosten voor het komende jaar', $this->pdf->rapport_taal), $this->formatGetal($vkmWaarde * $rekengemiddelde / 100, 0), $this->formatGetal($vkmWaarde, 2) . ' %'));
      unset($this->pdf->CellBorders);
      $this->pdf->excelData[] = array('', vertaalTekst('Vergelijkende kostenmaatstaf' ,$this->pdf->rapport_taal), '', round($vkmWaarde * $rekengemiddelde / 100, 2), round($vkmWaarde, 2));
      $this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
      ...
      if ($toekomst == false)
      {
        ...
        $this->pdf->setWidths(array(2, 110, 30, 30));
        if ($this->melding <> '')
        {
          $this->pdf->setXY($this->pdf->marge, 182);
          $this->pdf->row(array('', $this->melding));
          $this->pdf->excelData[] = array();
          $this->pdf->excelData[] = array('', $this->melding);
        }
        
        $this->pdf->setXY($this->pdf->marge, 190);
        $this->pdf->Cell(2, 0, '*', 0, 0, 'L');
        $this->pdf->Cell(195, 0, vertaalTekst('Deze kosten betreffen de lopende interne kosten van beleggingsfondsen (zoals ETF\'s en andere soorten fondsen).', $this->pdf->rapport_taal), 0, 0, 'L');
        $this->pdf->setXY($this->pdf->marge + 2, 194);
        $this->pdf->Cell(195, 0, vertaalTekst('Op uw verzoek kunnen wij een uitsplitsing van genoemde kosten verzorgen.', $this->pdf->rapport_taal), 0, 0, 'L');
        /*
...
TijdelijkeRapportage.rapportageDatum = '".$this->rapportageDatum."' ".$__appvar['TijdelijkeRapportageMaakUniek'];
   ...
   // $this->verdelingTotaal['perioden'][$this->rapportageDatum]=$portefeuileWaarde['actuelePortefeuilleWaardeEuro'];
    $this->verdelingTotaal['totaal']['eindwaarde']=$portefeuileWaarde['actuelePortefeuilleWaardeEuro'] ;
    
//listarray($this->verdelingTotaal);exit;
...
$totaalSom['bijdrage'] += $data['bijdrage'];
    ...
    $laatsteFonds='';
    $totaalKosten=0;$totaaldlKosten=0;
    $lastRegio='';
...
//listarray($data);
       ...
       //   echo $this->pdf->widths[0]." ".$this->pdf->widths[1]." ".$omschrijving."<br>\n";
        ...
        $totaalDoorlopendekostenEind+=$data['eindwaarde']*$totaalKostenPercentage/100;
//echo "$totaalDoorlopendekosten += ".$sub['gemWaarde']."*$totaalKostenPercentage/100 <br>\n";
				$perHoofdcategorie[$hoofdcategorie]['perf']['bijdrageVKM'] +=$bijdrageVKM;
  ...
  $lastHoofdcategorie=$hoofdcategorie;
    ...
    while($data=$DB->nextRecord())
    {
      ...
      if($gewensteKleuren[$data['Grootboekrekening']]['R']['value'] || $gewensteKleuren[$data['Grootboekrekening']]['G']['value'] || $gewensteKleuren[$data['Grootboekrekening']]['B']['value'])
        $grootboekKleuren[$data['Grootboekrekening']]=array($gewensteKleuren[$data['Grootboekrekening']]['R']['value'],$gewensteKleuren[$data['Grootboekrekening']]['G']['value'],$gewensteKleuren[$data['Grootboekrekening']]['B']['value']);
      else
        $grootboekKleuren[$data['Grootboekrekening']]=$mogelijkeKleuren[$n];
      
      if($data['Grootboekrekening']=='BEH')
        $data['Omschrijving']='Fee B.A. van Doorn';
      if($data['Grootboekrekening']=='KNBA')
        $data['Omschrijving']='Bankkosten';
    ...
    $grootboekOmschrijvingen['Doorlopende kosten']='Doorlopende kosten';
    $grootboekOmschrijvingen['Spread-kosten']='Spread-kosten';
    $this->grootboekOmschrijvingen=$grootboekOmschrijvingen;
 ...
 $perfTotaal['percentageIndirectVermogenMetKostenfactor']=$percentageIndirectVermogenMetKostenfactor;
      ...
      $this->kostenKader($totaalDoorlopendekosten, $perfTotaal,true,$totaalDoorlopendekostenEind);
			...
			if ($this->pdfVullen == true)
			{
				...
				//else
				//	$this->pdf->row(array(vertaalTekst('Deze rapportage is met de grootst mogelijke zorg samengesteld. Omwille van de snelheid van rapporteren wordt de laatste beschikbare data gebruikt. Deze data kan ouder zijn dan de rapportagedatum.',$this->pdf->rapport_taal)));
 ...
 WHERE Portefeuilles.portefeuille='".$this->portefeuille."'";
		...
		$db->Query();
		$start = $db->NextRecord();
		$beginwaarde = $start['actuelePortefeuilleWaardeEuro'] / $this->pdf->ValutaKoersEind;
    ...
    for($i=$this->pdf->rapport_jaar; $i<=$doelJaar; $i++)
		...
		$grafiekWaarden['legenda']=array(vertaalTekst('Waardeontwikkeling zonder kosten' ,$this->pdf->rapport_taal),
      vertaalTekst('Waardeontwikkeling na kosten' ,$this->pdf->rapport_taal),
      vertaalTekst('Cumulatieve kosten' ,$this->pdf->rapport_taal));
		
		//$grafiekWaarden['titel']="Overzicht verwachte waardeontwikkeling en kosten\n(exclusief stortingen & onttrekkingen)";
...
//		echo "$doelJaar $rendement $beginwaarde";
    ...
    $this->pdf->setXY($XPage,$YPage-8);
		$this->pdf->Cell($w,0,vertaalTekst('Overzicht verwachte waardeontwikkeling en kosten',$this->pdf->rapport_taal),0,0,'C');
    $this->pdf->setXY($XPage,$YPage-4);
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
    $this->pdf->Cell($w,0,vertaalTekst('(exclusief stortingen & onttrekkingen)',$this->pdf->rapport_taal),0,0,'C');
		$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
    ...
    $skipNull=false;
		for($i=$nulpunt; $i<= $bodem; $i+= $absUnit*$stapgrootte)
		{
			...
			if(round($n) <> 0)
			  $this->pdf->Cell(7 , 4 , ($this->pdf->rapportageValuta=='EUR'?"€":$this->pdf->rapportageValuta).' '. $this->formatGetal(0-($n*$stapgrootte),0) , 0, 1, "R");
		...
		for($i=$nulpunt; $i >= $top; $i-= $absUnit*$stapgrootte)
		{
			...
			else
			{
				$this->pdf->setXY($XDiag-7, $i);
        if(round($n) <> 0)
				  $this->pdf->Cell(7 , 4 , ($this->pdf->rapportageValuta=='EUR'?"€":$this->pdf->rapportageValuta).' '.$this->formatGetal((($n * $stapgrootte) + 0),0) , 0, 1, "R");
			...
			if($i%$jaren==0)
      {
        ...
        $this->pdf->Cell($unit , 4 ,$legendDatum[$i] , 0, 1, "C");
        
      //  $this->pdf->TextWithRotation($XDiag + ($i - 1) * $unit - 5 + $unit, $YDiag + $hDiag + 4, $legendDatum[$i], 0);
      }
			$yval2 = $YDiag + (($maxVal-$data[$i]) * $waardeCorrectie) ;

			if ($i>0 || $vanafBegin==true)
			{
				$this->pdf->line($XDiag+($i-1)*$unit, $yval, $XDiag+($i)*$unit, $yval2,$lineStyle);
			}
		...
		foreach ($legendaItems as $index=>$item)
		{
			...
			//$this->pdf->Rect($XPage+35 , $YPage+$h+$step+11, 3, 3, 'DF','',$kleur);
      $lineStyle=array('color'=>array($kleur[0],$kleur[1],$kleur[2]),'width' => 0.5,'cap' => 'butt');
      $this->pdf->line($XPage , $YPage+$h+$step+3+2.5,$XPage+3, $YPage+$h+$step+3+2.5,$lineStyle );
      
			$this->pdf->SetXY($XPage+3,$YPage+$h+$step+3+1);
...
//echo $periode['stop']." $aandeelPeriode <br>\n ";
 ...
 //   echo "transfee $totaal | $gemiddelde / = $weging | ".$this->verdelingTotaal['totaal']['gemiddelde'];exit;
  ...
  function fondsPerf($fonds,$van,$tot)
  {
	...
	function VBarVerdeling($w, $h, $data)
	{
		...
		foreach($grafiek as $categorie=>$val)
		{
			...
			$this->pdf->Cell(50, 4,$this->grootboekOmschrijvingen[$categorie]." ".number_format($val,2,',','.')."%",0,0,'L');
