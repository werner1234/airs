...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2019/04/03 15:53:18 $
File Versie					: $Revision: 1.21 $

$Log: RapportKERNV_L72.php,v $
Revision 1.21  2019/04/03 15:53:18  rvv
*** empty log message ***

Revision 1.20  2019/02/16 19:23:35  rvv
*** empty log message ***

Revision 1.19  2018/11/21 16:48:32  rvv
*** empty log message ***

Revision 1.18  2018/11/17 17:34:53  rvv
*** empty log message ***

Revision 1.17  2018/10/17 15:37:17  rvv
*** empty log message ***

Revision 1.16  2018/10/11 05:43:14  rvv
*** empty log message ***

Revision 1.15  2018/10/10 15:50:56  rvv
*** empty log message ***

Revision 1.14  2018/08/18 12:40:14  rvv
php 5.6 & consolidatie

Revision 1.13  2018/08/07 05:58:05  rvv
*** empty log message ***

Revision 1.12  2018/08/04 11:54:53  rvv
*** empty log message ***

Revision 1.11  2018/06/30 17:43:55  rvv
*** empty log message ***

Revision 1.10  2018/06/16 17:42:56  rvv
*** empty log message ***

Revision 1.9  2018/03/21 17:04:24  rvv
*** empty log message ***

Revision 1.8  2018/03/17 18:48:55  rvv
*** empty log message ***

Revision 1.7  2018/02/24 18:33:46  rvv
*** empty log message ***

Revision 1.6  2018/02/10 18:09:12  rvv
*** empty log message ***

Revision 1.5  2017/12/13 17:03:35  rvv
*** empty log message ***

Revision 1.4  2017/12/09 17:54:25  rvv
*** empty log message ***

Revision 1.3  2017/11/29 16:18:18  rvv
*** empty log message ***

Revision 1.2  2017/10/02 10:39:42  rvv
*** empty log message ***

Revision 1.1  2017/09/23 17:42:26  rvv
*** empty log message ***
...
class RapportKERNV_L72
{
	function RapportKERNV_L72($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
WHERE CategorienPerHoofdcategorie.Vermogensbeheerder='".$beheerder."'
GROUP BY CategorienPerHoofdcategorie.Hoofdcategorie
ORDER BY Beleggingscategorien.Afdrukvolgorde";
		$DB = new DB();
		$DB->SQL($q);
		$DB->Query();
		$this->hoofdCategorieVolgorde=array();
		while($data=$DB->nextRecord())
		{
			$this->hoofdCategorieVolgorde[$data['Hoofdcategorie']]=$data['Omschrijving'];
		}
    ...
    foreach ($this->verdiepteFondsen  as $key=>$fonds)
    {
...
/*
		foreach ($this->verdiepteFondsen  as $key=>$fonds)
		{
		  if($this->verdiept->FondsPortefeuilleData[$fonds]=='GAF')
			{
				$this->verdiept->bepaalVerdeling($fonds, $this->verdiept->FondsPortefeuilleData[$fonds], array('fonds'), $this->rapportageDatum);
			}
			else
			{
				unset($this->verdiepteFondsen[$key]);
				unset($this->verdiept->FondsPortefeuilleData[$fonds]);
			}
		}
*/
...
LEFT JOIN Beleggingscategorien on TijdelijkeRapportage.Beleggingscategorie = Beleggingscategorien.Beleggingscategorie
	...
	AND TijdelijkeRapportage.rapportageDatum ='".$rapportageDatum."' ".$this->rapportFilter." ".(count($this->verdiepteFondsen)>0?"AND Fonds NOT IN('".implode("','",$this->verdiepteFondsen) ."') ":'')
	.$__appvar['TijdelijkeRapportageMaakUniek'].
	" GROUP BY categorie
	...
	while($cat = $DB->nextRecord())
	{
	   ...
	   $data['hoofdcategorie']['pieData'][$cat['categorieOmschrijving']]= $data['hoofdcategorie']['data'][$cat['categorieOmschrijving']]['waardeEur']/$totaalWaarde;
	   $data['hoofdcategorie']['kleurData'][$cat['categorieOmschrijving']]=array($allekleuren['OIB'][$cat['categorie']]['R']['value'],$allekleuren['OIB'][$cat['categorie']]['G']['value'],$allekleuren['OIB'][$cat['categorie']]['B']['value']);
	    $percentage=$data['hoofdcategorie']['data'][$cat['categorieOmschrijving']]['waardeEur']/$totaalWaarde*100;
		...
		$query="SELECT ActuelePortefeuilleWaardeEuro, Fonds FROM TijdelijkeRapportage WHERE Fonds IN('".implode("','",$this->verdiepteFondsen ) ."') AND TijdelijkeRapportage.Portefeuille = '".$portefeuille."'
	AND TijdelijkeRapportage.rapportageDatum ='".$rapportageDatum."' ".$this->rapportFilter." ".$__appvar['TijdelijkeRapportageMaakUniek'];
		$DB->SQL($query);
		$DB->Query();
		while($cat = $DB->nextRecord())
		{
			$fondsWaarden[$cat['Fonds']]=$cat['ActuelePortefeuilleWaardeEuro'];
		}
		foreach($fondsWaarden as $samengesteldfonds=>$waarde)
		{
			$aandeelVanSamgensteldFonds=$waarde/$this->verdiept->pdf->fondsPortefeuille[$samengesteldfonds]['totaalWaarde'];
			//echo " $aandeelVanSamgensteldFonds = $waarde/".$waarde['totaalWaarde']."<br>\n";
			$typen=array('fondsen','rente','liquiditeiten');
			foreach($typen as $type)
			{
				if(isset($this->verdiept->pdf->fondsPortefeuille[$samengesteldfonds]['verdeling'][$type]))
				{
					foreach ($this->verdiept->pdf->fondsPortefeuille[$samengesteldfonds]['verdeling'][$type] as $subFonds => $subGegevens)
					{
						if(isset($subGegevens['overige']))
						{

							$cat['WaardeEuro'] = $subGegevens['overige']['WaardeEuro'] * $aandeelVanSamgensteldFonds;
							$cat['categorie'] = $subGegevens['overige']['hoofdcategorie'];
							$cat['categorieOmschrijving'] = $subGegevens['overige']['hoofdcategorieOmschrijving'];
							//echo "$type $subFonds " . $cat['categorieOmschrijving'] . "<br>\n";
		...
		$pieData=	$data['hoofdcategorie']['pieData'];
		$data['hoofdcategorie']['pieData']=array();
		foreach($this->hoofdCategorieVolgorde as $hoofdcategorie=>$omschrijving)
		{
			if(isset($pieData[$omschrijving]))
			  $data['hoofdcategorie']['pieData'][$omschrijving]=$pieData[$omschrijving];
		}
...
//listarray($this->verdiepteFondsen);exit;
...
.  (count($this->verdiepteFondsen)>0?"AND Fonds NOT IN('".implode("','",$this->verdiepteFondsen) ."') ":'').
			$__appvar['TijdelijkeRapportageMaakUniek']." GROUP BY beleggingscategorie";
			//echo $query."<br>\n"; echo $this->totaalWaarde; listarray($this->verdiepteFondsen);exit;
		$DB->SQL($query);
		$DB->Query();
		...
		while($cat = $DB->nextRecord())
		{
			...
			$categorieOmschrijving[$cat['beleggingscategorie']] = $cat['beleggingscategorieOmschrijving'];
			
		}

		$query="SELECT beleggingscategorie,Omschrijving FROM Beleggingscategorien ORDER BY afdrukvolgorde";
		$DB->SQL($query);
		$DB->Query();
		$categorieOmschrijving=array();
		while($cat = $DB->nextRecord())
		{
			$categorieOmschrijving[$cat['beleggingscategorie']] = $cat['Omschrijving'];
		}

		$query="SELECT ActuelePortefeuilleWaardeEuro, Fonds,beleggingscategorie,beleggingscategorieOmschrijving FROM TijdelijkeRapportage WHERE Fonds IN('".implode("','",$this->verdiepteFondsen) ."') AND TijdelijkeRapportage.Portefeuille = '".$this->portefeuille."'
			...
			foreach($typen as $type)
			{
				if(isset($this->verdiept->pdf->fondsPortefeuille[$samengesteldfonds]['verdeling'][$type]))
				{
					foreach ($this->verdiept->pdf->fondsPortefeuille[$samengesteldfonds]['verdeling'][$type] as $subFonds => $subGegevens)
					{
						...
						{
							$cat['WaardeEuro'] = $subGegevens['overige']['WaardeEuro'] * $aandeelVanSamgensteldFonds;
							$cat['beleggingscategorie'] = $subGegevens['overige']['beleggingscategorie'];
							$oudeWaarde=$categorieVerdeling[$cat['beleggingscategorie']]*$this->totaalWaarde/100;
							$categorieVerdeling[$cat['beleggingscategorie']] = ($oudeWaarde+$cat['WaardeEuro']) /$this->totaalWaarde*100;
							if( $subGegevens['overige']['beleggingscategorieOmschrijving']<>'')
								$categorieOmschrijving[$cat['beleggingscategorie']] = $subGegevens['overige']['beleggingscategorieOmschrijving'];
...
//listarray($categorieVerdeling);
		...
		$att=new ATTberekening_L72($this,true);
    ...
    $this->waarden['Periode']=$this->kostenCorrectie($this->waarden['Periode']);
    $this->waarden['Jaar']=$this->kostenCorrectie($this->waarden['Jaar']);

   // listarray($correcties);exit;


		//echo $this->tweedePerformanceStart.",  ".$this->rapportageDatum.",".$this->pdf->rapportageValuta."<br>\n";exit;
//listarray($this->waarden['Periode']);exit;
    $this->pdf->setY($ystart);
   	...
   	//$bovencat=$att->categorien;
		//foreach($categorieOmschrijving as $cat=>$omschrijving)
		//	$bovencat[$cat]=$omschrijving;

		$bovencat=$categorieOmschrijving;
		$this->pdf->CellBorders=array('','','L','','L','','L','');
		$max=1000;
		...
		foreach ($bovencat as $categorie=>$categorieOmschrijving)
		{
			...
			if($this->waarden['Periode'][$categorie]['weging'] <> 0 || $this->waarden['Jaar'][$categorie]['weging'] <> 0 ||
				$this->waarden['Periode'][$categorie]['procent'] <> 0 || $this->waarden['Jaar'][$categorie]['procent'] <> 0 ||
				$categorieVerdeling[$categorie] <> 0 )
			{
		...
		$this->pdf->row(array(vertaalTekst("Totaal",$this->pdf->rapport_taal),
											$this->formatGetal($this->jaarTotalen['weging'],$decimalen,false,$max),
											$this->formatGetal($this->waarden['Periode']['totaal']['procent'],$decimalen,false,$max),
											$this->formatGetal($jaarTotaal['procent'],$decimalen,false,$max),//$this->jaarTotalen['portBijdrage']
											$this->formatGetal($this->waarden['Periode']['totaal']['bijdrage'],$decimalen,false,$max),
											$this->formatGetal($jaarTotaal['bijdrage'] ,$decimalen,false,$max),'',''));//jaarTotalen['portBijdrage']
		$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
		$this->pdf->CellBorders=array('','','L','','L','','L','');
		$this->pdf->row(array(vertaalTekst("Kosten",$this->pdf->rapport_taal),'',$this->formatGetal($this->maandRendement-$this->waarden['Periode']['totaal']['procent'],$decimalen,false,$max),
											$this->formatGetal($this->ytdRendement-$jaarTotaal['procent'],$decimalen,false,$max),//$this->jaarTotalen['portBijdrage']
											$this->formatGetal($this->maandRendement-$this->waarden['Periode']['totaal']['bijdrage'],$decimalen,false,$max),
											$this->formatGetal($this->ytdRendement-$jaarTotaal['bijdrage'],$decimalen,false,$max),'',''));//$this->jaarTotalen['portBijdrage']

		$this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
		$this->pdf->CellBorders=array(array('','','T'),array('','T'),array('L','','T'),array('','T'),array('L','','T'),array('','T'),array('L','','T'),array('','','T'));
		$this->pdf->row(array(vertaalTekst("Totaal na kosten",$this->pdf->rapport_taal),$this->formatGetal($this->jaarTotalen['weging'],$decimalen,false,$max),
											$this->formatGetal($this->maandRendement,$decimalen,false,$max),
											$this->formatGetal($this->ytdRendement,$decimalen,false,$max),
											$this->formatGetal($this->maandRendement,$decimalen,false,$max),
											$this->formatGetal($this->ytdRendement,$decimalen,false,$max),
											$this->formatGetal($this->waarden['Periode']['totaal']['indexPerf'], $decimalen, true,$max),
											$this->formatGetal($jaarTotaal['indexPerf'], $decimalen, true,$max)));
		unset($this->pdf->CellBorders);
	}
...
//echo $this->pdf->rapportageValuta."$index $vanaf,$beginJaar <br>\n";ob_flush();
	...
	//	$interneboeking     = $stortingenTmp['kruispost']-$stortingenTmp['kruispost'];
			...
			*/
			...
			//echo "$vanaf, $tot<br>\n";
			//listarray($gerealiseerdeKoersResultaat);
...
//			$this->pdf->Rect($XDiag+$i*$unit-0.5, $yval-0.5, 1, 1 ,'F','',$color);
	...
	function VBarDiagram2($w, $h, $data, $format, $color=null,$nbDiv=4,$numBars=0)
	{
		...
		for($i=$nulpunt; $i< $bodem; $i+= $absUnit*$stapgrootte)
		{
			...
			$this->pdf->MultiCell(12, 3, $this->formatGetal($n*$stapgrootte*-1,1) ." %", 0, 'R');
		...
		for($i=$nulpunt; $i > $top; $i-= $absUnit*$stapgrootte)
		{
			...
			else
			{
				...
				$this->pdf->setXY($XstartGrafiek-12, $i);
				$this->pdf->MultiCell(12, 3, $this->formatGetal($n*$stapgrootte,1) ." %", 0, 'R');
			}
			$n++;
			if($n >20)
				break;
...
//echo "perf $eindDatum  $wegingsDatum $performance = (((".$totaalWaarde['eind']." - ".$totaalWaarde['begin'].") - ".$weging['totaal2'].") / $gemiddelde) * 100;<br>\n";
	...
	function getWaarden($datumBegin,$datumEind,$portefeuille,$specifiekeIndex='',$methode='maanden',$valuta='EUR',$output='')
	{
		...
		foreach ($datum as $periode)
		{
			...
			{
				//if($specifiekeIndex )
				$query="SELECT waarde as specifiekeIndex FROM PortefeuilleHistorischeParameters WHERE portefeuille='$portefeuille' AND tot > '".$periode['stop']."' AND veld='SpecifiekeIndex' ORDER BY tot desc limit 1";
				$db->SQL($query);
