...
Revision 1.3  2012/12/30 14:27:11  rvv
*** empty log message ***
...
Revision 1.4  2012/11/10 15:42:19  rvv
*** empty log message ***

Revision 1.3  2012/10/31 16:59:17  rvv
*** empty log message ***

Revision 1.2  2012/09/19 16:53:18  rvv
*** empty log message ***

Revision 1.1  2012/09/16 12:45:46  rvv
*** empty log message ***

Revision 1.10  2012/07/29 10:24:33  rvv
*** empty log message ***
...
class RapportHSE_L41
{
	function RapportHSE_L41($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		...
		$this->pdf->rapport_titel = "Vermogensverloop en rendementsbijdrage";
...
//    $this->pdf->SetFillColor($this->pdf->kopkleur[0],$this->pdf->kopkleur[1],$this->pdf->kopkleur[2]);
    $this->pdf->SetDrawColor($this->pdf->kopkleur[0],$this->pdf->kopkleur[1],$this->pdf->kopkleur[2]);
	    ...
	    $this->pdf->CellBorders = array('','','','T','T','T','T','T','T','T','T','T','T','T','T');
      if($style=='B')
        $this->pdf->CellBorders = array('U','U','U',array('T','U'),array('T','U'),array('T','U'),array('T','U'),array('T','U'),array('T','U'),array('T','U'),array('T','U'),array('T','U'),array('T','U'),array('T','U'),array('T','U'));
     ...
     $this->pdf->CellBorders = array('U','U','U','U','U','U','U','U','U','U','U','U','U','U','U');
	    //$this->pdf->CellBorders = array('','','',array('T','UU'),array('TS','UU'),array('TS','UU'),array('TS','UU'),array('TS','UU'),array('TS','UU'),array('TS','UU'),array('TS','UU'),array('TS','UU'),array('TS','UU'),array('TS','UU'),array('TS','UU'));
	  }
...
*/
   	...
   	$this->pdf->row(array(vertaalTekst($categorieData['omschrijving']),
...
INNER JOIN CategorienPerHoofdcategorie ON KeuzePerVermogensbeheerder.waarde = CategorienPerHoofdcategorie.Beleggingscategorie AND CategorienPerHoofdcategorie.Vermogensbeheerder = '".$this->pdf->portefeuilledata['Vermogensbeheerder']."'
...
ORDER BY hcat.Afdrukvolgorde,KeuzePerVermogensbeheerder.Afdrukvolgorde";
    $DB->SQL($query);
    $DB->Query();
    while($data=$DB->nextRecord())
    {
      $perCategorie[$data['Hoofdcategorie']][$data['Beleggingscategorie']]['categorie']=$data['Beleggingscategorie'];
      $perCategorie[$data['Hoofdcategorie']][$data['Beleggingscategorie']]['omschrijving']=$data['categorieOmschrijving'];
      $perCategorie[$data['Hoofdcategorie']][$data['Beleggingscategorie']]['fondsen']=array();
      $perCategorie[$data['Hoofdcategorie']][$data['Beleggingscategorie']]['fondsValuta']=array();
    } 
...
ORDER BY HoofdBeleggingscategorien.Afdrukvolgorde, Regios.Afdrukvolgorde,Beleggingscategorien.Afdrukvolgorde,fondsVolgorde,OptieBovenliggendFonds,FondsOmschrijving ";
...
foreach ($perCategorie as $hoofdcategorie=>$categorieData)
{
  ...
  $this->printKop($perHoofdcategorie[$hoofdcategorie]['omschrijving'],'B',false);
  ...
  //{
    ...
    {
    if(count($fondsData['fondsen']) > 0 || count($fondsData['rekeningen']) > 0)
    {
      if($categorie!=$lastCategorie)
        $this->printKop($perCategorie[$hoofdcategorie][$categorie]['omschrijving'],'R');
      ...
      foreach ($fondsData['fondsen'] as $id=>$fonds)
      {
        ...
        if($fondsGegevens[$fonds]['Fonds']==$fondsGegevens[$fonds]['fondsVolgorde'])
        {
          ...
          foreach($somVelden as $veld)
            $sub[$veld]+=$data[$veld];
          $laatsteFonds=$fondsData['fondsOmschrijving'][$id];
        }
        if($fondsGegevens[$fonds]['Fonds']!=$fondsGegevens[$fonds]['fondsVolgorde'])
        {
          foreach($somVelden as $veld)
...
//echo "begin ".substr($fondsData['fondsOmschrijving'][$id],0,30);        listarray($this->pdf->widths);
      	$this->pdf->row(array('',$fondsData['fondsOmschrijving'][$id],
                        ...
                        $this->formatGetal($data['weging']*100,2,true),
                        $this->formatGetal($data['bijdrage']*100,2,true)));
//echo "eind ".substr($fondsData['fondsOmschrijving'][$id],0,30);        listarray($this->pdf->widths);
      }
...
WHERE Rekeningen.Rekening='$rekening' AND Portefeuilles.Portefeuille <> 'C_$USR'";
        ...
        $valutaDepotVerdeling[$depot['Valuta']]['depotbanken'][$rekening]['percentage']=$data['eindwaarde']/$totaalRekeningen*100;
      	...
      	$this->pdf->row(array('',$rekening,
												...
												$this->formatGetal($data['resultaat'],0),
                        $this->formatGetal($data['gemWaarde'],0),
                        $this->formatGetal($data['resultaat']/$data['gemWaarde'],2),
                        $this->formatGetal($data['weging']*100,2,true),
                        $this->formatGetal($data['bijdrage']*100,2,true)));
      }

      $this->pdf->widths=$widthsBackup;
      $this->printSubTotaal($perCategorie[$hoofdcategorie][$categorie]['omschrijving'],$perCategorie[$hoofdcategorie][$categorie],'R');
    }  
    }
//listarray($perHoofdcategorie[$hoofdcategorie]);
  ...
  //}
  $this->printSubTotaal($perHoofdcategorie[$hoofdcategorie]['omschrijving'],$perHoofdcategorie[$hoofdcategorie],'B');
