...
$Log: RapportPERFD_L7.php,v $
Revision 1.4  2020/06/27 16:23:17  rvv
*** empty log message ***

Revision 1.3  2020/06/25 05:19:51  rvv
*** empty log message ***

Revision 1.2  2020/06/24 13:02:42  rvv
*** empty log message ***

Revision 1.1  2020/06/20 12:14:07  rvv
*** empty log message ***
...
include_once("rapport/include/ATTberekening_L7.php");

class RapportPERFD_L7
{

	function RapportPERFD_L7($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    $this->att=new ATTberekening_L7($this);
	}
...
//listarray($this->waarden['Periode']);
 ...
 // listarray($this->pdf->portefeuilles);
  ...
  $interneboekingen=array("",vertaalTekst("Interne boekingen",$this->pdf->rapport_taal));
...
$grootboekCategorieen=array('grootboekOpbrengsten','grootboekKosten');
  $liqWaarden=array();
  foreach($grootboekCategorieen as $grootboekType)
  {
    $liqWaarden[$grootboekType]=$this->waarden['Periode']['totaal'][$grootboekType];
  }
  
  foreach($this->waarden['Periode'] as $categorie=>$catDetails)
  {
    if($categorie<>'totaal' && $categorie<>'Liquiditeiten')
    {
      foreach($grootboekCategorieen as $grootboekType)
      {
        foreach($catDetails[$grootboekType] as $grootboek=>$waarde)
        $liqWaarden[$grootboekType][$grootboek]-=$waarde;
      }
  
    }

  }
  $this->waarden['Periode']['Liquiditeiten']['grootboekOpbrengsten']=$liqWaarden['grootboekOpbrengsten'];
  $this->waarden['Periode']['Liquiditeiten']['grootboekKosten']=$liqWaarden['grootboekKosten'];
  $this->waarden['Periode']['Liquiditeiten']['opbrengst']=array_sum($liqWaarden['grootboekOpbrengsten']);
  $this->waarden['Periode']['Liquiditeiten']['kosten']=array_sum($liqWaarden['grootboekKosten']);
  ...
  foreach($categorien as $categorie)
  {
    ...
    if($categorie=='totaal')
    {
     ...
     //$onttr=getOnttrekkingen($this->rapport->portefeuille, $datumBegin, $datumEind)
      ...
      $interneboekingen[]=$this->formatGetal($perfWaarden['interneboeking'],0);
      $interneboekingen[]='';
      $interneboekingen[]='';
    }
    else
    {
      if($categorie=='Liquiditeiten')
        $effectenmutaties[]=$this->formatGetal($perfWaarden['stort']-$perfWaarden['kosten']-$perfWaarden['interneboeking'],0);
      else
        $effectenmutaties[]=$this->formatGetal($perfWaarden['stort']-$perfWaarden['interneboeking'],0);
      $effectenmutaties[]='';
