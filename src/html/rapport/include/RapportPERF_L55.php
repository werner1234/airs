...
$Log: RapportPERF_L55.php,v $
Revision 1.6  2019/03/09 18:46:18  rvv
*** empty log message ***

Revision 1.5  2018/03/17 18:48:55  rvv
*** empty log message ***

Revision 1.4  2018/03/07 16:58:07  rvv
*** empty log message ***
...
Revision 1.1  2014/08/06 15:41:01  rvv
*** empty log message ***
...
class RapportPERF_L55
{

	function RapportPERF_L55($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    //echo "$rendementProcent  	= performanceMeting(".$this->portefeuille.", ".$this->rapportageDatumVanaf.", ".$this->rapportageDatum.", ".$this->pdf->portefeuilledata['PerformanceBerekening'].",".$this->pdf->rapportageValuta." ";exit;
		...
		*/
		...
		$query = "SELECT SUM((actuelePortefeuilleWaardeEuro / ".$this->pdf->ValutaKoersEind.") - (beginPortefeuilleWaardeEuro/".$this->pdf->ValutaKoersStart." )) AS resultaat,
		...
		$DB->Query();
		$totaal = $DB->nextRecord();
		//echo "$ongerealiseerdeKoersResultaat <br>\n";
		$ongerealiseerdeKoersResultaat = $totaal['resultaat'];
		$ongerealiseerdeFondsResultaat = $totaal['koersResultaat'];




		//huidigeJaarRapdatum - 01-01-HuidigeJaar = OngerealiseerdHuidigeJaar.
...
//rvv end
		...
		{
			if($gb['Grootboekrekening']=='RENTE')
				$gb['Omschrijving']='Rekening courant rente';
			if($gb['Grootboekrekening']=='RENOB')
				$gb['Omschrijving']='Uitgekeerde rente obligaties';
		...
		while($kosten = $DB->nextRecord())
		{
			...
			{
			  $kostenPerGrootboek[$kosten['Grootboekrekening']]['Omschrijving'] = "Kosten depotbank";
		...
		$this->pdf->row(array("",vertaalTekst("Ongerealiseerde koersresultaten",$this->pdf->rapport_taal),$this->formatGetal($ongerealiseerdeFondsResultaat,2),""));
		$this->pdf->row(array("",vertaalTekst("Ongerealiseerde valutaresultaten",$this->pdf->rapport_taal),$this->formatGetal($ongerealiseerdeKoersResultaat-$ongerealiseerdeFondsResultaat,2),""));
		$this->pdf->row(array("",vertaalTekst("Gerealiseerde koersresultaten ",$this->pdf->rapport_taal),$this->formatGetal($gerealiseerdeKoersResultaat['fonds'],2),""));
		$this->pdf->row(array("",vertaalTekst("Gerealiseerde valutaresultaten",$this->pdf->rapport_taal),$this->formatGetal($gerealiseerdeKoersResultaat['valuta'],2),""));
			if(round($koersResulaatValutas,2) != 0.00)
			  $this->pdf->row(array("",vertaalTekst("Koersresultaten valuta's op rekening courant",$this->pdf->rapport_taal),$this->formatGetal($koersResulaatValutas,2),""));



		foreach($opbrengstenPerGrootboek as $key=>$value)
		{
			if($key=='Meegekochte rente')
				$key='Meegekochte rente obligaties';
		  if(round($value,2) != 0.00)
			  $this->pdf->row(array("",vertaalTekst($key,$this->pdf->rapport_taal),$this->formatGetal($value,2),""));
		}
		$this->pdf->row(array("",vertaalTekst("Mutatie opgelopen rente obligaties",$this->pdf->rapport_taal),$this->formatGetal($opgelopenRente,2),""));
...
//listarray($grafiekData);
      ...
      $DB->SQL($query);
		...
		$this->pdf->setY(190);
		$this->pdf->row(array("Bovenstaande rendementsberekening is uitgevoerd in overeenstemming met de internationale performancemeting-standaarden (Global Investment Performance Standards)"));
		$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
