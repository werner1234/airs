...
$Log: RapportMODEL_L73.php,v $
Revision 1.6  2020/05/02 15:57:50  rvv
*** empty log message ***

Revision 1.5  2020/03/11 16:21:41  rvv
*** empty log message ***

Revision 1.4  2017/11/11 18:23:47  rvv
*** empty log message ***

Revision 1.3  2017/11/01 16:51:06  rvv
*** empty log message ***

Revision 1.2  2017/10/08 08:15:06  rvv
*** empty log message ***

Revision 1.1  2017/10/07 16:54:34  rvv
*** empty log message ***
...
class RapportMODEL_L73
{
	...
	*/
	...
	function RapportMODEL_L73($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	  $this->pdf = &$pdf;
		$this->selectData =
	...
	function writeRapport()
	{
      ...
      $DB->Query();
		...
		while($portefeuille = $DB2->NextRecord())
		{
      ...
      {
        logScherm("Modelvergelijking voor portefeuille ".$this->portefeuille." gestopt, geen waarden voor modelportefeuille '".$portefeuille['ModelPortefeuille']."' gevonden.",true);
        logScherm("",true);
        return 1;
      }
      ...
      if($this->selectData["modelcontrole_rapport"] == "vastbedrag")
		...
		TijdelijkeRapportage.rapportageDatum = '".$einddatum."' ".$__appvar['TijdelijkeRapportageMaakUniek'];
			if($DB->QRecords($query) >9)
			{
				$query="SELECT id FROM TijdelijkeRapportage WHERE TijdelijkeRapportage.portefeuille = '".$this->portefeuille."' AND
	    TijdelijkeRapportage.beleggingscategorie='AAND' AND	TijdelijkeRapportage.rapportageDatum = '".$einddatum."' ".$__appvar['TijdelijkeRapportageMaakUniek'];
...
subverdelingVolgorde,TijdelijkeRapportage.Lossingsdatum, TijdelijkeRapportage.fondsOmschrijving asc ";
          ...
          if($aankoopStuks > 0)
            ...
            $aankoopStuks=ceil($aankoopStuks/100)*100;
				...
				if((!isset($lastCategorie) && $fdata['beleggingscategorieOmschrijving']<>'') || $fdata['beleggingscategorieOmschrijving'] <> $lastCategorie )
				{
					if($this->pdf->getY() > 179)
						$this->pdf->addPage();
					$this->printKop($fdata['beleggingscategorieOmschrijving'],'b');
				}

				if((!isset($lastSubCategorie)  && $fdata['subverdelingOmschrijving']<>'') || $fdata['subverdelingOmschrijving'] <> $lastSubCategorie)
				{
					if($this->pdf->getY() > 179)
						$this->pdf->addPage();
					$this->printKop('  '.$fdata['subverdelingOmschrijving'],'i');
				}
			  ...
			  if(round($fdata['afwijking'],2) <> 0.00 && ($aankoopStuks <> 0 || $verkoopStuks <> 0))
			  {
				...
				$lastCategorie=$fdata['beleggingscategorieOmschrijving'];
				$lastSubCategorie=$fdata['subverdelingOmschrijving'];
			}
