...
$Log: RapportVKMS_L58.php,v $
Revision 1.11  2019/02/03 15:50:24  rvv
*** empty log message ***

Revision 1.10  2019/01/17 14:34:12  rvv
*** empty log message ***

Revision 1.9  2019/01/16 16:28:10  rvv
*** empty log message ***

Revision 1.8  2019/01/16 08:41:45  rvv
*** empty log message ***

Revision 1.7  2019/01/12 17:08:31  rvv
*** empty log message ***

Revision 1.6  2019/01/10 06:49:58  rvv
*** empty log message ***

Revision 1.5  2019/01/09 15:52:19  rvv
*** empty log message ***

Revision 1.4  2018/12/19 17:00:47  rvv
*** empty log message ***

Revision 1.3  2018/01/18 13:36:02  rvv
*** empty log message ***

Revision 1.2  2017/08/16 15:58:49  rvv
*** empty log message ***

Revision 1.1  2017/05/24 15:56:56  rvv
*** empty log message ***
...
class RapportVKMS_L58
{
	function RapportVKMS_L58($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    ...
    $kostenData=array('BEH'=>array('Grootboekrekening'=>'BEH','Omschrijving'=>'Adviesvergoeding'),
                      ...
                      'BEW'=>array('Grootboekrekening'=>'BEW','Omschrijving'=>'Service fee depotbank'));
    ...
    while($data = $DB->nextRecord())
    {
      ...
      $kostenData[$data['Grootboekrekening']]['totaal']+=$data['totaal'];
        ...
        if($data['Grootboekrekening']=='BEH')
        {
          ...
          $kostenRow[] = array(vertaalTekst($data['Omschrijving'], $this->pdf->rapport_taal).' Evolf', $this->formatGetal($bedrag, 0), $this->formatGetal($bedrag/$gemiddelde*100, 2) . ' %');
...
//				$this->pdf->excelData[]=array('',$data['Omschrijving'],'',round($data['totaal'],0),round($kostenProcent,2) );
    ...
    if($this->pdfVullen==true)
    {
      ...
      $this->pdf->row(array('Incidentele kosten (performance fee)',$this->formatGetal($vkmPercentagePortefeuille*$aandeel*$gemiddelde/100, 0), $this->formatGetal($vkmPercentagePortefeuille*$aandeel, 2,true,true)));
