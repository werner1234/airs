...
function Header_basis_L109($object)
{
 $pdfObject = &$object;
    ...
    elseif ($pdfObject->rapport_type == "MODEL")
    {
  ...
  //  $pdfObject->Rect(0, 210-$hoogte, 297, $hoogte , 'F');
    ...
    $pdfObject->SetXY($pageWidth-100-$pdfObject->marge,16);
		...
		$pdfObject->MultiCell(100,4,vertaalTekst('Risicoprofiel',$pdfObject->rapport_taal).': '.$pdfObject->rapport_risicoklasse,0,'R');
    ...
    $pdfObject->AutoPageBreak=true;
		...
		{ 
    	//	  1240,117	//	  548,117
      $pdfObject->Image($pdfObject->rapport_logo,$pageWidth-38,$pageHeight-15,30);
		}
...
function HeaderFRONT_L109($object)
{

}

function HeaderINHOUD_L109($object)
{

}
  
  function HeaderSCENARIO_L109($object)
{

}

function HeaderDOORKIJKVR_L109($object)
{

}

function HeaderORDERP_L109($object)
{

}

function HeaderJOURNAAL_L109($object)
{
exit;
}

function HeaderORDERL_L109($object)
{

}

function HeaderKERNV_L109($object)
{
  $pdfObject = &$object;
  $pdfObject->ln();
  ...
  function HeaderOIB_L109($object)
	{
    $pdfObject = &$object;
	  $pdfObject->ln();
...
function HeaderGRAFIEK_L109($object)
{
  $pdfObject = &$object;
}

function HeaderOIV_L109($object)
{
  $pdfObject = &$object;
  $pdfObject->ln();
...
function HeaderRISK_L109($object)
{
	$pdfObject = &$object;
	$pdfObject->SetFont($pdfObject->rapport_font,'',$pdfObject->rapport_fontsize);
}
...
function HeaderMOD_L109($object)
{
	$pdfObject = &$object;
	$pdfObject->ln();
...
function HeaderATT_L109($object)
	{
    $pdfObject = &$object;
...
*/
  ...
  function HeaderPERFG_L109($object)
	{
    $pdfObject = &$object;
    $colwidth=(297-3-2*$pdfObject->marge)/8;
    $pdfObject->widthA = array($colwidth,$colwidth,$colwidth,$colwidth,$colwidth,$colwidth,$colwidth,$colwidth+3);
    ...
    $pdfObject->SetFont($pdfObject->rapport_font,'',$pdfObject->rapport_fontsize);
    $sumWidth = array_sum($pdfObject->widthA);
    $pdfObject->Line($pdfObject->marge,$pdfObject->GetY(),$pdfObject->marge+$sumWidth,$pdfObject->GetY());
  }
  
  	function HeaderVKM_L109($object)
	{
		$pdfObject = &$object;
		$pdfObject->HeaderVKM();
	}

function HeaderVKMS_L109($object)
{
  $pdfObject = &$object;
  $pdfObject->ln(8);
  //$pdfObject->HeaderVKM();
...
function HeaderVKMA_L109($object)
{
  $pdfObject = &$object;

}

function HeaderZORG_L109($object)
{
  $pdfObject = &$object;
  $pdfObject->ln();
  $pdfObject->SetFont($pdfObject->rapport_font,'',$pdfObject->rapport_fontsize);
...
function HeaderHSE_L109($object)
{
	$pdfObject = &$object;
	$pdfObject->HeaderHSE();
}

function HeaderEND_L109($object)
{
	$pdfObject = &$object;
	
}

function HeaderFiscaal_L109($object)
{
	$pdfObject = &$object;
	$pdfObject->HeaderFiscaal();
}

function HeaderCASHY_L109($object)
{
	$pdfObject = &$object;
	$pdfObject->SetFont($pdfObject->rapport_font,'',$pdfObject->rapport_fontsize);
}

function HeaderINDEX_L109($object)
{
	$pdfObject = &$object;
	$pdfObject->SetFont($pdfObject->rapport_font,'',$pdfObject->rapport_fontsize);
  $pdfObject->ln();
}
  function HeaderPERF_L109($object)
	{
    $pdfObject = &$object;
	  $pdfObject->SetTextColor($pdfObject->rapport_kop_fontcolor['r'],$pdfObject->rapport_kop_fontcolor['g'],$pdfObject->rapport_kop_fontcolor['b']);
		$pdfObject->SetFont($pdfObject->rapport_font,'',$pdfObject->rapport_fontsize);
  ...
  function HeaderMUT_L109($object)
	{
	  $pdfObject = &$object;
  	$pdfObject->SetFont($pdfObject->rapport_font,$pdfObject->rapport_kop_fontstyle,$pdfObject->rapport_fontsize);
		$pdfObject->ln();
    ...
    //$pdfObject->setDrawColor(0,0,0);
	  ...
	  function HeaderVOLK_L109($object,$type='VOLK')
	  {
	    $pdfObject = &$object;
      if($type=='VAR')
      {
        $dataWidth = array(68,15, 15, 11, 20, 25, 5, 23, 23, 20, 5, 17, 20, 17);// 18->30
        $splits = array(3, 5, 6, 9, 10, 13);
      }
      else
      {
        $dataWidth = array(63, 25, 11, 20, 25, 5, 24, 25, 20, 5, 18, 25, 18);// 18->30
    ...
    function HeaderVOLKNB_L109($object,$type='VOLK')
    {
      $pdfObject = &$object;
...
function HeaderVOLKD_L109($object)
{
	$pdfObject = &$object;
  ...
  $kopWidth=array();
   ...
   function HeaderTRANS_L109($object)
   {
    $pdfObject = &$object;
    $pdfObject->Ln();
  	$pdfObject->SetFont($pdfObject->rapport_font,$pdfObject->rapport_kop_fontstyle,$pdfObject->rapport_fontsize);
...
function HeaderMODEL_L109($object)
  {
...
function HeaderVHO_L109($object)
{
  $pdfObject = &$object;
  HeaderVOLK_L109($pdfObject,'VHO');
}

function HeaderVHONB_L109($object)
{
  $pdfObject = &$object;
  HeaderVOLKNB_L109($pdfObject,'VHO');
}

function HeaderVAR_L109($object)
{
  $pdfObject = &$object;
  HeaderVOLK_L109($pdfObject,'VAR');
}
...
if(!function_exists('PieChart_L109'))
{
  function PieChart_L109($pdfObject,$w,$h,$data, $format, $colors=null,$titel='',$legendaStart='')
  {
...
// standaardkleuren vervangen voor eigen kleuren.
	...
	function getFondsPerformance_L109($fonds,$beginDatum,$eindDatum)
	{
		$tag='';
		if(is_array($fonds))
		{
			...
			{
				$beginKoers = getFondsKoers_L109($fondsDetail, $beginDatum);
				$eindKoers = getFondsKoers_L109($fondsDetail, $eindDatum);
				if($eindKoers['tag']<>'')
					$tag=$eindKoers['tag'];
		...
		{
			$beginKoers = getFondsKoers_L109($fonds, $beginDatum);
			$eindKoers = getFondsKoers_L109($fonds, $eindDatum);
			if($eindKoers['tag']<>'')
				$tag=$eindKoers['tag'];
	...
	function getFondsKoers_L109($fonds,$datum)
	{
		$db=new DB();
