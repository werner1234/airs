...
class RapportVOLK_L109
{
	function RapportVOLK_L109($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	...
	function writeRapport()
  {
   ...
   // $this->pdf->rapport_fontsize-=1;
    ...
    if ($this->pdf->rapport_type=='VAR')
    {
      ...
      $this->VOLK_L109_Normaal($this->pdf->rapport_type);
    }
...
//			$procentResultaat = (($data['actuelePortefeuilleWaardeEuro'] - $data['beginPortefeuilleWaardeEuro']) / ($data['beginPortefeuilleWaardeEuro'] /100));
      ...
      $this->pdf->row(array($this->testTxtLength("  ".$data['fondsOmschrijving'],0),
    ...
    function VOLK_L109_Normaal($type)
    {
       global $__appvar;
		// haal totaalwaarde op om % te berekenen
      $DB=new DB();
