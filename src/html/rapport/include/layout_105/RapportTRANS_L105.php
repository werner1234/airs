...
class RapportTRANS_L105
{
	function RapportTRANS_L105($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
		...
		$db2 = new DB();
			...
			// voor data
		$this->pdf->widthB = array(20,20,25,65,25,25,25,25,25,25);//,19.5,19.5,19,11=69
...
//	echo "lopende -> ".$resultaatlopende." <-  voorgaande ".$resultaatvoorgaande. " -  <br>" ;
          ...
          $datum=date("d-m",db2jul($mutaties['Boekdatum']));
				...
				$this->pdf->row(array($datum,
											...
											$this->formatAantal($mutaties['Aantal'],0,$this->pdf->rapport_HSE_aantalVierDecimaal),
											$omschrijving,
											$aankoop_koers,
											$aankoop_waardeinValuta,
											$aankoop_waarde,
    ...
    $this->pdf->SetFont($this->pdf->rapport_font,$this->pdf->rapport_fontstyle,$this->pdf->rapport_fontsize);
		$this->pdf->SetTextColor($this->pdf->rapport_default_fontcolor['r'],$this->pdf->rapport_default_fontcolor['g'],$this->pdf->rapport_default_fontcolor['b']);
    $this->pdf->ln();
    $this->pdf->MultiCell(280,4, 'Eventuele transacties die in de afgelopen periode hebben plaatsgevonden en vermeld staan in deze rapportage, zijn geschikt voor u als belegger in het beleggingsprofiel '.$this->pdf->portefeuilledata['Risicoklasse'].'. Zij passen bij de doelstellingen en kenmerken van beleggers in genoemd beleggingsprofiel. Reden van de transactie kan zijn een herbalancering in de portefeuille danwel een aan- of verkoop van een belegging zijn, omdat wij denken dat die transactie in uw belang is. ', 0, "L");
