...
class RapportVAR_L105
{
	function RapportVAR_L105($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
100 as actuelePortefeuilleWaardeEuro
FROM
...
INNER JOIN BeleggingscategoriePerFonds ON IndexPerBeleggingscategorie.fonds=BeleggingscategoriePerFonds.Fonds AND BeleggingscategoriePerFonds.Vermogensbeheerder='$beheerder'
WHERE IndexPerBeleggingscategorie.Vermogensbeheerder='".mysql_real_escape_string($beheerder)."'
ORDER BY Beleggingscategorien.Afdrukvolgorde, Fondsen.Omschrijving";
    ...
    {
      $dev->setFondsen($maand['stop'],'hoofdcategorie',$fondsen);
    }
    ...
    //  $fondsPerf[$fonds]=$perf/$periode*100;
