...
/* 	
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: RapportOIBS_L17.php,v $
 		Revision 1.6  2009/01/20 17:44:09  rvv
 		*** empty log message ***
 		
 		Revision 1.5  2008/05/16 08:13:26  rvv
 		*** empty log message ***
...
class RapportOIBS_L17
{
	function RapportOIBS_L17($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	  ...
	  foreach ($sortedData as $type=>$typeData)
	  {
	    ...
	    foreach ($typeData as $cat=>$catData)
	    {
	      ...
	      foreach ($catData as $data)
	      {
	        ...
	        if($type == 'rente' && $this->pdf->rapport_OIS_geenrentespec == 1)
	        {
	          //Toon geen regels	          
	        }
	        else
		...
		{
			echo "<script>
			alert('Fout : Fout in rapport ".$this->portefeuille.", totale waarde (".round($totaalWaarde,2).") komt niet overeen met afgedrukte totaal (".round($totalen['waardeEur'],2).") in rapport ".$this->pdf->rapport_type."');
			</script>";
			ob_flush();
		}
