...
$Log: RapportPERFG_L61.php,v $
Revision 1.7  2017/11/25 20:25:19  rvv
*** empty log message ***

Revision 1.6  2017/11/22 17:03:24  rvv
*** empty log message ***

Revision 1.5  2017/09/06 16:29:58  rvv
*** empty log message ***

Revision 1.4  2017/08/02 18:23:27  rvv
*** empty log message ***

Revision 1.3  2017/07/29 17:18:20  rvv
*** empty log message ***

Revision 1.2  2017/07/24 05:45:27  rvv
*** empty log message ***

Revision 1.1  2017/07/23 13:36:28  rvv
*** empty log message ***

Revision 1.6  2016/10/30 13:02:59  rvv
*** empty log message ***

Revision 1.5  2016/10/29 15:41:46  rvv
*** empty log message ***

Revision 1.4  2015/10/14 16:12:05  rvv
*** empty log message ***
...
class RapportPERFG_L61
{
	function RapportPERFG_L61($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//echo substr($this->pdf->PortefeuilleStartdatum,0,10) ;exit;
		/*
		if($this->pdf->portefeuilledata['startdatumMeerjarenrendement'] <> '' && $this->pdf->portefeuilledata['startdatumMeerjarenrendement'] <> '0000-00-00')
			$startdatum=$this->pdf->portefeuilledata['startdatumMeerjarenrendement'];
		else
			$startdatum=substr($this->pdf->PortefeuilleStartdatum,0,10);
		*/
		$DB = new DB();
		$query = "SELECT id,Datum FROM HistorischePortefeuilleIndex WHERE  Portefeuille='".$this->portefeuille."' AND  Categorie = 'Totaal' ORDER BY datum limit 1"; // Portefeuille = '".$this->portefeuille."' AND
		$DB->SQL($query);
		$DB->Query();
		$datum = $DB->nextRecord();
		...
		{
			$start = substr($datum['Datum'],0,7).'-01';
		}
		else
		{
  ...
  $indexData = $index->getWaarden($start,$this->rapportageDatum ,$this->portefeuille);
  $jaren=ceil(count($indexData)/12);
...
//listarray($this->pdf->rapport_kop_bgcolor);exit;
        ...
        $totaalRendamentIndex=100;
		    foreach ($rendamentWaarden as $row)
				{
					...
					if(!isset($jaarwaarden[$jaar]['waardeBegin']))
					  $jaarwaarden[$jaar]['waardeBegin']=$row['waardeBegin'];
					foreach($somvelden as $veld)
						$jaarwaarden[$jaar][$veld]+=$row[$veld];

					foreach($stapelvelden as $veld)
						$jaarwaarden[$jaar][$veld]=((1+$jaarwaarden[$jaar][$veld]/100)*(1+$row['performance']/100)-1)*100;

					$jaarwaarden[$jaar]['waardeHuidige']=$row['waardeHuidige'];
					$jaarwaarden[$jaar]['index']=$row['index'];
					$jaarwaarden[$jaar]['datum']=$row['datum'];
					$jaarwaarden[$jaar]['extra']=$row['extra'];
		    ...
		    foreach ($jaarwaarden as $jaar=>$row)
		    {
					...
					$grafiekData['Datum'][] = $row['datum'];
					$grafiekData['Index'][] = $row['index']-100;
					$grafiekData['benchmarkIndex'][] = $row['specifiekeIndex']-100;
						$barGraph['Index'][$row['datum']]['leeg']=0;
					foreach ($row['extra']['cat'] as $categorie=>$waarde)
					{
						if($categorie=='LIQ')
							$categorie='Liquiditeiten';

						if(!isset($barGraph['Index'][$row['datum']][$categorie]) || $barGraph['Index'][$row['datum']][$categorie]==0)
							$barGraph['Index'][$row['datum']][$categorie] = $waarde/$row['waardeHuidige']*100;
						if($waarde <> 0)
							$categorien[$categorie]=$categorie;
					}
...
*/
  ...
  function VBarDiagram($w, $h, $data)
  {
     ...
     // $numBars = count($legenda); echo $numBars;exit;
    //  $numBars=10;

		if(count($legenda)>12)
			$numBars=count($legenda)-2;
		else
			$numBars=10;
