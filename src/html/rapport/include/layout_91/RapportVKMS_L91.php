...
$Log: RapportVKMS_L91.php,v $
Revision 1.1  2020/07/25 15:37:41  rvv
*** empty log message ***

Revision 1.6  2019/02/03 15:50:24  rvv
*** empty log message ***

Revision 1.5  2019/01/19 13:54:10  rvv
*** empty log message ***

Revision 1.4  2019/01/16 08:41:44  rvv
*** empty log message ***

Revision 1.3  2018/04/04 15:46:58  rvv
*** empty log message ***

Revision 1.2  2018/03/28 15:53:44  rvv
*** empty log message ***

Revision 1.1  2018/03/10 18:24:22  rvv
*** empty log message ***
...
class RapportVKMS_L91
{
	function RapportVKMS_L91($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
...
//			$this->pdf->excelData[]=array('','Gemiddeld vermogen','','',round($gemiddelde,0),'EUR');
			...
			$this->pdf->row(array('','EUR', vertaalTekst('Percentage',$this->pdf->rapport_taal)));
			$this->pdf->row(array(vertaalTekst('Indirecte (fonds)kosten',$this->pdf->rapport_taal),$this->formatGetal($totaalDoorlopendekosten, 0), $this->formatGetal($vkmPercentagePortefeuille, 2) . ' %'));
			$barData=array();
			...
			$this->pdf->row(array(vertaalTekst('Directe kosten vanaf',$this->pdf->rapport_taal).' ' . date('d-m-Y', db2jul($this->vanafDatum)), '', ''));
		...
		while($data = $DB->nextRecord())
		{
			if($data['Omschrijving']=='Beheervergoeding')
				$data['Omschrijving']='Beheervergoeding (inclusief '.$this->formatGetal($btw['BeheerfeeBTW'],1,true).' BTW)';
			if($this->pdfVullen==true)
			{
    ...
    if($spreadKostenEUR <> 0)
		...
		if($this->pdfVullen==true)
		{
			...
			$this->pdf->row(array(vertaalTekst('Totale kosten (vergelijkende kostenmaatstaf)',$this->pdf->rapport_taal),$this->formatGetal($vkmWaarde*$gemiddelde/100, 0), $this->formatGetal($vkmWaarde, 2).' %'));
			$this->pdf->excelData[]=array('','Vergelijkende kostenmaatstaf','', round($vkmWaarde*$gemiddelde/100,2),round($vkmWaarde, 2));
			$this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
      $this->pdf->ln();
      $this->pdf->row(array(vertaalTekst(  "Het overzicht van de gerealiseerde kosten over het afgelopen jaar geeft een goede indicatie van de te verwachten kosten voor het komende jaar.")));
...
foreach ($perCategorie as $hoofdcategorie=>$categorieData)
{
 ...
 $perfTotaal['percentageIndirectVermogenMetKostenfactor']=$percentageIndirectVermogenMetKostenfactor;
      ...
      $keys=array('Doorlopende kosten','Beheervergoeding (inclusief 21,0% BTW)');
      foreach($keys as $key)
      {
