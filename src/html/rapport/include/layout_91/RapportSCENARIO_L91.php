...
class RapportSCENARIO_L91
{
	function RapportSCENARIO_L91($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function formatGetalKoers($waarde, $dec , $start = false)
    ...
    if(!isset($this->crmId))
    {
      $query="SELECT id,maximaalRisicoprofiel FROM CRM_naw WHERE portefeuille='".$this->portefeuille."'";
 	  	$DB->SQL($query);
	  	$DB->Query();
		  $crmId = $DB->nextRecord();   
    }
    ...
    {
      $crmId['id'] = $this->crmId;
      $crmId['maximaalRisicoprofiel'] = $this->maximaalRisicoprofiel;
    }
    ...
    if($sc->CRMdata['doelvermogen']==0 && $this->gewenstRisicoprofiel=='')
    {
      if($crmId['maximaalRisicoprofiel']<>'')
        $this->gewenstRisicoprofiel=$crmId['maximaalRisicoprofiel'];
      $sc= new scenarioBerekening($crmId['id'],$this->gewenstRisicoprofiel);
      //listarray($this->gewenstRisicoprofiel);listarray($sc->profieldata);exit;
      ...
      $sc->berekenDoelKans();
      $sc->berekenVerdeling();
      $doelvermogen=$sc->verwachteWaarden['Normaal'];
      $sc= new scenarioBerekening($crmId['id'],$this->gewenstRisicoprofiel);
...
//
  ...
  //  $this->pdf->row(array('',vertaalTekst('Conclusies',$this->pdf->rapport_taal)));
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
    $this->pdf->row(array('',vertaalTekst('Kans op Scenario-vermogen',$this->pdf->rapport_taal),$this->formatGetal($sc->doelKans,0).'%'));
...
*/
   ...
   // listarray($polly);
    foreach($pollyReverse as $value)
      $polly[]=$value;
   // listarray($polly);
    $this->pdf->Polygon($polly, 'F', null, array(255,255,255)) ;
    ...
    $this->pdf->TextWithRotation($XDiag+2,$YDiag+$h+10+1,vertaalTekst('Scenario-vermogen',$this->pdf->rapport_taal),0);
