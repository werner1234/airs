...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: PDFRapport_headers_L91.php,v $
...
function Header_basis_L91($object)
{
 $pdfObject = &$object;
	  ...
	  if($pdfObject->rapport_type != "VAR")
    {
      ...
      $voetTekst="| ".($pdfObject->rapport_depotbankOmschrijving<>''?$pdfObject->rapport_depotbankOmschrijving." | ":'').
        ($pdfObject->portefeuilledata['Portefeuille']<>''?$pdfObject->portefeuilledata['Portefeuille']." | ":'').
       // str_replace('SEQ ','Sequoia ',$pdfObject->portefeuilledata['SoortOvereenkomst'])." | ".
        ($pdfObject->portefeuilledata['Risicoklasse']<>''?$pdfObject->portefeuilledata['Risicoklasse']." |":'')." Productiedatum ".date('d-m-Y')." |";
	...
	function HeaderVKM_L91($object)
	{
		$pdfObject = &$object;
		$pdfObject->HeaderVKM();
	}

function HeaderFRONT_L91($object)
{
  $pdfObject = &$object;
}

function HeaderFRONT1_L91($object)
{
  $pdfObject = &$object;
}

function HeaderFISCAAL_L91($object)
{
  $pdfObject = &$object;
...
function HeaderJOURNAAL_L91($object)
{
  $pdfObject = &$object;
  $pdfObject->ln();
}

function HeaderVAR_L91($object)
{
  $pdfObject = &$object;
	$pdfObject->ln();
}

function HeaderVOLKD_L91($object)
{
  HeaderVOLK_L91($object);
}
function HeaderVOLK_L91($object)
{
    $pdfObject = &$object;
		$pdfObject->ln();
...
function HeaderHUIS_L91($object)
{
    $pdfObject = &$object;
   ...
   function HeaderPERF_L91($object)
  {
	  	$pdfObject = &$object;
...
function HeaderKERNZ_L91($object)
{
  $pdfObject = &$object;
...
function HeaderATT_L91($object)
   ...
   function HeaderTRANS_L91($object)
  {
    $pdfObject=&$object;
    $pdfObject->ln();
		$pdfObject->SetFont($pdfObject->rapport_font,$pdfObject->rapport_kop_fontstyle,$pdfObject->rapport_fontsize);
  ...
  function HeaderSCENARIO_L91($object)
  {
    $pdfObject=&$object;
  }
function HeaderEND_L91($object)
{
  $pdfObject=&$object;
}

  function HeaderMUT2_L91($object)
  {
    $pdfObject=&$object;
    $pdfObject->SetTextColor($pdfObject->rapport_kop_fontcolor['r'],$pdfObject->rapport_kop_fontcolor['g'],$pdfObject->rapport_kop_fontcolor['b']);
  ...
  function HeaderOIH_L91($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->HeaderOIH();
	}

	function HeaderOIBS_L91($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->HeaderOIBS();
	}

	function HeaderOIR_L91($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->HeaderOIR();
	}

	function HeaderHSE_L91($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->HeaderHSE();
	}

	function HeaderOIB_L91($object)
	{
  	  $pdfObject = &$object;
	...
	function HeaderOIV_L91($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->HeaderOIV();
	}

	function HeaderPERFG_L91($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->HeaderPERFG();
	}
	function HeaderVHO_L91($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->ln();
  	  $pdfObject->HeaderVHO();
	}
	function HeaderGRAFIEK_L91($object)
	{
  	  $pdfObject = &$object;
	...
	function HeaderCASH_L91($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->HeaderCASH();
	}
	function HeaderCASHY_L91($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->ln();
	...
	function HeaderMODEL_L91($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->HeaderMODEL();
	}
	function HeaderSMV_L91($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->HeaderSMV();
	}
	...
	function HeaderRISK_L91($object)
	{
  	  $pdfObject = &$object;
  	//  $pdfObject->Line($pdfObject->marge,$pdfObject->GetY()+7,$pdfObject->marge + 283,$pdfObject->GetY()+7);
	}

  function HeaderDOORKIJK_L91($object)
  {
    $pdfObject = &$object;
  }
