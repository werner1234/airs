...
include_once("rapport/include/layout_86/ATTberekening_L86.php");

class RapportPERF_L86
{

	function RapportPERF_L86($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//listarray($this->waarden['Periode']);
 ...
 // listarray($this->pdf->portefeuilles);
  ...
  $beginwaardeXls=array(vertaalTekst("Waarde portefeuille per",$this->pdf->rapport_taal)." ".date("j",db2jul($this->rapportageDatumVanaf))." ".vertaalTekst($this->pdf->__appvar["Maanden"][date("n",db2jul($this->rapportageDatumVanaf))],$this->pdf->taal)." ".date("Y",db2jul($this->rapportageDatumVanaf)));
  $eindwaardeXls=array(vertaalTekst("Waarde portefeuille per",$this->pdf->rapport_taal)." ".date("j",db2jul($this->rapportageDatum))." ".vertaalTekst($this->pdf->__appvar["Maanden"][date("n",db2jul($this->rapportageDatum))],$this->pdf->taal)." ".date("Y",db2jul($this->rapportageDatum)));
...
foreach($categorien as $categorie)
{
  ...
  foreach($categorien as $categorie)
  {
    ...
    $beginwaardeXls[]=$perfWaarden['beginwaarde'];
    $perbegin[]='';
    $waardeRapdatum[]=$this->formatGetal($perfWaarden['eindwaarde'],0,true);
    $eindwaardeXls[]=$perfWaarden['eindwaarde'];
    $waardeRapdatum[]='';
  ...
  $this->pdf->excelData[]=$beginwaardeXls;
  $this->pdf->excelData[]=$eindwaardeXls;
