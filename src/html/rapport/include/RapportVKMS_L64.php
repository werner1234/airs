...
Revision 1.2  2019/01/16 08:41:45  rvv
*** empty log message ***

Revision 1.1  2019/01/05 10:56:46  rvv
*** empty log message ***
...
class RapportVKMS_L64
{
	function RapportVKMS_L64($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
...
foreach ($perCategorie as $hoofdcategorie=>$categorieData)
{
 ...
 WHERE Portefeuilles.portefeuille='".$this->portefeuille."'";
    ...
    $this->pdf->SetY(210-12);
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_voetfontsize);
    $this->pdf->MultiCell(297-$this->pdf->marge*2+1,4,vertaalTekst('Een verdere uitsplitsing van de kosten is opvraagbaar bij uw vermogensbeheerder.',$this->pdf->rapport_taal),0,'L');
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
