...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: RapportPERFG_L37.php,v $
 		Revision 1.3  2012/05/30 16:02:38  rvv
 		*** empty log message ***
 		
 		Revision 1.2  2012/05/27 08:33:10  rvv
 		*** empty log message ***
...
class RapportPERFG_L37
{

  function RapportPERFG_L37($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		...
		if($this->pdf->rapport_PERFGRAFIEK_titel)
			...
			$this->pdf->rapport_titel = vertaalTekst("Historisch rendement vanaf aanvang portefeuille",$this->pdf->rapport_taal);
...
$this->pdf->AddPage();
$this->pdf->templateVars['PERFGPaginas']=$this->pdf->customPageNo;
...
$tmpArray0 = array('','');
$tmpArray1 = array('',vertaalTekst("Jaar",$this->pdf->rapport_taal));

foreach ($indexTabelFondsen as $fonds)
{

  array_push($tmpArray0,($indexNaam[$fonds] <> ""?"benchmark":$fonds));
  array_push($tmpArray1,vertaalTekst("per jaar",$this->pdf->rapport_taal));
  array_push($tmpArray1,vertaalTekst("cumu.",$this->pdf->rapport_taal));
}
$this->pdf->setY(55);
$this->pdf->Row(array(''));
