...
class RapportOIH_L70
{
	function RapportOIH_L70($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	  global $__appvar;
		$this->pdf = &$pdf;
	...
	function addDoorkijk()
  {
	...
	function writeRapport()
	{
		...
		$this->pdf->AddPage();
    $this->pdf->templateVars['OIHPaginas'] = $this->pdf->customPageNo;
		$this->pdf->templateVarsOmschrijving['OIHPaginas'] = $this->pdf->rapport_titel;
    ...
    foreach ($categorieData as $categorie=>$fondsData)
    {
      ...
      foreach ($fondsData['fondsen'] as $id=>$fonds)
      {
        ...
        if(round($data['beginwaarde'],2) <> 0 || round($data['eindwaarde'],2) <> 0 || round($data['stort'],2) <> 0 || round($data['resultaat'],2) <> 0 || round($data['gemWaarde'],2) <> 0 )
        {
          ...
          $this->pdf->fillCell = array($this->pdf->cellColorLight,$this->pdf->cellColorLight,$this->pdf->cellColorDark,$this->pdf->cellColorLight,$this->pdf->cellColorDark,$this->pdf->cellColorLight,$this->pdf->cellColorDark,$this->pdf->cellColorLight,$this->pdf->cellColorDark,$this->pdf->cellColorLight,$this->pdf->cellColorDark,$this->pdf->cellColorLight,$this->pdf->cellColorDark,$this->pdf->cellColorLight);
