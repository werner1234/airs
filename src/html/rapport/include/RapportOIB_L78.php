...
class RapportOIB_L78
{
  function RapportOIB_L78($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
  {
    $this->pdf = &$pdf;
...
//$this->pdf->setXY(65,40);
    $grafiekKleuren['beleggingscategorieBegin'] = $this->printPie($data['beleggingscategorieBegin']['pieData'], $data['beleggingscategorieBegin']['kleurData'], 'Categorieverdeling ' . date("d-m-Y", db2jul($rapportageDatumVanaf)), 60, 60);
    $this->pdf->wLegend = 0;
    $this->pdf->setXY(120, 40);
//$this->pdf->setXY(175,40);
    $grafiekKleuren['beleggingscategorieEind'] = $this->printPie($data['beleggingscategorieEind']['pieData'], $data['beleggingscategorieEind']['kleurData'], 'Categorieverdeling ' . date("d-m-Y", db2jul($rapportageDatum)), 60, 60);
    $this->pdf->wLegend = 0;
    
    $this->pdf->setXY(210, 40);
    $grafiekKleuren['valutaVerdeling'] = $this->printPie($data['valutaVerdeling']['pieData'], $data['valutaVerdeling']['kleurData'], 'Valutaverdeling ' . date("d-m-Y", db2jul($rapportageDatum)), 60, 60);
    ...
    $this->pdf->SetWidths(array(15, 38, 22, 15, 15, 38, 22, 15, 15, 38, 22, 15));
...
//$this->pdf->CellBorders = array('','','UU','UU','','','UU','UU','','','UU','UU');
    ...
    for ($i = 0; $i < 3; $i++)
    {
      $this->pdf->Rect($this->pdf->marge + 15 + $i * 90, $this->pdf->GetY() - 1, 75, 6, 'F');
    }
    ...
    foreach ($regelData as $index => $regel)
    {
  
      foreach ($grafiekKleuren as $type => $grafiekKleurenData)
      {
        if ($type == 'beleggingscategorieBegin')
        {
          $xOffset = 15;
        }
        elseif ($type == 'beleggingscategorieEind')
        {
          $xOffset = 15+ 38+ 22+ 15+ 15;
        }
        elseif ($type == 'valutaVerdeling')
        {
          $xOffset = 15+38+ 22+ 15+ 15+ 38+ 22+ 15+ 15;
        }
  
        if(isset($grafiekKleurenData[$index]))
        {
          $hLegend = 2;
          $x1 = 5 + $xOffset;
          ...
          $this->pdf->SetFillColor($grafiekKleurenData[$index][0], $grafiekKleurenData[$index][1], $grafiekKleurenData[$index][2]);
    ...
    function printPie($pieData, $kleurdata, $title = '', $width = 100, $height = 100)
    {
      ...
      if (isset($kleurdata))
      {
        ...
        foreach ($kleurdata as $key => $value)
        {
          ...
          $a++;
          $tabelLegenda[] = $grafiekKleuren[count($grafiekKleuren) - 1];
        }
      }
      else
      {
