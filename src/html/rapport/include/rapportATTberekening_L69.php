...
$Log: rapportATTberekening_L69.php,v $
...
class rapportATTberekening_L69
{

	function rapportATTberekening_L69( $pdata )
	{
	  global $__appvar;
	  $this->db = new DB();
	  if(!is_array($pdata))
    ...
    $this->perfSimpeleDeling=array();
    $this->beginTransactieWaarde=array();
  ...
  function attributiewaardenViaGrootboek($vanaf,$tot,$valuta)
  {
...
GROUP BY Rekeningmutaties.Grootboekrekening,BeleggingssectorPerFonds.AttributieCategorie";
    ...
    {
      if($attributieGrootboek['AttributieCategorie']=='' && (1))
  ...
  function getAttributieStortingen($van, $tot, $valuta = 'EUR' )
  {
   ...
   foreach ($this->categorien as $categorie)
   {
     $van = $vanOrigineel;
      ...
      if($categorie != 'Totaal' )
      {
        if($this->pdata['Vermogensbeheerder'] == 'WAT1' || $this->pdata['Vermogensbeheerder'] == 'WAT' || 1)
...
*/
	  ...
	  if($categorie == 'Totaal')
	  {
  	  ...
  	  //rvv test
  	  if($this->pdata['Vermogensbeheerder'] == 'WAT' || $this->pdata['Vermogensbeheerder'] == 'WAT1' || 1)
  	  {
  	   if(round($this->performance[$this->periodeId]['totaalWaarde'][$categorie]['begin'] ,2) == 0)
...
//	echo $periode['start']." ".substr($periode['start'],5,5 )." $startjaar<br>\n";
    ...
    if(!is_array($this->performance[$periodeId]['totaalWaarde']))
    {
      ...
      $query = "SELECT Rekeningmutaties.Fonds,
	    BeleggingssectorPerFonds.AttributieCategorie ".
	  	...
	  	LEFT Join BeleggingssectorPerFonds ON Rekeningmutaties.Fonds = BeleggingssectorPerFonds.Fonds AND BeleggingssectorPerFonds.Vermogensbeheerder='".$this->pdata['Vermogensbeheerder']."'
	  	WHERE ".
	  	"Rekeningmutaties.Rekening = Rekeningen.Rekening AND ".
	  	...
	  	"Rekeningmutaties.Boekdatum <= '".$periode['stop']."' ".
	  	...
	  	{
		  	$this->performance[$periodeId]['fondsen'][$mutaties['AttributieCategorie']][$mutaties['Fonds']]=$mutaties['Fonds'];
		  }
	...
	{
	   $this->performance[$periodeId]['attributiewaardenViaGrootboek'] = $this->attributiewaardenViaGrootboek($periode['start'],$periode['stop'],$valuta);
	}
...
//echo $this->liquiditeitenCategorie."<br>\n";listarray($grootboekOpbrengstTotaal);listarray($grootboekKostenTotaal);listarray($grootboekKostenOpbrengstTotaalGewogen);exit;
  ...
  {
    if($this->perfSimpeleDeling[$categorie]==true)
    {
      $performance=$AttributieStortingenOntrekkingen/ $this->beginTransactieWaarde[$categorie] * 100;
      //echo "$categorie $performance=$AttributieStortingenOntrekkingen/ ".$this->beginTransactieWaarde[$categorie]." * -100; <br>\n";
      $this->perfSimpeleDeling[$categorie]=false;
    }
    else
 ...
 //echo $query;
  ...
  { 
    ...
    $beginCorrectie=false;
	    ...
	    {
	      $query = "SELECT Rekeningmutaties.Boekdatum - INTERVAL 1 DAY as Boekdatum,
        (ABS(Rekeningmutaties.Credit) * Rekeningmutaties.Valutakoers ) - (ABS(Rekeningmutaties.Debet) * Rekeningmutaties.Valutakoers ) $koersQueryBoekdatum as waarde
        FROM  (Rekeningen, Portefeuilles)
	      ...
	      $this->beginTransactieWaarde[$categorie] = $start['waarde'];
        $beginCorrectie=true;
	    }
      ...
      {
        $this->perfSimpeleDeling[$categorie]=true;
      }
