...
$Log: RapportHSE_L42.php,v $
Revision 1.6  2017/08/12 12:17:24  rvv
*** empty log message ***

Revision 1.5  2017/07/29 17:18:20  rvv
*** empty log message ***

Revision 1.4  2015/05/02 14:57:32  rvv
*** empty log message ***

Revision 1.3  2014/10/25 14:39:09  rvv
*** empty log message ***

Revision 1.2  2014/08/02 15:25:09  rvv
*** empty log message ***
...
class RapportHSE_L42
{
	function RapportHSE_L42($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	  global $__appvar;
		$this->pdf = &$pdf;
    ...
    $this->pdf->excelData[]=array("Fondsnaam","Aantal","Bewaarder","Valuta","Koers","","Kostprijs","Bruto rendement","Marktwaarde","Kostprijs","Netto resultaat","Netto rendement","Weging");
  ...
  function ordersTonen()
  {
    ...
    $DB1 = new DB();
    		$query = "SELECT SUM(actuelePortefeuilleWaardeEuro) /".$this->pdf->ValutaKoersEind."  AS totaal ".
						 "FROM TijdelijkeRapportage WHERE ".
						 " rapportageDatum ='".$this->rapportageDatum."' AND ".
						 " portefeuille = '".$this->portefeuille."' AND type='rekening'"
						 .$__appvar['TijdelijkeRapportageMaakUniek'];
		debugSpecial($query,__FILE__,__LINE__);
		$DB1->SQL($query); 
		$DB1->Query();
		$totaalWaarde = $DB1->nextRecord();
		$totaalLiquiditeitenEuro = $totaalWaarde['totaal'];
...
(SELECT Fondskoersen.Koers FROM Fondskoersen WHERE Fondskoersen.Fonds=Orders.Fonds order by datum desc limit 1) as FondskoersKoersen
FROM
OrderRegels
...
ORDER BY Orders.fonds"; 
    ...
    $this->pdf->SetWidths(array(60,20,20,30));
    ...
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
		...
		{
		  if($data['Fondskoers']=='')
        $data['Fondskoers']=$data['FondskoersKoersen'];
      ...
      if ($data['transactieSoort'] == 'V' || $data['transactieSoort'] == 'VO' || $data['transactieSoort'] == 'VS')
       ...
       $geschatteOrdersEuro +=  $waardeEuro; 
		               
			}
		$this->pdf->ln();		
    $this->pdf->Row(array(vertaalTekst("Geschatte liquiditeiten na lopende orders"),'','',
			                        $this->formatGetal($totaalLiquiditeitenEuro-$geschatteOrdersEuro,2)		          
			                        ));
