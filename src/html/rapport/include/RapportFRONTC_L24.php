...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2016/01/17 18:09:57 $
File Versie					: $Revision: 1.1 $

$Log: RapportFRONTC_L24.php,v $
Revision 1.1  2016/01/17 18:09:57  rvv
*** empty log message ***
...
class RapportFrontC_L24
{
	function RapportFrontC_L24($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function FrontPagina()
  {
    ...
    if(is_file($this->pdf->rapport_logo))
      $this->pdf->Image($this->pdf->rapport_logo, 297/2-43/2, 5, 43, 15);	
    ...
    $portefeuilleString='';//listarray($this->pdf->portefeuilles);
    foreach($this->pdf->portefeuilles as $portefeuille)
    {
      if($portefeuilleString <> '')
        $portefeuilleString.= " en ";
	  	$oldPortefeuilleString = $portefeuille;
	    $i=1;
  ...
  function voorBrief()
  {
    ...
    if(is_file($this->pdf->rapport_logo))
      $this->pdf->Image($this->pdf->rapport_logo, 85, 5, 43, 15);	
	  $this->pdf->frontPage = true;
    ...
    $this->pdf->row(array('','','OHV Vermogensmeheer'));
	  $this->pdf->ln(10);
    ...
    $this->pdf->row(array('',"Bijgaand ontvangt u de kwartaalrapportage per ultimo ".vertaalTekst($__appvar["Maanden"][$this->rapportMaand],$pdf->rapport_taal)." ".$this->rapportJaar." betreffende uw effectenportefeuille.
...
"));
    ...
    $this->pdf->row(array('',$portefeuilledata['VermogensbeheerderNaam']."\nIngeschreven bij de K.v.K te breda onder nr.20130163"));
