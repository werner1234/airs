...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2018/10/07 13:39:46 $
File Versie					: $Revision: 1.4 $

$Log: RapportTRANS_L77.php,v $
Revision 1.4  2018/10/07 13:39:46  rvv
*** empty log message ***
...
class RapportTRANSFEE_L77
{
	function RapportTRANSFEE_L77($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	...
	function writeRapport()
	{
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
		...
		// voor data
		$this->pdf->widthB = array(15,10,20,70,20,1,26,20,1,26,22,22,22,6);
		$this->pdf->alignB = array('L','L','R','L','R','R','R','R','R','R','R','R','R','R');
		// voor kopjes
		$this->pdf->widthA = $this->pdf->widthB;//array(15,10,20,58,16,1,28,16,2,32,22,22,22,12);
    ...
    $lastFonds='';
      ...
      $t_aankoop_waardeinValuta=0;
			...
			if($lastFonds<>'' && $mutaties['Fonds'] <> $lastFonds)
      {
        //listarray($totalen[$lastFonds]);
        $this->pdf->fillCell=array();
        if($totalen[$lastFonds]['regels']>1)
        {
          $this->pdf->row(array('','','',
                            'Totaal',
                            '',
                            '',
                            $this->formatGetal($totalen[$lastFonds]['aankoop_waarde'],0),
                            $verkoop_koers,
                            '',
                            $this->formatGetal($totalen[$lastFonds]['verkoop_waarde'],0),
                            $this->formatGetal($totalen[$lastFonds]['historischekostprijs'],0),
                            $this->formatGetal($totalen[$lastFonds]['resultaatvoorgaande'],0),
                            $this->formatGetal($totalen[$lastFonds]['resultaatlopende'],0)));
...
//	echo "lopende -> ".$resultaatlopende." <-  voorgaande ".$resultaatvoorgaande. " -  <br>" ;
			...
			$this->pdf->SetFont($this->pdf->rapport_font,$this->pdf->rapport_fontstyle,$this->pdf->rapport_fontsize);
			if($this->pdf->GetStringWidth($mutaties['Omschrijving']) > $this->pdf->widths[3]-1)
				$omschrijving=substr($mutaties['Omschrijving'],0,40)."...";
			else
      ...
      $totalen[$mutaties['Fonds']]['regels']+=1;
			$totalen[$mutaties['Fonds']]['aankoop_waarde']+=$t_aankoop_waarde;
      $totalen[$mutaties['Fonds']]['verkoop_waarde']+=$t_verkoop_waarde;
      $totalen[$mutaties['Fonds']]['historischekostprijs']+=$historischekostprijs;
      $totalen[$mutaties['Fonds']]['resultaatvoorgaande']+=$resultaatvoorgaande;
      $totalen[$mutaties['Fonds']]['resultaatlopende']+=$resultaatlopende;
