...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2014/01/18 17:27:23 $
File Versie					: $Revision: 1.9 $

$Log: RapportATT_L39.php,v $
...
class RapportATT_L39
{
	function RapportATT_L39($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
       ...
       $decimalen=2;
          ...
          $this->pdf->CellBorders = array();
		      $this->pdf->row(array(date("Y",$datum).' '.vertaalTekst($__appvar["Maanden"][date("n",$datum)],$this->pdf->rapport_taal) ,
		                           $this->formatGetal($row['waardeBegin'],$decimalen),
		                           $this->formatGetal($row['stortingen']-$row['onttrekkingen'],$decimalen),
		                           $this->formatGetal($row['gerealiseerd']+$row['ongerealiseerd'],$decimalen),
		                           $this->formatGetal($row['opbrengsten'],$decimalen),
		                           $this->formatGetal($row['kosten'],$decimalen),
		                           $this->formatGetal($row['rente'],$decimalen),
		                           $this->formatGetal($row['resultaatVerslagperiode'],$decimalen),
		                           $this->formatGetal($row['waardeHuidige'],$decimalen),
        ...
        $this->pdf->CellBorders = array();
		    $this->pdf->row(array(vertaalTekst('Totaal',$this->pdf->rapport_taal),
		                           $this->formatGetal($waardeBegin,$decimalen),
		                           $this->formatGetal($totaalStortingenOntrekkingen,$decimalen),
		                           $this->formatGetal($totaalGerealiseerd+$totaalOngerealiseerd,$decimalen),
		                           $this->formatGetal($totaalOpbrengsten,$decimalen),
		                           $this->formatGetal($totaalKosten,$decimalen),
		                           $this->formatGetal($totaalRente,$decimalen),
		                           $this->formatGetal($totaalResultaat,$decimalen),
		                           $this->formatGetal($totaalWaarde,$decimalen),
		                           '',
