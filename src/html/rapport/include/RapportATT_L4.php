...
class RapportATT_L4
{
	...
	function RapportATT_L4($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    $this->att=new ATTberekening_L4($this);
...
foreach($categorien as $categorie=>$categorieOmschrijving)
{
  ...
  foreach($categorien as $categorie=>$categorieOmschrijving)
  {
    ...
    else
    {
      ...
      if($categorie=='Liquiditeiten')
      {
        $opbrengsten[]=$this->formatGetal($perfWaarden['resultaat']-($perfWaarden['kosten']+$perfWaarden['extraKosten']),0);
      }
      else
      {
