...
class RapportTRANSFEE_L35
{
	function RapportTRANSFEE_L35($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
SUM(Rekeningmutaties.Debet) AS Debet,
SUM(Rekeningmutaties.Credit) AS Credit,
SUM((Rekeningmutaties.Credit-Rekeningmutaties.Debet)*Rekeningmutaties.Valutakoers)/SUM(Rekeningmutaties.Credit-Rekeningmutaties.Debet) AS Valutakoers,
...
GROUP BY Rekeningmutaties.Transactietype, Rekeningmutaties.Boekdatum, Rekeningmutaties.Fonds
...
/*
			if($mutaties['rekeningOmschrijving']=='Koop Effecten'||$mutaties['rekeningOmschrijving']=='Verkoop Effecten'||$mutaties['rekeningOmschrijving']=='Stockdividend')
				$mutaties['rekeningOmschrijving']=vertaalTekst($mutaties['rekeningOmschrijving'],$this->pdf->rapport_taal)." ". $mutaties['Omschrijving'];
			if($this->pdf->getStringWidth($mutaties['rekeningOmschrijving'])>$this->pdf->widths[0])
			{
				$mutaties['rekeningOmschrijving']=substr($mutaties['rekeningOmschrijving'],0,40)."...";
			}*/
			if($transactietype=='A')
      {
