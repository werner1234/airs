...
Revision 1.3  2016/10/23 11:32:33  rvv
*** empty log message ***

Revision 1.2  2015/10/11 16:51:46  rvv
*** empty log message ***

Revision 1.1  2014/09/13 14:38:35  rvv
*** empty log message ***
...
class RapportSCENARIO_L76
{
	function RapportSCENARIO_L76($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//    {
      ...
      $this->pdf->widthA = array(175,30);
		  $this->pdf->widthB = array(145,35,25,25,25,25);
      ...
      $this->pdf->row(array('','Scenario','Defensief','Neutraal','Offensief'));
      $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
      $negatiefAdvies=true;
      $maxKansTmp=0;
      $kansData=$sc->berekenKansBijOpgehaaldeRisicoklassen();
     // listarray($kansData['risicoklassen']);
      $scenarioTabel=array();
      foreach($kansData['risicoklassen'] as $risicoklasse=>$klasseData)
      {
        //$scenarioTabel['Kans'][$risicoklasse]=$klasseData['uitkomstKans']['kans'];
        foreach($klasseData['uitkomstKans']['scenarioEindwaarden'] as $scenario=>$bedrag)
        {
          $scenarioTabel[$scenario][$risicoklasse]=$bedrag;
        }
        $scenarioTabel['Kans'][$risicoklasse]=$klasseData['uitkomstKans']['kans'];
      }
      
      foreach($scenarioTabel as $scenario=>$risicoklasseData)
      {
        $this->pdf->SetWidths($this->pdf->widthB);
        if($scenario=='Kans')
        {
          $this->pdf->Ln(1);
          $this->pdf->row(array('',"Kans op doel",
                            $this->formatGetalNegatief($risicoklasseData['Defensief']).'%',
                            $this->formatGetalNegatief($risicoklasseData['Neutraal']).'%',
                            $this->formatGetalNegatief($risicoklasseData['Offensief']).'%'));        
        }
        else
          $this->pdf->row(array('',$scenario,
                            $this->formatGetalNegatief($risicoklasseData['Defensief']),
                            $this->formatGetalNegatief($risicoklasseData['Neutraal']),
                            $this->formatGetalNegatief($risicoklasseData['Offensief'])));
 ...
 */     
      ...
      //listarray($besteProfiel);
     // listarray($kansData);exit;
     ...
     //   listarray($sc->CRMdata['gewenstRisicoprofiel']);
              
    $this->pdf->setXY(160,115);
    $this->pdf->SetFont($this->pdf->rapport_font,'b',$this->pdf->rapport_fontsize);
    ...
    if(!$sc->loadMatrix())
      $sc->createNewMatix(true);
//
   	$this->pdf->widthA = array(40,40,25);
		$this->pdf->alignA = array('L','R','R');
    $this->pdf->SetWidths($this->pdf->widthA);
    $this->pdf->SetAligns($this->pdf->alignA);
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
    ...
    if($sc->profieldata['ScenarioGewenstProfiel']==1 && $gewenstCrmRisicoprofiel <> '')
       $this->pdf->row(array('Gewenst profiel',$gewenstCrmRisicoprofiel));
    else
    ...
    $this->pdf->SetFont($this->pdf->rapport_font,'b',$this->pdf->rapport_fontsize);
    $this->pdf->setY(35);
    $this->pdf->widthB = array(145,40,30,20);
    ...
    {
      $this->pdf->widthB = array(88,22,22);
		  $this->pdf->alignB = array('L','L','R','R');
      $this->pdf->SetWidths($this->pdf->widthB);
      $this->pdf->SetAligns($this->pdf->alignB);
      $this->pdf->SetFont($this->pdf->rapport_font,'b',$this->pdf->rapport_fontsize);
      ...
      $i=0;
      $restbedrag=0;
      foreach($sc->cashflow as $datum=>$bedragData)
      {
        if($i<6)
          $this->pdf->row(array('',$datum,$this->formatGetal($bedragData,0)));
        else
          $restbedrag+=$bedragData; 
        $i++;  
      }
      if($restbedrag <> 0)
        $this->pdf->row(array('','Overige jaren',$this->formatGetal($restbedrag,0)));
    }
    ...
    $this->pdf->SetAligns($this->pdf->alignB);
    $this->pdf->SetFont($this->pdf->rapport_font,'b',$this->pdf->rapport_fontsize);
    $this->pdf->row(array('',"Scenario \n".$sc->CRMdata['gewenstRisicoprofiel'],"\nKans ongeveer","\nEindvermogen"));
