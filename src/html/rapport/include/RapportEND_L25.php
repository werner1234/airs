...
/*
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2018/07/04 14:08:35 $
 		File Versie					: $Revision: 1.4 $

 		$Log: RapportEND_L25.php,v $
 		Revision 1.4  2018/07/04 14:08:35  rvv
 		*** empty log message ***
 		
 		Revision 1.3  2018/07/04 08:25:01  rvv
 		*** empty log message ***
 		
 		Revision 1.2  2018/06/24 12:47:04  rvv
 		*** empty log message ***
 		
 		Revision 1.1  2018/04/14 17:23:49  rvv
 		*** empty log message ***
...
class RapportEND_L25
{
	function RapportEND_L25($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	 //
		$this->pdf = &$pdf;
  ...
  //  listarray($categorieKoppeling);
		...
		else
    {
      if($zpMethode['Selectieveld1']=='ZEE' && $zpMethode['Vermogensbeheerder']=='AUR')
        ...
        $voldoetTxt='Wij zijn van mening dat onze vermogensbeheer- of beleggingsadviesdienst op dit moment nog steeds geschikt voor u is. Uw beleggingsportefeuille voldoet mogelijk (nog) niet aan de geschiktheidseisen. Dit kan diverse oorzaken hebben zoals bijvoorbeeld recente stortingen, onttrekkingen of wijzigingen in uw beleggingsprofiel. Wij zullen contact met u opnemen om dit te bespreken, voor zover dit nog niet is gebeurd.';
		}
		...
		$this->pdf->ln();
		$this->pdf->Row(array("Deze geschiktheidsrapportage heeft tot doel vast te stellen of de door Auréus verleende vermogensbeheer- of beleggingsadviesdienst nog geschikt voor u is. De geschiktheidsbeoordeling stelt ons in staat (blijvend) te kunnen handelen in uw belang."));
    $this->pdf->ln();
    $this->pdf->Row(array("In dit verband beoordelen wij of uw beleggingsportefeuille (nog) past bij uw beleggingsprofiel. Uw beleggingsprofiel is onder meer gebaseerd op uw beleggingsdoelstellingen, beleggingshorizon, financiële situatie, risicobereidheid (zowel emotioneel als uw mogelijkheid om beleggingsrisico’s financieel te kunnen dragen) en kennis en ervaring. Deze informatie is bij aanvang van en gedurende de relatie met u ingewonnen en vastgelegd, bijvoorbeeld in inventarisatiedocumenten, uw beleggingsvoorstel, vermogensplan en/of overige bij ons beschikbare gegevens naar aanleiding van onder andere periodieke evaluatie(s) van uw persoonlijke en financiële omstandigheden."));
		$this->pdf->ln();
  	$this->pdf->Row(array("Het met u overeengekomen beleggingsprofiel is momenteel ‘".$this->pdf->portefeuilledata['Risicoklasse']."’. De kaders en de bandbreedtes van dit profiel zijn bij aanvang van onze dienstverlening aan u verstrekt. Mocht u deze opnieuw willen ontvangen, vraag ons dan naar de bijlage ‘overzicht beleggingsprofielen’. Uw beleggingsportefeuille bevindt zich momenteel $binnnenBuiten de kaders van uw beleggingsprofiel en de daarbij gestelde bandbreedtes. De huidige weging in uw portefeuille is:"));
		$this->pdf->ln();
    ...
    $this->pdf->Row(array($voldoetTxt));
		$this->pdf->ln();
    $this->pdf->Row(array("Wij zullen verder periodiek (minimaal) jaarlijks in overleg met u treden voor een update van uw persoonlijke en financiële omstandigheden en een evaluatie van het door ons gevoerde vermogensbeheer of beleggingsadvies."));
