...
$Log: RapportVOLK_L57.php,v $
Revision 1.6  2020/04/08 15:42:42  rvv
*** empty log message ***

Revision 1.5  2020/03/14 18:42:03  rvv
*** empty log message ***

Revision 1.4  2020/03/11 15:18:12  rvv
*** empty log message ***
...
Revision 1.2  2017/12/23 18:14:34  rvv
*** empty log message ***
...
class RapportVOLK_L57
{
	function RapportVOLK_L57($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
$minX=1;
		$this->pdf->widthB = array(5,60,22,18,20-$minX,21-$minX,1,17-$minX,21-$minX,21-$minX,18-$minX,18-$minX,18-$minX,18-$minX,12);
		$this->pdf->widthA = array(  65,22,18,20-$minX,21-$minX,1,17-$minX,21-$minX,21-$minX,18-$minX,18-$minX,18-$minX,18-$minX,12);
    ...
    $this->pdf->templateVarsOmschrijving[$this->pdf->rapport_type.'Paginas']=$this->pdf->rapport_titel;
		...
		$actueleWaardePortefeuille = 0;
			...
			$query = "SELECT TijdelijkeRapportage.BeleggingscategorieOmschrijving as Omschrijving, TijdelijkeRapportage.Beleggingscategorie as cat, TijdelijkeRapportage.regio, TijdelijkeRapportage.regioOmschrijving, ".
    ...
    $totaalactueel=0; $totaalbegin=0; $totaaldividendCorrected=0;
		while($categorien = $DB->NextRecord())
		{
           ...
           //echo " $procentResultaat = (($totaalactueel - $totaalbegin + $totaaldividendCorrected) / ($totaalbegin /100)); <br>\n";
		    if($totaalbegin < 0)
        ...
        $totaaldividendCorrected = 0;
			...
			if(in_array($categorien['cat'],array('ALTERN','OG')))
			{
				$orderRegio='';
				$regioTonen=false;
			}
			...
			{
        $orderRegio='TijdelijkeRapportage.regioVolgorde, ';
        $regioTonen=true;
			}
			...
			//	" TijdelijkeRapportage.beginPortefeuilleWaardeEuro / $beginQuery as beginPortefeuilleWaardeEuro, ".
				...
				.$__appvar['TijdelijkeRapportageMaakUniek'].
				" ORDER BY  $orderRegio TijdelijkeRapportage.Lossingsdatum, TijdelijkeRapportage.fondsOmschrijving asc";//exit;
    ...
    //  $subtotaal=array();
      ...
      $lastRegio='';
			while($subdata = $DB2->NextRecord())
			{
				...
				/*
		    if($n>$regels-2 && $this->pdf->GetY()>180)//
        {
           $this->pdf->AddPage();
        }
        */
        ...
        if($lastRegio<> '' && $subdata['regioOmschrijving'] <> $lastRegio)
				{
          
          $procentResultaat = (($regiototaal['subtotaalactueel'] - $regiototaal['subtotaalbegin'] + $regiototaal['totaalDividendCorrected']) / ($regiototaal['subtotaalbegin'] /100));
          if($subdata['subtotaalbegin'] < 0)
            $procentResultaat = -1 * $procentResultaat;
          
					if($regioTonen)
            $this->printSubTotaal(vertaalTekst("Subtotaal:",$this->pdf->rapport_taal), $regiototaal['subtotaalbegin'],$regiototaal['subtotaalactueel'],$regiototaal['percentageVanTotaal'], $regiototaal['fondsResultaat'], $regiototaal['valutaResultaat'], $procentResultaat,$regiototaal['totaalDividend']);
          $regiototaal=array();
				}
        if($subdata['regioOmschrijving'] <> $lastRegio && $regioTonen==true)
          $this->printKop($subdata['regioOmschrijving'], "");//vertaalTekst("Waarden",$this->pdf->rapport_taal)." ".
        $lastRegio=$subdata['regioOmschrijving'];
        ...
        $this->pdf->Cell($this->pdf->widthB[1],4,$subdata['valuta'],null,null,null,null,null);
				$this->pdf->setX($this->pdf->marge);
        ...
        $regiototaal['subtotaalbegin']+=$subdata['beginPortefeuilleWaardeEuro'];
        $regiototaal['subtotaalactueel']+=$subdata['actuelePortefeuilleWaardeEuro'];
        $regiototaal['percentageVanTotaal'] +=$percentageVanTotaal;
        $regiototaal['fondsResultaat'] +=$fondsResultaat;
        $regiototaal['valutaResultaat'] +=$valutaResultaat;
        $regiototaal['totaalDividend'] += $dividend['totaal'];
        $regiototaal['totaalDividendCorrected'] += $dividend['corrected'];
        
        
        
        $valutaWaarden[$categorien['valuta']] = $subdata['actueleValuta'];
        $subtotaal['subtotaalbegin']+=$subdata['beginPortefeuilleWaardeEuro'];
        $subtotaal['subtotaalactueel']+=$subdata['actuelePortefeuilleWaardeEuro'];
        ...
        $n++;
			...
			// print categorie footers
				$procentResultaat = (($regiototaal['subtotaalactueel']  - $regiototaal['subtotaalbegin'] + $regiototaal['totaalDividendCorrected'] ) / ($regiototaal['subtotaalbegin']  /100));
				if($regiototaal['subtotaalbegin'] < 0)
					$procentResultaat = -1 * $procentResultaat;
     // echo "	$procentResultaat = ((".$categorien['subtotaalactueel']." - ".$categorien['subtotaalbegin']." + ".$subtotaal['totaalDividendCorrected']." ) / (".$categorien['subtotaalbegin']."  /100)); <br>\n";
      if($regioTonen)
        $this->printSubTotaal(vertaalTekst("Subtotaal:",$this->pdf->rapport_taal), $regiototaal['subtotaalbegin'],$regiototaal['subtotaalactueel'],$regiototaal['percentageVanTotaal'], $regiototaal['fondsResultaat'], $regiototaal['valutaResultaat'], $procentResultaat,$regiototaal['totaalDividend']);
    //  $subtotaal=array();
//rvv
