...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2018/06/09 15:58:54 $
File Versie					: $Revision: 1.14 $

$Log: RapportATT_L44.php,v $
Revision 1.14  2018/06/09 15:58:54  rvv
*** empty log message ***

Revision 1.13  2018/03/04 10:14:13  rvv
*** empty log message ***

Revision 1.12  2017/01/18 07:37:14  rvv
*** empty log message ***

Revision 1.11  2016/03/20 14:32:23  rvv
*** empty log message ***

Revision 1.10  2016/02/03 17:00:18  rvv
*** empty log message ***

Revision 1.9  2015/12/13 17:36:44  rvv
*** empty log message ***

Revision 1.8  2015/12/13 09:03:13  rvv
*** empty log message ***

Revision 1.7  2015/02/07 20:37:51  rvv
*** empty log message ***

Revision 1.6  2014/08/16 15:31:50  rvv
*** empty log message ***

Revision 1.5  2013/10/05 15:58:48  rvv
*** empty log message ***

Revision 1.4  2013/04/17 16:00:15  rvv
*** empty log message ***
...
Revision 1.8  2013/02/27 17:04:41  rvv
*** empty log message ***

Revision 1.7  2013/02/20 15:12:14  rvv
*** empty log message ***

Revision 1.6  2013/01/16 12:10:33  rvv
*** empty log message ***
...
class RapportATT_L44
{
	function RapportATT_L44($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//listarray($this->categorieVolgorde);
  ...
  $indexJaren=array();
  $jaarPerf=1;
  if(substr($historieTot,0,4)>2015)
  {
    $historie2Tot=$historieTot;
    $historieTot="2015-12-31";
  }
  if(db2jul($this->pdf->PortefeuilleStartdatum) <  db2jul($historieTot))
  {
    $indexHistorie = $index->getWaarden($this->pdf->PortefeuilleStartdatum ,$historieTot  ,$this->portefeuille,'','maanden');
    ...
    { 
      ...
      if($jaar <> $lastJaar)
        $jaarPerf=1;
      $jaarPerf=($jaarPerf * ($maandData['performance']+100)/100);
      ...
      foreach($somVelden as $veld)    
        $indexJaren[$jaar][$veld]+=$maandData[$veld];  
      $lastJaar=$jaar;  
    }
  }
  if(isset($historie2Tot))
  {
    $indexHistorie = $index->getWaarden($this->pdf->PortefeuilleStartdatum ,$historie2Tot  ,$this->portefeuille,'','jaar');
...
//exit;
    ...
    $query="select Beleggingscategorie,Hoofdcategorie FROM CategorienPerHoofdcategorie WHERE vermogensbeheerder='". $this->pdf->portefeuilledata['Vermogensbeheerder']."'";
    $DB->SQL($query);
    $DB->Query();
    $conversie=array();
