...
$Log: RapportVKMS_L77.php,v $
Revision 1.11  2020/05/27 16:14:13  rvv
*** empty log message ***

Revision 1.10  2020/05/10 10:58:29  rvv
*** empty log message ***

Revision 1.9  2020/05/09 16:56:11  rvv
*** empty log message ***

Revision 1.8  2020/05/06 17:10:29  rvv
*** empty log message ***

Revision 1.7  2020/04/26 13:34:24  rvv
*** empty log message ***

Revision 1.6  2020/04/25 17:15:30  rvv
*** empty log message ***

Revision 1.5  2019/02/03 15:50:24  rvv
*** empty log message ***

Revision 1.4  2019/01/16 08:41:45  rvv
*** empty log message ***

Revision 1.3  2019/01/09 15:52:19  rvv
*** empty log message ***

Revision 1.2  2019/01/05 09:16:36  rvv
*** empty log message ***
...
class RapportVKMS_L77
{
	function RapportVKMS_L77($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    $txt=vertaalTekst('* Onder “indirecte kosten” worden verstaan de kosten die gemoeid zijn met beleggingsfondsen, indextrackers en structured products zoals transactiekosten, beheervergoedingen en servicefees en die niet direct zichtbaar zijn.',$this->pdf->rapport_taal);
    ...
    if($this->pdfVullen==true)
		{
			...
			$this->pdf->setWidths(array(98, 20, 20));
			$startY=$this->pdf->getY();
      $this->pdf->setXY($this->pdf->marge,185);
      $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize-2);
      $this->pdf->Cell(40,4,$txt,0,'L');
  
      $this->pdf->setXY($this->pdf->marge,$startY);
      $this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
      $this->pdf->row(array(vertaalTekst('Directe en indirecte kosten vanaf',$this->pdf->rapport_taal).' ' . date('d-m-Y', db2jul($this->vanafDatum)), 'EUR', vertaalTekst('Percentage',$this->pdf->rapport_taal)));
      $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
      /*
$this->pdf->row(array(vertaalTekst('Indirecte kosten',$this->pdf->rapport_taal).'*', $this->formatGetal($totaalDoorlopendekosten, 0) . ' EUR'));
$this->pdf->excelData[]=array('','Indirecte kosten', '','',round($totaalDoorlopendekosten, 0), 'EUR');
$this->pdf->row(array(vertaalTekst('Indirecte kosten ten opzichte van onderliggend vermogen',$this->pdf->rapport_taal), $this->formatGetal($doorlopendeKostenPercentage * 100, 2) . ' %'));
$this->pdf->excelData[]=array('','Indirecte kosten ten opzichte van onderliggend vermogen','','', round($doorlopendeKostenPercentage * 100, 2) ,'%');
$this->pdf->ln();
$this->pdf->excelData[]=array();
    */
    ...
    $barData=array();
    $barData['totaal']['Indirecte kosten']=$vkmPercentagePortefeuille;
    $barData['kosten']['Indirecte kosten']=$vkmPercentagePortefeuille;
		...
		if($this->pdfVullen==true)
		{
		  /*
			...
			$this->pdf->row(array(vertaalTekst("Herrekende indirecte kosten",$this->pdf->rapport_taal), $this->formatGetal($herrekendeKosten * 100, 2) . ' %'));
			$this->pdf->excelData[]=array('',"Herrekende indirecte kosten", '','',round($herrekendeKosten * 100, 2),'%');
			...
			$this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
			$this->pdf->excelData[]=array('','Indirecte kosten factor van de portefeuille','','',round($vkmPercentagePortefeuille, 2),'%');
			$this->pdf->ln();
			$this->pdf->excelData[]=array();
...
//listarray($risicoklasse);exit;
		$totaal=0;
		$totaalKosten=0;
   // $barData=array('kosten'=>array(),'totaal'=>array());
		$totaalBelasting=0;
    $nulTonen=true;
    $kostenRow=array();
    $kostenRowBelasting=array();
		while($data = $DB->nextRecord())
		{
			if($this->pdfVullen==true)
			{
        $kostenProcent=$data['totaal']/$gemiddelde*100;
        if($data['Grootboekrekening']=='DIVBE' || $data['Grootboekrekening']=='ROER')
        {
          $kostenRowBelasting[] = array(vertaalTekst($data['Omschrijving'], $this->pdf->rapport_taal),$this->formatGetal($data['totaal'], 0,false,$nulTonen), $this->formatGetal($kostenProcent, 2,false,$nulTonen) . ' %');
          $totaalBelasting+=$data['totaal'];
          $barData['totaal'][$data['Omschrijving']]+=$kostenProcent;
          $barData['belasting'][$data['Omschrijving']]+=$kostenProcent;
        }
        elseif($data['Grootboekrekening']=='BEH')
        {
          $bedrag=$data['totaal']/(1+($risicoklasse['BeheerfeeBTW']/100));
          $btw=$data['totaal']-$bedrag;
          $totaalKosten+=$bedrag;
          $totaalBelasting+=$btw;
          $barData['kosten']['Beheervergoeding excl. BTW']+=$bedrag/$gemiddelde*100;
          $barData['totaal']['Beheervergoeding excl. BTW']+=$bedrag/$gemiddelde*100;
         
          $kostenRow[] = array(vertaalTekst('Beheervergoeding excl. BTW', $this->pdf->rapport_taal).'',$this->formatGetal($bedrag, 0), $this->formatGetal($bedrag/$gemiddelde*100, 2) . ' %');
          if($risicoklasse['BeheerfeeBTW'] <> 0)
          {
            $kostenRowBelasting[] = array("BTW over beheervergoeding",$this->formatGetal($btw, 0), $this->formatGetal($btw/$gemiddelde*100, 2) . ' %');
            $barData['totaal']["BTW over beheervergoeding"]+=$btw/$gemiddelde*100;
            $barData['belasting']["BTW over beheervergoeding"]+=$btw/$gemiddelde*100;
          }
        }
        else
        {
          ...
          $totaalKosten+=$data['totaal'];
          $barData['kosten'][vertaalTekst($data['Omschrijving'], $this->pdf->rapport_taal)]+=$kostenProcent;
          $barData['totaal'][vertaalTekst($data['Omschrijving'], $this->pdf->rapport_taal)]+=$kostenProcent;
        }
...
//listarray($barData);
    ...
    $this->pdf->row(array(vertaalTekst('Indirecte kosten*',$this->pdf->rapport_taal),$this->formatGetal($totaalDoorlopendekosten, 0) , $this->formatGetal($vkmPercentagePortefeuille, 2) . ' %'));
    $totaalKosten+=$totaalDoorlopendekosten;
    $totaal+=$totaalDoorlopendekosten;
    ...
    if($spreadKostenEUR <> 0)
      ...
      $totaalKosten+= $spreadKostenEUR;
			//$barData['Spread-kosten'] = $kostenProcent;
      $barData['kosten']['Spread-kosten']=$kostenProcent;
      $barData['totaal']['Spread-kosten']=$kostenProcent;
		}
    
    

    
    $kostenPercentage=$totaalKosten/$gemiddelde*100;
    $vkmWaarde=$vkmPercentagePortefeuille + $kostenPercentage;
    $this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
    $this->pdf->row(array(vertaalTekst('Vergelijkende kostenmaatstaf excl. belastingen',$this->pdf->rapport_taal),$this->formatGetal($totaalKosten, 0), $this->formatGetal($kostenPercentage, 2).' %'));
    $this->pdf->ln();
    $this->pdf->row(array(vertaalTekst('Belastingen',$this->pdf->rapport_taal),'',''));
    $this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
    foreach($kostenRowBelasting as $row)
    {
      $this->pdf->row($row);
    }
    
    
    $kostenPercentageBelasting=$totaalBelasting/$gemiddelde*100;
    $kostenPercentage=$totaal/$gemiddelde*100;
		$vkmWaarde= $kostenPercentage;
		if($this->pdfVullen==true)
		{
			$this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
			$this->pdf->row(array(vertaalTekst('Totaal aan belastingen',$this->pdf->rapport_taal), $this->formatGetal($totaalBelasting, 0), $this->formatGetal($kostenPercentageBelasting, 2).' %'));
			...
			$this->pdf->row(array(vertaalTekst('Vergelijkende kostenmaatstaf incl. belastingen',$this->pdf->rapport_taal),$this->formatGetal($vkmWaarde*$gemiddelde/100, 0), $this->formatGetal($vkmWaarde, 2).' %'));
...
foreach ($perCategorie as $hoofdcategorie=>$categorieData)
{
 ...
 WHERE Portefeuilles.portefeuille='".$this->portefeuille."'";
    ...
    for($i=$this->pdf->rapport_jaar; $i<=$doelJaar; $i++)
		...
		$this->LineDiagram(120, 45, $grafiekWaarden,array($waardeZonderKostenKleur,$waardeNaKostenKleur,$cumulatieveKostenKleur),0,0,4,4,false);//50
...
//		echo "$doelJaar $rendement $beginwaarde";
  ...
  function fondsPerf($fonds,$van,$tot)
  {
    ...
    $n=0;
		$colors=array();
  	$aantal=count($data['totaal']);
		$kleurStap=floor((255-75)/$aantal);
    
    $grafiek=$data;
    unset($grafiek['totaal']);

		foreach ($data['totaal'] as $categorie=>$waarde)
		{
		...
		$maxVal=ceil(array_sum($data['kosten'])*2)/2;
		if($maxVal <= 0)
			$maxVal=0;
    ...
    $skipNull=false;
		...
		$n=0;
	//	listarray($grafiek);exit;
    foreach($grafiek as $type=>$grafiekdata)
    {
      
      unset($YstartGrafiekLast);
      foreach ($grafiekdata as $categorie => $val)
      {
        ...
        //Bar
        $xval = $XstartGrafiek + $vBar - $eBaton *1.5  +$n*$vBar;
        $lval = $eBaton;
      ...
      $n++;
		...
		$yval = $YstartGrafiek  ;
		foreach($data['totaal'] as $categorie=>$val)
		{
			$yval-=7;
			$this->pdf->Rect($xval, $yval, 2, 2, 'DF',null,$colors[$categorie]);
			$this->pdf->SetXY($xval+4, $yval-1);
			$this->pdf->Cell(50, 4, vertaalTekst($categorie,$this->pdf->rapport_taal)." ".number_format($val,2,',','.')."%",0,0,'L');
