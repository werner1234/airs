...
$Log: RapportVOLK_L80.php,v $
Revision 1.6  2020/05/16 15:57:02  rvv
*** empty log message ***

Revision 1.5  2019/12/01 17:55:28  rvv
*** empty log message ***

Revision 1.4  2019/12/01 07:51:04  rvv
*** empty log message ***
...
class RapportVOLK_L80
{
	function RapportVOLK_L80($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
           ...
           $procentResultaat = (($totaalactueel - $totaalbegin + $totaaldividendCorrected) / ($totaalbegin /100));
			...
			$subquery = "SELECT TijdelijkeRapportage.fondsOmschrijving, ".
				...
				round((UNIX_TIMESTAMP(TijdelijkeRapportage.rapportageDatum) - UNIX_TIMESTAMP(TijdelijkeRapportage.koersDatum))/86400) as koersLeeftijd ,
				 ...
				 TijdelijkeRapportage.beleggingscategorie,
				 TijdelijkeRapportage.valuta, TijdelijkeRapportage.portefeuille,Fondsen.vkm ".
				" FROM TijdelijkeRapportage
...
//echo $subquery."<br><br>";exit;
      ...
      $kopPrinted=false;
			...
			while($subdata = $DB2->NextRecord())
			{
				if($subdata['vkm']==0)
				{
					$categorie='Top 10 holdings directe instrumenten';
				}
				else
				{
          $categorie='Top 10 holding indirecte instrumenten';
				}
        $topTien[$subdata['fonds']]=$subdata;
        $topTienWaarde[$categorie][$subdata['fonds']]=$subdata['actuelePortefeuilleWaardeEuro'];
				/*
...
//rvv
    ...
    if(true)//count($topTienWaarde)>0)
    {
      $this->pdf->rapport_titel = "Top 10 holding indirecte instrumenten";
  
      $this->pdf->addPage();
      $this->pdf->templateVars['VOLK2Paginas'] = $this->pdf->page;
      $this->pdf->ln();
      $offset = 75;
      $posY = $this->pdf->getY();

      foreach ($topTienWaarde as $categorie => $fondsWaarden)
      {
        ...
        $this->pdf->SetFont($this->pdf->rapport_font, 'b', $this->pdf->rapport_fontsize);
        $this->pdf->row(array('', vertaalTekst($categorie, $this->pdf->rapport_taal), vertaalTekst('Absoluut', $this->pdf->rapport_taal), vertaalTekst('in %', $this->pdf->rapport_taal)));
        $this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
  
        $n = 0;
        foreach ($fondsWaarden as $fonds => $waarde)
        {
          $percentage = $waarde / $totaalWaarde * 100;
          $this->pdf->row(array('', $topTien[$fonds]['fondsOmschrijving'], $this->formatGetal($waarde, 0), $this->formatGetal($percentage, 1)));
          $n++;
          if ($n > 9)
          {
            break;
