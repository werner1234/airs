...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2014/07/19 14:27:59 $
File Versie					: $Revision: 1.1 $

$Log: RapportPERFD_L25.php,v $
Revision 1.1  2014/07/19 14:27:59  rvv
*** empty log message ***

Revision 1.1  2013/07/20 16:26:07  rvv
*** empty log message ***
...
class RapportPERFD_L25
{

	function RapportPERFD_L25($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//  echo "$ongerealiseerdeKoersResultaat = $ongerealiseerdFondsResultaat + $ongerealiseerdValutaResultaat <br>\n";
...
$RapStartJaar = date("Y", db2jul($this->rapportageDatumVanaf));
		...
		$totaalOpbrengst += ($ongerealiseerdFondsResultaat + $ongerealiseerdValutaResultaat);

		$gerealiseerdeKoersResultaatTmp = gerealiseerdKoersresultaat($this->portefeuille, $this->rapportageDatumVanaf, $this->rapportageDatum,$this->pdf->rapportageValuta,true,'Totaal',true);
		$totaalOpbrengst += $gerealiseerdeKoersResultaatTmp['totaal'];
		...
		$this->pdf->SetAligns($this->pdf->alignA);
			...
			$this->pdf->row(array("",vertaalTekst("Ongerealiseerde fonds resultaten",$this->pdf->rapport_taal),$this->formatGetal($ongerealiseerdFondsResultaat,2),""));
		$this->pdf->row(array("",vertaalTekst("Ongerealiseerde valuta resultaten",$this->pdf->rapport_taal),$this->formatGetal($ongerealiseerdValutaResultaat,2),""));
    //($ongerealiseerdFondsResultaat + $ongerealiseerdValutaResultaat);exit; 

// $gerealiseerdeKoersResultaatTmp
    	$this->pdf->row(array("",vertaalTekst("Gerealiseerde fonds resultaten",$this->pdf->rapport_taal),$this->formatGetal($gerealiseerdeKoersResultaatTmp['fonds'],2),""));
    	$this->pdf->row(array("",vertaalTekst("Gerealiseerde valuta resultaten",$this->pdf->rapport_taal),$this->formatGetal($gerealiseerdeKoersResultaatTmp['valuta'],2),""));
...
//listarray($grafiekData);
    ...
    $this->pdf->underlinePercentage=0.8;
			...
			$this->pdf->SetAligns($this->pdf->alignA);
      $this->pdf->SetY(140);
   $this->pdf->row(array("",vertaalTekst("Belasting Duitsland",$this->pdf->rapport_taal)));
   $this->pdf->Rect($this->pdf->marge+5, $this->pdf->GetY(), 80+30, 4*4+2 );

   $this->pdf->row(array("",vertaalTekst("Belasting gerealiseerde resultaten",$this->pdf->rapport_taal),
                             $this->formatGetal(($gerealiseerdeKoersResultaatTmp['fonds']+$gerealiseerdeKoersResultaatTmp['valuta'])*.25,2),""));
    $this->pdf->row(array("",vertaalTekst("Belasting dividenden",$this->pdf->rapport_taal),
                             $this->formatGetal($opbrengstenPerGrootboek['Dividend']*.25,2),""));
                             $this->pdf->CellBorders = array('','','UU'); 
    $this->pdf->row(array("",vertaalTekst("Belasting rente",$this->pdf->rapport_taal),
                             $this->formatGetal(($opbrengstenPerGrootboek['Rente obligaties']+$opbrengstenPerGrootboek['Rente'])*.25,2),""));
     unset($this->pdf->CellBorders);
     $this->pdf->Ln(2);                 
    $this->pdf->row(array("",vertaalTekst("Totaal ",$this->pdf->rapport_taal),
                             $this->formatGetal(($opbrengstenPerGrootboek['Rente obligaties']+$opbrengstenPerGrootboek['Rente']+
                                                 $opbrengstenPerGrootboek['Dividend']+$gerealiseerdeKoersResultaatTmp['fonds']+$gerealiseerdeKoersResultaatTmp['valuta'])*.25,2),""));
