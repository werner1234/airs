...
$Log: RapportHSE_L68.php,v $
Revision 1.27  2020/07/26 06:50:14  rvv
*** empty log message ***

Revision 1.26  2020/07/25 15:36:13  rvv
*** empty log message ***

Revision 1.25  2020/07/22 16:35:31  rvv
*** empty log message ***

Revision 1.24  2019/07/17 15:34:55  rvv
*** empty log message ***

Revision 1.23  2019/06/19 15:59:09  rvv
*** empty log message ***

Revision 1.22  2019/05/30 05:54:30  rvv
*** empty log message ***

Revision 1.21  2019/05/25 16:22:07  rvv
*** empty log message ***

Revision 1.20  2019/05/01 15:53:25  rvv
*** empty log message ***

Revision 1.19  2018/09/01 16:53:24  rvv
*** empty log message ***

Revision 1.18  2018/08/18 12:40:14  rvv
php 5.6 & consolidatie

Revision 1.17  2018/06/20 16:40:16  rvv
*** empty log message ***

Revision 1.16  2018/06/16 17:42:56  rvv
*** empty log message ***

Revision 1.15  2017/11/15 17:03:35  rvv
*** empty log message ***

Revision 1.14  2017/11/08 17:12:56  rvv
*** empty log message ***

Revision 1.13  2017/11/04 17:40:21  rvv
*** empty log message ***

Revision 1.12  2017/10/28 18:03:18  rvv
*** empty log message ***

Revision 1.11  2017/10/08 14:09:23  rvv
*** empty log message ***
...
Revision 1.8  2017/02/05 16:22:39  rvv
*** empty log message ***
...
class RapportHSE_L68
{
	function RapportHSE_L68($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
if($this->pdf->lastPOST['doorkijk']==1)
	...
	function printTotaal($title, $totaalA, $totaalB, $totaalC, $totaalD, $totaalE, $totaalF = 0, $grandtotaal=false, $durationTotaal = 0, $ytdTotaal = 0 )
	{
		$hoogte = 20;
...
Left Join Fondsen as optie ON Fondsen.OptieBovenliggendFonds = optie.Fonds   
    ...
    while($fonds = $DB->NextRecord())
		{
			if($eersteHcat=='')
        $eersteHcat=$fonds['hoofdcategorie'];
		  $regels[]=$fonds;
    }
    $this->pdf->hoofdcategorie=$fonds['hoofdcategorie']=$eersteHcat;
    $this->pdf->AddPage();
    $this->pdf->templateVars['HSEPaginas']=$this->pdf->page;
    $this->pdf->templateVarsOmschrijving['HSEPaginas']=$this->pdf->rapport_titel;
 ...
 //$this->pdf->SetFillColor($this->pdf->rapport_kop_bgcolor['r']*1.2,$this->pdf->rapport_kop_bgcolor['g']*1.2,$this->pdf->rapport_kop_bgcolor['b']*1.2);
    ...
    $ytm='';        
			...
			$aandeel=0;
			if(!empty($lastCategorie) && $lastCategorie <> $fonds['Omschrijving'])
			{
       ...
       // $totaalbegin=$fonds['beginPortefeuilleWaardeEuro'];
        ...
        $this->extraVoet();
        $actueleWaardePortefeuille += $this->printTotaal($title, '', $totaalactueel, $totaalpercentage , '', '', $procentResultaat,'',$durationTotaal,$ytdTotaal);
        $this->extraVoet();
  ...
  $fonds['beginwaardeLopendeJaar']=0;
        ...
        elseif($fonds['hoofdcategorie']=='VAR')
        {
         ...
         // $q = "SELECT Datum, Rentepercentage FROM Rentepercentages WHERE Fonds = '".$fonds['fonds']."' ORDER BY Datum DESC LIMIT 1";
          ...
          $modifiedDuration=0;
          $restLooptijd=0;
          $query="SELECT AverageCreditQuality,AverageEffDuration,YieldtoMaturity
FROM FondsenFundInformatie
WHERE fonds='".mysql_real_escape_string($fonds['fonds'])."' AND datumVanaf <= '".$this->rapportageDatum."' AND AverageEffDuration <> 0 AND YieldtoMaturity <> 0
order by datumVanaf desc ";
          $DB2->SQL($query);
          $fundInfo=$DB2->lookupRecord();

          if($lossingsJul > 0 || count($fundInfo)>1)
          {
            //$this->lossingsWaardeTotaal += $fonds['totaalAantal'] * 100 * $fonds['fondsEenheid'] * $fonds['actueleValuta'];
						
						if(count($fundInfo)>1)
						{
							if($fundInfo['AverageCreditQuality'] <> '' && $fundInfo['AverageCreditQuality'] <> 0)
                $fonds['fondsRating']=$fundInfo['AverageCreditQuality'];
              $duration=$fundInfo['AverageEffDuration'];
              $ytm=$fundInfo['YieldtoMaturity'];
						}
						else
            {
          ...
          if($duration<>0)
            $durationTxt=$this->formatGetal($duration, 2);
          else
            $durationTxt='';
...
//    listarray($totaalBijdrage);
      ...
      //vertaalTekst("Subtotaal",$this->pdf->rapport_taal)." ".vertaalTekst($lastCategorie,$this->pdf->rapport_taal)
		$title = 'Totaal '.$lastCategorie;
		$actueleWaardePortefeuille += $this->printTotaal($title, $totaalbegin, $totaalactueel,$totaalpercentage,$totaalResultaat,'',$totaalBijdrage);
