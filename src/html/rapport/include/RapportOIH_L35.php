...
class RapportOIH_L35
{
	function RapportOIH_L35($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
  	$this->pdf = &$pdf;
...
TijdelijkeRapportage.Lossingsdatum, TijdelijkeRapportage.fondsOmschrijving asc";
    ...
    foreach($dataregel as $data)
    {
      ...
      $extraFondsInfo[$data['fonds']]=$data;
...
//echo $data['fondsOmschrijving']." | ".$data['actuelePortefeuilleWaardeEuro']."+".$extra['opbrengstTotaal'].")/$aankoopEur <br>\n";
    ...
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
    $this->AddRestricties($extraFondsInfo);
    ...
    PieChart_L35($this->pdf, $chartsize, $vwh, $grafiek['Aanvang']['Percentage'], '%l', $grafiek['Aanvang']['Kleur'], vertaalTekst('Aanvang', $this->pdf->rapport_taal), $legendaStart);

    $i=1;
    $this->pdf->setXY($this->pdf->marge+5+90*$i , $ystart);
    $legendaStart=array($this->pdf->marge+5+90*$i,$ystart+$chartsize+10);
    PieChart_L35($this->pdf, $chartsize, $vwh, $grafiek['Strategie']['Percentage'], '%l', $grafiek['Strategie']['Kleur'], vertaalTekst('Strategie', $this->pdf->rapport_taal), $legendaStart);
...
JOIN Portefeuilles ON contractueleUitsluitingen.Portefeuille=Portefeuilles.Portefeuille AND Portefeuilles.Einddatum>'".$this->rapportageDatum."'
LEFT JOIN Fondsen ON contractueleUitsluitingen.fonds=Fondsen.Fonds
WHERE contractueleUitsluitingen.Portefeuille IN('".implode("','",$portefeuilles)."') AND contractueleUitsluitingen.soortReservering='Commitment' ORDER BY Fondsen.Omschrijving";
    $db->SQL($query);
    $db->Query();
    $restricties=array();
    $fondsen=array();
...
//listarray($opgevraagd);listarray($restricties);exit;
    if(count($restricties)>0)
    {
      ...
      $this->pdf->ln();
      $this->pdf->SetFont($this->pdf->rapport_font, 'b', $this->pdf->rapport_fontsize);
      $this->pdf->Row(array('Afgegeven commitments'));
      //$this->pdf->SetWidths(array(75 + 1, 65, 25, 25));
      
      $this->pdf->Row(array('Fondsomschrijving','','','Aanvang','Valuta','','Commitment','Opgevraagd','Nog opvraagbaar'));
      $this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
      $totalen[]=array();
      foreach($restricties as $data)
      {
        ...
        $this->pdf->Row(array($data['Omschrijving'],'','',$extraFondsInfo[$data['fonds']]['Aanvang'],$data['Valuta'],'',$this->formatGetal($data['bedrag'],0),$this->formatGetal($opgevraagd,0),$this->formatGetal($verplichting,0)));
      ...
      $this->pdf->Row(array('Totaal','','','','','',$this->formatGetal($totalen['bedrag'],0),$this->formatGetal($totalen['opgevraagd'],0),$this->formatGetal($totalen['verplichting'],0)));
