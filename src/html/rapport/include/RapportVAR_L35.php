...
class RapportVAR_L35
{
	function RapportVAR_L35($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = $pdf;
...
WHERE bronKoppeling='$airsCategorie' AND doorkijkCategoriesoort='".mysql_real_escape_string($doorkijkSoort)."' AND systeem='AIRS' AND vermogensbeheerder='". $this->pdf->portefeuilledata['Vermogensbeheerder']."'";
    ...
    $this->pdf->row(array('',$titel,'Portefeuille (%)','Index (%)','Verschil (%)'));
    ...
    if($this->benchmarkGetoond==false)
    {
      ...
      $this->pdf->row(array('', 'Gehanteerde index voor aandelen: ' . $this->benchmarkOmschrijving));
...
//listarray($doorKijk);
      ...
      $this->pdf->excelData[]=array($doorkijkCategorieSoort,'Portefeuille','Benchmark','Verschil');
          ...
          $this->pdf->excelData[]=array($categorie,$percentage,$percentageBenchmark,$verschil);
      ...
      $this->toonTabel($tabelData,$xOffset,$doorkijkTitels[$doorkijkCategorieSoort], $this->kleuren[$doorkijkCategorieSoort]);
			...
			if($grafiekTonen==true)
			  $this->printPie( $grafiekdata,0+$xOffset,40,$doorkijkTitels[$doorkijkCategorieSoort]); //+$yOffset);
      else
        $this->pdf->setY(40);
     
      $this->pdf->setXY($xOffset+65,$this->pdf->getY()+5);
      $max=10;
      $this->BarDiagram(50,55, $verschilData,'',$verschilKleuren,'Verschil portefeuille/index',$max);
			$pieTeller++;
		}
    ...
    $query="SELECT IndexPerBeleggingscategorie.Fonds,Fondsen.Omschrijving FROM IndexPerBeleggingscategorie JOIN Fondsen ON IndexPerBeleggingscategorie.Fonds=Fondsen.Fonds WHERE Vermogensbeheerder='$beheerder'  limit 1";
    $db->SQL($query);
    $tmp=$db->lookupRecord();
...
CategorienPerHoofdcategorie.Hoofdcategorie='ZAK'";
		$db->SQL($query);
		$db->Query();
