...
/*
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2016/03/02 16:59:05 $
 		File Versie					: $Revision: 1.6 $

 		$Log: RapportCASHY_L55.php,v $
 		Revision 1.6  2016/03/02 16:59:05  rvv
 		*** empty log message ***
 		
 		Revision 1.5  2014/08/06 15:41:01  rvv
 		*** empty log message ***
 		
 		Revision 1.4  2014/06/14 16:40:37  rvv
 		*** empty log message ***
 		
 		Revision 1.3  2014/06/08 15:27:58  rvv
 		*** empty log message ***
...
class RapportCASHY_L55
{
	function RapportCASHY_L55($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	 //
		$this->pdf = &$pdf;
    ...
    $this->kleur['rente']=array($allekleuren['OIB']['EFI_AAND']['R']['value'],
                                $allekleuren['OIB']['EFI_AAND']['G']['value'],
                                $allekleuren['OIB']['EFI_AAND']['B']['value']);
    $this->kleur['lossing']=array($allekleuren['OIB']['EFI_OBL']['R']['value'],
                                $allekleuren['OIB']['EFI_OBL']['G']['value'],
                                $allekleuren['OIB']['EFI_OBL']['B']['value']);
    ...
    $Rapportmaand=date('m',$this->pdf->rapport_datum);
    $jaar=date('Y',$this->pdf->rapport_datum);
$maanden=array(0,'jan','feb','mrt','apr','mei','jun','jul','aug','sep','okt','nov','dec');

		for($i=intval($Rapportmaand);$i<13+intval($Rapportmaand);$i++)
		{
		  if($i>12)
        $maandInt=$i-12;
      else
        $maandInt=$i; 
		  $cashflowHuidigjaar[$maanden[$maandInt]]['lossing'] +=0;
		  $cashflowHuidigjaar[$maanden[$maandInt]]['rente'] +=0;
      ...
      $maand=substr($regel['0'],3,2);
 		  if($jaar > ($rapJaar+13))
	      $jaar='Overig';
		  $maandTxt=$maanden[intval($maand)];
		  $cashflowJaar[$jaar]['lossing'] +=0;
		  $cashflowJaar[$jaar]['rente'] +=0;
		  if($jaar==$rapJaar||$jaar==$rapJaar+1 && $maand<=$Rapportmaand)
		    $cashflowHuidigjaar[$maandTxt][$regel[2]] +=$regel[3];
    ...
    $n=0;
		...
		{
		   $n=fillLine($this->pdf,$n,array(0,1,1,1,1));
...
$this->pdf->CellBorders = array();
    ...
    $this->VBarDiagram(160,60,$cashflowHuidigjaar,"Komende 12 maanden");
		$this->pdf->setXY(160,110);
    $this->VBarDiagram(160,60,$cashflowJaar,"Lange termijn");
	}
	...
	function VBarDiagram($w, $h, $data,$titel)
  {
      ...
      if($this->kleur['lossing'][0] || $this->kleur['lossing'][1] || $this->kleur['lossing'][2] ||
         $this->kleur['rente'][0] || $this->kleur['rente'][1] || $this->kleur['rente'][2] )
      {

        $colors=array('rente'=>$this->kleur['rente'],'lossing'=>$this->kleur['lossing']);
      }
      else
      {
        $colors=array('rente'=>array(155,168,185),
        'lossing'=>array(162,167,170));
      }
      ...
      $this->pdf->Rect($XstartGrafiek, $YstartGrafiek-$hGrafiek, $bGrafiek, $hGrafiek,'FD','',$this->pdf->grafiekAchtergrondKleur);
