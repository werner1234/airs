...
$Log: RapportDOORKIJK_L56.php,v $
Revision 1.2  2020/02/29 16:23:08  rvv
*** empty log message ***

Revision 1.1  2019/06/22 16:31:44  rvv
*** empty log message ***
...
class RapportDOORKIJK_L56
{
	function RapportDOORKIJK_L56($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = $pdf;
...
*/
    ...
    $doorkijkTitelsGrafiek=array('Beleggingscategorien'=>'Beleggingscategorie','Regios'=>'Regio zakelijke waarden','Beleggingssectoren'=>'Beleggingssector zakelijke waarden');
		foreach($doorkijkCategorieSoorten as $index=>$doorkijkCategorieSoort)
		{
			$xOffset =  $index * 98;
			if($doorkijkCategorieSoort=='Regios' || $doorkijkCategorieSoort=='Beleggingssectoren')
		  	$filter=$belCategorien;
			...
			if($grafiekTonen==true)
			  $this->printPie( $grafiekdata,30+$xOffset,125,$doorkijkTitelsGrafiek[$doorkijkCategorieSoort]); //+$yOffset);
			$pieTeller++;
		}
