...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: ATTberekening_L48.php,v $
 		Revision 1.16  2020/07/18 14:58:47  rvv
 		*** empty log message ***
 		
 		Revision 1.15  2019/11/13 14:49:22  rvv
 		*** empty log message ***
 		
 		Revision 1.14  2019/11/06 16:11:20  rvv
 		*** empty log message ***
 		
 		Revision 1.13  2017/05/26 16:45:07  rvv
 		*** empty log message ***
 		
 		Revision 1.12  2015/12/19 09:03:50  rvv
 		*** empty log message ***
...
class rapportHulp_L48
{
  function rapportHulp_L48()
  {
    $this->test='';
  }
}
...
class ATTberekening_L48
{

	function ATTberekening_L48($rapportData)
	{
  ...
  {
    $this->rapport = new rapportHulp_L48();
    $db=new DB();
    ...
    $perfData=$this->bereken(substr($datumBegin,0,10), $datumEind,'totaal');
...
foreach ($perioden as $periode)
{
    ...
    $datumEind=$periode['stop'];
     ...
     while($grootboekrekening=$DB->nextRecord())
      ...
      if(empty($storting['gewogen']) && $storting['totaal']>0)
        $storting['gewogen']=$storting['totaal'];
    ...
    if($categorie=='totaal')
    {
      ...
      //echo $categorie." $datumEind  ". ($performance*100) ." = ((($eindwaarde - $beginwaarde) - ".$storting['totaal'].") / ($totaalBeginwaarde + ".$storting['gewogen']."))<br>\n";ob_flush();
