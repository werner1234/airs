...
class RapportVOLKD_L13
{
	function RapportVOLKD_L13($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function flipFill($fill)
  {
	...
	function writeRapport()
	{
		...
		$this->pdf->widthB = array(5,65,20,20,1,21,1,27,1,21,20,20,20,20,12);
		$this->pdf->widthA = array(  70,20,20,1,21,1,27,1,21,20,20,20,20,12);
		...
		while($categorien = $DB->NextRecord())
		{
           ...
           $procentResultaat = (($totaalactueel - $totaalbegin + $totaaldividendCorrected) / ($totaalbegin /100));
			...
			//	" TijdelijkeRapportage.beginPortefeuilleWaardeEuro / $beginQuery as beginPortefeuilleWaardeEuro, ".
				...
				.$__appvar['TijdelijkeRapportageMaakUniek'].
				" ORDER BY TijdelijkeRapportage.valutaVolgorde asc, TijdelijkeRapportage.Lossingsdatum, TijdelijkeRapportage.fondsOmschrijving asc";//exit;
...
//echo $subquery."<br><br>";exit;
      ...
      $prevValuta = '';
			...
			{
		    if($n>$regels-2 && $this->pdf->GetY()>200)//
        {
           $this->pdf->AddPage();
        }
        if($kopPrinted==false)
        ...
        if ( $prevValuta !== $subdata['valuta'] ) {

          $prevValuta = $subdata['valuta'];
          if($this->pdf->getY()+$this->pdf->rowHeight > $this->pdf->PageBreakTrigger)
          	$this->pdf->addPage();
          $this->pdf->SetWidths($this->pdf->widthA);
          $this->pdf->SetAligns($this->pdf->alignA);
          $this->pdf->row(array(
            "Waarden " . $subdata['valuta'],
            '',
            '',
            ...
            $lastFlip = $this->flipFill($lastFlip);
        }
