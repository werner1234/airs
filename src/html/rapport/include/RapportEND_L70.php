...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2016/08/13 16:55:26 $
File Versie					: $Revision: 1.4 $

$Log: RapportEND_L70.php,v $
...
Revision 1.1  2016/06/19 15:22:08  rvv
*** empty log message ***
...
class RapportEND_L70
{
	function RapportEND_L70($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
      ...
      for($i=0;$i<5;$i++)
      {
        if ((count($this->pdf->pages)+1)%4)
        {
          ...
          if(!isset($this->pdf->templateVars['NotitiePaginas']))
          {
            ...
            $this->pdf->templateVarsOmschrijving['NotitiePaginas']='Notities';
          }
          $this->maakNotities();
        }
        else
  ...
  $this->pdf->SetWidths(array(30,105,32,100));
 //   echo array_sum($this->pdf->widths);exit;//267
  ...
  {
    $this->pdf->SetFillColor($this->pdf->cellColorLight[0],$this->pdf->cellColorLight[1],$this->pdf->cellColorLight[2]);
    $this->pdf->rect($this->pdf->marge,30+5,267,160,'F');
