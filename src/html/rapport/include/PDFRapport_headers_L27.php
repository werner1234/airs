...
/*
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2019/01/19 13:54:10 $
 		File Versie					: $Revision: 1.8 $

 		$Log: PDFRapport_headers_L27.php,v $
 		Revision 1.8  2019/01/19 13:54:10  rvv
 		*** empty log message ***
 		
 		Revision 1.7  2017/11/09 06:49:14  rvv
 		*** empty log message ***
 		...
 		*** empty log message ***
 		...
 		Revision 1.4  2011/03/23 17:01:47  rvv
 		*** empty log message ***
...
*/
function Header_basis_L27($object)
{
 $pdfObject = &$object;
    ...
    elseif ($pdfObject->rapport_type == "FRONT"||$pdfObject->rapport_type == "END")
    {
    ...
    else
    {
  	if($pdfObject->rapportCounter <> $pdfObject->rapportCounterLast)
		...
		{
		  if($pdfObject->rapport_layout == 12 || $pdfObject->rapport_layout == 5 || $pdfObject->rapport_layout == 7 || $pdfObject->rapport_layout == 25)
		  {
	...
	function HeaderVKM_L27($object)
	{
		$pdfObject = &$object;
		$pdfObject->HeaderVKM();
	}

function HeaderEND_L27($object)
{
  $pdfObject = &$object;
  
}

  function HeaderVOLK_L27($object)
	{
	    $pdfObject = &$object;
  ...
  function HeaderATT_L27($object)
	...
	function HeaderPERFG_L27($object)
	{
    $pdfObject = &$object;
	}


  function HeaderTRANS_L27($object)
	{
    $pdfObject = &$object;
...
class RapportHulpVKM_L27
{
	function RapportHulpVKM_L27($portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
