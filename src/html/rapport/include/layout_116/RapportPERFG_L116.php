...
class RapportPERFG_L116
{
	function RapportPERFG_L116($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
foreach ($indexData as $index=>$data)
{
  if($data['datum'] != '0000-00-00')
  {
    ...
    if ( ! empty ($this->pdf->portefeuilledata['SpecifiekeIndex']) ) {
  ...
  if(count($rendamentWaarden) > 0)
   {
     ...
     $totaal=array();
        ...
        foreach($kleuren as $index=>$kleur)
        {
          ...
          if($index==1 && ! empty ($this->pdf->portefeuilledata['SpecifiekeIndex'])) {
            $this->pdf->rect($this->pdf->getX()-2,$this->pdf->getY()+1,2,2,'F','',$kleur);
