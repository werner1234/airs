...
class RapportVKMS_L116
{
  function RapportVKMS_L116($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum,$laatstejaar=true)
  {
    if(is_object($pdf))
    {
      $this->pdf = &$pdf;
    ...
    while($data = $DB->nextRecord())
    {
      if($data['Kosten']==1)
      {
        if ($this->pdfVullen == true)
        {
          ...
          $rowOmschrijving = $data['Omschrijving'];
          if ( $rowOmschrijving === 'Beheervergoeding' ) {
            $rowOmschrijving = $rowOmschrijving . ' inclusief transacties en BTW';
          }
          $this->pdf->row(array(vertaalTekst($rowOmschrijving, $this->pdf->rapport_taal), $this->formatGetal($data['totaal'], 0), $this->formatGetal($kostenProcent, 2) . ' %'));
