...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2019/12/18 15:53:15 $
File Versie					: $Revision: 1.6 $

$Log: RapportVKMS_L40.php,v $
Revision 1.6  2019/12/18 15:53:15  rvv
*** empty log message ***

Revision 1.5  2019/02/03 15:50:24  rvv
*** empty log message ***

Revision 1.4  2019/01/16 08:41:44  rvv
*** empty log message ***

Revision 1.3  2019/01/12 17:08:31  rvv
*** empty log message ***
...
class RapportVKMS_L40
{
  function RapportVKMS_L40($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
  {
    if(is_object($pdf))
    {
      $this->pdf = &$pdf;
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    if($this->pdfVullen==true)
    {
      ...
      $this->pdf->setWidths(array(118, 30, 30,25));
    ...
    while($data = $DB->nextRecord())
    {
      if($this->pdfVullen==true)
      {
        ...
        if($data['Grootboekrekening']=='BEH')
        {
          ...
          $this->pdf->row(array('Groenstate Vermogensbeheer'));
          $this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
...
ORDER BY HoofdcategorienVolgorde.Afdrukvolgorde, HoofdBeleggingscategorien.Afdrukvolgorde,BeleggingscategorienVolgorde.Afdrukvolgorde, BeleggingscategorienVolgorde.Afdrukvolgorde, Beleggingscategorien.Afdrukvolgorde,fondsVolgorde,OptieBovenliggendFonds,FondsOmschrijving ";
 ...
 WHERE Portefeuilles.portefeuille='".$this->portefeuille."'";
    ...
    $waardeNaKostenKleur=array(255,227,144);
    $cumulatieveKostenKleur=array(208,188,188);
    
    if($this->pdf->getY()+65>$this->pdf->pagebreak)
      $this->pdf->addPage();
