...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2019/02/16 19:23:35 $
File Versie					: $Revision: 1.29 $

$Log: RapportHSE_L33.php,v $
Revision 1.29  2019/02/16 19:23:35  rvv
*** empty log message ***

Revision 1.28  2019/02/13 16:42:08  rvv
*** empty log message ***
...
class RapportHSE_L33
{
	function RapportHSE_L33($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    if($data['bijdrage'] < 0)
      ...
      $this->pdf->CellFontColor = array('','','','','','','','','','','','',$this->pdf->rapport_font_groen);
   	...
   	$this->pdf->row(array(substr(vertaalTekst($categorieData['omschrijving'],$this->pdf->rapport_taal),0,25),
                        ...
                        $this->formatGetal($data['weging']*100,2,true,true),
                        $this->formatGetal($data['bijdrage']*100,2,true,true)));
...
$this->pdf->printValutaPerformanceOverzicht($this->portefeuille, $this->rapportageDatum, $this->rapportageDatumVanaf);
    ...
    if($useBar==true)
    {
      $this->pdf->setXY($startX + 25, $y - 15);
      $this->BarDiagram($width * 2, $height, $valutaVerdeling, '%l (%p)', $grafiekKleur);
    }
    else
    {
...
$this->pdf->SetXY($this->pdf->GetX(),$y1-5);
      ...
      //for($i=0; $i<$this->pdf->NbVal; $i++)
       ...
       $aantalRegels=0;
      foreach($depotVerdeling as $valuta=>$valutaData)
      {
        $aantalRegels+=count($valutaData['depotbanken'])+1;
      }
      if($y1+$aantalRegels*4>$this->pdf->PageBreakTrigger)
      {
        $depotbankenOnderdrukken=true;
      }
      else
      {
        $depotbankenOnderdrukken=false;
      }
      ...
      foreach ($depotVerdeling as $valuta=>$waarden)
      {
          ...
          if($depotbankenOnderdrukken==false)
          {
