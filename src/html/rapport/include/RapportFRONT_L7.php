...
Revision 1.14  2016/03/31 06:30:24  rvv
*** empty log message ***

Revision 1.13  2016/03/30 10:35:05  rvv
*** empty log message ***

Revision 1.12  2016/03/28 15:53:33  rvv
*** empty log message ***

Revision 1.11  2016/03/27 17:35:07  rvv
*** empty log message ***

Revision 1.10  2016/03/16 14:24:20  rvv
*** empty log message ***

Revision 1.9  2016/02/18 15:26:16  rvv
*** empty log message ***

Revision 1.8  2016/02/18 05:14:09  rvv
*** empty log message ***
...
class RapportFront_L7
{
	function RapportFront_L7($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
else  
{
		...
		if($portefeuilledata['Depotbank']=='TGB')
      $portefeuilledata['Depotbank']='IGS';
    if($portefeuilledata['crmNaam']<>'')
    {
      $portefeuilledata['Naam']=$portefeuilledata['crmNaam'];
      $portefeuilledata['Naam1']=$portefeuilledata['crmNaam1'];
			/*
...
*/
    ...
    $this->pdf->SetAligns(array('L','C','L'));
	  ...
	  $this->pdf->ln(3);
	  $this->pdf->row(array('',vertaalTekst("Rapportage ",$this->pdf->rapport_taal)." $kwartaal ".vertaalTekst("kwartaal",$this->pdf->rapport_taal)." ".date("Y",$this->rapportageDatumJul)));
		$this->pdf->ln(15);
		$this->pdf->SetTextColor($this->pdf->rapport_fontcolor[r],$this->pdf->rapport_fontcolor[g],$this->pdf->rapport_fontcolor[b]);
...
"; 
    ...
    $this->pdf->setWidths(array(5,40,40,140));
    $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
    $this->pdf->Ln();
    $this->pdf->row(array('','Depotbank','Portefeuille','Naam'));
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
    $this->pdf->row(array('',$portefeuilledata['Depotbank'],$portefeuilledata['Portefeuille'],$portefeuilledata['Naam'].' '.$portefeuilledata['Naam1']));
    $this->pdf->setY(100);
