...
Revision 1.2  2015/11/07 16:45:15  rvv
*** empty log message ***
...
class RapportMUT_L24
{
	function RapportMUT_L24($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    $this->pdf->excelData[]=array('Periode','Bankafschrift','Omschrijving','Boekdatum','Rekening','Debet','Credit');
	...
	function writeRapport()
	{
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
		...
		$DB = new DB();
		// voor data
		$this->pdf->widthA = array(20,25,90,25,35,20,25,25,25);
		$this->pdf->alignA = array('R','R','L','R','R','R','R','R');

		// voor kopjes
		$this->pdf->widthB = array(20,25,90,25,35,20,25,25,25);
      ...
      $this->pdf->excelData[]=array(date("n",db2jul($mutaties['Boekdatum'])),  
          	          $mutaties['Afschriftnummer'],
											vertaalTekst($mutaties['Omschrijving'],$this->pdf->rapport_taal),
											date("d-m-Y",db2jul($mutaties['Boekdatum'])),
											$mutaties['Rekening'],
