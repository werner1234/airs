...
$Log: RapportWaardeprognose_L40.php,v $
Revision 1.3  2020/03/14 18:42:03  rvv
*** empty log message ***

Revision 1.2  2020/03/11 16:21:41  rvv
*** empty log message ***

Revision 1.1  2020/03/04 16:40:47  rvv
*** empty log message ***
...
class RapportWaardeprognose_L40
{
	/*
		PDF en CSV
	*/
	...
	function RapportWaardeprognose_L40( $selectData )
	{
		global $__appvar,$USR;
  ...
  function langeTermijngrafiek($pdata)
  {
    ...
    $kostenVelden=array('waardeprognose_kosten_beheer'=>'Kosten van de beleggingsdienstverlening',
                        'waardeprognose_kosten_transactie'=>'Overige',
                        'waardeprognose_kosten_bank'=>'Overige',
                        'waardeprognose_kosten_indirect'=>'Kosten van de financiele instrumenten');
  ...
  //  $this->pdf->row(array('Verwacht rendement',$this->formatGetal($rendement,2).' %'));
 //   $this->pdf->row(array('Doeljaar',$doelJaar));
    ...
    $this->pdf->setY(40);
    $margeLinks=0;
    $this->pdf->setWidths(array($margeLinks,70,25,35,30));
    $this->pdf->setAligns(array('L','L','R','R','R'));
    $this->pdf->SetFillColor($this->pdf->rapport_kop_bgcolor['r'],$this->pdf->rapport_kop_bgcolor['g'],$this->pdf->rapport_kop_bgcolor['b']);
    $this->pdf->ln();
    $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
    $this->pdf->Rect($this->pdf->marge+$margeLinks,$this->pdf->GetY(),160, 8, 'F');
  
    $this->pdf->SetTextColor($this->pdf->rapport_kop_fontcolor['r'],$this->pdf->rapport_kop_fontcolor['g'],$this->pdf->rapport_kop_fontcolor['b']);
    $this->pdf->row(array('',vertaalTekst("Kostensoort (eerste 12 maanden) incl. BTW",$this->pdf->rapport_taal)."\n ",
                      vertaalTekst("Bedrag",$this->pdf->rapport_taal),
                      vertaalTekst("% Portefeuillewaarde",$this->pdf->rapport_taal),
                      vertaalTekst("% Totale kosten",$this->pdf->rapport_taal)."\n "));
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
    $this->pdf->SetTextColor($this->pdf->rapport_fontcolor['r'],$this->pdf->rapport_fontcolor['g'],$this->pdf->rapport_fontcolor['b']);
  
  
   $kostenTonen=array();
    foreach($kostenVelden as $veld=>$omschrijving)
    {
      $percentage=$this->selectData[$veld];
      $kostenPercentage += $percentage;
      if($veld=='waardeprognose_kosten_transactie'||$veld=='waardeprognose_kosten_bank')
        $veld='waardeprognose_kosten_beheer';
      $kostenTonen[$veld]+=$percentage;
    }
  ...
  //echo $this->waardeprognose_bedrag;exit;
  
    foreach($kostenTonen as $veld=>$percentage)
    {
      if($percentage<>0)
      {
        $omschrijving=$kostenVelden[$veld];
        $kostenAandeel= $percentage/$kostenPercentage;
        $nieuweWaarde=$this->waardeprognose_bedrag*pow(1+($rendement/100),1);
        $kosten=$nieuweWaarde*$percentage/100;
       // echo " $kosten=".$this->waardeprognose_bedrag."*$kostenAandeel/100;<br>\n";
        $this->pdf->row(array('',$omschrijving,$this->formatGetal($kosten,2),$this->formatGetal($percentage,2).' %',$this->formatGetal($kostenAandeel*100,2).' %'));
    ...
    $this->pdf->row(array('','Totale kosten per jaar',$this->formatGetal($kosten,2),$this->formatGetal($kostenPercentage,2).' %',$this->formatGetal(100,2).' %'));
    ...
    $grafiekWaarden['titel']="";
    $waardeZonderKostenKleur=array(30,92,61);
    $waardeNaKostenKleur=array(8,132,206);
    $cumulatieveKostenKleur=array(208,188,188);
    
    if($this->pdf->getY()+70>$this->pdf->pagebreak)
      $this->pdf->addPage();
    
    $this->pdf->setXY(25,68);
    $this->LineDiagram(130, 55, $grafiekWaarden,array($waardeZonderKostenKleur,$waardeNaKostenKleur,$cumulatieveKostenKleur),0,0,4,4,false);//50
  ...
  function LineDiagram($w, $h, $data, $color=null, $maxVal=0, $minVal=0, $horDiv=4, $verDiv=4,$vanafBegin=false)
  {
    ...
    $this->pdf->Rect($XDiag, $YDiag, $w-$margin, $h,'D','',array(255,255,255));
    ...
    if(is_array($data2))
    {
      ...
      $this->pdf->SetFillColor($color2[0],$color2[1],$color2[2]);
      for ($i=0; $i<count($data2); $i++)
      {
        ...
        //{
         // $this->pdf->line($XDiag+$i*$unit, $yval, $XDiag+($i+1)*$unit, $yval2,$lineStyle );
          $this->pdf->rect($XDiag+($i+.75)*$unit,$YDiag+$h,($unit/2),($data2[$i] * $waardeCorrectie*-1),'F',$lineStyle);
        //}
        //$yval = $yval2;
      }
    }
 ...
 //   $aantal=count($legendaItems);
    foreach ($legendaItems as $index=>$item)
    {
      ...
      $this->pdf->Rect($XPage-5+$step , $bodem+15, 3, 3, 'DF','',$kleur);
      $this->pdf->SetXY($XPage+3-5+$step,$bodem+15);
