...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2017/09/20 13:13:13 $
File Versie					: $Revision: 1.4 $

$Log: RapportSCENARIO_L42.php,v $
Revision 1.4  2017/09/20 13:13:13  rvv
*** empty log message ***

Revision 1.3  2017/09/16 18:07:12  rvv
*** empty log message ***

Revision 1.2  2017/07/02 12:10:56  rvv
*** empty log message ***

Revision 1.1  2017/07/01 17:03:24  rvv
*** empty log message ***
...
class RapportSCENARIO_L42
{
	function RapportSCENARIO_L42($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//    {
      ...
      $this->pdf->widthA = array(185,30);
		  $this->pdf->widthB = array(150,35,25,25,22,23);
      ...
      $this->pdf->row(array('',
                            ...
                            vertaalTekst('Z pessimistisch',$this->pdf->rapport_taal),
                            vertaalTekst('Normaal',$this->pdf->rapport_taal),
                            vertaalTekst('Z optimistisch',$this->pdf->rapport_taal)));
      $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
      $negatiefAdvies=true;
      $maxKansTmp=0;
      ...
      foreach($kansData['risicoklassen'] as $risicoklasse=>$klasseData)
      {
        ...
        $this->pdf->row(array('',vertaalTekst($risicoklasse,$this->pdf->rapport_taal),
                            ...
                            $this->formatGetalNegatief($klasseData['uitkomstKans']['scenarioEindwaarden']['Zeer pessimistisch']),
                            $this->formatGetalNegatief($klasseData['uitkomstKans']['scenarioEindwaarden']['Normaal']),
                            $this->formatGetalNegatief($klasseData['uitkomstKans']['scenarioEindwaarden']['Zeer optimistisch'])));
    ...
    $this->pdf->setXY(160,72);
    $this->pdf->SetFont($this->pdf->rapport_font,'b',$this->pdf->rapport_fontsize);
    $this->pdf->Cell(130,0,vertaalTekst('1 Jaarsrendementen',$this->pdf->rapport_taal),0,0,'C');
    $this->pdf->setXY(160,79);
    $this->risicoGrafiek(100,40,$sc->risicoklassen);
...
//
    ...
    $this->pdf->SetFont($this->pdf->rapport_font,'b',$this->pdf->rapport_fontsize);
    $this->pdf->Cell(125,0,vertaalTekst('Scenario-analyse',$this->pdf->rapport_taal)." ".vertaalTekst('vanaf',$this->pdf->rapport_taal).
                        " ".$sc->CRMdata['startdatum']." ".vertaalTekst('tot',$this->pdf->rapport_taal)." ".$sc->CRMdata['doeldatum'],0,0,'C');
    ...
    else
    {
      ...
      foreach($sc->cashflowText as $bedragData)
      {
          $this->pdf->row(array('',$bedragData[0],$this->formatGetal($bedragData[1],0),($bedragData[2]==$bedragData[1])?"":$bedragData[2]));
      }
    }
  ...
  function risicoGrafiek($w, $h, $data)
  {
    $XPage = $this->pdf->GetX();
    $YPage = $this->pdf->GetY();
    $hDiag = $h;
    ...
    $maxHigh=0;
    $n=0;
    $kleuren=array(array(array(165,165,165),array(115,115,115)),
      array(array(198,206,198),array(165,173,173)),
      array(array(132,156,186),array(66,107,156)),
      array(array(206,214,222),array(181,198,206)),
      array(array(165,173,198),array(115,123,181)));
    foreach($data as $categorie=>$categorieData)
    {
      $low=$categorieData['verwachtRendement']-2*$categorieData['klasseStd'];
      $high=$categorieData['verwachtRendement']+2*$categorieData['klasseStd'];
      $plotData[$categorie]=array('low'=>$low,'exp'=>$categorieData['verwachtRendement'],'high'=>$high,'lowCol'=>$kleuren[$n][1],'highCol'=>$kleuren[$n][0]);
      $maxLow=min($low,$maxLow);
      $maxHigh=max($high,$maxHigh);
      $n++;
    }
    $maxLow=floor($maxLow/10)*10;
    $maxHigh=ceil($maxHigh/10)*10;

    $bereik=$maxHigh-$maxLow;
//echo " $maxLow $maxHigh $bereik <br>\n ";
   // listarray($plotData);
    ...
    $yStep=$h/$bereik;
    $xStep=$w/(count($plotData)+0.5);
    $yNul=$bodem+($maxHigh-$bereik)*$yStep;

    //$this->pdf->Rect($XDiag, $YPage,$w ,$h,'F','',array(200,200,200));

    $this->pdf->Text($XDiag-6, $YPage-4, "Rendement (%)");
    $n=0;
    for($i=$maxLow; $i<= $maxHigh; $i+= 10)
    {
      $xplot=$XDiag;
      $this->pdf->Line($xplot, $yNul-$yStep*$i, $xplot+$w,$yNul-$yStep*$i,array('dash' => 1,'color'=>array(125,125,125)));
      $this->pdf->Text($xplot-6, $yNul-$yStep*$i, $i." %");
      $n++;
      if($n >20)
        break;
    }
    ...
    $n=0;
    $yExtra=0;
    foreach($plotData as $categorie=>$waarden)
    {
      $XStart=$XDiag+($n+0.5)*$xStep;
      $lowY=$waarden['low']*$yStep*-1;
      $expY=$waarden['exp']*$yStep*-1;
      $highY=$waarden['high']*$yStep*-1;

     // listarray($waarden);
      if(is_array($waarden['lowCol']))
        $lowCol=$waarden['lowCol'];
      else
        $lowCol=array(100,100,100);
      if(is_array($waarden['highCol']))
        $highCol=$waarden['highCol'];
      else
        $highCol=array(150,150,150);

      $this->pdf->Rect($XStart, $yNul+$expY, $wCol, $lowY-$expY, 'F', '', $lowCol);
      $this->pdf->Rect($XStart, $yNul+$expY, $wCol, $highY-$expY, 'F', '', $highCol);
      $this->pdf->Text($XStart, $yNul+$lowY+2, round($waarden['low'],1)." %");
      $this->pdf->Text($XStart+6, $yNul+$expY+1, round($waarden['exp'],1)." %");
      $this->pdf->Text($XStart, $yNul+$highY-1, round($waarden['high'],1)." %");

      $this->pdf->Rect($XDiag+$w+2, $YPage+$yExtra+$n*6,1, 2, 'F', '', $highCol);
      $this->pdf->Rect($XDiag+$w+3, $YPage+$yExtra+$n*6,1, 2, 'F', '', $lowCol);
      $this->pdf->Text($XDiag+$w+5, $YPage+$yExtra+2+$n*6, $categorie);
