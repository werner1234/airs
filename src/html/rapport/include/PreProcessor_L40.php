...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: PreProcessor_L40.php,v $
 		Revision 1.26  2020/03/28 15:45:39  rvv
 		*** empty log message ***
 		
 		Revision 1.25  2020/03/25 16:43:07  rvv
 		*** empty log message ***
 		
 		Revision 1.24  2020/03/23 07:36:42  rvv
 		*** empty log message ***
 		
 		Revision 1.23  2020/03/11 15:18:12  rvv
 		*** empty log message ***
 		
 		Revision 1.22  2019/10/23 13:34:01  rvv
 		*** empty log message ***
 		
 		Revision 1.21  2017/09/20 13:13:13  rvv
 		*** empty log message ***
 		
 		Revision 1.20  2017/08/05 17:25:06  rvv
 		*** empty log message ***
 		
 		Revision 1.19  2017/08/02 18:23:27  rvv
 		*** empty log message ***
 		
 		Revision 1.18  2017/07/29 17:18:20  rvv
 		*** empty log message ***
 		
 		Revision 1.17  2017/07/23 13:36:28  rvv
 		*** empty log message ***
...
class PreProcessor_L40
{
	function PreProcessor_L40($portefeuille,$datum='',$pdf)
  {
    global $__appvar;
    ...
    {
      if(abs($waarden['waardeEUR']-$waardenPerDagAfter[$datum]['waardeEUR']) > 1)
      {
        echo "voor/na pre_processor $portefeuille $datum waardeEUR ".$waarden['waardeEUR']." <> ".$waardenPerDagAfter[$datum]['waardeEUR']."<br>\n";
        if (!requestType('ajax'))
        {
          exit;
        }
      }
      if(abs($waarden['aantal'] - $waardenPerDagAfter[$datum]['aantal']) > 1)
      {
        echo "voor/na pre_processor $portefeuille $datum aantal ".$waarden['aantal']." <> ".$waardenPerDagAfter[$datum]['aantal']."<br>\n";
        if (!requestType('ajax'))
        {
          exit;
        }
...
//exit;
  ...
  {
    global $__appvar;
    $optieCategorien=array('AAND-KO','AAND-LO','AAND');
...
WHERE Fondsen.OptieBovenliggendFonds <> '' AND $vasteWhere";
    ...
    $queries=array();
	  while($data=$this->db->nextRecord())
	  {
	   ...
	   if(db2jul(substr($data['OptieExpDatum'],0,4)."-".substr($data['OptieExpDatum'],4,2)."-15") < (db2jul($dag)+(30*24*3600)))
       $cat="AAND-KO";
     else
       $cat="AAND-LO";
  ...
  function verwijderKoppelingen($dag)
  {
    ...
    $veldenSql.="TijdelijkeRapportage.Beleggingscategorie='AAND-KO', TijdelijkeRapportage.BeleggingscategorieVolgorde='2', TijdelijkeRapportage.BeleggingscategorieOmschrijving='Aandelen icm opties', ";
    $query="UPDATE TijdelijkeRapportage SET $veldenSql TijdelijkeRapportage.add_date=now() WHERE beleggingscategorie IN('AAND-KO','AAND-LO') AND
    TijdelijkeRapportage.portefeuille = '".$this->portefeuille."' AND TijdelijkeRapportage.rapportageDatum='$dag' ".$__appvar['TijdelijkeRapportageMaakUniek'];
    $this->db->SQL($query); 
    $this->db->Query();
    
    $query="UPDATE TijdelijkeRapportage
INNER JOIN CategorienPerHoofdcategorie as hoofd ON TijdelijkeRapportage.Beleggingscategorie=hoofd.Beleggingscategorie AND hoofd.Vermogensbeheerder='GRO'
INNER JOIN Beleggingscategorien as hoofdOmschrijving ON hoofd.Hoofdcategorie=hoofdOmschrijving.Beleggingscategorie
SET TijdelijkeRapportage.hoofdcategorie=hoofd.Hoofdcategorie,
TijdelijkeRapportage.hoofdcategorieOmschrijving=hoofdOmschrijving.Omschrijving
WHERE   TijdelijkeRapportage.portefeuille = '".$this->portefeuille."' AND TijdelijkeRapportage.rapportageDatum='$dag' ".$__appvar['TijdelijkeRapportageMaakUniek'];
    $this->db->SQL($query);
    $this->db->Query();
  ...
  function updateRegel($fondsData,$optieData,$paarId,$dag)
  {
   ...
   //   $fondsAantalCorrectie=true;
    ...
    elseif(($fondsData['totaalAantal'] - $aantal) > 0)
    {
      ...
      //echo "$paarId $dag $bovenliggendFonds ".$fondsData['totaalAantal']." == $aantal ".$optieData['fondsOmschrijving']." ------<br>\n";
      $query="SELECT historischeWaarde,historischeValutakoers,beginwaardeLopendeJaar,fondsOmschrijving,actueleFonds,valuta,rapportageDatum,
