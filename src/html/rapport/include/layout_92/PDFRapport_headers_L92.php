...
/* 	
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: PDFRapport_headers_L92.php,v $
...
*/
function Header_basis_L92($object)
{
 $pdfObject = &$object;
...
function HeaderHSE_L92($object)
{
  ...
  $pdfObject->SetFont($pdfObject->rapport_font,$pdfObject->rapport_kop_fontstyle,$pdfObject->rapport_fontsize);
    ...
    $eindactueel 	= $actueel+ $pdfObject->widthB[8]+ $pdfObject->widthB[9]+ $pdfObject->widthB[10];//+array_sum($pdfObject->widthB);
  ...
  $pdfObject->MultiCell($pdfObject->widthB[8]+ $pdfObject->widthB[9]+ $pdfObject->widthB[10],4, vertaalTekst("Actuele koers",$pdfObject->rapport_taal), 0, "C");
  ...
  $pdfObject->SetFont($pdfObject->rapport_font,'',$pdfObject->rapport_fontsize);
	...
	function HeaderVKM_L92($object)
	{
		$pdfObject = &$object;
		$pdfObject->HeaderVKM();
	}
	
	  function HeaderFRONT_L92($object)
	  {
	    $pdfObject = &$object;
	    //$pdfObject->headerSCENARIO();

	  }
	  function HeaderSCENARIO_L92($object)
	  {
	    $pdfObject = &$object;
	    //$pdfObject->headerSCENARIO();
    ...
    function HeaderRISK_L92($object)
    {
	    $pdfObject = &$object;
			$pdfObject->Ln();
			$pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
...
function HeaderGRAFIEK_L92($object)
{
  $pdfObject = &$object;
  $pdfObject->Ln();
  $pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
  ...
  $pdfObject->SetTextColor($pdfObject->rapport_kop_fontcolor['r'],$pdfObject->rapport_kop_fontcolor['g'],$pdfObject->rapport_kop_fontcolor['b']);
    ...
    function HeaderPERFD_L92($object)
    {
      $pdfObject = &$object;
      HeaderPERF_L92($object);
      $pdfObject->SetTextColor($pdfObject->rapport_fontcolor['r'],$pdfObject->rapport_fontcolor['g'],$pdfObject->rapport_fontcolor['b']);
    }
  
    
    function HeaderTRANS_L92($object)
	  {
	    $pdfObject = &$object;
    	$pdfObject->SetFont($pdfObject->rapport_font,$pdfObject->rapport_kop_fontstyle,$pdfObject->rapport_fontsize);
  		$pdfObject->SetX(100);
...
$pdfObject->ln(2);
	  ...
	  function HeaderINHOUD_L92($object)
	  {
	    $pdfObject = &$object;
	    //$pdfObject->headerSCENARIO();

	  }	  
 	  function HeaderPERF_L92($object)
	  {
	    $pdfObject = &$object;
      $pdfObject->ln();
      $pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
    ...
    function HeaderINDEX_L92($object)
	  {
	    $pdfObject = &$object;
	  }
function HeaderTRANSFEE_L92($object)
{
  $pdfObject = &$object;
}

function HeaderFISCAAL_L92($object)
{
  $pdfObject = &$object;
  $pdfObject->headerFiscaal();
}
    function HeaderOIB_L92($object)
	  {
	    $pdfObject = &$object;
      $pdfObject->Ln();
   		$pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
  ...
  function HeaderATT_L92($object)
	{
    $pdfObject = &$object;
    $w=($pdfObject->w-$pdfObject->marge*2)/10;
...
*/
   ...
   function HeaderPERFG_L92($object)
	{
    $pdfObject = &$object;
...
*/
  ...
  function HeaderVOLK_L92($object)
	{
    $pdfObject = &$object;
   	$pdfObject->ln();
		$pdfObject->SetFont($pdfObject->rapport_font,$pdfObject->rapport_kop_fontstyle,$pdfObject->rapport_fontsize);
...
function HeaderVHO_L92($object)
  ...
  $tmpY = $pdfObject->GetY();
  $pdfObject->SetX($pdfObject->marge+$huidige+5);
  $pdfObject->MultiCell($eindhuidige - $huidige - 5 ,4, vertaalTekst("Kostprijs",$pdfObject->rapport_taal), 0, "C");
  $pdfObject->SetY($tmpY);
  $pdfObject->SetX($pdfObject->marge+$actueel);
...
function printRendement_L92($object,$portefeuille, $rapportageDatum, $rapportageDatumVanaf, $kort=false)
{
  $pdfObject = &$object;
  global $__appvar;
  
  $rapStartJaar=substr($rapportageDatumVanaf,0,4);
  if(db2jul($object->PortefeuilleStartdatum) > db2jul( "$rapStartJaar-01-01"))
    $beginJaar =  substr($object->PortefeuilleStartdatum,0,10);
  else
    $beginJaar = $rapStartJaar.'-01-01';
  
  if($rapportageDatumVanaf<> $beginJaar)
  {
    $perioden=array('verslagperiode'=>array($rapportageDatumVanaf,$rapportageDatum),'lopend jaar'=>array($beginJaar,$rapportageDatum));
  }
  else
  {
    $perioden=array('verslagperiode'=>array($rapportageDatumVanaf,$rapportageDatum));
  }

  $DB= new DB();
  $resultaat=array();
  $performance=array();
  foreach($perioden as $kop=>$periode)
  {
    $rapportageDatumVanaf=$periode[0];
    $rapportageDatum=$periode[1];
    ...
    $resultaat[$kop] = ($actueleWaardePortefeuille -
      $vergelijkWaarde -
    ...
    $performance[$kop] = performanceMeting($portefeuille, $rapportageDatumVanaf, $rapportageDatum, $pdfObject->portefeuilledata['PerformanceBerekening'], $pdfObject->rapportageValuta);
  }
  $pdfObject->ln(2);
  ...
  $pdfObject->Rect($pdfObject->marge,$pdfObject->getY(),110,$hoogte,'F');
  $pdfObject->SetFillColor(0);
  $pdfObject->Rect($pdfObject->marge,$pdfObject->getY(),110,$hoogte);
  $pdfObject->ln(1);
  //$pdfObject->SetX($pdfObject->marge);
  $pdfObject->SetX($pdfObject->marge);
  ...
  foreach($perioden as $kop=>$periode)
  {
    if (!$kort)
    {
      ...
      {
        $pdfObject->Cell(80, 4, vertaalTekst("Resultaat over $kop", $pdfObject->rapport_taal), 0, 0, "L");
      }
      $pdfObject->Cell(30, 4, $pdfObject->formatGetal($resultaat[$kop], 2), 0, 1, "R");
    }
    $pdfObject->SetX($pdfObject->marge);
    ...
    {
      $pdfObject->Cell(80, 4, vertaalTekst("Rendement over $kop", $pdfObject->rapport_taal), 0, 0, "L");
    }
    $pdfObject->Cell(30, 4, $pdfObject->formatGetal($performance[$kop], 2) . "%", 0, 1, "R");
    $pdfObject->ln();
  }
  $pdfObject->ln(2);
}
