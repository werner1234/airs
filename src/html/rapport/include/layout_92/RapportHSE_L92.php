...
class RapportHSE_L92
{
	function RapportHSE_L92($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum, $valuta = 'EUR')
	{
		$this->pdf = &$pdf;
    ...
    $totaalBtxt='';
		...
		$this->pdf->SetFont($this->pdf->rapport_font,$this->pdf->rapport_totaal_fontstyle,$this->pdf->rapport_fontsize);
			...
			$this->pdf->Cell($this->pdf->widthB[6],4,$totaalAtxt, 0,0, "R");
			$this->pdf->Cell($this->pdf->widthB[7] + $this->pdf->widthB[8]+ $this->pdf->widthB[9]+ $this->pdf->widthB[10],4,$totaalBtxt, 0,0, "R");
    ...
    $maxDagenOud=$dagen['VerouderdeKoersDagen'];
	  ...
	  $this->pdf->widthB = array(10,55,20,12,20,25,25,15,20,25,25,25);
		$this->pdf->alignB = array('L','L','R','R','R','R','R','R','R','R','R','R');
    ...
    $totaalactueel=0;
		while($categorien = $DB->NextRecord())
		{
		  ...
		  $DB2 = new DB();
			...
			if(!empty($lastCategorie) && $lastCategorie <> $categorien['BeleggingscategorieOmschrijving'])
			{
        ...
        if($this->pdf->rapport_type=='HSE')
        {
          $subdata['historischeWaardeTotaal'] = $subdata['beginPortefeuilleWaardeEuro'];
        }
        else
        {
          $subdata['beginwaardeLopendeJaar']=$subdata['historischeWaarde'];
          $subdata['beginPortefeuilleWaardeInValuta']=$subdata['historischeWaardeTotaal'];
          $subdata['beginPortefeuilleWaardeEuro']=$subdata['historischeWaardeTotaalValuta'];
          $subdata['historischeWaardeTotaal']=$subdata['historischeWaardeTotaalValuta'];
        }
        ...
        $procentResultaat = (($subdata['actuelePortefeuilleWaardeEuro'] - $subdata['historischeWaardeTotaal']) / ($subdata['historischeWaardeTotaal'] /100));
        //echo $subdata['fondsOmschrijving']." ".$this->rapportageDatum."<br>\n";
        //echo "$procentResultaat = ((".$subdata['actuelePortefeuilleWaardeEuro']." - ".$subdata['historischeWaardeTotaal'].") / (".$subdata['historischeWaardeTotaal']." /100));<br>\n";
        //$procentResultaat = (($subdata['actueleFonds'] - $subdata['beginwaardeLopendeJaar']) / ($subdata['beginwaardeLopendeJaar'] /100));
    ...
    printRendement_L92($this->pdf,$this->portefeuille, $this->rapportageDatum, $this->rapportageDatumVanaf,false,$this->pdf->rapportageValuta);
