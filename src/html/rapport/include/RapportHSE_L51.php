...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2018/11/07 17:08:06 $
File Versie					: $Revision: 1.3 $

$Log: RapportHSE_L51.php,v $
Revision 1.3  2018/11/07 17:08:06  rvv
*** empty log message ***

Revision 1.2  2017/10/28 18:03:18  rvv
*** empty log message ***

Revision 1.1  2017/06/21 16:10:36  rvv
*** empty log message ***

Revision 1.11  2017/06/18 09:18:24  rvv
*** empty log message ***
...
class RapportHSE_L51
{

  function RapportHSE_L51($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
  {
    $this->pdf = &$pdf;
    $this->pdf->rapport_type = "HSE";
    $this->pdf->rapport_datum = db2jul($rapportageDatum);
    $this->pdf->rapport_datumvanaf = db2jul($rapportageDatumVanaf);
    $this->pdf->rapport_titel = "Portefeuille risicoparameters";
  ...
  function rechtsOnder($riskData)
  {
    ...
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
    $this->pdf->row(array('',vertaalTekst('Standaarddeviatie',$this->pdf->rapport_taal),$this->formatGetal($riskData['standaarddeviatie'],1).'%',$body));
    $this->pdf->ln(2);
    $this->pdf->row(array('',vertaalTekst('AFM-Standaarddeviatie',$this->pdf->rapport_taal),$this->formatGetal($riskData['standaarddeviatieAFM'],1).'%',$body));
    $this->pdf->ln(2);
    $this->pdf->row(array('',vertaalTekst('Standaarddeviatie benchmark',$this->pdf->rapport_taal),$this->formatGetal($riskData['standaarddeviatieBenchmark'],1).'%',$body));
    $this->pdf->ln(2);
  ...
  function addPerfGrafiek($stdev)
  {
    ...
    $db=new DB();
    $query="SELECT grafiekKleur FROM BeleggingscategoriePerFonds WHERE Vermogensbeheerder='".$this->pdf->portefeuilledata['Vermogensbeheerder']."' AND Fonds='".$this->index['SpecifiekeIndex']."' ";
    $db->SQL($query);
    $db->query();
    $kleurData=$db->lookupRecord();
    $kleur=unserialize($kleurData['grafiekKleur']);

    if($kleur['R']['value'] <> 0 || $kleur['G']['value'] <> 0 || $kleur['B']['value'] <> 0)
      $indexKleur=array($kleur['R']['value'],$kleur['G']['value'],$kleur['B']['value']);
    else
      $indexKleur=array($this->pdf->rapport_kop_kleur[0],$this->pdf->rapport_kop_kleur[1],$this->pdf->rapport_kop_kleur[2]);
