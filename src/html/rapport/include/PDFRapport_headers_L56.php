...
/* 	
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2020/03/28 16:26:42 $
 		File Versie					: $Revision: 1.31 $
 		
 		$Log: PDFRapport_headers_L56.php,v $
 		Revision 1.31  2020/03/28 16:26:42  rvv
 		*** empty log message ***
 		
 		Revision 1.30  2019/06/22 16:31:44  rvv
 		*** empty log message ***
 		
 		Revision 1.29  2019/04/17 11:20:00  rvv
 		*** empty log message ***
 		
 		Revision 1.28  2019/04/06 17:11:28  rvv
 		*** empty log message ***
 		
 		Revision 1.27  2019/02/07 07:19:42  rvv
 		*** empty log message ***
 		
 		Revision 1.26  2018/12/27 15:10:29  rvv
 		*** empty log message ***
 		
 		Revision 1.25  2018/10/03 15:43:35  rvv
 		*** empty log message ***
 		
 		Revision 1.24  2018/07/18 15:46:07  rvv
 		*** empty log message ***
 		
 		Revision 1.23  2018/04/21 17:55:51  rvv
 		*** empty log message ***
 		
 		Revision 1.22  2017/12/30 16:38:17  rvv
 		*** empty log message ***
...
*/
function Header_basis_L56($object)
{
 $pdfObject = &$object;
  ...
  if ($pdfObject->rapport_type == "BRIEF")
    ...
    {
     // echo $pdfObject->w." -> ".$pdfObject->h."<br>\n"; ob_flush();
      if($pdfObject->w > 250)
      {
    ...
    $pdfObject->SetY($pdfObject->h-15);
    $pdfObject->SetFont($pdfObject->rapport_font,'',$pdfObject->rapport_voetfontsize);
    $pdfObject->MultiCell(240,4,$pdfObject->rapport_voettext2,'0','L');
    ...
    $pdfObject->AutoPageBreak=true;
		...
		{		
		  ...
		  $pdfObject->rapport_koptext = str_replace("{Naam2}", trim($pdfObject->rapport_naam2.' '.getExtraCrmNaam($pdfObject->rapport_portefeuille)), $pdfObject->rapport_koptext);
		}
	...
	function HeaderVKM_L56($object)
	{
		$pdfObject = &$object;
		$pdfObject->HeaderVKM();
	}

function HeaderVKMS_L56($object)
{
  $pdfObject = &$object;
}
	
	  function HeaderFRONT_L56($object)
	  {
	    $pdfObject = &$object;
...
function HeaderTRANSFEE_L56($object)
{
	$pdfObject = &$object;
	$pdfObject->ln();
  ...
  $dataWidth=array(28,50,28,28,28,28,28,28,28);
	...
	$pdfObject->SetLineWidth(0.1);
	  ...
	  function HeaderSCENARIO_L56($object)
	  {
	    $pdfObject = &$object;
      $pdfObject->Ln(2);
    ...
    function HeaderRISK_L56($object)
    {
	    $pdfObject = &$object;
			$pdfObject->Ln();
			$pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
    ...
    function HeaderGRAFIEK_L56($object)
    {
	    $pdfObject = &$object;
    }

    function HeaderEND_L56($object)
    {
     	$pdfObject = &$object;
			$pdfObject->Ln();
			$pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
    ...
    function HeaderPERFD_L56($object)
    {
      $pdfObject = &$object;
      HeaderPERF_L56($object);
      $pdfObject->SetTextColor($pdfObject->rapport_fontcolor['r'],$pdfObject->rapport_fontcolor['g'],$pdfObject->rapport_fontcolor['b']);
    }
  
    
    function HeaderTRANS_L56($object)
	  {
	    $pdfObject = &$object;
...
$pdfObject->ln(2);
	  ...
	  function HeaderINHOUD_L56($object)
	  {
	    $pdfObject = &$object;
	    //$pdfObject->headerSCENARIO();

	  }	  
 	  function HeaderPERF_L56($object)
	  {
	    $pdfObject = &$object;
      $pdfObject->ln();
      $pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
...
$pdfObject->ln();
    ...
    function HeaderINDEX_L56($object)
	  {
	    $pdfObject = &$object;
	  }

function HeaderDOORKIJK_L56($object)
{
  $pdfObject = &$object;
}
    function HeaderOIB_L56($object)
	  {
	    $pdfObject = &$object;
      $pdfObject->Ln();
      		$pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
...
*/
   ...
   function HeaderPERFG_L56($object)
	{
    $pdfObject = &$object;
...
*/
  ...
  function HeaderVOLK_L56($object)
	{
    $pdfObject = &$object;
   	$pdfObject->ln();
		$pdfObject->SetFont($pdfObject->rapport_font,$pdfObject->rapport_kop_fontstyle,$pdfObject->rapport_fontsize);
...
if(!function_exists('formatPortefeuille'))
{
	...
	function getExtraCrmNaam($portefeuille)
	{
    $naw=new NAW();
    $veld='naam2';
    if(isset($naw->data['fields'][$veld]))
    {
      $nawField=$naw->data['table'].'.'.$veld;
      $query="SELECT $nawField FROM ".$naw->data['table']." WHERE portefeuille='".mysql_real_escape_string($portefeuille)."'";
      $db = new DB();
      $db->SQL($query);
      $crmData=$db->lookupRecord();
      return trim($crmData[$veld]);
    }
    else
    {
      return '';
