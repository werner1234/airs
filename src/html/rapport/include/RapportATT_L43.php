...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2016/10/30 13:02:59 $
File Versie					: $Revision: 1.4 $

$Log: RapportATT_L43.php,v $
Revision 1.4  2016/10/30 13:02:59  rvv
*** empty log message ***

Revision 1.3  2016/10/29 15:41:46  rvv
*** empty log message ***

Revision 1.2  2016/10/26 16:13:40  rvv
*** empty log message ***
...
class RapportATT_L43
{
	function RapportATT_L43($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//listarray($this->categorieVolgorde);
  ...
  $indexData = $index->getWaarden(substr($this->pdf->PortefeuilleStartdatum,0,10),$this->rapportageDatum ,$this->portefeuille);
...
foreach ($indexData as $index=>$data)
{
  ...
  if($data['datum'] != '0000-00-00')
  {
    ...
    if($jaar >= $rapportageJaar &&  $kwartaal <=$rapportageKwartaal)
    {
      if(!isset($periodeWaarden['kwartaal'][$kwartaal]))
        $periodeWaarden['kwartaal'][$kwartaal] = $data;
      else
      {
        foreach($somVelden as $veld)
          $periodeWaarden['kwartaal'][$kwartaal][$veld] += $data[$veld];

        $periodeWaarden['kwartaal'][$kwartaal]['waardeHuidige'] = $data['waardeHuidige'];
        $periodeWaarden['kwartaal'][$kwartaal]['index'] = $data['index'];
        $periodeWaarden['kwartaal'][$kwartaal]['datum'] = $data['datum'];
        $periodeWaarden['kwartaal'][$kwartaal]['performance'] = (( (1+$periodeWaarden['kwartaal'][$kwartaal]['performance']/100)  * (1+$data['performance']/100))-1)*100;
      }
    ...
    if($jaar >= $rapportageJaar &&  $kwartaal >= $rapportageKwartaal)
    {
        ...
        $periodeWaarden['maand'][$maand] = $data;
     ...
     //   $this->pdf->CellBorders = array('','US','US','US','US','US','US','US','US','US','US','US');
       ...
       //$this->pdf->SetFillColor(230,230,230);
        ...
        $totaalRendamentIndex=100;
		 ...
		 foreach($periodeWaarden as $periodeNaam=>$rendamentWaardenPeriode)
		 {
			 $totalen=array('totaalRendament'=>100);
       if(isset($lastPeriodeNaam) && $periodeNaam<>$lastPeriodeNaam)
				 $this->pdf->ln();
			 $lastPeriodeNaam=$periodeNaam;
		    foreach ($rendamentWaardenPeriode as $periodeIndex=>$row)
		    {
		      //listarray($row);
          ...
          $this->pdf->CellBorders = array();
					if($periodeNaam=='jaar')
						 $periode=date("Y",$datum);
					elseif($periodeNaam=='kwartaal')
						$periode=date("Y",$datum)." Q".$periodeIndex;
					else
		      ...
		      $this->pdf->row(array($periode ,
		                           ...
		                           if(!isset($totalen['waardeBegin']))
		     										 ...
		     										 $totalen['totaalRendament'] = ($totalen['totaalRendament']*(100+$row['performance']))/100;
		     }
			 ...
			 $this->pdf->row(array(vertaalTekst('Totaal',$this->pdf->rapport_taal),
												 ...
												 '',
												 $this->formatGetal($totalen['totaalRendament']-100,2)
        ...
        $this->LineDiagram(270, 30, $grafiekData,$this->pdf->rapport_grafiek_color,0,0,6,5,false);//50
