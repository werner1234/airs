...
class RapportVOLK_L102
{
	function RapportVOLK_L102($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		$this->pdf->rapport_type = "VOLK";
		$this->pdf->rapport_datum = db2jul($rapportageDatum);
		$this->pdf->rapport_datumvanaf = db2jul($rapportageDatumVanaf);
		$this->pdf->rapport_titel = "Portefeuillestaat";
...
*/
  ...
  function addLines($title='')
	{
		$skip=array(0,5);
    $doubleLine=array(1,4,8);
		$width=$this->pdf->marge;
		$huidigeY=$this->pdf->getY();
	//	echo "$title $huidigeY ".$this->pageStart."<br>\n";
		if($huidigeY==$this->pageStart)
			return;
		foreach($this->pdf->widthA as $i=>$w)
		{
			$width+=$w;
			if(!in_array($i,$skip))
      {
      	if(in_array($i,$doubleLine))
      		$this->pdf->setLineWidth($this->lineWidthBackup*3);
      	else
      		$this->pdf->setLineWidth($this->lineWidthBackup);
        $this->pdf->line($width, $this->pageStart, $width, $this->pdf->getY());
      }
		}
	...
	function writeRapport()
	{
		...
		$this->pdf->widthB = array(1,63,16,18,20,21,1,17,20,20,18,18,18,18,12);
		$this->pdf->widthA = array(  64,16,18,20,21,1,17,20,20,18,18,18,18,12);
		...
		$this->pdf->AddPage();
    $this->lineWidthBackup=$this->pdf->LineWidth;
