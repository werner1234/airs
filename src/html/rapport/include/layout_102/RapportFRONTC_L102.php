...
class RapportFrontC_L102
{
	function RapportFrontC_L102($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    $query = "SELECT
                ...
                Vermogensbeheerders.Email
                          ...
                          WHERE
                            Portefeuilles.Portefeuille IN('".implode("','",$this->pdf->portefeuilles)."') ORDER BY Portefeuilles.Portefeuille ";
   ...
   //background
    ...
    if($_POST['anoniem']!=1)
    {
      ...
      {
        $this->pdf->row(array('',$pdata['Portefeuille'],$pdata['DepotbankOmschrijving'],vertaalTekst($pdata['Naam'],$this->pdf->rapport_taal),vertaalTekst($pdata['SoortOvereenkomst'],$this->pdf->rapport_taal)));
        $this->pdf->Ln(1);
      }
    }
    $this->pdf->SetWidths($this->pdf->widthA);
