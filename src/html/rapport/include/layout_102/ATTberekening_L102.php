...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		*** empty log message ***
 		...
 		Revision 1.3  2015/09/05 16:48:04  rvv
 		*** empty log message ***
 		
 		Revision 1.2  2015/04/01 16:00:45  rvv
 		*** empty log message ***
 		
 		Revision 1.1  2015/03/04 16:30:29  rvv
 		*** empty log message ***
...
class ATTberekening_L102
{

	function ATTberekening_L102($rapportData,$periode='maanden')
	{
...
ORDER BY HoofdBeleggingscategorien.Afdrukvolgorde,Beleggingscategorien.Afdrukvolgorde,Beleggingssectoren.Afdrukvolgorde,Fondsen.Omschrijving ";
 		  ...
 		  if($data['Beleggingssector']=='')
      {
        ...
        $perFonds[$data['Fonds']]['omschrijving']=$data['FondsOmschrijving'];
        $perFonds[$data['Fonds']]['fondsen'][]=$data['Fonds'];
        $perCategorie[$data['Beleggingscategorie']]['omschrijving']=$data['categorieOmschrijving'];
...
WHERE
IndexPerBeleggingscategorie.Beleggingscategorie='" . mysql_real_escape_string($categorie) . "' AND
IndexPerBeleggingscategorie.Vermogensbeheerder =  '" . $this->rapport->pdf->portefeuilledata['Vermogensbeheerder'] . "'
AND (IndexPerBeleggingscategorie.Portefeuille = ''	OR IndexPerBeleggingscategorie.Portefeuille = '" . $this->rapport->portefeuille . "')
ORDER BY IndexPerBeleggingscategorie.Portefeuille limit 1";
  
      $DB->SQL($query);

      $DB->Query();
      $fondsData = $DB->nextRecord();
    //if(!isset($this->normData[$categorie]))
    //{
      if ($DB->records() > 0)
      {
        $query="SELECT DatumVanaf FROM NormwegingPerBeleggingscategorie WHERE Portefeuille='" . $this->rapport->portefeuille . "' AND DatumVanaf <='$van' ORDER BY DatumVanaf desc limit 1 ";
        $DB->SQL($query);
        $DB->Query();
        $datum=$DB->nextRecord();
        
        $query = "SELECT Normweging FROM NormwegingPerBeleggingscategorie WHERE Beleggingscategorie='" . mysql_real_escape_string($categorie) . "' AND Portefeuille= '" . $this->rapport->portefeuille . "' AND DatumVanaf ='".$datum['DatumVanaf']."' ORDER BY DatumVanaf desc limit 1";
        $DB->SQL($query);
        $DB->Query();
        $weging = $DB->nextRecord();
        $this->normData[$categorie] = $weging['Normweging'];
        $fondsData['Percentage']  = $weging['Normweging']/100;
      }
    //}
    /*
    if(!isset($this->normData[$categorie]))
...
";
      ...
      if ($DB->records() == 0)
      {
        $query = "SELECT IndexPerBeleggingscategorie.Fonds, ZorgplichtPerRisicoklasse.norm/100 as Percentage
FROM IndexPerBeleggingscategorie
JOIN ZorgplichtPerBeleggingscategorie ON IndexPerBeleggingscategorie.Beleggingscategorie = ZorgplichtPerBeleggingscategorie.Beleggingscategorie AND ZorgplichtPerBeleggingscategorie.Vermogensbeheerder = '" . $this->rapport->pdf->portefeuilledata['Vermogensbeheerder'] . "'
JOIN ZorgplichtPerRisicoklasse ON ZorgplichtPerRisicoklasse.Zorgplicht = ZorgplichtPerBeleggingscategorie.Zorgplicht AND ZorgplichtPerRisicoklasse.Vermogensbeheerder = '" . $this->rapport->pdf->portefeuilledata['Vermogensbeheerder'] . "'
WHERE	IndexPerBeleggingscategorie.Vermogensbeheerder = '" . $this->rapport->pdf->portefeuilledata['Vermogensbeheerder'] . "' AND IndexPerBeleggingscategorie.Beleggingscategorie = '$categorie'";
        $DB->SQL($query);
        $fondsData = $DB->lookupRecord();
        //  echo $query;listarray($fondsData);
...
//	     $AttributieStortingenOntrekkingen['totaal'] += $nietToegerekendeKosten['kostenTotaal'];
 ...
 //echo $categorie." $rekeningFondsenWhere  $datumEind  $performance = ((($eindwaarde - $beginwaarde) - ".$AttributieStortingenOntrekkingen['totaal'].") / $gemiddelde)<br>\n";
...
//listarray($indexData);
      ...
      //echo "$resultaat=($eindwaarde - $beginwaarde) - ".$AttributieStortingenOntrekkingen['totaal']."+ ".$RekeningDirecteKostenOpbrengsten['kostenTotaal']."  $rekeningFondsenWhere <br>;\n";
...
if($stapeling == true)
{
 ...
 $query = "SELECT  Grootboekrekeningen.Opbrengst,Grootboekrekeningen.Kosten, Grootboekrekeningen.Grootboekrekening,".
  ...
  $kostenPerGrootboekAllen=array('BEH','BEW','KOST','KOBU');
  $totaalOpbrengst=0;
  $totaalKosten=0;
    ...
    {
      if(!in_array($grootboek['Grootboekrekening'],$kostenPerGrootboekAllen))
        $grootboek['Grootboekrekening']='overige';
      if($grootboek['Grootboekrekening']=='KOBU')
        $grootboek['Grootboekrekening']='KOST';
