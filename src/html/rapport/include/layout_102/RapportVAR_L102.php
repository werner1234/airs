...
class RapportVAR_L102
{
	function RapportVAR_L102($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//echo "</table>";
    ...
    if($aantalRegels>1)
    {
      ...
      {
        $this->pdf->addPage();
        $this->toonRating(20, 50);
        $this->toonDuration(150, 50);
      }
      else
      {
...
ORDER BY  Afdrukvolgorde";
  ...
  //  arsort ($ratingData);
    ...
    PieChart_L102($this->pdf,50,50,$this->ratingGrafiek,'%l',$ratingGrafiekKleuren,vertaalTekst('Rating',$this->pdf->rapport_taal),$legendaStart);
...
ORDER BY TijdelijkeRapportage.beleggingscategorieVolgorde,TijdelijkeRapportage.fondsOmschrijving,TijdelijkeRapportage.rekening";
    ...
    $DB->SQL($query);
    $DB->Query();
    $durationChart=array('0 tot 1 jaar'=>0,'1 tot 3 jaar'=>0,'3 tot 7 jaar'=>0,'7 tot 12 jaar'=>0,'>12 jaar'=>0,'overig'=>0);
    ...
    while ($data=$DB->nextRecord())
    {
      ...
      if($lossingsJul > 0)
      {
        ...
        if($duration<1)
          $durationChart['0 tot 1 jaar']+=$aandeel;
        elseif($duration<3)
          $durationChart['1 tot 3 jaar']+=$aandeel;
        elseif($duration<7)
          $durationChart['3 tot 7 jaar']+=$aandeel;
        elseif($duration<12)
          $durationChart['7 tot 12 jaar']+=$aandeel;
        else
          $durationChart['>12 jaar']+=$aandeel;
    ...
    //$kleuren=array('0 tot 1'=>array(0,200,250),'1 tot 3'=>array(10,190,240),'3 tot 7'=>array(20,160,230),'7 tot 12'=>array(30,140,220),'>12'=>array(40,110,220),'overig'=>array(40,80,210));
  //  arsort ($durationChart);
    ...
    foreach($durationChart as $key=>$value)
    {
      if($value<>0)
      {
        ...
        $durationChartFiltered[vertaalTekst($key,$this->pdf->rapport_taal)."  (".$this->formatGetal($value,1).' %)']=$value;
        $i++;
      }
      else
      {
    ...
    */
    ...
    PieChart_L102($this->pdf,50,50,$durationChartFiltered,'%l',$durationChartKleuren,vertaalTekst('Duration',$this->pdf->rapport_taal),$legendaStart);
