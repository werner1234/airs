...
class RapportATT_L102
{
	function RapportATT_L102($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
   ...
   /*
    if ($this->pdf->rapport_type=='ATT' &&  $RapStartJaar != $RapStopJaar)
    {
   ...
   if(count($rendamentWaarden) > 0)
   {
     ...
     //   $this->pdf->ln(3);
        ...
        $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
		  ...
		  if (count($barGraph) > 0 && $this->pdf->rapport_type=='ATT')
		  {
		    if(date("Y", db2jul($this->rapportageDatumVanaf)) != date("Y", db2jul($this->rapportageDatum)))
        {
          $this->pdf->addPage();
          $this->pdf->ln(8);
          $grafiekY=120;
        ...
        else
        {
          ...
          $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
		      ...
		      $this->pdf->SetXY(15,$grafiekY)		;//112
...
function formatGetalLength ($getal,$decimaal,$gewensteLengte)
{
 ...
 return number_format($getal,$decimaal,',','');
  ...
  function VBarDiagram($w, $h, $data)
  {
      ...
      $bGrafiek = ($w - $margin * 1) - ($w/$numBars); // - legenda
