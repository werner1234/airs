...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2019/10/19 08:15:15 $
File Versie					: $Revision: 1.36 $

$Log: RapportOIB_L33.php,v $
Revision 1.36  2019/10/19 08:15:15  rvv
*** empty log message ***

Revision 1.35  2019/10/18 17:40:36  rvv
*** empty log message ***

Revision 1.34  2014/06/08 07:56:12  rvv
*** empty log message ***

Revision 1.33  2013/07/17 15:53:14  rvv
*** empty log message ***
...
class RapportOIB_L33
{
	function RapportOIB_L33($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    elseif($this->zakelijk)
		{
			}
		else
		{
		  $this->pdf->rapport_titel = vertaalTekst("Kerngegevens rapportage in",$this->pdf->rapport_taal)." ".$this->pdf->rapportageValuta;
		  $this->vastWhere="";
		}
	}
  ...
  function formatGetalKoers($waarde, $dec , $start = false)
	...
	function writeRapport()
	{
      ...
      $this->pdf->rapport_titel = vertaalTekst("Kerngegevens vastrentende portefeuille in",$this->pdf->rapport_taal)." ".$this->pdf->rapportageValuta;
    ...
    {
      $this->vastWhere="";
      $this->pdf->rapport_titel = vertaalTekst("Kerngegevens zakelijke portefeuille in",$this->pdf->rapport_taal)." ".$this->pdf->rapportageValuta;
...
$this->printPie($data['emittentRating']['pieData'],$data['regioverdeling']['kleurData'], vertaalTekst("Regioverdeling",$this->pdf->rapport_taal),50,40,$hcat);
    ...
    $this->portefeuilleStartdatum=$portefeuilledata['startDatum'];
		$this->pdf->AddPage();
    $this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize+2);
		$rapportageDatum = $this->rapportageDatum;
		$janDatum=date("Y-01-01",$this->pdf->rapport_datum);
		...
		elseif(db2jul($portefeuilledata['startDatum'])> db2jul($janDatum))
			$rapportageStartJaar=substr($portefeuilledata['startDatum'],0,10);
	  else
	   	$rapportageStartJaar= $janDatum;
     ...
     if($lossingsJul > 0 && $fonds['type'] <> 'rekening' )
	   {
	    ...
	    // echo "O ".$fonds['fonds']." $ytm ".$totaalWaarde[$rapportageDatum]."<br>\n";
...
//'Macaulay duration'=>$this->formatGetal($totalen['duration'],2),
		...
		$this->pdf->ln();
		$this->pdf->SetWidths(array(20,50,50));
		$this->pdf->SetAligns(array('L','L','R'));
		$this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);

		$this->pdf->row(array('',vertaalTekst("Karakteristieken vastrentende portefeuille",$this->pdf->rapport_taal)));
...
if($vastrentend)
  $this->pdf->templateVars['OIBVPaginas'] = $this->pdf->customPageNo;//+$this->pdf->extraPage
else
...
$this->pdf->SetXY($this->pdf->GetX(),$y1-5);
  ...
  function renteResultaat($portefeuille,$startDatum,$eindDatum)
  {
    ...
    $DB=new DB();
		...
		if($this->pdf->rapportageValuta <> 'EUR' && $this->pdf->rapportageValuta <> '')
       $koers=getValutaKoers($this->pdf->rapportageValuta,$eindDatum);
    else
       $koers=1;
