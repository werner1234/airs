...
$Log: RapportTRANS_L39.php,v $
Revision 1.9  2019/06/21 14:52:45  rm
7880

Revision 1.8  2015/12/19 08:29:17  rvv
*** empty log message ***

Revision 1.7  2013/07/11 17:29:38  rvv
*** empty log message ***

Revision 1.6  2013/07/10 16:01:24  rvv
*** empty log message ***

Revision 1.5  2013/07/06 16:00:55  rvv
*** empty log message ***

Revision 1.4  2013/07/04 15:40:04  rvv
*** empty log message ***

Revision 1.3  2013/04/17 11:37:12  rvv
*** empty log message ***

Revision 1.2  2012/10/17 09:16:53  rvv
*** empty log message ***

Revision 1.1  2012/09/19 16:53:18  rvv
*** empty log message ***
...
class RapportTRANS_L39
{
	function RapportTRANS_L39($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	...
	function formatGetal($waarde, $dec, $nullNietTonen=false)
	{
	...
	{ 
	  $transactietypenÓmschrijving= array('K'=>'Koop',
	                                      'K/O'=>'Koop / openen',
	                                      'K'=>'Koop',
	                                      'K/S'=>'Koop / sluiten',
	                                      'D'=>'Deponering',
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
		...
		// voor data
		$this->pdf->widthB = array(25,17,22,60,25,30,7,32,32,31);
		$this->pdf->alignB = array('L','L','R','L','R','R','R','R','R','R','R','R','R','R');
		// voor kopjes
		$this->pdf->widthA = $this->pdf->widthB;//array(15,12,20,45,27,27,27,27,27,27,27);
...
//	echo "lopende -> ".$resultaatlopende." <-  voorgaande ".$resultaatvoorgaande. " -  <br>" ;
        ...
        $t_aankoop_waarde=$historischekostprijs;
				...
				if($percentageTotaal <>0)
				{
					...
					$percentageTotaalTekst = "";
						...
						$this->pdf->row(array(date("d-m-Y",db2jul($mutaties['Boekdatum'])),
											str_replace('A','K',$mutaties['Transactietype']),
											$this->formatGetal($mutaties['Aantal'],0),
											"",
											$mutaties['Valuta'],
											$this->formatGetal($t_aankoop_koers + $t_verkoop_koers,2,true),
											'',
											$this->formatGetal($t_aankoop_waarde,2,true),
											$this->formatGetal($t_verkoop_waarde,2,true),
                      $this->formatGetal($resultaatvoorgaande+$resultaatlopende,2,true)));
          $totaalResultaat+=$resultaatvoorgaande+$resultaatlopende;
			...
			$transactietypen[] = str_replace('A','K',$mutaties['Transactietype']);
		}
//    $this->pdf->ln(1);
//    $this->pdf->SetFont($this->pdf->rapport_font,'b',$this->pdf->rapport_fontsize);
//    		$this->pdf->row(array('','','','','','','','','',$this->formatGetal($totaalResultaat,2)));
//				$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);						
...
/*
    ...
    $this->pdf->SetFont($this->pdf->rapport_font,'b',$this->pdf->rapport_fontsize);
		$this->pdf->row(array(vertaalTekst("Totalen",$this->pdf->rapport_taal),
								"",
								"",
								"",
								'',
  ...
  */
		...
		if($this->pdf->rapport_TRANS_legenda == 1 && count($transactietypen) > 0)
		{
   		...
   		foreach($transactietypen as $val)
   		{
				...
				$this->pdf->Cell(80,4, vertaalTekst($transactietypenÓmschrijving[$val],$this->pdf->rapport_taal), 0,1, "L");
			}
		}
