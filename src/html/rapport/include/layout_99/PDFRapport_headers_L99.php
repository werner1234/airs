...
/* 	
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: PDFRapport_headers_L99.php,v $
 		Revision 1.16  2019/04/17 11:20:00  rvv
 		*** empty log message ***
 		
 		Revision 1.15  2019/04/10 15:50:36  rvv
 		*** empty log message ***
 		
 		Revision 1.14  2019/04/07 11:07:21  rvv
 		*** empty log message ***
...
*/
function Header_basis_L99($object)
{
 $pdfObject = &$object;
	...
	function HeaderVKM_L99($object)
	{
		$pdfObject = &$object;
		$pdfObject->HeaderVKM();
	}
	
	  function HeaderFRONT_L99($object)
	  {
	    $pdfObject = &$object;
...
function HeaderModel_L99($object)
{
  $pdfObject = &$object;
  $pdfObject->SetFont($pdfObject->rapport_font,'b',$pdfObject->rapport_fontsize);
...
function HeaderVKMD_L99($object)
{
  HeaderVKMS_L99($object);
}

	function HeaderVKMS_L99($object)
	{
		$pdfObject = &$object;
		$pdfObject->ln();
		...
		else
    {
	  ...
	  function HeaderSCENARIO_L99($object)
	  {
	    $pdfObject = &$object;
	    //$pdfObject->headerSCENARIO();
    ...
    function HeaderRISK_L99($object)
    {
	    $pdfObject = &$object;
			$pdfObject->Ln();
			$pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
    ...
    function HeaderGRAFIEK_L99($object)
    {
	    $pdfObject = &$object;
      $pdfObject->Ln();
      $pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
    ...
    function HeaderPERFD_L99($object)
    {
      $pdfObject = &$object;
      HeaderPERF_L99($object);
      $pdfObject->SetTextColor($pdfObject->rapport_fontcolor['r'],$pdfObject->rapport_fontcolor['g'],$pdfObject->rapport_fontcolor['b']);
    }
  
    
    function HeaderTRANS_L99($object)
	  {
	    $pdfObject = &$object;
...
$pdfObject->ln(2);
	  ...
	  function HeaderINHOUD_L99($object)
	  {
	    $pdfObject = &$object;
	    //$pdfObject->headerSCENARIO();

	  }	  
 	  function HeaderPERF_L99($object)
	  {
	    $pdfObject = &$object;
      $pdfObject->ln();
      $pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
...
$pdfObject->ln();
    ...
    function HeaderINDEX_L99($object)
	  {
	    $pdfObject = &$object;
	  }
    function HeaderOIB_L99($object)
	  {
	    $pdfObject = &$object;
      $pdfObject->Ln();
      		$pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
...
*/
   ...
   function HeaderPERFG_L99($object)
	{
    $pdfObject = &$object;
...
*/
  ...
  function HeaderVOLK_L99($object)
	{
    $pdfObject = &$object;
   	$pdfObject->ln();
		$pdfObject->SetFont($pdfObject->rapport_font,$pdfObject->rapport_kop_fontstyle,$pdfObject->rapport_fontsize);
...
function HeaderKERNZ_L99($object)
{
	$pdfObject = &$object;
	$pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
	$pdfObject->Rect($pdfObject->marge,$pdfObject->getY(),297-$pdfObject->marge*2,8,'F');
...
function HeaderHUIS_L99($object)
{
	$pdfObject = &$object;
	$pdfObject->ln();
	$widthBackup=$pdfObject->widths;
...
function getFondsKoers_L99($fonds,$datum)
{
	$db=new DB();
...
function getValutaKoers_L99($valuta,$datum)
{
	$db=new DB();
...
function formatGetal_L99($waarde, $dec)
{
	return number_format($waarde,$dec,",",".");
}


function indexKader_L99($object)
{
	$pdfObject = &$object;
	$db=new DB();
...
WHERE Portefeuilles.SpecifiekeIndex = Fondsen.Fonds AND Portefeuilles.Portefeuille = '".$portefeuille."' ";
	...
	foreach($indices as $data)
	{
		foreach($perioden as $periode=>$datum)
			$data[$periode]=getFondsKoers_L99($data['indexFonds'],$datum);
	...
	$pdfObject->SetTextColor($pdfObject->rapport_kop_fontcolor['r'],$pdfObject->rapport_kop_fontcolor['g'],$pdfObject->rapport_kop_fontcolor['b']);
		foreach($indices as $fonds=>$fondsData)
		{
			...
			$pdfObject->SetFont($pdfObject->rapport_font,'',$pdfObject->rapport_fontsize);
			$pdfObject->row(array(date("d-m-Y",db2jul($perioden['beginPeriode']))." / ".date("d-m-Y",db2jul($perioden['eind'])),formatGetal_L99($fondsData['beginPeriode'],2),formatGetal_L99($fondsData['eind'],2),formatGetal_L99($fondsData['periode'],2).'%'));
			$pdfObject->row(array(date("d-m-Y",db2jul($perioden['begin']))." / ".date("d-m-Y",db2jul($perioden['eind'])),formatGetal_L99($fondsData['begin'],2),formatGetal_L99($fondsData['eind'],2),formatGetal_L99($fondsData['kwartaal'],2).'%'));
...
function risicoMeter_L99($object,$pageX,$pageY,$width,$riscoklasse)
{
	$pdfObject = &$object;
