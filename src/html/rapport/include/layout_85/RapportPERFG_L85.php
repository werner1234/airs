...
$Log: RapportPERFG_L85.php,v $
Revision 1.4  2020/01/04 18:56:57  rvv
*** empty log message ***

Revision 1.3  2019/09/14 17:09:52  rvv
*** empty log message ***
...
class RapportPERFG_L85
{
	function RapportPERFG_L85($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
$indexData = $index->getWaarden($start,$eind,$this->portefeuille,$this->pdf->portefeuilledata['SpecifiekeIndex'],'jaar');
    $jaarPerfCum=0;
    $lastPerf=0;
foreach ($indexData as $i=>$data)
{
  if($data['datum'] != '0000-00-00')
  {
    ...
    $maanden=$index->getMaanden(db2jul($periode[0]),db2jul($periode[1]));
    ...
    foreach($maanden as $periode)
    {
      if($start=='')
        $start=$periode['start'];
      $stop=$periode['stop'];
      $perf=$this->performance($this->portefeuille,$start,$stop);
      $lastPerf=$perf;
    //  listarray($periode);
      $grafiekData['Datum'][] = $stop;
      $grafiekData['Index'][] = ((1+$jaarPerfCum/100)*(1+$lastPerf/100)-1)*100;// $data['index'] - 100;
    }
    $jaarPerfCum=((1+$jaarPerfCum/100)*(1+$lastPerf/100)-1)*100;
  ...
  if(count($rendamentWaarden) > 0)
   {
     ...
     $totaal=array();
        ...
        $kleuren=array(array(0,132,134),array(61,59,56));
        ...
        foreach($kleuren as $index=>$kleur)
        {
          if($index==0)
          {
            ...
            $this->pdf->Cell(50, 4, vertaalTekst('Portefeuille',$this->pdf->rapport_taal), 0, 0, "L");
          }
         // if($index==1)
...
{
   	    $this->pdf->row(array(vertaalTekst('Totaal',$this->pdf->rapport_taal).' '.$jaar,
