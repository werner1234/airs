...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2020/07/22 16:36:49 $
File Versie					: $Revision: 1.7 $

$Log: RapportPERF_L85.php,v $
Revision 1.7  2020/07/22 16:36:49  rvv
*** empty log message ***

Revision 1.6  2020/01/04 18:56:57  rvv
*** empty log message ***

Revision 1.5  2019/12/14 17:49:52  rvv
*** empty log message ***

Revision 1.4  2019/11/13 15:30:11  rvv
*** empty log message ***

Revision 1.3  2019/11/02 15:26:11  rvv
*** empty log message ***

Revision 1.2  2019/09/14 17:09:52  rvv
*** empty log message ***
...
include_once("rapport/include/layout_85/ATTberekening_L85.php");

class RapportPERF_L85
{

	function RapportPERF_L85($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    $this->att=new ATTberekening_L85($this);
	}
...
IndexPerBeleggingscategorie.Vermogensbeheerder='".$this->pdf->portefeuilledata['Vermogensbeheerder']."'";
    ...
    $query="SELECT Valutas.Valuta as Beursindex,
    Valutas.Omschrijving,
    'Valuta' as catOmschrijving FROM Valutas WHERE Valutas.Valuta='USD'";
    $DB->SQL($query);
    $DB->Query();
    while($index = $DB->nextRecord())
    {
      $index['valutaBenchmark']=1;
      $index['specialeIndex']=1;
      $indices[]=$index;
    }
    ...
    foreach($indices as $index)
    {
      ...
      if($index['specialeIndex']==1)
      {
        ...
        {
          if($index['valutaBenchmark']==1)
          {
            $specialeIndexData[$index['Beursindex']]['fondsKoers_'.$periode]=getValutaKoers($index['Beursindex'],$datum);
            $specialeIndexData[$index['Beursindex']]['valutaKoers_' . $periode] =1;///getValutaKoers($index['Beursindex'],$datum);
          }
          else
          {
            $specialeIndexData[$index['Beursindex']]['fondsKoers_' . $periode] = $this->getFondsKoers($index['Beursindex'], $datum);
            $specialeIndexData[$index['Beursindex']]['valutaKoers_' . $periode] = getValutaKoers($index['Valuta'], $datum);
          }
          ...
          $specialeIndexData[$index['Beursindex']]['performanceEur'] =     ($specialeIndexData[$index['Beursindex']]['fondsKoers_eind']*$specialeIndexData[$index['Beursindex']]['valutaKoers_eind'] - $specialeIndexData[$index['Beursindex']]['fondsKoers_begin']*$specialeIndexData[$index['Beursindex']]['valutaKoers_begin'])/($specialeIndexData[$index['Beursindex']]['fondsKoers_begin']*$specialeIndexData[$index['Beursindex']]['valutaKoers_begin']/100 );
        }
      }
      else
      {
        ...
        foreach ($perioden as $periode=>$datum)
        {
          if($index['valutaBenchmark']==1)
          {
            $indexData[$index['Beursindex']]['fondsKoers_'.$periode]=getValutaKoers($index['Beursindex'],$datum);
            $indexData[$index['Beursindex']]['valutaKoers_' . $periode] =1;
          }
          else
          {
    ...
    $hoogte=count($benchmarks)*4+4 + count($specialeBenchmarks)*4;
    ...
    $this->pdf->SetWidths(array(123,60,20,20,20,30));
    $this->pdf->SetAligns(array('L','L','R','R','R','R'));
    $this->pdf->Rect($this->pdf->marge+123,$this->pdf->getY(),150,$hoogte);
    $this->pdf->row(array("",vertaalTekst("Marktindices ter informatie",$this->pdf->rapport_taal),
    ...
    foreach ($benchmarks as $fonds)
    {
      ...
      if($fondsData['Omschrijving']=='')
        ...
        $this->pdf->row(array('',vertaalTekst($fondsData['Omschrijving'],$this->pdf->rapport_taal),
  ...
  //    $this->pdf->SetY(150);
      ...
      $this->pdf->SetWidths(array(123,60,20,20,20,30));
      $this->pdf->SetAligns(array('L','L','R','R','R','R'));
     // $this->pdf->Rect($this->pdf->marge+123,150,150,count($specialeBenchmarks)*4+4);
      
    //  $this->pdf->row(array("",vertaalTekst("Marktindices ter informatie",$this->pdf->rapport_taal),"".date("d-m-Y",db2jul($perioden['begin'])),"".date("d-m-Y",db2jul($perioden['eind'])),vertaalTekst("Rendement",$this->pdf->rapport_taal),vertaalTekst("Rendement EUR",$this->pdf->rapport_taal)));
      unset($this->pdf->CellBorders);
      $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
...
function addResultaat()
{
  ...
  $vetralingGrootboek=$this->getGrootboeken();
    ...
    //$att=new ATTberekening_L85($this);
