...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2019/11/13 15:44:54 $
File Versie					: $Revision: 1.2 $

$Log: RapportSCENARIO_L85.php,v $
Revision 1.2  2019/11/13 15:44:54  rvv
*** empty log message ***
...
class RapportSCENARIO_L85
{
	function RapportSCENARIO_L85($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//
    ...
    foreach($sc->verwachteWaarden as $scenario=>$eindvermogen)
    {
      ...
      if($sc->scenarios[$scenario]<50)
        $kansTxt=$this->formatGetal($sc->scenarios[$scenario],1).' <';
      elseif($sc->scenarios[$scenario]>50)
        $kansTxt=$this->formatGetal(100-$sc->scenarios[$scenario],1).' >';
      else
      {
        $kansTxt = $this->formatGetal($sc->scenarios[$scenario], 1) . '   ';
        $this->pdf->Text($this->pdf->marge+$this->pdf->widthB[0]+$this->pdf->widthB[1]+$this->pdf->widthB[2]-3,$this->pdf->getY()+4,'~');
        $this->pdf->Text($this->pdf->marge+$this->pdf->widthB[0]+$this->pdf->widthB[1]+$this->pdf->widthB[2]-3,$this->pdf->getY()+4.5,'~');
      }

    //  $this->pdf->SetFont('times','',$this->pdf->rapport_fontsize);
      $this->pdf->row(array('',vertaalTekst($scenario,$this->pdf->rapport_taal),$kansTxt,$this->formatGetalNegatief($eindvermogen)));
    }
    //$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
