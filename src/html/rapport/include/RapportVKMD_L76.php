...
Revision 1.6  2018/11/09 06:04:30  rvv
*** empty log message ***

Revision 1.5  2018/11/08 07:45:25  rvv
*** empty log message ***

Revision 1.4  2018/11/07 17:08:06  rvv
*** empty log message ***

Revision 1.3  2018/11/04 11:15:32  rvv
*** empty log message ***
...
class RapportVKMD_L76
{
	function RapportVKMD_L76($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
...
*/
      ...
      $this->pdf->row(array(vertaalTekst('Overzicht van de gerealiseerde kosten over de afgelopen 12 maanden',$this->pdf->rapport_taal)));
      $this->pdf->ln();
      $this->pdf->setWidths(array(28+50+20,20,20,20,20,18,18,18,18,18,15));
			$this->pdf->row(array(vertaalTekst('Kosten dienstverlening',$this->pdf->rapport_taal), 'Bedrag in €', vertaalTekst('Percentage',$this->pdf->rapport_taal)));
        ...
        if($data['Grootboekrekening']=='BEH')
          $data['Omschrijving']='Beheervergoeding (incl. BTW) incl Toezichtkosten';
        if($data['Grootboekrekening']=='KOBU')
          $data['Omschrijving']='Kosten buitenland, belastingen';
    ...
    if($spreadKostenEUR <> 0)
		...
		{
			$this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
			$this->pdf->row(array(vertaalTekst('Totaal kosten dienstverlening',$this->pdf->rapport_taal), $this->formatGetal($totaal, 0), $this->formatGetal($kostenPercentage, 2).' %'));
			$this->pdf->excelData[]=array('','Totaal directe kosten','', round($totaal, 0), round($kostenPercentage, 2));
			$this->pdf->ln();

      $this->pdf->row(array(vertaalTekst('Totaal kosten beleggingsfondsen en ETF\'s',$this->pdf->rapport_taal), $this->formatGetal($vkmPercentagePortefeuille*$gemiddelde/100, 0) , $this->formatGetal($vkmPercentagePortefeuille, 2) . ' %'));
      $this->pdf->ln();
      $this->pdf->excelData[]=array();
      ...
      $this->pdf->CellBorders = array('',array('TS','UU'),array('TS','UU'));
			...
			$this->pdf->row(array(vertaalTekst('Totaal kosten (vergelijkende kostenmaatstaf)',$this->pdf->rapport_taal),$this->formatGetal($vkmWaarde*$gemiddelde/100, 0), $this->formatGetal($vkmWaarde, 2).' %'));
			unset($this->pdf->CellBorders);
			$this->pdf->excelData[]=array('','Totaal kosten (vergelijkende kostenmaatstaf)','', round($vkmWaarde*$gemiddelde/100,2),round($vkmWaarde, 2));
			$this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
			$startYGrafiek=$this->pdf->getY();
			$this->pdf->setXY(30,170);
			arsort($barData);
...
//		echo "$doelJaar $rendement $beginwaarde";
	...
	function LineDiagram($w, $h, $data, $color=null, $maxVal=0, $minVal=0, $horDiv=4, $verDiv=4,$vanafBegin=false)
	{
		...
		$this->pdf->SetFont($this->pdf->rapport_font,'b',$this->pdf->rapport_fontsize);
		$this->pdf->setXY($XPage,$this->pdf->getY()-4);
	...
	//	$maxVal=round($maxVal,floor(log10($maxVal))*-1+1);
		...
		for($i=$nulpunt; $i<= $bodem; $i+= $absUnit*$stapgrootte)
		{
			...
			$this->pdf->setXY($XDiag+$w, $i-1);
			$this->pdf->Cell(7 , 4 , "€ ". $this->formatGetal(0-($n*$stapgrootte),0,false,true) , 0, 1, "L");
		...
		for($i=$nulpunt; $i >= $top; $i-= $absUnit*$stapgrootte)
		{
			...
			{
				$this->pdf->setXY($XDiag+$w, $i-1);
				$this->pdf->Cell(7 , 4 , "€ " .$this->formatGetal(($n * $stapgrootte) + 0,0,false,true) , 0, 1, "L");
