...
class RapportVKMS_L125
{
	function RapportVKMS_L125($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
			...
			$this->pdf->rapport_titel = vertaalTekst("Uw totale kosten",$this->pdf->rapport_taal);
			$this->pdfVullen=true;
...
//		$spreadKostenEUR=(5 / 10000 * $this->verdelingTotaal['totaal']['gemiddelde']);
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    if($this->pdfVullen==true)
      ...
      $CellFontStyle=array(array($this->pdf->rapport_font,'b',$this->pdf->rapport_fontsize+2),array($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize));
      subHeader_L125($this->pdf,  $this->pdf->getY(), array(50,100), array('Vergelijkende kostenmaatstaf',"van $startPeriodeTxt t/m $eindPeriodeTxt"),null,null,$CellFontStyle);
      ...
      $this->pdf->setWidths(array(20-$this->pdf->marge,88, 30, 30));
      $startY = $this->pdf->getY();
      
      $this->pdf->CellFontStyle=array('',array($this->pdf->rapport_font,'b',$this->pdf->rapport_fontsize),array($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize+2),array($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize));
      $this->pdf->CellFontColor=array('',array('r'=>$this->pdf->textGroen[0],'g'=>$this->pdf->textGroen[1],'b'=>$this->pdf->textGroen[2]),array('r'=>0,'g'=>0,'b'=>0));
			...
			$this->pdf->row(array('',vertaalTekst('Uw gemiddelde totale vermogen', $this->pdf->rapport_taal), '€ '.$this->formatGetal($gemiddelde, 0)));
			unset($this->pdf->CellFontStyle);
      unset($this->pdf->CellFontColor);
			$this->pdf->ln();


			$this->pdf->setWidths(array(20-$this->pdf->marge,28 + 50, 20, 20, 20, 20, 18, 18, 18, 18, 18, 15));
		...
		$barData=array();
			while($data = $DB->nextRecord())
			{
				if($this->pdfVullen==true)
				{
					...
					$this->pdf->row(array('',$data['Omschrijving'],$this->formatGetal($kostenProcent,2).'%','€ '.$this->formatGetal($data['totaal'],0) ));
		...
		if($this->pdfVullen==true)
		{
			$this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
			$this->pdf->ln(2);
			$this->pdf->row(array('','', $this->formatGetal($kostenPercentage, 2).'%', '€ '.$this->formatGetal($totaal, 0)));
			$this->pdf->ln();
			$this->pdf->CellBorders=array();
			...
			$this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
			$this->pdf->row(array('',vertaalTekst('Doorlopende fondskosten',$this->pdf->rapport_taal), $this->formatGetal($vkmPercentagePortefeuille, 2) . ' %','€ '.$this->formatGetal($totaalDoorlopendekosten, 0)));
      $barData['Doorlopende fondskosten']=$vkmPercentagePortefeuille;
      ...
      if($spreadKostenEUR <> 0)
      {
        ...
        if($this->pdfVullen==true)
        {
          $this->pdf->row(array('',vertaalTekst('Spreadkosten',$this->pdf->rapport_taal), $this->formatGetal($kostenProcent, 2) . ' %','€ '. $this->formatGetal($spreadKostenEUR, 0)));
          $this->pdf->excelData[]=array('','Spreadkosten','',round($spreadKostenEUR,0),round($kostenProcent,2) );
        }
        $totaal += $spreadKostenEUR;
  
        $totaalDoorlopendekosten += $spreadKostenEUR;
        $vkmWaarde+=$kostenProcent;
        $vkmPercentagePortefeuille+=$kostenProcent;
        
        $barData['Spreadkosten'] = $kostenProcent;
      }
      ...
      $this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
      $this->pdf->ln(2);
			$this->pdf->row(array('','',$this->formatGetal($vkmPercentagePortefeuille, 2) . ' %','€ '.$this->formatGetal($totaalDoorlopendekosten, 0)));
			$this->pdf->ln();
      $this->pdf->line(20,$this->pdf->getY(),140,$this->pdf->getY(),array('color'=>$this->pdf->textGrijs));
      $this->pdf->ln();
			$this->pdf->row(array('',vertaalTekst('Totaal',$this->pdf->rapport_taal),$this->formatGetal($vkmWaarde, 2).' %','€ '.$this->formatGetal($vkmWaarde*$gemiddelde/100, 0)));


		//	$this->pdf->row(array(vertaalTekst('Gemiddeld vermogen met een kostenfactor',$this->pdf->rapport_taal), $this->formatGetal($perfTotaal['gemWaarde'], 0) . ' EUR'));
...
*/
		...
		//	$this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
			...
			$this->VBarVerdeling(50,$huidigeY-$startY,$barData);
			//$this->pdf->line(155,$huidigeY,155,$startY);
			//echo ($startYGrafiek-$startY)." line(155,$huidigeY,155,$startY); <br>\n";
...
foreach ($perCategorie as $hoofdcategorie=>$categorieData)
{
 ...
 $perfTotaal['percentageIndirectVermogenMetKostenfactor']=$percentageIndirectVermogenMetKostenfactor;
      ...
      foreach($gewensteKleuren as $gb=>$kleurdata)
      {
        if($kleurdata['R']['value']||$kleurdata['G']['value']||$kleurdata['B']['value'])
          $grootboekKleuren[$gb]=array($kleurdata['R']['value'],$kleurdata['G']['value'],$kleurdata['B']['value']);
      }
      while($data=$DB->nextRecord())
      {
        $mogelijkeKleuren[$n];
      ...
      if($gewensteKleuren[$key]['R']['value'] || $gewensteKleuren[$key]['G']['value'] || $gewensteKleuren[$key]['B']['value'])
        $grootboekKleuren['Doorlopende fondskosten']=array($gewensteKleuren[$key]['R']['value'],$gewensteKleuren[$key]['G']['value'],$gewensteKleuren[$key]['B']['value']);
      else
        $grootboekKleuren['Doorlopende fondskosten']=$mogelijkeKleuren[$n];
      ...
      $this->grootboekKleuren=$grootboekKleuren;
			...
			if ($this->pdfVullen == true)
			{
				...
				if($this->skipLangeTermijn==false)
					$this->langeTermijngrafiek($mogelijkeKleuren);
	...
	function langeTermijngrafiek($mogelijkeKleuren)
	{
		global $__appvar;
		$db=new DB();
 ...
 WHERE Portefeuilles.portefeuille='".$this->portefeuille."'";
    ...
    for($i=$this->pdf->rapport_jaar; $i<=$doelJaar; $i++)
		...
		$grafiekWaarden['legenda']=array('Waardeontwikkeling zonder kosten','Waardeontwikkeling na kosten','Cumulatief effect van kosten');
		
		//$grafiekWaarden['titel']="Impact van kosten op het langetermijnrendement van de portefeuille";
    if(count($mogelijkeKleuren) > 3)
    {
      $waardeZonderKostenKleur = $mogelijkeKleuren[2];
      $waardeNaKostenKleur = $mogelijkeKleuren[0];
      $cumulatieveKostenKleur = $mogelijkeKleuren[1];
    }
    else
    {
    ...
    subHeader_L125($this->pdf,$this->pdf->getY(),array(140),array('Effect van de kosten op uw langetermijnrendement'));
    $this->pdf->setXY(35,$this->pdf->getY()+15);
		$this->LineDiagram(140, 50, $grafiekWaarden,array($waardeZonderKostenKleur,$waardeNaKostenKleur,$cumulatieveKostenKleur),0,0,4,4,false);//50
...
//		echo "$doelJaar $rendement $beginwaarde";
    ...
    $this->pdf->setTextColor($this->pdf->textGrijs[0],$this->pdf->textGrijs[1],$this->pdf->textGrijs[2]);
		...
		{
			$skipNull = true;
			$this->pdf->Line($XDiag, $i, $XPage+$w ,$i,array('dash' => 1,'color'=>$this->pdf->textGrijs));
			$this->pdf->setXY($XDiag-7, $i-2);
			$this->pdf->Cell(7 , 4 , "€ ". $this->formatGetal(0-($n*$stapgrootte)+0,0,false,true), 0, 1, "R");
      ...
      $this->pdf->Circle($XPage+$w+5, $YPage+$step+11.25, 1.5,0,360, 'F',null,$kleur);
  ...
  function fondsPerf($fonds,$van,$tot)
  {
	...
	function VBarVerdeling($w, $h, $data)
	{
		...
		for($i=$nulpunt; $i >= $top; $i-= $absUnit*$stapgrootte)
		{
			...
			$this->pdf->Line($XstartGrafiek, $i, $XstartGrafiek + $w ,$i,array('dash' => 1,'color'=>$this->pdf->textGrijs));
			if($skipNull == true)
				$skipNull = false;
			else
			{
		...
		$this->pdf->SetLineStyle(array('dash' => 0,'color'=>$this->pdf->textGrijs));
		$this->pdf->SetLineWidth(0.2);
	  ...
	  $hstap=10;
    $aantal=count($grafiek);
    $legendaHoogte=$aantal*$hstap;
    
    $yOffstet=($hGrafiek-$legendaHoogte)/2;

		$xval = $XstartGrafiek +  $bGrafiek +5;
		$yval = $YstartGrafiek - $yOffstet ;
    
    $this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
    $n=0;
    //$this->pdf->line(222,$YstartGrafiek,222,$YstartGrafiek-$yOffstet,array('color'=>array(0,0,0)));
    //$this->pdf->line(222,$YstartGrafiek-$h,222,$YstartGrafiek-$h+$yOffstet,array('color'=>array(0,0,0)));
		foreach($grafiek as $categorie=>$val)
		{
      $yval-=$hstap/2;
			$this->pdf->Circle($xval, $yval, 1.5,0,360, 'F',null,$colors[$categorie]);
			// $this->pdf->Circle($XDiag+$i*$unitw, $yval, $markerSize, 0,360, $style = 'F', $circleStyle, $this->pdf->grafiekBruin);
			$this->pdf->SetXY($xval+4, $yval-1.75);
			$this->pdf->Cell(50, 4,$categorie,0,0,'L');//." ".number_format($val,2,',','.')."%"
      $yval-=$hstap/2;
      $n++;
		}
