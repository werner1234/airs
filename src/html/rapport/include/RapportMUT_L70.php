...
$Log: RapportMUT_L70.php,v $
Revision 1.5  2020/03/18 17:44:11  rvv
*** empty log message ***

Revision 1.4  2017/05/28 09:58:52  rvv
*** empty log message ***

Revision 1.3  2016/06/29 16:04:07  rvv
*** empty log message ***

Revision 1.2  2016/06/12 10:20:31  rvv
*** empty log message ***

Revision 1.1  2016/06/08 15:40:53  rvv
*** empty log message ***
...
class RapportMUT_L70
{
	function RapportMUT_L70($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	...
	function printSubTotaal($title, $totaalA, $totaalB)
	{
		...
		$totaal2 = $totaal1 + $this->pdf->widthA[5];
	  $this->pdf->Line($totaal1+2,$this->pdf->GetY(),$totaal1 + $this->pdf->widthA[5],$this->pdf->GetY());
	 	$this->pdf->Line($totaal2+2,$this->pdf->GetY(),$totaal2 + $this->pdf->widthA[6],$this->pdf->GetY());
		...
		$this->pdf->SetX($totaal1 - $this->pdf->widthA[4]);

		$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);

		$this->pdf->Cell($this->pdf->widthA[4],4,$title, 0,0, "R");
		$this->pdf->Cell($this->pdf->widthA[5],4,$totaalAtxt, 0,0, "R");
		$this->pdf->Cell($this->pdf->widthA[6],4,$totaalBtxt, 0,0, "R");
...
//		$this->pdf->Line($totaal2+2,$this->pdf->GetY(),$totaal2 + $this->pdf->widthA[7],$this->pdf->GetY());
		...
		$this->pdf->SetX($totaal1 - $this->pdf->widthA[4]- $this->pdf->widthA[3]);
		...
		if($grandtotal)
		{
		  ...
		  {
		    $this->pdf->Line($totaal2+2,$this->pdf->GetY(),$totaal2 + $this->pdf->widthA[6],$this->pdf->GetY());
		  	$this->pdf->Line($totaal2+2,$this->pdf->GetY()+1,$totaal2 + $this->pdf->widthA[6],$this->pdf->GetY()+1);
		  }
		  $this->pdf->ln();
		}
		else
	...
	function writeRapport()
	{
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
		...
		$DB = new DB();
		// voor data
		$this->pdf->widthA = array(18.7368+23.4211,84.3158,23.4211,38.1053,18.7368,23.4211,23.4211,13.4211);
		$this->pdf->alignA = array('R','L','R','R','R','R','R');

		// voor kopjes
		$this->pdf->widthB = array(18.7368+23.4211,84.3158,23.4211,38.1053,18.7368,23.4211,23.4211,13.4211);
		$this->pdf->alignB = array('R','L','R','R','R','R','R');
