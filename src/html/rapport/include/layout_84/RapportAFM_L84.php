...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2020/01/04 18:56:30 $
File Versie					: $Revision: 1.11 $

$Log: RapportAFM_L84.php,v $
Revision 1.11  2020/01/04 18:56:30  rvv
*** empty log message ***

Revision 1.10  2019/12/21 14:09:37  rvv
*** empty log message ***

Revision 1.9  2019/10/27 16:35:34  rvv
*** empty log message ***

Revision 1.8  2019/10/05 17:39:27  rvv
*** empty log message ***

Revision 1.7  2019/09/04 15:33:05  rvv
*** empty log message ***

Revision 1.6  2019/08/07 12:34:52  rvv
*** empty log message ***

Revision 1.5  2019/08/03 17:03:51  rvv
*** empty log message ***

Revision 1.4  2019/07/31 14:46:28  rvv
*** empty log message ***

Revision 1.3  2019/07/27 18:01:41  rvv
*** empty log message ***

Revision 1.2  2019/07/24 15:48:02  rvv
*** empty log message ***
...
class RapportAFM_L84
{
	function RapportAFM_L84($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	  global $__appvar;
		$this->pdf = &$pdf;
...
*/
  ...
  function rechtsOnder($riskData,$riskBenchmark)
  {
    ...
    if($this->pdf->portefeuilledata['SpecifiekeIndex']<>'')
      $benchmarkVerdeling[$this->pdf->portefeuilledata['SpecifiekeIndex']]=1;
...
WHERE benchmark='" . mysql_real_escape_string($fonds) . "'";
      ...
      {
        $query = "SELECT BeleggingscategoriePerFonds.fonds,1 as percentage, BeleggingscategoriePerFonds.afmCategorie FROM BeleggingscategoriePerFonds
WHERE fonds='" . mysql_real_escape_string($fonds) . "' AND  BeleggingscategoriePerFonds.Vermogensbeheerder='".$this->pdf->portefeuilledata['Vermogensbeheerder']."'";
        $DB->SQL($query);
        $DB->Query();
        if($DB->records()>0)
        {
    ...
    $this->pdf->SetWidths(array(152,60,20,20));
    $this->pdf->SetAligns(array('L','L','R','R'));
    $this->pdf->SetFillColor($this->pdf->rapport_kop_bgcolor['r'],$this->pdf->rapport_kop_bgcolor['g'],$this->pdf->rapport_kop_bgcolor['b']);
    $this->pdf->Rect($this->pdf->marge+152, $this->pdf->getY(), 100, 8 , 'F');
    $this->pdf->SetTextColor($this->pdf->rapport_kop_fontcolor['r'],$this->pdf->rapport_kop_fontcolor['g'],$this->pdf->rapport_kop_fontcolor['b']);
    $this->pdf->SetFont($this->pdf->rapport_font,'b',$this->pdf->rapport_fontsize);
    $n=0;
    if(count($riskBenchmark) >1)
    {
      $this->pdf->row(array('', 'Risico parameters', 'Portefeuille', 'Benchmark'));
      $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
      $this->pdf->ln();
      $this->pdf->SetTextColor(0,0,0);
      $n=fillLine($this->pdf,$n,array(0,1,1,1));
      $this->pdf->row(array('', 'Standaarddeviatie', $this->formatGetal($afm, 1) . '%', $this->formatGetal($afmBenchmark, 1) . '%'));
      $n=fillLine($this->pdf,$n,array(0,1,1,1));
    }
    else
    {
      ...
      $this->pdf->SetTextColor(0,0,0);
      $n=fillLine($this->pdf,$n,array(0,1,1,1));
      $this->pdf->row(array('', 'Standaarddeviatie', $this->formatGetal($afm, 1) . '%'));
      $n=fillLine($this->pdf,$n,array(0,1,1,1));
    }
    // $this->pdf->row(array('','AFM-Standaarddeviatie',$this->formatGetal($riskData['standaarddeviatieAFM'],1).'%'));
    //  $this->pdf->ln(2);

    $riskData['valueAtRisk']=100-(1.96 *$afm);
    $riskBenchmark['valueAtRisk']=100-(1.96 *$afmBenchmark);

    
    if($riskData['valueAtRisk'] <> 0)
    {
      $riskData['valueAtRiskEUR'] = 1.96 *$afm * $totaalWaarde/100;
      $riskBenchmark['valueAtRiskEUR'] = 1.96 *$afmBenchmark * $totaalWaarde/100;
    }
  /*
    echo  $riskData['valueAtRisk']."=100-(1.96 *".$afm.")<br>\n";
    echo $this->formatGetal(100-$riskData['valueAtRisk'], 1). "%  = formatGetal(100-".$riskData['valueAtRisk'].", 1).'%')<br>\n<br>\n";
    
    echo  $riskBenchmark['valueAtRisk']."=100-(1.96 *".$afmBenchmark.")<br>\n";
    echo $this->formatGetal(100-$riskBenchmark['valueAtRisk'], 1). "%  = formatGetal(100-".$riskBenchmark['valueAtRisk'].", 1).'%')"; exit;
    */
    if(count($riskBenchmark) >1)
    {
      $this->pdf->row(array('', 'Value at Risk (zekerheid 95%)', '€ ' . $this->formatGetal($riskData['valueAtRiskEUR'], 0), '€ ' . $this->formatGetal($riskBenchmark['valueAtRiskEUR'], 0)));//'Value at Risk geeft het verwachte maximale verlies aan met een waarschijnlijkheid van 95%. De historische VaR is bepaald aan de hand van de werkelijke jaarlijkse rendementsverdeling over de afgelopen tien jaar.'
      $n=fillLine($this->pdf,$n,array(0,1,1,1));
      $this->pdf->row(array('', 'Value at Risk in % (zekerheid 95%)', $this->formatGetal(100-$riskData['valueAtRisk'], 1).'%', $this->formatGetal(100-$riskBenchmark['valueAtRisk'], 1).'%'));//'Value at Risk geeft het verwachte maximale verlies aan met een waarschijnlijkheid van 95%. De historische VaR is bepaald aan de hand van de werkelijke jaarlijkse rendementsverdeling over de afgelopen tien jaar.'
      $n=fillLine($this->pdf,$n,array(0,1,1,1));
    ...
    {
      $this->pdf->row(array('', 'Value at Risk (zekerheid 95%)', '€ ' . $this->formatGetal($riskData['valueAtRiskEUR'], 0)));//'Value at Risk geeft het verwachte maximale verlies aan met een waarschijnlijkheid van 95%. De historische VaR is bepaald aan de hand van de werkelijke jaarlijkse rendementsverdeling over de afgelopen tien jaar.'
      $n=fillLine($this->pdf,$n,array(0,1,1,1));
      $this->pdf->row(array('', 'Value at Risk in % (zekerheid 95%)',  $this->formatGetal($riskData['valueAtRisk'], 1).'%'));//'Value at Risk geeft het verwachte maximale verlies aan met een waarschijnlijkheid van 95%. De historische VaR is bepaald aan de hand van de werkelijke jaarlijkse rendementsverdeling over de afgelopen tien jaar.'
      $n=fillLine($this->pdf,$n,array(0,1,1,1));
