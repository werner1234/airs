...
class RapportFRONTC_L84
{
	function RapportFRONTC_L84($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  //  $this->pdf->rowHeight = 4;
    ...
    $this->DB->SQL($query);
      ...
      foreach ($this->pdf->portefeuilles as $portefeuille)
      {
        ...
        $this->DB->SQL("SELECT Clienten.Naam,Clienten.Client,Portefeuilles.depotbank, Depotbanken.Omschrijving
        FROM Portefeuilles 
        ...
        $this->pdf->row(array('',$portefeuille,$client['Omschrijving']));
      }
		}
/*
	  $this->pdf->frontPage=true;
