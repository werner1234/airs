...
/*
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2019/10/05 17:39:27 $
 		File Versie					: $Revision: 1.3 $

 		$Log: ATTberekening_L84.php,v $
 		Revision 1.3  2019/10/05 17:39:27  rvv
 		*** empty log message ***
 		
 		Revision 1.2  2019/09/25 15:34:27  rvv
 		*** empty log message ***
 		
 		Revision 1.1  2019/07/24 15:48:02  rvv
 		*** empty log message ***
 		
 		Revision 1.16  2019/03/09 18:45:47  rvv
 		*** empty log message ***
 		
 		Revision 1.15  2016/12/17 18:57:35  rvv
 		*** empty log message ***
 		
 		Revision 1.14  2016/07/27 15:50:38  rvv
 		*** empty log message ***
 		
 		Revision 1.13  2016/07/24 09:50:58  rvv
 		*** empty log message ***
 ...
 */
/*
class rapportHulp_L84
{
  function rapportHulp_L84()
  {
    $this->test='';
  }
}
...
class ATTberekening_L84
{

	function ATTberekening_L84($rapportData)
	{
  ...
  {
    $this->rapport = new rapportHulp_L84();
    $db=new DB();
...
ORDER BY HoofdBeleggingscategorien.Afdrukvolgorde, Regios.Afdrukvolgorde,Beleggingscategorien.Afdrukvolgorde";
    ...
    {
		    $perfData[$categorie] = $this->fondsPerformance($categorieData,$van,$tot,true,false);
...
WHERE 
IndexPerBeleggingscategorie.Vermogensbeheerder='".$this->rapport->pdf->portefeuilledata['Vermogensbeheerder']."' AND IndexPerBeleggingscategorie.Beleggingscategorie='$categorie' AND IndexPerBeleggingscategorie.vanaf <= '$van'
ORDER BY IndexPerBeleggingscategorie.vanaf desc";
	  $DB->SQL($query);
...
INNER JOIN Indices ON IndexPerBeleggingscategorie.Fonds = Indices.Beursindex AND IndexPerBeleggingscategorie.Vermogensbeheerder = Indices.Vermogensbeheerder
WHERE
...
ZorgplichtPerPortefeuille.extra=0 AND IndexPerBeleggingscategorie.Portefeuille = ''  AND IndexPerBeleggingscategorie.vanaf <= '$van'
order by Indices.specialeIndex DESC, ZorgplichtPerPortefeuille.vanaf desc , IndexPerBeleggingscategorie.vanaf desc limit 1 ";
	  $DB->SQL($query);
	  $fondsData=$DB->lookupRecord();
...
IndexPerBeleggingscategorie.Portefeuille = '' AND IndexPerBeleggingscategorie.vanaf <= '$van'
   ...
   //           WHERE Portefeuilles.Portefeuille='".$this->rapport->portefeuille."' AND Fonds='".$fondsData['Fonds']."' AND Datum <='$tot' ORDER BY Datum DESC LIMIT 1";
	 ...
	 //   $percentage=$DB->lookupRecord();
      $query="SELECT Fonds, Datum, Koers FROM Fondskoersen WHERE datum  <= '$van' AND Fonds='".$fondsData['Fonds']."' ORDER BY Datum DESC LIMIT 1";
		...
		//echo "$perf=(".$eindKoers['Koers']." - ".$startKoers['Koers'].") / (".$startKoers['Koers']."); <br>\n";
    //  $waarden[$tot]=array('perf'=>$perf,'aandeel'=>$fondsData['Percentage']);
...
// verwijderd    $AttributieStortingenOntrekkingen['totaal'] += $nietToegerekendeKosten['kostenTotaal'];
      ...
      if($beginwaarde==0)
      {
        ...
        // echo $fondsData['categorie']." ATT berekening $performance = ((($eindwaarde - $beginwaarde) - ".$AttributieStortingenOntrekkingen['totaal'].") / ".$AttributieStortingenOntrekkingen['totaal'].");<br>\n";
...
//echo $fondsData['categorie']." rvv $performance = ((($eindwaarde - $beginwaarde) - ".$AttributieStortingenOntrekkingen['totaal'].") / $gemiddelde);<br>\n";//exit;
      }
...
//echo "$performance = ((($eindwaarde - $beginwaarde) - ".$AttributieStortingenOntrekkingen['totaal'].") / $gemiddelde);<br>\n";
  ...
  'kosten'=>$FondsDirecteKostenOpbrengsten['kostenTotaal'],
	'opbrengsten'=>$FondsDirecteKostenOpbrengsten['opbrengstTotaal'],
...
foreach ($waarden as $datum=>$waarde)
{
  ...
  $perfData['totaal']['stortEnOnttrekking'] +=$waarde['stortEnOnttrekking'];
	$perfData['totaal']['opbrengsten'] +=$waarde['opbrengsten'];
	$perfData['totaal']['RENMETotaal'] +=$waarde['RENMETotaal'];
...
if($stapeling == true)
{
  ...
  $perfData['totaal']['ongerealiseerdValutaResultaat']=$ongerealiseerdValutaResultaat; 
	...
	$query = "SELECT SUM(actuelePortefeuilleWaardeEuro) / $startValutaKoers AS totaal FROM TijdelijkeRapportage WHERE rapportageDatum ='$tot' AND portefeuille = '".$this->rapport->portefeuille."' AND  type = 'rente' $fondsenWhere ".$__appvar['TijdelijkeRapportageMaakUniek'];
	$DB->SQL($query);
	$DB->Query();
	$totaalA = $DB->nextRecord();
	$query = "SELECT SUM(actuelePortefeuilleWaardeEuro) / $eindValutaKoers AS totaal FROM TijdelijkeRapportage WHERE rapportageDatum ='$van' AND portefeuille = '".$this->rapport->portefeuille."' AND  type = 'rente' $fondsenWhere ".$__appvar['TijdelijkeRapportageMaakUniek'];
	$DB->SQL($query);
	$DB->Query();
