...
$Log: RapportDOORKIJK_L84.php,v $
Revision 1.5  2020/02/29 16:23:08  rvv
*** empty log message ***

Revision 1.4  2019/09/07 16:08:37  rvv
*** empty log message ***

Revision 1.3  2019/09/04 15:33:05  rvv
*** empty log message ***

Revision 1.2  2019/08/03 17:03:51  rvv
*** empty log message ***
...
class RapportDOORKIJK_L84
{
	function RapportDOORKIJK_L84($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = $pdf;
...
*/
	...
	function vulPagina($belCategorien=array())
	{
		$pieTeller = 0;
    ...
    foreach($allekleuren['OIV'] as $kleur=>$kleurdata)
      $this->kleuren['OIV'][$kleur]=array($kleurdata['R']['value'],$kleurdata['G']['value'],$kleurdata['B']['value']);

    
    if ($this->pdf->rapport_GRAFIEK_sortering == 1)
      $order = 'TijdelijkeRapportage.valuta ASC';
    else
	...
	AND TijdelijkeRapportage.rapportageDatum ='".$this->rapportageDatum."' AND TijdelijkeRapportage.Beleggingscategorie IN('".implode("','",$belCategorien)."')"
      .$__appvar['TijdelijkeRapportageMaakUniek'].  " GROUP BY TijdelijkeRapportage.valuta	ORDER BY $order";
    $DB->SQL($query);
    $DB->Query();
    $doorKijk=array();
    $OIVKleuren=array();
    ...
    {
      $doorKijk['categorien'][$cat['valuta']]=$cat['WaardeEuro']/$this->totaalWaarde*100;
      $doorKijk['details'][$cat['Omschrijving']]['percentage']=$cat['WaardeEuro']/$this->totaalWaarde*100;
      $doorKijk['details'][$cat['Omschrijving']]['waardeEUR']=$cat['WaardeEuro'];
      $OIVKleuren[$cat['Omschrijving']]=$this->kleuren['OIV'][$cat['valuta']];
    }
   
    $xOffset =  2 * 98;
   // $doorKijk= $this->bepaalWeging($doorkijkCategorieSoort,$belCategorien);
    $doorkijkCategorieSoort='OIV';
    $this->toonTabel($doorKijk['details'],$xOffset,"Valuta",$OIVKleuren);
    $grafiekdata=array();
    $grafiekTonen=true;
    foreach($this->kleuren[$doorkijkCategorieSoort] as $categorie=>$kleurdata) //foreach($doorKijk['categorien'] as $categorie=>$percentage)
    {
    ...
    if($grafiekTonen==true)
      $this->printPie( $grafiekdata,30+$xOffset,125,'Valuta'); //+$yOffset);
...
JOIN Portefeuilles ON KeuzePerVermogensbeheerder.vermogensbeheerder = Portefeuilles.Vermogensbeheerder
INNER JOIN CategorienPerHoofdcategorie ON CategorienPerHoofdcategorie.Beleggingscategorie = KeuzePerVermogensbeheerder.waarde
AND Portefeuilles.Vermogensbeheerder = CategorienPerHoofdcategorie.Vermogensbeheerder
WHERE
 portefeuille='".$this->portefeuille ."'
AND KeuzePerVermogensbeheerder.categorie = 'Beleggingscategorien' AND CategorienPerHoofdcategorie.Hoofdcategorie='H-Aand'";
		$db->SQL($query);
		$db->Query();
		...
		{
			$this->pdf->rapport_titel = "Allocaties inclusief uitsplitsing aandelen";;
			$this->pdf->AddPage();
