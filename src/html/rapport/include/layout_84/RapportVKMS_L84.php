...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2019/07/10 15:38:33 $
File Versie					: $Revision: 1.2 $

$Log: RapportVKMS_L84.php,v $
...
class RapportVKMS_L84
{
	function RapportVKMS_L84($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
			...
			$this->pdf->rapport_titel = vertaalTekst("Vergelijkende kostenmaatstaf afgelopen 12 maanden",$this->pdf->rapport_taal);
			$this->pdfVullen=true;
	...
	{
		$query="SELECT SUM(ABS(Rekeningmutaties.Bedrag)) AS totaal
...
AND Rekeningen.Memoriaal = 0 AND Rekeningmutaties.Grootboekrekening='FONDS'
...
IF (Rekeningmutaties.Grootboekrekening = 'KOBU','KOST',            IF (Rekeningmutaties.Grootboekrekening = 'BEW','KNBA',      Rekeningmutaties.Grootboekrekening)) AS Grootboekrekening,
IF (Rekeningmutaties.Grootboekrekening = 'KOBU','Transactiekosten',IF (Rekeningmutaties.Grootboekrekening = 'BEW','Bankkosten',Grootboekrekeningen.Omschrijving)) AS Omschrijving
FROM Rekeningmutaties
JOIN Rekeningen ON Rekeningmutaties.Rekening=Rekeningen.Rekening
    ...
    if($this->pdfVullen==true)
		{
	  	...
	  	$this->pdf->row(array(vertaalTekst('Doorlopende kosten financiële instrumenten',$this->pdf->rapport_taal), $this->formatGetal($totaalDoorlopendekosten, 0) . ' EUR'));
		...
		while($data = $DB->nextRecord())
		{
			if($data['Grootboekrekening']=='KNBA')
				$data['Omschrijving']='Kosten depotbank';
			if($this->pdfVullen==true)
			{
    ...
    if($spreadKostenEUR <> 0)
		{
			...
			if($this->pdfVullen==true)
			{
				$this->pdf->row(array('Spread-kosten', $this->formatGetal($spreadKostenEUR, 0), $this->formatGetal($kostenProcent, 2) . ' %'));
		...
		if($this->pdfVullen==true)
		{
			...
			$this->pdf->excelData[]=array('','Totale kosten','', round($vkmWaarde*$gemiddelde/100,2),round($vkmWaarde, 2));
			$this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
...
foreach ($perCategorie as $hoofdcategorie=>$categorieData)
{
 ...
 WHERE Portefeuilles.portefeuille='".$this->portefeuille."'";
    ...
    for($i=$this->pdf->rapport_jaar; $i<=$doelJaar; $i++)
		...
		$waardeZonderKostenKleur=array($this->pdf->rapport_logoKleurBlauw[0],$this->pdf->rapport_logoKleurBlauw[1],$this->pdf->rapport_logoKleurBlauw[2]);
		$waardeNaKostenKleur=array($this->pdf->rapport_logoKleurPaars[0],$this->pdf->rapport_logoKleurPaars[1],$this->pdf->rapport_logoKleurPaars[2]);
		$cumulatieveKostenKleur=array(133,140,140);
		
//		waardeontwikkeling zonder kosten Efidenz-blauw,
//		waardeontwikkeling na kosten in wit,
//		Cumulatieve kosten in Efidenz-grijs

		if($this->pdf->getY()+70>$this->pdf->pagebreak)
			$this->pdf->addPage();

		$this->pdf->setXY(20,$this->pdf->getY()+15);
		$this->BarDiagram(125, 55, $grafiekWaarden,array($waardeZonderKostenKleur,$waardeNaKostenKleur,$cumulatieveKostenKleur),0,0,4,4,false);//50
