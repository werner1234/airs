...
$Log: RapportDOORKIJK_L80.php,v $
Revision 1.4  2020/05/23 16:39:00  rvv
*** empty log message ***
...
class RapportDOORKIJK_L80
{
	function RapportDOORKIJK_L80($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = $pdf;
		$this->pdf->rapport_type = "INDEX";
		$this->pdf->rapport_datum = db2jul($rapportageDatum);
     	$this->pdf->rapport_titel = "Allocaties aandelen inclusief doorkijk";
...
WHERE bronKoppeling='$airsCategorie' AND doorkijkCategoriesoort='".mysql_real_escape_string($doorkijkSoort)."' AND systeem='AIRS' AND vermogensbeheerder='". $this->pdf->portefeuilledata['Vermogensbeheerder']."'";
    ...
    //$aandFilter='';
    //if($doorkijkSoort=='Regios' || $doorkijkSoort=='Beleggingssectoren')
    //{
      $aandFilter="AND TijdelijkeRapportage.Beleggingscategorie='AAND'";
    //}

		$db = new DB();
		$query = "SELECT SUM(actuelePortefeuilleWaardeEuro) AS totaal 
                  FROM TijdelijkeRapportage 
                  WHERE rapportageDatum ='" . $this->rapportageDatum . "' $fondsFilter AND portefeuille = '" . $this->portefeuille . "' $aandFilter " .	$__appvar['TijdelijkeRapportageMaakUniek'];
		$db->SQL($query);
		$db->Query();
		...
		$query = "SELECT fonds,rekening, actuelePortefeuilleWaardeEuro as waardeEUR, ".$vertaling[$doorkijkSoort]." as airsSoort
					FROM TijdelijkeRapportage	WHERE rapportageDatum ='".$this->rapportageDatum."'  $fondsFilter $aandFilter AND portefeuille = '" . $this->portefeuille . "'" .	$__appvar['TijdelijkeRapportageMaakUniek']." Order by fonds";
...
*/
	...
	function vulPagina($hoofdCategorie='')
	{
		...
		$doorkijkTitels=array('Beleggingscategorien'=>'Beleggingscategorie','Regios'=>'Regio Aandelen','Beleggingssectoren'=>'Beleggingssector Aandelen');//array();Beleggingscategorie
