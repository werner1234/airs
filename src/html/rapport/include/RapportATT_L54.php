...
class RapportATT_L54
{
	function RapportATT_L54($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
   ...
   if(count($rendamentWaarden) > 0)
   {
     ...
     //   $this->pdf->CellBorders = array('','US','US','US','US','US','US','US','US','US','US','US');
       ...
       //$this->pdf->SetFillColor(230,230,230);
        ...
        //$this->pdf->SetFillColor($this->pdf->rapport_kop_bgcolor['r']*2.5,$this->pdf->rapport_kop_bgcolor['g']*2.5,$this->pdf->rapport_kop_bgcolor['b']*1.9);
...
*/
   ...
   // $color=array(200,0,0);
    ...
    for ($i=0; $i<count($data); $i++)
    {
      ...
      if($data[$i] <> 0)
        $this->pdf->Text($XDiag+($i+1)*$unit-2+$extrax,$yval2-2.5,$this->formatGetal($data[$i],2));
    ...
    if(is_array($data1))
    {
     // listarray($data1);
      $yval=$YDiag + (($maxVal) * $waardeCorrectie) ;
      $lineStyle = array('width' => 0.3, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => $color1);
  ...
  function VBarDiagram($w, $h, $data)
  {
      ...
      foreach ($data as $datum=>$waarden)
      {
        ...
        //{
          ...
          if($waarden[$categorie] < 0)
          {
             ...
             $tmpCategorieTotalen['neg']+=abs($waarden[$categorie]);
          }
          else
          {
             $grafiekNegatief[$datum][$categorie]=0;
             $tmpCategorieTotalen['pos']+=abs($waarden[$categorie]);
          }
        ...
        $categorieTotalen=array('pos'=>max(array($categorieTotalen['pos'],$tmpCategorieTotalen['pos'])),
                                'neg'=>max(array($categorieTotalen['neg'],$tmpCategorieTotalen['neg'])));
      }
     ...
     // echo "$maxVal $minVal <br>\n";exit;
      if($categorieTotalen['pos'] <= 100)
        $maxVal=100;
      elseif($categorieTotalen['pos'] < 125)
        $maxVal=125;
      elseif($categorieTotalen['pos'] < 150)
        $maxVal=150;
 
      if($categorieTotalen['neg'] <= 0)
        $minVal = 0;
      elseif($categorieTotalen['neg'] < 25)
        $minVal=-25;
      elseif($categorieTotalen['neg'] < 50)
        $minVal=-50;
        
          //   echo $categorieTotalen['neg']." $minVal ";exit;  
