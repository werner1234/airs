...
$Log: RapportSCENARIO_L40.php,v $
Revision 1.5  2017/02/26 10:04:26  rvv
*** empty log message ***

Revision 1.4  2016/10/23 11:32:33  rvv
*** empty log message ***

Revision 1.3  2015/09/21 08:20:18  rvv
*** empty log message ***

Revision 1.2  2015/09/21 07:07:00  rvv
*** empty log message ***
...
class RapportSCENARIO_L40
{
	function RapportSCENARIO_L40($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function formatGetalKoers($waarde, $dec , $start = false)
    ...
    $portefeuilleStartdatum=$check_module_SCENARIO['Startdatum'];
...
//
    ...
    else
    {
      ...
      if($indexatie)    
        $this->pdf->widthB = array(75,18,25,20);
      else
      ...
      $this->pdf->row(array('',vertaalTekst('Cashflow',$this->pdf->rapport_taal)));
       ...
       $nu=getLaatsteValutadatum();
       $stortingen=getStortingen($this->portefeuille,$portefeuilleStartdatum,$nu)-getOnttrekkingen($this->portefeuille,$portefeuilleStartdatum,$nu);

      if($indexatie)
        $this->pdf->row(array('',vertaalTekst('Jaar',$this->pdf->rapport_taal),
                                 vertaalTekst('Verwachting',$this->pdf->rapport_taal).' €',
                                 vertaalTekst('Werkelijk',$this->pdf->rapport_taal).' €'));
      if($stortingen <> 0)                           
        $this->pdf->row(array('',substr($portefeuilleStartdatum,0,4)."-".substr($nu,0,4),'',$this->formatGetal($stortingen,0)));
      $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
      $totaalCashflow=0;
      foreach($sc->cashflowText as $bedragData)
      {  
    ...
    $this->pdf->row(array('',vertaalTekst('Scenario',$this->pdf->rapport_taal).' '.vertaalTekst($sc->CRMdata['gewenstRisicoprofiel'],$this->pdf->rapport_taal),
                             vertaalTekst('Eindvermogen meer dan',$this->pdf->rapport_taal),
                             vertaalTekst('Kans op behalen',$this->pdf->rapport_taal)));
    ...
    foreach($sc->verwachteWaarden as $scenario=>$eindvermogen)
    {
      ...
      $this->pdf->row(array('',vertaalTekst($scenario,$this->pdf->rapport_taal),
      $this->formatGetalNegatief($eindvermogen),
      $this->formatGetal( round((100-$sc->scenarios[$scenario])/5)*5,0).'%'));
    }
