...
class RapportPERFD_L57
{
	function RapportPERFD_L57($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
     ...
     $totaal=array();
        ...
        $this->pdf->CellBorders = array();
		    $this->pdf->row(array(vertaalTekst('Totaal',$this->pdf->rapport_taal),
		                           ...
		                           '',
		                           $this->formatGetal($totaal['totaal']['Rendament']-100,1)
        ...
        $kleuren=array($this->pdf->rapport_grafiek_pcolor);//,array(61,59,56));
  ...
  function printTotaal($totaal,$jaar)
{
   	    $this->pdf->row(array(vertaalTekst('Totaal '.$jaar,$this->pdf->rapport_taal),
		                           ...
		                           $this->formatGetal($totaal['jaar']['JaarRendament']-100,1),
                               $this->formatGetal($totaal['totaal']['Rendament']-100,1)
		                           ));
...
function LineDiagram($w, $h, $data, $color=null, $maxVal=0, $minVal=0, $horDiv=4, $verDiv=4,$jaar=0)
  {
    ...
    $unit = $lDiag / $aantalWaarden;

    if($jaar && $aantalWaarden < 12)
      $unit = $lDiag / 12;
