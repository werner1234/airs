...
$Log: RapportTRANS_L37.php,v $
Revision 1.6  2019/08/07 15:30:49  rvv
*** empty log message ***

Revision 1.5  2012/07/08 07:03:49  rvv
*** empty log message ***
...
class RapportTRANS_L37
{
	function RapportTRANS_L37($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		$this->pdf->rapport_type = "TRANS";
		$this->pdf->rapport_datum = db2jul($rapportageDatum);
		$this->pdf->rapport_datumvanaf = db2jul($rapportageDatumVanaf);
		$this->pdf->rapport_titel = vertaalTekst("Transactie-overzicht",$this->pdf->rapport_taal);
	...
	function writeRapport()
	{
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
		...
		{
		// voor data
		$this->pdf->widthB = array(15,10,20,38,16,22,22,16,22,22,22,22,22,12);
		$this->pdf->alignB = array('L','L','R','L','R','R','R','R','R','R','R','R','R','R');
		// voor kopjes
		$this->pdf->widthA = array(15,10,20,38,16,22,22,16,22,22,22,22,22,12);
...
*/
     ...
     Rekeningmutaties.omschrijving as rekeningOmschrijving,
		...
		"FROM (Rekeningmutaties, Rekeningen, Portefeuilles, Grootboekrekeningen) LEFT JOIN Fondsen  ON  Rekeningmutaties.Fonds = Fondsen.Fonds ".
		"WHERE ".
...
//	echo "lopende -> ".$resultaatlopende." <-  voorgaande ".$resultaatvoorgaande. " -  <br>" ;
			...
			else
			{
			  ...
			  {
			    $aankoop_waardeinValuta=abs($mutaties['Credit']);
			    $aankoop_waarde=abs($mutaties['Credit']) * $mutaties['Valutakoers'] * $mutaties['Rapportagekoers'];
			    $verkoop_waardeinValuta=abs($mutaties['Debet']);
			    $verkoop_waarde=abs($mutaties['Debet']) * $mutaties['Valutakoers'] * $mutaties['Rapportagekoers'];
			  }
		  	...
		  	$percentageTotaal=0;
						...
						if(isset($lastType) && ($mutaties['Transactietype'] != $lastType) )
			  $this->pdf->ln();
			$lastType=$mutaties['Transactietype'];
      ...
      if($mutaties['Omschrijving']=='')
        $mutaties['Omschrijving']=$mutaties['rekeningOmschrijving'];
			...
			//$this->pdf->Cell(55,4,"");
			//$this->pdf->Cell(80,4,rclip($mutaties['Omschrijving'],27));
			...
			// % van totaal
				...
				vertaalTekst($mutaties['Transactietype'],$this->pdf->rapport_taal),
...
//		$this->pdf->line($totaal3+2,$this->pdf->GetY()+1,$totaal3 + $this->pdf->widthA[12],$this->pdf->GetY()+1);
		...
		if($this->pdf->rapport_TRANS_legenda == 1)
		{
   		...
   		while (list($key, $val) = each($transactietypen))
   		{
				...
				{
					case "A" :
						$this->pdf->Cell(30,4, vertaalTekst("A",$this->pdf->rapport_taal), 0,0, "L");
						$this->pdf->Cell(80,4, vertaalTekst("Aankoop",$this->pdf->rapport_taal), 0,1, "L");
					break;
					case "A/O" :
						$this->pdf->Cell(30,4, vertaalTekst("A/O",$this->pdf->rapport_taal), 0,0, "L");
						$this->pdf->Cell(80,4, vertaalTekst("Aankoop / openen",$this->pdf->rapport_taal), 0,1, "L");
					break;
					case "A/S" :
						$this->pdf->Cell(30,4, vertaalTekst("A/S",$this->pdf->rapport_taal), 0,0, "L");
						$this->pdf->Cell(80,4, vertaalTekst("Aankoop / sluiten",$this->pdf->rapport_taal), 0,1, "L");
					break;
					case "D" :
						$this->pdf->Cell(30,4, vertaalTekst("D",$this->pdf->rapport_taal), 0,0, "L");
						$this->pdf->Cell(80,4, vertaalTekst("Deponering",$this->pdf->rapport_taal), 0,1, "L");
					break;
					case "L" :
						$this->pdf->Cell(30,4, vertaalTekst("L",$this->pdf->rapport_taal), 0,0, "L");
						$this->pdf->Cell(80,4, vertaalTekst("Lichting",$this->pdf->rapport_taal), 0,1, "L");
					break;
					case "V" :
						$this->pdf->Cell(30,4, vertaalTekst("V",$this->pdf->rapport_taal) , 0,0, "L");
						$this->pdf->Cell(80,4, vertaalTekst("Verkoop",$this->pdf->rapport_taal), 0,1, "L");
					break;
					case "V/O" :
						$this->pdf->Cell(30,4, vertaalTekst("V/O",$this->pdf->rapport_taal) , 0,0, "L");
						$this->pdf->Cell(80,4, vertaalTekst("Verkoop / openen",$this->pdf->rapport_taal), 0,1, "L");
					break;
					case "V/S" :
						$this->pdf->Cell(30,4, vertaalTekst("V/S",$this->pdf->rapport_taal) , 0,0, "L");
     ...
     Rekeningmutaties.omschrijving as rekeningOmschrijving,
		...
		"Rekeningmutaties.Grootboekrekening = Grootboekrekeningen.Grootboekrekening AND ".
		"Rekeningmutaties.Transactietype <> 'B' AND ".
		"(Grootboekrekeningen.Storting = '1' OR Grootboekrekeningen.Onttrekking = '1') AND ".
		"Rekeningmutaties.Boekdatum > '".$this->rapportageDatumVanaf."' AND ".
		"Rekeningmutaties.Boekdatum <= '".$this->rapportageDatum."' ".
		"ORDER BY Rekeningmutaties.Transactietype, Rekeningmutaties.Boekdatum, Rekeningmutaties.Fonds ";
		$DB = new DB();
		...
		$this->pdf->ln();
		$this->pdf->MultiCell(60,1, vertaalTekst("Autonome transacties",$this->pdf->rapport_taal), 0, "L");
		$this->pdf->ln();
		$this->pdf->SetWidths(array(15,15,20,80 ,25,25,25,  25,25,25,25));
		while($mutaties = $DB->nextRecord())
		{
      if(strlen($mutaties['rekeningOmschrijving']) > 40)
        $mutaties['rekeningOmschrijving']=substr($mutaties['rekeningOmschrijving'],0,39)."...";
			$this->pdf->row(array(date("d-m",db2jul($mutaties['Boekdatum'])),
											$mutaties['Transactietype'],
											'',
											$mutaties['rekeningOmschrijving'],
											$this->formatGetal($mutaties['Credit']-$mutaties['Debet']),
											'',
											'',
											'',
											'',
