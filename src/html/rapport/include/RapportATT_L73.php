...
Revision 1.4  2017/08/30 15:03:56  rvv
*** empty log message ***

Revision 1.3  2017/08/26 17:37:43  rvv
*** empty log message ***

Revision 1.4  2017/08/16 15:58:49  rvv
*** empty log message ***

Revision 1.3  2016/04/21 19:31:19  rvv
*** empty log message ***

Revision 1.2  2016/04/20 15:46:31  rvv
*** empty log message ***

Revision 1.1  2016/04/18 18:51:11  rvv
*** empty log message ***
...
include_once("rapport/include/ATTberekening_L65.php");

class RapportATT_L73
{

	function RapportATT_L73($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		$this->pdf->rapport_type = "ATT";
		$this->pdf->rapport_datum = db2jul($rapportageDatum);
		$this->pdf->rapport_datumvanaf = db2jul($rapportageDatumVanaf);
		$this->pdf->rapport_titel = "Verdeling resultaat";
		$this->portefeuille = $portefeuille;
		$this->rapportageDatumVanaf = $rapportageDatumVanaf;
		$this->rapportageDatum = $rapportageDatum;
		$this->att=new ATTberekening_L65($this);
	}
    ...
    if(db2jul($beginKwartaal)<db2jul($this->pdf->PortefeuilleStartdatum))
      $beginKwartaal=$this->pdf->PortefeuilleStartdatum;
  */
		$startJaar=$rapParts[0]."-01-01";
		if(db2jul($startJaar)<db2jul($this->pdf->PortefeuilleStartdatum))
			$startJaar=$this->pdf->PortefeuilleStartdatum;
		...
		$this->waarden['Jaar']=$this->att->bereken($startJaar,$this->rapportageDatum,'Hoofdcategorie');
...
//listarray($this->waarden['Periode']);
		...
		$header=array("",vertaalTekst("Verdeling resultaat over verslagperiode",$this->pdf->rapport_taal));
...
//$gerealiseerdValuta=array("",vertaalTekst("Gerealiseerde valutaresultaten",$this->pdf->rapport_taal)); //
		...
		$totaal=array("Totaal resultaat over verslagperiode","");   //totaalOpbrengst-totaalKosten
		...
		foreach($categorien as $categorie)
		{
			...
			else
			{
				if($categorie=='G-LIQ')
				{
					$effectenmutaties[] = $this->formatGetal($perfWaarden['stort']-
																									 $this->waarden['Periode']['totaal']['storting']+
																									 $this->waarden['Periode']['totaal']['onttrekking']-
																									 $perfWaarden['resultaat']-$perfWaarden['kosten']+$perfWaarden['opbrengst'], 0);
					$effectenmutaties[] = '';
					$stortingen[] = $this->formatGetal($this->waarden['Periode']['totaal']['storting'],0);
					$stortingen[] = '';
					$onttrekking[] = $this->formatGetal($this->waarden['Periode']['totaal']['onttrekking'],0);
					$onttrekking[] = '';
				}
				else
				{
					...
					$effectenmutaties[] = '';
					$stortingen[] = '0';//'$this->formatGetal($perfWaarden['kosten'],0);
					$stortingen[] = '';
					$onttrekking[] = '0';//$this->formatGetal($perfWaarden['opbrengst'],0);
					$onttrekking[] = '';
				}
			}
...
//    $this->pdf->fillCell=array();
		...
		$this->pdf->CellBorders = $subOnder2;
