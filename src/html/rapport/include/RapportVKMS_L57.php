...
$Log: RapportVKMS_L57.php,v $
Revision 1.2  2020/03/07 14:41:15  rvv
*** empty log message ***
...
class RapportVKMS_L57
{
	function RapportVKMS_L57($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
		...
		$this->melding="";
    $this->brutoRendementCorrectie=0;
		$this->perioden=array();
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    ...
    while($data = $DB->nextRecord())
    {
      ...
      $kostenData[$data['Grootboekrekening']]['totaal']+=$data['totaal'];
        ...
        if($data['Grootboekrekening']=='BEH'||$data['Grootboekrekening']=='PERF')
        {
          if($data['Grootboekrekening']=='PERF')
            $this->brutoRendementCorrectie+=$kostenProcent;
...
In het onderstaande overzicht zijn de werkelijke directe en indirecte kosten met betrekking tot uw beleggingsportefeuille opgenomen over de laatste 12 maanden. Door middel van twee illustraties is weergegeven wat de impact is van de kosten op het rendement van de afgelopen 12 maanden en wat de impact is van de verwachte kosten op het verwachte rendement van de portefeuille op lange termijn. Indien u meer informatie wenst over de specificatie van de te verwachten kosten, kunt u uiteraard contact met ons opnemen.'));
 // listarray($risicoklasse);
      $this->pdf->ln();
    $this->pdf->excelData[]=array();
      $this->pdf->excelData[]=array();
      $this->pdf->setAligns(array('L', 'R','R', 'R'));
      $this->pdf->setWidths(array(135, 30, 30));
      ...
      {
        $this->pdf->row(array('Totaal te verwachten directe en indirecte kosten op jaarbasis', '','', $this->formatGetal($vkmWaarde-$this->brutoRendementCorrectie, 2, true, true)));
      }
      $this->pdf->ln();
      $this->pdf->SetFont($this->pdf->rapport_font, '', $fontsize);
      ...
      if($this->pdf->rapport_layout <> 25)
      {
        $this->pdf->row(array('Verwacht netto rendement per jaar', '', '', $this->formatGetal($risicoklasse['verwachtRendement'] - $vkmWaarde+$this->brutoRendementCorrectie, 2) . ' %'));
...
foreach ($perCategorie as $hoofdcategorie=>$categorieData)
{
 ...
 WHERE Portefeuilles.portefeuille='".$this->portefeuille."'";
    ...
    $vkmwaarde-=$this->brutoRendementCorrectie;
    ...
    {
      $waardeZonderKostenKleur = array(0,149,219);//array( 0,112,192 );
      $waardeNaKostenKleur = array(102,36,131);//array(12,173,71);
      $cumulatieveKostenKleur = array(250,123,18);//array(250,6,87);
    }
		//if($this->pdf->getY()+70>$this->pdf->pagebreak)
		//	$this->pdf->addPage();
    $this->pdf->AutoPageBreak=false;
    $this->pdf->setXY(195,72-10);
