...
class RapportATT_L80
{
	function RapportATT_L80($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
ORDER BY Beleggingscategorien.Afdrukvolgorde desc"; */
    ...
    if(isset($categorien['geen']))
    {
      $this->categorieVolgorde['geen'] = 'geen';
      $this->categorieOmschrijving['geen'] = vertaalTekst('Geen categorie', $this->pdf->rapport_taal);
    }
...
function formatGetalLength ($getal,$decimaal,$gewensteLengte)
{
 ...
 return number_format($getal,$decimaal,',','');
  ...
  function VBarDiagram($w, $h, $data)
  {
        ...
        //{
         ...
         // $minVal=min(array($minVal,$waarden[$categorie]));
          ...
          if($waarden[$categorie] < 0)
          {
            ...
            $somOpDatumNeg[$datum]+=$waarden[$categorie];
          }
          else
          {
            $grafiekNegatief[$datum][$categorie] = 0;
            $somOpDatum[$datum]+=$waarden[$categorie];
          }
      ...
      $maxVal=max($somOpDatum);
      $minVal=min($somOpDatumNeg);
