...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2019/04/24 12:01:24 $
File Versie					: $Revision: 1.2 $

$Log: RapportPERFG_L52.php,v $
Revision 1.2  2019/04/24 12:01:24  rvv
*** empty log message ***
...
class RapportPERFG_L52
{
	function RapportPERFG_L52($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
$specifiekeIndex=array('performance'=>0);
$beginPeriode=$start;
    
    $query2 = "SELECT Fondsen.Fonds as SpecifiekeIndex, Fondsen.Omschrijving, Fondsen.Valuta FROM Fondsen
	             WHERE Fondsen.Fonds  = '" . mysql_real_escape_string($this->pdf->portefeuilledata['SpecifiekeIndex']) . "' ";
    $DB->SQL($query2);
    $DB->Query();
    $fondsOmschrijvingen = $DB->lookupRecord();
    $fondsWissel='';
foreach ($indexData as $index=>$data)
{
  if($data['datum'] != '0000-00-00')
  {
    ...
    $specifiekeIndexFonds=getSpecifiekeIndex($this->portefeuille,$data['datum']);
    $maandPerf=getFondsPerformance($specifiekeIndexFonds,$beginPeriode,$data['datum']);
    $specifiekeIndex['performance']=((1+$specifiekeIndex['performance']/100)*(1+$maandPerf/100)-1)*100;
    //echo "$beginPeriode -> ".$data['datum']." | ".round($specifiekeIndex['performance'],2)." - ".round($data['specifiekeIndex']-100,2)." | $specifiekeIndexFonds - ".$this->pdf->portefeuilledata['SpecifiekeIndex']."<br>\n";
    $grafiekData['benchmarkIndex'][] = $specifiekeIndex['performance'];
//    $grafiekData['benchmarkIndex'][] = $data['specifiekeIndex']-100;
  
    if($specifiekeIndexFonds<>$this->pdf->portefeuilledata['SpecifiekeIndex'])
    {
      ...
      $fondsWissel = vertaalTekst("Benchmark is op",$this->pdf->rapport_taal).' '. date('d-m-Y',db2jul($data['datum'])).' '.
        vertaalTekst("gewijzigd van",$this->pdf->rapport_taal).' '. $fondsOmschrijvingen[$specifiekeIndexFonds].' '.
        vertaalTekst("naar",$this->pdf->rapport_taal).' '.$fondsOmschrijvingen['Omschrijving'];
    }

  }
  $beginPeriode=$data['datum'] ;
}
  ...
  if(count($rendamentWaarden) > 0)
   {
     ...
     $totaal=array();
        ...
        $kleuren=array(array(210,16,68),array(61,59,56));
        ...
        foreach($kleuren as $index=>$kleur)
        {
          ...
          {
            if($fondsWissel<>'')
              $omschrijving=$fondsWissel;
            else
              $omschrijving=$fondsOmschrijvingen['Omschrijving'];
            $this->pdf->Cell(50, 4, $omschrijving, 0, 0, "L");
          }
        }
