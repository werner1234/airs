...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2017/03/31 15:39:22 $
File Versie					: $Revision: 1.2 $

$Log: RapportHSE_L71.php,v $
Revision 1.2  2017/03/31 15:39:22  rvv
*** empty log message ***

Revision 1.1  2016/10/16 15:14:53  rvv
*** empty log message ***
...
class RapportHSE_L71
{
	function RapportHSE_L71($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function getDividend($fonds)
  {
	...
	function writeRapport()
	{
		...
		$this->pdf->widthB = array(2,63,30,18,17,20,12,17,1,21,16,17,17,17,12);
		$this->pdf->alignB = array('R','L','L','R','R','R','R','R','R','R','R','R','R','R','R','R');

		// voor kopjes
		$this->pdf->widthA = array(   65,18,17,20,12,17,21,1,16,17,17,17,12);
    ...
    $this->pdf->templateVars['VOLKPaginas']=$this->pdf->page;
      ...
      TijdelijkeRapportage.BeleggingscategorieOmschrijving as Omschrijving,".
			...
			" TijdelijkeRapportage.beleggingscategorie,TijdelijkeRapportage.beleggingssectorOmschrijving,  ".
  ...
  (TijdelijkeRapportage.beginPortefeuilleWaardeEuro) / ".$this->pdf->ValutaKoersBegin. ")) as subtotaalbegin, 
      ".
			...
			" ORDER BY TijdelijkeRapportage.hoofdcategorieVolgorde, TijdelijkeRapportage.beleggingscategorieVolgorde asc";
...
//echo $subquery."<br><br>";exit;
      ...
      if($regel%2!=0)
        ...
        unset($this->pdf->fillCell);
				...
				$this->pdf->row(array("",
													$subdata['fondsOmschrijving'],$subdata['ISINCode'],
    ...
    if($categorien['beleggingscategorie']=='AAND')
      ...
      $subtotaal = array();
		...
		if($DB->records() > 0)
		{
			...
			//		$this->printKop(vertaalTekst("Waarden",$this->pdf->rapport_taal)." ".$categorien[valuta],"");
				...
				//	"AND TijdelijkeRapportage.valuta =  '".$categorien[valuta]."'".
					...
					$DB2 = new DB();
					$DB2->SQL($subquery); //echo $subquery;exit;
					$DB2->Query();
          $regel=0;
					while($subdata = $DB2->NextRecord())
					{
		...
		$query = "SELECT TijdelijkeRapportage.fondsOmschrijving, ".
			...
			" TijdelijkeRapportage.actuelePortefeuilleWaardeEuro / ".$this->pdf->ValutaKoersEind. " as actuelePortefeuilleWaardeEuro, ".
			" TijdelijkeRapportage.valuta, TijdelijkeRapportage.portefeuille,
			Rekeningen.Deposito, Rekeningen.Termijnrekening, Rekeningen.Memoriaal, Rekeningen.IBANnr ".
    ...
    $regel=0;
		...
		if($DB1->records() > 0)
		{
			...
			foreach($liqiteitenBuffer as $data)
			{
				...
				$omschrijving = vertaalTekst(str_replace("{Rekening}",$data['IBANnr'],$omschrijving),$this->pdf->rapport_taal);
		...
		{
			echo "<script>alert('Fout : Fout in rapport ".$this->portefeuille.", totale waarde (".round($totaalWaarde,2).") komt niet overeen met afgedrukte totaal (".round($actueleWaardePortefeuille,2).") in rapport ".$this->pdf->rapport_type."');</script>";
			ob_flush();
	  ...
	  unset($this->pdf->fillCell);
		...
		$orderModule=GetModuleAccess("ORDER");
		if($orderModule > 0)
		{
		if($orderModule==1)
		{
...
WHERE OrderRegelsV2.portefeuille = '" . $this->portefeuille . "' AND OrderRegelsV2.orderregelStatus < 3
ORDER BY OrdersV2.fonds";
		}
		$DB1 = new DB();
		$DB1->SQL($query);
		$DB1->Query();
      ...
      $widths=array(75,30,20,30);
			...
			while($data = $DB1->NextRecord())
			{
				if($data['fondsValuta'] <> '' && $data['valutakoers']==0)
				{
					$query = "SELECT Koers FROM Valutakoersen WHERE Valutakoersen.valuta='" . $data['fondsValuta'] . "' ORDER BY Valutakoersen.Datum desc limit 1";
					$DB->SQL($query);
					$tmp = $DB->lookupRecord();
					$data['valutakoers'] = $tmp['Koers'];
				}
				$query="SELECT Koers FROM Fondskoersen WHERE Fondskoersen.fonds='".$data['fonds']."' ORDER BY Fondskoersen.Datum desc limit 1";
				$DB->SQL($query);
				$tmp=$DB->lookupRecord();
				$data['FondskoersAirs']=$tmp['Koers'];
				if($orderModule==2)
					$query="SELECT OrderUitvoeringV2.uitvoeringsPrijs FROM OrderUitvoeringV2 WHERE OrderUitvoeringV2.orderid='".$data['orderid']."' limit 1";
				else
				  $query="SELECT OrderUitvoering.uitvoeringsPrijs FROM OrderUitvoering WHERE OrderUitvoering.orderid='".$data['orderid']."' limit 1";
				$DB->SQL($query);
				$tmp=$DB->lookupRecord();
				$data['Fondskoers']=$tmp['uitvoeringsPrijs'];
				...
				if($data['Fondskoers']==0 && $data['FondskoersAirs'] <> 0)
					$data['Fondskoers']=$data['FondskoersAirs'];
