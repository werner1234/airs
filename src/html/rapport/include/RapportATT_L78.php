...
class RapportATT_L78
{
	function RapportATT_L78($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
function formatGetalLength ($getal,$decimaal,$gewensteLengte)
{
 ...
 return number_format($getal,$decimaal,',','');
  ...
  function VBarDiagram($w, $h, $data)
  {
      ...
      foreach ($data as $datum=>$waarden)
      {
        ...
        //{
          ...
          if($waarden[$categorie]>0)
            $waardenPerDatum['pos'][$datum]+=$waarden[$categorie];
          else
            $waardenPerDatum['neg'][$datum]+=$waarden[$categorie];
      ...
      foreach($waardenPerDatum as $type=>$datumData)
      {
        foreach($datumData as $datum=>$waarde)
        {
