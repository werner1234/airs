...
/*
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2019/11/23 18:37:04 $
 		File Versie					: $Revision: 1.5 $

 		$Log: RapportRISK_L56.php,v $
 		Revision 1.5  2019/11/23 18:37:04  rvv
 		*** empty log message ***
 		
 		Revision 1.4  2019/10/19 16:45:25  rvv
 		*** empty log message ***
 		
 		Revision 1.3  2019/10/11 17:39:17  rvv
 		*** empty log message ***
...
class RapportRISK_L56
{
	function RapportRISK_L56($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
{
  $riskData = $stdev->riskAnalyze('totaal', 'benchmark');
}
else
{
  $riskData=$stdev->riskAnalyze('totaal','totaal');
  $riskData['standaarddeviatieBenchmark']=-9999;
}
...
//    $totaalWaarde = $totaalWaarde['totaal'];
    ...
    $this->pdf->ln(2);
    $this->pdf->row(array('','Standaarddeviatie benchmark',($riskData['standaarddeviatieBenchmark']==-9999?'':$this->formatGetal($riskData['standaarddeviatieBenchmark'],1).'%')));
    $this->pdf->ln(2);
    $this->pdf->row(array('','Value at Risk',$this->formatGetal((100-$riskData['valueAtRisk'])/100*$totaalWaarde['totaal'],2).'',''));//'Value at Risk geeft het verwachte maximale verlies aan met een waarschijnlijkheid van 95%. De historische VaR is bepaald aan de hand van de werkelijke jaarlijkse rendementsverdeling over de afgelopen tien jaar.'
    $this->pdf->ln(2);
...
//        $this->pdf->Rect($XDiag+$i*$unit-0.5, $yval-0.5, 1, 1 ,'F','',$color);
      ...
      if ($i==count($data)-1)
      {
        ...
        $this->pdf->TextWithRotation($XDiag + ($i + 1) * $unit+1 , $yval2 + 0.5,$this->formatGetal($data[$i],1).'%',0);
        $this->pdf->SetFont($this->pdf->rapport_font, '', 8);
      }
...
//          $this->pdf->Rect($XDiag+($i+1)*$unit-0.5, $yval2-0.5, 1, 1 ,'F','',$color1);
        if ($i==count($data)-1)
        {
          $this->pdf->Rect($XDiag + ($i + 1) * $unit - 0.5, $yval2 - 0.5, 1, 1, 'F', '', $color1);
          $this->pdf->SetFont($this->pdf->rapport_font, '', 6);
          $this->pdf->TextWithRotation($XDiag + ($i + 1) * $unit+1 , $yval2 + 0.5,$this->formatGetal($data1[$i],1).'%',0);
          $this->pdf->SetFont($this->pdf->rapport_font, '', 8);
        }
         $yval = $yval2;
      }
