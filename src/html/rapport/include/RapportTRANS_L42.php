...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2018/07/25 15:37:42 $
File Versie					: $Revision: 1.7 $

$Log: RapportTRANS_L42.php,v $
Revision 1.7  2018/07/25 15:37:42  rvv
*** empty log message ***

Revision 1.6  2015/12/19 08:29:17  rvv
*** empty log message ***

Revision 1.5  2014/07/06 12:34:34  rvv
*** empty log message ***

Revision 1.4  2013/08/24 15:48:47  rvv
*** empty log message ***

Revision 1.3  2013/08/18 12:23:35  rvv
*** empty log message ***

Revision 1.2  2013/03/23 16:19:36  rvv
*** empty log message ***
...
class RapportTRANS_L42
{
	function RapportTRANS_L42($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		$this->pdf->rapport_type = "TRANS";
		$this->pdf->rapport_datum = db2jul($rapportageDatum);
		$this->pdf->rapport_datumvanaf = db2jul($rapportageDatumVanaf);
		$this->pdf->rapport_titel = "Transactie overzicht ".date("j",$this->pdf->rapport_datumvanaf)." ".
	...
	function writeRapport()
	{
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
		...
		{
		// voor data
		$this->pdf->widthB = array(15,11,20,58,16,22,8,22,16,8,22,22,21,20);
...
//	echo "lopende -> ".$resultaatlopende." <-  voorgaande ".$resultaatvoorgaande. " -  <br>" ;
        ...
        $result_beginditjaar = $this->formatGetal($beginditjaar,$this->pdf->rapport_TRANS_decimaal);
  ...
  $fondsResultaat=0;
  $valutaResultaat=0;
   if($verkoop_koers <> '' && $toonResultaat == true)
   {
   // listarray($historie);
    ...
    // $historischekostprijsValuta = $mutaties['Aantal']*$historie['historischeWaarde']* $mutaties['Fondseenheid'];//$historischekostprijs = $mutaties[Aantal]        * $historie[historischeWaarde]       * $historie[historischeValutakoers]        * $mutaties[Fondseenheid];
     ...
     $totaalValutaResultaat+=$valutaResultaat;
      ...
      $datum=date("d-m",db2jul($mutaties['Boekdatum']));
		...
		$totaal2 = $totaal1  + $this->pdf->widthA[5] + $this->pdf->widthA[6] + $this->pdf->widthA[7]  ;
		$totaal3 = $totaal2 + $this->pdf->widthA[8] + $this->pdf->widthA[9] + $this->pdf->widthA[10] ;
    $totaal4 = $totaal3 + $this->pdf->widthA[11];
		...
		$this->pdf->line($totaal2 + 2, $this->pdf->GetY(), $totaal2 + $this->pdf->widthA[8],  $this->pdf->GetY());
    ...
    $this->pdf->line($totaal4 + 2, $this->pdf->GetY(), $totaal4 + $this->pdf->widthA[12], $this->pdf->GetY());
		$this->pdf->row(array("",
       	  			...
       	  			"",
								$this->formatGetal($totaal_verkoop_waarde,$this->pdf->rapport_TRANS_decimaal),
 								"",
								"",
								$this->formatGetal($totaalFondsResultaat,$this->pdf->rapport_TRANS_decimaal),
								$this->formatGetal($totaalValutaResultaat,$this->pdf->rapport_TRANS_decimaal)));
		...
		$this->pdf->line($totaal2+2,$this->pdf->GetY()+1,$totaal2 + $this->pdf->widthA[8],$this->pdf->GetY()+1);
    ...
    $this->pdf->SetFont($this->pdf->rapport_font,$this->pdf->rapport_fontstyle,$this->pdf->rapport_fontsize);
      ...
      foreach($transactietypen as $index=>$type)
      {
        if($index==0)
          $this->pdf->CellBorders=array('',array('T','L'),'T',array('T','R'));
        elseif($index==count($transactietypen)-1)
          $this->pdf->CellBorders=array('',array('U','L'),'U',array('U','R'));  
        else
          $this->pdf->CellBorders=array('','L','','R');    
        $this->pdf->row(array('',$type,'',$transactietypenÓmschrijving[$type]));
      }
