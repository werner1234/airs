...
class RapportOIS_L77
{
	function RapportOIS_L77($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
function printSubTotaalRestricties($totalen,$valuta)
{
  ...
  $this->pdf->Row(array('','Subtotaal '.$valuta,'','','',$this->formatGetal($totalen['bedrag'],2),'','',$this->formatGetal($totalen['opgevraagd'],2),$this->formatGetal($totalen['verplichting'],2)));
...
//echo $data['fondsOmschrijving']." | ".$data['actuelePortefeuilleWaardeEuro']."+".$extra['opbrengstTotaal'].")/$aankoopEur <br>\n";
    ...
    PieChart_L77($this->pdf, $chartsize, $vwh, $grafiek['Aanvang']['Percentage'], '%l', $grafiek['Aanvang']['Kleur'], vertaalTekst('Aanvang', $this->pdf->rapport_taal), $legendaStart);

    $i=1;
    $this->pdf->setXY($this->pdf->marge+5+90*$i , $ystart);
    $legendaStart=array($this->pdf->marge+5+90*$i,$ystart+$chartsize+10);
    PieChart_L77($this->pdf, $chartsize, $vwh, $grafiek['Strategie']['Percentage'], '%l', $grafiek['Strategie']['Kleur'], vertaalTekst('Strategie', $this->pdf->rapport_taal), $legendaStart);
  ...
  function AddRestricties()
  {
    ...
    $this->pdf->SetWidths(array(0,70,8,26,15,25,1,1,25,25));
