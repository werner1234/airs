...
Revision 1.2  2018/05/16 15:32:27  rvv
*** empty log message ***

Revision 1.1  2018/05/12 15:46:42  rvv
*** empty log message ***
...
class RapportRISK_L33
{
	function RapportRISK_L33($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = $pdf;
		$this->pdf->rapport_type = "RISK";
		$this->pdf->rapport_datum = db2jul($rapportageDatum);
  	$this->pdf->rapport_titel = "Allocatie per regio";
...
*/
	...
	function vulPagina($belCategorien='')
	{
		...
		$doorkijkCategorieSoorten=array('Regios');

		$doorkijkTitels=array('Regios'=>'Regio');//array();Beleggingscategorie
		foreach($doorkijkCategorieSoorten as $index=>$doorkijkCategorieSoort)
		{
			...
			arsort($doorKijk['categorien']);
			$newDetail=array();
			foreach($doorKijk['categorien'] as $categorie=>$percentage)
      {
        $newDetail[$categorie]=$doorKijk['details'][$categorie];
      }
      $doorKijk['details']=$newDetail;


			$this->toonTabel($doorKijk['details'],$xOffset,$doorkijkTitels[$doorkijkCategorieSoort],$this->kleuren[$doorkijkCategorieSoort]);
			$grafiekdata=array();
			$grafiekTonen=true;
			foreach($doorKijk['categorien'] as $categorie=>$percentage) //foreach($doorKijk['categorien'] as $categorie=>$percentage)
			{
			...
			if($grafiekTonen==true)
			  $this->printPie( $grafiekdata,30+$xOffset+98,60,$doorkijkTitels[$doorkijkCategorieSoort]); //+$yOffset);
			$pieTeller++;
		}
