...
/* 	
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: PDFRapport_headers_L52.php,v $
...
*/
function Header_basis_L52($object)
{
 $pdfObject = &$object;
	...
	function HeaderVKM_L52($object)
	{
		$pdfObject = &$object;
		$pdfObject->HeaderVKM();
	}
	
	  function HeaderFRONT_L52($object)
	  {
	    $pdfObject = &$object;
...
function HeaderModel_L52($object)
{
  $pdfObject = &$object;
  $pdfObject->SetFont($pdfObject->rapport_font,'b',$pdfObject->rapport_fontsize);
...
function HeaderVKMD_L52($object)
{
  HeaderVKMS_L52($object);
}

	function HeaderVKMS_L52($object)
	{
		$pdfObject = &$object;
		$pdfObject->ln();
		...
		else
    {
	  ...
	  function HeaderSCENARIO_L52($object)
	  {
	    $pdfObject = &$object;
	    //$pdfObject->headerSCENARIO();
    ...
    function HeaderRISK_L52($object)
    {
	    $pdfObject = &$object;
			$pdfObject->Ln();
			$pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
    ...
    function HeaderGRAFIEK_L52($object)
    {
	    $pdfObject = &$object;
      $pdfObject->Ln();
      $pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
    ...
    function HeaderPERFD_L52($object)
    {
      $pdfObject = &$object;
      HeaderPERF_L52($object);
      $pdfObject->SetTextColor($pdfObject->rapport_fontcolor['r'],$pdfObject->rapport_fontcolor['g'],$pdfObject->rapport_fontcolor['b']);
    }
  
    
    function HeaderTRANS_L52($object)
	  {
	    $pdfObject = &$object;
...
$pdfObject->ln(2);
	  ...
	  function HeaderINHOUD_L52($object)
	  {
	    $pdfObject = &$object;
	    //$pdfObject->headerSCENARIO();

	  }	  
 	  function HeaderPERF_L52($object)
	  {
	    $pdfObject = &$object;
      $pdfObject->ln();
      $pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
...
$pdfObject->ln();
    ...
    function HeaderINDEX_L52($object)
	  {
	    $pdfObject = &$object;
	  }
    function HeaderOIB_L52($object)
	  {
	    $pdfObject = &$object;
      $pdfObject->Ln();
      		$pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
...
*/
   ...
   function HeaderPERFG_L52($object)
	{
    $pdfObject = &$object;
...
*/
  ...
  function HeaderVOLK_L52($object)
	{
    $pdfObject = &$object;
   	$pdfObject->ln();
		$pdfObject->SetFont($pdfObject->rapport_font,$pdfObject->rapport_kop_fontstyle,$pdfObject->rapport_fontsize);
...
function HeaderKERNZ_L52($object)
{
	$pdfObject = &$object;
	$pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
	$pdfObject->Rect($pdfObject->marge,$pdfObject->getY(),297-$pdfObject->marge*2,8,'F');
...
function HeaderHUIS_L52($object)
{
	$pdfObject = &$object;
	$pdfObject->ln();
	$widthBackup=$pdfObject->widths;
...
function getFondsKoers_L52($fonds,$datum)
{
	$db=new DB();
...
function getValutaKoers_L52($valuta,$datum)
{
	$db=new DB();
...
function formatGetal_L52($waarde, $dec)
{
	return number_format($waarde,$dec,",",".");
}


function indexKader_L52($object)
{
	$pdfObject = &$object;
	$db=new DB();
...
WHERE Portefeuilles.SpecifiekeIndex = Fondsen.Fonds AND Portefeuilles.Portefeuille = '".$portefeuille."' ";
	...
	foreach($indices as $data)
	{
		foreach($perioden as $periode=>$datum)
			$data[$periode]=getFondsKoers_L52($data['indexFonds'],$datum);
	...
	$pdfObject->SetTextColor($pdfObject->rapport_kop_fontcolor['r'],$pdfObject->rapport_kop_fontcolor['g'],$pdfObject->rapport_kop_fontcolor['b']);
		foreach($indices as $fonds=>$fondsData)
		{
			...
			$pdfObject->SetFont($pdfObject->rapport_font,'',$pdfObject->rapport_fontsize);
			$pdfObject->row(array(date("d-m-Y",db2jul($perioden['beginPeriode']))." / ".date("d-m-Y",db2jul($perioden['eind'])),formatGetal_L52($fondsData['beginPeriode'],2),formatGetal_L52($fondsData['eind'],2),formatGetal_L52($fondsData['periode'],2).'%'));
			$pdfObject->row(array(date("d-m-Y",db2jul($perioden['begin']))." / ".date("d-m-Y",db2jul($perioden['eind'])),formatGetal_L52($fondsData['begin'],2),formatGetal_L52($fondsData['eind'],2),formatGetal_L52($fondsData['kwartaal'],2).'%'));
...
function risicoMeter_L52($object,$pageX,$pageY,$width,$riscoklasse)
{
	$pdfObject = &$object;
