...
class RapportTRANS_L66
{
	function RapportTRANS_L66($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//	echo "lopende -> ".$resultaatlopende." <-  voorgaande ".$resultaatvoorgaande. " -  <br>" ;
   		...
   		while (list($key, $val) = each($transactietypen))
   		{
				switch($val)
				{
					...
					case "A/O" :
						...
						$this->pdf->Cell(80,4, vertaalTekst("Aankoop open (Open Buy)",$this->pdf->rapport_taal), 0,1, "L");
					break;
					case "A/S" :
						$this->pdf->Cell(30,4, "A/S", 0,0, "L");
						$this->pdf->Cell(80,4, vertaalTekst("Aankoop sluiten (Close Buy)",$this->pdf->rapport_taal), 0,1, "L");
					break;
					case "D" :
					...
					case "V/O" :
						...
						$this->pdf->Cell(80,4, vertaalTekst("Open verkoop (Open Sell)",$this->pdf->rapport_taal), 0,1, "L");
					break;
					case "V/S" :
						$this->pdf->Cell(30,4, "V/S", 0,0, "L");
						$this->pdf->Cell(80,4, vertaalTekst("Verkoop sluiten (Close Sell)",$this->pdf->rapport_taal), 0,1, "L");
					break;
				}
			}
		}
   ...
   // $this->pdf->MultiCell(280,4, $this->pdf->rapport_TRANS_disclaimerText, 0, "L");
    $this->pdf->ln();
		...
		if($this->pdf->getY()+32>$this->pdf->PageBreakTrigger)
    {
      $this->pdf->addPage();
      $this->pdf->ln();
    }
		$this->pdf->rect($this->pdf->marge,$this->pdf->getY()-1,280,32+2);
    $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
    $this->pdf->MultiCell(280,4, 'Geschiktheidsverklaring', 0, "L");
    $this->pdf->ln();
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
    $this->pdf->MultiCell(280,4, 'Wij willen onze dienstverlening zo goed mogelijk afstemmen op uw persoonlijke situatie.
Om uw vermogen goed te kunnen beheren en/of adviseren is het van belang dat wij beleggen op basis van actuele wensen en uitgangspunten.
Deze wensen en uitgangspunten hebben betrekking op uw persoonlijke financiële situatie, doelstellingen, beleggingshorizon en risicobereidheid.
Wij baseren ons op de uitgangspunten zoals die bij ons bekend zijn. Deze vindt u kort samengevat in het risicoprofiel zoals in deze rapportage weergegeven.
Hierbij verklaren wij dat het beleggingsbeleid en de mutaties die verricht zijn, passen bij deze wensen en uitgangspunten.
Mochten er zich in uw persoonlijke situatie veranderingen hebben voorgedaan die van invloed kunnen zijn op het door ons te voeren beleggingsbeleid, dan vernemen we dit graag van u.', 0, "L");
