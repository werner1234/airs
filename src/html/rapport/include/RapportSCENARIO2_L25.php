...
class RapportSCENARIO2_L25
{
	function RapportSCENARIO2_L25($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		$this->pdf->rapport_type = "SCENARIO2";
  ...
  function formatGetalKoers($waarde, $dec , $start = false)
    ...
    $this->fontsize=$this->pdf->rapport_fontsize+3;
    $this->rowHeightBackup=$this->pdf->rowHeight;
    $this->pdf->rowHeight=$this->pdf->rowHeight+1;
...
//    {
    ...
    */
    ...
    if(!$sc->loadMatrix())
      $sc->createNewMatix(true);
//
   	$this->pdf->widthA = array(55,40,20);
		$this->pdf->alignA = array('L','R','R');
    $this->pdf->SetWidths($this->pdf->widthA);
    $this->pdf->SetAligns($this->pdf->alignA);
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->fontsize);
...
*/
    ...
    $this->pdf->setXY(130,$this->baseY+25);
    $this->pdf->SetFont($this->pdf->rapport_font,'b',$this->fontsize);
    $this->pdf->Cell(145,0,vertaalTekst('Scenario-analyse',$this->pdf->rapport_taal),0,0,'C');
    $this->pdf->setXY(130,$this->baseY+25);
    if($this->pdf->lastPOST['scenario_werkelijkVerloop']==1)
    {
      ...
      $this->LineDiagram(145,70,$grafiek,$sc->werkelijkVerloop,$sc->CRMdata['doelvermogen']);
    }
    else
      $this->LineDiagram(145,70,$sc->scenarioGemiddelde,'',$sc->CRMdata['doelvermogen']);


     $this->pdf->setY($this->baseY+50);
    $n=0; 
    if($this->pdf->lastPOST['scenario_werkelijkVerloop']==1)
    { 
      ksort($cashflow);
      $this->pdf->widthB = array(25,25,25);
    ...
    else
    {
      ...
      if($indexatie)    
        $this->pdf->widthB = array(25,30,20);
      else
        $this->pdf->widthB = array(25,30,2);
		  $this->pdf->alignB = array('L','R','R','L');
      $this->pdf->SetWidths($this->pdf->widthB);
      $this->pdf->SetAligns($this->pdf->alignB);
      $this->pdf->SetFont($this->pdf->rapport_font,'b',$this->fontsize);
    ...
    //$this->pdf->setY($this->baseY);
    $this->pdf->ln();
		$this->pdf->widthB = array(35,20,30);
    ...
    $this->pdf->setY($this->baseY);
    $w=125;
    $this->pdf->widthB = array($w,80,35,20);
    ...
    $this->pdf->row(array('',vertaalTekst('Gemiddeld eindvermogen',$this->pdf->rapport_taal),"€ ".$this->formatGetalNegatief($verwachtEindvermogen,0)));
  ...
  function VBarDiagram($w, $h, $data,$titel,$kleurdata,$doelvermogen)
  {
    ...
    $this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->fontsize+2);
...
function LineDiagram($w, $h, $data,$werkelijkVerloop,$doelVermogen)
  {
   ...
   if ($maxVal < 0)
    ...
    $this->pdf->SetFont($this->pdf->rapport_font, '', 6+3);
    $this->pdf->SetTextColor(0,0,0);
