...
Revision 1.2  2019/06/21 14:52:45  rm
7880

Revision 1.1  2017/11/18 18:58:17  rvv
*** empty log message ***

Revision 1.8  2017/05/10 14:44:58  rvv
*** empty log message ***

Revision 1.7  2017/03/11 20:27:43  rvv
*** empty log message ***

Revision 1.6  2015/12/19 08:29:17  rvv
*** empty log message ***

Revision 1.5  2015/05/02 14:57:32  rvv
*** empty log message ***

Revision 1.4  2015/04/29 15:28:24  rvv
*** empty log message ***

Revision 1.3  2015/03/11 17:13:49  rvv
*** empty log message ***

Revision 1.2  2015/03/01 14:08:16  rvv
*** empty log message ***

Revision 1.1  2015/02/18 17:09:13  rvv
*** empty log message ***
...
class RapportTRANS_L87
{
	function RapportTRANS_L87($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
		...
		$db2 = new DB();
			...
			// voor data
		$this->pdf->widthB = array(11,13,15,65,18,20,25,25,18,20,25,25);//,19.5,19.5,19,11=69
		$this->pdf->alignB = array('L','L','R','L','R','R','R','R','R','R','R','R','R','R','R','R');
		// voor kopjes
    ...
    $db2=new DB();
      ...
      $query="SELECT SUM(Debet*Valutakoers) as Debet, SUM(Credit*Valutakoers) as Credit,
 (SUM(Credit*Valutakoers) - SUM(Debet*Valutakoers)) as totaal, Grootboekrekening
      ...
      $db2->SQL($query);
      $db2->Query();
      while($koppeling=$db2->nextRecord())
      {
        $mutaties['grootboekKoppeling'][$koppeling['Grootboekrekening']]=$koppeling;
      }
	...
	//	listarray($buffer);
		...
		foreach ($buffer as $mutaties)
		{
			...
			$kosten=$mutaties['grootboekKoppeling']['KOST']['totaal']+$mutaties['grootboekKoppeling']['KOBU']['totaal'];
...
//	echo "lopende -> ".$resultaatlopende." <-  voorgaande ".$resultaatvoorgaande. " -  <br>" ;
				...
				if($kosten<>0)
				  $verkoopKostenTxt=$this->formatGetal(      $kosten,$this->pdf->rapport_TRANS_decimaal);
				else
          $verkoopKostenTxt='';
        $aankoopKostenTxt='';
			}
        ...
        $verkoopKostenTxt='';
        if($kosten<>0)
          $aankoopKostenTxt=$this->formatGetal(      $kosten,$this->pdf->rapport_TRANS_decimaal);
        else
          $aankoopKostenTxt='';
			}
		...
		$totaal1 = $this->pdf->marge + $this->pdf->widthA[0] + $this->pdf->widthA[1] + $this->pdf->widthA[2] + $this->pdf->widthA[3] + $this->pdf->widthA[4] + $this->pdf->widthA[5] + $this->pdf->widthA[6]  ;
		$totaal2 = $totaal1 + $this->pdf->widthA[7] + $this->pdf->widthA[8] + $this->pdf->widthA[9] + $this->pdf->widthA[10] ;
		$totaal3 = $totaal2  + $this->pdf->widthA[11] + $this->pdf->widthA[12] + $this->pdf->widthA[13] + $this->pdf->widthA[14];
		...
		$this->pdf->line($totaal1 + 2, $this->pdf->GetY(), $totaal1 + $this->pdf->widthA[7],  $this->pdf->GetY());
		$this->pdf->line($totaal2 + 2, $this->pdf->GetY(), $totaal2 + $this->pdf->widthA[10],  $this->pdf->GetY());
		...
		$this->pdf->line($totaal1+2,$this->pdf->GetY()+1,$totaal1 + $this->pdf->widthA[7],$this->pdf->GetY()+1);
		$this->pdf->line($totaal2+2,$this->pdf->GetY()+1,$totaal2 + $this->pdf->widthA[10],$this->pdf->GetY()+1);
