...
$Log: RapportPERFG_L87.php,v $
Revision 1.6  2020/01/19 13:36:46  rvv
*** empty log message ***

Revision 1.5  2020/01/18 13:31:05  rvv
*** empty log message ***

Revision 1.4  2020/01/15 16:29:25  rvv
*** empty log message ***
...
class RapportPERFG_L87
{
	function RapportPERFG_L87($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
*/
    ...
    $lineStyle = array('width' => 0.6, 'cap' => 'round', 'join' => 'miter', 'dash' => 0, 'color' => $color);
    //listarray($data);
   // $color=array(200,0,0);
   
   $aantal=count($data);
   
   $percentageExtraYEerste=0;
   $percentageExtraYTweede=0;

   if($data1[count($data1)-1] < $data[count($data)-1])
     $percentageExtraYTweede=5;
   else
     $percentageExtraYEerste=5;
    ...
    for ($i=0; $i<$aantal; $i++)
    {
     ...
     // $this->pdf->Rect($XDiag+($i+1)*$unit-0.5+$extrax, $yval2-0.5, 1, 1 ,'F','',$color);
      ...
      if($data[$i] <> 0 && $i==$aantal-1)
        $this->pdf->Text($XDiag+($i+1)*$unit-1+$extrax,$yval2-2+$percentageExtraYEerste,$this->formatGetal($data[$i],1).'%');
    ...
    if(is_array($data1))
    {
     // listarray($data1);
      $yval=$YDiag + (($maxVal) * $waardeCorrectie) ;
      $lineStyle = array('width' => 0.6, 'cap' => 'round', 'join' => 'miter', 'dash' => 0, 'color' => $color1);
      for ($i=0; $i<count($data1); $i++)
      {
       ...
       // $this->pdf->Rect($XDiag+($i+1)*$unit-0.5, $yval2-0.5, 1, 1 ,'F','',$color1);
        if($i==$aantal-1)
        $this->pdf->Text($XDiag+($i+1)*$unit-1,$yval2-2+$percentageExtraYTweede,$this->formatGetal($data1[$i],1).'%');
