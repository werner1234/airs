...
$Log: RapportVOLK_L87.php,v $
Revision 1.2  2020/01/19 13:36:46  rvv
*** empty log message ***
...
class RapportVOLK_L87
{
	function RapportVOLK_L87($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    //  ".
		...
		while($categorien = $DB->NextRecord())
		{
           ...
           $procentResultaat = (($totaalactueel - $totaalbegin + $totaaldividendCorrected) / ($totaalbegin /100));
			...
			$subquery = "SELECT TijdelijkeRapportage.fondsOmschrijving, TijdelijkeRapportage.beleggingssectorOmschrijving,".
			...
			//	" TijdelijkeRapportage.beginPortefeuilleWaardeEuro / $beginQuery as beginPortefeuilleWaardeEuro, ".
				...
				.$__appvar['TijdelijkeRapportageMaakUniek'].
				" ORDER BY TijdelijkeRapportage.beleggingssectorVolgorde, TijdelijkeRapportage.Lossingsdatum, TijdelijkeRapportage.fondsOmschrijving asc";//exit;
...
//		    if($n>$regels-2 && $this->pdf->GetY()>160)//
 ...
 //       }
        ...
        if($subdata['beleggingssectorOmschrijving']<> $lastSector)
				{
          $this->printKop(vertaalTekst($subdata['beleggingssectorOmschrijving'],$this->pdf->rapport_taal), "");
          $lastSector=$subdata['beleggingssectorOmschrijving'];
				}
        ...
        //echo "<br>\n".$subdata['fonds']."<br>\n";
				//echo "$fondsResultaat = (".$subdata['actuelePortefeuilleWaardeInValuta']." - ".$subdata['beginPortefeuilleWaardeInValuta'].") * ".$subdata['actueleValuta']." / ".$this->pdf->ValutaKoersEind."<br>\n";
        //echo "$valutaResultaat = ".$subdata['actuelePortefeuilleWaardeEuro']." - ".$subdata['beginPortefeuilleWaardeEuro']." - $fondsResultaat <br>\n";
