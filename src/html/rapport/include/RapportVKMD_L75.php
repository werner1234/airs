...
$Log: RapportVKMD_L75.php,v $
Revision 1.17  2020/07/26 06:50:14  rvv
*** empty log message ***

Revision 1.16  2020/07/25 15:36:13  rvv
*** empty log message ***

Revision 1.15  2020/06/03 15:41:22  rvv
*** empty log message ***

Revision 1.14  2019/05/25 16:22:07  rvv
*** empty log message ***

Revision 1.13  2019/04/13 17:42:49  rvv
*** empty log message ***

Revision 1.12  2019/04/10 15:50:36  rvv
*** empty log message ***

Revision 1.11  2019/02/03 15:50:24  rvv
*** empty log message ***

Revision 1.10  2019/01/16 08:41:44  rvv
*** empty log message ***

Revision 1.9  2018/10/31 17:23:34  rvv
*** empty log message ***

Revision 1.8  2018/06/30 17:43:55  rvv
*** empty log message ***

Revision 1.7  2018/06/23 14:21:39  rvv
*** empty log message ***
...
Revision 1.4  2018/06/10 11:45:56  rvv
*** empty log message ***
...
Revision 1.1  2018/05/19 16:24:53  rvv
*** empty log message ***
...
class RapportVKMD_L75
{
	function RapportVKMD_L75($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
			...
			$this->pdf->rapport_titel = vertaalTekst("Kosten-analyse, gedetailleerd",$this->pdf->rapport_taal);
			$this->pdfVullen=true;
		...
		if($this->pdf->portefeuilledata['startdatumMeerjarenrendement']<>'0000-00-00' && db2jul($this->pdf->portefeuilledata['startdatumMeerjarenrendement']) > db2jul(($this->rapport_jaar-1).date('-m-d',$this->rapport_datum)) )
			$this->vanafDatum=$this->pdf->portefeuilledata['startdatumMeerjarenrendement'];
		else
		...
		if($portefeuilleStartJul>$this->vanafJul)
		{
			...
			$this->melding= vertaalTekst("* Door onvoldoende historie bedraagt de rapportage periode van de directe kosten",$this->pdf->rapport_taal)." ".round($dagen)." ".vertaalTekst("dagen",$this->pdf->rapport_taal).".";
		}
	...
	{
		$DB=new DB();
		$query="SHOW fields FROM CRM_naw like 'KostenValyoux'";

		if($DB->QRecords($query) > 0)
		{
      $query="SELECT KostenValyoux FROM CRM_naw WHERE portefeuille='".$this->portefeuille."'";
			$DB->SQL($query);
			$DB->Query();
			$KostenValyoux=$DB->nextRecord();
			$KostenValyoux=$KostenValyoux['KostenValyoux'];
		}
		else
			$KostenValyoux=0;
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    if($this->pdfVullen==true)
		{
			...
			$this->pdf->row(array(vertaalTekst('Indirecte kosten, afgelopen 12 maanden',$this->pdf->rapport_taal)));
			$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
      $this->pdf->row(array(vertaalTekst('Gemiddeld vermogen beleggingsfondsen',$this->pdf->rapport_taal), $this->formatGetal($perfTotaal['gemWaarde'], 0) . ' EUR'));
	  	$this->pdf->row(array(vertaalTekst('Kosten van beleggingsfondsen (absoluut)',$this->pdf->rapport_taal), $this->formatGetal($totaalDoorlopendekosten, 0) . ' EUR'));
			$this->pdf->excelData[]=array('','Doorlopende kosten', '','',round($totaalDoorlopendekosten, 0), 'EUR');
			$this->pdf->row(array(vertaalTekst('Kosten van beleggingsfondsen (in %)',$this->pdf->rapport_taal), $this->formatGetal($doorlopendeKostenPercentage * 100, 2) . ' %'));
		...
		if($this->pdfVullen==true)
		{
			...
			//$this->pdf->ln();
			$this->pdf->excelData[]=array();
			$this->pdf->row(array(vertaalTekst('Totaal gemiddeld vermogen',$this->pdf->rapport_taal), $this->formatGetal($gemiddelde, 0) . ' EUR'));
			$this->pdf->excelData[]=array('','Gemiddeld vermogen','','',round($gemiddelde,0),'EUR');
			$this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
			$this->pdf->row(array(vertaalTekst('Indirecte kosten factor van de beleggingsfondsen',$this->pdf->rapport_taal), $this->formatGetal($vkmPercentagePortefeuille, 2) . ' %'));
			$barData=array();
			$barData['Kosten van beleggingsfondsen']=$vkmPercentagePortefeuille;
      ...
      $this->pdf->excelData[] = array('', 'Directe kosten vanaf ' . date('d-m-Y', db2jul($this->vanafDatum)), '', 'EUR', 'Percentage');
		...
		while($data = $DB->nextRecord())
		{
			if($this->pdfVullen==true)
			{
        if($data['Omschrijving']=='Belasting toegevoegde waarde')
          $data['Omschrijving']='BTW';
		...
		if($KostenValyoux>0)
		{
			$kostenProcent=$KostenValyoux/$gemiddelde*100;
			$barData['Kosten Valyoux']=$kostenProcent;
		}
    ...
    if($spreadKostenEUR <> 0)
		...
		if($this->pdfVullen==true)
		{
			...
			$this->pdf->row(array(vertaalTekst('Totale kosten',$this->pdf->rapport_taal),$this->formatGetal($vkmWaarde*$gemiddelde/100, 0), $this->formatGetal($vkmWaarde, 2).' %'));
			$this->pdf->excelData[]=array('','Jaarlijkse kosten Valyoux','', round($vkmWaarde*$gemiddelde/100,2),round($vkmWaarde, 2));

			$this->pdf->ln();
			if($KostenValyoux>0)
			{
				$this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
				$valyouVKM = $KostenValyoux / $gemiddelde * 100;
				$this->pdf->row(array(vertaalTekst('Kosten Valyoux', $this->pdf->rapport_taal), $this->formatGetal($KostenValyoux, 0), $this->formatGetal($valyouVKM, 2) . ' %'));
				$this->pdf->ln();
				$vkmWaarde += $valyouVKM;
				$this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
				$this->pdf->row(array(vertaalTekst('Totale kosten inclusief kosten Valyoux', $this->pdf->rapport_taal), $this->formatGetal($vkmWaarde * $gemiddelde / 100, 0), $this->formatGetal($vkmWaarde, 2) . ' %'));
				$this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
			}
...
foreach ($perCategorie as $hoofdcategorie=>$categorieData)
{
	...
	if($this->pdfVullen==true)
    $this->printSubTotaal($perHoofdcategorie[$hoofdcategorie]['omschrijving'],$perHoofdcategorie[$hoofdcategorie],'I');
 ...
 $perfTotaal['percentageIndirectVermogenMetKostenfactor']=$percentageIndirectVermogenMetKostenfactor;
		...
		if ($this->pdfVullen == true)
		{
			$this->printSubTotaal('Totaal', array('perf' => $perfTotaal), 'I');
      ...
      $key='Kosten van beleggingsfondsen';
      if($gewensteKleuren[$key]['R']['value'] || $gewensteKleuren[$key]['G']['value'] || $gewensteKleuren[$key]['B']['value'])
        ...
        $grootboekKleuren['Kosten van beleggingsfondsen']=$mogelijkeKleuren[$n];
 ...
 WHERE Portefeuilles.portefeuille='".$this->portefeuille."'";
    ...
    for($i=$this->pdf->rapport_jaar; $i<=$doelJaar; $i++)
		...
		$waardeZonderKostenKleur=$this->pdf->rapport_grafiek_pcolor;
		$waardeNaKostenKleur=$this->pdf->rapport_grafiek_drie;
		$cumulatieveKostenKleur=$this->pdf->rapport_grafiek_icolor;

		if($this->pdf->getY()+70>$this->pdf->pagebreak)
			$this->pdf->addPage();

		$this->pdf->setXY(30,$this->pdf->getY()+14);
