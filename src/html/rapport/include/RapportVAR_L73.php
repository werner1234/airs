...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2017/10/09 05:29:19 $
File Versie					: $Revision: 1.7 $

$Log: RapportVAR_L73.php,v $
Revision 1.7  2017/10/09 05:29:19  rvv
*** empty log message ***

Revision 1.6  2017/10/07 16:54:34  rvv
*** empty log message ***

Revision 1.5  2017/08/26 17:37:43  rvv
*** empty log message ***

Revision 1.4  2017/07/01 17:03:24  rvv
*** empty log message ***

Revision 1.3  2017/06/07 16:28:08  rvv
*** empty log message ***
...
class RapportVAR_L73
{
	function RapportVAR_L73($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function formatGetalKoers($waarde, $dec , $start = false)
	...
	function writeRapport()
	{
		  ...
		  $this->vastWhere="AND TijdelijkeRapportage.beleggingscategorie='OBL' AND ( TijdelijkeRapportage.hoofdcategorie='G-RISM' OR Fondsen.Lossingsdatum <> '0000-00-00') AND TijdelijkeRapportage.Type <> 'rekening'";
...
//echo "<tr><td>". $data['fonds'] . "</td><td> $p </td><td> $r </td><td> $b </td><td>$y  </td><td>  $ytm </td>  <td> ".$data['Lossingsdatum']."</td> </tr>\n";
         ...
         //  {
          //   $modifiedDuration = ($renteDag - db2jul($this->rapportageDatum)) / 86400 / 365;
            // echo $data['fonds']." ".date('Y-m-d',$renteDag)." $modifiedDuration = ($renteDag - db2jul(".$this->rapportageDatum.")) / 86400 / 365 <br>\n";
         //  }
	        // else
	           $modifiedDuration=$duration/(1+$ytm/100);

 //      echo $data['fonds']." $modifiedDuration=$duration/(1+$ytm/100);<br>\n";
/*
if($data['variabeleCoupon'] )
{
 echo $data['variabeleCoupon'] ." ". date("d-m-Y",$renteDag)."->". date("d-m-Y",$rapportJul) ." ".$data['renteperiode']."<br>\n";

          echo $data['variabeleCoupon'] ."|".round($modifiedDuration,4)." ".$data['fonds']."<br>\n";
}
*/
	         $aandeel=$data['actuelePortefeuilleWaardeEuro']/$portefeuilleWaarde;
