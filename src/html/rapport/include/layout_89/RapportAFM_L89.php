...
class RapportAFM_L89
{
	function RapportAFM_L89($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	  global $__appvar;
		$this->pdf = &$pdf;
  ...
  function formatGetalKoers($waarde, $dec , $start = false)
    ...
    if($this->pdf->debug)
      listarray($afm);
		...
		//listarray($this->pdf->pieData);listarray($kleurdata);
		$omschrijvingen=array();
		$percentage=array();
		$kleur=array();
		foreach($this->pdf->pieData as $categorie=>$p)
		{
			$omschrijvingen[]=$categorie;
			$percentage[]=$p;
			$tmpKleur=array($kleurdata[$categorie]['kleur']['R']['value'],$kleurdata[$categorie]['kleur']['G']['value'],$kleurdata[$categorie]['kleur']['B']['value']);
			if ($tmpKleur[0]==0 && $tmpKleur[1]==0 && $tmpKleur[2]==0)
				$tmpKleur=array(rand(1,255),rand(1,255),rand(1,255));
			$kleur[]=$tmpKleur;
		}
	//	listarray($percentage);exit;
		$y=75;
		$this->pdf->set3dLabels($omschrijvingen,243,$y-5,$kleur,-55,-5);
		$this->pdf->Pie3D($percentage,$kleur,233,$y+5,28,20,6,"AFM-standaarddeviatie");//
