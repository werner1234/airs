...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2020/05/16 15:57:45 $
File Versie					: $Revision: 1.4 $

$Log: RapportVOLK_L89.php,v $
Revision 1.4  2020/05/16 15:57:45  rvv
*** empty log message ***

Revision 1.3  2020/05/13 15:37:13  rvv
*** empty log message ***
...
class RapportVOLK_L89
{
	function RapportVOLK_L89($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//	$this->printSubTotaal("Subtotaal:" A'subtotaalbegin' B'subtotaalactueel' C'percentageVanTotaal' D'fondsResultaat' E'valutaResultaat' F$procentResultaat);
	function printSubTotaal($title, $totaalA, $totaalB, $totaalC, $totaalD, $totaalE, $totaalF, $TotaalG = 0, $totaalH = 0)
	{
		...
		*/
		if(($this->pdf->GetY() + $hoogte) > $this->pdf->pagebreak)
		{
			...
			if($totaalE <>0)
				$this->printCol(10,$this->formatGetal($totaalE,$this->pdf->rapport_VOLK_decimaal),"subtotaal");
      if($TotaalG <>0)
        $this->printCol(11,$this->formatGetal($TotaalG,$this->pdf->rapport_VOLK_decimaal),"subtotaal");
			if($totaalF <>0)
	...
	function printTotaal($title, $totaalA, $totaalB, $totaalC, $totaalD, $totaalE, $totaalF = 0, $grandtotaal=false, $totaalG = 0, $totaalH = 0 )
	{
		...
		if($grandtotaal == true)
			...
			if($totaalE <>0)
				$this->printCol(10,$this->formatGetal($totaalE,$this->pdf->rapport_VOLK_decimaal),$grandtotaal);
      if($totaalG <>0)
        $this->printCol(11,$this->formatGetal($totaalG,$this->pdf->rapport_VOLK_decimaal),$grandtotaal);
			if($totaalF <>0)
  ...
  function getDividend($fonds)
  {
	...
	function writeRapport()
	{
		...
		$query = "SELECT Clienten.Naam, Clienten.Naam1, Portefeuilles.Depotbank, Portefeuilles.AEXVergelijking, Portefeuilles.SpecifiekeIndex, Portefeuilles.Vermogensbeheerder, Portefeuilles.ClientVermogensbeheerder, Portefeuilles.Risicoklasse, Portefeuilles.Client, Portefeuilles.memo FROM Portefeuilles, Clienten WHERE Portefeuille = '".$this->portefeuille."' AND Portefeuilles.Client = Clienten.Client ";
		$DB = new DB();
		$DB->SQL($query);
		$DB->Query();
		$this->portefeuilledata = $DB->nextRecord();
			...
			$this->pdf->widthB = array(22,62,18,21,21,5,21,21,5,22,21,21,21,13);
...
//echo $subquery."<br><br>";exit;
    ...
    if ( ! empty ($this->portefeuilledata['memo']) ) {
      $this->pdf->ln(15);
      $this->pdf->SetWidths(array(22,180));
      $this->pdf->SetFont($this->pdf->rapport_font,'bi',$this->pdf->rapport_fontsize);
      $this->pdf->SetTextColor($this->pdf->rapport_kop3_fontcolor['r'],$this->pdf->rapport_kop3_fontcolor['g'],$this->pdf->rapport_kop3_fontcolor['b']);
      $this->pdf->row(array("",'Bijzonderheden'));
      $this->pdf->ln(1);
      $this->pdf->SetTextColor($this->pdf->rapport_fontcolor['r'],$this->pdf->rapport_fontcolor['g'],$this->pdf->rapport_fontcolor['b']);
      $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
      $this->pdf->row(array("",$this->portefeuilledata['memo']));
    }
