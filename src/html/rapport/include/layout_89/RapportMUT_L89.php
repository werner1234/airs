...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2020/06/10 15:35:05 $
File Versie					: $Revision: 1.3 $

$Log: RapportMUT_L89.php,v $
Revision 1.3  2020/06/10 15:35:05  rvv
*** empty log message ***
...
class RapportMUT_L89
{
	function RapportMUT_L89($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	...
	//		$this->printCol(4,$this->formatGetal($totaalC,$this->pdf->rapport_MUT2_decimaal),$totType);
		if($totaalD <>0)
			$this->printCol(4,$this->formatGetal($totaalD,$this->pdf->rapport_MUT2_decimaal),$totType);
		if($totaalE <>0)
			$this->printCol(5,$this->formatGetal($totaalE,$this->pdf->rapport_MUT2_decimaal),$totType);
		if($totaalF <>0)
			$this->printCol(6,$this->formatGetal($totaalF,$this->pdf->rapport_MUT2_decimaal),$totType);
        ...
        $mutaties['gbOmschrijving']='Servicekosten Bank';
			...
			if($mutaties['Grootboekrekening'] == "KNBA" || $mutaties['Grootboekrekening'] == "DIVBE")
			{
				...
				$query = "SELECT ".
					...
					"AND Rekeningmutaties.bankTransactieId = '".mysql_escape_string($mutaties['bankTransactieId'])."' ".
					"AND Rekeningmutaties.Verwerkt = '1' ".
			...
			if(!$skip)
			{
				...
				else if($mutaties['Grootboekrekening'] == "DIVBE")
				{
					...
					$knba['Debet']	= 0;
         	$divbe['Credit']=0;
          $divbe['Debet'] = (($mutaties['Credit']-$mutaties['Debet']) * $mutaties['Valutakoers'])*-1;// $credit * -1;
					$netto = 0;
					$kost = 0;
					$credit = 0;
					$credittxt = "";
				...
				$subknba += $knba['Debet'];
				$subkost += $kost;
				$subbel += $divbe['Debet']-$divbe['Credit'];
				$subnetto += $netto;
				...
				$totaalkost += $kost;
				$totaalbel += $divbe[Debet]-$divbe['Credit'];
				$totaalnetto += $netto;
