...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: RapportVAR_L89.php,v $
 		Revision 1.4  2020/07/11 17:31:58  rvv
 		*** empty log message ***
...
class RapportVAR_L89
{
	function RapportVAR_L89($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	 //
		$this->pdf = &$pdf;
    ...
    $this->pdf->templateVarsOmschrijving[$this->pdf->rapport_type.'Paginas']='Rendement & Risicokenmerken 2';//trim(str_replace("\n",'. ',$this->pdf->rapport_titel));
...
GROUP BY TijdelijkeRapportage.Beleggingssector ORDER BY beleggingssectorVolgorde desc ";
  ...
  function vastrentendeDeel()
  {
    ...
    $this->pdf->Rect($this->pdf->marge, $this->pdf->getY(), $this->pdf->w-$this->pdf->marge*2, 12 , 'F');
