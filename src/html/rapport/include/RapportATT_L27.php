...
$Log: RapportATT_L27.php,v $
Revision 1.18  2018/12/08 18:28:30  rvv
*** empty log message ***

Revision 1.17  2018/12/05 16:36:17  rvv
*** empty log message ***

Revision 1.16  2016/04/13 11:19:32  rvv
*** empty log message ***
...
class RapportATT_L27
{
	function RapportATT_L27($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    $this->categorieKleuren=$allekleuren['OIB'];
	 // $this->categorieOmschrijving=array('LIQ'=>'Liquiditeiten','ZAK'=>'Zakelijke waarden','VAR'=>'Vastrentende waarden','Liquiditeiten'=>'Liquiditeiten');
		...
		$q="SELECT Beleggingscategorie,Omschrijving,Afdrukvolgorde FROM Beleggingscategorien WHERE Beleggingscategorie IN('LIQ','ZAK','VAR','Liquiditeiten') ORDER BY Afdrukvolgorde desc";
		$DB->SQL($q);
		$DB->Query();
...
//listarray($this->categorieVolgorde);
  ...
  foreach ($indexData as $index=>$maanden)
  {
    foreach ($indexDataReal as $realData)
    {
      ...
      {
        if(isset($realData['extra']['cat']['Liquiditeiten'])&&isset($realData['extra']['cat']['Liquiditeiten']))
        {
          $realData['extra']['cat']['LIQ']+=$realData['extra']['cat']['Liquiditeiten'];
          unset($realData['extra']['cat']['Liquiditeiten']);
        }  
   ...
   if(count($rendamentWaarden) > 0)
   {
     ...
     //   $this->pdf->CellBorders = array('','US','US','US','US','US','US','US','US','US','US','US');
       ...
       //$this->pdf->SetFillColor(230,230,230);
        ...
        $this->LineDiagram(270, 25, $grafiekData,$this->pdf->rapport_grafiek_color,0,0,6,5,1);//50
        $this->pdf->SetXY($valX, $valY + 80);
		  }
		  $this->pdf->SetXY(8, 155);//165
    
    $huidigeJaar=substr($this->rapportageDatum,0,4);
    $begin=($huidigeJaar-5).'-01-01';
    $eind=($huidigeJaar-1).'-12-31';
    if(db2jul($this->pdf->PortefeuilleStartdatum) > db2jul($begin))
      $begin=substr($this->pdf->PortefeuilleStartdatum,0,10);
    
    if(db2jul($this->pdf->PortefeuilleStartdatum) > db2jul($eind))
      $eind=substr($this->pdf->PortefeuilleStartdatum,0,10);
    
  //  echo $this->rapportageDatumVanaf  ." ".$this->rapportageDatum." | $begin -> $eind";
    $this->pdf->fillCell = array();
    if(db2jul($eind) > db2jul($begin))
    {
 
      $index=new indexHerberekening();
      $jaarRendementen=array();
      $headerRow=array('Jaar rendement');
      $rendementRow=array('');
      $rendementen = $index->getWaarden($begin, $eind, $this->portefeuille);
      foreach($rendementen as $maandData)
      {
        $jaar=substr($maandData['datum'],0,4);
        if(!isset($jaarRendementen[$jaar]))
          $jaarRendementen[$jaar]=0;
        $jaarRendementen[$jaar]=((1+($jaarRendementen[$jaar]/100))*(1+($maandData['performance']/100))-1)*100;
      }
      foreach($jaarRendementen as $jaar=>$rendement)
      {
        $headerRow[]=$jaar;
        $rendementRow[]=$this->formatGetal($rendement,2)."%";
      }
      $headerRow[]='';
      $this->pdf->SetXY($this->pdf->marge,185 );
      $this->pdf->setWidths(array(50,30,30,30,30,30,81));
...
*/
   ...
   // $color=array(200,0,0);
    ...
    {
      $this->pdf->TextWithRotation($XDiag+($i)*$unit-10+$unit,$YDiag+$hDiag,date("d-M-Y",db2jul($legendDatum[$i])),0);
