...
$Log: RapportVKMS_L20.php,v $
Revision 1.4  2019/07/03 15:37:22  rvv
*** empty log message ***

Revision 1.3  2019/02/03 15:50:24  rvv
*** empty log message ***

Revision 1.2  2019/01/16 08:41:44  rvv
*** empty log message ***
...
class RapportVKMS_L20
{
	function RapportVKMS_L20($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
...
FROM Rekeningmutaties
JOIN Rekeningen ON Rekeningmutaties.Rekening=Rekeningen.Rekening
JOIN Grootboekrekeningen ON Rekeningmutaties.Grootboekrekening=Grootboekrekeningen.Grootboekrekening AND (Grootboekrekeningen.Kosten=1 OR Grootboekrekeningen.Grootboekrekening IN('BEH','BEW'))
WHERE Rekeningen.Portefeuille='".$this->portefeuille."' AND Rekeningmutaties.Boekdatum>'".$this->vanafDatum."' AND Rekeningmutaties.Boekdatum<='".$this->rapportageDatum."'
GROUP BY Rekeningmutaties.Grootboekrekening
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
      ...
      $startY=$this->pdf->getY();
		...
		while($data = $DB->nextRecord())
		{
			if($this->pdfVullen==true)
			{
				...
				$barData['Directe kosten']=$kostenProcent;
			}
			$totaal+=$data['totaal'];
		}
    ...
    $barData['Directe kosten'] = $kostenPercentage;
    ...
    if($this->pdfVullen==true)
    {
      ...
      $this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
      $this->pdf->row(array(vertaalTekst('Gewogen gemiddelde kosten van de beleggingen',$this->pdf->rapport_taal), $this->formatGetal($totaalDoorlopendekosten, 0), $this->formatGetal($vkmPercentagePortefeuille, 2) . ' %'));
      $this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
      ...
      $this->pdf->excelData[]=array();
		...
		if($this->pdfVullen==true)
		{
			...
			$this->pdf->row(array(vertaalTekst('Totaal directe en indirecte kosten',$this->pdf->rapport_taal),$this->formatGetal($vkmWaarde*$gemiddelde/100, 0), $this->formatGetal($vkmWaarde, 2).' %'));
			$this->pdf->excelData[]=array('','Vergelijkende kostenmaatstaf','', round($vkmWaarde*$gemiddelde/100,2),round($vkmWaarde, 2));
			$this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
      
      $this->pdf->ln();
      $this->pdf->row(array(vertaalTekst('Berekend over het gemiddeld vermogen:',$this->pdf->rapport_taal), $this->formatGetal($gemiddelde, 0)));
...
foreach ($perCategorie as $hoofdcategorie=>$categorieData)
{
 ...
 $perfTotaal['percentageIndirectVermogenMetKostenfactor']=$percentageIndirectVermogenMetKostenfactor;
		...
		if($this->skipSummary==false)
		{
      $intro="Hieronder is een overzicht te vinden van de directe kosten van Wolkers & Co en de depotbank over de verslagperiode. Daaronder zijn de indirecte kosten te vinden. Dit laatste bestaat onder meer uit management en performance fees die beheerders van fondsen in rekening brengen, alsmede transactiekosten en overige kosten die door deze fondsen worden gedragen. De percentages voor zowel de directe als indirecte kosten zijn berekend over het gemiddelde vermogen gedurende de verslagperiode.
De verwachte kosten voor het lopende jaar zullen in lijn liggen met de kosten over het afgelopen jaar.
Wij hebben vastgesteld dat er geen wijzigingen in de vermogenspositie of de risicobereidheid zijn die een aanpassing van het beleid noodzakelijk zou maken.";
      $this->pdf->MultiCell(297-$this->pdf->marge*2,4,$intro,0,'L');
 ...
 WHERE Portefeuilles.portefeuille='".$this->portefeuille."'";
    ...
    $this->pdf->setY(130);
    $txt="Ter illustratie is hiernaast grafisch weergegeven wat het effect van de hierboven
genoemde kosten is over meerdere jaren. De grafiek laat de ontwikkeling van de
portefeuille zien in de komende 10 jaar, indien de jaarlijkse kosten over al die jaren
identiek zou zijn aan de kosten over de afgelopen verslagperiode (groene lijn), inclusief
de hypothetische ontwikkeling indien de portefeuille hetzelfde rendeert maar zonder
deze kosten (blauwe lijn).";
    
    $this->pdf->MultiCell(140,4,$txt,0,'L');
...
//		echo "$doelJaar $rendement $beginwaarde";
	...
	//	$maxVal=round($maxVal,floor(log10($maxVal))*-1+1);
		...
		foreach ($legendaItems as $index=>$item)
		{
			...
			$this->pdf->Rect($XPage-70+5 , $YPage+$step+30, 3, 3, 'DF','',$kleur);
			$this->pdf->SetXY($XPage-70+3+5,$YPage+$step+30);
  ...
  function fondsPerf($fonds,$van,$tot)
  {
	...
	function VBarVerdeling($w, $h, $data)
	{
		...
		$grafiekPunt = array();
			...
			$minVal=0;
    $this->grootboekKleuren['Indirecte kosten']=array(218,202,188);
    $this->grootboekKleuren['Directe kosten']=array(173,141,111);
