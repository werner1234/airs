...
class RapportSCENARIO_L61
{
	function RapportSCENARIO_L61($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
   ...
   foreach($data as $reeks=>$waarden)
   {
     ...
     if($reeks==$beste['scenario'])
     {
      // $this->pdf->MemImage(base64_decode($this->vuurtorenIMG), $XDiag + $waarden['x'] * $Xunit - 1.0, $bodem + $waarden['y'] * $Yunit - 8.3, 2);
       ...
       $this->pdf->Circle( $XDiag + $waarden['x'] * $Xunit, $bodem + $waarden['y'] * $Yunit,5,0,360,'D',$circleStyle,array(0,0,0));
     }
