...
$Log: RapportGRAFIEK_L52.php,v $
Revision 1.5  2019/05/04 18:22:49  rvv
*** empty log message ***

Revision 1.4  2019/05/01 15:53:25  rvv
*** empty log message ***

Revision 1.3  2019/04/27 18:28:16  rvv
*** empty log message ***

Revision 1.2  2019/04/08 06:05:33  rvv
*** empty log message ***
...
include_once("rapport/include/ATTberekening_L52.php");


class RapportGRAFIEK_L52
{
  function RapportGRAFIEK_L52($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
  {
    $this->pdf = &$pdf;
  ...
  function writeRapport()
  {
   ...
   // $DB = new DB();
    ...
    $att=new ATTberekening_L52($this);
      ...
      */
      ...
      foreach ($categorieData['perfWaarden'] as $datum=>$waarden)
      {
        ...
        if($categorie!='totaal')
        {
          ...
          //echo " $selectieEffect=(".$waarden['procentBruto']."-".$waarden['indexPerf'].")*".$waarden['indexBijdrageWaarde']."*100;<br>\n";
...
//echo "$categorie <br>\n";
  ...
  function LineDiagram($x,$y,$w, $h, $data, $title,$color=null, $maxVal=0, $minVal=0, $horDiv=8, $verDiv=4,$jaar=0)
  {
    global $__appvar;
    ...
    $legendaStep=1;
    if($aantalMaanden>12)
      $legendaStep=ceil($aantalMaanden/12);
    ...
    foreach($data as $type=>$maandData)
    {
      ...
      //  $this->pdf->TextWithRotation($XDiag+($i)*$unit-5+$unit,$YDiag+$hDiag+8,$legendDatum[$i],0);
        ...
        if(!isset($maandPrinted[$maand]))
        {
          ...
          if($i%$legendaStep==0)
            $this->pdf->Text($XDiag+($i+1)*$unit-3,$bodem+5,date('M-y',db2jul($maand)));
  ...
  function VBarDiagram2($w, $h, $data, $format, $color=null,$nbDiv=4,$numBars=0)
  {
    ...
    foreach($data as $maand=>$maandData)
    {
      ...
      if(($i/4)%$legendaStep==0)
        $this->pdf->Text($XstartGrafiek + ($i -2) * $vBar - $eBaton / 2,$YstartGrafiek +3 ,date('M-y',db2jul($maand)));
