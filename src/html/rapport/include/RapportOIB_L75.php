...
$Log: RapportOIB_L75.php,v $
Revision 1.20  2020/01/18 16:37:36  rvv
*** empty log message ***

Revision 1.19  2020/01/18 13:30:29  rvv
*** empty log message ***

Revision 1.18  2019/10/26 16:07:18  rvv
*** empty log message ***

Revision 1.17  2019/09/28 17:20:17  rvv
*** empty log message ***

Revision 1.16  2019/09/21 16:31:25  rvv
*** empty log message ***

Revision 1.15  2019/05/25 16:22:07  rvv
*** empty log message ***

Revision 1.14  2018/09/12 14:50:18  rvv
*** empty log message ***

Revision 1.13  2018/08/18 12:40:14  rvv
php 5.6 & consolidatie

Revision 1.12  2018/06/30 17:43:55  rvv
*** empty log message ***

Revision 1.11  2018/06/23 14:21:39  rvv
*** empty log message ***

Revision 1.10  2018/06/16 17:42:56  rvv
*** empty log message ***

Revision 1.9  2018/06/10 11:45:56  rvv
*** empty log message ***

Revision 1.8  2018/06/09 15:58:54  rvv
*** empty log message ***

Revision 1.7  2018/05/30 16:10:58  rvv
*** empty log message ***
...
class RapportOIB_L75
{
	function RapportOIB_L75($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		$this->pdf->rapport_type = "OIB";
		$this->pdf->rapport_datum = db2jul($rapportageDatum);
		$this->pdf->rapport_titel = "Resultaten-analyse lopend jaar";
	...
	function writeRapport()
	{
		...
		{
			if($this->pdf->getY()+35>115)
				$startY=$this->pdf->getY()+35;
			else
				$startY=115;
			$this->portefeuillePerf($startY);
		}
    ...
    $waardenOpDatum=array();
		$portefeuilleNaamConversie=array();
		foreach($this->pdf->portefeuilles as $portefeuille)
		{

			$index=new indexHerberekening();
			$indexData = $index->getWaarden($this->tweedePerformanceStart, $this->rapportageDatum,$portefeuille,'','maanden',$this->pdf->rapportageValuta);
			foreach($indexData as $indexDetails)
			{
				$waardenOpDatum['portefeuilles'][$portefeuille][$indexDetails['datum']]['waarde']=$indexDetails['waardeHuidige'];
				$waardenOpDatum['portefeuilles'][$portefeuille][$indexDetails['datum']]['performance']+=$indexDetails['performance'];
				$waardenOpDatum['portefeuilles'][$portefeuille][$indexDetails['datum']]['index']+=$indexDetails['index']-100;
				$waardenOpDatum['portefeuilles'][$portefeuille][$indexDetails['datum']]['resultaatVerslagperiode']+=$indexDetails['resultaatVerslagperiode'];
				$waardenOpDatum['totaleWaarde'][$indexDetails['datum']]+=$indexDetails['waardeHuidige'];
			}
			...
			$DB->SQL("SELECT kleurcode,ClientVermogensbeheerder,eindDatum FROM Portefeuilles WHERE portefeuille='".$portefeuille."'");
			...
			$portefeuilleNaamConversie[$portefeuille]=$portefeuilleNaam;

			$tmp=unserialize($kleur['kleurcode']);
			$portefeuilleWaarden[$portefeuilleNaam]['kleur']=array('R'=>array('value'=>$tmp[0]),'G'=>array('value'=>$tmp[1]),'B'=>array('value'=>$tmp[2]));
...
/*
			...
			{
				$portefeuilleWaarden[$portefeuilleNaam]['eind']+=$waarde['actuelePortefeuilleWaardeEuro'];
			...
			foreach($gegevens as $waarde)
			{
				$portefeuilleWaarden[$portefeuilleNaam]['begin']+=$waarde['actuelePortefeuilleWaardeEuro'];
			...
			$waardeMutatie=$portefeuilleWaarden[$portefeuilleNaam]['eind']-$portefeuilleWaarden[$portefeuilleNaam]['begin'];
			$resultaatVerslagperiode = $waardeMutatie - $stortingen + $onttrekkingen;
			//echo " $portefeuille 	$resultaatVerslagperiode = $waardeMutatie - $stortingen + $onttrekkingen; <br>\n"; ob_flush();
			
	//		$portefeuilleWaarden[$portefeuilleNaam]['rendement']=$rendementProcent;
	//		$portefeuilleWaarden[$portefeuilleNaam]['resultaat']=$resultaatVerslagperiode;
      */
      if(db2jul($kleur['eindDatum']) < $this->pdf->tweedePerformanceStart && round($portefeuilleWaarden[$portefeuilleNaam]['eind']) == 0)
      {
        unset($portefeuilleWaarden[$portefeuilleNaam]);
        continue;
      }
...
//listarray($portefeuilleWaarden);
		foreach($waardenOpDatum['portefeuilles'] as $portefeuille=>$portDetails)
		{
			$naam=$portefeuilleNaamConversie[$portefeuille];
        foreach($portDetails as $datum=>$details)
				{
					$maandAandeel = $details['waarde'] / $waardenOpDatum['totaleWaarde'][$datum];
					$maandRendementBijdrage = $maandAandeel * $details['performance'];
//$last=$portefeuilleWaarden[$naam]['bijdrage'];
					$portefeuilleWaarden[$naam]['bijdrage']=((1+$portefeuilleWaarden[$naam]['bijdrage']/100)*(1+$maandRendementBijdrage/100)-1)*100;
	//				echo "$portefeuille $maandAandeel $datum ".$portefeuilleWaarden[$naam]['bijdrage']."=(1+".$last.")*(1+$maandRendementBijdrage)-1 <br>\n";
					$portefeuilleWaarden[$naam]['aandeel']=$maandAandeel*100;
					$portefeuilleWaarden[$naam]['resultaat']+=$details['resultaatVerslagperiode'];
					//$portefeuilleWaarden[$naam]['bijdrage']+=$maandRendementBijdrage;
					$portefeuilleWaarden[$naam]['rendement']=$details['index'];
				}

		}

			//$portefeuilleWaarden[$naam]['rendement']=
			//$portefeuilleWaarden[$naam]['resultaat']+=$portDetails['resultaat'];
	//	listarray($waardenOpDatum);
	//	listarray($portefeuilleWaarden);
		//exit;
    ...
    if(count($portefeuilleWaarden)>5)
		...
		foreach($portefeuilleWaarden as $portefeuille=>$data)
		{
			...
			$this->pdf->row(array(vertaalTekst($portefeuille,$this->pdf->rapport_taal),
												$this->formatGetal($data['aandeel'], 2,false,$max),
												$this->formatGetal($data['resultaat'], 2,false,$max),
												$this->formatGetal($data['rendement'], 2,false,$max),
												$this->formatGetal($data['bijdrage'], 2,false,$max)));
			if($data['rendement']<0)
				$barGraph=true;
			$jaarTotalen['resultaat']+=$data['resultaat'];
			$jaarTotalen['aandeel']+=$data['aandeel'];

			$categorieVerdeling['percentage'][$portefeuille]=$data['rendement'];
			$categorieVerdeling['kleur'][]=array($data['kleur']['R']['value'],$data['kleur']['G']['value'],$data['kleur']['B']['value']);
			$categorieVerdeling['kleurBar'][$portefeuille]=array($data['kleur']['R']['value'],$data['kleur']['G']['value'],$data['kleur']['B']['value']);
		...
		$this->pdf->ln($witruimte);
		$this->pdf->row(array(vertaalTekst("Totaal",$this->pdf->rapport_taal),$this->formatGetal($jaarTotalen['aandeel'],2,false,$max),$this->formatGetal($jaarTotalen['resultaat'],2,false,$max),'',
											$this->formatGetal(	$this->ytdRendement,2,false,$max)));

		$grafiekY=$ystart;
		$xOffset=150;
		$this->pdf->setXY(50+$xOffset,$grafiekY+2);
		$this->BarDiagram(80, 100, $categorieVerdeling['percentage'], '',$categorieVerdeling['kleurBar'],'',$krimp);//"Portefeuillewaarde € ".$this->formatGetal($this->portTotaal[$this->rapportageDatum],2)
...
WHERE TijdelijkeRapportage.portefeuille='".$this->portefeuille."' AND TijdelijkeRapportage.rapportageDatum='".$this->rapportageDatum."'".
    ...
    //include_once($__appvar["basedir"]."/html/rapport/include/ATTberekening_L72.php");
    ...
    $this->waarden['Jaar']=$this->kostenCorrectie($this->waarden['Jaar']);
		...
		foreach ($bovencat as $categorie=>$categorieOmschrijving)
		{
			if($this->waarden['Jaar'][$categorie]['weging'] <> 0)
			{
				...
				$categorieVerdeling['kleurBar'][$categorieOmschrijving]=array($this->pdf->grafiekKleuren['OIB'][$categorie]['R']['value'],$this->pdf->grafiekKleuren['OIB'][$categorie]['G']['value'],$this->pdf->grafiekKleuren['OIB'][$categorie]['B']['value']);

			}
		}
    
    
   // echo $this->waarden['Jaar']['totaal']['bijdrage']."<br>\n";
		//echo $jaarTotalen['bijdrage']."<br>\n";
		//exit;
		$this->pdf->CellBorders=array('','TS','TS','','TS');
		$this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
	...
	//	exit;
		...
		$hBar = 8+$krimp;//floor($hDiag / ($this->pdf->NbVal + 1));
