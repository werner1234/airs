...
$Log: RapportATT_L13.php,v $
Revision 1.4  2019/01/12 17:08:31  rvv
*** empty log message ***

Revision 1.3  2018/12/24 08:20:14  rvv
*** empty log message ***

Revision 1.2  2018/12/23 08:53:45  rvv
*** empty log message ***
...
include_once("rapport/include/ATTberekening_L13.php");

class RapportATT_L13
{
  
  function RapportATT_L13($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
  {
    $this->pdf = &$pdf;
    ...
    $this->att=new ATTberekening_L13($this);
  ...
  function addResultaat()
  {
    ...
    $kosten=array("",vertaalTekst("Toegerekende kosten",$this->pdf->rapport_taal));
    ...
    foreach($categorien as $categorie=>$categorieOmschrijving)
    {
      ...
      else
      {
        ...
        $effectenmutaties[]='';
        $stortingen[]=$this->formatGetal($perfWaarden['onttrekking'],0);
        $stortingen[]='';
        $onttrekking[]=$this->formatGetal($perfWaarden['storting']*-1,0);
        $onttrekking[]='';
        
        if($categorie=='Liquiditeiten')
        {
          $opbrengsten[]=$this->formatGetal($perfWaarden['resultaat']-($perfWaarden['kosten']),0);//+$perfWaarden['extraKosten']
        }
        else
        {
       ...
       // listarray($perfWaarden);
        $opbrengsten[]='';
        $kosten[]=$this->formatGetal(($perfWaarden['kosten'])*-1,0);//+$perfWaarden['extraKosten']
        $kosten[]='';
