...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2017/07/01 11:24:54 $
File Versie					: $Revision: 1.10 $

$Log: RapportVHO_L7.php,v $
Revision 1.10  2017/07/01 11:24:54  rvv
*** empty log message ***

Revision 1.9  2017/06/25 14:50:15  rvv
*** empty log message ***

Revision 1.8  2016/07/06 16:09:31  rvv
*** empty log message ***

Revision 1.7  2016/05/15 17:15:00  rvv
*** empty log message ***

Revision 1.6  2016/03/27 17:35:07  rvv
*** empty log message ***

Revision 1.5  2014/10/29 16:47:20  rvv
*** empty log message ***

Revision 1.4  2014/10/25 14:39:09  rvv
*** empty log message ***
...
Revision 1.1  2013/07/07 08:14:49  rvv
*** empty log message ***
...
class RapportVHO_L7
{
	function RapportVHO_L7($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		...
		$this->pdf->excelData[]=array('Fondsomschrijving','Aantal','Gem KP in Val','KP Waarde in Val','KP Waarde in EUR','Per stuk in valuta','Portefeuille in valuta','Portefeuille in EUR','Fondsresultaat','Valutaresultaat');
	}
  ...
  function formatKoers($waarde, $dec)
	{
	  if($waarde > 10000)
      return number_format($waarde,0,",",".");
    else
	  	return number_format($waarde,$dec,",",".");
	}
...
//			$this->pdf->Cell($this->pdf->widthB[14],4,$totaalFtxt, 0,1, "R");
		...
		if($grandtotaal)
		{
				...
				if(!empty($totaalB))
				{
					...
					$this->pdf->Line($actueel+$extra,$this->pdf->GetY()+5,$actueel + $this->pdf->widthB[9],$this->pdf->GetY()+5);
				}
				if(!empty($totaalA))
				{
					$this->pdf->Line($begin+$extra,$this->pdf->GetY(),$begin + $this->pdf->widthB[5],$this->pdf->GetY());
	...
	function writeRapport()
	{
		...
		$DB = new DB();
		$this->pdf->widthB = array(5,65,20,18,22,22,10,18,22,22,10,20,2,20,2);
		$this->pdf->alignB = array('L','L','R','R','R','R','R','R','R','R','R','R','R','R','R');
		// voor kopjes
		$this->pdf->widthA = array(60,18,15,22,22,10,15,25,22,12,22,10,22,5);
    ...
    $this->pdf->templateVars['VHOPaginas']=$this->pdf->page;
		...
		" WHERE TijdelijkeRapportage.portefeuille = '".$this->portefeuille."' AND TijdelijkeRapportage.type IN('fondsen') AND TijdelijkeRapportage.rapportageDatum = '".$this->rapportageDatum."'" .$__appvar['TijdelijkeRapportageMaakUniek'].
      ...
      foreach($tmpData as $subdata)
        ...
        $this->pdf->AddPage();
          ...
          if($subdata['type']=='rente')
          {
            $this->pdf->row(array("","","","","","","","","",$this->formatGetal($subdata['actuelePortefeuilleWaardeEuro'],$this->pdf->rapport_VOLK_decimaal)));
						$this->pdf->excelData[]=array($subdata['fondsOmschrijving'],'','','','','','',round($subdata['actuelePortefeuilleWaardeEuro'],2));
          }
          else
					{
						$this->pdf->row(array("",
															...
															$this->formatKoers($subdata['historischeWaarde'], 2),
															...
															"",
															$this->formatKoers($subdata['actueleFonds'], 2),
						...
						$this->pdf->excelData[]=array($subdata['fondsOmschrijving'],
							...
							round($subdata['historischeWaardeTotaal'],2),
							round($subdata['historischeWaardeTotaalValuta'],2),
      ...
      foreach($liqiteitenBuffer as $data)
        ...
        if($n>=$totaalRegels && $this->pdf->GetY()>175)
          $this->pdf->AddPage();
				...
				$this->pdf->SetTextColor($this->pdf->rapport_fontcolor[r],$this->pdf->rapport_fontcolor[g],$this->pdf->rapport_fontcolor[b]);
				$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
				$this->pdf->excelData[]=array($omschrijving,'','','','','',round($data['actuelePortefeuilleWaardeInValuta'],2),round($data['actuelePortefeuilleWaardeEuro'],2));
			  $this->pdf->row(array("",
												"",
												"",
												"",
