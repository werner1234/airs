...
/*
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2015/04/19 08:35:38 $
 		File Versie					: $Revision: 1.11 $

 		$Log: RapportRISK_L5.php,v $
 		Revision 1.11  2015/04/19 08:35:38  rvv
 		*** empty log message ***
 		
 		Revision 1.10  2014/03/16 11:17:35  rvv
 		*** empty log message ***
 		
 		Revision 1.9  2014/01/15 14:21:32  rvv
 		*** empty log message ***
 		
 		Revision 1.8  2014/01/11 15:46:35  rvv
 		*** empty log message ***
 		
 		Revision 1.7  2014/01/08 17:02:12  rvv
 		*** empty log message ***
 		
 		Revision 1.6  2014/01/08 16:52:37  rvv
 		*** empty log message ***
 		
 		Revision 1.5  2013/10/19 15:57:25  rvv
 		*** empty log message ***
 		
 		Revision 1.4  2013/10/16 15:35:04  rvv
 		*** empty log message ***
 		
 		Revision 1.3  2013/09/01 13:33:07  rvv
 		*** empty log message ***
 		
 		Revision 1.2  2013/08/25 08:50:52  rvv
 		*** empty log message ***
 		
 		Revision 1.1  2012/05/17 07:00:02  rvv
 		*** empty log message ***
 		
 		Revision 1.13  2011/06/25 16:51:45  rvv
 		*** empty log message ***

 		Revision 1.12  2010/11/17 17:16:33  rvv
 		*** empty log message ***

 		Revision 1.11  2008/03/18 09:30:24  rvv
 		*** empty log message ***

 		Revision 1.10  2007/10/04 11:57:04  rvv
 		*** empty log message ***
...
class RapportRISK_L5
{
	function RapportRISK_L5($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		...
		if($this->pdf->rapport_GRAFIEK_titel)
			$this->pdf->rapport_titel = vertaalTekst($this->pdf->rapport_GRAFIEK_titel,$this->pdf->rapport_taal);
		else
...
if($printOIBgrafiek==true)
{
  ...
  $this->pdf->Pie3D($grafiekData['OIB']['Percentage'],$grafiekData['OIB']['Kleur'],$Xas,$yas,$diameter,$hoek,$dikte,
  vertaalTekst("Onderverdeling in beleggingscategorie",$this->pdf->rapport_taal)); 
}
else
...
if($printOIRgrafiek==true)
{
  ...
  $this->pdf->Pie3D($grafiekData['OIR']['Percentage'],$grafiekData['OIR']['Kleur'],$Xas+135,$yas,$diameter,$hoek,$dikte,
  vertaalTekst("Onderverdeling in Regio",$this->pdf->rapport_taal));
}
else
...
function afmAfbeelding($w,$afmstd)
{
  ...
  function plotZorgBar2($w,$h,$data)
  {
    ...
    $kleurenGroen=array(0,100,0);
    $kleurenRood=array(200,0,0);
    $margeW=2;
    $margeH=0.2;
