...
$Log: RapportVKMS_L73.php,v $
Revision 1.18  2019/02/03 15:50:24  rvv
*** empty log message ***

Revision 1.17  2019/01/16 08:41:45  rvv
*** empty log message ***

Revision 1.16  2018/12/30 08:15:11  rvv
*** empty log message ***

Revision 1.15  2018/12/29 13:57:23  rvv
*** empty log message ***

Revision 1.14  2018/12/22 16:15:52  rvv
*** empty log message ***

Revision 1.13  2018/08/04 13:27:17  rvv
*** empty log message ***

Revision 1.12  2018/08/04 13:02:56  rvv
*** empty log message ***

Revision 1.11  2018/08/04 11:54:53  rvv
*** empty log message ***

Revision 1.10  2018/04/08 07:29:47  rvv
*** empty log message ***

Revision 1.9  2018/04/07 15:21:44  rvv
*** empty log message ***

Revision 1.8  2017/10/07 16:54:34  rvv
*** empty log message ***

Revision 1.7  2017/08/26 17:37:43  rvv
*** empty log message ***

Revision 1.6  2017/08/16 15:58:49  rvv
*** empty log message ***

Revision 1.5  2017/07/01 17:03:24  rvv
*** empty log message ***

Revision 1.4  2017/06/11 15:07:59  rvv
*** empty log message ***

Revision 1.3  2017/06/11 15:06:34  rvv
*** empty log message ***
...
class RapportVKMS_L73
{
	function RapportVKMS_L73($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
		...
		if($portefeuilleStartJul>$this->vanafJul)
		{
			...
			$dagenInJaar=(db2jul($this->pdf->rapport_jaar.'-12-31')-db2jul($this->pdf->rapport_jaar.'-01-01'))/86400+1;
			$this->deelVanJaar=round($dagen)/round($dagenInJaar);
		}
	...
	function printSubTotaal($lastCategorieOmschrijving,$allData,$style='')
	{
		...
		$this->pdf->row(array('','',
											...
											$this->formatGetal($data['transkosten'],0),
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
		...
		if($this->pdfVullen==true)
		{
	  	...
	  	$this->pdf->row(array(vertaalTekst('Indirecte (fonds)kosten',$this->pdf->rapport_taal), "€   ".$this->formatGetal($totaalDoorlopendekosten, 0) ));
		...
		if($this->pdfVullen==true)
		{
			...
			$this->pdf->setWidths(array(28+50+20,5,15,20,20,20,20,20,20,20,20));
		...
		while($data = $DB->nextRecord())
		{
			if($this->pdfVullen==true)
			{
				...
				if($data['Grootboekrekening']=='BEH')
				{
					...
					$this->pdf->row(array('Alpha Capital'));
					$this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
...
ORDER BY HoofdcategorienVolgorde.Afdrukvolgorde, HoofdBeleggingscategorien.Afdrukvolgorde,BeleggingscategorienVolgorde.Afdrukvolgorde, BeleggingscategorienVolgorde.Afdrukvolgorde, Beleggingscategorien.Afdrukvolgorde,fondsVolgorde,OptieBovenliggendFonds,FondsOmschrijving ";
      ...
      $somVelden=array('beginwaarde','eindwaarde','stort','resultaat','gemWaarde','weging','bijdrage');
		...
		if($this->skipDetail==true)
		{
			...
			$this->pdf->addPage();
			$this->pdf->setY(25);
			$this->pdf->templateVars[$this->pdf->rapport_type.'SPaginas']=$this->pdf->page;
			$this->pdf->templateVarsOmschrijving[$this->pdf->rapport_type.'SPaginas']=$this->pdf->rapport_titel;
			$this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
		}
      ...
      $this->grootboekKleuren=$grootboekKleuren;
          ...
          $this->pdf->ln();
          $this->pdf->row(array('De invloed van kosten op het langjarige rendement wordt nog niet weergegeven in een grafiek omdat u bij ons korter dan 12 maanden belegt.'));
    ...
    for($i=$this->pdf->rapport_jaar; $i<=$doelJaar; $i++)
		{
			...
			$kosten+=$nieuweWaarde*($this->vkmWaarde['vkmWaarde']/$this->deelVanJaar/100);
		...
		$this->pdf->setXY(30,$this->pdf->getY()+12);
		$this->LineDiagram(120, 50, $grafiekWaarden,array($waardeZonderKostenKleur,$waardeNaKostenKleur,$cumulatieveKostenKleur),0,0,4,4,false);//50
