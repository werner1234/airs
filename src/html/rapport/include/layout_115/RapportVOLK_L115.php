...
class RapportVOLK_L115
{
	function RapportVOLK_L115($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    " TijdelijkeRapportage.beleggingscategorie =  '".$categorien['beleggingscategorie']."' AND ".
      ...
      if($totaalRenteInValuta <> 0)
      {
        ...
        $this->pdf->row(array("",
                          ...
                          "","",$this->formatGetal($totaalRenteInValuta,0),$this->formatGetal($subtotaalPercentageVanTotaal,2).' %',"","","","",""));
...
//rvv
      ...
      $grandtotaaldividendCorrected  += $subtotaal['totaalDividendCorrected'];
		...
		$this->pdf->printValutaoverzicht($this->portefeuille, $this->rapportageDatum,true,true);
    $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
