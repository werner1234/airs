...
class RapportPERFG_L115
{
	function RapportPERFG_L115($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
if($this->pdf->portefeuilledata['SpecifiekeIndex']<>'')
{
  ...
  $DB->SQL($query);
  $DB->Query();
  $benchmarkOmschrijving=$DB->lookupRecord();
  $benchmarkOmschrijving[$this->pdf->portefeuilledata['SpecifiekeIndex']]=$benchmarkOmschrijving['Omschrijving'];
}
  ...
  if(count($rendamentWaarden) > 0)
   {
     ...
     $totaal=array();
        ...
        $kleuren=array(array($this->pdf->rapport_kop_bgcolor['r'],$this->pdf->rapport_kop_bgcolor['g'],$this->pdf->rapport_kop_bgcolor['b']),array(242,21,21));
        $this->LineDiagram(270, 60, $grafiekData,$kleuren,0,0,6,5,1);//50
        $this->pdf->SetXY($valX, $valY + 75+$yShift);
        $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
        if($benchmarkOmschrijving[$this->pdf->portefeuilledata['SpecifiekeIndex']]=='')
          unset($kleuren[1]);
        foreach($kleuren as $index=>$kleur)
        {
          ...
          if($index==1)
            $this->pdf->Cell(50, 4, $benchmarkOmschrijving[$this->pdf->portefeuilledata['SpecifiekeIndex']], 0, 0, "L");
        }
...
*/
   ...
   // $color=array(200,0,0);
    ...
    for ($i=0; $i<count($data); $i++)
    {
      ...
      if($kwartalen==true)
      {
        $maand=intval(substr($legendDatum[$i],5,2));
        if($maand%3==0 || $i==$aantalPunten-1)
          $tonen=true;
        else
          $tonen=false;
      }
