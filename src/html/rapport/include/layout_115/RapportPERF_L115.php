...
include_once("rapport/include/layout_115/ATTberekening_L115.php");

class RapportPERF_L115
{

	function RapportPERF_L115($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    $this->att=new ATTberekening_L115($this);
	}
...
WHERE Portefeuilles.Portefeuille = '$this->portefeuille'";
    ...
    $query = "SELECT DISTINCT(TijdelijkeRapportage.valuta) AS val, Valutas.Omschrijving AS ValutaOmschrijving, TijdelijkeRapportage.actueleValuta".
      " FROM TijdelijkeRapportage, Valutas ".
      " WHERE TijdelijkeRapportage.portefeuille = '".$this->portefeuille."' AND ".
      " TijdelijkeRapportage.rapportageDatum = '".$this->rapportageDatum."' AND ".
      " TijdelijkeRapportage.valuta NOT IN('".$this->pdf->rapportageValuta."','NLG') AND ".
    ...
    while($index = $DB->nextRecord())
    {
      $valutas[$index['val']]=$index;
    }
    foreach($valutas as $val=>$index)
    {
      foreach ($perioden as $periode=>$datum)
        $valutas[$val]['valutaKoers_'.$periode]=getValutaKoers($val,$datum);
  
      $valutas[$val]['performance'] =     ($valutas[$val]['valutaKoers_eind'] - $valutas[$val]['valutaKoers_begin']) / ($valutas[$val]['valutaKoers_begin']/100 );
    }
    ...
    $this->pdf->Rect($this->pdf->marge+128,120,145,(count($benchmarks)+count($valutas))*4+4);
    ...
    foreach($valutas as $val=>$valData)
    {
      $this->pdf->row(array('',$valData['ValutaOmschrijving'],
                        $this->formatGetal(1/$valData['valutaKoers_begin'],4),
                        $this->formatGetal(1/$valData['valutaKoers_eind'],4),
                        $this->formatGetal($valData['performance'],2)."%",
                        $this->formatGetal($valData['performance'],2)."%"));
...
function addResultaat()
{
  ...
  */
  $beginJaar=$rapParts[0]."-01-01";
  if(db2jul($beginJaar)<db2jul($this->pdf->PortefeuilleStartdatum))
    $beginJaar=$this->pdf->PortefeuilleStartdatum;
  
  $vetralingGrootboek=$this->getGrootboeken();
  
    $att=new ATTberekening_L115($this);
    ...
    $this->waarden['Jaar']=$this->att->bereken($beginJaar,$this->rapportageDatum,'Categorien');
