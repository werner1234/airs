...
$Log: RapportOIB_L65.php,v $
Revision 1.6  2019/03/31 12:19:56  rvv
*** empty log message ***

Revision 1.5  2016/07/07 05:29:15  rvv
*** empty log message ***

Revision 1.4  2016/07/06 16:09:31  rvv
*** empty log message ***
...
class RapportOIB_L65
{
	function RapportOIB_L65($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
$this->printPie($data['beleggingscategorieBegin']['pieData'],$data['beleggingscategorieBegin']['kleurData'],vertaalTekst('Categorieverdeling',$this->pdf->rapport_taal).' '.date("d-m-Y",db2jul($rapportageDatumVanaf)),60,50);
$this->pdf->wLegend=0;
...
foreach ($data as $type=>$typeData)
{
  ...
  foreach ($typeData['data'] as $categorie=>$gegevens)
  {
    ...
    if($type=='valutaVerdeling')
     ...
     $regelData[$n][0]='';
     $regelData[$n][1+$offset]=vertaalTekst($gegevens['Omschrijving'],$this->pdf->rapport_taal);
     ...
     $regelTotaal[$type]['percentage']+=round($data[$type]['kleurData'][$gegevens['Omschrijving']]['percentage'],1);
  }
...
$this->pdf->SetFont($this->pdf->rapport_font,'b',$this->pdf->rapport_fontsize);
//$this->pdf->row(array('','Totaal '.date("d-m-Y",db2jul($rapportageDatumVanaf)), $this->formatGetal($regelTotaal['beleggingscategorieBegin']['waardeEur']),$this->formatGetal($regelTotaal['beleggingscategorieBegin']['percentage'],1).'%','',
//'Totaal '.date("d-m-Y",db2jul($rapportageDatum)), $this->formatGetal($regelTotaal['beleggingscategorieEind']['waardeEur']),$this->formatGetal($regelTotaal['beleggingscategorieEind']['percentage'],1).'%'
//,'','Totaal '.date("d-m-Y",db2jul($rapportageDatum)), $this->formatGetal($regelTotaal['valutaVerdeling']['waardeEur']),$this->formatGetal($regelTotaal['valutaVerdeling']['percentage'],1).'%'
//));

		$this->pdf->row(array('',vertaalTekst('Totaal',$this->pdf->rapport_taal).' '.date("d-m-Y",db2jul($rapportageDatumVanaf)), $this->formatGetal($regelTotaal['beleggingscategorieBegin']['waardeEur']),$this->formatGetal($regelTotaal['beleggingscategorieBegin']['percentage'],1).'%','',
											vertaalTekst('Totaal',$this->pdf->rapport_taal).' '.date("d-m-Y",db2jul($rapportageDatum)), $this->formatGetal($regelTotaal['beleggingscategorieEind']['waardeEur']),$this->formatGetal($regelTotaal['beleggingscategorieEind']['percentage'],1).'%'
										,''	));
$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
