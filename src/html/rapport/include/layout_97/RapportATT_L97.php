...
class RapportATT_L97
{
	function RapportATT_L97($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		$this->pdf->rapport_type = "ATT";
		$this->pdf->rapport_datum = db2jul($rapportageDatum);
		$this->pdf->rapport_datumvanaf = db2jul($rapportageDatumVanaf);
		$this->pdf->rapport_titel = "Performance per maand";
	...
	function addPagina($portefeuille)
  {
    global $__appvar;
...
//  $indexData = $this->getWaarden($this->rapportageDatumVanaf ,$this->rapportageDatum ,$portefeuille);
...
//exit;
    ...
    */
    ...
    $this->addPagina($this->portefeuille);
    ...
    if($this->consolidatie==true)
    {
      foreach($this->pdf->portefeuilles as $portefeuille)
      {
        ...
        $this->pdf->rapport_titel = "Performance verslagperiode : $kop";
        $this->addPagina($portefeuille);
      }
    }
	}
  ...
  function addGrafiek($portefeuille,$grafiekData,$grafiekNr=-1)
  {
    if (count($grafiekData) > 1)
    {
      ...
      foreach($kleuren as $index=>$kleur)
      {
        ...
        if($index==0)
          $this->pdf->Cell(($grafiekNr<>-1?150:50), 4, ($grafiekNr<>-1?$portefeuille.' - '.$pData['naam'].' - '.$pData['naam1']:'Portefeuille'), 0, 0, "L");
        if($index==1)
