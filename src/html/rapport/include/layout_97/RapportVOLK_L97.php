...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2017/12/23 18:14:34 $
File Versie					: $Revision: 1.2 $

$Log: RapportVOLK_L97.php,v $
...
class RapportVOLK_L97
{
	function RapportVOLK_L97($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		$this->pdf->rapport_type = "VOLK";
		$this->pdf->rapport_datum = db2jul($rapportageDatum);
		$this->pdf->rapport_datumvanaf = db2jul($rapportageDatumVanaf);
    $this->rapport_titel = "Overzicht depot(s)";
		$this->pdf->rapport_titel = $this->rapport_titel ;
  ...
  function getDividend($fonds)
  {
    ...
    $this->addPortefeuille($this->portefeuille);
    if(is_array($this->pdf->portefeuilles ) && count($this->pdf->portefeuilles)> 1)
    {
      foreach ($this->pdf->portefeuilles as $portefeuille)
      {
        ...
        if($naam['naam']<>'')
          $this->pdf->rapport_titel = "Overzicht ".$naam['naam'];
        else
          $this->pdf->rapport_titel = "Overzicht $portefeuille";
        $this->addPortefeuille($portefeuille);
      }
    }
	}
	
  function	addPortefeuille($portefeuille)
  {
  	if($portefeuille <> $this->portefeuille)
    {
      $this->pdf->AddPage();
      $fondswaarden = berekenPortefeuilleWaarde($portefeuille,$this->rapportageDatum,(substr($this->rapportageDatum, 5, 5) == '01-01')?true:false,$this->pdf->portefeuilledata['RapportageValuta'],$this->rapportageDatumVanaf);
      vulTijdelijkeTabel($fondswaarden ,$portefeuille,$this->rapportageDatum);
    }
    global $__appvar;
    $query = "SELECT Clienten.Naam, Clienten.Naam1, Portefeuilles.Depotbank, Portefeuilles.AEXVergelijking, Portefeuilles.SpecifiekeIndex, Portefeuilles.Vermogensbeheerder, Portefeuilles.ClientVermogensbeheerder, Portefeuilles.Risicoklasse, Portefeuilles.Client FROM Portefeuilles, Clienten WHERE Portefeuille = '".$portefeuille."' AND Portefeuilles.Client = Clienten.Client ";
    $DB = new DB();
    $DB->SQL($query);
    $DB->Query();
    ...
    $query = "SELECT TijdelijkeRapportage.BeleggingscategorieOmschrijving as Omschrijving, ".
      ...
      " WHERE TijdelijkeRapportage.portefeuille = '".$portefeuille."' AND TijdelijkeRapportage.type = 'fondsen' AND TijdelijkeRapportage.rapportageDatum = '".$this->rapportageDatum."'"
      .$__appvar['TijdelijkeRapportageMaakUniek'].
...
//rvv
    ...
    {
      echo "<script>
			alert('Fout : Fout in rapport ".$portefeuille.", totale waarde (".round($totaalWaarde,2).") komt niet overeen met afgedrukte totaal (".round($actueleWaardePortefeuille,2).") in rapport ".$this->pdf->rapport_type."  Verschil (".round($actueleWaardePortefeuille-$totaalWaarde,2).") ');
			</script>";
      ob_flush();
    ...
    if($portefeuille == $this->portefeuille)
    {
      ...
      $this->pdf->printValutaoverzicht($portefeuille, $this->rapportageDatum);
      $this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
      $this->pdf->printRendement($portefeuille, $this->rapportageDatum, $this->rapportageDatumVanaf);
