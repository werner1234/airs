...
class RapportFRONTC_L97
{
	function RapportFrontC_L97($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  //  $this->pdf->memImage($this->pdf->logoString,(297/2)-($this->pdf->logoSize/2),4,$this->pdf->logoSize);

   	$this->pdf->widthA = array(30,70);
    ...
    if($_POST['anoniem']!=1)
    {
      ...
      {
        $portefeuilleCounter++;
        if ( $portefeuilleCounter >= 13 ) {
          $this->pdf->SetY(45);
          $this->pdf->SetWidths(array(30+ ( $loopCounter * 70 ),70));
          $portefeuilleCounter = 0;
          $loopCounter++;
        }
        $this->pdf->row(array('', ($pdata['Naam'] <> ''?$pdata['Naam']:$pdata['Portefeuille'])));
        $this->pdf->Ln(1);
      }
    }
    /*
