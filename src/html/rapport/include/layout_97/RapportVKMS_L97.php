...
class RapportVKMS_L97
{
	function RapportVKMS_L97($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum,$laatstejaar=true)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
...
//    $indirecteKostenVermogen=array(vertaalTekst('Indirecte (fonds)kosten ten opzichte van onderliggend vermogen',$this->pdf->rapport_taal));
    ...
    //  $gemiddelde=$this->verdelingTotaal[$portefeuille]['totaal']['gemiddelde'];
     ...
     // echo "$portefeuille ".($vkmWaarden['fondsGemiddeldeWaarde']/$vkmWaarden['gemiddeldeWaarde'] * 100)."=".$vkmWaarden['fondsGemiddeldeWaarde']."/".$vkmWaarden['gemiddeldeWaarde']."  * 100<br>\n";
      $indirecteKosten[]=$this->formatGetal($vkmWaarden['totaalDoorlopendekosten'], 0);
      $indirecteKosten[]= $this->formatGetal($vkmWaarden['doorlopendeKostenPercentage'] * 100, 2) ;
//        $indirecteKostenVermogen[]= $this->formatGetal($vkmWaarden['doorlopendeKostenPercentage'] * 100, 2) . ' %';
      ...
      $gemiddeldVermogen[]=$this->formatGetal($vkmWaarden['gemiddeldeWaarde'], 0);
      $gemiddeldVermogen[]=''; //extra witregel waar normaal anders het percentage staat.
      $indirecteKostenfactor[]=$this->formatGetal($vkmWaarden['totaalDoorlopendekosten'], 0);
    ...
    $halveCollWidth = $colWidth / 2;
    $percentCollWidth = $colWidth / 3;

    $colWidth = $halveCollWidth + $percentCollWidth;
    if($this->pdfVullen==true)
    {
      ...
      $this->pdf->excelData[]=array();
      $this->pdf->excelData[]=array();
      $this->pdf->setAligns(array('L', 'L', 'L', 'L', 'L', 'L', 'L', 'L', 'L', 'L', 'L', 'L', 'L', 'L', 'L', 'L', 'L', 'L', 'L'));
      $this->pdf->setWidths(array($col1+5, $colWidth, $colWidth, $colWidth, $colWidth, $colWidth, $colWidth, $colWidth, $colWidth, $colWidth, $colWidth));
      $this->pdf->SetFont($this->pdf->rapport_font, 'b', $this->pdf->rapport_fontsize);
      $ystart=$this->pdf->getY();
      $this->pdf->row($kop);

      $this->pdf->setWidths(array($col1,$halveCollWidth,$percentCollWidth,$halveCollWidth,$percentCollWidth,$halveCollWidth,$percentCollWidth,$halveCollWidth,$percentCollWidth,$halveCollWidth,$percentCollWidth,$halveCollWidth,$percentCollWidth));
...
//      $this->pdf->ln();
    ...
    //  $this->pdf->row($herrekendIndirect);
      ...
      foreach($vkmPerPortefeuille as $portefeuille=>$vkmWaarden)
      {
        ...
        $this->pdf->setXY($col1+20+$n*$colWidth, 190);
