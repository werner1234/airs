...
include_once("rapport/include/layout_97/ATTberekening_L97.php");

class RapportPERF_L97
{

	function RapportPERF_L97($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    $this->att=new ATTberekening_L97($this);
	}
...
function addResultaat()
{
  ...
  $vetralingGrootboek=$this->getGrootboeken();
    ...
    $att=new ATTberekening_L97($this);
...
foreach($categorien as $categorie)
{
  ...
  foreach($categorien as $categorie)
  {
    ...
    $width=($this->pdf->w-70-$this->pdf->marge*2);
    $aantalCategorien=8;
    $w=($width-$aantalCategorien*6)/$aantalCategorien;
  
    $wArray=array(0,70);
  	for($i=0;$i<$aantalCategorien;$i++)
    {
      $wArray[]=$w;
      $wArray[]=6;
    }
    $this->pdf->widthB = $wArray;
		$this->pdf->alignB = array('L','L','R','L','R','L','R','L','R','L','R','L','R','L','R','L','R');
