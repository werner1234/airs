...
class RapportOIB_L67
{
	function RapportOIB_L67($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
$query="SELECT
if(TijdelijkeRapportage.beleggingscategorie <> '',TijdelijkeRapportage.beleggingscategorie,if(TijdelijkeRapportage.type='rekening','Liquiditeiten', 'geen')) as categorie,
sum(TijdelijkeRapportage.ActuelePortefeuilleWaardeEuro) AS WaardeEuro,
if(TijdelijkeRapportage.beleggingscategorie <> '',Beleggingscategorien.Omschrijving,if(TijdelijkeRapportage.type='rekening','Liquiditeiten','geen')) as categorieOmschrijving
FROM TijdelijkeRapportage LEFT JOIN Beleggingscategorien on TijdelijkeRapportage.beleggingscategorie = Beleggingscategorien.beleggingscategorie
	...
	{
		$cat['categorieOmschrijving']=vertaalTekst($cat['categorieOmschrijving'],$this->pdf->rapport_taal);
...
ORDER BY WaardeEuro desc";
	...
	while($cat = $DB->nextRecord())
	{
	   $cat['Omschrijving']=vertaalTekst($cat['Omschrijving'],$this->pdf->rapport_taal);
...
$this->pdf->SetWidths(array(20, 38,22,15, 15, 38,22,15, 15, 38,22,15));
...
//$this->pdf->CellBorders = array('','','UU','UU','','','UU','UU','','','UU','UU');
        $this->pdf->SetFillColor($this->pdf->rapport_totaal_fillcolor[0],$this->pdf->rapport_totaal_fillcolor[1],$this->pdf->rapport_totaal_fillcolor[2]);
      	for($i=0;$i<3;$i++)
 	        $this->pdf->Rect($this->pdf->marge+20+$i*90,$this->pdf->GetY()-1,75,6,'F'); 
...
$this->pdf->row(array('',vertaalTekst('Totaal',$this->pdf->rapport_taal).' '.date("d-m-Y",db2jul($rapportageDatumVanaf)), $this->formatGetal($regelTotaal['beleggingscategorieBegin']['waardeEur']),$this->formatGetal($regelTotaal['beleggingscategorieBegin']['percentage'],0).'%','',
vertaalTekst('Totaal',$this->pdf->rapport_taal).' '.date("d-m-Y",db2jul($rapportageDatum)), $this->formatGetal($regelTotaal['beleggingscategorieEind']['waardeEur']),$this->formatGetal($regelTotaal['beleggingscategorieEind']['percentage'],0).'%'
,'',vertaalTekst('Totaal',$this->pdf->rapport_taal).' '.date("d-m-Y",db2jul($rapportageDatum)), $this->formatGetal($regelTotaal['valutaVerdeling']['waardeEur']),$this->formatGetal($regelTotaal['valutaVerdeling']['percentage'],0).'%'
));
