...
$Log: RapportKERNV_L25.php,v $
Revision 1.6  2020/02/19 15:02:02  rvv
*** empty log message ***

Revision 1.5  2020/02/01 18:11:55  rvv
*** empty log message ***

Revision 1.4  2019/09/04 15:31:42  rvv
*** empty log message ***

Revision 1.3  2019/07/18 13:48:28  rvv
*** empty log message ***

Revision 1.2  2019/07/18 05:55:18  rvv
*** empty log message ***

Revision 1.1  2019/07/17 15:35:19  rvv
*** empty log message ***
...
class RapportKERNV_L25
{
	function RapportKERNV_L25($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	...
	function printSubTotaal($lastCategorieOmschrijving,$allData,$style='')
	{
	  ...
	  {
	    $prefix='Subtotaal';
	    $this->pdf->CellBorders = array('','','TS','TS','TS','TS','TS','TS','TS','TS','TS','TS','TS','TS','TS');
	  }
	  else
	  {
	    $prefix='';
	    $this->pdf->CellBorders = array('','',array('TS','UU'),array('TS','UU'),array('TS','UU'),array('TS','UU'),array('TS','UU'),array('TS','UU'),array('TS','UU'),array('TS','UU'),array('TS','UU'),array('TS','UU'),array('TS','UU'),array('TS','UU'),array('TS','UU'));
	  }
      ...
      $this->pdf->CellFontColor = array('','','','','','','','','','','','',$this->pdf->rapport_font_groen);
   	...
   	$this->pdf->row(array('',
                        ...
                        $this->formatGetal($data['bijdrage']*100,2,true),
                               ...
                               $this->formatGetal(($data['bijdrage']*100-($data['weging']*$data['indexPerf'])*100),2,true)));
...
ORDER BY HoofdBeleggingscategorien.Afdrukvolgorde,Beleggingscategorien.Afdrukvolgorde,fondsVolgorde,OptieBovenliggendFonds,FondsOmschrijving ";
...
ORDER BY HoofdBeleggingscategorien.Afdrukvolgorde, Beleggingscategorien.Afdrukvolgorde";
...
$this->totalen['gemiddeldeWaarde']=$perfTotaal['gemWaarde'];
  ...
  {
    $hoofdcategorieData['categorie']=$hoofdCategorie;
...
$totaalSom['indexPerf'] += ($data['indexPerf']*$data['weging']);
}
    ...
    foreach ($categorieData as $categorie=>$fondsData)
    {
      ...
      foreach ($fondsData['fondsen'] as $id=>$fonds)
      {
        ...
        if(in_array($fonds,$heeftOptie) && $lastLn==false)
          $this->pdf->Ln();
        $resultaat=$data['resultaat']/$data['gemWaarde']*100*$spiegeling;
...
limit 1 ";
	  ...
	  if($categorie=='totaal')
    {
      $fondsData['Fonds']=$this->pdf->portefeuilledata['SpecifiekeIndex'];
    }
...
limit 1 ";
	  $DB->SQL($query);
	  $fondsData=$DB->lookupRecord();// echo $query; listarray($fondsData);
}
...
if($fondsData['Fonds'] <> '')
{
  ...
  {   
     ...
     $perfIndexCum=0;
//echo $fondsData['Fonds']." $van  $tot <br>\n";
