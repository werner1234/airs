...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2016/10/13 12:53:17 $
File Versie					: $Revision: 1.8 $

$Log: RapportVHO_L58.php,v $
Revision 1.8  2016/10/13 12:53:17  rvv
*** empty log message ***

Revision 1.6  2015/01/31 20:02:23  rvv
*** empty log message ***

Revision 1.5  2014/12/20 16:32:36  rvv
*** empty log message ***

Revision 1.4  2014/11/23 14:13:22  rvv
*** empty log message ***

Revision 1.3  2014/11/08 18:37:31  rvv
*** empty log message ***

Revision 1.2  2014/10/25 14:39:09  rvv
*** empty log message ***
...
class RapportVHO_L58
{
	function RapportVHO_L58($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    $this->aandeel=1;
    $this->rapport_VOLK_decimaal_proc=1;
	}
	...
	function printSubTotaal($title, $totaalA, $totaalB, $totaalC, $totaalD, $totaalE, $totaalF = 0, $grandtotaal=false, $totaalG = 0, $totaalH = 0 )
	{
		...
		$this->pdf->SetAligns($this->pdf->alignB);
			...
			if($totaalC <>0)
				$this->printCol(5,$this->formatGetal($totaalC,$this->rapport_VOLK_decimaal_proc),$grandtotaal);
      if($totaalG <>0)
			...
			if($totaalF <>0)
				$this->printCol(13,$this->formatGetal($totaalF,$this->rapport_VOLK_decimaal_proc),$grandtotaal);
			$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
			$this->pdf->ln();
...
//echo $subquery."<br><br>";exit;
        ...
        //$fondsResultaat+=$dividend;
				...
				$percentageVanTotaaltxt = $this->formatGetal($percentageVanTotaal,$this->rapport_VOLK_decimaal_proc)." %";

				if($procentResultaat > 1000 || $procentResultaat < -1000)
					$procentResultaattxt = "p.m.";
				else
					$procentResultaattxt = $this->formatGetal($procentResultaat,$this->rapport_VOLK_decimaal_proc);
        ...
        if($dividend['totaal'] <> 0)  
          ...
          $dividendTxt='';
				$this->pdf->row(array($this->formatAantal($subdata['totaalAantal'],$this->pdf->rapport_VOLK_aantal_decimaal,$this->pdf->rapport_VOLK_aantalVierDecimaal),
												...
												$this->formatGetal($percentageVanTotaal,$this->rapport_VOLK_decimaal_proc),
												"",
     ...
     $subtotaal=array();
		...
		if($DB1->records() > 0)
		{
			...
			foreach($liqiteitenBuffer as $data)
			{
				...
				$subtotaalPercentageVanTotaaltxt = $this->formatGetal($subtotaalPercentageVanTotaal,$this->rapport_VOLK_decimaal_proc);
 ...
 //echo $query;
	...
	while($data = $DB->NextRecord())
	{
      ...
      $indexCategorien[$data['Beleggingscategorie']]=$data['Beleggingscategorie'];
		    ...
		    $this->pdf->Rect($this->pdf->marge,$this->pdf->getY(),233,$hoogte,'F');
		    $this->pdf->SetFillColor(0);
		    $this->pdf->Rect($this->pdf->marge,$this->pdf->getY(),233,$hoogte);
		    $this->pdf->SetX($this->pdf->marge);
		    ...
		    $this->pdf->SetX($this->pdf->marge);
		    $this->pdf->SetWidths(array(50,33,33,23,23,23,23,23));
        ...
        "Perf. Rapp.per.","Perf. 3MND","Perf. 1Y","Perf. 3Y","Perf. 5Y"));
