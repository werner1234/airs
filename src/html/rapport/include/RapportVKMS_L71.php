...
class RapportVKMS_L71
{
	function RapportVKMS_L71($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    ...
    $kostenRow=array();
    $kostenData=array('BEH'=>array('Grootboekrekening'=>'BEH','Omschrijving'=>'Beheer- of bemiddelingsvergoeding (niet over vermogen belegd in beleggingsfondsen VEC Fondsbeheer)'),
                      'KOST'=>array('Grootboekrekening'=>'KOST','Omschrijving'=>'Transactiekosten (VEC en/of depotbank)'),
                      'BEW'=>array('Grootboekrekening'=>'BEW','Omschrijving'=>'Servicefee depotbank (servicekosten, bewaarloon, administratie, eventueel all-in fee)'));
    ...
    while($data = $DB->nextRecord())
    {
      ...
      if($data['Grootboekrekening']=='BEW' AND !isset($kostenData['VALK']))
        $kostenData['VALK']=array('Grootboekrekening'=>'VALK','Omschrijving'=>'Overige kosten derden (o.a valutakosten,bied-laat spreadkosten)');
      ...
      $kostenData[$data['Grootboekrekening']]['totaal']+=$data['totaal'];
        ...
        if($data['Grootboekrekening']=='BEH')
        {
          ...
          {
            $kostenRow[] = array("BTW over beheer- of bemiddelingsvergoeding", '€',$this->formatGetal($btw, 0), $this->formatGetal($btw/$gemiddelde*100, 2) . ' %');
          }
        }
        else
        {
...
//				$this->pdf->excelData[]=array('',$data['Omschrijving'],'',round($data['totaal'],0),round($kostenProcent,2) );
    ...
    if($this->pdfVullen==true)
    {
      ...
      if($this->spreadKostenPunten<>0)
      {
        $spreadkostenTxt = 'Bied/laat kosten van ' . $this->formatGetal($this->spreadKostenPunten / 100, 2) . '% over het transactievolume van uw portefeuille zijn een inschatting op basis van een gemiddelde van kosten van vergelijkbare portefeuilles.';
      }
      else
      {
        $spreadkostenTxt = '';
...
- Directe kosten: de kosten van onze dienstverlening, inclusief de kosten van de depotbank voor het aanhouden van uw beleggingsrekening. '.$spreadkostenTxt.'
