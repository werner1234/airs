...
$Log: RapportDOORKIJKVR_L35.php,v $
Revision 1.4  2020/02/29 16:23:08  rvv
*** empty log message ***

Revision 1.3  2019/09/07 16:07:47  rvv
*** empty log message ***

Revision 1.2  2019/09/04 15:31:42  rvv
*** empty log message ***

Revision 1.1  2019/08/03 17:03:12  rvv
*** empty log message ***

Revision 1.7  2019/07/07 12:24:12  rvv
*** empty log message ***

Revision 1.6  2019/07/06 15:44:52  rvv
*** empty log message ***

Revision 1.5  2019/06/19 15:50:55  rvv
*** empty log message ***

Revision 1.4  2019/06/15 20:51:39  rvv
*** empty log message ***
...
class RapportDOORKIJKVR_L35
{
	function RapportDOORKIJKVR_L35($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = $pdf;
...
WHERE bronKoppeling='$airsCategorie' AND doorkijkCategoriesoort='" . mysql_real_escape_string($doorkijkSoort) . "' AND systeem='AIRS' AND vermogensbeheerder='" . $this->pdf->portefeuilledata['Vermogensbeheerder'] . "'";
    ...
    $gemiddelde=0;
      ...
      elseif($doorkijkSoort=='Rating')
      {
        ...
        if($row['rekening']<>'')
          ...
          $gemiddelde+=($this->bucketAvg[$doorkijkSoort][$categorie] * $totaalPercentage/100);
				}
				if($check==0)
        {
          $overige = true;
		...
		if($doorkijkSoort=='Rating')
    {
      $gemiddelde=round($gemiddelde);
      foreach($this->bucketAvg['Rating'] as $rating=>$getal)
      {
        if($getal==$gemiddelde)
        {
          $gemiddelde=$rating;
    ...
    $doorkijkVerdeling['gemiddelde']=$gemiddelde;
    //listarray($doorkijkSoort);
	...
	function toonTabel($regels,$xOffset,$titel,$kleuren)
	{
		...
		$totalen=array();
	//	listarray($kleuren);
		foreach($regels['details'] as $categorie=>$data)
		...
		//foreach($regels as $categorie=>$data)
			if(isset($regels['details'][$categorie]))
			{
				$data =$regels['details'][$categorie];
    ...
    if($titel=='Rating')
      $this->pdf->row(array('','Gemiddelde '. $regels['gemiddelde']));
    else
      $this->pdf->row(array('','Gemiddelde '. $this->formatGetal($regels['gemiddelde'], 2)));
...
*/
    ...
    //$doorkijkCategorieSoorten=array('Rating');//,'Coupon','Rating');
		...
		foreach($doorkijkCategorieSoorten as $index=>$doorkijkCategorieSoort)
		{
			...
			$this->toonTabel($doorKijk,$xOffset,$doorkijkTitels[$doorkijkCategorieSoort],$this->kleuren[$doorkijkCategorieSoort]);
			$grafiekdata=array();
			$grafiekTonen=true;
      ...
      {
          $this->pdf->setXY($this->pdf->marge + $xOffset, 115);
	...
	function writeRapport()
	{
      ...
      $this->bucketAvg[$data['doorkijkCategorieSoort']][$data['doorkijkCategorie']]=0;
		}
...
ORDER BY doorkijk_categoriePerVermogensbeheerder.doorkijkCategoriesoort,doorkijk_categoriePerVermogensbeheerder.afdrukVolgorde,doorkijk_categoriePerVermogensbeheerder.min,doorkijk_categoriePerVermogensbeheerder.doorkijkCategorie";
    ...
    while($row = $db->nextRecord())
    {
      ...
      if($row['doorkijkCategoriesoort']=='Looptijd' && $row['max']>30)
        $row['max']=30;
      elseif($row['doorkijkCategoriesoort']=='Coupon' && $row['max']>15)
        $row['max']=15;
      $this->bucketAvg[$row['doorkijkCategoriesoort']][$row['doorkijkCategorie']]=($row['max']+$row['min'])/2;
    }
    $i=1;
    foreach($this->bucketAvg['Rating'] as $rating=>$avg)
    {
      $this->bucketAvg['Rating'][$rating]=$i;
      $i++;
    }
...
KeuzePerVermogensbeheerder.categorieIXP='Obligaties' ";
		...
		if(count($beleggingscategorien)>0)
		{
			...
			$this->pdf->AddPage();
      $this->pdf->templateVars['DOORKIJKVRPaginas'] = $this->pdf->page;
