...
class RapportKERNZ_L13
{
	function RapportKERNZ_L13($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
  {
    $this->pdf = &$pdf;
    ...
    $this->pdf->excelData[] = array('Categorie','Valuta',"Fondsomschrijving", 'Aantal', 'Koers', 'Waarde', 'Waarde EUR', 'Koers',
      'Waarde', 'Waarde EUR', 'in %', 'Fondsresultaat', 'Valutaresultaat', 'Directresultaat','in %');
...
//     " Rekeningmutaties.Boekdatum >= '".	$this->rapportageDatumVanaf."' AND ".
  ...
  function writeRapport()
  {
    ...
    $this->pdf->widthB = array(5,55,18,15,20,21,1,16,21,21,17,20,20,19,12);
    $this->pdf->alignB = array('R','L','R','R','R','R','R','R','R','R','R','R','R','R','R','R');
    
    // voor kopjes
    $this->pdf->widthA = array(   60,18,15,20,21,1,16,21,21,17,20,20,19,12);
...
//echo $subquery."<br><br>";exit;
      ...
      while($subdata = $DB2->NextRecord())
      {
        ...
        if($n>$regels-2 && $this->pdf->GetY()>170)//
        {
          $this->pdf->AddPage();
        }
        if($kopPrinted==false)
        ...
        $this->pdf->excelData[] = array($categorien['Omschrijving'],$categorien['valuta'],$subdata['fondsOmschrijving'], round($subdata['totaalAantal'],6), round($subdata['historischeWaarde'],2), round($subdata['historischeWaardeTotaal'],2), round($subdata['historischeWaardeTotaalValuta'],2),
          round($subdata['actueleFonds'],2), round($subdata['actuelePortefeuilleWaardeInValuta'],2), round($subdata['actuelePortefeuilleWaardeEuro'],2),
          round($percentageVanTotaal,2), round($fondsResultaat,2), round($valutaResultaat,2), round($dividend['totaal'],2), round($procentResultaat,2));
...
//rvv
      ...
      if($DB2->records() > 0)
      {
        ...
        if($totaalRenteInValuta <> 0)
        {
          ...
          $this->pdf->excelData[] = array('Opgelopen Rente',$categorien['valuta'],'Opgelopen Rente', '','','','','','',round($totaalRenteInValuta,2),round($subtotaalPercentageVanTotaal,2));
    ...
    if($DB1->records() > 0)
    {
     ...
     // if($DB1->records()==1 && ($DB1->records()+6)*$this->pdf->rowHeight+$this->pdf->GetY() > $this->pdf->PageBreakTrigger )
      ...
      foreach($liqiteitenBuffer as $data)
      {
        ...
        $omschrijving = vertaalTekst(str_replace("{PortefeuilleVoorzet}",'',$omschrijving),$this->pdf->rapport_taal);
        ...
        $this->pdf->excelData[] = array('Liquiditeiten',$data['valuta'],$omschrijving, '','','','','',round($data['actuelePortefeuilleWaardeInValuta'],2),round($data['actuelePortefeuilleWaardeEuro'],2),round($subtotaalPercentageVanTotaal,2));
      }
