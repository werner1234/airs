...
$Log: RapportVKMS_L107.php,v $
Revision 1.14  2019/02/20 16:50:39  rvv
*** empty log message ***

Revision 1.13  2019/02/03 15:50:24  rvv
*** empty log message ***

Revision 1.12  2019/01/16 08:41:44  rvv
*** empty log message ***

Revision 1.11  2018/12/05 16:36:17  rvv
*** empty log message ***

Revision 1.10  2018/11/21 16:48:32  rvv
*** empty log message ***

Revision 1.9  2018/07/27 08:57:10  rvv
*** empty log message ***

Revision 1.8  2018/07/21 15:54:40  rvv
*** empty log message ***

Revision 1.7  2018/07/18 15:45:00  rvv
*** empty log message ***

Revision 1.6  2018/05/30 16:10:58  rvv
*** empty log message ***

Revision 1.5  2018/04/07 15:21:44  rvv
*** empty log message ***

Revision 1.4  2018/03/21 17:04:24  rvv
*** empty log message ***

Revision 1.3  2018/03/11 10:53:28  rvv
*** empty log message ***
...
class RapportVKMS_L107
{
	function RapportVKMS_L107($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
    ...
    if($this->pdfVullen==true)
		{
			...
			$this->pdf->row(array(vertaalTekst('Gemiddeld totale vermogen', $this->pdf->rapport_taal), $this->formatGetal($gemiddelde, 0) . ' EUR'));
			$this->pdf->ln();
		...
		{
			$this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
			$this->pdf->CellBorders=array('',array('SUB','TS'),array('SUB','TS'));
			...
			$this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
			$this->pdf->row(array(vertaalTekst('Doorlopende fondskosten',$this->pdf->rapport_taal),$this->formatGetal($totaalDoorlopendekosten, 0), $this->formatGetal($vkmPercentagePortefeuille, 2) . ' %'));
...
/*
			if($spreadKostenEUR <> 0)
			{
				...
				if($this->pdfVullen==true)
				{
					$this->pdf->row(array('Spreadkosten + transactiekosten in het fonds', $this->formatGetal($spreadKostenEUR, 0), $this->formatGetal($kostenProcent, 2) . ' %'));
					$this->pdf->excelData[]=array('','Spreadkosten + transactiekosten in het fonds','',round($spreadKostenEUR,0),round($kostenProcent,2) );
				}
				$totaalDoorlopendekosten += $spreadKostenEUR;
				$vkmWaarde+=$kostenProcent;
				$vkmPercentagePortefeuille+=$kostenProcent;
				$barData['Spreadkosten + transactiekosten in het fonds'] = $kostenProcent;
			}
      */
      if($spreadKostenEUR <> 0)
      {
      ...
      $this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
			$this->pdf->CellBorders=array('',array('SUB','TS'),array('SUB','TS'));
			$this->pdf->row(array(vertaalTekst('Totaal indirecte kosten',$this->pdf->rapport_taal),$this->formatGetal($totaalDoorlopendekosten, 0), $this->formatGetal($vkmPercentagePortefeuille, 2) . ' %'));
			$this->pdf->CellBorders=array();
			$this->pdf->ln();
			$this->pdf->SetFont($this->pdf->rapport_font, 'bi', $this->pdf->rapport_fontsize);
			$this->pdf->row(array(vertaalTekst('Vergelijkende kostenmaatstaf (VKM)',$this->pdf->rapport_taal)));
			$this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
			$this->pdf->CellBorders=array('',array('UU','TS'),array('UU','TS'));
...
foreach ($perCategorie as $hoofdcategorie=>$categorieData)
{
 ...
 WHERE Portefeuilles.portefeuille='".$this->portefeuille."'";
    ...
    for($i=$this->pdf->rapport_jaar; $i<=$doelJaar; $i++)
		...
		$this->pdf->setXY(25,$this->pdf->getY()+15);
