...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2019/10/12 17:11:44 $
File Versie					: $Revision: 1.2 $

$Log: RapportVOLKD_L107.php,v $
Revision 1.2  2019/10/12 17:11:44  rvv
*** empty log message ***

Revision 1.1  2019/10/11 17:39:17  rvv
*** empty log message ***
...
include_once("rapport/include/layout_107/ATTberekening_L107.php");

class RapportVOLKD_L107
{
	function RapportVOLKD_L107($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    date('d-m-Y',$this->pdf->rapport_datum),'Mutaties','Resultaat','Gemiddeld vermogen','Resultaat %','Weging','Bijdrage rendement');
    ...
    $this->att=new ATTberekening_L107($this);
	}
...
$perfTotaal = $this->att->fondsPerformance($alleData,$this->rapportageDatumVanaf,$this->rapportageDatum,true,'totaal');
//listarray($perfTotaal);
$this->totalen['gemiddeldeWaarde']=$perfTotaal['gemWaarde'];
	...
	foreach ($perHoofdcategorie as $hoofdCategorie=>$hoofdcategorieData)
  {
    ...
    $perHoofdcategorie[$hoofdCategorie]['perf'] = $this->att->fondsPerformance($hoofdcategorieData,$this->rapportageDatumVanaf,$this->rapportageDatum,true,$hoofdCategorie);
  }
  /*
     ...
     foreach ($categorieData as $categorie=>$regioData)
       ...
       {
         //$perRegio[$hoofdCategorie][$categorie][$regio]['perf'] = $this->fondsPerformance($regioWaarden);
         $perRegio[$hoofdCategorie][$categorie][$regio]['perf']  = $this->att->fondsPerformance($regioWaarden,$this->rapportageDatumVanaf,$this->rapportageDatum,true,$regio);
       }
*/
	foreach ($perCategorie as $hoofdCategorie=>$regioData)
	    foreach ($regioData as $categorie=>$categorieData)
      {
        ...
        $perCategorie[$hoofdCategorie][$categorie]['perf'] = $this->att->fondsPerformance($categorieData,$this->rapportageDatumVanaf,$this->rapportageDatum,true,$categorie);
      }
  ...
  //{
    ...
    foreach ($categorieData as $categorie=>$fondsData)
    {
      ...
      if($categorie!=$lastCategorie)
        $this->printKop($perCategorie[$hoofdcategorie][$categorie]['omschrijving'],'B');
     ...
     */
        ...
        $data = $this->att->fondsPerformance($tmp,$this->rapportageDatumVanaf,$this->rapportageDatum,true,$fonds);
        ...
        { //echo " $laatsteFonds ".$sub['aantal']."<br>\n";ob_flush();
          ...
          { 
            ...
            $this->pdf->row(array('','        subtotaal '. $laatsteFonds,
                        ...
                        $this->formatGetal($sub['bijdrage'],2,true)));
            unset($this->pdf->CellBorders);
            $this->pdf->Ln();
            $lastLn=true;
          }
          ...
          if($regio!=$lastRegio)
          {
            ...
            if($lastRegio <> '')
            {
              ...
              $this->pdf->row(array('','  subtotaal '.vertaalTekst($perRegio[$hoofdcategorie][$categorie][$lastRegio]['omschrijving'],$this->pdf->rapport_taal),
                        ...
                        $this->formatGetal($subregio['bijdrage'],2,true)));
              $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
              $this->pdf->Ln();
              unset($this->pdf->CellBorders);
            }
        ...
        if(in_array($fonds,$heeftOptie) && $lastLn==false)
          $this->pdf->Ln();
      	$this->pdf->row(array('',substr($fondsData['fondsOmschrijving'][$id],0,40),
                        ...
                        $this->formatGetal($data['weging']*100,2,true),
                        $this->formatGetal($data['bijdrage'],2,true),
                        $this->formatGetal($data['indexPerf'],2,true)
                        ));
            // listarray($data);                    
          ...
          if($sub['aantal']>1)
          {
            $this->pdf->CellBorders = array('','','','TS','TS','TS','TS','TS');
               	$this->pdf->row(array('','        subtotaal '.$laatsteFonds,
                        ...
                        $this->formatGetal($sub['gemWaarde'],0),
                        $this->formatGetal($sub['resultaat']/$sub['gemWaarde']*100,2),
                        $this->formatGetal($sub['weging'],2,true),
                        $this->formatGetal($sub['bijdrage'],2,true)));
...
$data['bijdrage']*100);  
            ...
            if($lastRegio <> '')
            {
              ...
              $this->pdf->row(array('','  subtotaal '.vertaalTekst($perRegio[$hoofdcategorie][$categorie][$lastRegio]['omschrijving'],$this->pdf->rapport_taal),
                        ...
                        $this->formatGetal($subregio['gemWaarde'],0),
                        $this->formatGetal($subregio['resultaat']/$subregio['gemWaarde']*100,2),
                        $this->formatGetal($subregio['weging'],2,true),
                        $this->formatGetal($subregio['bijdrage'],2,true)));
              $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
              $this->pdf->Ln();
              unset($this->pdf->CellBorders);
      ...
      $this->printSubTotaal($perCategorie[$hoofdcategorie][$categorie]['omschrijving'],$perCategorie[$hoofdcategorie][$categorie],'B');
    }
//listarray($perHoofdcategorie[$hoofdcategorie]);
 ...
 //listarray($perfTotaal);
   // $this->printSubTotaal('Totaal',array('perf'=>$perfTotaal),'BI');
    $perfTotaal=$this->att->bereken($this->rapportageDatumVanaf,$this->rapportageDatum,'totaal');
    $this->printSubTotaal('Totaal',array('perf'=>$perfTotaal['totaal']),'BI');
