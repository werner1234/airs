...
class RapportTemplate_L52
{
	function RapportTemplate_L52($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	  global $__appvar;
		$this->pdf = &$pdf;
    ...
    $totPagina = ($lastpage-$startpagina+$paginaCorrectie);//-1
       ...
       $this->pdf->ln(10);
	     ...
	     $inhoudsItems=array('PERFPaginas'=>'Performance over de beleggingscategorieën',
                           ...
                           'KERNZPaginas'=>"Informatie over risicoprofielen",
                           'HUISPaginas'=>"Vergelijkende kostenmaatstaf",
        ...
        foreach ($this->pdf->templateVars as $key=>$value)
        {
          if($this->pdf->templateVarsOmschrijving[$key] <> '')
            $omschrijving=$this->pdf->templateVarsOmschrijving[$key];
          elseif($inhoudsItems[$key] <> '')
            $omschrijving=$inhoudsItems[$key];
          
          if($omschrijving<>'') //$this->pdf->templateVarsOmschrijving
          {
            $text=$omschrijving.' ';
