<?php
include_once($__appvar["basedir"]."/html/rapport/include/PDFRapport_headers_L7.php");

class rapportHulp_L7
{
  function rapportHulp_L7()
  {
    $this->test='';
  }
}

class ATTberekening_L7
{

	function ATTberekening_L7($rapportData,$periode='maanden')
	{
  ...
  {
    $this->rapport = new rapportHulp_L7();
    $db=new DB();
  ...
  function getPerfArray($portefeuille, $rapportStart,$rapportDatum,$valuta)
  {
    ...
    $indexData = $index->getWaarden($rapportStart,$rapportDatum,$portefeuille,'','maanden','EUR',true);
    foreach($indexData as $periodeData)
    {
      $perfData[$periodeData['datum']]['index'] = $periodeData['index']-100;
      $perfData[$periodeData['datum']]['performance'] = $periodeData['performance'];
    }
    return $perfData;
  }

	function bereken($van,$tot,$verdeling='Hoofdcategorie',$valuta='EUR')
	{
	  global $__appvar;
 		$DB=new DB();
   ...
   //   $grootboekKostenFilter='';  
    ...
    foreach ($perioden as $periode)
    {
      ...
      { 
        ...
        if ($valuta <> 'EUR')
        {
          $valutaKoers= getValutaKoers($valuta,$rapDatum);
        }
        else
        {
          $valutaKoers= 1;
        ...
        {  
          ...
          foreach($fondswaarden as $id=>$fondsWaarde)
          {
            ...
            elseif($fondsWaarde['type']=='rekening')
              ...
              $instrument='geen';  
            $this->totalen[$rapDatum]['totaalWaardeEur']+=$fondsWaarde['actuelePortefeuilleWaardeEuro']/$valutaKoers;
            $this->totalen[$rapDatum]['WaardeEur'][$instrument]+=$fondsWaarde['actuelePortefeuilleWaardeEuro']/$valutaKoers;
          }
        }
...
foreach ($perioden as $periode)
{
  ...
  if($kruispost==true)
    ...
    $kruispostGb='';
     ...
     $query="SELECT Grootboekrekening FROM Grootboekrekeningen WHERE Grootboekrekeningen.Storting=1 OR Grootboekrekeningen.Onttrekking=1 $kruispostGb";
      if ($valuta <> 'EUR')
      {
...
//echo "$categorie $totaalGemiddelde = $totaalBeginwaarde - ".$storting['gewogen']."<br>\n"; 
    if($categorie=='totaal')
    {
      ...
      $stortingen 			 	= getStortingenKruis($this->rapport->portefeuille,$datumBegin,$datumEind,$valuta,$kruispost);
      $onttrekkingen 		 	= getOnttrekkingenKruis($this->rapport->portefeuille,$datumBegin,$datumEind,$valuta,$kruispost);
    ...
    //  echo "$performance = ((($totaalEindwaarde - $totaalBeginwaarde) - ".$storting['totaal']." - $interneboeking) / ".$this->totaalGemiddelde.")<br>\n";
    ...
    else
    {
	    ...
	    $queryAttributieStortingenOntrekkingenRekening = "SELECT SUM(((TO_DAYS('".$datumEind."') - TO_DAYS(Rekeningmutaties.Boekdatum)) / (TO_DAYS('".$datumEind."') - TO_DAYS('".$weegDatum."'))  * ((ABS(Rekeningmutaties.Credit) * Rekeningmutaties.Valutakoers ) - (ABS(Rekeningmutaties.Debet) * Rekeningmutaties.Valutakoers ) )))*-1 $koersQuery AS gewogen, ".
	     ...
	     $queryRekeningDirecteKostenOpbrengsten = "SELECT 
                SUM((ABS(Rekeningmutaties.Credit) * Rekeningmutaties.Valutakoers ) - (ABS(Rekeningmutaties.Debet) * Rekeningmutaties.Valutakoers )) $koersQuery AS totaal,
	             SUM(if(Grootboekrekeningen.Opbrengst =1,(ABS(Rekeningmutaties.Credit) * Rekeningmutaties.Valutakoers),0)) $koersQuery AS opbrengstTotaal,
      ...
      $DB->SQL($queryRekeningDirecteKostenOpbrengsten);//logscherm ($queryRekeningDirecteKostenOpbrengsten);
	    $DB->Query(); 
       ...
       $DB->SQL($queryAttributieStortingenOntrekkingen." AND (Rekeningmutaties.Grootboekrekening='FONDS' OR Grootboekrekeningen.Opbrengst=1) "); //echo "$queryAttributieStortingenOntrekkingen <br><br>\n";
   	  ...
   	  $query = "SELECT 
                SUM(ABS(Rekeningmutaties.Credit) * Rekeningmutaties.Valutakoers $koersQuery)  - SUM(ABS(Rekeningmutaties.Debet) * Rekeningmutaties.Valutakoers $koersQuery)  as totaal,
   	            ...
   	            SUM( IF(Grootboekrekeningen.Kruispost = 1,(ABS(Rekeningmutaties.Credit)-ABS(Rekeningmutaties.Debet)) * Rekeningmutaties.Valutakoers,0)) AS internTotaal
 	              FROM (Rekeningen, Portefeuilles) 
                JOIN Rekeningmutaties on Rekeningmutaties.Rekening = Rekeningen.Rekening 
                JOIN Grootboekrekeningen ON Rekeningmutaties.Grootboekrekening = Grootboekrekeningen.Grootboekrekening
	     ...
	     $AttributieStortingenOntrekkingen['storting'] +=$data['storting'];
	     $AttributieStortingenOntrekkingen['onttrekking'] +=$data['onttrekking'];
       $interneboeking+=$data['internTotaal'];
...
$somItems=array('resultaat','storting','onttrekking','kosten','opbrengst','stort','interneboeking');
