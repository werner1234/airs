...
$Log: RapportGRAFIEK_L67.php,v $
Revision 1.4  2020/06/24 13:02:41  rvv
*** empty log message ***

Revision 1.3  2019/11/06 16:11:20  rvv
*** empty log message ***

Revision 1.2  2019/04/07 11:06:41  rvv
*** empty log message ***

Revision 1.1  2019/04/06 17:11:28  rvv
*** empty log message ***

Revision 1.2  2019/01/09 15:52:19  rvv
*** empty log message ***
...
class RapportGRAFIEK_L67
{
	function RapportGRAFIEK_L67($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function formatGetalKoers($waarde, $dec , $start = false)
    ...
    $this->waarden['Historie']=$att->bereken($rapportageStartJaar,  $this->rapportageDatum,'EUR','Beleggingscategorien','maanden');
    ...
    $effecten=array('allocatieEffect','selectieEffect','interactieEffect');
    foreach ($this->waarden['Historie'] as $categorie=>$categorieData)
    { 
      $laatste=array();
       ...
       foreach ($categorieData['perfWaarden'] as $datum=>$waarden)
      {
        ...
        { 
         // echo $categorie."<br>\n";
          //listarray($waarden);
         // $attributieEffect=(($waarden['weging']*$waarden['procentBruto'])-($waarden['indexBijdrageWaarde']*$waarden['indexPerf']))*100;
          //echo "$categorie $attributieEffect=((".$waarden['weging']."*".$waarden['procentBruto'].")-(".$waarden['indexBijdrageWaarde']."*".$waarden['indexPerf']."))*100;<br>";
          ...
          $maandEffecten=array('allocatieEffect'=>$allocatieEffect,'selectieEffect'=>$selectieEffect,'interactieEffect'=>$interactieEffect);
      ...
      //  echo " $attributieEffect=((".$waarden['weging']."*".$waarden['procentBruto'].")-(".$waarden['indexPerf']."*".$waarden['indexBijdrageWaarde']."))*100 <br>\n";
       ...
       //   $attributieEffect=$allocatieEffect+$selectieEffect+$interactieEffect;
        //  echo "b: $attributieEffect  $categorie $datum<br>\n"; ob_flush();
          ...
          {
            $stapelTotalen[$categorie][$jaar][$effect]=((1+$maandEffecten[$effect]/100)*(1+$stapelTotalen[$categorie][$jaar][$effect]/100)-1)*100;
          }
       //   listarray($stapelTotalen);
    ...
    foreach($stapelTotalen as $categorie=>$jaarData)
    {

      foreach($jaarData as $jaar=>$effectData)
      {
        foreach ($effectData as $effect => $waarde)
        {
          
          $this->jaarTotalen[$categorie][$jaar]['attributieEffect'] += $waarde;
  
          $this->jaarTotalen['totaal'][$jaar]['attributieEffect'] += $waarde;
          $this->jaarTotalen['totaal'][$jaar][$effect] += $waarde;
          
          
          $this->jaarTotalen[$categorie][$jaar][$effect] = $waarde;
        }
      }
    }
 //   listarray($stapelTotalen);
...
*/
    ...
    $this->pdf->SetWidths(array(48.1429,28.6071,28.6071,28.6071,28.6071,28.6071,28.6071,28.6071,28.6071));
  ...
  // 	$this->pdf->ln(5);
    ...
    $this->pdf->row(array("",vertaalTekst("Tactische\nWeging",$this->pdf->rapport_taal),vertaalTekst("Strategische\nWeging",$this->pdf->rapport_taal),vertaalTekst("Rendement\nPortefeuille",$this->pdf->rapport_taal),
                      ...
                      vertaalTekst("Selectie\neffect",$this->pdf->rapport_taal),vertaalTekst("Interactie\neffect",$this->pdf->rapport_taal)));
...
while($data=$DB->nextRecord())
  ...
  $categorieLookup['geen-Hcat']='Geen hoofdcategorie';
    ...
    $categorieLookup= $att->categorien;
    
//listarray($this->jaarTotalen);listarray($categorieLookup);
   //    foreach ($stapelTotalen as $categorie=>$jaarWaarden)
   foreach ($this->jaarTotalen as $categorie=>$jaarWaarden)
    {
//echo "$categorie <br>\n";
      $waarden=$jaarWaarden[$startJaar];
      if(round($att->normData[$categorie],2)<>0.0 || round($waarden['weging'],4) <> 0)
      {
        $this->pdf->row(array(vertaalTekst($categorieLookup[$categorie],$this->pdf->rapport_taal) ,
