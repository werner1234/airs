...
$Log: RapportVKMS_L67.php,v $
Revision 1.6  2020/06/24 13:02:42  rvv
*** empty log message ***

Revision 1.5  2019/10/18 17:40:37  rvv
*** empty log message ***

Revision 1.4  2019/02/03 15:50:24  rvv
*** empty log message ***

Revision 1.3  2019/01/24 11:09:36  cvs
kleuren grafiek
...
class RapportVKMS_L67
{
	function RapportVKMS_L67($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
    ...
    if($this->pdfVullen==true)
		{
	  	...
	  	$this->pdf->row(array(vertaalTekst('Indirecte (fonds)kosten',$this->pdf->rapport_taal), $this->formatGetal($totaalDoorlopendekosten, 0,false,true) . ' EUR'));
			$this->pdf->excelData[]=array('','Indirecte (fonds)kosten', '','',round($totaalDoorlopendekosten, 0), 'EUR');
			$this->pdf->row(array(vertaalTekst('Indirecte (fonds)kosten ten opzichte van onderliggend vermogen',$this->pdf->rapport_taal), $this->formatGetal($doorlopendeKostenPercentage * 100, 2,false,true) . ' %'));
