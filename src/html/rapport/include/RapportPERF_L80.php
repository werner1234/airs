...
$Log: RapportPERF_L80.php,v $
Revision 1.5  2020/03/14 18:42:03  rvv
*** empty log message ***

Revision 1.4  2019/12/28 11:28:02  rvv
*** empty log message ***
...
class RapportPERF_L80
{
  
  function RapportPERF_L80($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
  {
    global $__appvar;
    ...
    $query="SELECT Beleggingscategorie as Beleggingscategorie,BeleggingscategorieOmschrijving as Omschrijving
FROM TijdelijkeRapportage WHERE Portefeuille='".$this->portefeuille."' AND Beleggingscategorie <>'' GROUP BY BeleggingscategorieVolgorde   ORDER BY BeleggingscategorieVolgorde asc"; //WHERE Beleggingscategorie IN('LIQ','ZAK','VAR','Liquiditeiten')
  ...
  function createRows()
  {
    ...
    foreach($this->waarden['rapportagePeriode'] as $cat=>$catData)
    {
      if($catData['procent']==0 && $catData['beginwaarde']==0 && $catData['eindwaarde']==0)
      {
        unset($this->waarden['rapportagePeriode'][$cat]);
        unset($this->categorien[$cat]);
      }
    }
    ...
    $row['kopregel'] = array("","",vertaalTekst('Totaal',$this->pdf->rapport_taal),'');
    
    $pageSets=array();
    $aantalPaginas=ceil(count($this->categorien)/$maxCols);
    ...
    foreach ($this->categorien as $categorie=>$omschrijving)
    {
      ...
      if($categorie<>'totaal')
      {
        $pageSets[$j]['kopregel'][] = vertaalTekst($omschrijving, $this->pdf->rapport_taal);
        $pageSets[$j]['kopregel'][] = '';
      }
      $i++;
    }
    ...
    foreach ($this->categorien as $categorie=>$omschrijving)
    {
      ...
      if($categorie == 'totaal')
      {
        ...
        $liquiditeitenMutatie -= $this->waarden['rapportagePeriode'][$categorie]['stort'];
        
      }
     // echo "$categorie $liquiditeitenMutatie<br>\n";
    }
    ...
    foreach ($this->categorien as $categorie=>$omschrijving)
    {
      ...
      if($categorie=='totaal')
      {
        ...
        for($k=0;$k<$aantalPaginas;$k++)
        {
          $pageSets[$k]['totaalStortingen'][]=$this->formatGetal($this->waarden['rapportagePeriode'][$categorie]['storting'],$this->bedragDecimalen);
          $pageSets[$k]['totaalOnttrekkingen'][]=$this->formatGetal($this->waarden['rapportagePeriode'][$categorie]['onttrekking'],$this->bedragDecimalen);
          $pageSets[$k]['totaalStortingen'][]='';
          $pageSets[$k]['totaalOnttrekkingen'][]='';
          $pageSets[$k]['totaalMutaties'][]='';
          $pageSets[$k]['totaalMutaties'][]='';
          $pageSets[$k]['directeOpbrengsten'][]='';
          $pageSets[$k]['directeOpbrengsten'][]='';
          $pageSets[$k]['toegerekendeKosten'][]='';
          $pageSets[$k]['toegerekendeKosten'][]='';
          $pageSets[$k]['resultaatBruto'][]='';
          $pageSets[$k]['resultaatBruto'][]='';
          $pageSets[$k]['rendementProcentBruto'][]='';
          $pageSets[$k]['rendementProcentBruto'][]='';
  
          $pageSets[$k]['totaalNettoRentement'][]=$this->formatGetal($this->waarden['rapportagePeriode'][$categorie]['procent'],2);
          if(isset($this->waarden['rapportagePeriode']['Liq-Extern']))
            $pageSets[$k]['rendementProcent'][]=$this->formatGetal($this->waarden['rapportagePeriode']['Liq-Extern']['procentZonderExtLiq'],2);
          else
            $pageSets[$k]['rendementProcent'][]=$this->formatGetal($this->waarden['rapportagePeriode'][$categorie]['procent'],2);
          $pageSets[$k]['totaalNettoRentement'][]='%';
  
          $pageSets[$k]['waardeVanaf'][]=$this->formatGetal($this->waarden['rapportagePeriode'][$categorie]['beginwaarde'],$this->bedragDecimalen,true);
          $pageSets[$k]['waardeVanaf'][]='';
          $pageSets[$k]['waardeTot'][]=$this->formatGetal($this->waarden['rapportagePeriode'][$categorie]['eindwaarde'],$this->bedragDecimalen);
          $pageSets[$k]['waardeTot'][]='';
          $pageSets[$k]['mutatiewaarde'][]=$this->formatGetal($this->waarden['rapportagePeriode'][$categorie]['mutatie'],$this->bedragDecimalen);
          $pageSets[$k]['mutatiewaarde'][]='';
  

          $pageSets[$k]['resultaatVerslagperiode'][]=$this->formatGetal($resultaatVerslagperiode[$categorie],$this->bedragDecimalen);
          $pageSets[$k]['resultaatVerslagperiode'][]='';
        }
      ...
      else
      {
        ...
        if($categorie == 'H-Liq')
        {
          ...
          $pageSets[$j]['totaalOnttrekkingen'][]=$this->formatGetal($liqOnttrekkingen,$this->bedragDecimalen);
          $pageSets[$j]['totaalStortingen'][]=$this->formatGetal($liqStorting,$this->bedragDecimalen);
          $pageSets[$j]['totaalOnttrekkingen'][]='';
          $pageSets[$j]['totaalStortingen'][]='';
        }
        elseif($categorie=='Liq-Extern')
        {
          $pageSets[$j]['totaalStortingen'][]=$this->formatGetal($this->waarden['rapportagePeriode'][$categorie]['storting']*-1,$this->bedragDecimalen);
          $pageSets[$j]['totaalOnttrekkingen'][]=$this->formatGetal($this->waarden['rapportagePeriode'][$categorie]['onttrekking']*-1,$this->bedragDecimalen);
        ...
        if($categorie=='Liquiditeiten' || $categorie == 'H-Liq')
          $pageSets[$j]['totaalMutaties'][]=$this->formatGetal($liquiditeitenMutatie,$this->bedragDecimalen);
        elseif($categorie=='Liq-Extern')
          $pageSets[$j]['totaalMutaties'][]=$this->formatGetal(0,$this->bedragDecimalen);
        else
          $pageSets[$j]['totaalMutaties'][]=$this->formatGetal($this->waarden['rapportagePeriode'][$categorie]['storting']+$this->waarden['rapportagePeriode'][$categorie]['onttrekking'],$this->bedragDecimalen);
  
        $pageSets[$j]['totaalMutaties'][]='';
  
        $pageSets[$j]['waardeVanaf'][]=$this->formatGetal($this->waarden['rapportagePeriode'][$categorie]['beginwaarde'],$this->bedragDecimalen,true);
        $pageSets[$j]['waardeVanaf'][]='';
        $pageSets[$j]['waardeTot'][]=$this->formatGetal($this->waarden['rapportagePeriode'][$categorie]['eindwaarde'],$this->bedragDecimalen);
        $pageSets[$j]['waardeTot'][]='';
        $pageSets[$j]['mutatiewaarde'][]=$this->formatGetal($this->waarden['rapportagePeriode'][$categorie]['mutatie'],$this->bedragDecimalen);
        $pageSets[$j]['mutatiewaarde'][]='';

        $pageSets[$j]['resultaatVerslagperiode'][]=$this->formatGetal($resultaatVerslagperiode[$categorie],$this->bedragDecimalen);
        $pageSets[$j]['resultaatVerslagperiode'][]='';
      }
      ...
      {
        $pageSets[$j]['rendementProcent'][]='';
        $pageSets[$j]['rendementProcent'][]='';
      }
      else
      {
        if($categorie<>'totaal')
          $pageSets[$j]['rendementProcent'][]=$this->formatGetal($this->waarden['rapportagePeriode'][$categorie]['procent'],2);
      ...
      if ($categorie <> 'totaal')
      {
        ...
        if($categorie=='Liquiditeiten' || $categorie == 'H-Liq')
        {
          $pageSets[$j]['rendementProcentBruto'][]='';
          $pageSets[$j]['rendementProcentBruto'][]='';
          $pageSets[$j]['directeOpbrengsten'][]='';
          $pageSets[$j]['directeOpbrengsten'][]='';
          $pageSets[$j]['toegerekendeKosten'][]=$this->formatGetal(-1*$this->waarden['rapportagePeriode'][$categorie]['kosten'],$this->bedragDecimalen);
          $pageSets[$j]['toegerekendeKosten'][]='';
          $liqBruto=$this->waarden['rapportagePeriode'][$categorie]['mutatie']+$liqStorting-$liqOnttrekkingen+$liquiditeitenMutatie;
          $pageSets[$j]['resultaatBruto'][]=$this->formatGetal($liqBruto,$this->bedragDecimalen);
          $pageSets[$j]['resultaatBruto'][]='';
        ...
        else
        {
          ...
          $pageSets[$j]['rendementProcentBruto'][]=$this->formatGetal($this->waarden['rapportagePeriode'][$categorie]['procentBruto'],2);
          $pageSets[$j]['rendementProcentBruto'][]='%';
       
          $pageSets[$j]['directeOpbrengsten'][]=$this->formatGetal($this->waarden['rapportagePeriode'][$categorie]['opbrengst'],$this->bedragDecimalen);
          $pageSets[$j]['directeOpbrengsten'][]='';
          $pageSets[$j]['toegerekendeKosten'][]=$this->formatGetal(-1*$this->waarden['rapportagePeriode'][$categorie]['kosten'],$this->bedragDecimalen);
          $pageSets[$j]['toegerekendeKosten'][]='';
          $pageSets[$j]['resultaatBruto'][]=$this->formatGetal($this->waarden['rapportagePeriode'][$categorie]['resultaatBruto'],$this->bedragDecimalen);
          $pageSets[$j]['resultaatBruto'][]='';
        }
  ...
  function writeRapport()
  {
    ...
    $att=new ATTberekening_L80($this);
    $hcatData=$att->bereken($this->rapportageDatumVanaf,$this->rapportageDatum,'Categorieen',$this->pdf->rapportageValuta);
...
//listarray($this->pdf->hcatData);exit;
    $pW=5;
    $portW=21.5;
    $this->pdf->widthA = array(1,70,$portW,$pW,$portW,$pW,$portW,$pW,$portW,$pW,$portW,$pW,$portW,$pW,$portW,$pW,$portW,$pW,$portW,$pW,$portW,$pW);
    ...
    $paginas=$this->createRows();
    
    
    foreach($paginas as $i=>$row)
    {
      if($i>0)
        $this->pdf->addPage();
  
      $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
      $this->pdf->row($row['kopregel']);
      $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
...
//    $row['rendementProcentBruto'][13]='%';
      ...
      $this->pdf->CellBorders = array('', '', 'UU', '', 'UU', '', 'UU', '', 'UU', '', 'UU', '', 'UU', '', 'UU', '', 'UU');
