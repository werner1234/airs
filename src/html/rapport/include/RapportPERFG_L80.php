...
$Log: RapportPERFG_L80.php,v $
Revision 1.4  2020/05/16 15:57:02  rvv
*** empty log message ***

Revision 1.3  2019/05/08 15:11:07  rvv
*** empty log message ***
...
class RapportPERFG_L80
{
	function RapportPERFG_L80($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//listarray($extraIndicesPerformance);
  ...
  if(count($rendamentWaarden) > 0)
   {
     ...
     $totaal=array();
        ...
        $kleurExtraIndices=$this->LineDiagram(270, 60, $grafiekData,$kleuren,0,0,6,5,1);//50
...
function LineDiagram($w, $h, $data, $color=null, $maxVal=0, $minVal=0, $horDiv=4, $verDiv=4,$jaar=0)
  {
    ...
    $extraIndices=$data['extraIndices'];
    $data = $data['Index'];
    if(count($data1)>0)
    ...
    foreach($extraIndices as $i=>$indices)
    {
       foreach($indices as $fonds=>$rendement)
       {
         if(!isset($extraIndicesKleur[$fonds]))
         {
           $query = "SELECT grafiekKleur,Omschrijving FROM BeleggingscategoriePerFonds JOIN Fondsen ON BeleggingscategoriePerFonds.Fonds=Fondsen.Fonds WHERE BeleggingscategoriePerFonds.Fonds='$fonds' AND Vermogensbeheerder='" . $this->pdf->portefeuilledata['Vermogensbeheerder'] . "'";
           $db->SQL($query);
