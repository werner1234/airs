...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: ATTberekening_L19.php,v $
 		Revision 1.4  2017/05/26 16:45:07  rvv
 		*** empty log message ***
 		
 		Revision 1.3  2016/06/22 16:15:05  rvv
 		*** empty log message ***
 		
 		Revision 1.2  2016/06/07 04:57:35  rvv
 		*** empty log message ***
 ...
 */
class rapportHulp_L19
{
  function rapportHulp_L19()
  {
    $this->test='';
  }
}
...
class ATTberekening_L19
{

	function ATTberekening_L19($rapportData)
	{
  ...
  {
    $this->rapport = new rapportHulp_L19();
    $db=new DB();
...
ORDER BY HoofdBeleggingscategorien.Afdrukvolgorde, Regios.Afdrukvolgorde,Beleggingscategorien.Afdrukvolgorde";
    ...
    foreach($perfData as $categorie=>$categorieTotalen)
    {
      $benchmarkCategorie[$categorie]=0;
      foreach($categorieTotalen['perfWaarden'] as $datum=>$maandWaarden)
      {
        $benchmarkDatum[$datum]+=$maandWaarden['indexBijdrage'];

        $benchmarkCategorie[$categorie]=((1+$benchmarkCategorie[$categorie]) * (1+$maandWaarden['indexPerf']))-1;
        $benchmarkCategorieVerdeling[$categorie]=$maandWaarden['indexBijdrageWaarde'];
      }
    }
    //listarray($benchmarkCategorie);

    //listarray($perfData);exit;

    $perfData['totaal'] = $this->fondsPerformance($alleData,$van,$tot,true,true);
    $indexPerf=0;
    foreach($perfData['totaal']['perfWaarden'] as $datum=>$maandWaarden)
    {
      $perfData['totaal']['perfWaarden'][$datum]['indexPerf']=$benchmarkDatum[$datum];
      $indexPerf=((1+$indexPerf)*(1+$benchmarkDatum[$datum]))-1;
    }
   // listarray($perfData['totaal']);
    $perfData['totaal']['indexPerf']=$indexPerf*100;
    $perfData['totaal']['benchmarkCategoriePerf']=$benchmarkCategorie;
    $perfData['totaal']['benchmarkCategorieVerdeling']=$benchmarkCategorieVerdeling;
...
IndexPerBeleggingscategorie.vanaf <= '$van' and
...
foreach ($perioden as $periode)
{
   ...
   // $weging = $DB->NextRecord();
    $totaalGemiddelde = $totaalEindwaarde;//$totaalBeginwaarde + $weging['totaal1'];//aangepast naar eindwaarde.
...
listarray($indexData);
      ...
      $weging=$eindwaarde/$totaalGemiddelde;//$this->totalen['gemiddeldeWaarde'];
...
$laatsteItems=array('weging','indexBijdrageWaarde');
...
foreach ($waarden as $datum=>$waarde)
{
  ...
  foreach($laatsteItems as $item)
   $perfData['totaal'][$item]=$waarde[$item]*100;
}
