...
/*
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2015/01/21 16:53:08 $
 		File Versie					: $Revision: 1.3 $

 		$Log: RapportCASHY_L53.php,v $
 		Revision 1.3  2015/01/21 16:53:08  rvv
 		*** empty log message ***
...
class RapportCASHY_L53
{
	function RapportCASHY_L53($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	 //
		$this->pdf = &$pdf;
		...
		$this->pdf->rapport_titel = "Kasstroom projectie";
    ...
    for($jaar=$rapJaar;$jaar<=($rapJaar+1);$jaar++)
    {
      ...
      $kwartaal=ceil($maand/3);
		  ...
		  if($jaar==$rapJaar||$jaar==$rapJaar+1)
		    $cashflowKwartalen[$jaar.'Q'.$kwartaal][$regel[2]] +=$regel[3];
    ...
    $this->pdf->SetWidths(array(150,15,25,25,25,25,25));

		$this->pdf->SetAligns(array('L','L','R','R','R','R','R'));
    $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
    $this->pdf->Row(array('','Periode','Coupon','Lossing','Totaal','Cumulatief'));
    $this->pdf->Line($this->pdf->marge+150,$this->pdf->GetY(),$this->pdf->marge+265,$this->pdf->GetY(),array('color'=>$this->pdf->rapport_balkKleur));
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
    $this->pdf->SetFillColor($this->pdf->rapport_regelKleur[0],$this->pdf->rapport_regelKleur[1],$this->pdf->rapport_regelKleur[2]);
		foreach ($cashflowKwartalen as $kwartaal=>$waarden)
		{
		  if($fill==true)
		  {
      ...
      $this->pdf->Row(array('',$kwartaal,$this->formatGetal($waarden['rente'],0),
...
$this->pdf->SetLineStyle(array('dash' => 0,'color'=>array(0,0,0)));
    ...
    $this->VBarDiagram(160,60,$cashflowKwartalen,"Kasstroom projectie op kwartaal basis");
		$this->pdf->setXY(20,185);
    $this->VBarDiagram(160,60,$cashflowJaar,"Kasstroom projectie op jaarbasis");
	}
	...
	function VBarDiagram($w, $h, $data,$titel)
  {
      ...
      $colors=array('rente'=>array(102,102,102),
      'lossing'=>array(255,204,0));
        ...
        if(!isset($YstartGrafiekLast[$datum]))
         ...
         if($legendaPrinted[$datum] != 1)
         {
           if(strlen($legenda[$datum])==4)
             ...
             $this->pdf->TextWithRotation($xval-0.75,$YstartGrafiek+7.0,$legenda[$datum],45);  
         }
