...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: RapportGRAFIEK_L51.php,v $
 		Revision 1.14  2019/10/23 13:34:01  rvv
 		*** empty log message ***
 		
 		Revision 1.13  2018/08/18 12:40:14  rvv
 		php 5.6 & consolidatie
 		
 		Revision 1.12  2017/06/21 16:10:36  rvv
 		*** empty log message ***
 		
 		Revision 1.10  2017/03/31 15:39:22  rvv
 		*** empty log message ***

 		Revision 1.9  2016/01/04 06:31:07  rvv
 		*** empty log message ***
 		
 		Revision 1.8  2015/12/20 16:46:36  rvv
 		*** empty log message ***
 		
 		Revision 1.7  2015/03/18 16:29:56  rvv
 		*** empty log message ***
 		
 		Revision 1.6  2015/03/14 17:01:49  rvv
 		*** empty log message ***
...
class RapportGRAFIEK_L51
{
	function RapportGRAFIEK_L51($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//if ($this->pdf->rapport_GRAFIEK_sortering == 1)
  $order = 'Valutas.Afdrukvolgorde asc, Valutas.Omschrijving asc';
//else
//  $order = 'WaardeEuro desc';
...
$this->pdf->setXY($this->pdf->marge+(($lwb/4)*1.5-$chartsize/2),$headerHeight);
...
{  
  ...
  PieChart_L51($this->pdf,$chartsize,$vwh,$grafiekData['OIB']['Percentage'],'%l',$grafiekData['OIB']['Kleur'],vertaalTekst('Beleggingscategorie',$this->pdf->rapport_taal),$legendaStart);
}

$this->pdf->setXY($this->pdf->marge+(($lwb/4)*5.5-$chartsize/2),$headerHeight);
...
$this->pdf->setXY($this->pdf->marge+(($lwb/4)*1.5-$chartsize/2),$headerHeight+$vwh-10);
...
{ 
  ...
  PieChart_L51($this->pdf,$chartsize,$vwh,$grafiekData['OIV']['Percentage'],'%l',$grafiekData['OIV']['Kleur'],vertaalTekst('Valuta',$this->pdf->rapport_taal),$legendaStart);
}

if(isset($grafiekData['OIS2']))
{
  $this->pdf->setXY($this->pdf->marge+(($lwb/4)*5.5-$chartsize/2),$headerHeight+$vwh-10);
 //  $this->pdf->setXY($this->pdf->marge+(($lwb/4)*7-$chartsize/2),$headerHeight);
  ...
  { 
    ...
    PieChart_L51($this->pdf,$chartsize,$vwh,$grafiekData['OIS2']['Percentage'],'%l',$grafiekData['OIS2']['Kleur'],vertaalTekst('Sector',$this->pdf->rapport_taal),$legendaStart);
  }
}
