...
$Log: RapportVKMS_L24.php,v $
Revision 1.5  2019/02/03 15:50:24  rvv
*** empty log message ***

Revision 1.4  2019/01/23 16:27:16  rvv
*** empty log message ***
...
class RapportVKMS_L24
{
	function RapportVKMS_L24($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    ...
    if($spreadKostenEUR <> 0)
		...
		if($this->pdfVullen==true)
		{
			...
			$this->pdf->ln();
      $this->pdf->row(array(vertaalTekst('Indirecte (fonds)kosten factor van de portefeuille',$this->pdf->rapport_taal),  $this->formatGetal($vkmPercentagePortefeuille*$gemiddelde/100, 0), $this->formatGetal($vkmPercentagePortefeuille, 2) . ' %'));
      $this->pdf->ln();
			$this->pdf->excelData[]=array();
			...
			$this->VBarVerdeling(50,$startYGrafiek-$startY,$barData);
...
foreach ($perCategorie as $hoofdcategorie=>$categorieData)
{
 ...
 WHERE Portefeuilles.portefeuille='".$this->portefeuille."'";
    ...
    for($i=$this->pdf->rapport_jaar; $i<=$doelJaar; $i++)
		...
		if($this->pdf->getY()+75>$this->pdf->pagebreak)
			$this->pdf->addPage();

		$this->pdf->setXY(22,$this->pdf->getY()+15);
