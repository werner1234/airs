...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: ATTberekening_L36.php,v $
 		Revision 1.10  2015/12/19 08:29:17  rvv
 		*** empty log message ***
 		
 		Revision 1.9  2015/12/09 19:46:07  rvv
 		*** empty log message ***
 		
 		Revision 1.8  2015/09/30 15:53:11  rvv
 		*** empty log message ***
 		
 		Revision 1.7  2015/09/23 15:05:33  rvv
 		*** empty log message ***
 		
 		Revision 1.6  2015/09/20 17:32:28  rvv
 		*** empty log message ***
 		
 		Revision 1.5  2015/08/30 11:44:35  rvv
 		*** empty log message ***
...
class ATTberekening_L36
{

	function ATTberekening_L36($rapportData)
	{
	...
	function bereken($van,$tot,$valuta='EUR',$stapeling='categorie',$kostenInPerf=true)
	{
	  $this->rapport_jaar =substr($van,0,4);
    $this->kostenInPerf=$kostenInPerf;
	  global $__appvar;
 		$DB=new DB();
		$query="SELECT
...
*/
      ...
      if($this->kostenInPerf==false)
        $FondsDirecteKostenOpbrengsten['kostenTotaal']=0;

      if($catnaam=='totaal')
        $filter=" AND Rekeningmutaties.Grootboekrekening = ''";
      else
        $filter=" AND Rekeningmutaties.Grootboekrekening = 'FONDS' ";  
	    ...
	    $queryAttributieStortingenOntrekkingen = "SELECT ".
	              ...
	              "Rekeningmutaties.Boekdatum > '".$datumBegin."' AND ".
	              "Rekeningmutaties.Boekdatum <= '".$datumEind."' AND ".
	              " $rekeningFondsenWhere $filter";//Rekeningmutaties.Grootboekrekening = 'FONDS' AND
     ...
     // listarray($AttributieStortingenOntrekkingen);
       ...
       if($catnaam=='totaal')
        $filter=" ";
       elseif($this->kostenInPerf==false)
        $filter="OR (Rekeningmutaties.Fonds <> '' AND Grootboekrekeningen.Kosten <> 1)";  
       else
        $filter="OR  Rekeningmutaties.Fonds <> ''";  
   	  ...
   	  $query = "SELECT SUM(ABS(Rekeningmutaties.Credit) * Rekeningmutaties.Valutakoers $koersQuery)  - SUM(ABS(Rekeningmutaties.Debet) * Rekeningmutaties.Valutakoers $koersQuery) as totaal,
	              ...
	              $rekeningRekeningenWhere  AND
 	              Rekeningmutaties.Verwerkt = '1' AND
	              Rekeningmutaties.Boekdatum > '$datumBegin' AND
	               Rekeningmutaties.Boekdatum <= '$datumEind' AND (Grootboekrekeningen.Storting=1 OR Grootboekrekeningen.Onttrekking=1 $filter  )";
	    $DB->SQL($query);
	    $DB->Query();
	    $data = $DB->nextRecord();
...
$alleKosten=array('kostenTotaal'=>0);

if($catnaam=='geen-Hcat'||$catnaam=='Liquiditeiten'||$catnaam=='G-LIQ')
{
      ...
      if($catnaam=='totaal')
        ...
        $performance = ((($eindwaarde - $beginwaarde) - $AttributieStortingenOntrekkingen['totaal'] +
            $FondsDirecteKostenOpbrengsten['kostenTotaal'] + $FondsDirecteKostenOpbrengsten['opbrengstTotaal'] - $alleKosten['kostenTotaal']) / $gemiddelde);

if($catnaam=='geen-Hcat')
{
//echo $queryKostenOpbrengsten."<br>\n";
//echo  round($performance*100,2)." $gemiddelde = $beginwaarde - ".$AttributieStortingenOntrekkingen['gewogen']."<br>\n";
//echo "$catnaam ".round($performance*100,2)."= ((($eindwaarde - $beginwaarde) - ". $AttributieStortingenOntrekkingen['totaal']." + ".$FondsDirecteKostenOpbrengsten['kostenTotaal']." + ".$FondsDirecteKostenOpbrengsten['opbrengstTotaal']." - ".$alleKosten['kostenTotaal'].") / $gemiddelde) <br>\n";
}
      ...
      $eindWeging=$eindwaarde/$totaalEindwaarde;
      //echo "$catnaam $weging=$gemiddelde/$totaalGemiddelde <br>\n";
      $resultaatOld=($eindwaarde - $beginwaarde) - $AttributieStortingenOntrekkingen['totaal'];
      ...
      if($catnaam=='totaal')
        ...
        $resultaat=($eindwaarde - $beginwaarde) - $AttributieStortingenOntrekkingen['totaal'] +
                    $FondsDirecteKostenOpbrengsten['kostenTotaal'] + $FondsDirecteKostenOpbrengsten['opbrengstTotaal'] - $alleKosten['kostenTotaal'];
      
     // if($resultaatOld <> $resultaat)
     //   echo "$catnaam $datumEind $resultaatOld $resultaat <br>";
