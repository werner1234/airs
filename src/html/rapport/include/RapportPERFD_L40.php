...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2016/01/20 17:16:44 $
File Versie					: $Revision: 1.3 $

$Log: RapportPERFD_L40.php,v $
Revision 1.3  2016/01/20 17:16:44  rvv
*** empty log message ***
...
include_once('../html/rapport/include/PreProcessor_L40.php');

class RapportPERFD_L40
{
	function RapportPERFD_L40($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
     ...
     //   $this->pdf->CellBorders = array('','US','US','US','US','US','US','US','US','US','US','US');
       ...
       //$this->pdf->SetFillColor(230,230,230);
        ...
        $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
  		  ...
  		  $this->pdf->Line($this->pdf->marge, $this->pdf->GetY(),$this->pdf->marge+277-50,$this->pdf->GetY());
...
function formatGetalLength ($getal,$decimaal,$gewensteLengte)
{
 ...
 return number_format($getal,$decimaal,',','');
  ...
  $julEind = db2jul($datumEind);
    ...
    if($portefeuilleStartJul > db2jul($beginDatum) && $portefeuilleStartJul  < db2jul($eindDatum))
    {
      $beginDatum=substr($this->pdf->PortefeuilleStartdatum,0,10);
      $wegingsDatum=date('Y-m-d',$portefeuilleStartJul+86400);
    }
    else
    {
      if(db2jul($beginDatum) < $portefeuilleStartJul)
        $wegingsDatum=substr($this->pdf->PortefeuilleStartdatum,0,10);
      else
    ...
    vulTijdelijkeTabel($fondswaarden['beginmaand'],$portefeuille,$beginDatum);
    ...
    $processor=new PreProcessor_L40($portefeuille,$eindDatum); 
    $processor=new PreProcessor_L40($portefeuille,$beginDatum); 
    $fondswaarden=array();
    $query="SELECT * FROM TijdelijkeRapportage WHERE 
    portefeuille = '$portefeuille' AND rapportageDatum = '$beginDatum' ".$__appvar['TijdelijkeRapportageMaakUniek']."";
    $DB=new DB();
    $DB->SQL($query);
    $DB->Query();
    while($data=$DB->nextRecord())
      $fondswaarden['beginmaand'][]=$data;
    $query="SELECT * FROM TijdelijkeRapportage WHERE 
    portefeuille = '$portefeuille' AND rapportageDatum = '$eindDatum' ".$__appvar['TijdelijkeRapportageMaakUniek']."";
    $DB=new DB();
    $DB->SQL($query);
    $DB->Query();
    while($data=$DB->nextRecord())
      $fondswaarden['eindmaand'][]=$data;
  ...
  $gemiddelde = $totaalWaarde['begin'] + $weging['totaal1'];
    ...
    //echo "PERFD ".$data['periodeForm']." $wegingsDatum $performance (((".$totaalWaarde['eind']." - ".$totaalWaarde['begin'].") - ".$weging[totaal2].") / $gemiddelde) * 100;.<br>\n";
    return $data;
  ...
  function VBarDiagram($w, $h, $data)
  {
      ...
      foreach (($this->categorieVolgorde) as $categorie)//array_reverse
      {
        if(is_array($grafiekCategorie[$categorie]))
        {
          ...
          $this->pdf->MultiCell(55, 4,$this->categorieOmschrijving[$categorie],0,'L');
          $n++;
        }
      }
