...
class RapportAFM_L12
{
	function RapportAFM_L12($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
TijdelijkeRapportage.Lossingsdatum, TijdelijkeRapportage.fondsOmschrijving asc";
    ...
    foreach($dataregel as $data)
    {
      $query = "SELECT
          ...
          SUM(if(Grootboekrekeningen.Opbrengst =1,ABS(Rekeningmutaties.Credit) - ABS(Rekeningmutaties.Debet),0)) as opbrengstTotaalValuta,
...
'',
                        ...
                        $this->formatGetal($aankoopValuta/$commitmentValuta*100,1),
												"",
                        $this->formatGetal($tansacties[$data['fonds']]['verkoop']['valuta']*-1+$extra['opbrengstTotaalValuta'],0),
                        $this->formatGetal($tansacties[$data['fonds']]['verkoop']['EUR']*-1+$extra['opbrengstTotaal'],0),
                        '',//$this->formatGetal(,0),
...
//echo $data['fondsOmschrijving']." | ".$data['actuelePortefeuilleWaardeEuro']."+".$extra['opbrengstTotaal'].")/$aankoopEur <br>\n";
    ...
    $this->pdf->row(array('','','','',
                      ...
                      '','','',
                      $this->formatGetal($totalen['verkopen']*-1+$totalen['opbrengstTotaal'],0),
                      '',//$this->formatGetal(,0),
                      '',
    ...
    $chartFunction='PieChart_L'.$this->pdf->portefeuilledata['Layout'];
    if(function_exists($chartFunction))
      $chartFunction($this->pdf, $chartsize, $vwh, $grafiek['Aanvang']['Percentage'], '%l', $grafiek['Aanvang']['Kleur'], vertaalTekst('Aanvang', $this->pdf->rapport_taal), $legendaStart);

    $i=1;
    $this->pdf->setXY($this->pdf->marge+5+90*$i , $ystart);
    $legendaStart=array($this->pdf->marge+5+90*$i,$ystart+$chartsize+10);
    if(function_exists($chartFunction))
      $chartFunction($this->pdf, $chartsize, $vwh, $grafiek['Strategie']['Percentage'], '%l', $grafiek['Strategie']['Kleur'], vertaalTekst('Strategie', $this->pdf->rapport_taal), $legendaStart);
...
JOIN Portefeuilles ON contractueleUitsluitingen.Portefeuille=Portefeuilles.Portefeuille AND Portefeuilles.Einddatum>'".$this->rapportageDatum."'
LEFT JOIN Fondsen ON contractueleUitsluitingen.fonds=Fondsen.Fonds
LEFT JOIN TijdelijkeRapportage ON TijdelijkeRapportage.fonds=Fondsen.Fonds AND TijdelijkeRapportage.type='fondsen' AND TijdelijkeRapportage.rapportageDatum='".$this->rapportageDatum."'
...
TijdelijkeRapportage.Lossingsdatum, TijdelijkeRapportage.fondsOmschrijving asc";
    ...
    $query="SELECT (Rekeningmutaties.Debet*Rekeningmutaties.Valutakoers) as Bedrag,Rekeningmutaties.Debet,Rekeningmutaties.Fonds,Rekeningmutaties.transactietype
     FROM Rekeningmutaties
     JOIN Rekeningen ON Rekeningmutaties.Rekening=Rekeningen.Rekening
     WHERE Rekeningen.Portefeuille='".$this->portefeuille."' AND
    ...
    while($data=$db->nextRecord())
    {
      ...
      if($data['transactietype']=='B')
      {
        if(!isset($eersteBoeking[$data['Fonds']]))
        {
          ...
          $opgevraagdValuta[$data['Fonds']] += $data['Debet'];
        }
      }
      else
      {
...
//listarray($opgevraagd);listarray($restricties);exit;
    if(count($restricties)>0)
    {
      ...
      foreach($restricties as $data)
      {
        ...
        $commitment=$data['bedrag']*$data['valutakoers'];
        $verplichting=($commitment-$opgevraagd);//*$data['valutakoers'];
       // $opgevraagd=(($data['bedrag']/$data['valutakoers'])-$opgevraagdValuta[$data['fonds']]);
        $this->pdf->Row(array('',$data['Omschrijving'],'','','',$this->formatGetal($commitment,2),'','',$this->formatGetal($opgevraagd,2),$this->formatGetal($verplichting,2)));
        $totalen['bedrag']+=$commitment;//$data['bedrag'];
