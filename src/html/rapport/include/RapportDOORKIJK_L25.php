...
class RapportDOORKIJK_L25
{
	function RapportDOORKIJK_L25($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = $pdf;
...
//listarray($doorKijk);
    ...
    $query="SELECT IndexPerBeleggingscategorie.Fonds,Fondsen.Omschrijving FROM IndexPerBeleggingscategorie JOIN Fondsen ON IndexPerBeleggingscategorie.Fonds=Fondsen.Fonds WHERE Vermogensbeheerder='$beheerder' AND Beleggingscategorie='H-Aand' limit 1";
    $db->SQL($query);
    $tmp=$db->lookupRecord();
...
CategorienPerHoofdcategorie.Hoofdcategorie='H-Aand' AND CategorienPerHoofdcategorie.Beleggingscategorie <> 'AU-A PrivEquity'";
		$db->SQL($query);
		$db->Query();
