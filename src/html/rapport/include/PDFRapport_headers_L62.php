...
/* 	
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: PDFRapport_headers_L62.php,v $
 		Revision 1.7  2017/02/26 10:04:26  rvv
 		*** empty log message ***
 		
 		Revision 1.6  2016/12/30 08:17:59  rvv
 		*** empty log message ***
 		
 		Revision 1.5  2016/10/12 16:30:27  rvv
 		*** empty log message ***
 		
 		Revision 1.4  2016/04/11 12:35:39  rvv
 		*** empty log message ***
...
*/
function Header_basis_L62($object)
{
 $pdfObject = &$object;
  ...
  $pdfObject->lastPortefeuille=$pdfObject->portefeuilledata['Portefeuille'];
	  ...
	  function HeaderFRONT_L62($object)
	  {
	    $pdfObject = &$object;
	  ...
	  function HeaderSCENARIO_L62($object)
	  {
	    $pdfObject = &$object;
	    //$pdfObject->headerSCENARIO();
	  }
  
	function HeaderVKM_L62($object)
	{
		$pdfObject = &$object;
		$pdfObject->HeaderVKM();
	}  
    
    function HeaderTRANS_L62($object)
	  {
	    $pdfObject = &$object;
...
function HeaderINDEX_L62($object)
{
	$pdfObject = &$object;
	$pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
	...
	$pdfObject->SetFont($pdfObject->rapport_font,'B',$pdfObject->rapport_fontsize);
	$pdfObject->Ln(1.5);
	$pdfObject->SetWidths(array(60));
	$pdfObject->row(array(vertaalTekst("Method of evaluation",$pdfObject->rapport_taal)));
	$pdfObject->SetFont($pdfObject->rapport_font,'',$pdfObject->rapport_fontsize);
	$pdfObject->SetWidths($pdfObject->widthA);
	$pdfObject->SetAligns($pdfObject->alignA);
	$pdfObject->ln(1.5);
	...
	$pdfObject->SetFont($pdfObject->rapport_font,'B',$pdfObject->rapport_fontsize);
	$pdfObject->SetWidths(array(50,51,30+30+30,30+30+30));
	$pdfObject->SetAligns(array('L','L','C','C'));

	$pdfObject->CellBorders = array('L','L','L',array('L','R'));
	$pdfObject->row(array(vertaalTekst("Investement categorie",$pdfObject->rapport_taal),
										vertaalTekst("Benchmark",$pdfObject->rapport_taal),
										vertaalTekst("Performance reporting period",$pdfObject->rapport_taal),
										vertaalTekst("Performance Year-To-Date",$pdfObject->rapport_taal)));
	$pdfObject->SetAligns(array('L','L','R','R','R','R','R','R','R','R'));
	$pdfObject->SetWidths(array(50,51,30,30,30,30,30,30));
	$pdfObject->CellBorders = array(array('L','U'),array('L','U'),array('L','U'),array('L','U'),array('L','U'),array('L','U'),array('L','U'),array('L','R','U'));
	$pdfObject->row(array('','',
										vertaalTekst(date("d-M-Y",$pdfObject->rapport_datum_vanaf),$pdfObject->rapport_taal),
										vertaalTekst(date("d-M-Y",$pdfObject->rapport_datum),$pdfObject->rapport_taal),
										vertaalTekst("Performance",$pdfObject->rapport_taal),
										vertaalTekst(date("d-M-Y",$pdfObject->tweedePerformanceStart),$pdfObject->rapport_taal),
										vertaalTekst(date("d-M-Y",$pdfObject->rapport_datum),$pdfObject->rapport_taal),
										vertaalTekst("Performance",$pdfObject->rapport_taal)));
...
$pdfObject->ln(2);
	  ...
	  function HeaderINHOUD_L62($object)
	  {
	    $pdfObject = &$object;
	    //$pdfObject->headerSCENARIO();

	  }	  
 	  function HeaderPERF_L62($object)
	  {
	    $pdfObject = &$object;
      $pdfObject->ln();
      $pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
...
$pdfObject->ln();
  ...
  function HeaderATT_L62($object)
   ...
   function HeaderPERFG_L62($object)
	{
    $pdfObject = &$object;
    $pdfObject->widthA = array(26,25,30,30,23,23,23,24,28,24,26);
		$pdfObject->alignA = array('L','R','R','R','R','R','R','R','R','R','R','R');
...
*/
  ...
  function HeaderVOLK_L62($object)
	{
    $pdfObject = &$object;
   	$pdfObject->ln();
		$pdfObject->SetFont($pdfObject->rapport_font,$pdfObject->rapport_kop_fontstyle,$pdfObject->rapport_fontsize);
...
//    else
      $teken=$pdfObject->rapportageValuta;
		...
		$pdfObject->row(array("",
										...
										vertaalTekst("Valuta-\nresultaat",$pdfObject->rapport_taal),
                    vertaalTekst("Direct\nresultaat",$pdfObject->rapport_taal),
										vertaalTekst("Rendement in %",$pdfObject->rapport_taal)
                    ));
