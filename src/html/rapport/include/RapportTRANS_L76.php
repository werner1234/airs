...
$Log: RapportTRANS_L76.php,v $
Revision 1.8  2019/07/03 15:37:22  rvv
*** empty log message ***

Revision 1.7  2019/06/21 14:52:45  rm
7880
...
Revision 1.3  2013/11/27 16:29:11  rvv
*** empty log message ***

Revision 1.2  2013/10/09 06:39:45  rvv
*** empty log message ***

Revision 1.1  2013/10/05 15:58:48  rvv
*** empty log message ***
...
class RapportTRANS_L76
{
	function RapportTRANS_L76($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	...
	function writeRapport()
	{
		...
		{
			// voor data
			$this->pdf->widthB = array(15,20,20,68,1,21,22,1,21,22,22,22,22,4);
			$this->pdf->alignB = array('L','L','R','L','R','R','R','R','R','R','R','R','R','R');
			// voor kopjes
			$this->pdf->widthA = $this->pdf->widthB;//array(15,10,20,38,16,22,22,16,22,22,22,22,22,12);
...
//	echo "lopende -> ".$resultaatlopende." <-  voorgaande ".$resultaatvoorgaande. " -  <br>" ;
				...
				$totaal_resulstaatVoorgaande +=$resultaatvoorgaande;
			...
			$this->pdf->setX($this->pdf->marge);
				...
				$this->pdf->row(array($datum,
													...
													rclip($mutaties['Omschrijving'],50),
													'',
													$aankoop_koers,
													$aankoop_waarde,
													'',
		...
		$totaal4 = $totaal3 + $this->pdf->widthA[11];
		...
		if(round(($totaalWaarde - $actueleWaardePortefeuille),2) <> 0)
		{
		  ...
		  $this->pdf->line($totaal4 + 2, $this->pdf->GetY(), $totaal4 + $this->pdf->widthA[12], $this->pdf->GetY());
			...
			$this->pdf->row(array("",
												...
												"",
												$this->formatGetal($totaal_resulstaatVoorgaande,$this->pdf->rapport_TRANS_decimaal),
		  ...
		  $this->pdf->line($totaal4+2,$this->pdf->GetY()+1,$totaal4 + $this->pdf->widthA[12],$this->pdf->GetY()+1);
...
Verzekerd beleggen
  */
		$soortOvereenkomsten=array('Beheer','Individueel beheer','Hoog Inkomen Mandaat','Vastgoed');
    $soortOvereenkomstenAdvies=array('Advies');
//		if(in_array($this->pdf->portefeuilledata['SoortOvereenkomst'],$soortOvereenkomsten) && $this->pdf->lastPOST['periode']=='Kwartaalrapportage')
//    {
//      $this->pdf->ln();
//      $this->pdf->SetTextColor($this->pdf->rapport_default_fontcolor['r'],$this->pdf->rapport_default_fontcolor['g'],$this->pdf->rapport_default_fontcolor['b']);
//      $txt = 'De in dit overzicht gespecificeerde transacties waren, op het moment dat door Ambassador Vermogensbeheer werd besloten ze uit te laten voeren, naar de mening van Ambassador Vermogensbeheer geschikt om opgenomen te worden binnen het met u overeengekomen risicoprofiel. Voorafgaand aan elk door Ambassador Vermogensbeheer genomen beslissing over door te voeren wijzigingen in uw beleggingsportefeuille heeft Ambassador Vermogensbeheer een grondige beoordeling uitgevoerd van de mogelijke beleggingsalternatieven, waarbij rekening is gehouden met de kosten en de complexiteit van producten. De effectenposities in uw portefeuille sluiten aan bij de bij Ambassador Vermogensbeheer bekende uitgangspunten met betrekking tot uw doelstelling(en), beleggingshorizon, persoonlijke- en financiële situatie, risicobereidheid en kennis en ervaring.';
//      $this->pdf->SetFont($this->pdf->rapport_font,$this->pdf->rapport_fontstyle,$this->pdf->rapport_fontsize);
//      $this->pdf->MultiCell(280,4, $txt, 0, "L");
//    }
//    else
    if(in_array($this->pdf->portefeuilledata['SoortOvereenkomst'],$soortOvereenkomstenAdvies) && $this->pdf->lastPOST['periode']=='Kwartaalrapportage')
    {
      $this->pdf->ln();
      $this->pdf->SetTextColor($this->pdf->rapport_default_fontcolor['r'],$this->pdf->rapport_default_fontcolor['g'],$this->pdf->rapport_default_fontcolor['b']);
      $txt = 'De in dit overzicht gespecificeerde transacties waren, voor zover Ambassador Vermogensbeheer hierover specifiek advies heeft gegeven, naar de mening van Ambassador Vermogensbeheer voor u geschikt gegeven het met u overeengekomen risicoprofiel. Bij elk door Ambassador Vermogensbeheer gegeven beleggingsadvies heeft Ambassador Vermogensbeheer een grondige beoordeling uitgevoerd van de mogelijke beleggingsalternatieven, waarbij rekening is gehouden met de kosten en de complexiteit van producten. De door Ambassador Vermogensbeheer aan u gegeven beleggingsadviezen sluiten aan bij de bij Ambassador Vermogensbeheer bekende uitgangspunten met betrekking tot uw doelstelling(en), beleggingshorizon, persoonlijke- en financiële situatie, risicobereidheid en kennis en ervaring.';
      $this->pdf->SetFont($this->pdf->rapport_font,$this->pdf->rapport_fontstyle,$this->pdf->rapport_fontsize);
      $this->pdf->MultiCell(280,4, $txt, 0, "L");
    }
