...
$Log: RapportOIB_L58.php,v $
Revision 1.7  2020/05/16 15:57:02  rvv
*** empty log message ***

Revision 1.6  2018/08/18 12:40:14  rvv
php 5.6 & consolidatie

Revision 1.5  2015/04/22 14:26:44  rvv
*** empty log message ***
...
class RapportOIB_L58
{
	function RapportOIB_L58($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
*/
    ...
    foreach ($this->pdf->grafiekData['Beleggingscategorie']['grafiek'] as $omschrijving=>$waarde)
    {
      ...
      $grafiekData['OIB']['Totaal'][$omschrijving]=$waarde;
      
    }
$this->pdf->SetXY(200,50);
DonutChart($this,75,75,$grafiekData['OIB']['Totaal'],$this->pdf->grafiekData['Beleggingscategorie']['grafiekKleur']);
/*
    getTypeGrafiekData($this,'Regio');
    foreach ($this->pdf->grafiekData['Regio']['grafiek'] as $omschrijving=>$waarde)
    {
      $grafiekData['OIR']['Omschrijving'][]=$omschrijving." (".$this->formatGetal($waarde,1)."%)";
      $grafiekData['OIR']['Percentage'][]=$waarde;
    }
    
    $yas= 145;
    $this->pdf->set3dLabels($grafiekData['OIR']['Omschrijving'],$Xas+$xRechts+10,$yas,$this->pdf->grafiekData['Regio']['grafiekKleur']);
    $this->pdf->Pie3D($grafiekData['OIR']['Percentage'],$this->pdf->grafiekData['Regio']['grafiekKleur'],$Xas+$xRechts,$yas,$diameter,$hoek,$dikte,"Regio",'titel');
*/
...
FROM TijdelijkeRapportage
JOIN ZorgplichtPerBeleggingscategorie ON TijdelijkeRapportage.hoofdcategorie = ZorgplichtPerBeleggingscategorie.beleggingscategorie AND ZorgplichtPerBeleggingscategorie.Vermogensbeheerder='".$this->pdf->portefeuilledata['Vermogensbeheerder']."'
WHERE TijdelijkeRapportage.Portefeuille =  '".$this->portefeuille."' AND
