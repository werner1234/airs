...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2019/12/07 17:48:23 $
File Versie					: $Revision: 1.7 $

$Log: RapportTRANS_L48.php,v $
Revision 1.7  2019/12/07 17:48:23  rvv
*** empty log message ***

Revision 1.6  2019/06/21 14:52:45  rm
7880

Revision 1.5  2018/03/10 18:24:22  rvv
*** empty log message ***
...
class RapportTRANS_L48
{
	function RapportTRANS_L48($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	...
	function writeRapport()
	{
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
		...
		// voor data
		$this->pdf->widthB = array(15,12,20,70,25,25,25,23,23,23,20);
...
//	echo "lopende -> ".$resultaatlopende." <-  voorgaande ".$resultaatvoorgaande. " -  <br>" ;
			...
			{
				$percentageTotaal = (($resultaatlopende+$resultaatvoorgaande) / $historischekostprijs) *100;
    ...
    $this->pdf->setY(185);
		$this->pdf->SetFont($this->pdf->rapport_font,$this->pdf->rapport_fontstyle,$this->pdf->rapport_fontsize-2);
		$this->pdf->MultiCell(280,3,"Eventuele transacties die in de afgelopen periode hebben plaatsgevonden en vermeld staan in deze rapportage, zijn geschikt voor u als belegger in het beleggingsprofiel ". $this->pdf->portefeuilledata['Risicoklasse'].". Zij passen bij de doelstellingen en kenmerken van beleggers in genoemd beleggingsprofiel. Reden van de transactie kan zijn een herbalancering in de portefeuille danwel een aan- of verkoop van een belegging zijn, omdat wij denken dat die transactie in uw belang is.", 0, "L");
		$this->pdf->SetFont($this->pdf->rapport_font,$this->pdf->rapport_fontstyle,$this->pdf->rapport_fontsize);
