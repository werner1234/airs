...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: ATTberekening_L78.php,v $
 		Revision 1.3  2019/10/12 17:11:44  rvv
 		*** empty log message ***
 		
 		Revision 1.2  2019/10/09 15:13:20  rvv
 		*** empty log message ***
...
class ATTberekening_L78
{

	function ATTberekening_L78($rapportData,$periode='maanden')
	{
...
";
  ...
  //  $perioden=array(array('start'=>$van,'stop'=>$tot));
 ...
 $query = "SELECT  Grootboekrekeningen.Opbrengst,Grootboekrekeningen.Kosten, Grootboekrekeningen.Grootboekrekening,".
    ...
    if($categorie <> 'totaal')
    {
      ...
      $query = "SELECT Rekeningmutaties.Grootboekrekening,".
        ...
        (Rekeningmutaties.Grootboekrekening='RENTE') AND $filter GROUP BY  Rekeningmutaties.Grootboekrekening
        ";    
      $DB2->SQL($query); 
			$DB2->Query();
      while($grootboek = $DB2->nextRecord())
     ...
     Rekeningmutaties.omschrijving as rekeningOmschrijving,
      ...
      $mutaties['Rapportagekoers']=1;
			...
			*/
			...
			{
				$historie = berekenHistorischKostprijs($this->rapport->portefeuille, $mutaties['Fonds'], $mutaties['Boekdatum'],$this->rapport->pdf->rapportageValuta,$rapportageDatumVanaf,0);
      ...
      {
        listarray($mutaties);
        listarray($verkoop_koers);
        listarray($result_voorgaandejaren);
      }
    */
      $fondsResultaat=0;
      $valutaResultaat=0;
      if($mutaties['Aantal'] <>0 )//$verkoop_koers <> '' )
      { //listarray($historie);
