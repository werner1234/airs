...
$Log: RapportVOLKD_L42.php,v $
Revision 1.13  2018/08/18 12:40:15  rvv
php 5.6 & consolidatie

Revision 1.12  2017/08/12 12:17:24  rvv
*** empty log message ***

Revision 1.11  2017/07/29 17:18:20  rvv
*** empty log message ***

Revision 1.10  2017/04/19 15:39:24  rvv
*** empty log message ***

Revision 1.9  2017/04/15 19:11:50  rvv
*** empty log message ***

Revision 1.8  2016/12/14 19:22:46  rvv
*** empty log message ***

Revision 1.7  2016/12/14 15:11:00  rvv
*** empty log message ***

Revision 1.6  2016/12/11 12:52:31  rvv
*** empty log message ***

Revision 1.5  2016/10/29 15:41:46  rvv
*** empty log message ***

Revision 1.4  2016/10/23 11:32:33  rvv
*** empty log message ***

Revision 1.3  2016/10/05 16:19:00  rvv
*** empty log message ***

Revision 1.2  2016/10/02 12:38:58  rvv
*** empty log message ***

Revision 1.1  2016/09/25 18:53:45  rvv
*** empty log message ***
...
class RapportVOLKD_L42
{
	function RapportVOLKD_L42($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	  global $__appvar;
		$this->pdf = &$pdf;
    ...
    foreach ($verdiepteFondsen as $fonds)
      $this->verdiept->bepaalVerdeling($fonds,$this->verdiept->FondsPortefeuilleData[$fonds],array('fonds'),$this->rapportageDatum,'TijdelijkeRapportage.FondsOmschrijving ASC',$this->rapportageDatumVanaf);
      ...
      foreach($catData as $cat=>$secData)
      {
        ...
        foreach($secData as $sec=>$fondsData)
        {
          ...
          foreach($fondsData as $fonds)
          {
            ...
            if(in_array($fonds['fonds'],$verdiepteFondsen))
            {
              $inspringen='   ';
              $this->pdf->ln();
              $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
              $this->pdf->row(array($inspringen.$fonds['fondsOmschrijving']));
              //$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);

              $subFondsData = $this->verdiept->getFondsVerdeling($fonds['fonds'],30);
              //$this->pdf->SetTextColor(150);
            //  listarray($fonds);
              foreach ($subFondsData as $subFonds)
              {

                $correctie=$subFonds['percentage'];// *  0.01;//$fonds['aandeel'] *
                $subFonds['fondsOmschrijving']=$inspringen.$subFonds['Omschrijving'];
                $subFonds['fonds']=$subFonds['Fonds'];
                $subFonds['actuelePortefeuilleWaardeEuro']=$fonds['actuelePortefeuilleWaardeEuro'] * $correctie ;
                $subFonds['beginPortefeuilleWaardeEuro']=$fonds['beginPortefeuilleWaardeEuro'] * $correctie  ;
                $subFonds['historischeWaarde']=$subFonds['overige']['historischeWaarde']  ;
                //$subFonds['beginwaardeLopendeJaar']=$subFonds['historischeWaarde'];
                //beginPortefeuilleWaardeEuro
                $subFonds['actuelePortefeuilleWaardeInValuta'] =$subFonds['overige']['actuelePortefeuilleWaardeInValuta']* $correctie;
                $subFonds['aandeel']=$subFonds['actuelePortefeuilleWaardeEuro']/$totaalWaarde*100;//  $subFonds['percentage'] *100;
                //$subFonds['actueleFonds']=$subFonds['fondskoers']  ;
                $subFonds['beginwaardeLopendeJaar']=$subFonds['overige']['beginwaardeLopendeJaar']  ;
                //$subFonds['overige']['beginwaardeLopendeJaar'] ;
                $subFonds['actueleFonds']=$subFonds['overige']['actueleFonds'];



                $subFonds['totaalAantal']=$subFonds['actuelePortefeuilleWaardeEuro']/$subFonds['overige']['actueleFonds'];
                $subFonds['beginPortefeuilleWaardeInValuta']=$subFonds['totaalAantal']* $subFonds['beginwaardeLopendeJaar']* $subFonds['overige']['fondsEenheid'];
                $subFonds['beginPortefeuilleWaardeEuro']=$subFonds['totaalAantal']* $subFonds['beginwaardeLopendeJaar']* $subFonds['overige']['fondsEenheid']*$subFonds['overige']['beginwaardeValutaLopendeJaar'];
             //   $subFonds['actuelePortefeuilleWaardeEuro']=$subFonds['totaalAantal']* $subFonds['actueleFonds']* $subFonds['overige']['fondsEenheid']*$subFonds['overige']['actueleValuta'];
                //echo $subFonds['fonds']." ".$subFonds['beginPortefeuilleWaardeEuro']." ".$subFonds['actuelePortefeuilleWaardeEuro']."<br>\n";

        // echo $fonds['fonds']." | ".$subFonds['Omschrijving']." -> ".$subFonds['beginPortefeuilleWaardeEuro']." = ".$subFonds['totaalAantal']."*".$subFonds['beginwaardeLopendeJaar']."*".$subFonds['overige']['fondsEenheid']."*".$subFonds['overige']['beginwaardeValutaLopendeJaar']."<br>\n";
                $subFonds['fondsOngerealiseerd'] = ($subFonds['actuelePortefeuilleWaardeInValuta'] - $subFonds['beginPortefeuilleWaardeInValuta']) * $subFonds['actueleValuta'];
                $subFonds['fondsValutaresultaat'] = $subFonds['actuelePortefeuilleWaardeEuro'] - $subFonds['beginPortefeuilleWaardeEuro'] - $subFonds['fondsOngerealiseerd'] ;



//listarray($subFonds);
                $subFonds['type']='fondsen';
                $this->printRow($subFonds,'verdiept');


              }
              $liq=$this->verdiept->pdf->fondsPortefeuille[$fonds['fonds']]['verdeling']['liquiditeiten'];
             //listarray($liq['liquiditeiten']);
              if($liq['liquiditeiten']['percentage'] <> 0)
              {
                $subFonds = array('type'                          => 'rekening',
                                  'valuta'                        => '',
                                  'beginPortefeuilleWaardeEuro'   => $fonds['beginPortefeuilleWaardeEuro'] * $liq['liquiditeiten']['percentage'],
                                  'actuelePortefeuilleWaardeEuro' => $fonds['actuelePortefeuilleWaardeEuro'] * $liq['liquiditeiten']['percentage'],
                                  'fondsOmschrijving'             => $inspringen.'Liquiditeiten en reserveringen', 'aandeel' => $fonds['actuelePortefeuilleWaardeEuro'] * $liq['liquiditeiten']['percentage']/$totaalWaarde * 100);
                $this->printRow($subFonds, 'verdiept');//listarray($subFonds);
              }
              $this->pdf->CellBorders=array('','','','','','','','','TS','TS','TS','','TS');
              $fondsOmschrijving=$fonds['fondsOmschrijving'];
              $fonds['fondsOmschrijving']='';//$fonds['fondsOmschrijving']
              $this->printRow($fonds,'','');
              $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
              $this->pdf->ln(-1*$this->pdf->rowHeight);
              $this->pdf->row(array('  Totaal '.$fondsOmschrijving));
              $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
              $this->pdf->ln();
