...
class RapportOIB_L77
{
	function RapportOIB_L77($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
WHERE bronKoppeling='$airsCategorie' AND doorkijkCategoriesoort='".mysql_real_escape_string($doorkijkSoort)."' AND systeem='AIRS' AND vermogensbeheerder='". $this->pdf->portefeuilledata['Vermogensbeheerder']."'";
  ...
  $order = 'WaardeEuro DESC';
	$query="SELECT TijdelijkeRapportage.beleggingscategorie as beleggingscategorie,
	sum(TijdelijkeRapportage.ActuelePortefeuilleWaardeEuro) AS WaardeEuro,
...
//eind kleuren instellen
    ...
    */
    ...
    if(count($belCategorien)>0)
    {
      ...
      foreach ($tmp['categorien'] as $categorie => $percentage)
      {
        if (isset($tmp['details'][$categorie]))
        {
          ...
          $grafiekData['OIR']['Kleur'][] = $standaardKleuren[$i];
          $i++;
        }
      }
    }
    ...
    foreach($tmp['categorien'] as $categorie=>$percentage)
    {
        ...
        $grafiekData['OIS2']['Kleur'][] = $standaardKleuren[$i];
        $i++;
...
{  
  $legendaStart=$this->correctLegentHeight(count($grafiekData['OIB']['Percentage']));
	PieChart_L77($this->pdf,$chartsize,$vwh,$grafiekData['OIB']['Percentage'],'%l',$grafiekData['OIB']['Kleur'],vertaalTekst('Beleggingscategorie',$this->pdf->rapport_taal),$legendaStart);
}
...
if(isset($grafiekData['OIR']))
{
  ...
  {
    $this->BarDiagram($chartsize + $extraBarW, $chartsize, $grafiekData['OIR']['Percentage'], '%l', $grafiekData['OIR']['Kleur'], vertaalTekst('Regioverdeling aandelen (incl. doorkijk door de beleggingsfondsen)', $this->pdf->rapport_taal));
  }
  else
  {
    $legendaStart = $this->correctLegentHeight(count($grafiekData['OIR']['Percentage']));
    PieChart_L77($this->pdf, $chartsize, $vwh, $grafiekData['OIR']['Percentage'], '%l', $grafiekData['OIR']['Kleur'], vertaalTekst('Regioverdeling aandelen (incl. doorkijk door de beleggingsfondsen)', $this->pdf->rapport_taal), $legendaStart);
  }
}
...
{ 
  $legendaStart=$this->correctLegentHeight(count($grafiekData['OIV']['Percentage']));
	PieChart_L77($this->pdf,$chartsize,$vwh,$grafiekData['OIV']['Percentage'],'%l',$grafiekData['OIV']['Kleur'],vertaalTekst('Valuta',$this->pdf->rapport_taal),$legendaStart);
}
...
if(isset($grafiekData['OIS2']))
{
 ...
 //  $this->pdf->setXY($this->pdf->marge+(($lwb/4)*7-$chartsize/2),$headerHeight);
  if(min($grafiekData['OIS2']['Percentage']) < 0)
    $this->BarDiagram($chartsize+$extraBarW,$chartsize,$grafiekData['OIS2']['Percentage'],'%l',$grafiekData['OIS2']['Kleur'],vertaalTekst('Sectorverdeling aandelen (incl. doorkijk door de beleggingsfondsen)',$this->pdf->rapport_taal));
  else
  { 
    $legendaStart=$this->correctLegentHeight(count($grafiekData['OIS2']['Percentage']));
		PieChart_L77($this->pdf,$chartsize,$vwh,$grafiekData['OIS2']['Percentage'],'%l',$grafiekData['OIS2']['Kleur'],vertaalTekst('Sectorverdeling aandelen (incl. doorkijk door de beleggingsfondsen)',$this->pdf->rapport_taal),$legendaStart);
  }
}
