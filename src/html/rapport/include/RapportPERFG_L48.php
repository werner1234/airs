...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2020/01/19 13:09:52 $
File Versie					: $Revision: 1.5 $

$Log: RapportPERFG_L48.php,v $
Revision 1.5  2020/01/19 13:09:52  rvv
*** empty log message ***

Revision 1.4  2018/08/18 12:40:14  rvv
php 5.6 & consolidatie

Revision 1.3  2018/05/26 17:23:51  rvv
*** empty log message ***

Revision 1.2  2015/04/15 18:25:11  rvv
*** empty log message ***
...
class RapportPERFG_L48
{

	function RapportPERFG_L48($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function getBenchmarkRendement($categorie)
  {
    $db=new DB();
    $verm=$this->pdf->portefeuilledata['Vermogensbeheerder'];
    
    if($categorie=='totaal')
    {
      $query="SELECT Portefeuilles.SpecifiekeIndex as Fonds ,Fondsen.Omschrijving,Fondsen.Valuta  
      FROM Portefeuilles 
    ...
    {
      $query="SELECT 
        IndexPerBeleggingscategorie.Fonds,Fondsen.Omschrijving,Fondsen.Valuta  
      FROM
        IndexPerBeleggingscategorie
        JOIN Fondsen ON IndexPerBeleggingscategorie.Fonds=Fondsen.Fonds   
      WHERE
       IndexPerBeleggingscategorie.Vermogensbeheerder='".$verm."' AND 
       IndexPerBeleggingscategorie.Beleggingscategorie='$categorie' ";
    }
    $db->SQL($query);

    $indexData=$db->lookupRecord();   
    $fonds=$indexData['Fonds'];
...
//echo "De '$categorie' categorie heeft index: $fonds ".$indexData[$fonds]['performance']." <br>\n";
    return $indexData[$fonds];
  }
...
$benchmarkRendementRegel=array("",vertaalTekst("Benchmark rendement",$this->pdf->rapport_taal));//
...
foreach($categorien as $categorie=>$categorieOmschrijving)
{
  ...
  foreach($categorien as $categorie=>$categorieOmschrijving)
  {
    ...
    $mutwaarde[]='';
    $benchmarkRendement= $this->getBenchmarkRendement($categorie);
    if($categorie=='totaal')
    {
      $effectenmutaties[]='';
      $effectenmutaties[]=''; 
      ...
      $benchmarkRendementRegel[]=$this->formatGetal($benchmarkRendement['performance'],2);//performanceEur
      $benchmarkRendementRegel[]='%';
...
//    $this->pdf->fillCell=array();
  ...
  //  $this->pdf->CellBorders = array();
    ...
    $this->pdf->CellBorders = array();
		$ypos = $this->pdf->GetY();
		$this->pdf->SetY($ypos);
    $this->pdf->row($benchmarkRendementRegel);
		$this->pdf->ln();
		$this->pdf->SetWidths($this->pdf->widthB);
		$this->pdf->SetAligns($this->pdf->alignB);
