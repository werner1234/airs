...
class RapportMUT_L49
{
	function RapportMUT_L49($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	...
	function printSubTotaal($title, $totaalA, $totaalB)
	{
		...
		//$this->pdf->SetFillColor($this->pdf->rapport_subtotaal_omschr_fontcolor[r],$this->pdf->rapport_subtotaal_omschr_fontcolor[g],$this->pdf->rapport_subtotaal_omschr_fontcolor[b]);
	...
	function printTotaal($title, $totaalA, $totaalB,$n )
	{
	...
	function writeRapport()
	{
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
		...
		$this->pdf->widthA = $this->pdf->widths;//array(20,25,90,25,30,20,25,25,25);
		$this->pdf->alignA = $this->pdf->aligns;//array('R','R','L','R','R','R','R','R');

		// voor kopjes
		$this->pdf->widthB = $this->pdf->widthA ;//array(20,25,90,25,30,20,25,25,25);
		$this->pdf->alignB = $this->pdf->alignA;//array('R','R','L','R','R','R','R','R');
	   ...
	   $extraquery .= "AND Rekeningmutaties.Grootboekrekening IN('$grootboekSelectie')  ";
		...
		foreach($buffer as $mutaties)
		{
			...
			if(!empty($lastCategorie) && $lastCategorie <> $mutaties['gbOmschrijving'])
			{
				...
				$this->printSubTotaal(vertaalTekst("Saldo",$this->pdf->rapport_taal),$subdebet, $subcredit,$n);

				$totaal=$subcredit-$subdebet;
				if($totaal < 0)
					$n=$this->printTotaal(vertaalTekst("Totaal",$this->pdf->rapport_taal)." ".vertaalTekst($lastCategorie,$this->pdf->rapport_taal), abs($totaal), 0);
				else
					$n=$this->printTotaal(vertaalTekst("Totaal",$this->pdf->rapport_taal)." ".vertaalTekst($lastCategorie,$this->pdf->rapport_taal), 0, abs($totaal));
				$subdebet = 0;
				$subcredit = 0;
			}
      ...
      {
        $mutaties['Rekening']='';
        if(!in_array($mutaties['Grootboekrekening'],array('DIV','DIVBE','KOST','KOBU')))
        {
          $omschrijving='';
        }
      }
