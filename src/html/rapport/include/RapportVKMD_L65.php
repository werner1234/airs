...
$Log: RapportVKMD_L65.php,v $
Revision 1.4  2019/03/31 12:19:56  rvv
*** empty log message ***

Revision 1.3  2019/02/03 15:50:24  rvv
*** empty log message ***

Revision 1.2  2019/01/26 19:33:28  rvv
*** empty log message ***
...
class RapportVKMD_L65
{
	function RapportVKMD_L65($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    if($this->pdfVullen==true)
		...
		if($this->pdfVullen==true)
		{
			...
			$this->pdf->setWidths(array(28+48+20,20,20,20,20,18,18,18,18,18,15));
...
foreach ($perCategorie as $hoofdcategorie=>$categorieData)
{
 ...
 WHERE Portefeuilles.portefeuille='".$this->portefeuille."'";
    ...
    for($i=$this->pdf->rapport_jaar; $i<=$doelJaar; $i++)
		...
		$waardeZonderKostenKleur=array(151,203,227);
		$waardeNaKostenKleur=array(31,162,46);
		$cumulatieveKostenKleur=array(203,178,203);
