...
$Log: RapportHSE_L63.php,v $
Revision 1.11  2020/05/30 15:30:39  rvv
*** empty log message ***

Revision 1.10  2019/12/14 17:46:24  rvv
*** empty log message ***

Revision 1.9  2018/02/08 07:40:41  rvv
*** empty log message ***

Revision 1.8  2017/11/15 17:03:35  rvv
*** empty log message ***

Revision 1.7  2016/11/27 11:09:29  rvv
*** empty log message ***

Revision 1.6  2016/04/30 15:33:27  rvv
*** empty log message ***

Revision 1.5  2016/03/19 16:51:09  rvv
*** empty log message ***

Revision 1.4  2015/12/02 08:26:21  rvv
*** empty log message ***

Revision 1.3  2015/12/01 07:39:18  rvv
*** empty log message ***

Revision 1.2  2015/12/01 07:30:57  rvv
*** empty log message ***
...
class RapportHSE_L63
{
	function RapportHSE_L63($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum, $valuta = 'EUR')
	{
		$this->pdf = &$pdf;
    ...
    $this->rapport_xls_titel = "Posities";  
	  ...
	  $this->pdf->excelData[]=array('Portefeuille','PAS-code','Soort','ISIN-code / ISO-code','Bedrag','Valuta','Datum');
	}
...
LEFT JOIN Fondsen ON TijdelijkeRapportage.fonds = Fondsen.Fonds
      WHERE ".
			...
			" GROUP BY TijdelijkeRapportage.type,TijdelijkeRapportage.fonds,TijdelijkeRapportage.valuta
      ORDER BY TijdelijkeRapportage.Lossingsdatum, TijdelijkeRapportage.fondsOmschrijving asc";
        ...
        $WcodesPerImportCode[$data['FondsImportCode']]=$data['beleggingssector'];
		}
		  ...
		  if($data['type']=='rekening')
      {
        ...
        $isin=$data['valuta'];
				...
				if($data['Termijnrekening']=='1')
					$data['valuta']=$data['valuta']."T";

        $data['beleggingssector']='W06';
      }
      else  
		  {
		    $soort='FONDS';
		    if($data['type']=='rente')
        {
          if($data['rekening']<>'')
          {
            $data['beleggingssector']='W05';
            $soort='REKENING';
          }  
          else
            $data['beleggingssector']='W02';
					if($WcodesPerImportCode[$data['FondsImportCode']] =='W01')
					{
						$data['beleggingssector']='W01';
					}
        }
		    
        $isin=$data['FondsImportCode'];
		  } 
       //listarray($data);
		  ...
		  $this->pdf->excelData[]=array($data['portefeuille'],
                                       ...
                                       $soort,
                                       $isin,
                                       $data['actuelePortefeuilleWaardeEuro'],
                                       $data['valuta'],
                                       date('d-m-Y',db2jul($data['rapportageDatum'])));
                                       
      if($data['beleggingssector']=='W03')                                   
		    $this->pdf->excelData[]=array($data['portefeuille'],
                                       'W04',
                                       $soort,
                                       $isin,
...
//		{
			...
			while($categorien = $DB->NextRecord())
			{
				if(!$this->pdf->rapport_HSE_geenrentespec)
				{
					...
					while($subdata = $DB2->NextRecord())
					{
						...
						{
							$this->pdf->row(array("","","","",$this->formatGetal($subdata[actuelePortefeuilleWaardeInValuta],$this->pdf->rapport_decimaal),$this->formatGetalKoers($subdata[actuelePortefeuilleWaardeEuro],$this->pdf->rapport_decimaal),"","",
														"",
														"",
														($this->pdf->rapport_inprocent)?$this->formatGetal($percentageVanTotaal,2)." %":""));
						}
	...
	//	}
		...
		{
		  $kaderVolgorde = array('rendement','AEX','valutaoverzicht');
		}
		else
		{
  ...
  function autorun($selectData,$settings)
  {
    ...
    include_once('RapportTRANS_L63.php');
    ...
    {
      $filePath=$path.'/'.$portefeuille.'.xls';
      ...
      logIt('autorun HSE_TRANS_L63: '.$portefeuille." over ".$this->rapportageDatumVanaf."->".$this->rapportageDatum);
      ...
      if(isset($this->rapport_xls_titel))
        $xlsTitel=$this->rapport_xls_titel;
      else
        $xlsTitel='HSE';
      
      $worksheet[0] =& $workbook->addWorksheet($xlsTitel);
      if(is_object($this))
        $this->pdf->fillXlsSheet($worksheet[0],$workbook);
  
      $this->pdf->excelData 	= array();
      $TRANS=new RapportTrans_L63($this->pdf,$this->portefeuille,	$this->rapportageDatumVanaf,$this->rapportageDatum);
      $TRANS->writeRapport();
  
      if(isset($TRANS->rapport_xls_titel))
        $xlsTitel=$TRANS->rapport_xls_titel;
      else
        $xlsTitel='TRANS';
      $worksheet[1] =& $workbook->addWorksheet($xlsTitel);
      if(is_object($this))
        $this->pdf->fillXlsSheet($worksheet[1],$workbook);

      $workbook->close();
      $fileList[]=$filePath;

      logIt('autorun HSE_TRANS_L63: '.$path.'/'.$portefeuille.'.xls');
    }
    include_once($__appvar["basedir"]."/classes/pclzip.lib.php");
    $zipfile=$path."/export.zip";
