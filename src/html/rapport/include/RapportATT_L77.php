...
class RapportATT_L77
{

	function RapportATT_L77($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		...
		$this->pdf->rapport_titel = "Contributie";
...
/*
  ...
  */
    ...
    //$tmp=getFondsPerformanceGestappeld($verdeling,$this->portefeuille,$van,$tot,'maanden',false)/100;
  
    $tmp =getFondsPerformanceGestappeld2($fonds,$this->portefeuille,$van,$tot,'maanden',false,true)/100;
  //echo $categorie." ".($tmp*100)." =getFondsPerformanceGestappeld2($fonds,$this->portefeuille,$van,$tot,'maanden',false,true)<br>\n";
    //echo "$categorie $fonds <br>\n"; listarray($verdeling);listarray($tmp); ob_flush();
    return $tmp;
    ...
    */
    ...
    $this->getZorgplichtCategorien();
		...
		// voor data
		$this->pdf->widthA = array(5,95,30,5,30,5,30,5,30,85);
		$this->pdf->alignA = array('L','L','R','L','R');

		// voor kopjes
		$this->pdf->widthB = array(0,100,30,5,30,5,30,5,30,85);
...
function addResultaat()
{
  ...
  if(!isset($this->pdf->__appvar['consolidatie']))
  {
   ...
   $vetralingGrootboek=$this->getGrootboeken();
    ...
    $this->att->indexPerformance=false;
    $this->waarden['Periode']=$this->att->bereken($this->rapportageDatumVanaf,$this->rapportageDatum,'Beleggingscategorien');
...
$totaalOpbrengst=array("",vertaalTekst("Bruto resultaat",$this->pdf->rapport_taal));//totaalOpbrengst
  ...
  $totaalKostenBoven=array("",vertaalTekst("Ingehouden kosten en belastingen",$this->pdf->rapport_taal));//
    ...
    $totaal=array("",vertaalTekst("Netto resultaat",$this->pdf->rapport_taal));  //totaalOpbrengst-totaalKosten
...
foreach($categorien as $categorie)
{
  ...
  foreach($categorien as $categorie)
  {
    ...
    $perfWaarden['resultaatValuta']=$perfWaarden['resultaatBruto']-($totaalOpbrengstEUR);

    $totaalOpbrengstEUR+=$perfWaarden['resultaatValuta'];

    $resultaat[]=$this->formatGetal($perfWaarden['resultaat'],0);
    $resultaat[]='';
    $resultaatBruto[]=$this->formatGetal($perfWaarden['resultaatBruto'],0);
    $resultaatBruto[]='';
    ...
    foreach($perfWaarden['grootboekKosten'] as $categorie=>$waarde)
  	...
  	$this->pdf->widthB = array(0,85,25,5,25,5,25,5,25,5,25,5,25,5,25,5);
		$this->pdf->alignB = array('L','L','R','L','R','L','R','L','R','L','R','L','R');
    $this->pdf->widthA = $this->pdf->widthB;//array(0,65,30,5,30,5,30,5,30,5,30,5,30,5);
		$this->pdf->alignA = array('L','L','R','L','R','L','R','L','R','L','R','L','R');
