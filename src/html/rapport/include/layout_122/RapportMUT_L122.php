...
class RapportMUT_L122
{
	function RapportMUT_L122($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	...
	function printSubTotaal($title, $totaalA, $totaalB)
	{
	  ...
	  $this->pdf->Line($totaal1+2,$this->pdf->GetY(),$totaal1 + $this->pdf->widthA[6],$this->pdf->GetY());
		...
		if(!empty($totaalA))
		{
			$totaalAtxt = $this->formatGetal($totaalA,0);
		}

		if(!empty($totaalB))
		{
			$totaalBtxt = $this->formatGetal($totaalB,0);
		}
	...
	function writeRapport()
	{
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
		...
		$DB = new DB();
		// voor data
		$this->pdf->widthA = array(25,100,5,25,30,20,25,25,25);
		$this->pdf->alignA = array('R','L','L','R','R','R','R','R');
...
Fondsen.Omschrijving as fondsOmschrijving ".
		...
		foreach($buffer as $mutaties)
    {
			...
			if($debet <> 0)
				$debettxt = $this->formatGetal($debet,0);
			else
				$debettxt = "";

			if($credit <> 0)
				$credittxt = $this->formatGetal($credit,0);
			else
				$credittxt = "";
