...
/* 	
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2019/05/05 13:04:29 $
 		File Versie					: $Revision: 1.14 $
 		
 		$Log: PDFRapport_headers_L61.php,v $
 		Revision 1.14  2019/05/05 13:04:29  rvv
 		*** empty log message ***
 		
 		Revision 1.13  2019/05/04 18:44:05  rvv
 		*** empty log message ***
 		
 		Revision 1.12  2019/05/04 18:22:48  rvv
 		*** empty log message ***
 		
 		Revision 1.11  2017/07/23 13:36:28  rvv
 		*** empty log message ***
 		
 		Revision 1.10  2016/12/30 08:17:59  rvv
 		*** empty log message ***
 		
 		Revision 1.9  2016/11/02 16:34:11  rvv
 		*** empty log message ***
 		
 		Revision 1.8  2016/10/29 15:41:46  rvv
 		*** empty log message ***
 		
 		Revision 1.7  2016/10/12 16:30:27  rvv
 		*** empty log message ***
 		
 		Revision 1.6  2016/04/29 08:41:30  rvv
 		*** empty log message ***
 		
 		Revision 1.5  2015/12/30 19:52:52  rvv
 		*** empty log message ***
 		...
 		*** empty log message ***
 		...
 		Revision 1.12  2015/05/31 10:15:24  rvv
 		*** empty log message ***
 		
 		Revision 1.11  2015/05/02 14:57:32  rvv
 		*** empty log message ***
 		
 		Revision 1.10  2015/04/26 12:26:58  rvv
 		*** empty log message ***
 		...
 		*** empty log message ***
 		...
 		Revision 1.7  2015/03/11 17:13:49  rvv
 		*** empty log message ***
 		
 		Revision 1.6  2015/03/04 16:30:29  rvv
 		*** empty log message ***
 		
 		Revision 1.5  2015/03/01 14:08:16  rvv
 		*** empty log message ***
 		...
 		*** empty log message ***
 		...
 		Revision 1.2  2015/02/15 10:26:57  rvv
 		*** empty log message ***
...
*/
function Header_basis_L61($object)
{
 $pdfObject = &$object;
    ...
    {  
  	...
  	if($pdfObject->lastPortefeuille != $pdfObject->portefeuilledata['Portefeuille'] && !empty($pdfObject->lastPortefeuille))
        ...
        $logopos=(297/2)-($xSize/2);
	      $pdfObject->Image($pdfObject->rapport_logo, $logopos, 1, $xSize, $ySize);
		}
	...
	function HeaderVKM_L61($object)
	{
		$pdfObject = &$object;
		$pdfObject->HeaderVKM();
	}
	
function HeaderVKMS_L61($object)
{
  $pdfObject = &$object;
  
}
	  function HeaderFRONT_L61($object)
	  {
	    $pdfObject = &$object;
	  }


function HeaderVAR_L61($object)
{
  $pdfObject = &$object;
  $pdfObject->SetFont($pdfObject->rapport_font,'B',$pdfObject->rapport_fontsize);
  ...
  unset($pdfObject->CellBorders);//"Modified\nduration",
	  ...
	  function HeaderSCENARIO_L61($object)
	  {
	    $pdfObject = &$object;
 	  ...
 	  function HeaderRISK_L61($object)
	  {
	    $pdfObject = &$object;
	    //$pdfObject->headerSCENARIO();
    ...
    function HeaderTRANS_L61($object)
	  {
	    $pdfObject = &$object;
...
$pdfObject->ln(2);
	  ...
	  function HeaderINHOUD_L61($object)
	  {
	    $pdfObject = &$object;
	    //$pdfObject->headerSCENARIO();

	  }	  
 	  function HeaderPERF_L61($object)
	  {
	    $pdfObject = &$object;
      $pdfObject->ln();
      $pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
...
$pdfObject->ln();
    ...
    function HeaderINDEX_L61($object)
	  {
	    $pdfObject = &$object;
	  }
    function HeaderOIB_L61($object)
	  {
	    $pdfObject = &$object;
      $pdfObject->Ln();
      		$pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
  ...
  function HeaderATT_L61($object)
	{
   $pdfObject = &$object;
    $pdfObject->widthA = array(30,25,30,30,23,23,23,24,28,22,24);
...
function HeaderPERFG_L61($object)
{
	$pdfObject = &$object;
	$pdfObject->widthA = array(30,25,30,30,23,23,23,24,28,22,24);
	$pdfObject->alignA = array('L','R','R','R','R','R','R','R','R','R','R','R');
  ...
  function HeaderVOLK_L61($object)
	{
    $pdfObject = &$object;
   	$pdfObject->ln();
		$pdfObject->SetFont($pdfObject->rapport_font,$pdfObject->rapport_kop_fontstyle,$pdfObject->rapport_fontsize);
      ...
      $teken=$pdfObject->rapportageValuta;
		...
		$pdfObject->row(array("",
                    ...
                    "\n".vertaalTekst("Valuta",$pdfObject->rapport_taal),
										...
										vertaalTekst("Fonds-\nresultaat",$pdfObject->rapport_taal),
										vertaalTekst("Valuta-\nresultaat",$pdfObject->rapport_taal),
                    vertaalTekst("Dividenden\nen coupons",$pdfObject->rapport_taal),
										"\n".vertaalTekst("in %",$pdfObject->rapport_taal)
                    ));
...
if(!function_exists('PieChart_L61'))
{
  function PieChart_L61($pdfObject,$w,$h,$data, $format, $colors=null,$titel='',$legendaStart='')
  {
      ...
      if ($angle != 0)
      {
        ...
        if(abs($angleEnd-$angleStart) > 1)
        {
         // $pdfObject->Sector($XDiag, $YDiag, $radius, $angleStart, $angleEnd, 'F');
          $pdfObject->Sector($XDiag+(sin($avgAngle)*$factorUitElkaar), $YDiag-(cos($avgAngle)*$factorUitElkaar), $radius, $angleStart, $angleEnd);
        }
        if($val > 2)
        {
