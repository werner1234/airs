...
class RapportMUT_L13
{
  function RapportMUT_L13($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
  {
    $this->pdf = &$pdf;
...
//		$this->pdf->Line($totaal2+2,$this->pdf->GetY(),$totaal2 + $this->pdf->widthA[7],$this->pdf->GetY());
  ...
  function writeRapport()
  {
    ...
    foreach($buffer as $mutaties)
    {
      ...
      {
        $gbOmschrijving = $mutaties['gbOmschrijving'];
        if ( $gbOmschrijving === 'Transactiekosten' ) {
          $gbOmschrijving = $gbOmschrijving . ' (exclusief valutakosten)';
        }

        $this->printKop(vertaalTekst($gbOmschrijving,$this->pdf->rapport_taal), $this->pdf->rapport_kop3_fontstyle);
        $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
      }
