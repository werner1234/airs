...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: PDFRapport_headers_L76.php,v $
 		Revision 1.9  2019/07/03 15:37:22  rvv
 		*** empty log message ***
 		
 		Revision 1.8  2018/10/06 17:20:57  rvv
 		*** empty log message ***
 		
 		Revision 1.7  2018/09/26 15:53:28  rvv
 		*** empty log message ***
 		
 		Revision 1.6  2018/07/14 14:04:37  rvv
 		*** empty log message ***
 		
 		Revision 1.5  2018/05/02 16:13:47  rvv
 		*** empty log message ***
 		
 		Revision 1.4  2018/04/28 18:36:15  rvv
 		*** empty log message ***
 		
 		Revision 1.3  2018/04/22 09:30:28  rvv
 		*** empty log message ***
...
function Header_basis_L76($object)
{
  global $__appvar;
 $pdfObject = &$object;
    ...
    else
    {
      ...
      $pdfObject->SetDrawColor($pdfObject->rapport_oranje[0],$pdfObject->rapport_oranje[1],$pdfObject->rapport_oranje[2]);
      ...
      elseif($pdfObject->rapport_type == "Course" || $pdfObject->rapport_type == "Participatie")
      {
        ...
        $pdfObject->SetY($voetbeginY);
        $pdfObject->MultiCell(297,4,"Aan deze opgave kunnen geen rechten worden ontleend.",0,'C');
   	    $pdfObject->SetY($voetbeginY);
...
if(!function_exists('getValutaKoers'))
{
  ...
  function HeaderCourse_L76($object)
  {
    $pdfObject = &$object;
    $pdfObject->Ln(5.5);
    
  }
function HeaderINHOUD_L76($object)
{
	$pdfObject = &$object;
...
function HeaderParticipatie_L76($object)
  {
    $pdfObject = &$object;
  }

 	function HeaderVKM_L76($object)
	{
		$pdfObject = &$object;
		$pdfObject->HeaderVKM();
	}
...
function HeaderWaardePrognose_L76($object)
{
	$pdfObject = &$object;
	$pdfObject->setY(10);
	...
	$pdfObject->SetX($pdfObject->marge);
	$pdfObject->Cell(200,8, vertaalTekst("Waardeprognose & kosten", $pdfObject->rapport_taal) ,0,1,"L");
	$pdfObject->SetX(250);
	$pdfObject->ln(10);
	$pdfObject->SetFont($pdfObject->rapport_font,'',$pdfObject->rapport_fontsize);
...
function HeaderVKMD_L76($object)
{
	$pdfObject = &$object;
	$pdfObject->ln();
...
function HeaderFISCAAL_L76($object)
{
	$pdfObject = &$object;
	$pdfObject->ln(7);
...
function HeaderVOLK_L76($object)
{
	$pdfObject = &$object;
  $pdfObject->Ln(7);
	...
	$pdfObject->SetAligns($pdfObject->alignA);
 	$pdfObject->row(array("\n".vertaalTekst("Fondsomschrijving",$pdfObject->rapport_taal),
                    ...
                    vertaalTekst("Opgelopen\nRente",$pdfObject->rapport_taal),
										...
										vertaalTekst("Rendement\nhuidig jaar",$pdfObject->rapport_taal),
										vertaalTekst("Rendement\nvanaf start",$pdfObject->rapport_taal))
										);
...
unset($pdfObject->fillCell);
   ...
   function HeaderPERF_L76($object)
  {
	  	$pdfObject = &$object;
  ...
  function HeaderINDEX_L76($object)
  {
    $pdfObject = &$object;
  }

  function HeaderSCENARIO_L76($object)
{
    $pdfObject = &$object;
    $pdfObject->Ln();
  ...
  function HeaderMOD_L76($object)
{
  ...
  function HeaderMODEL_L76($object)
{
    $pdfObject = &$object;
    $pdfObject->SetY(5);
    $pdfObject->SetFont($pdfObject->rapport_font,$pdfObject->rapport_kop_fontstyle,$pdfObject->rapport_fontsize);
...
function HeaderTRANSFEE_L76($object)
{
  $pdfObject = &$object;
  $pdfObject->setY(25);
}

function HeaderTRANS_L76($object)
{
	$pdfObject = &$object;
	$pdfObject->SetFont($pdfObject->rapport_font,$pdfObject->rapport_kop_fontstyle,$pdfObject->rapport_fontsize);
	$pdfObject->SetX(100);
...
//	$pdfObject->ln();
	...
	$pdfObject->row(array(vertaalTekst("Datum",$pdfObject->rapport_taal),
										...
										vertaalTekst("Fonds",$pdfObject->rapport_taal),
										'',
										vertaalTekst("Aankoop koers",$pdfObject->rapport_taal),
										vertaalTekst("Aankoop waarde in ".$pdfObject->rapportageValuta,$pdfObject->rapport_taal),
										'',
										vertaalTekst("Verkoop koers",$pdfObject->rapport_taal),
  ...
  function HeaderMUT2_L76($object)
  {
   	$pdfObject = &$object;
    $pdfObject->Ln(7);
		$pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor[r],$pdfObject->rapport_kop_bgcolor[g],$pdfObject->rapport_kop_bgcolor[b]);
	...
	function HeaderOIB_L76($object)
	{
  	$pdfObject = &$object;
	}
  function HeaderOIV_L76($object)
  {
    $pdfObject = &$object;
  }
function HeaderOIS_L76($object)
{
	$pdfObject = &$object;
	...
	function HeaderCASHY_L76($object)
	{
  	  $pdfObject = &$object;
  	//  $pdfObject->ln();
	...
	function HeaderRISK_L76($object)
	{
	  $pdfObject = &$object;
    $pdfObject->SetTextColor($pdfObject->rapport_kop_fontcolor['r'],$pdfObject->rapport_kop_fontcolor['g'],$pdfObject->rapport_kop_fontcolor['b']);
    $pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
