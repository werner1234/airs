...
include_once("rapport/include/layout_114/ATTberekening_L114.php");


class RapportKERNV_L114
{
	function RapportKERNV_L114($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function formatGetalKoers($waarde, $dec , $start = false)
    ...
    $rapportageStartJaar=$this->rapportageDatumVanaf;
	  ...
	  $this->jaarTotalen=array();

    $att=new ATTberekening_L114($this);
    $att->indexPerformance=true;

    $this->waarden['Historie']=$att->bereken($rapportageStartJaar,  $this->rapportageDatum);
    ...
    { 
      ...
      if($lastCategorie <> '')
      {
        $this->pdf->excelData[]=array('Totaal',
           $this->jaarTotalen[$lastCategorie]['procent'],'','',
           $this->jaarTotalen[$lastCategorie]['indexPerf'],
           $this->jaarTotalen[$lastCategorie]['procent']- $this->jaarTotalen[$lastCategorie]['indexPerf'],
          $this->jaarTotalen[$lastCategorie]['allocatieEffect'],
          ( $this->jaarTotalen[$lastCategorie]['procent']- $this->jaarTotalen[$lastCategorie]['indexPerf'])-$this->jaarTotalen[$lastCategorie]['allocatieEffect']
          );
       ...
       foreach ($categorieData['perfWaarden'] as $datum=>$waarden)
      {
       // listarray($waarden);
        ...
        if($categorie!='totaal')
        { 
          $attributieEffect=(($waarden['weging']*$waarden['procentBruto'])-($waarden['indexPerf']*$waarden['indexBijdrageWaarde']));
          $allocatieEffect=($waarden['weging']-$waarden['indexBijdrageWaarde'])*($waarden['indexPerf']);
          $selectieEffect=($waarden['procentBruto']-$waarden['indexPerf'])*$waarden['indexBijdrageWaarde'];
          $interactieEffect=($waarden['weging']-$waarden['indexBijdrageWaarde'])*($waarden['procentBruto']-$waarden['indexPerf']);
          ...
          $this->maandEffecten[$categorie][$datum]['attributieEffect']=$attributieEffect;
          $this->maandEffecten[$categorie][$datum]['allocatieEffect']=$allocatieEffect;
          $this->maandEffecten[$categorie][$datum]['selectieEffect']=$selectieEffect;
          $this->maandEffecten[$categorie][$datum]['interactieEffect']=$interactieEffect;
          ...
          $this->jaarTotalen[$categorie]['attributieEffect']+=$attributieEffect;
          $this->jaarTotalen[$categorie]['allocatieEffect']+=$allocatieEffect;
          $this->jaarTotalen[$categorie]['selectieEffect']+=$selectieEffect;
          $this->jaarTotalen[$categorie]['interactieEffect']+=$interactieEffect;
                    
          $this->jaarTotalen['totaal']['portBijdrage']+=$waarden['bijdrage'];
          $this->jaarTotalen['totaal']['attributieEffect']+=$attributieEffect;
          $this->jaarTotalen['totaal']['allocatieEffect']+=$allocatieEffect;
          $this->jaarTotalen['totaal']['selectieEffect']+=$selectieEffect;
          $this->jaarTotalen['totaal']['interactieEffect']+=$interactieEffect;
...
/*
   ...
   $this->jaarTotalen[$categorie]['selectieEffect']+=($waarden['procent']-$waarden['indexPerf'])*$waarden['indexBijdrageWaarde'];
   $this->jaarTotalen[$categorie]['interactieEffect']+=($waarden['weging']-$waarden['indexBijdrageWaarde'])*($waarden['procent']-$waarden['indexPerf'])*100;
...
*/
    ...
    $this->pdf->templateVarsOmschrijving[$this->pdf->rapport_type.'Paginas']='Performance en attributie';
...
foreach ($this->jaarTotalen as $categorie=>$jaarWaarden)
{
  ...
  if($categorie<>'totaal')
  {
    if($categorie=='Liq-Extern'||$categorie=='Liquiditeiten')
    {
      $liqWerkelijkeAandeel+=round($jaarWaarden['weging'],3);
      $att->normData[$categorie]=$jaarWaarden['weging']*100;
    }
    elseif($categorie <> 'OBL')
    {
      $overigeWeging+= round($att->normData[$categorie],1);
    }
  }
}
if(isset($this->jaarTotalen['OBL']))
{
  $att->normData['OBL']=100-$overigeWeging-$liqWerkelijkeAandeel*100;
}

$att->normData['totaal']=0;
foreach($att->normData as $cat=>$waarde)
  $att->normData['totaal']+=$waarde;
   ...
   foreach ($this->jaarTotalen as $categorie=>$jaarWaarden)
    {
      ...
      $waarden=$jaarWaarden;
      if($categorie=='totaal' || $categorie=='Liq-Extern'||$categorie=='Liquiditeiten')
      {
      ...
      else
      {
        $this->pdf->row(array($categorieLookup[$categorie],
          ...
          $this->formatGetal($waarden['attributieEffect']*100 ,2),
          $this->formatGetal($waarden['allocatieEffect']*100,2),
          $this->formatGetal($waarden['selectieEffect']*100,2),
          $this->formatGetal($waarden['interactieEffect']*100,2)));
     ...
     // $this->pdf->AddPage();
      ...
      $this->maandEffectenCumulatief=array();
//listarray($this->maandEffecten);exit;
foreach($this->maandEffecten as $categorie=>$maandData)
{
  $laatsteWaarde=array();
  foreach($maandData as $maand=>$effectData)
  {
    foreach($effectData as $effect=>$waarde)
    {
      if(!isset($laatsteWaarde[$effect]))
        $laatsteWaarde[$effect]=0;
      
      $nieuweWaarde=$laatsteWaarde[$effect]+$waarde*100;
     // $nieuweWaarde=(((($laatsteWaarde[$effect]+100)/100) * (1+$waarde))*100)-100;
            
      $this->maandEffectenCumulatief[$categorie][$effect][$maand]=$nieuweWaarde;
      $laatsteWaarde[$effect]=$nieuweWaarde;
    }
  }
}


//listarray($this->maandEffectenCumulatief['AAND']);exit;
     ...
     // $this->maandTotalenCumulatief=array('attributieEffect'=>array(),'allocatieEffect'=>array(),'selectieEffect'=>array(),'interactieEffect'=>array());
      ...
      $this->LineDiagram(20,50+50,120,120-50,$this->maandEffectenCumulatief['RD-Aand'],'Aandelen');
      $this->LineDiagram(160,50+50,120,120-50,$this->maandEffectenCumulatief['RM-Bonds'],'Obligaties');
...
function LineDiagram($x,$y,$w, $h, $data, $title,$color=null, $maxVal=0, $minVal=0, $horDiv=4, $verDiv=4,$jaar=0)
  ...
  $colors=array('attributieEffect'=>array(241,89,38),'allocatieEffect'=>array(151,185,199),'selectieEffect'=>array(154,178,148),'interactieEffect'=>array(127,127,127)); //
    ...
    $xval=$x+5;
    $yval=$YPage+$hDiag+10;
    $colorsOmschrijving=array('attributieEffect'=>'attributie effect','allocatieEffect'=>'allocatie effect','selectieEffect'=>'selectie effect','interactieEffect'=>'interactie effect'); //
     ...
     foreach($colors as $effect=>$color)
    {
      ...
      $this->pdf->Cell(50, 3, $colorsOmschrijving[$effect],0,0,'L');
      $xval+=30;
    }   
