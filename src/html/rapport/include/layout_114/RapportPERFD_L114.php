...
class RapportPERFD_L114
{
	function RapportPERFD_L114($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
foreach ($indexData as $index=>$data)
{
  ...
  if($data['datum'] != '0000-00-00')
  {
    ...
    if($eindJaar==true || $laatste==true)
        ...
        $barGraph['absoluut'][$data['datum']][$categorie] += $waarde;
				if ($waarde <> 0)
				{
  ...
  function VBarDiagram($w, $h, $data,$absoluut=false)
  {
    ...
    $maxVal=100;
      foreach ($data as $datum=>$waarden)
      {
        ...
        {
          if ( ! isset ($waarden[$categorie]) ) {continue;}
    ...
    //  $numBars=10;
      ...
      if($stapgrootte<10000)
        $stapgrootte=ceil($stapgrootte);
