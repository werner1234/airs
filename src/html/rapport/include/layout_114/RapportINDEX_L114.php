...
class RapportINDEX_L114
{
	function RapportINDEX_L114($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		...
		$this->pdf->rapport_titel = "Tactische Asset Allocatie";
  ...
  //  echo($maxVal-$minVal). " $minVal | $maxVal<br>\n"; ob_flush();
    
    $maxVal=ceil($maxVal/5)*5+5;
    $minVal=floor($minVal/5)*5-5;
   /*
    $max=max(abs($maxVal),abs($minVal));
    $maxVal=$max;
    $minVal=$max*-1;
    ...
    */
    $nbDiv=($maxVal-$minVal)/5;
    if($nbDiv>8)
      $nbDiv=($maxVal-$minVal)/10;
    
  //   echo($maxVal-$minVal). " $minVal | $maxVal<br>\n"; ob_flush();
    $offset=$minVal;
    $valIndRepere = ($maxVal-$minVal) / $nbDiv;
  ...
  //  $this->pdf->Rect($XDiag, $YDiag, $lDiag, $hDiag);
    ...
    if(round($legendaStep,5) <> 0.0)
    {
      ...
      for($x=$nullijn;round($x)>=$XDiag; $x=$x-$legendaStep)
      {
        if($x<>$nullijn)
        {
          $this->pdf->Line($x, $YDiag, $x, $YDiag + $hDiag);
          $this->pdf->setXY($x, $YDiag + $hDiag);
          $waarde = round(($x - $nullijn) / $unit, 0);
          $this->pdf->Cell(0.1, 5, $waarde . "%", 0, 0, 'C');
        }
        $i++;
        if($i>100)
          break;
      ...
      for($x=$nullijn;round($x)<=($XDiag+$lDiag); $x=$x+$legendaStep)
      {
        if($x<>$nullijn)
        {
          $this->pdf->Line($x, $YDiag, $x, $YDiag + $hDiag);
...
//listarray($colorArray);listarray($data);
    ...
    foreach($data as $key=>$val)
    {
      ...
      if($val>0)
        ...
        $this->pdf->SetXY($nullijn-2-$this->pdf->GetStringWidth($key), $yval);
      $this->pdf->Cell($w , $hval, $key,0,0,'L');
      $i++;
    }
  ...
  function SetLegends2($data, $format)
  {
    ...
    global $__appvar;
       ...
       INNER JOIN ZorgplichtPerBeleggingscategorie ON TijdelijkeRapportage.hoofdcategorie = ZorgplichtPerBeleggingscategorie.Beleggingscategorie AND ZorgplichtPerBeleggingscategorie.Vermogensbeheerder='".$this->pdf->portefeuilledata['Vermogensbeheerder']."'
             WHERE ".
			" rapportageDatum ='".$this->rapportageDatum."' AND ".
			" portefeuille = '".$this->portefeuille."' "
			.$__appvar['TijdelijkeRapportageMaakUniek']." 
      GROUP BY Zorgplicht ORDER BY beleggingscategorieVolgorde";
		debugSpecial($query,__FILE__,__LINE__);
		$DB->SQL($query);
		$DB->Query();
		while($data=$DB->nextRecord())
...
WHERE ZorgplichtPerPortefeuille.Vermogensbeheerder='".$this->pdf->portefeuilledata['Vermogensbeheerder']."' AND ZorgplichtPerPortefeuille.Portefeuille='".$this->portefeuille."' $vanafWhere
 ORDER BY Zorgplicht";
    ...
    $verschilData=array();
      ...
      $percentage=$data['totaal']/$totaalWaarde*100;
      $verschilData[$zorgplichtCategorie]=$verdeling[$zorgplichtCategorie]['percentage']-$zorgData['norm'];
      $colorArray[$zorgplichtCategorie]=array(150,200,150);
    ...
    $this->pdf->setXY(30,$this->pdf->getY()+15);
    
    $w=100;
    $h=30;
    $format='';
    $titel='Afwijking tov de strategische asset allocatie';
    $this->BarDiagram($w, $h, $verschilData, $format,$colorArray,$titel,$max=0);
