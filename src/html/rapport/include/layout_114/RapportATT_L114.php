...
class RapportATT_L114
{
	function RapportATT_L114($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//listarray($this->categorieVolgorde);
  ...
  $index->voorStartdatumNegeren=true;
    $index->extraVerdeling = $extraVerdeling = 'hoofdcategorie';
...
foreach ($indexData as $index=>$data)
{
  ...
  //  foreach ($data['categorieVerdeling'] as $categorie=>$waarde)
    ...
    foreach ($data['extra'][$extraVerdeling] as $categorie=>$waarde)
    {
      if($categorie=='LIQ')
...
*/
      ...
      SELECT 
             hoofdcategorie, 
             hoofdcategorieOmschrijving as Omschrijving,
             hoofdcategorieVolgorde
                 
      FROM TijdelijkeRapportage 
      WHERE Portefeuille='".$this->portefeuille."' 
      AND hoofdcategorie <>'' 
      AND hoofdcategorieOmschrijving <> '' 
      GROUP BY hoofdcategorie  
      ORDER BY hoofdcategorieVolgorde asc"; //WHERE Beleggingscategorie IN('LIQ','ZAK','VAR','Liquiditeiten')

    $DB->SQL($q);
		$DB->Query();
		while($data=$DB->nextRecord())
		{
		  $this->categorieVolgorde[$data['hoofdcategorie']]=$data['Omschrijving'];
  ...
  function VBarDiagram($w, $h, $data)
  {
    ...
    $maxVal=100;
      foreach ($data as $datum=>$waarden)
      {
        ...
        foreach (array_reverse($this->categorieVolgorde) as $categorie => $categorieName)
        {
      ...
      $n=0;
      foreach (($this->categorieVolgorde) as $categorie => $catName)//array_reverse
      {
