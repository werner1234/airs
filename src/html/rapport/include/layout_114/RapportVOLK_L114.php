...
class RapportVOLK_L114
{
	function RapportVOLK_L114($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function getDividend($fonds)
  {
    ...
    $this->pdf->templateVars['VOLKPaginas']=$this->pdf->page;
		...
		$actueleWaardePortefeuille = 0;
			...
			$query = "SELECT hoofdcategorieOmschrijving, TijdelijkeRapportage.BeleggingscategorieOmschrijving as Omschrijving, ".
			" TijdelijkeRapportage.beleggingscategorie,
			SUM(if(TijdelijkeRapportage.type = 'fondsen',TijdelijkeRapportage.beginPortefeuilleWaardeEuro,TijdelijkeRapportage.actuelePortefeuilleWaardeEuro)) / ".$this->pdf->ValutaKoersBegin. " as subtotaalbeginRendement,
      SUM(if(TijdelijkeRapportage.type = 'fondsen',TijdelijkeRapportage.beginPortefeuilleWaardeEuro,0)) / ".$this->pdf->ValutaKoersBegin. " as subtotaalbegin,".
	  //  "
    ...
    //  ".
		...
		while($categorien = $DB->NextRecord())
		{
           ...
           $procentResultaat = (($totaalactueel - $totaalbeginRendement + $totaaldividendCorrected) / ($totaalbeginRendement /100));
		    if($totaalbegin < 0)
      ...
      if($lastHoofdCategorie <> $categorien['hoofdcategorieOmschrijving'])
        $this->printKop(vertaalTekst($categorien['hoofdcategorieOmschrijving'],$this->pdf->rapport_taal), "bi");
			...
			$subquery = "SELECT TijdelijkeRapportage.type, TijdelijkeRapportage.fondsOmschrijving, ".
...
//echo $subquery."<br><br>";exit;
      ...
      $kopPrinted=false;
        ...
        $dividendtxt='';
          ...
          $subtotaalPercentageVanTotaal  = ($subdata['actuelePortefeuilleWaardeEuro']) / ($totaalWaarde/100);
        ...
        $n++;
			...
			// print categorie footers
				$procentResultaat = (($categorien['subtotaalactueel']  - $categorien['subtotaalbeginRendement'] + $subtotaal['totaalDividendCorrected'] ) / ($categorien['subtotaalbeginRendement']  /100));
...
//rvv
      ...
      $totaalbeginRendement   += $categorien['subtotaalbeginRendement'];
