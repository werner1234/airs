...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2014/03/22 15:47:14 $
File Versie					: $Revision: 1.1 $

$Log: RapportOIB_L10.php,v $
Revision 1.1  2014/03/22 15:47:14  rvv
*** empty log message ***
...
class RapportOIB_L10
{
	function RapportOIB_L10($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function formatGetalKoers($waarde, $dec , $start = false)
	...
	function printSubTotaal($title, $totaalA, $totaalB)
	{
   	...
   	$this->pdf->SetWidths($this->pdf->widthB);
		...
		if(!empty($totaalB))
			$totaalprtxt = $this->formatGetal($totaalB,1);
      
    $this->pdf->SetX($this->pdf->marge);
    $this->pdf->SetFont($this->pdf->rapport_font,'bi',$this->pdf->rapport_fontsize);
    $this->pdf->Cell($this->pdf->widthB[0],4,$title, 0,0, "L");
...
$this->pdf->Line($actueel+2,$this->pdf->GetY(),$actueel + $this->pdf->widthB[4]+$this->pdf->widthB[5],$this->pdf->GetY());
	...
	function writeRapport()
	{
		...
		// voor data
		$this->pdf->widthB = array(60,20,20,25,25,15,115);
    ...
    TijdelijkeRapportage.HoofdcategorieOmschrijving as HOmschrijving,".
      ...
      if($lastHCategorie <> $categorien['HOmschrijving'] && !empty($lastHCategorie) )
      {

        $percentageVanTotaal = $totaalHcat / ($totaalWaarde/100);
        $this->printSubTotaal("Totaal ".$lastHCategorie, $totaalHcat, $percentageVanTotaal);
        $totaalHcat=0;
      }
      ...
      $totaalHcat += $categorien['subtotaalactueel'];
      $lastCat       = $categorien['beleggingscategorie'];
			$lastCategorie = $categorien['Omschrijving'];
      $lastHCategorie = $categorien['HOmschrijving'];
		}
		...
		if($this->pdf->rapport_layout != 14)
		{
          ...
          if($lastHCategorie <> $data['HOmschrijving'] && !empty($lastHCategorie) )
          {
            $percentageVanTotaal = $totaalHcat / ($totaalWaarde/100);
            $this->printSubTotaal("Totaal ".$lastHCategorie, $totaalHcat, $percentageVanTotaal);
            $totaalHcat=0;
          }
          $lastHCategorie = $data['HOmschrijving'];
		}
