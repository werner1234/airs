...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2018/11/07 10:25:22 $
File Versie					: $Revision: 1.1 $

$Log: RapportVOLK_L13.php,v $
Revision 1.1  2018/11/07 10:25:22  rvv
*** empty log message ***
...
class RapportVOLK_L13
{
	function RapportVOLK_L13($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//echo $subquery."<br><br>";exit;
        ...
        if ($subdata['OptieExpDatum'] != '')
          ...
          $vervalDatum = '';
                ...
                if ($subdata['OptieExpDatum'] != '')
                {
                  $optieParts = explode(" ", $subdata['fondsOmschrijving'], 2);
                  $this->pdf->row(array("",
                                    trim($optieParts[0]).' '. trim($optieParts[1]." ".$vervalDatum),
