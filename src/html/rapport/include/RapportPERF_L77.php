...
class RapportPERF_L77
{

	function RapportPERF_L77($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  //  echo $this->pdf->portefeuilledata['SpecifiekeIndex']."<br>\n";
   // foreach($indexWaardenHuidigeJaar as $row)
   //   echo $row['datum']." | ".$row['specifiekeIndex']." | ".$row['specifiekeIndexPerformance']."<br>\n";
   // listarray($indexWaardenHuidigeJaar);
    //exit;
    $jaarWaarden=array();
    ...
    $verslagperiodeWaarden=array();
		...
		if(db2jul($this->pdf->PortefeuilleStartdatum)>$huidigeJaarJul)
      $huidigeJaarJul=db2jul($this->pdf->PortefeuilleStartdatum);
   ...
   // listarray($indexWaarden);
      ...
      {
        $brutoResultaat=$maandwaarden['resultaatVerslagperiode']-$maandwaarden['kosten'];
        $gemiddeldVermogen=$maandwaarden['resultaatVerslagperiode']/($maandwaarden['performance']*0.01);
        $brutoRendement=$brutoResultaat/$gemiddeldVermogen*100;
      ...
      $jaarWaarden[$jaar]['brutoResultaat']+=($maandwaarden['resultaatVerslagperiode']-$maandwaarden['kosten']);
     ...
     // $jaarWaardenGrafiek[$jaar]['benchmark']=$jaarWaarden[$jaar]['benchmark'];
		}
		...
		foreach($indexWaardenHuidigeJaar as $maandwaarden)
    {
      ...
      if(db2jul($beginDatum)>=$this->pdf->rapport_datumvanaf)
      {
        ...
        $verslagperiodeWaarden['brutoResultaat']+=($maandwaarden['resultaatVerslagperiode']-$maandwaarden['kosten']);
    ...
    $this->pdf->row(array(vertaalTekst("Waarde per",$this->pdf->rapport_taal)." ".date("d.m.Y",$this->pdf->rapport_datumvanaf),'',$this->formatGetal($waardeBegin,2)));
    $this->pdf->row(array(vertaalTekst("Waarde per",$this->pdf->rapport_taal)." ".date("d.m.Y",db2jul($this->rapportageDatum)),'',$this->formatGetal($waardeEind,2)));
    $this->pdf->row(array(vertaalTekst("Waardeverandering",$this->pdf->rapport_taal),'',$this->formatGetal($waardeMutatie,2),""));
    $this->pdf->ln();
    $this->pdf->row(array(vertaalTekst("Stortingen",$this->pdf->rapport_taal),'',$this->formatGetal($stortingen,2),''));
    $this->pdf->row(array(vertaalTekst("Onttrekkingen",$this->pdf->rapport_taal),'',$this->formatGetal($onttrekkingen,2),''));
    ...
    //$gemiddeldVermogen=$resultaatVerslagperiode/($rendementProcent*0.01);
    //$brutoRendement=$brutoResultaat/$gemiddeldVermogen*100;
    $this->pdf->row(array(vertaalTekst("Bruto resultaat voor kosten en belastingen",$this->pdf->rapport_taal),'',$this->formatGetal($verslagperiodeWaarden['brutoResultaat'],2),''));
    $this->pdf->row(array(vertaalTekst("Bruto rendement voor kosten en belastingen",$this->pdf->rapport_taal),'',$this->formatGetal($perfVerslagperiodeBruto,2)."%",''));
    $this->pdf->ln();
    //$this->pdf->row(array(vertaalTekst("Resultaat",$this->pdf->rapport_taal),'',$this->formatGetal($resultaatVerslagperiode,2),''));
    //$this->pdf->ln();
    
    $this->pdf->row(array(vertaalTekst("Netto rendement na ingehouden kosten en belastingen",$this->pdf->rapport_taal),'',$this->formatGetal($rendementProcent,2)."%"));
    ...
    //	listarray($huidigeJaarGrafiek);
		...
		$this->pdf->ln();
		$this->pdf->SetWidths(array(20,10,20,20,20,20,20));
    ...
    $this->pdf->ln(1);
  	$this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
		$this->pdf->row(array(vertaalTekst("Jaar",$this->pdf->rapport_taal),vertaalTekst("Val",$this->pdf->rapport_taal),vertaalTekst("Waarde",$this->pdf->rapport_taal),vertaalTekst("Mutaties",$this->pdf->rapport_taal),
											vertaalTekst("Kosten en belasting",$this->pdf->rapport_taal),vertaalTekst("Bruto resultaat",$this->pdf->rapport_taal),vertaalTekst("Bruto rendement",$this->pdf->rapport_taal)));
		$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
    $this->pdf->SetTextColor(0);
    $this->pdf->ln(1);
    $n=0;
		...
		foreach($jaarWaarden as $jaar=>$jaarData)
    {
      ...
      $this->pdf->row(array($datum, $this->pdf->rapportageValuta, $this->formatGetal($jaarData['waardeHuidige'], 0),
                        ...
                        $this->formatGetal($jaarData['brutoResultaat'], 0),
      ...
      $totalen['brutoResultaat']+=$jaarData['brutoResultaat'];
    ...
    $this->pdf->row(array(vertaalTekst('Totaal sinds',$this->pdf->rapport_taal).' '.$begin,
                      ...
                      $this->formatGetal($totalen['brutoResultaat'], 0),
    ...
    //listarray($huidigeJaarGrafiek);listarray($jaarWaardenGrafiek);
    $grafiekX=120;
    ...
    $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
    $this->pdf->MultiCell($grafiekX,4,vertaalTekst('Historisch rendement',$this->pdf->rapport_taal).' (%)',0,'C');
