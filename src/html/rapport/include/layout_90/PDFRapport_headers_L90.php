...
/*
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2020/06/27 16:24:16 $
 		File Versie					: $Revision: 1.2 $

 		$Log: PDFRapport_headers_L90.php,v $
...
function Header_basis_L90($object)
{
 $pdfObject = &$object;
    ...
    if(empty($pdfObject->lastPortefeuille) || $pdfObject->lastPortefeuille != $pdfObject->rapport_portefeuille)
    {
		...
		{
		   $factor=0.03;
		   $x=1916*$factor;//$x=885*$factor;
		   $y=706*$factor;//$y=849*$factor;
       $xStart=(297)/2-($x/2);
		   $pdfObject->Image($pdfObject->rapport_logo, $xStart, 4, $x, $y);
		}
...
function HeaderFRONT_L90($object)
{
$pdfObject = &$object;
}

	function HeaderVKM_L90($object)
	{
		$pdfObject = &$object;
		$pdfObject->HeaderVKM();
	}
...
function HeaderVOLK_L90($object)
{
	$pdfObject = &$object;
  $pdfObject->Ln();
...
function HeaderHUIS_L90($object)
{
    $pdfObject = &$object;
 ...
 function HeaderSCENARIO_L90($object)
{
    $pdfObject = &$object;
   ...
   function HeaderPERF_L90($object)
  {
	  	$pdfObject = &$object;
   ...
   function HeaderTRANS_L90($object)
  {
    $pdfObject=&$object;
    $pdfObject->ln();
		$pdfObject->SetFont($pdfObject->rapport_font,$pdfObject->rapport_kop_fontstyle,$pdfObject->rapport_fontsize);
  ...
  function HeaderOIH_L90($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->HeaderOIH();
	}

	function HeaderOIBS_L90($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->HeaderOIBS();
	}

	function HeaderOIR_L90($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->HeaderOIR();
	}

	function HeaderHSE_L90($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->HeaderHSE();
	}

	function HeaderOIB_L90($object)
	{
  	  $pdfObject = &$object;
	...
	function HeaderOIV_L90($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->HeaderOIV();
	}

	function HeaderPERFG_L90($object)
	{
  	  $pdfObject = &$object;
    $pdfObject = &$object;
	...
	function HeaderPERFD_L90($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->HeaderPERFD();
	}
	function HeaderVOLKD_L90($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->HeaderVOLKD();
	}
	function HeaderVHO_L90($object)
	{
	$pdfObject = &$object;
  $pdfObject->Ln();
...
unset($pdfObject->fillCell);
	...
	function HeaderGRAFIEK_L90($object)
	{
  	  $pdfObject = &$object;
	...
	function HeaderCASH_L90($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->HeaderCASH();
	}
	function HeaderCASHY_L90($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->ln();
	...
	function HeaderMODEL_L90($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->HeaderMODEL();
	}
	function HeaderSMV_L90($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->HeaderSMV();
	}
	...
	function HeaderRISK_L90($object)
	{
  	  $pdfObject = &$object;
  	  $pdfObject->Line($pdfObject->marge,$pdfObject->GetY()+7,$pdfObject->marge + 283,$pdfObject->GetY()+7);
	}
