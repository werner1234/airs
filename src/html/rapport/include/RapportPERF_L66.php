...
$Log: RapportPERF_L66.php,v $
Revision 1.6  2020/03/28 15:45:39  rvv
*** empty log message ***

Revision 1.5  2018/08/18 12:40:15  rvv
php 5.6 & consolidatie

Revision 1.4  2017/11/29 16:19:12  rvv
*** empty log message ***
...
include_once("rapport/include/ATTberekening_L66.php");

class RapportPERF_L66
{

	function RapportPERF_L66($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    $this->att=new ATTberekening_L66($this);
	}
	...
	function writeRapport()
	{
		...
		// voor kopjes
		$this->pdf->widthB = array(0,85,30,5,30,5,30,97);
  ...
  foreach($categorien as $categorie)
  {
    ...
    else
    {
      if($categorie<>'G-LIQ')
      {
        ...
        $onttrekking[]='';
        $totaalEffMut += $perfWaarden['stort'];
      }
      else //G-LIQ
      {
        $effectenmutaties[]=$this->formatGetal($totaalEffMut*-1,0);
        $effectenmutaties[]='';
        $stortingen[]=$this->formatGetal(getOnttrekkingen($this->portefeuille, $this->rapportageDatumVanaf, $this->rapportageDatum),0);
        $stortingen[]='';
        $onttrekking[]=$this->formatGetal(getStortingen($this->portefeuille, $this->rapportageDatumVanaf, $this->rapportageDatum),0);
        $onttrekking[]='';
      }
...
WHERE Portefeuilles.Portefeuille = '$this->portefeuille'";
   	$DB->SQL($query);
		$DB->Query();
    $specifiekeIndex=array();
	  while($index = $DB->nextRecord())
    {
      $specifiekeIndex=$index;
    }

   // $specifiekeIndex['Beursindex']='VAL BM Neutraal';
    $query="SELECT benchmarkverdeling.fonds,benchmarkverdeling.percentage,Fondsen.Omschrijving FROM benchmarkverdeling JOIN Fondsen ON benchmarkverdeling.fonds=Fondsen.Fonds WHERE benchmarkverdeling.benchmark='".mysql_real_escape_string($specifiekeIndex['Beursindex'])."'";
    $DB->SQL($query);
    $DB->Query();
    $verdeling=array();
    $verdelingTxt='* De ER Capital '.$specifiekeIndex['Omschrijving'].' bestaat voor ';
    $n=0;
    while($data=$DB->nextRecord())
    {
      if($n>0)
        $verdelingTxt.=" en voor ";
      $verdelingTxt.=''.$this->formatGetal($data['percentage'],1).'% uit de '.$data['Omschrijving'];
      $verdeling[]=$data;
      $n++;
    }
    if(count($verdeling)>0)
    {
      $specifiekeIndex['Omschrijving'].='*';
      $verdelingTxt.=".";
    }
    else
    {
    ...
    $indices[]=$specifiekeIndex;
      ...
      //$indexData[$index['Beursindex']]['performanceEur'] =     ($indexData[$index['Beursindex']]['fondsKoers_eind']*$indexData[$index['Beursindex']]['valutaKoers_eind'] - $indexData[$index['Beursindex']]['fondsKoers_begin']*$indexData[$index['Beursindex']]['valutaKoers_begin'])/($indexData[$index['Beursindex']]['fondsKoers_begin']*$indexData[$index['Beursindex']]['valutaKoers_begin']/100 );
		...
		$this->pdf->SetY(120);
    $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
  	$this->pdf->SetWidths(array(150,70,20,20,20));
  	$this->pdf->SetAligns(array('L','L','R','R','R'));
    $this->pdf->Rect($this->pdf->marge+150,120,130,count($benchmarks)*4+4);
    ...
    if(count($specialeBenchmarks) > 0)
    {
    	 ...
    	 $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
  	 	$this->pdf->SetWidths(array(150,70,20,20,20));
  	 	$this->pdf->SetAligns(array('L','L','R','R','R'));
     	$this->pdf->Rect($this->pdf->marge+150,150,130,count($specialeBenchmarks)*4+4);
    ...
    if($verdelingTxt<>'')
    {
      $this->pdf->ln();
      $this->pdf->SetWidths(array(150, 70 + 20 + 20 + 20));
      $this->pdf->row(array('', $verdelingTxt));
    }
