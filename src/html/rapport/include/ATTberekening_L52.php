...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: ATTberekening_L52.php,v $
 		Revision 1.3  2019/05/01 15:53:25  rvv
 		*** empty log message ***
 		
 		Revision 1.2  2019/04/27 18:28:16  rvv
 		*** empty log message ***
 		
 		Revision 1.1  2019/04/07 11:07:21  rvv
 		*** empty log message ***
...
class ATTberekening_L52
{

	function ATTberekening_L52($rapportData,$periode='maanden')
	{
...
ORDER BY HoofdBeleggingscategorien.Afdrukvolgorde, Regios.Afdrukvolgorde,Beleggingscategorien.Afdrukvolgorde";
    ...
    /*
    $categorieZorgplicht=array('H-Aand'=>'ZAK','H-Obl'=>'VAR','H-Liq'=>'LIQ','H-Alter'=>'ALT');
    $zorgplicht=$categorieZorgplicht[$categorie];
    $query="SELECT ZorgplichtPerRisicoklasse.norm / 100 as Percentage FROM ZorgplichtPerRisicoklasse WHERE Vermogensbeheerder='".$this->rapport->pdf->portefeuilledata['Vermogensbeheerder']."' AND Zorgplicht='$zorgplicht'";
    $DB->SQL($query);
    $normData=$DB->lookupRecord();
    
    if($categorie=='totaal')
      $normData['Percentage']=1;
    
    if(!isset($this->normData[$categorie]))
      $this->normData[$categorie]=$normData['Percentage']*100;
    */
    
    if($categorie=='totaal')
      $this->normData[$categorie]=100;
    
    if(!isset($this->normData[$categorie]))
    {
      $query = "SELECT
sum(benchmarkverdeling.percentage) as percentage,
CategorienPerHoofdcategorie.Hoofdcategorie
FROM
benchmarkverdeling
INNER JOIN BeleggingscategoriePerFonds ON benchmarkverdeling.fonds = BeleggingscategoriePerFonds.Fonds
AND BeleggingscategoriePerFonds.Vermogensbeheerder='" . $this->rapport->pdf->portefeuilledata['Vermogensbeheerder'] . "'
INNER JOIN CategorienPerHoofdcategorie ON BeleggingscategoriePerFonds.Beleggingscategorie = CategorienPerHoofdcategorie.Beleggingscategorie
AND CategorienPerHoofdcategorie.Vermogensbeheerder='" . $this->rapport->pdf->portefeuilledata['Vermogensbeheerder'] . "'
WHERE
benchmarkverdeling.benchmark='" . $this->rapport->pdf->portefeuilledata['SpecifiekeIndex'] . "' GROUP BY CategorienPerHoofdcategorie.Hoofdcategorie";
      $DB->SQL($query);
      $DB->Query();
      while($data=$DB->nextRecord())
      {
        $this->normData[$data['Hoofdcategorie']]=$data['percentage'];
      }
    }
    $normData['Percentage']=$this->normData[$categorie]/100;
    
    
    if($categorie=='totaal')
      $query="SELECT Specifiekeindex as Fonds FROM Portefeuilles where Portefeuille='".$this->rapport->portefeuille."'";
    else
      $query="SELECT IndexPerBeleggingscategorie.Fonds FROM IndexPerBeleggingscategorie WHERE IndexPerBeleggingscategorie.Vermogensbeheerder='".$this->rapport->pdf->portefeuilledata['Vermogensbeheerder']."' AND IndexPerBeleggingscategorie.Beleggingscategorie='$categorie' ";
   // echo $query."<br>\n";
	  $DB->SQL($query);
   ...
   // $waarden[$periode['stop']]=array('perf'=>$perf,'aandeel'=>$fondsData['Percentage']);
    $tmp= array('perf'=>$perf,'bijdrage'=>$perf*$normData['Percentage'],'datum'=>$tot,'percentage'=>$normData['Percentage'],'categorie'=>$categorie,'koersVan'=>$startKoers['Koers'],'koersEind'=>$eindKoers['Koers']);//,'waarden'=>$waarden)
