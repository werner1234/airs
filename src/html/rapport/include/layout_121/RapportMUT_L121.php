...
class RapportMUT_L121
{
	function RapportMUT_L121($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	...
	function writeRapport()
	{
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   ...
	   $extraquery .= "AND Rekeningmutaties.Grootboekrekening IN('$grootboekSelectie')  ";
		...
		{
      if ( $mutaties['Grootboekrekening'] == 'BEH' ) {
        $mutaties['gbOmschrijving']="Adviesvergoeding";
      }
      if($_POST['anoniem']==1)
      {
