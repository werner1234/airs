...
class RapportVKM_L121
{
	function RapportVKM_L121($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum,$laatstejaar=true)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    ...
    $alleGrootboeken=array();
		...
		{
      if ( isset ($this->changeGrootboekOmschrijving) && isset ($this->changeGrootboekOmschrijving[$data['Grootboekrekening']]) ) {
        $data['Omschrijving'] = $this->changeGrootboekOmschrijving[$data['Grootboekrekening']];
      }
      if($data['Kosten']==1)
      {
        if ($this->pdfVullen == true)
