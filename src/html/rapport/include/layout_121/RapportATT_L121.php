...
class RapportATT_L121
{
	function RapportATT_L121($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
   ...
   if(count($rendamentWaarden) > 0)
   {
     ...
     //   $this->pdf->ln(3);
        ...
        $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
		  ...
		  if (count($barGraph) > 0)
		  {
		    $this->pdf->SetXY($this->pdf->marge,115)		;//112
        $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
...
//    listarray($tmp);exit;
 ...
 //   foreach ($tmp as $cat=>$percentage)
    ...
    foreach ($categorieWaarden as $cat=>$percentage)
    {
      ...
      {
        $this->pdf->MemImage($this->checkImg, 95 + $xMarge, $this->pdf->getY(), 3.9, 3.9);
      }
      else
      {
        $this->pdf->MemImage($this->deleteImg, 95 + $xMarge, $this->pdf->getY(), 3.9, 3.9);
        $voldoet=false;
      }
