...
class RapportGRAFIEK_L121
{
  function RapportGRAFIEK_L121($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
  {
    $this->pdf = $pdf;
...
WHERE bronKoppeling='$airsCategorie' AND doorkijkCategoriesoort='" . mysql_real_escape_string($doorkijkSoort) . "' AND systeem='AIRS' AND vermogensbeheerder='" . $this->pdf->portefeuilledata['Vermogensbeheerder'] . "'";
  ...
  function bepaalWegingNormaal($doorkijkSoort, $belCategorie = '')
  {
    ...
    if( is_array($belCategorie) && ! empty ($belCategorie) ) {
      $fondsFilter="AND Beleggingscategorie IN('".implode("','",$belCategorie)."')";
    } elseif ( ! empty ($belCategorie) )
    {
    ...
    $query = "
      SELECT TijdelijkeRapportage.type, 
      if(" . $vertaling[$doorkijkSoort] . "Volgorde = 0,127," . $vertaling[$doorkijkSoort] . "Volgorde) as volgorde, 
      sum(actuelePortefeuilleWaardeEuro) as waardeEUR, 
      " . $vertaling[$doorkijkSoort] . " as verdeling , 
      " . $vertaling[$doorkijkSoort] . "Omschrijving as Omschrijving
					
					FROM TijdelijkeRapportage	
					WHERE rapportageDatum ='" . $this->rapportageDatum . "'  
					$fondsFilter 
  ...
  function vulPagina($belCategorie = '')
  {
    ...
    $doorkijkTitels = array('Beleggingscategorien' => 'Beleggingscategorie', 'Regios' => 'Regioverdeling Aandelen', 'Beleggingssectoren' => 'Sectorverdeling Aandelen');//array();Beleggingscategorie
    foreach ($doorkijkCategorieSoorten as $index => $doorkijkCategorieSoort)
    {
      ...
      {
        $this->printPie($grafiekdata, 30 +$xStartOffset+ $xOffset, 45, $doorkijkTitels[$doorkijkCategorieSoort]);
      } //+$yOffset);
      $pieTeller++;
...
*/
    ...
    $query="
      ...
      WHERE KeuzePerVermogensbeheerder.categorieIXP = 'Aandelen'
      AND portefeuille='" . $this->portefeuille . "'
    ...
    while($data = $db->nextRecord())
    {
      $beleggingscategorien[$data['beleggingscategorie']]=$data['beleggingscategorie'];
    }
