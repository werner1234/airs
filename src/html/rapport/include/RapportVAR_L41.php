...
class RapportVAR_L41
{
	function RapportVAR_L41($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	 //
		$this->pdf = &$pdf;
...
GROUP BY TijdelijkeRapportage.Beleggingssector ORDER BY beleggingssectorVolgorde desc ";
  ...
  function vastrentendeDeel()
  {
  	//$this->pdf->templateVars['VARPaginas'] = $this->pdf->customPageNo;
		...
		$this->pdf->SetWidths(array(30,60,20,20,20,20,20,20));
		$this->pdf->SetAligns(array("L",'L','R','R','R','R','R','R'));

    //$this->pdf->CellBorders = array('','','US','US','US','US','US','US','US');
    ...
    $this->pdf->SetTextColor(255,255,255);
     ...
     vertaalTekst("Instrument\n ",$this->pdf->rapport_taal),
		 vertaalTekst("Actuele\nWaarde",$this->pdf->rapport_taal),
		 vertaalTekst("Rating instrument",$this->pdf->rapport_taal),

		 vertaalTekst("Coupon Yield",$this->pdf->rapport_taal),
		 vertaalTekst("Yield to Maturity",$this->pdf->rapport_taal),
...
ORDER BY TijdelijkeRapportage.beleggingscategorieVolgorde,TijdelijkeRapportage.fondsOmschrijving,TijdelijkeRapportage.rekening";
      ...
      $this->ratingVerdeling[$fondsRating]+=$data['actuelePortefeuilleWaardeEuro']/$this->actueleWaardePortefeuille;
...
$colors[]=array($this->pdf->lightgreen[0]*$tint,$this->pdf->lightgreen[1]*$tint,$this->pdf->lightgreen[2]*$tint);//$this->pdf->lightgreen;
    ...
    PieChart_L41($this->pdf,$chartsize,$chartsize,$ratingGrafiek,'%l',$colors,'Rating',$legendaStart);//'%l (%p)'
