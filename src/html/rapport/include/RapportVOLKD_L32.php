...
class RapportVOLKD_L32
{
	function RapportVOLKD_L32($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//echo $data['fondsOmschrijving']." | ".$data['actuelePortefeuilleWaardeEuro']."+".$extra['opbrengstTotaal'].")/$aankoopEur <br>\n";
    ...
    PieChart_L32($this->pdf, $chartsize, $vwh, $grafiek['Aanvang']['Percentage'], '%l', $grafiek['Aanvang']['Kleur'], vertaalTekst('Aanvang', $this->pdf->rapport_taal), $legendaStart);

    $i=1;
    $this->pdf->setXY($this->pdf->marge+5+90*$i , $ystart);
    $legendaStart=array($this->pdf->marge+5+90*$i,$ystart+$chartsize+10);
    PieChart_L32($this->pdf, $chartsize, $vwh, $grafiek['Strategie']['Percentage'], '%l', $grafiek['Strategie']['Kleur'], vertaalTekst('Strategie', $this->pdf->rapport_taal), $legendaStart);
    /*  */
