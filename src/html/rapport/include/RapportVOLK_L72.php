...
$Log: RapportVOLK_L72.php,v $
Revision 1.32  2018/12/12 16:19:08  rvv
*** empty log message ***

Revision 1.31  2017/10/07 17:19:52  rvv
*** empty log message ***

Revision 1.30  2017/08/19 18:18:00  rvv
*** empty log message ***

Revision 1.29  2017/08/16 15:58:06  rvv
*** empty log message ***

Revision 1.28  2017/07/22 18:22:05  rvv
*** empty log message ***

Revision 1.27  2017/05/26 16:45:07  rvv
*** empty log message ***

Revision 1.26  2017/04/15 19:11:50  rvv
*** empty log message ***
...
class RapportVOLK_L72
{
	function RapportVOLK_L72($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function gemiddeldeTransactieValutaKoers($fonds)
	...
	//	$rapDatumTekst=date("j",$this->pdf->rapport_datum)." ".vertaalTekst($this->pdf->__appvar["Maanden"][date("n",$this->pdf->rapport_datum)],$this->pdf->rapport_taal)." ".date("Y",$this->pdf->rapport_datum);
		...
		$query="SELECT SUM(actuelePortefeuilleWaardeEuro) / ".$this->pdf->ValutaKoersEind." * ".$this->aandeel." as actuelePortefeuilleWaardeEuro FROM TijdelijkeRapportage WHERE
...
SUM(IF(TijdelijkeRapportage.type = 'fondsen',TijdelijkeRapportage.historischeWaarde,0)) * ".$this->aandeel." AS historischeWaarde,
SUM(IF(TijdelijkeRapportage.type = 'rente' , (actuelePortefeuilleWaardeEuro),0)) / ".$this->pdf->ValutaKoersEind." * ".$this->aandeel." AS rente,
SUM(TijdelijkeRapportage.actuelePortefeuilleWaardeEuro) / ".$this->pdf->ValutaKoersEind." * ".$this->aandeel." AS actuelePortefeuilleWaardeEuro,
SUM(IF(TijdelijkeRapportage.type = 'fondsen',(beginPortefeuilleWaardeEuro),0)) * ".$this->aandeel." AS beginPortefeuilleWaardeEuro,
SUM(IF(TijdelijkeRapportage.type = 'rekening' ,actuelePortefeuilleWaardeInValuta, IF(TijdelijkeRapportage.type = 'fondsen',totaalAantal,0))) * ".$this->aandeel." AS totaalAantal,
...
";
		...
		if(isset($this->pdf->__appvar['consolidatie']) && is_array($this->pdf->__appvar['consolidatie']))
		{
		  ...
		  {
				if ($data['type'] == 'rekening' && $data['beleggingscategorie']=='LIQ' )
				{
			...
			else
      {
        $markering = "";
			...
			else
			{
				...
				$aantal=$this->formatAantal($data['totaalAantal'],0,true);
			}
