...
Revision 1.5  2018/02/17 19:18:57  rvv
*** empty log message ***

Revision 1.4  2017/12/04 06:58:52  rvv
*** empty log message ***

Revision 1.3  2017/12/02 19:13:32  rvv
*** empty log message ***
...
class RapportVOLKD_L33
{
	function RapportVOLKD_L33($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	  ...
	  $this->db=new DB();
    $this->pdf->excelData[]=array("Naam",'Valuta','Aantal','Koers','Begin koers','Marktwaarde','Beginwaarde','Fondsresultaat','Valutaresultaat','% portf.','Opgelopen rente');
...
SUM(TijdelijkeRapportage.actuelePortefeuilleWaardeInValuta) / ".$this->pdf->ValutaKoersEind." AS actuelePortefeuilleWaardeInValuta ,
...
//echo $data['fonds']." -> ".$data['beginPortefeuilleWaardeEuro']." = ".$data['beginPortefeuilleWaardeEuro']."  / ".$data['historischeValutakoers']."<br>\n";        
      ...
      else
      {
        ...
        $fondsResultaat = ($data['actuelePortefeuilleWaardeInValuta'] - $data['beginPortefeuilleWaardeInValuta']) * $data['actueleValuta'] / $this->pdf->ValutaKoersEind -$data['rente'];
        $valutaResultaat = $ongerealiseerdResultaat - $fondsResultaat;
      }
      ...
      $totalenCat[$data['categorieOmschrijving']]['valutaResultaat'] += $valutaResultaat;
      ...
      $totalenHcat[$Hcategorie]['valutaResultaat'] += $valutaResultaat;
      ...
      $totalen['valutaResultaat'] += $valutaResultaat;
      ...
      if($data['categorieOmschrijving'] <> $lastcategorieOmschrijving)
      {
        if(!empty($lastcategorieOmschrijving))
        {
          ...
          $this->formatGetal($totalenCat[$lastcategorieOmschrijving]['valutaResultaat'],0),
        ...
        if($Hcategorie <> $lastHcategorie)
        {
          if(!empty($lastHcategorie))
          {
            ...
            $this->formatGetal($totalenHcat[$lastHcategorie]['fondsResultaat'],0),
            $this->formatGetal($totalenHcat[$lastHcategorie]['valutaResultaat'],0),
      ...
      $this->pdf->excelData[]=array($data['fondsOmschrijving'],$data['valuta'],round($data['totaalAantal'],0),round($data['actueleFonds'],2),round($data['beginwaardeLopendeJaar'],2),round($data['actuelePortefeuilleWaardeEuro'],0),round($data['beginPortefeuilleWaardeEuro'],0),round($fondsResultaat,0),round($valutaResultaat,0),round($aandeel,1),round($data['rente'],0));
    ...
    $this->formatGetal($totalen['valutaResultaat'],0),
