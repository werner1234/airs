...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: ATTberekening_L49.php,v $
 		Revision 1.7  2018/06/09 15:58:54  rvv
 		*** empty log message ***
 		
 		Revision 1.6  2017/01/18 17:02:28  rvv
 		*** empty log message ***
 		
 		Revision 1.5  2015/12/19 08:29:17  rvv
 		*** empty log message ***
 		
 		Revision 1.4  2014/08/16 15:31:50  rvv
 		*** empty log message ***
 		
 		Revision 1.3  2014/04/23 19:02:29  rvv
 		*** empty log message ***
...
class ATTberekening_L49
{

	function ATTberekening_L49($rapportData)
	{
    ...
    $this->rapport_startjaar=substr($van,0,4);
...
WHERE
Rekeningen.Portefeuille='".$this->rapport->portefeuille."'  AND
Rekeningmutaties.Boekdatum >= '".($this->rapport_startjaar)."-01-01' AND  Rekeningmutaties.Boekdatum <= '$tot' $extraWhere
...
//listarray($categorien);
	...
	function fondsPerformance($fondsData,$van,$tot,$stapeling=false,$totaal=false,$valuta='EUR',$catnaam='leeg')
  {
    global $__appvar;
    if($stapeling==true)
    ...
    foreach ($perioden as $periode)
    {
      ...
      $query ="SELECT SUM(TijdelijkeRapportage.actuelePortefeuilleWaardeEuro)/$startValutaKoers as actuelePortefeuilleWaardeEuro,
	    ...
	    $beginwaarde = $start['actuelePortefeuilleWaardeEuro'];
          ...
          ( $rekeningRekeningenWhere OR $rekeningFondsenWhere ) ORDER BY Rekeningmutaties.Boekdatum asc limit 1";
				$DB->SQL($query);
				$DB->Query();
				$datum=$DB->nextRecord();
				$weegDatum=$datum['Boekdatum'];
				//echo "$datumBegin->$datumEind=>$weegDatum<br>\n";
			}
     ...
     // listarray($AttributieStortingenOntrekkingen);
      ...
      $performance = ((($eindwaarde - $beginwaarde) - round($AttributieStortingenOntrekkingen['totaal'],2)) / round($gemiddelde,2));
...
$perfData['totaal']['waarden']=$waarden;
