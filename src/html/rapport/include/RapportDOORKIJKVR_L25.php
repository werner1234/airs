...
$Log: RapportDOORKIJKVR_L25.php,v $
Revision 1.5  2020/05/23 16:39:00  rvv
*** empty log message ***
...
Revision 1.1  2019/07/10 15:37:54  rvv
*** empty log message ***
...
class RapportDOORKIJKVR_L25
{
	function RapportDOORKIJKVR_L25($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = $pdf;
...
WHERE bronKoppeling='$airsCategorie' AND doorkijkCategoriesoort='" . mysql_real_escape_string($doorkijkSoort) . "' AND systeem='AIRS' AND vermogensbeheerder='" . $this->pdf->portefeuilledata['Vermogensbeheerder'] . "'";
	...
	{
		$this->pdf->setWidths(array($xOffset+3,45,20,20,20));
...
//listarray($doorKijk);
	...
	function writeRapport()
	{
		...
		$query="SELECT Fonds FROM IndexPerBeleggingscategorie WHERE Vermogensbeheerder='$beheerder' AND Beleggingscategorie='H-Oblig' limit 1";
    $db->SQL($query);
    $tmp=$db->lookupRecord();
...
KeuzePerVermogensbeheerder.categorieIXP='Obligaties' ";
    */
    ...
    CategorienPerHoofdcategorie.Hoofdcategorie='H-Oblig'";
