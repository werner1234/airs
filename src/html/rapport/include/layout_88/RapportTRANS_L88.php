...
class RapportTRANS_L88
{
	function RapportTRANS_L88($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	...
	function writeRapport()
	{
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
		...
		{
		// voor data
		$this->pdf->widthB = array(18,10,18,38,16,22,22,16,22,22,22,23,23,12);
    ...
    $this->pdf->SetFont($this->pdf->rapport_font,$this->pdf->rapport_fontstyle,$this->pdf->rapport_fontsize);
     ...
     Rekeningmutaties.omschrijving as rekeningOmschrijving,
		...
		"ORDER BY Rekeningmutaties.Transactietype,Rekeningmutaties.Boekdatum, Rekeningmutaties.Fonds, Rekeningmutaties.id";
		$DB = new DB();
		$DB->SQL($query);
		$DB->Query();
    ...
    $subtotaal=array();
    $subtotaal['aankoop_waarde'] =0;
    $subtotaal['verkoop_waarde'] =0;
    $subtotaal['result_lopendejaar'] =0;
      ...
      $t_aankoop_waardeinValuta=0;
			...
			if(isset($lastTransactieType) && $lastTransactieType<>$mutaties['Transactietype'] &&  count($subtotaal)>0)
      {
        $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
       	$this->pdf->row(array("","","",vertaalTekst("Subtotaal",$this->pdf->rapport_taal).' '.$transactietypenOmschrijving[$lastTransactieType],
                          "","",($subtotaal['aankoop_waarde']<>0?$this->formatGetal($subtotaal['aankoop_waarde'],$this->pdf->rapport_TRANS_decimaal):''),
                          "","",($subtotaal['verkoop_waarde']<>0?$this->formatGetal($subtotaal['verkoop_waarde'],$this->pdf->rapport_TRANS_decimaal):''),
                          "","",($subtotaal['result_lopendejaar']<>0?$this->formatGetal($subtotaal['result_lopendejaar'],$this->pdf->rapport_TRANS_decimaal):''),
                        ));
        $this->pdf->SetFont($this->pdf->rapport_font,$this->pdf->rapport_fontstyle,$this->pdf->rapport_fontsize);
        $subtotaal=array();
        $this->pdf->ln();
...
//	echo "lopende -> ".$resultaatlopende." <-  voorgaande ".$resultaatvoorgaande. " -  <br>" ;
          ...
          $datum=date("d-m-Y",db2jul($mutaties['Boekdatum']));
			...
			$subtotaal['aankoop_waarde'] +=$t_aankoop_waarde;
      $subtotaal['verkoop_waarde'] +=$t_verkoop_waarde;
      $subtotaal['result_lopendejaar'] +=$resultaatlopende;
    ...
    if(isset($lastTransactieType) &&  count($subtotaal)>0)
    {
      $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
      $this->pdf->row(array("","","",vertaalTekst("Subtotaal",$this->pdf->rapport_taal).' '.$transactietypenOmschrijving[$lastTransactieType],
                        "","",($subtotaal['aankoop_waarde']<>0?$this->formatGetal($subtotaal['aankoop_waarde'],$this->pdf->rapport_TRANS_decimaal):''),
   		...
   		foreach($transactietypen as $key=>$val)
   		{
				switch($val)
				{
					...
					case "A/O" :
						...
						$this->pdf->Cell(80,4, vertaalTekst("Aankoop / openen (Open Buy)",$this->pdf->rapport_taal), 0,1, "L");
					break;
					case "A/S" :
						$this->pdf->Cell(30,4, "A/S", 0,0, "L");
						$this->pdf->Cell(80,4, vertaalTekst("Aankoop / sluiten (Open Sell)",$this->pdf->rapport_taal), 0,1, "L");
					break;
					case "D" :
					...
					case "V/O" :
						...
						$this->pdf->Cell(80,4, vertaalTekst("Verkoop / openen (Close Buy)",$this->pdf->rapport_taal), 0,1, "L");
					break;
					case "V/S" :
