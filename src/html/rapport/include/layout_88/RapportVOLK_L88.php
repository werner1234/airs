...
class RapportVOLK_L88
{
	function RapportVOLK_L88($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//echo $subquery."<br><br>";exit;
      ...
      if($this->pdf->getStringWidth($omschrijving) > $this->pdf->widths[1]-5)
			{
				for($i=strlen($subdata['fondsOmschrijving']);$i>1;$i--)
        {
          $string=substr($subdata['fondsOmschrijving'],0,$i).'...';
          if($this->pdf->getStringWidth($string) < $this->pdf->widths[1]-5)
					{
            $omschrijving=$string;
            break;
					}
