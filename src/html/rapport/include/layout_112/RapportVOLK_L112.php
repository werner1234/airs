...
class RapportVOLK_L112
{
	function RapportVOLK_L112($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    $this->pdf->excelData[]=array("Categorie",'Fonds','Aantal','Koers','Kostprijs','Kostprijs EUR','Koers','Waarde','Waarde EUR','in %','Fondsresultaat','Valutaresultaat','Direct resultaat','Totaal resultaat','in %');
  ...
  //F $procentResultaat);
			...
			$this->printCol(2,$title,"tekst");
			$this->pdf->SetFont($this->pdf->rapport_font,'b',$this->pdf->rapport_fontsize);
			if($totaalB <>0)
				$this->printCol(8,$this->formatGetal($totaalB,$this->pdf->rapport_VOLK_decimaal),"subtotaal");
			if($totaalA <>0)
				$this->printCol(4,$this->formatGetal($totaalA,$this->pdf->rapport_VOLK_decimaal,true),"subtotaal");
			if($totaalC <>0)
				$this->printCol(9,$this->formatGetal($totaalC,$this->pdf->rapport_VOLK_decimaal_proc)." %","subtotaal");
      ...
      if($totaalH <>0) //$procentResultaat
        $this->printCol(13,$this->formatGetal($totaalH,$this->pdf->rapport_VOLK_decimaal_proc),"subtotaal");
    ...
    if($grandtotaal)
      ...
      if($totaalH <>0)
        $this->printCol(13,$this->formatGetal($totaalH,$this->pdf->rapport_VOLK_decimaal),$grandtotaal);
			if($totaalF <>0)
  ...
  function getDividend($fonds)
  {
	...
	function writeRapport()
	{
		...
		$this->pdf->widthB = array(55,18,18,18,18,1,20,18,18,15,18,18,18,18,12);
		$this->pdf->widthA = array(60,18,18,18,18,1,20,18,18,15,18,18,18,18,12);
		$this->pdf->alignB = array('L','R','R','R','R','R','R','R','R','R','R','R','R','R','R');
    ...
    $this->pdf->templateVars['VOLKPaginas']=$this->pdf->page;
		...
		$actueleWaardePortefeuille = 0;
			...
			SUM( if(TijdelijkeRapportage.type='rekening',TijdelijkeRapportage.actuelePortefeuilleWaardeEuro / ".$this->pdf->ValutaKoersEind. ", TijdelijkeRapportage.beginPortefeuilleWaardeEuro) / ".$this->pdf->ValutaKoersBegin. ") as subtotaalbegin,".
	  //  "
    ...
    //  ".
		...
		while($categorien = $DB->NextRecord())
		{
           ...
           $procentResultaat = (($totaalactueel - $totaalbegin + $totaaldividendCorrected) / ($totaalbegin /100));
		    if($totaalbegin < 0)
				...
				$actueleWaardePortefeuille += $this->printTotaal($title, $totaalbegin, $totaalactueel, $totaalpercentage , $totaalfondsresultaat, $totaalvalutaresultaat, $procentResultaat,false,$totaaldividend,$totaalResultaat);
        ...
        $totaalResultaat = 0;
			...
			$subquery = "SELECT TijdelijkeRapportage.fondsOmschrijving, ".
				" TijdelijkeRapportage.fonds,TijdelijkeRapportage.type, TijdelijkeRapportage.rekening, ".
...
//echo $subquery."<br><br>";exit;
      ...
      $kopPrinted=false;
        ...
        {
          $subdata['beginPortefeuilleWaardeEuro']=$subdata['actuelePortefeuilleWaardeEuro'];
          $subdata['beginPortefeuilleWaardeInValuta']=$subdata['actuelePortefeuilleWaardeInValuta'];
        }
        ...
        $dividendtxt='';
				...
				if($fondsResultaat+$valutaResultaat+$dividend['totaal'] <> 0)
          $totaalResultaattxt = $this->formatGetal($fondsResultaat+$valutaResultaat+$dividend['totaal'],$this->pdf->rapport_VOLK_decimaal);
        else
          $totaalResultaattxt ='';
        ...
        if($subdata['type']=='rekening')
        {
          if(trim($subdata['fondsOmschrijving'])<>'Effectenrekening')
            $omschrijving='{Tenaamstelling}';
          else
          ...
          $subdata['fondsOmschrijving']=$omschrijving;
        }
				...
				$this->pdf->row(array("",
                          ($subdata['type']=='rekening'?'':$this->formatAantal($subdata['totaalAantal'],$this->pdf->rapport_VOLK_aantal_decimaal,$this->pdf->rapport_VOLK_aantalVierDecimaal)),
                          ($subdata['type']=='rekening'?'':$this->formatGetal($subdata['beginwaardeLopendeJaar'],2)),
                          ($subdata['type']=='rekening'?'':$this->formatGetal($subdata['beginPortefeuilleWaardeInValuta'],$this->pdf->rapport_VOLK_decimaal)),
                          ($subdata['type']=='rekening'?'':$this->formatGetal($subdata['beginPortefeuilleWaardeEuro'],$this->pdf->rapport_VOLK_decimaal)),
													"",
                          ...
                          $dividendtxt,
                          $totaalResultaattxt,
                          ($subdata['type']=='rekening'?'':$procentResultaattxt)));
        
        $this->pdf->excelData[]=array($categorien['Omschrijving'],$subdata['fondsOmschrijving'],
          ($subdata['type']=='rekening'?'':round($subdata['totaalAantal'],4)),
          ($subdata['type']=='rekening'?'':round($subdata['beginwaardeLopendeJaar'],2)),
          ($subdata['type']=='rekening'?'':round($subdata['beginPortefeuilleWaardeInValuta'],$this->pdf->rapport_VOLK_decimaal)),
          ($subdata['type']=='rekening'?'':round($subdata['beginPortefeuilleWaardeEuro'],$this->pdf->rapport_VOLK_decimaal)),
          ($subdata['type']=='rekening'?'':round($subdata['actueleFonds'],2)),
          ...
          round($fondsResultaat+$valutaResultaat+$dividend['totaal'],$this->pdf->rapport_VOLK_decimaal),
          ($subdata['type']=='rekening'?'':round($procentResultaat,$this->pdf->rapport_VOLK_decimaal_proc)));
				...
				$subtotaal['totaalResultaat'] +=$fondsResultaat+$valutaResultaat+$dividend['totaal'];
    ...
    " TijdelijkeRapportage.beleggingscategorie =  '".$categorien['beleggingscategorie']."' AND ".
      ...
      if($totaalRenteInValuta <> 0)
      {
        ...
        $this->pdf->excelData[]=array("Opgelopen Rente",'','','','','','','',
          round($totaalRenteInValuta,$this->pdf->rapport_VOLK_decimaal),
...
//rvv
      ...
      $grandtotaalResultaat +=	$subtotaal['totaalResultaat'] ;
		...
		$actueleWaardePortefeuille += $this->printTotaal(vertaalTekst("Subtotaal",$this->pdf->rapport_taal)." ".vertaalTekst($lastCategorie,$this->pdf->rapport_taal), $totaalbegin, $totaalactueel,$totaalpercentage,$totaalfondsresultaat,$totaalvalutaresultaat,$procentResultaat,false,$totaaldividend,$totaalResultaat);
...
*/
		...
		$this->printTotaal(vertaalTekst("Totale actuele waarde portefeuille",$this->pdf->rapport_taal), "", $actueleWaardePortefeuille,100,$grandtotaalfonds,$grandtotaalvaluta,"",true,$grandtotaaldividend,$grandtotaalResultaat);
