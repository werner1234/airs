...
class RapportKERNV_L112
{
	function RapportKERNV_L112($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function header($categorieVolgorde,$categorieOmschrijving,$extraW)
  {
    ...
    $header=array(vertaalTekst("Investering", $this->pdf->rapport_taal));
  ...
  function setTxtColor($bedrag)
  {
    ...
    while($cat = $DB->NextRecord())
    {
      ...
      $this->clientVermogensbeheerder[$cat['BeleggingscategorieOmschrijving']]=$cat['BeleggingscategorieOmschrijving'];
    }
    ...
    if(is_array($this->pdf->portefeuilles))
    {
      ...
      {
        if($portefeuille['Selectieveld1']<>$soortOvereenkomstDetail)
        {
    ...
    {
      $consolidatie = false;
      $portefeuilles[$this->portefeuille]=$this->portefeuille;
    }
     ...
     // listarray($this->clientVermogensbeheerder);
      ...
      foreach($portefeuilles as $portefeuille)
      {
        ...
        foreach($gegevens as $waarde)
        {
          ...
          //  $omschrijving=$portefeuille." ".$waarde['beleggingscategorieOmschrijving'];
            $portefeuilleWaardenDetails[$waarde['beleggingscategorieOmschrijving']][$omschrijving]['belCatWaarde'][$waarde['hoofdcategorie']] += $waarde['actuelePortefeuilleWaardeEuro'];
            $portefeuilleWaardenDetails[$waarde['beleggingscategorieOmschrijving']][$omschrijving]['totaleWaarde'] += $waarde['actuelePortefeuilleWaardeEuro'];
            $portefeuilleWaardenDetails[$waarde['beleggingscategorieOmschrijving']][$omschrijving]['fonds'] = $waarde['fonds'];
          }
          else
          {
...
//listarray($portefeuilleWaarden);
  ...
  //  {
    ...
    foreach($this->clientVermogensbeheerder as $portefeuille=>$ClientVermogensbeheerder)
    {
      ...
      if(1)//count($this->pdf->portefeuilles) < 60)
      {
        ...
        foreach ($portefeuilleWaarden as $portefeuille=>$pdata)
        {
          ...
          {
            ksort($portefeuilleWaardenDetails[$portefeuille]);
            foreach($portefeuilleWaardenDetails[$portefeuille] as $omschrijving=>$veldData)
            {
              $tonen=false;
              foreach ($categorieVolgorde as $categorie => $volgorde)
              {
                if(round($veldData['belCatWaarde'][$categorie],1)<>0.0)
                {
                  $tonen = true;
                }
              }
          ...
          else
          {
            ...
            foreach ($categorieVolgorde as $categorie => $volgorde)
            {
              if(round($pdata['belCatWaarde'][$categorie],1)<>0.0)
              {
                $tonen = true;
              }
            }
        ...
        $this->pdf->SetFont($this->pdf->rapport_font,'B', $this->pdf->rapport_fontsize);
        $this->pdf->Cell(33, 6,vertaalTekst('Totaal (€)',$this->pdf->rapport_taal), 0, 0, "L",0);
   ...
   //   listarray($this->selectieveldPortefeuille);
      ...
      foreach ($portefeuilleWaarden as $portefeuille=>$waarde)
      {
        ...
        if(isset($portefeuilleWaardenDetails[$portefeuille]))
        {
          ...
          foreach($portefeuilleWaardenDetails[$portefeuille] as $omschrijving=>$veldData)
          {
            ...
            //	echo $kop."<br>\n";ob_flush();
            $percentage=$portWaarde/$totaalWaarde*100;
            if($percentage<>0)
            {
              $portefeuilleAandeel[$kop] = $percentage;
        ...
        else
        {
         // echo "$portefeuille $portefeuilleTxt <br>\n";
          ...
          //	echo $kop."<br>\n";ob_flush();
          $percentage=$waarde['totaleWaarde'] / $totaalWaarde * 100;
          if($percentage<>0)
          {
            $portefeuilleAandeel[$kop] = $percentage;
            $portefeuilleKleur[] = $kleur;
    ...
    if($this->pdf->getY()>105 || count($portefeuilleAandeel)>10)
    {
      $this->pdf->addPage();
      $grafiekY=40;
    }
  ...
  function clientTotaal($data,$client,$extraW)
  {
    ...
    $this->pdf->SetFont($this->pdf->rapport_font,'B', $this->pdf->rapport_fontsize);
    $this->pdf->Cell(33, 6,vertaalTekst('Totaal',$this->pdf->rapport_taal).' '.$client, 0, 0, "L",0);
