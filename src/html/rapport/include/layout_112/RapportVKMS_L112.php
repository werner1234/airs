...
class RapportVKMS_L112
{
	function RapportVKMS_L112($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum,$laatstejaar=true)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
			...
			$this->pdf->rapport_titel = vertaalTekst("Kosten",$this->pdf->rapport_taal);
			$this->pdfVullen=true;
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    ...
    $vkmArray=array('vkmPercentagePortefeuille'=>$vkmPercentagePortefeuille,'kostenPercentage'=>$kostenPercentage,'vkmWaarde'=>$vkmWaarde,'grootboekKosten'=>$grootboekKosten,'grootboekOmschrijving'=>$grootboekOmschrijving,
      ...
      'fondsGemiddeldeWaarde'=>$perfTotaal['gemWaarde'],'alleGrootboekWaarden'=>$alleGrootboekWaarden);
...
ORDER BY HoofdcategorienVolgorde.Afdrukvolgorde, HoofdBeleggingscategorien.Afdrukvolgorde,BeleggingscategorienVolgorde.Afdrukvolgorde, BeleggingscategorienVolgorde.Afdrukvolgorde, Beleggingscategorien.Afdrukvolgorde,fondsVolgorde,OptieBovenliggendFonds,FondsOmschrijving ";
	...
	function writeRapport()
  {
    ...
    $query = "SELECT Clienten.Naam, Clienten.Naam1, Portefeuilles.Depotbank,Portefeuilles.spreadKosten, Portefeuilles.AEXVergelijking, Portefeuilles.SpecifiekeIndex, Portefeuilles.Vermogensbeheerder, Portefeuilles.ClientVermogensbeheerder, Portefeuilles.Risicoklasse, Portefeuilles.Client
FROM Portefeuilles, Clienten
WHERE Portefeuille = '" . $this->portefeuille . "' AND Portefeuilles.Client = Clienten.Client ";
    $DB = new DB();
    $DB->SQL($query);
    $DB->Query();
    ...
    {
      $vkmData = array();
      $vkmData[$this->portefeuille] = $this->kostenKader($this->portefeuille, $details['totaalDoorlopendekosten'], $details['perfTotaal'], $details['totaalDoorlopendekostenGesplitst']);
      ...
      if (is_array($this->pdf->portefeuilles) && count($this->pdf->portefeuilles) > 1)
      {
        foreach ($this->pdf->portefeuilles as $portefeuille)
        {
          ...
          $vkmData[$portefeuille] = $this->kostenKader($portefeuille, $details['totaalDoorlopendekosten'], $details['perfTotaal'], $details['totaalDoorlopendekostenGesplitst']);
          
        }
      }
      $this->pdfVullen = true;
      if ($this->returnVkmData == true)
      {
        return $vkmData;
      }
      else
      {
        ...
        $pagina = 1;
        $paginas = array();
        foreach ($vkmData as $portefeuille => $data)
        {
          
          if (!isset($paginas[$pagina][$this->portefeuille]))
          {
            $paginas[$pagina][$this->portefeuille] = $vkmData[$this->portefeuille];
          }
          
          $paginas[$pagina][$portefeuille] = $data;
          $n++;
          if ($n > 5)
          {
            $n = 0;
  ...
  function toonVKMWaarden($vkmPerPortefeuille)
  {
    ...
    $kop=array('');
    $indirecteKosten=array(vertaalTekst('Indirecte (fonds)kosten',$this->pdf->rapport_taal));
    $indirecteKostenVermogen=array(vertaalTekst('Indirecte (fonds)kosten ten opzichte van onderliggend vermogen',$this->pdf->rapport_taal));
    ...
    $herrekendIndirect=array(vertaalTekst("Herrekende indirecte (fonds)kosten",$this->pdf->rapport_taal));
    $aandeelIndirect=array(vertaalTekst('Aandeel beleggingsfondsen',$this->pdf->rapport_taal));
    $gemiddeldVermogen=array(vertaalTekst('Gemiddeld vermogen',$this->pdf->rapport_taal));
    $indirecteKostenfactor=array(vertaalTekst('Indirecte (fonds)kosten factor van de portefeuille',$this->pdf->rapport_taal));
    $grootboekKosten=array();
    $totaalDirect=array(vertaalTekst('Totaal directe kosten',$this->pdf->rapport_taal));
    $vkm=array(vertaalTekst('Vergelijkende kostenmaatstaf',$this->pdf->rapport_taal));
    ...
    {
      $grootboekKop[]=$this->pdf->rapportageValuta;
    ...
    //  $gemiddelde=$this->verdelingTotaal[$portefeuille]['totaal']['gemiddelde'];
     ...
     // echo "$portefeuille ".($vkmWaarden['fondsGemiddeldeWaarde']/$vkmWaarden['gemiddeldeWaarde'] * 100)."=".$vkmWaarden['fondsGemiddeldeWaarde']."/".$vkmWaarden['gemiddeldeWaarde']."  * 100<br>\n";
        $indirecteKosten[]=$this->formatGetal($vkmWaarden['totaalDoorlopendekosten'], 0) . ' '.$this->pdf->rapportageValuta;
        $indirecteKostenVermogen[]= $this->formatGetal($vkmWaarden['doorlopendeKostenPercentage'] * 100, 2) . ' %';
        $percentageGemIndirect[]= $this->formatGetal($vkmWaarden['percentageIndirectVermogenMetKostenfactor'] * 100, 2) . ' %';
        $herrekendIndirect[]= $this->formatGetal($vkmWaarden['doorlopendeKostenPercentage']/$vkmWaarden['percentageIndirectVermogenMetKostenfactor'] * 100, 2) . ' %';
        $aandeelIndirect[]=$this->formatGetal($vkmWaarden['fondsGemiddeldeWaarde']/$vkmWaarden['gemiddeldeWaarde'] * 100, 2) . ' %';
        $gemiddeldVermogen[]=$this->formatGetal($vkmWaarden['gemiddeldeWaarde'], 0) . ' '.$this->pdf->rapportageValuta;
        $indirecteKostenfactor[]=$this->formatGetal($vkmPercentagePortefeuille, 2) . ' %';
      //if($this->portefeuille==$portefeuille)
        $totaalDirect[]=$this->formatGetal($vkmWaarden['totaalDirecteKosten'],0);
      //else
      //  $totaalDirect[]='';
      $totaalDirect[]=$this->formatGetal($vkmWaarden['totaalDirecteKosten']/$vkmWaarden['gemiddeldeWaarde']*100,2).' %';
       ...
       //   $vkmWaarden['grootboekKosten'] as $grootboek=>$bedrag
          ...
          if($this->portefeuille==$portefeuille)
          {
            $grootboekKosten[$grootboek][] = $this->formatGetal($bedrag, 0);
            $grootboekKosten[$grootboek][] = $this->formatGetal($bedrag / $vkmWaarden['gemiddeldeWaarde'] * 100, 2) . ' %';
          }
          else
          {
      ...
      //if($this->portefeuille==$portefeuille)
        $vkm[]=$this->formatGetal($vkmWaarden['vkmWaarde']*$vkmWaarden['gemiddeldeWaarde']/100, 0);
      //else
      //  $vkm[]='';
      $vkm[]=$this->formatGetal($vkmWaarden['vkmWaarde'], 2).' %';
  
  
      $fiscaliteit[]=$this->formatGetal($vkmWaarden['alleGrootboekWaarden']['DIVBE']+$vkmWaarden['alleGrootboekWaarden']['ROER'], 0);
      $fiscaliteit[]=$this->formatGetal(($vkmWaarden['alleGrootboekWaarden']['DIVBE']+$vkmWaarden['alleGrootboekWaarden']['ROER'])/$vkmWaarden['gemiddeldeWaarde']*100, 2).' %';
    ...
    if($this->pdfVullen==true)
    {
      ...
      $this->pdf->setWidths(array(80, 36, 32, 32, 32, 32, 32, 32, 32, 32, 32));
      $this->pdf->SetFont($this->pdf->rapport_font, 'b', $this->pdf->rapport_fontsize);
    ...
    //  $this->pdf->row($herrekendIndirect);
      ...
      $this->pdf->setWidths(array(80,18,18,16,16,16,16,16,16,16,16,16,16));
      ...
      foreach($grootboekKosten as $grootboek=>$regel)
        $this->pdf->row($regel);
      $this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
      $this->pdf->CellBorders=array('','TS','TS','TS','TS','TS','TS','TS','TS','TS','TS','TS','TS','TS','TS','TS','TS','TS');
      $this->pdf->row($totaalDirect);
      unset($this->pdf->CellBorders);
      $this->pdf->ln();
      $this->pdf->row($vkm);
      if($this->fiscaliteitTonen==true)
      {
        $this->pdf->ln();
      ...
      foreach($vkmPerPortefeuille as $portefeuille=>$vkmWaarden)
      {
        ...
        $this->pdf->setXY(100+$n*33, 175);
        $this->VBarVerdeling(23, 50, $this->barData[$portefeuille],$portefeuille,$maxVkm);
        $this->pdf->setXY($this->pdf->marge, $startYGrafiek);
        $n++;
        if($n>$maxPortefeuilles)
          break;
      }
    }
  }
