...
function Header_basis_L112($object)
{
 $pdfObject = &$object;
	...
	function HeaderVKM_L112($object)
	{
		$pdfObject = &$object;
		$pdfObject->HeaderVKM();
	}

function HeaderVKMS_L112($object)
{
  $pdfObject = &$object;
  ...
  $pdfObject->ln(1);
	  ...
	  function HeaderFRONT_L112($object)
	  {
	    $pdfObject = &$object;
	    //$pdfObject->headerSCENARIO();

	  }
	  function HeaderSCENARIO_L112($object)
	  {
	    $pdfObject = &$object;
	    //$pdfObject->headerSCENARIO();

	  }
function HeaderDOORKIJKVR_L112($object)
{
  $pdfObject = &$object;
  //$pdfObject->headerSCENARIO();
...
function HeaderKERNV_L112($object)
{
  $pdfObject = &$object;
  //$pdfObject->headerSCENARIO();
  
}
function HeaderKERNZ_L112($object)
{
  $pdfObject = &$object;
  //$pdfObject->headerSCENARIO();
  
}
    function HeaderRISK_L112($object)
    {
	    $pdfObject = &$object;
			$pdfObject->Ln();
			$pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
    ...
    function HeaderGRAFIEK_L112($object)
    {
	    $pdfObject = &$object;
    }

    function HeaderEND_L112($object)
    {
     	$pdfObject = &$object;
			$pdfObject->Ln();
			$pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
    ...
    function HeaderPERFD_L112($object)
    {
      $pdfObject = &$object;
...
function HeaderVAR_L112($object)
{
  $pdfObject = &$object;
  $pdfObject->SetFont($pdfObject->rapport_font,'B',$pdfObject->rapport_fontsize);
  ...
  unset($pdfObject->CellBorders);//"Modified\nduration",
    ...
    function HeaderTRANS_L112($object)
	  {
	    $pdfObject = &$object;
    ...
    function HeaderMUT_L112($object)
	  {
	    $pdfObject = &$object;
   		$pdfObject->SetFont($pdfObject->rapport_font,$pdfObject->rapport_kop_fontstyle,$pdfObject->rapport_fontsize);
  	  $pdfObject->SetX(100);
	  ...
	  function HeaderAFM_L112($object)
	  {
	    $pdfObject = &$object;


	  }
	  function HeaderINHOUD_L112($object)
	  {
	    $pdfObject = &$object;
	    //$pdfObject->headerSCENARIO();

	  }	  
 	  function HeaderPERF_L112($object)
	  {
	    $pdfObject = &$object;
      $pdfObject->ln();
      $pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
...
$pdfObject->ln();
    ...
    function HeaderINDEX_L112($object)
	  {
	    $pdfObject = &$object;
	  }

function HeaderCASHY_L112($object)
{
  $pdfObject = &$object;
}

function HeaderOIH_L112($object)
{
  $pdfObject = &$object;
}

function HeaderOIS_L112($object)
{
  $pdfObject = &$object;
}

function HeaderHSE_L112($object)
{
  $pdfObject = &$object;
  $pdfObject->ln();
  $dataWidth=array(28,50,15,20,20,20,22,22,22,18,20,5,20);
  ...
  // listarray($pdfObject->widths);echo "new page <br>\n";
    ...
    function HeaderOIB_L112($object)
	  {
	    $pdfObject = &$object;
      $pdfObject->Ln();
      		$pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
  ...
  function HeaderOIV_L112($object)
  {
    $pdfObject = &$object;
    $pdfObject->Ln();
    $pdfObject->SetFillColor($pdfObject->rapport_kop_bgcolor['r'],$pdfObject->rapport_kop_bgcolor['g'],$pdfObject->rapport_kop_bgcolor['b']);
  ...
  function HeaderATT_L112($object)
	{
    $pdfObject = &$object;
...
*/
   ...
   function HeaderPERFG_L112($object)
	{
    $pdfObject = &$object;
    $pdfObject->widthA = array(26,25,30,30,23,23,23,24,28,24,26);
		$pdfObject->alignA = array('L','R','R','R','R','R','R','R','R','R','R','R');
...
*/
  ...
  function HeaderVOLK_L112($object)
	{
    $pdfObject = &$object;
   	$pdfObject->ln();
		$pdfObject->SetFont($pdfObject->rapport_font,$pdfObject->rapport_kop_fontstyle,$pdfObject->rapport_fontsize);
...
function HeaderVHO_L112($object)
	{
    $pdfObject = &$object;
   	$pdfObject->ln();
		$pdfObject->SetFont($pdfObject->rapport_font,$pdfObject->rapport_kop_fontstyle,$pdfObject->rapport_fontsize);
...
if(!function_exists('getRekeningOnttrekkingenKruis'))
{
  function getRekeningOnttrekkingenKruis($rekening, $van, $tot, $valuta = 'EUR', $kruispostOphalen = false ,$fondsBoekingen=false)
  {
    if ($valuta != "EUR")
    {
    ...
    $query = "SELECT " .
      ...
      " ";
    $DB = new DB();
    $DB->SQL($query . "Rekeningmutaties.Grootboekrekening IN (SELECT Grootboekrekening FROM Grootboekrekeningen WHERE Onttrekking=1)");
    $DB->Query();
    $data = $DB->nextRecord();
    ...
    {
      $DB->SQL($query . "Rekeningmutaties.Grootboekrekening IN ('KRUIS')");
      $DB->Query();
      $data = $DB->nextRecord();
      $kruispost += $data['subdebet'];
    }
    if($fondsBoekingen==true)
    {
      $DB->SQL($query . "(Rekeningmutaties.Fonds <> '' AND Rekeningmutaties.Grootboekrekening NOT IN ('KRUIS') AND Rekeningmutaties.Grootboekrekening NOT IN (SELECT Grootboekrekening FROM Grootboekrekeningen WHERE Storting=1 ))");
    ...
    return array('onttrekking' => $onttrekking, 'kruispost' => $kruispost, 'fonds'=>$fonds);
  }
}

if(!function_exists('getRekeningStortingenKruis'))
{
  function getRekeningStortingenKruis($rekening, $van, $tot, $valuta = 'EUR', $kruispostOphalen = false,$fondsBoekingen=false)
  {
    if ($valuta != "EUR")
    {
      $koersQuery = " / (SELECT Koers FROM Valutakoersen WHERE Valuta='" . $valuta . "' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
    ...
    $query = "SELECT " .
      ...
      "";
    $DB = new DB();
    $DB->SQL($query . "Rekeningmutaties.Grootboekrekening IN (SELECT Grootboekrekening FROM Grootboekrekeningen WHERE Storting=1)");
    $DB->Query();
    $data = $DB->nextRecord();
    ...
    if($fondsBoekingen==true)
    {
      ...
      $DB->Query();
      $data = $DB->nextRecord();
      $fonds += $data['subcredit'];
    }
    
    return array('storting' => $storting, 'kruispost' => $kruispost, 'fonds'=>$fonds);
  }
}
?>