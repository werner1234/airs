...
class RapportVAR_L112
{
	function RapportVAR_L112($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function formatGetalKoers($waarde, $dec , $start = false)
    ...
    if($this->pdf->rapport_taal==1)
      ...
      $valutaTxt=vertaalTekst("Valuta",$this->pdf->rapport_taal);
    
    $this->pdf->excelData[]=array(
                      vertaalTekst("Naam",$this->pdf->rapport_taal),
                      vertaalTekst("ISIN",$this->pdf->rapport_taal),
                      vertaalTekst("Coupon-\ndatum",$this->pdf->rapport_taal),
                      vertaalTekst("Rating instr.",$this->pdf->rapport_taal),
                      '',
                      $valutaTxt,
                      vertaalTekst("Nominaal",$this->pdf->rapport_taal),
                      vertaalTekst("Koers",$this->pdf->rapport_taal),
                      vertaalTekst("Marktwaarde",$this->pdf->rapport_taal),'',
                      vertaalTekst("Coupon",$this->pdf->rapport_taal)." ".vertaalTekst("Yield",$this->pdf->rapport_taal),
                      vertaalTekst("Yield to",$this->pdf->rapport_taal)." ".vertaalTekst("Maturity",$this->pdf->rapport_taal),
                      vertaalTekst("Modified",$this->pdf->rapport_taal)." ".vertaalTekst("duration",$this->pdf->rapport_taal),
                      vertaalTekst("Resterende",$this->pdf->rapport_taal)." ".vertaalTekst("looptijd",$this->pdf->rapport_taal),
                      vertaalTekst("%",$this->pdf->rapport_taal)." ".vertaalTekst("port.",$this->pdf->rapport_taal));
...
$modifiedDuration=0;
     ...
     //   unset($this->pdf->fillCell);
      ...
      $this->pdf->excelData[]=array($data['fondsOmschrijving'],$data['ISINcode'], date('d-m',db2jul($data['Rentedatum'])),
        $data['fondsRating'],$data['emittentRating'],$data['valuta'],round($data['totaalAantal'],0),
        round($data['actueleFonds'],2),round($data['actuelePortefeuilleWaardeEuro'],0),'',
        round($koers['Rentepercentage'],3),round($ytm,2),round($modifiedDuration,2),
        round($restLooptijd,2), round($aandeel*100,1));
...
//listarray($totalenCat);    
    ...
    if($eindTotaal==true)
    {
      ...
      $this->pdf->excelData[]=array(vertaalTekst("Totaal", $this->pdf->rapport_taal), '', '', '', '', '', '', '', round($totalen['actuelePortefeuilleWaardeEuro'], 0), '',
        round($totalen['yield'], 3),
        round($totalen['ytm'], 2),
        round($totalen['modifiedDuration'], 2),
        round($totalen['restLooptijd'], 2),
        round($totalen['aandeel'] * 100, 1), '');
    }
    else
