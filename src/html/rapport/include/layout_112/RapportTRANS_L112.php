...
class RapportTRANS_L112
{
	function RapportTRANS_L112($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
		...
		$db2 = new DB();
			...
			// voor data
		$this->pdf->widthB = array(20,21,25,65,25,25,25,25,25,25);//,19.5,19.5,19,11=69
    ...
    else
    {
      $portefeuilleFilter=" Rekeningen.Portefeuille = '".$this->portefeuille."'";
		...
		$query = "SELECT Rekeningen.Portefeuille,Portefeuilles.selectieveld1,Portefeuilles.Depotbank, Fondsen.Omschrijving, ".
     ...
     Rekeningmutaties.omschrijving as rekeningOmschrijving,
		...
		"ORDER BY Rekeningen.Portefeuille, Rekeningmutaties.Boekdatum, Rekeningmutaties.Fonds, Rekeningmutaties.id ";
		$DB = new DB();
		$DB->SQL($query);
		$DB->Query();
