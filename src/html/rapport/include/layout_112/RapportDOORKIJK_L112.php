...
class RapportDOORKIJK_L112
{
	function RapportDOORKIJK_L112($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = $pdf;
...
*/
  ...
  function BarDiagram($w, $h, $data, $format,$colorArray,$titel)
  {
    ...
    $YDiag = $YPage+5 ;//+ 30 - ((count($data) * 5) / 2);
    $hDiag = floor($h);
    ...
    $hBar = 4;//floor($hDiag / ($this->pdf->NbVal + 1));
    ...
    function vulPagina($belCategorie='')
	{
      ...
      $pieData=array();
       ...
       //   $grafiekKleuren[]=$standaardKleuren[$a];
       // else
        $grafiekKleuren[] =  $this->kleuren[$doorkijkCategorieSoort][$categorie];//array($value['kleur'][0],$value['kleur'][1],$value['kleur'][2]);
        $pieData[$categorie] = $percentage;
			}
      ...
      {
        $this->pdf->SetXY(30 + $xOffset, 125);
        $y = $this->pdf->getY();
        $this->BarDiagram(50,50,$pieData,'%l',$grafiekKleuren,$doorkijkTitels[$doorkijkCategorieSoort]);//
        $this->pdf->setY($y);
        $this->pdf->SetLineWidth($this->pdf->lineWidth);
      }
			$pieTeller++;
