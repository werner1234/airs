...
class ATTberekening_L112
{

	function ATTberekening_L112($rapportData,$periode='maanden')
	{
...
//	     $AttributieStortingenOntrekkingen['totaal'] += $nietToegerekendeKosten['kostenTotaal'];
       ...
       */
       ...
       $performanceBruto = ($resultaatBruto / $gemiddelde);
...
$somItems=array('resultaat','resultaatBruto','storting','onttrekking','kosten','opbrengst','stort');
...
if($stapeling == true)
{
  ...
  elseif($categorie=='H-Liq')
    $filter="AND (Fonds IN('".implode('\',\'',$fondsData['fondsen'])."') OR Fonds='')";
  else
    $filter=$fondsenWhere;
