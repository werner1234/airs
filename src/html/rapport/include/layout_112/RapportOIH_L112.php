...
class RapportOIH_L112
{

  function RapportOIH_L112($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
if(substr($start,5,5)=='12-31')
  $begin=substr($start,0,4)+1;
else
  $begin=substr($start,0,4);

if($begin<>substr($eind,0,4))
{
...
//listarray($maandData);
  $tmp=array();
  $somVelden=array('stort','stort','storting','onttrekking','kosten','opbrengst','kostenNietGekoppeld','resultaat','ongerealiseerd','gerealiseerd','resultaatBruto');
    ...
    foreach ($perfdata[$cat]['perfWaarden'] as $datum=>$data)
    {
      ...
      { //echo date("Y-m-d",mktime(0,0,0,1,1,substr($datum,0,4)))." $datum <br>\n";
       ...
       // $perfIndex=$this->fondsPerf($indexLookup[$cat],date("Y-m-d",mktime(0,0,0,substr($datum,5,2),0,substr($datum,0,4))),$datum);
        if($data['beginwaarde']==0 && $data['eindwaarde']==0 && $data['stort']==0 && $begonnen==false)
        {
          $perfIndex=0;
  ...
  foreach ($categorien as $cat)
  {
    ...
    if ( $this->firstHeader === true ) {
      $this->pdf->excelData[]=array( vertaalTekst('Categorie', $this->pdf->rapport_taal), vertaalTekst('periode', $this->pdf->rapport_taal),  vertaalTekst('beginvermogen', $this->pdf->rapport_taal),  vertaalTekst('mutaties', $this->pdf->rapport_taal),
        vertaalTekst('rendement', $this->pdf->rapport_taal), vertaalTekst('eindvermogen', $this->pdf->rapport_taal),
    ...
    foreach($hcatWaarden['periode'][$cat]['datum'] as $i=>$datum)
    {
     ...
     // $brutoRendement=$brutoResultaat/$hcatWaarden['periode'][$cat]['waarde'][$i]['gemWaarde']*100;
      ...
      $this->formatGetal($hcatWaarden['periode'][$cat]['waarde'][$i]['stort'],2),
      ...
      $this->pdf->excelData[]= array($hCategorieOmschrijvingen[$cat],
        ...
        round($hcatWaarden['periode'][$cat]['waarde'][$i]['stort'],2),
    ...
    if($indexLookup[$cat]=='')
    {
      $kleuren=array($this->pdf->rapport_portefeuillekleur);
    }
    else
    {
      $kleuren=array($this->pdf->rapport_portefeuillekleur,$this->pdf->rapport_benchmarkkleur);
    }
    $this->pdf->ln();
    if($n%2==0)
    {
