...
class RapportCASHY_L112
{
	function RapportCASHY_L112($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	 //
		$this->pdf = &$pdf;
    ...
    $beginMaand=date('n',$this->pdf->rapport_datum);
		for($i=1;$i<13;$i++)
		{
		  $maandNr=$i+$beginMaand;
		  if($maandNr>12)
        $maandNr=$maandNr-12;
      $maand=$maanden[$maandNr];
		...
		$jaarLater=db2jul(($rapJaar+1).'-'.substr($this->rapportageDatum,5,5));
		foreach ($cashfow->regelsRaw as $regel)
		{
		  $regelJul=form2jul($regel['0']);
		  $jaar=substr($regel['0'],6,4);
 		  if($jaar > ($rapJaar+11))
	      $jaar='> '.($rapJaar+11).' '.vertaalTekst('cumulatief' ,$this->pdf->rapport_taal);
		  $maand=$maanden[intval(substr($regel['0'],3,2))];
		  $cashflowJaar[$jaar]['lossing'] +=0;
		  $cashflowJaar[$jaar]['rente'] +=0;
		  //echo $regel['0']."=".jul2db($regelJul)." < ".jul2db($jaarLater)."<br>\n";
		  if($regelJul<=$jaarLater)
...
$this->pdf->CellBorders = array();
	...
	function VBarDiagram($w, $h, $data,$titel)
  {
      ...
      $colors=array('rente'=>$this->pdf->rapport_rentekleur,//$this->pdf->rapport_rentekleur,
      'lossing'=>$this->pdf->rapport_lossingkleur);
