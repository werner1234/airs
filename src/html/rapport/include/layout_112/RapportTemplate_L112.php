...
class RapportTemplate_L112
{
	function RapportTemplate_L112($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	  global $__appvar;
		$this->pdf = &$pdf;
    ...
    $totPagina = ($lastpage-$startpagina+$paginaCorrectie);//-1
       ...
       $this->pdf->ln(10);
	     ...
	     $inhoudsItems=array('PERFPaginas'=>'Performance over de beleggingscategorieën',
                           ...
                           'KERNVPaginas'=>'KERNV',
                           'KERNZPaginas'=>'KERNZ',
													 'DOORKIJKPaginas'=>'Allocaties inclusief uitsplitsing aandelen',
        ...
        foreach ($this->pdf->templateVars as $key=>$value)
        {
          ...
          if($tonen==true) //$this->pdf->templateVarsOmschrijving
          {
            //$text=$inhoudsItems[$key].' ';
            $stringWidth=$this->pdf->GetStringWidth($text);
            $dots=round(($this->pdf->widths[1]-$stringWidth)/($this->pdf->CurrentFont['cw']['.']*$this->pdf->FontSize/1000));
