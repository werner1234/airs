...
$Log: RapportATT_L64.php,v $
Revision 1.3  2017/11/04 17:40:21  rvv
*** empty log message ***

Revision 1.2  2015/12/23 16:55:01  rvv
*** empty log message ***
...
include_once($__appvar["basedir"]."/html/rapport/include/ATTberekening_L64.php");


class RapportATT_L64
{
	function RapportATT_L64($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//listarray($this->categorieVolgorde);
  ...
  $index=new indexHerberekening_L64();
...
foreach ($indexData as $index=>$data)
{
  ...
  //  foreach ($data['categorieVerdeling'] as $categorie=>$waarde)
    ...
    WHERE Portefeuille='".$this->portefeuille."' AND Beleggingscategorie <>'' 
...
JOIN KeuzePerVermogensbeheerder ON Beleggingscategorien.Beleggingscategorie = KeuzePerVermogensbeheerder.waarde 
AND KeuzePerVermogensbeheerder.categorie='Beleggingscategorien' AND KeuzePerVermogensbeheerder.vermogensbeheerder='$beheerder'
   ...
   if(count($rendamentWaarden) > 0)
   {
     ...
     //   $this->pdf->CellBorders = array('','US','US','US','US','US','US','US','US','US','US','US');
       ...
       //$this->pdf->SetFillColor(230,230,230);
        $this->pdf->SetFillColor(189,255,251);
