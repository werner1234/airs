...
$Log: RapportPERF_L78.php,v $
Revision 1.7  2019/10/12 17:11:44  rvv
*** empty log message ***
...
include_once("rapport/include/ATTberekening_L78.php");

class RapportPERF_L78
{

	function RapportPERF_L78($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    $this->att=new ATTberekening_L78($this);
	}
...
WHERE Portefeuilles.Portefeuille = '$this->portefeuille'";
    ...
    $this->pdf->SetWidths(array(138,60,20,20,20));
    $this->pdf->SetAligns(array('L','L','R','R','R','R'));
    $this->pdf->Rect($this->pdf->marge+138,120,120,count($benchmarks)*4+4);
...
function addResultaat()
{
  ...
  //$att = new ATTberekening_L78($this);
  ...
  if (db2jul($this->pdf->PortefeuilleStartdatum) > db2jul(substr($this->rapportageDatum, 0, 4) . '-01-01'))
  {
    $ytdStart=substr($this->pdf->PortefeuilleStartdatum,0,10);
  }
  else
  {
    $ytdStart=substr($this->rapportageDatum, 0, 4).'-01-01';
  }
    $this->waarden['YTD']=$this->att->bereken($ytdStart,$this->rapportageDatum,'Categorien');
...
//listarray($this->waarden['Periode']);
 ...
 // listarray($this->pdf->portefeuilles);
  ...
  $rendementYTD=array("",vertaalTekst("Rendement lopend jaar",$this->pdf->rapport_taal));
...
foreach($categorien as $categorie)
{
  ...
  foreach($categorien as $categorie)
  {
    ...
    $rendement[]='%';
    $rendementYTD[]=$this->formatGetal($this->waarden['YTD'][$categorie]['procent'],2);
    $rendementYTD[]='%';
    }
    if($categorie=='totaal')
    {
