...
class RapportKERNV_L111
{
	function RapportKERNV_L111($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function header($categorieVolgorde,$categorieOmschrijving,$extraW)
  {
    ...
    foreach($categorieVolgorde as $categorie=>$volgorde)
    {
      ...
      $this->pdf->Cell(35+$extraW, 6, vertaalTekst($categorieOmschrijving[$categorie], $this->pdf->rapport_taal), 0, 0, "C", 1);
      $xlsData[] = $categorieOmschrijving[$categorie];
    }
    $this->pdf->Cell(2, 6, '', 0, 0, "C",0);
    $this->pdf->Cell(35+$extraW, 6, vertaalTekst('Totaal (€)',$this->pdf->rapport_taal), 0, 0, "C",1);
    $xlsData[] = vertaalTekst('Totaal (€)',$this->pdf->rapport_taal);
    $this->pdf->Cell(2, 6, '', 0, 0, "C",0);
    $this->pdf->Cell(35+$extraW, 6, vertaalTekst('Totaal (%)',$this->pdf->rapport_taal), 0, 0, "C",1);
    $xlsData[] = vertaalTekst('Totaal (%)',$this->pdf->rapport_taal);
    $this->pdf->Ln();
  ...
  function setTxtColor($bedrag)
  {
    ...
    {
      $query="SELECT Portefeuille,ClientVermogensbeheerder,Selectieveld1  FROM Portefeuilles WHERE Portefeuille IN('".implode("','",$this->pdf->portefeuilles)."')";
      $DB->SQL($query);
      $DB->Query();
   ...
   // echo $portefeuilleAantal."<br>\n";listarray($portefeuilleWaarden);exit;
    ...
    $firstHeader = true;
      ...
      if(1)//count($this->pdf->portefeuilles) < 60)
      {
        ...
        $header = $this->header($categorieVolgorde,$categorieOmschrijving,$extraW);
        ...
        foreach ($portefeuilleWaarden as $portefeuille=>$pdata)
        {
          ...
          if(isset($portefeuilleWaardenDetails[$portefeuille]))
          {
            foreach($portefeuilleWaardenDetails[$portefeuille] as $omschrijving=>$veldData)
            {
              ...
              foreach ($categorieVolgorde as $categorie => $volgorde)
              {
                ...
                $xlsRowPortefeuilleWaarden[] = $veldData['belCatWaarde'][$categorie];
              ...
              $xlsRowPortefeuilleWaarden[] = $portWaarde;
              $this->pdf->Cell(37+$extraW, 4, $this->formatGetal($portWaarde/$totaalWaarde*100,1).' %', 0, 1, "R");
              $xlsRowPortefeuilleWaarden[] = ($portWaarde/$totaalWaarde*100);
          ...
          else
          {
            ...
            foreach ($categorieVolgorde as $categorie => $volgorde)
            {
              ...
              $xlsRowPortefeuilleWaarden[] = $pdata['belCatWaarde'][$categorie];
    ...
    if(isset($this->pdf->__appvar['consolidatie']))
    {
      ...
      foreach ($portefeuilleWaarden as $portefeuille=>$waarde)
      {
        ...
        {
          $overigePercentage+=$percentage;
        }
        if($waarde['totaleWaarde'] < 0)
          $barGraph=true;
        $i++;
      }
      if(abs($overigePercentage)>0.1)
      {
        $kop=($i).'.||'."Overige portefeuilles";
        $portefeuilleAandeel[$kop] = $overigePercentage;
        $portefeuilleKleur[] = array(100,100,100);
        $portefeuilleKleurBar[$kop] = array(100,100,100);
      }
     // listarray($overigePercentage);exit;
