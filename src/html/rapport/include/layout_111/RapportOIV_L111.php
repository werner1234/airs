...
class RapportOIV_L111
{
	function RapportOIV_L111($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    if($this->pdfOutput==true)
    {
      ...
      if (!empty($totaalA))
      {
        //$this->pdf->Line($actueel+2+ $this->pdf->widthA[2]+ $this->pdf->widthA[3],$this->pdf->GetY(),$actueel + $this->pdf->widthA[3],$this->pdf->GetY());
      ...
      $this->pdf->Cell($this->pdf->widthA[2], 4, $title, 0, 0, "R");
      $this->pdf->Cell($this->pdf->widthA[3], 4, $totaalAtxt, 0, 0, "R");
      $this->pdf->Cell($this->pdf->widthA[4], 4, $totaalBtxt, 0, 0, "R");
      $this->pdf->Cell($this->pdf->widthA[5], 4, $totaalprtxt, 0, 0, "R");
	...
	function renteEnLiquiditeiten($koersPrinted=true)
	{
		...
		if($this->pdfOutput==true && round($rente['subtotaalactueelvaluta'],2) <> 0)
		{
      $this->pdf->excelData[] = array (
        "",
        "",
        vertaalTekst("Opgelopen Rente",$this->pdf->rapport_taal),
        $rente['subtotaalactueelvaluta'],
        $rente['subtotaalactueel'],
        "",
        ""
			...
			$this->pdf->row(array("",
                        ...
                        $this->formatGetal($rente['subtotaalactueel'],$this->pdf->rapport_OIV_decimaal),
											"",
											""));
		}
    ...
    $this->pdf->excelData[] = array(
      vertaalTekst("Valutasoort",$this->pdf->rapport_taal),
      vertaalTekst("Valutakoers",$this->pdf->rapport_taal),
      vertaalTekst("Beleggingscategorie",$this->pdf->rapport_taal),
      vertaalTekst("in valuta",$this->pdf->rapport_taal),
      vertaalTekst("in ".$this->pdf->rapportageValuta,$this->pdf->rapport_taal)
      );

		// voor data
		$this->pdf->widthA = array(25,15,50,25,25,15,15,110);
		$this->pdf->alignA = array('L','R','L','R','R','R','R');

		// voor kopjes
		$this->pdf->widthB = array(40,50,25,25,15,15,102);
    ...
    {
       $GrafiekValuta[$categorien['ValutaOmschrijving']]=$categorien['subtotaalactueel']/$totaalWaarde*100;//toevoeging kleuren.
    }
    $y=$this->pdf->getY();
    $this->addGrafiek($GrafiekValuta);
    $this->pdf->setY($y);
		...
		while($categorien = $DB->NextRecord())
    {
			...
			if(!empty($lastValuta) && $lastValuta <> $categorien['ValutaOmschrijving'])
			{
				$this->renteEnLiquiditeiten($koersPrinted);

				$percentageVanTotaal = $this->totaalactueel / ($totaalWaarde/100);
				$actueleWaardePortefeuille += $this->printTotaal("", $this->totaalactueelvaluta,$this->totaalactueel,  $percentageVanTotaal);
        ...
        $this->pdf->excelData[] =array(
          ...
          $categorien['subtotaalactueelvaluta'],
          $categorien['subtotaalactueel']
        );
	...
	//		$grafiekCategorien[$lastValuta]=$categorien[ValutaOmschrijving];
		...
		$actueel = $this->pdf->marge + $this->pdf->widthA[0] + $this->pdf->widthA[1] + $this->pdf->widthA[2] + $this->pdf->widthA[3];
		$proc = $this->pdf->marge + $this->pdf->widthA[0] + $this->pdf->widthA[1] + $this->pdf->widthA[2] + $this->pdf->widthA[3] + $this->pdf->widthA[4];
		$this->pdf->Line($actueel+2,$this->pdf->GetY(),$actueel + $this->pdf->widthA[4],$this->pdf->GetY());
		$this->pdf->Line($proc+2,$this->pdf->GetY(),$proc + $this->pdf->widthA[5],$this->pdf->GetY());
		...
		$this->pdf->Cell($this->pdf->widthA[4],4,$this->formatGetalKoers($actueleWaardePortefeuille,$this->pdf->rapport_OIV_decimaal), 0,0, "R");
		$this->pdf->Cell($this->pdf->widthA[5],4,$this->formatGetal(100,$this->pdf->rapport_OIV_decimaal_proc), 0,1, "R");

		$this->pdf->Line($actueel+2,$this->pdf->GetY(),$actueel + $this->pdf->widthA[4],$this->pdf->GetY());
		$this->pdf->Line($actueel+2,$this->pdf->GetY()+1,$actueel + $this->pdf->widthA[4],$this->pdf->GetY()+1);
		$this->pdf->Line($proc+2,$this->pdf->GetY(),$proc + $this->pdf->widthA[5],$this->pdf->GetY());
		$this->pdf->Line($proc+2,$this->pdf->GetY()+1,$proc + $this->pdf->widthA[5],$this->pdf->GetY()+1);
...
//listarray(//$this->pdf->pieData);
	...
	function addGrafiek($GrafiekValuta)
	{
    ...
    $this->pdf->printPie(array(),$kleurdata);
	}
}
?>
