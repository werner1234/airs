...
class ATTberekening_L111
{

	function ATTberekening_L111($rapportData,$periode='maanden')
	{
...
//listarray($RekeningDirecteKostenOpbrengsten); ob_flush();
     ...
     // listarray($AttributieStortingenOntrekkingenBruto);
      ...
      $resultaatBruto=($eindwaarde - $beginwaarde) - $AttributieStortingenOntrekkingen['totaal'];
  ...
  'stortEnOnttrekking'=>$AttributieStortingenOntrekkingen['storting']+$AttributieStortingenOntrekkingen['onttrekking'],
...
$totaalKosten=0;
$sum=array();
$stapelItems=array('procent','bijdrage','procentBruto','indexPerf');
$avgItems=array('weging','gemWaarde');
$somItems=array('resultaat','resultaatBruto','storting','onttrekking','kosten','opbrengst','stort','stortEnOnttrekking','gerealiseerd');
...
if($stapeling == true)
{
  ...
  //historischeWaarde
  $fondsenWhere=" Fonds IN('".implode('\',\'',$fondsData['fondsen'])."')";
  ...
  SUM(totaalAantal*fondsEenheid*(actueleFonds-beginwaardeLopendeJaar)*actueleValuta) as fondsresultaatEUR".
						 ...
						 " rapportageDatum ='$tot' AND $fondsenWhere AND".
  ...
  elseif($categorie=='H-Liq')
    $filter="AND ($fondsenWhere OR Fonds='')";
  else
    $filter='AND '.$fondsenWhere;
	...
	$query = "SELECT SUM(actuelePortefeuilleWaardeEuro) AS totaal FROM TijdelijkeRapportage WHERE rapportageDatum ='$van' AND portefeuille = '".$this->rapport->portefeuille."' AND  type = 'rente' $filter".$__appvar['TijdelijkeRapportageMaakUniek'];
	$DB->SQL($query);
	$DB->Query();
  ...
  function genereerMutatieLijst($rapportageDatumVanaf,$rapportageDatum,$fonds='')
    ...
    elseif($fonds!='')
     ...
     Rekeningmutaties.omschrijving as rekeningOmschrijving,
      ...
      $fondsResultaat=0;
      $valutaResultaat=0;
      if($verkoop_koers <> '' && $mutaties['Transactietype'] != "V/O" )
      { //listarray($historie);
