...
class RapportKERNZ_L111
{
	function RapportKERNZ_L111($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	...
	function header($categorieVolgorde,$extraW)
  {
    ...
    $xlsData[] = vertaalTekst("Investering", $this->pdf->rapport_taal);
    foreach($categorieVolgorde as $categorie=>$veld)
    {
      $this->pdf->Cell(2, 6, '', 0, 0, "C", 0);
      $this->pdf->Cell(35 + $extraW, 6,vertaalTekst($categorie, $this->pdf->rapport_taal), 0, 0, "C", 1);
      $xlsData[] = $categorie;
    }
    $this->pdf->Ln();
    ...
    if(is_array($this->pdf->portefeuilles))
    {
      $query="SELECT Portefeuille,ClientVermogensbeheerder,Selectieveld1,soortOvereenkomst  FROM Portefeuilles WHERE Portefeuille IN('".implode("','",$this->pdf->portefeuilles)."')";
      $DB->SQL($query);
      $DB->Query();
    ...
    asort($this->clientVermogensbeheerder);
      ...
      {
        if($this->soortOvereenkomst['soortOvereenkomst']==$soortOvereenkomstDetail)
   ...
   // echo $portefeuilleAantal."<br>\n";listarray($portefeuilleWaarden);exit;
    ...
    $firstHeader = true;
      ...
      if(1)//count($this->pdf->portefeuilles) < 60)
      {
        ...
        $header = $this->header($categorieVolgorde,$extraW);
        ...
        {
          $xlsRowPortefeuilleWaarden = array();
          ...
          else
          {
            ...
            $xlsRowPortefeuilleWaarden[] = $portefeuilleTxt;
            foreach ($categorieVolgorde as $categorie => $veld)
            {
              if ($veld == 'rendementProcent')
              {
                $this->pdf->Cell(37 + $extraW, 4, $this->formatGetal($portefeuilleWaarden[$portefeuille][$veld], 1) . ' %', 0, 0, "R");
                $xlsRowPortefeuilleWaarden[] = $portefeuilleWaarden[$portefeuille][$veld];
              }
              else
              {
          ...
          $this->pdf->excelData[] = $xlsRowPortefeuilleWaarden;
        }
        ...
        foreach($portefeuilleWaarden as $portefeuille=>$pData)
        {
          ...
          $aandeelOpTotaal=$portefeuilleWaarden[$portefeuille]['totaleWaarde']/$portefeuilleWaardenTotaal[$this->portefeuille]['totaleWaarde']*100;
          
        //  echo $portefeuilleWaardenTotaal[$this->portefeuille]['totaleWaarde']." ".round($portefeuilleWaardenTotaal[$this->portefeuille]['totaleWaarde']/100000);exit;
          $filterResultaat=round($portefeuilleWaardenTotaal[$this->portefeuille]['totaleWaarde']/100000);
         // echo "Aandeel op totaal:".round($aandeelOpTotaal,4). "| $portefeuille | rendement:$percentage | waarde:".$portefeuilleWaarden[$portefeuille]['totaleWaarde']." | resultaat:".$portefeuilleWaarden[$portefeuille]['resultaatVerslagperiode']."<br>\n";
          //if(abs(round($portefeuilleWaarden[$portefeuille]['resultaatVerslagperiode'])) <>0)
          if($aandeelOpTotaal>0.1 || abs(round($portefeuilleWaarden[$portefeuille]['resultaatVerslagperiode']))>$filterResultaat)
          {
         ...
         //   listarray($portefeuilleWaarden[$portefeuille]);
  ...
  //
    ...
    if($this->pdf->getY()>130 || $aantalPortefeuilles>10)
    {
      $this->pdf->addPage();
      $grafiekY=40;
