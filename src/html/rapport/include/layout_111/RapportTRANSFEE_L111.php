...
class RapportTRANSFEE_L111
{
  function RapportTRANSFEE_L111($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
  {
    $this->pdf = &$pdf;
    ...
    $this->pdf->excelData[] = array(vertaalTekst("Datum",$pdfObject->rapport_taal),
...
//	echo "lopende -> ".$resultaatlopende." <-  voorgaande ".$resultaatvoorgaande. " -  <br>" ;
      ...
      $xlsArray[] = $mutaties['Boekdatum'];
      $xlsArray[] = $transactietypenÓmschrijving[$mutaties['Transactietype']];
      $xlsArray[] = $mutaties['Omschrijving'];
      $xlsArray[] = $mutaties['Aantal'];
      ...
      $xlsArray[] = $totaalTransactie-$mutaties['kosten'];
...
//debug($mutaties);
      $bps = '';
      $rowProvisie = (round($mutaties['kosten'],2) >0) ? $mutaties['kosten']:0;
      $rowtotaal = (round($totaalTransactie,2) != 0) ? $totaalTransactie-$mutaties['kosten']:0;

      if ( ! empty ($rowProvisie) ) {
        if ( $mutaties['Transactietype'] === 'A' ) {
          $bps = $rowProvisie / ($rowtotaal - $rowProvisie) * 10000;
          $xlsArray[] = abs($bps);
          $bps = $this->formatGetal(abs($bps), 2);
        } elseif ( $mutaties['Transactietype'] === 'V' ) {
          $bps = $rowProvisie / ($rowtotaal + $rowProvisie) * 10000;
          $xlsArray[] = abs($bps);
          $bps = $this->formatGetal(abs($bps), 2);
        }
      ...
      $this->pdf->row(array($datum,
        vertaalTekst($transactietypenÓmschrijving[$mutaties['Transactietype']], $this->pdf->rapport_taal),
        '',
