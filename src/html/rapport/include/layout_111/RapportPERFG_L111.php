...
class RapportPERFG_L111
{
	function RapportPERFG_L111($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
       ...
       // $this->pdf->SetFillColor($this->pdf->rapport_kop_bgcolor['r']*1.2,$this->pdf->rapport_kop_bgcolor['g']*1.2,$this->pdf->rapport_kop_bgcolor['b']*1.2);
        ...
        $totaalRendamentIndex=100;
          ...
          $this->pdf->excelData[] = array(
            ...
            $row['waardeBegin'],
            $row['stortingen']-$row['onttrekkingen'],
            $row['gerealiseerd']+$row['ongerealiseerd'],
            $row['opbrengsten'],
            $row['kosten'],
            $row['rente'],
            $row['resultaatVerslagperiode'],
            $row['waardeHuidige'],
            $row['performance'],
            $row['index']-100
          );
