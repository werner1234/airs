...
include_once($__appvar["basedir"]."/html/rapport/include/layout_111/RapportVOLK_L111.php");

class RapportHSE_L111
{
	function RapportHSE_L111($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->volk = new RapportVOLK_L111($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum);
		$this->pdf = &$pdf;
		$this->pdf->rapport_type = "HSE";
		$this->pdf->rapport_datum = db2jul($rapportageDatum);
		$this->pdf->rapport_datumvanaf = db2jul($rapportageDatumVanaf);
		$this->pdf->rapport_titel = "Huidige portefeuilles";

		$this->portefeuille = $portefeuille;
		$this->rapportageDatumVanaf = $rapportageDatumVanaf;
		$this->rapportageDatum = $rapportageDatum;
    $this->pdf->excelData=array();
    $this->pdf->excelData[]=array('Categorie','Fondsomschrijving','Valuta','Aantal','Koers','Waarde','Waarde EUR','Koers','Waarde','Waarde EUR','Weging in %','Fondsresultaat','Valutaresultaat','Directresultaat','in %');
	}
...
//     " Rekeningmutaties.Boekdatum >= '".	$this->rapportageDatumVanaf."' AND ".
	...
	function writeRapport()
	{
		...
		$this->pdf->widthB = array(51,10,20,15,20,21,1,16,21,21,17,20,20,19,12);
		$this->pdf->alignB = array('L','L','R','R','R','R','R','R','R','R','R','R','R','R','R','R');

		// voor kopjes
		$this->pdf->widthA = $this->pdf->widthB;// array(   60,18,15,20,21,1,16,21,21,17,20,20,19,12);
		$this->pdf->alignA = $this->pdf->alignB;//array('L','R','R','R','R','R','R','R','R','R','R','R','R','R','R');
...
/*
      ...
      ".
       ...
       FROM ".
			...
			.$__appvar['TijdelijkeRapportageMaakUniek'].
			" GROUP BY TijdelijkeRapportage.beleggingscategorie,TijdelijkeRapportage.AttributieCategorie ".
			" ORDER BY TijdelijkeRapportage.beleggingscategorieVolgorde asc, TijdelijkeRapportage.attributieCategorieVolgorde,  TijdelijkeRapportage.valutaVolgorde asc";
*/
    $query = "SELECT ".
      " TijdelijkeRapportage.valuta, ".
      "  ".
      ...
      " WHERE TijdelijkeRapportage.portefeuille = '".$this->portefeuille."' AND TijdelijkeRapportage.type = 'fondsen' AND TijdelijkeRapportage.rapportageDatum = '".$this->rapportageDatum."'"
      .$__appvar['TijdelijkeRapportageMaakUniek'].
      " GROUP BY TijdelijkeRapportage.AttributieCategorie ".
      " ORDER BY  TijdelijkeRapportage.attributieCategorieVolgorde,  TijdelijkeRapportage.valutaVolgorde asc";
    ...
    $subtotaalATT=array();
      ...
      if($lastAttCategorie<>'' && (($lastAttCategorie <> $categorien['attributieCategorieOmschrijving']) || ($lastCategorie<>'' && $lastCategorie <> $categorien['Omschrijving'])))
      {
        $title = vertaalTekst("Subtotaal",$this->pdf->rapport_taal)." ".vertaalTekst($lastAttCategorie,$this->pdf->rapport_taal);
        
        $procentResultaat = (( $subtotaalATT['actuelePortefeuilleWaardeEuro'] -  $subtotaalATT['historischeWaardeTotaalValuta'] + $subtotaalATT['totaalDividendCorrected']) / ( $subtotaalATT['historischeWaardeTotaalValuta'] /100));
        if( $subtotaalATT['historischeWaardeTotaalValuta'] < 0)
          $procentResultaat = -1 * $procentResultaat;
  
        $actueleWaardePortefeuille +=    $this->printTotaal($title,  $subtotaalATT['historischeWaardeTotaalValuta'],  $subtotaalATT['actuelePortefeuilleWaardeEuro'], $subtotaalATT['percentageVanTotaal'] ,  $subtotaalATT['fondsResultaat'],  $subtotaalATT['valutaResultaat'], $procentResultaat,false,$subtotaalATT['totaalDividend']);
        $subtotaalATT=array();
      }
      
			if($lastCategorie<>'' && $lastCategorie <> $categorien['Omschrijving'])
			{
...
//				$actueleWaardePortefeuille += $this->printTotaal($title, $totaalbegin, $totaalactueel, $totaalpercentage , $totaalfondsresultaat, $totaalvalutaresultaat, $procentResultaat,false,$totaaldividend);
      ...
      if($lastAttCategorie <> $categorien['attributieCategorieOmschrijving'])
        $this->printKop(vertaalTekst($categorien['attributieCategorieOmschrijving'],$this->pdf->rapport_taal), "b");
      $lastAttCategorie=$categorien['attributieCategorieOmschrijving'];
...
".
				...
				"   TijdelijkeRapportage.AttributieCategorie =  '".$categorien['AttributieCategorie']."' AND ". //TijdelijkeRapportage.beleggingscategorie =  '".$categorien['beleggingscategorie']."' AND
...
//echo $subquery."<br><br>";exit;
      ...
      $kopPrinted=false;
        ...
        $this->pdf->excelData[]=array($categorien['Omschrijving'],$subdata['fondsOmschrijving'],
          $subdata['Valuta'],
          round($subdata['totaalAantal'],$this->pdf->rapport_VOLK_aantal_decimaal),
          round($subdata['historischeWaarde'],2),
          round($subdata['historischeWaardeTotaal'],$this->pdf->rapport_VOLK_decimaal),
          round($subdata['historischeWaardeTotaalValuta'],$this->pdf->rapport_VOLK_decimaal),
          ...
          round($percentageVanTotaal,$this->pdf->rapport_VOLK_decimaal_proc),
        ...
        $subtotaalATT['historischeWaardeTotaalValuta'] +=$subdata['historischeWaardeTotaalValuta'];
        $subtotaalATT['actuelePortefeuilleWaardeEuro'] +=$subdata['actuelePortefeuilleWaardeEuro'];
        $subtotaalATT['percentageVanTotaal'] +=$percentageVanTotaal;
        $subtotaalATT['fondsResultaat'] +=$fondsResultaat;
        $subtotaalATT['valutaResultaat'] +=$valutaResultaat;
        $subtotaalATT['totaalResultaat'] +=$subTotaalResultaat;
        $subtotaalATT['totaalBijdrage'] += $subTotaalBijdrage;
        $subtotaalATT['totaalDividend'] += $dividend['totaal'];
        $subtotaalATT['totaalDividendCorrected'] += $dividend['corrected'];
...
//rvv
   ...
   // " TijdelijkeRapportage.beleggingscategorie =  '".$categorien['beleggingscategorie']."' AND ".
 		" TijdelijkeRapportage.AttributieCategorie =  '".$categorien['AttributieCategorie']."' AND ".
		" TijdelijkeRapportage.rapportageDatum = '".$this->rapportageDatum."' "
		.$__appvar['TijdelijkeRapportageMaakUniek'].
      ...
      if($totaalRenteInValuta <> 0)
      {
        ...
        $this->printSubTotaal(vertaalTekst("Subtotaal",$this->pdf->rapport_taal)." ".vertaalTekst("Opgelopen rente",$this->pdf->rapport_taal), "", $totaalRenteInValuta,$subtotaalPercentageVanTotaal,"","");
		    $categorien['subtotaalactueel']+=$totaalRenteInValuta;
        $subtotaal['percentageVanTotaal']+=$subtotaalPercentageVanTotaal;

        $this->pdf->excelData[]=array("Opgelopen Rente",'','',
          "",
          "",
          "",
          "",
    ...
    if($subtotaalATT['actuelePortefeuilleWaardeEuro'] <> 0 || $subtotaalATT['historischeWaardeTotaalValuta'] <> 0)
    {
      $title = vertaalTekst("Subtotaal",$this->pdf->rapport_taal)." ".vertaalTekst($lastAttCategorie,$this->pdf->rapport_taal);
...
//		$actueleWaardePortefeuille += $this->printTotaal(vertaalTekst("Subtotaal",$this->pdf->rapport_taal)." ".vertaalTekst($lastCategorie,$this->pdf->rapport_taal), $totaalbegin, $totaalactueel,$totaalpercentage,$totaalfondsresultaat,$totaalvalutaresultaat,$procentResultaat,false,$totaaldividend);
		...
		if($DB1->records() > 0)
		{
		  //echo $this->pdf->rowHeight." ". ($DB1->records()+6)*$this->pdf->rowHeight ." + ". $this->pdf->GetY() ." > " .$this->pdf->PageBreakTrigger."<br>\n";ob_flush();
      ...
      if($DB1->records()==1 && ($DB1->records()+6)*$this->pdf->rowHeight+$this->pdf->GetY() > $this->pdf->PageBreakTrigger )
        ...
        $this->pdf->excelData[]=array($categorien['Omschrijving'],$omschrijving,
          $data['Valuta'],
          "",
          "",
          "",
          ...
          round($subtotaalPercentageVanTotaal,$this->pdf->rapport_VOLK_decimaal_proc)          );
			}
		...
		//echo "Fout : Fout in rapport ".$this->portefeuille.", totale waarde (".round($totaalWaarde,2).") komt niet overeen met afgedrukte totaal (".round($actueleWaardePortefeuille,2).") in rapport ".$this->pdf->rapport_type."' "
		//.round($actueleWaardePortefeuille-$totaalWaarde,2);
		if(round(($totaalWaarde - $actueleWaardePortefeuille),2) <> 0)
		{
			echo "<script>
			alert('Fout : Fout in rapport ".$this->portefeuille.", totale waarde (".round($totaalWaarde,2).") komt niet overeen met afgedrukte totaal (".round($actueleWaardePortefeuille,2).") in rapport ".$this->pdf->rapport_type." verschil:".round($totaalWaarde - $actueleWaardePortefeuille)."')
			</script>";
			ob_flush();
		}
