...
$Log: RapportVKMS_L70.php,v $
Revision 1.15  2020/07/15 16:38:19  rvv
*** empty log message ***
...
class RapportVKMS_L70
{
	function RapportVKMS_L70($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    if($this->pdfVullen==true)
		{
	  	...
	  	$this->pdf->row(array(vertaalTekst('Doorlopende kosten beleggingsfondsen',$this->pdf->rapport_taal), $this->formatGetal($totaalDoorlopendekosten, 0) . ' EUR'));
			$this->pdf->excelData[]=array('','Doorlopende kosten beleggingsfondsen', '','',round($totaalDoorlopendekosten, 0), 'EUR');
			$this->pdf->row(array(vertaalTekst('Doorlopende kosten vermogenbeleggingsfondsen t.a.v. onderliggend vermogen',$this->pdf->rapport_taal), $this->formatGetal($doorlopendeKostenPercentage * 100, 2) . ' %'));
			$this->pdf->excelData[]=array('','Doorlopende kosten vermogenbeleggingsfondsen t.a.v. onderliggend vermogen','','', round($doorlopendeKostenPercentage * 100, 2) ,'%');
			$this->pdf->ln();
			$this->pdf->excelData[]=array();
    ...
    $grootboekKosten=array();
        ...
        if($data['Grootboekrekening']=='BEH')
        {
          //$this->pdf->row(array(vertaalTekst($data['Omschrijving'] . ' (incl. BTW)', $this->pdf->rapport_taal), $this->formatGetal($data['totaal'], 0), $this->formatGetal($kostenProcent, 2) . ' %'));
          ...
          if($risicoklasse['BeheerfeeBTW']<>0)
            $this->pdf->row(array(vertaalTekst('BTW over '.$data['Omschrijving'] ,$this->pdf->rapport_taal),$this->formatGetal($data['totaal']-$zonderBTWBedrag,0),$this->formatGetal(($data['totaal']-$zonderBTWBedrag)/$gemiddelde*100,2). ' %'));
        }
        else
    ...
    $this->vkmWaarde=array('vkmPercentagePortefeuille'=>$vkmPercentagePortefeuille,'kostenPercentage'=>$kostenPercentage,'vkmWaarde'=>$vkmWaarde,'grootboekKosten'=>$grootboekKosten,
      ...
      'totaalDoorlopendekostenGesplitst'=>$totaalDoorlopendekostenGesplitst);
			...
			$this->pdf->setXY(30,$this->pdf->getY()+60);
			arsort($barData);
			//echo $startYGrafiek-$startY-10;exit;
			$huidigeY=$this->pdf->getY();
			$this->VBarVerdeling(50,50,$barData);
    ...
    $huidigeY=265;
		...
		$this->pdf->setXY($this->pdf->marge,$huidigeY);
		$this->pdf->setWidths(array($prefix,$cellWidth));
		$this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
		$this->pdf->setAligns(array('L','L'));
	//	$this->pdf->rect($this->pdf->marge+$prefix,$huidigeY-1,$cellWidth,$this->pdf->rowHeight*2+2);
		$this->pdf->row(array('',vertaalTekst('Berekening gebaseerd op gemiddelde spread kosten.',$this->pdf->rapport_taal).' '.vertaalTekst('Gedetailleerd kosten overzicht op verzoek beschikbaar.',$this->pdf->rapport_taal)));
	//	$this->pdf->row(array('','- '.));
	}
...
" $rekeningFondsenWhere ";
	...
	function writeRapport()
	{
		...
		$query = "SELECT Clienten.Naam, Clienten.Naam1, Portefeuilles.Depotbank,Portefeuilles.spreadKosten, Portefeuilles.SpecifiekeIndex, Portefeuilles.Vermogensbeheerder, Portefeuilles.ClientVermogensbeheerder, Portefeuilles.Risicoklasse, Portefeuilles.Client FROM Portefeuilles, Clienten WHERE Portefeuille = '".$this->portefeuille."' AND Portefeuilles.Client = Clienten.Client ";
		$DB = new DB();
		$DB->SQL($query);
		$DB->Query();
