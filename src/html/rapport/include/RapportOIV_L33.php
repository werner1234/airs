...
$Log: RapportOIV_L33.php,v $
Revision 1.4  2018/02/21 17:15:09  rvv
*** empty log message ***

Revision 1.3  2018/02/17 19:18:57  rvv
*** empty log message ***

Revision 1.2  2017/11/29 16:18:18  rvv
*** empty log message ***
...
class RapportOIV_L33
{
	function RapportOIV_L33($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
SUM(IF(TijdelijkeRapportage.type = 'fondsen',(TijdelijkeRapportage.totaalAantal * TijdelijkeRapportage.historischeWaarde * TijdelijkeRapportage.fondsEenheid),0)) AS historischeWaardeTotaal,
...
//echo $data['fonds']." ".$data['historischeValutakoers']." <br>\n";
      ...
      else
      {
        ...
        $valutaResultaat = round($ongerealiseerdResultaat- $fondsResultaat,2);
...
//echo $data['categorieOmschrijving']." <> $lastcategorieOmschrijving ? ".$data['fondsOmschrijving']." ".round($data['rente'])." ".round($totalen['rente'])."<br>\n";
      ...
      '',$this->formatGetal($data['actuelePortefeuilleWaardeEuro'],0),$this->formatGetal($data['historischeWaardeEuro'],0),$this->formatGetal($fondsResultaat,0),$this->formatGetal($valutaResultaat,0),$this->formatGetal($aandeel,1),$this->formatGetal($data['rente'],0)));
      $lastcategorieOmschrijving=$data['categorieOmschrijving'];
      $this->pdf->excelData[]=array($data['fondsOmschrijving'],$data['valuta'],round($data['totaalAantal'],$afronding),round($data['actueleFonds'],8),round($data['historischeWaarde'],2),
       round($data['actuelePortefeuilleWaardeEuro'],0),round($data['historischeWaardeEuro'],0),round($fondsResultaat,0),round($valutaResultaat,0),round($aandeel,1),round($data['rente'],0));
    }
