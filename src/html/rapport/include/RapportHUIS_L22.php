...
class RapportHUIS_L22
{
	function RapportHUIS_L22($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	...
	function writeRapport()
	{
		...
		$query = "SELECT Fondsen.Portefeuille,
              ...
              WHERE Fondsen.Huisfonds=1 AND rapportageDatum ='".$this->rapportageDatum."' AND FondsenBuitenBeheerfee.LayoutNr=0 AND
    ...
    foreach($portefeuilles as $portefeuille=>$pdata)
    {
      ...
      foreach($fondswaarden['b'] as $fonds)
        $portefeuilleWaarde+=$fonds['actuelePortefeuilleWaardeEuro'];
			...
			{
				$rapport = new RapportVOLK_L22($this->pdf, $portefeuille, $rapportageDatum['a'], $rapportageDatum['b']);
