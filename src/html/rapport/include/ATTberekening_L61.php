...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: ATTberekening_L61.php,v $
 		Revision 1.6  2017/05/26 16:45:07  rvv
 		*** empty log message ***
 		
 		Revision 1.5  2016/06/15 15:58:41  rvv
 		*** empty log message ***
...
class ATTberekening_L61
{

	function ATTberekening_L61($rapportData,$periode='maanden')
	{
...
//echo "$categorie $totaalGemiddelde = $totaalBeginwaarde - ".$storting['gewogen']."<br>\n"; 
    ...
    if($categorie=='totaal')
    {
      ...
      $query="SELECT SUM((ABS(Rekeningmutaties.Credit) * Rekeningmutaties.Valutakoers ) - 
      (ABS(Rekeningmutaties.Debet) * Rekeningmutaties.Valutakoers ))  AS totaal 
      FROM Rekeningmutaties 
      JOIN Rekeningen ON Rekeningen.Rekening = Rekeningmutaties.Rekening 
      JOIN Grootboekrekeningen on Grootboekrekeningen.Grootboekrekening = Rekeningmutaties.Grootboekrekening AND Grootboekrekeningen.Kosten =1
      WHERE Rekeningen.Portefeuille = '".$this->rapport->portefeuille."' AND Rekeningmutaties.Verwerkt = '1' AND
	    Rekeningmutaties.Boekdatum > '".$datumBegin."' AND Rekeningmutaties.Boekdatum <= '".$datumEind."'  ";
	    $DB->SQL($query);
	    $DB->Query();
	    $kosten = $DB->NextRecord();
      $kostenpercentage=$kosten['totaal']/$this->totaalGemiddelde*100;
    }  
    else
    {
...
//listarray($AttributieStortingenOntrekkingen);
      ...
      $queryKostenOpbrengsten = "SELECT
       ...
       if(count($fondsData['rekeningen']) > 0 && $fondsData['rekeningen'][0] <> 'geen')
       {
//listarray($fondsData);listarray($nietToegerekendeKosten);
