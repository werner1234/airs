...
/*
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2018/02/28 16:37:25 $
 		File Versie					: $Revision: 1.2 $

 		$Log: RapportVOLKD_L69.php,v $
 		Revision 1.2  2018/02/28 16:37:25  rvv
 		*** empty log message ***
 		
 		Revision 1.1  2016/10/19 15:37:34  rvv
 		*** empty log message ***
 		
 		Revision 1.40  2015/12/14 07:13:50  rvv
 		*** empty log message ***
 		
 		Revision 1.39  2015/07/22 13:16:13  rvv
 		*** empty log message ***
 		
 		Revision 1.38  2015/01/24 19:55:49  rvv
 		*** empty log message ***
 		
 		Revision 1.37  2014/06/04 16:13:28  rvv
 		*** empty log message ***
 		
 		Revision 1.36  2014/04/16 15:51:22  rvv
 		*** empty log message ***
 		
 		Revision 1.35  2014/02/05 16:02:14  rvv
 		*** empty log message ***
 		
 		Revision 1.34  2013/12/11 17:13:16  rvv
 		*** empty log message ***
 		
 		Revision 1.33  2013/06/09 18:01:53  rvv
 		*** empty log message ***
 		
 		Revision 1.32  2013/06/05 15:56:07  rvv
 		*** empty log message ***
 		
 		Revision 1.31  2013/04/24 16:01:30  rvv
 		*** empty log message ***
 		
 		Revision 1.30  2013/02/10 10:06:07  rvv
 		*** empty log message ***
 		
 		Revision 1.29  2013/01/20 13:27:16  rvv
 		*** empty log message ***
 		
 		Revision 1.28  2012/10/28 11:04:15  rvv
 		*** empty log message ***
 		
 		Revision 1.27  2012/09/26 16:05:54  rvv
 		*** empty log message ***
...
class RapportVOLKD_L69
{
	function RapportVOLKD_L69($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
$query = "SELECT TijdelijkeRapportage.fondsOmschrijving,
		           ...
		           LEFT JOIN Beleggingssectoren ON (TijdelijkeRapportage.beleggingssector = Beleggingssectoren.Beleggingssector)
				       ...
				       (TijdelijkeRapportage.type = 'fondsen' OR TijdelijkeRapportage.type = 'rente' )  AND TijdelijkeRapportage.Fonds <> '' AND
				       TijdelijkeRapportage.rapportageDatum $datum
				       ".$__appvar['TijdelijkeRapportageMaakUniek']."
				       GROUP BY TijdelijkeRapportage.fonds
				       ORDER BY Beleggingscategorien.Afdrukvolgorde asc, Beleggingssectoren.Afdrukvolgorde asc, TijdelijkeRapportage.Lossingsdatum, TijdelijkeRapportage.fondsOmschrijving asc";
$DB->SQL($query);
$DB->Query();
while($data=$DB->nextRecord())
{
  ...
  $this->fondsenTree[$data['beleggingscategorie']][$data['beleggingssector']][$data['fonds']]=$data['fonds'];

  foreach ($this->perioden as $periode)
  {
    $this->waarden[$periode][$data['beleggingscategorie']][$data['beleggingssector']][$data['fonds']]= $data;
	    ...
	    $query = "SELECT TijdelijkeRapportage.fondsOmschrijving, TijdelijkeRapportage.rapportageDatum,
               ...
               LEFT JOIN Fondsen ON (TijdelijkeRapportage.fonds = Fondsen.Fonds)
                ...
                $this->fondsForward[$data['fonds']]=$data['forward'];
		            $sectorTotalen[$periode][$data['beleggingscategorie']][$data['beleggingssector']]['actuelePortefeuilleWaardeInValuta'] += $data['actuelePortefeuilleWaardeInValuta'];
		            $sectorTotalen[$periode][$data['beleggingscategorie']][$data['beleggingssector']]['actuelePortefeuilleWaardeEuro'] += $data['actuelePortefeuilleWaardeEuro'];

		          //  $superTotalen[$periode]['actuelePortefeuilleWaardeInValuta'] += $data['actuelePortefeuilleWaardeInValuta'];
		          //  $superTotalen[$periode]['actuelePortefeuilleWaardeEuro'] += $data['actuelePortefeuilleWaardeEuro'];
    ...
    $periodeDone=array();
	  foreach ($this->perioden as $periode)
	  {
		...
		$query = "SELECT (TijdelijkeRapportage.fondsOmschrijving) as fondsOmschrijving,  TijdelijkeRapportage.rapportageDatum, ".
			...
			" (TijdelijkeRapportage.type = 'rekening' OR TijdelijkeRapportage.type = 'rente' ) AND ".
			" TijdelijkeRapportage.rapportageDatum = '$periode' "
			.$__appvar['TijdelijkeRapportageMaakUniek'].
		...
		if($DB1->records() > 0)
		{
         ...
         if($data['beleggingscategorie'] == '')
	       ...
	       $this->waarden[$periode]['LIQUIDITEITEN'][$data['beleggingscategorie']][$data['rekening']]= $data;
	       $this->fondsenPerCategorie[$data['beleggingscategorie']][$data['rekening']]=$data['rekening'];
	       $this->fondsenTree[$data['beleggingscategorie']][$data['beleggingssector']][$data['rekening']]=$data['rekening'];
	       $this->fondsOmschrijvingen[$data['rekening']]=$data['fondsOmschrijving']." ".$data['rekening'];
	       $categorieTotalen[$periode][$data['beleggingscategorie']]['actuelePortefeuilleWaardeInValuta'] += $data['actuelePortefeuilleWaardeInValuta'];
	       $categorieTotalen[$periode][$data['beleggingscategorie']]['actuelePortefeuilleWaardeEuro'] += $data['actuelePortefeuilleWaardeEuro'];
	 ...
	 // $hoofdcategorieKoppeling['LIQUIDITEITEN'][]='Liquiditeiten';
	 // $superCategorieKoppeling['LIQUIDITEITEN']['LIQUIDITEITEN']='LIQUIDITEITEN';



	  //listarray($this->waarden);flush();exit;
...
foreach ($this->waarden as $periode=>$data)
{
  ...
  {
     $this->toon['fonds'][$waarden[6]][$waarden[5]][$fonds]=1;
     if(!isset($this->waarden[$periode][$waarden[6]][$waarden[5]][$fonds]))
     {
       $this->waarden[$periode][$waarden[6]][$waarden[5]][$fonds]['fonds']=$fonds;
    ...
    //   listarray($fonds);
//listarray($waarden);
       $this->fondsenTree[$waarden[6]][$waarden[5]][$fonds]=$fonds;
       ...
       $this->fondsForward[$fonds]=$fondsOmschrijving['forward'];
		...
		$totalen=array();
foreach ($this->waarden as $periode=>$hoofdcategorien)
{
      ...
      foreach ($categorien as $categorie=>$fondsData)
      {
	      foreach ($fondsData as $fonds=>$waarden)
        {
         ...
         if(is_array($fondsMutaties[$fonds]))
         {
            ...
            $this->toon['fonds'][$hoofdcategorie][$categorie][$fonds]=1;
         }
       }
     }
   }
...
//listarray($fondsmutatiesPerCategorie);
...
//{
  foreach ($hoofdcategorieKoppeling as $hoofdcategorie =>$hoofdCategorieData)
  {
  //echo "$supercategorie $hoofdcategorie <br>\n ";ob_flush();listarray($hoofdcategorieKoppeling[$hoofdcategorie]);
       if($hoofdcategorieTotalen[$this->perioden['start']][$hoofdcategorie]['actuelePortefeuilleWaardeEuro'] <> 0 || $hoofdcategorieTotalen[$this->perioden['eind']][$hoofdcategorie]['actuelePortefeuilleWaardeEuro'] <>0 || $this->toon['hoofdcategorie'][$hoofdcategorie]==1)//|| $hoofdcategorieTotalen[$this->perioden['jan']][$hoofdcategorie]['actuelePortefeuilleWaardeEuro'] <>0
       {
         ...
         foreach ($hoofdCategorieData as $categorie)
         {
             if($sectorTotalen[$this->perioden['start']][$hoofdcategorie][$categorie]['actuelePortefeuilleWaardeEuro'] <>0 || $sectorTotalen[$this->perioden['eind']][$hoofdcategorie][$categorie]['actuelePortefeuilleWaardeEuro'] <>0 || $this->toon['categorie'][$categorie]==1)//|| $categorieTotalen[$this->perioden['jan']][$categorie]['actuelePortefeuilleWaardeEuro'] <>0
             {
               $regels['overzicht'][] = array('kopCategorie',array('omschrijving'=>$categorieOmschrijving[$categorie]));

               foreach ($this->fondsenTree[$hoofdcategorie][$categorie] as $fonds)//($this->fondsenPerCategorie[$categorie] as $fonds)
		           {
		             $totaalMutaties  = $this->waarden[$this->perioden['eind']][$hoofdcategorie][$categorie][$fonds]['actuelePortefeuilleWaardeEuro'] - $this->waarden[$this->perioden['start']][$hoofdcategorie][$categorie][$fonds]['actuelePortefeuilleWaardeEuro'];
		             $totaalMutatiesJaar  = $this->waarden[$this->perioden['eind']][$hoofdcategorie][$categorie][$fonds]['actuelePortefeuilleWaardeEuro'] - $this->waarden[$this->perioden['jan']][$hoofdcategorie][$categorie][$fonds]['actuelePortefeuilleWaardeEuro'];
                 ...
                 //echo $stortingenontrekkingen." = ".$mutaties['periode'][$fonds][0]." - $directeKostenOpbrengsten <br>\n";
		             $beleggingsresultaat = $totaalMutaties - $stortingenontrekkingen ;

  	             $bijdrageResultaat = $beleggingsresultaat / ($hoofdcategorieTotalen[$this->perioden['eind']][$hoofdcategorie]['actuelePortefeuilleWaardeEuro'] - $hoofdcategorieTotalen[$this->perioden['start']][$hoofdcategorie]['actuelePortefeuilleWaardeEuro'])*100;
                 ...
                 if($this->fondsForward[$fonds]==1)
                 {
		             ...
		             $bijdrageResultaatJaar = $beleggingsresultaat / ($hoofdcategorieTotalen[$this->perioden['eind']][$hoofdcategorie]['actuelePortefeuilleWaardeEuro'] - $hoofdcategorieTotalen[$this->perioden['jan']][$hoofdcategorie]['actuelePortefeuilleWaardeEuro'])*100;
...
//supercategorie
		           ...
		           if($this->waarden[$this->perioden['eind']][$hoofdcategorie][$categorie][$fonds]['actuelePortefeuilleWaardeEuro'] <> 0 ||
                  $this->waarden[$this->perioden['start']][$hoofdcategorie][$categorie][$fonds]['actuelePortefeuilleWaardeEuro'] <> 0 ||
                  $this->waarden[$this->perioden['eind']][$hoofdcategorie][$categorie][$fonds]['totaalAantal'] <> 0 ||
                  $this->toon['fonds'][$hoofdcategorie][$categorie][$fonds])
		            $regels['overzicht'][] = array('fonds',
		                                     array('omschrijving'=>$this->fondsOmschrijvingen[$fonds],
		                                         'totaalAantal'=>$this->waarden[$this->perioden['eind']][$hoofdcategorie][$categorie][$fonds]['totaalAantal'],
		                                         'actueleFonds'=> $this->waarden[$this->perioden['eind']][$hoofdcategorie][$categorie][$fonds]['actueleFonds'],
		                                         'startWaardeVal'=>$this->waarden[$this->perioden['start']][$hoofdcategorie][$categorie][$fonds]['actuelePortefeuilleWaardeInValuta'],
		                                         'startWaardeEur'=>$this->waarden[$this->perioden['start']][$hoofdcategorie][$categorie][$fonds]['actuelePortefeuilleWaardeEuro'],
		                                         'stort'=>$stortingenontrekkingen,
		                                         'actuelePortefeuilleWaardeInValuta'=>$this->waarden[$this->perioden['eind']][$hoofdcategorie][$categorie][$fonds]['actuelePortefeuilleWaardeInValuta'],
		                                         'actuelePortefeuilleWaardeEuro'=>$this->waarden[$this->perioden['eind']][$hoofdcategorie][$categorie][$fonds]['actuelePortefeuilleWaardeEuro'],
		             ...
		             {
                   //$gewogenResultaatJaar = $this->fondsPerformance2($this->fondsenPerCategorie[$categorie],$this->perioden['jan'],$this->perioden['eind']);
                   //$gewogenResultaat = $this->fondsPerformance2($this->fondsenPerCategorie[$categorie],$this->perioden['start'],$this->perioden['eind']);

                   //


                   $gewogenResultaatJaar = $this->fondsPerformance2($this->fondsenTree[$hoofdcategorie][$categorie],$this->perioden['jan'],$this->perioden['eind']);
                   $gewogenResultaat = $this->fondsPerformance2($this->fondsenTree[$hoofdcategorie][$categorie],$this->perioden['start'],$this->perioden['eind']);
		             }
		           ...
		           //$regels['overzicht'][] = array('categorieTotaal',array(''));
		              $regels['overzicht'][] = array('categorieTotaal',
		                              array('actuelePortefeuilleWaardeInValuta'=>'Subtotaal',//$categorieOmschrijving[$categorie]
		                                    'actuelePortefeuilleWaardeEuro'=>$sectorTotalen[$this->perioden['eind']][$hoofdcategorie][$categorie]['actuelePortefeuilleWaardeEuro'],//$categorieTotalen[$this->perioden['eind']][$categorie]['actuelePortefeuilleWaardeEuro'],
		                                    'startWaardeEur'=>$sectorTotalen[$this->perioden['start']][$hoofdcategorie][$categorie]['actuelePortefeuilleWaardeEuro'],
		                                    'resultaat'=>$gewogenResultaat,
		                                    'resultaatJaar'=>$gewogenResultaatJaar,
...
*/
      ...
      $performance='';$performanceJaar='';
		//echo $hoofdcategorie;ob_flush();exit;
      if($hoofdcategorie != 'LIQUIDITEITEN')
           {
$performance = $this->fondsPerformance2($this->fondsenPerCategorie[$hoofdcategorie],$this->perioden['start'],$this->perioden['eind'],false);
$performanceJaar = $this->fondsPerformance2($this->fondsenPerCategorie[$hoofdcategorie],$this->perioden['jan'],$this->perioden['eind'],false);
           }
