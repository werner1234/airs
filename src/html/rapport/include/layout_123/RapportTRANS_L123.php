...
class RapportTRANS_L123
{

  function RapportTRANS_L123($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
  {
    $this->pdf = &$pdf;
  ...
  function printRegel($data)
  {
    ...
    $this->pdf->Row(array($data['FONDS']['ISINCode'],
      '','','',
      ( $data['FONDS']['Valuta'] !== 'EUR' ? $this->formatGetal($data['FONDS']['Valutakoers'],2) : ''),
      '','','',
...
//    Rentepercentage
  ...
  function writeRapport()
  {
    ...
    if( $this->pdf->rapport_taal == 2 ) {
      ...
      $intro = "Le récapitulatif des transactions rend compte des transactions de placement exécutées pendant la période du reporting. La date indiquée est cell de l; execution de la transaction. Le montant de la transaction est toujours indiqué. Pour les achats et les ventes une monnaie étrangère, nous indiquons le taux de change le jour de la transaction. Pour les transactions liées aux obligations, les montants des coupons à percevoir sond indiqués séparément. ";
    } else {
