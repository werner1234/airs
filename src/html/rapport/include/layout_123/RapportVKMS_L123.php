...
class RapportVKMS_L123
{

	function RapportVKMS_L123($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function writeRapport()
    ...
    $poly=array($this->pdf->marge,25,
      $this->pdf->marge+90,25,
      $this->pdf->marge+90,180,
      $this->pdf->marge+90,187,
      $this->pdf->marge,187);
...
Trais payés à la banque pour les services (frais de transation, droits de garde, frais de gestion de portefeuille).

Taxes:
Tous impôts (Belges et étrangers) ainsi que toutes retenues à la source.

Remboursement des commissions de commercialisation:
Les fonds d'investissement facturent des frais de gestion poul la gestion des actifs du fonds. Si ABN AMRO recoit un remboursement de (une partie de) ces frais de gestion, cette partie vous seront remboursées. Le calcul est effectué sur la base de la moyenne des actifs investis dans le fond d'investissement à la fin de chaque trimestre.

Frais liés aux produits:
Aussi appelé frais récurrents. Il s'agit des frais engagés par le fonds d'investissement pour la gesion de ses actifs. Le coût principal est celui de la gestion (dont un partie est remboursée par la rétrocession des commissions de commercialasation), mais aussi les frais de transactions, les droits de garde, les honoraires des auditeurs, l'administration, etc. Ces frais sont déjà inclus dans le prix du fonds, mais sont précisés pour vous permettre d'avoir une vue globale de votre portefeuille.

Total indicatif des frais sur la période:
Cela donne simplement une indication des frais totaux (y compris les frais liés aux produits) en fonction du capital moyen investi. Ce pourcentage s'écarte du pourcentage que vous trouverez sous \"trais et taxes\" sur la page Performances, oú seuls les frais de services d'investissement sont pris en compte et où le calcul du taux de rendement prondéré par le temps s'applique.";
      $this->pdf->MultiCell(85,4,$tekst,0,'L');
    } else {
      $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
      $this->pdf->MultiCell(85,4,'Directe kosten',0,'L');
      $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);

      $tekst = '    Kosten beleggingsdienstverlening:
    In “Aan de bank” zijn alle kosten verwerkt die Triodos Bank 
    maakt. Het gaat daarbij om kosten voor: het beheer van uw 
    beleggingsportefeuille; de administratie van uw 
    beleggingsportefeuille; de beleggingsinformatie die u ontvangt;
    en de transacties (in Euro\'s). Separaat vermelde transactiekosten
    betreffen de valutakosten voor een koop- of verkooporder
    in buitenlandse valuta.
    
    Belastingen:
    De dividendbelasting en buitenlandse bronbelasting kunt u 
    mogelijk verrekenen bij uw belastingaangifte. Dit is afhankelijk van
    uw persoonlijke situatie. De transactiebelasting is onderdeel van
    een transactie in een buitenlands beleggingsproduct, deze 
    kunt u niet verrekenen in uw belastingaangifte.
      ';

      $this->pdf->MultiCell(88,4,$tekst,0,'L');

      $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
      $this->pdf->MultiCell(85,4,'    Indirecte kosten',0,'L');
      $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
      $tekst = '    Product kosten:
    Ook wel lopende kosten factor genoemd. Dit zijn de kosten van de
    beleggingsfondsen in uw portefeuille. Deze kosten worden
    verrekend in de koers van het fonds en dus niet afzonderlijk aan u
    doorberekend. Wij tonen deze kosten om u een correct beeld te
    geven van het volledige kostenoverzicht van uw portefeuille.
    
    Fonds transactie kosten:
    Dit zijn de kosten die een fondsbeheerder maakt om beleggingen
    in zijn fonds te kopen en te verkopen. Deze kosten betaalt u niet
    apart, maar zijn in de koers van het beleggingsfonds verwerkt. Wij
    tonen deze kosten om u een correct beeld te geven van het
    volledige kostenoverzicht van uw portefeuille.
      ';

      $this->pdf->MultiCell(88,4,$tekst,0,'L');

      $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
      $this->pdf->MultiCell(85,4,'    Indicatieve kosten rapportageperiode',0,'L');
      $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);


      $tekst = '    Dit geeft louter een indicatie van de totale kosten (incl. de externe
    productkosten) over het gemiddeld geïnvesteerd vermogen. Dit
    percentage zal afwijken van het percentage wat u kan terugvinden
    onder "kosten en belastingen" op de pagina van het
    beleggingsresultaat, waar enkel rekening gehouden wordt met
    effectief door de bank aangerekende kosten en waar TWR
    berekening van toepassing is.
      ';
      $this->pdf->MultiCell(89,4,$tekst,0,'L');
    }
    ...
    $grootboekrekeningenObj = new Grootboekrekening();
    $grootboeken = $grootboekrekeningenObj->getList(
      array('Grootboekrekening' => array_keys($this->vkmdata['grootboekKosten'])),
      'Grootboekrekening','Omschrijving'
    );

    if ( ! isset ($grootboeken['overige']) ) {
      $grootboeken['overige'] = 'Overige';
    }

    foreach($this->vkmdata['grootboekKosten'] as $gb=>$waarde)
    {
      $waarde=$waarde*-1;
//      $this->pdf->Row(array('', '', vertaalTekst('Kosten ' . $gb . '->'.(isset($vertalingen[$gb])?$vertalingen[$gb]:$gb), $this->pdf->rapport_taal), $this->formatGetal($waarde, 0), $this->formatGetal($waarde / $this->vkmdata['gemiddeldeWaarde'] * 100, 2) . '%'));
      $this->pdf->Row(array('', '', vertaalTekst((isset($grootboeken[$gb])?$grootboeken[$gb]:$gb), $this->pdf->rapport_taal), $this->formatGetal($waarde, 0), $this->formatGetal($waarde / $this->vkmdata['gemiddeldeWaarde'] * 100, 2) . '%'));
    }
    $this->pdf->Row(array('',vertaalTekst('Externe kosten', $this->pdf->rapport_taal)));
    //listarray($this->vkmdata);

    $this->pdf->Row(array('','','Belastingen',$this->formatGetal($this->vkmdata['belasting']*-1,0),$this->formatGetal($this->vkmdata['belasting']*-1 / $this->vkmdata['gemiddeldeWaarde'] * 100,2).'%'));
    ...
    $this->pdf->ln(2);
    $this->pdf->Row(array('',vertaalTekst('Totaal Externe kosten', $this->pdf->rapport_taal),$this->formatGetal($this->vkmdata['belasting']*-1,0),$this->formatGetal($this->vkmdata['belasting']/$this->vkmdata['gemiddeldeWaarde']*-100,2).'%'));
    $this->pdf->ln(2);
