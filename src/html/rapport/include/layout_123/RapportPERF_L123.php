...
include_once($__appvar["basedir"]."/html/rapport/include/layout_123/RapportATT_L123.php");

class RapportPERF_L123
{

	function RapportPERF_L123($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		$this->att= new RapportATT_L123($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum);
...
In the second table, we split te result in realised (booked) and unrealised results. The breakdown of the result is based on the above-mentiond categories."));
      ...
      $this->pdf->Row(array("La performance brute (avant frais) représente l'évolution de la valeur de vos actifs pendant la période de référence. Elle est composée des gains et/ou pertes enregistrés sur vos placements, du résultat des opérations de change, de tous les revenus, coupons et dividendes, et, si vouw avez des obligations individuelles en portefeuille, des intérêts courus.

La performance est corrigée de tous les dépôts et/ou retraits que vous aureiz pu faire pendant cette période. La performance ne tient compte ni des apports hi des retraits.

La performance nette correspond à la performance brute diminuée des frais et taxes.

Dans le deuxième tableua, nous distinguons les plus ou moins values réalisées et le plu ou mions values latentes. La répartition suit les catégories mentionnées ci-dessus."));
    } else {
...
In de tweede tabel splitsen we het resultaat op in gerealiseerd (geboekt) en ongerealiseerd resultaat, op basis van de hierboven genoemde categorieën."));
    ...
    $this->pdf->Row(array('',vertaalTekst('Bruto resultaat beleggingen', $this->pdf->rapport_taal),$this->formatGetal(($jaarWaarden['resultaatVerslagperiode']-$jaarWaarden['kosten']),2),$this->formatGetal(($jaarWaarden['resultaatVerslagperiode']-$jaarWaarden['kosten'])/$gemiddeldeVermogen*100,2).'%'));
    unset($this->pdf->fillCell);
    $this->pdf->Row(array('',vertaalTekst('Kosten & belastingen', $this->pdf->rapport_taal),$this->formatGetal($jaarWaarden['kosten'],2),$this->formatGetal($jaarWaarden['kosten']*-1/$gemiddeldeVermogen*-100,2).'%'));
  ...
  function LineDiagram($w, $h, $data, $color=null, $maxVal=0, $minVal=0, $horDiv=6, $verDiv=6,$jaar=0)
  {
    ...
    $lDiag = floor($w - 5);
    $color=array($this->pdf->triodosImpactRed[0],$this->pdf->triodosImpactRed[1],$this->pdf->triodosImpactRed[2]);
    ...
    $n=0;
    $this->pdf->Line($XDiag, $nulpunt, $XPage+$w ,$nulpunt,array('color'=>array(0,0,0)));
    for($i=$nulpunt; $i<= $bodem; $i+= $absUnit*$stapgrootte)
    {
      ...
      if($n%2!=0)
      {
        if($i+$absUnit * $stapgrootte < $bodem)
          $this->pdf->Rect($XDiag, $i, $w, $absUnit * $stapgrootte, 'F', null, $this->pdf->triodosLightPetrol);
      }
...
*/
    ...
    for ($i=0; $i<count($data); $i++)
    {
      ...
      $this->pdf->Cell($unit,4, date("d-M",db2jul($legendDatum[$i])),0,0,'C'); //Cell($w,$h=0,$txt='',$border=0,$ln=0,$align='',$fill=0,$link='')
      
      $yval2 = $YDiag + (($maxVal-$data[$i]) * $waardeCorrectie) ;
     
      $this->pdf->line($XDiag+$i*$unit+0.5, $yval, $XDiag+($i+1)*$unit, $yval2,$lineStyle );
     
      if($i==count($data)-1)
      {
        $this->pdf->Circle($XDiag + $i * $unit + 0.5,$yval,1,0,360, 'DF', '', array(255,255,255));
        $this->pdf->Circle($XDiag + ($i+1) * $unit + 0.5,$yval2,1,0,360, 'F', '',$color);
      }
      else
      {
        $this->pdf->Circle($XDiag + $i * $unit + 0.5,$yval,1,0,360, 'DF', '', array(255,255,255));
