...
class RapportMUT_L123
{

	function RapportMUT_L123($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function getRecords($type)
  {
    ...
    {
      $grootboekFilter = "AND NOT (Grootboekrekeningen.Onttrekking = '1' OR Grootboekrekeningen.Storting = '1' OR Grootboekrekeningen.Kruispost = '1')";
  ...
  function printFondsRegel($data)
  {
   ...
   // listarray($data);
    ...
    $this->pdf->Row(array($data['ISINCode'],
                      ...
                      '',
                      ($data['Valuta'] !== 'EUR' ? $this->formatGetal($data['Valutakoers'],4) : ''),
                      '','','','',
                      date('d/m/Y',db2jul($data['Boekdatum']))));
...
These deposits and/of withdrawals are taken into account when caculating investment results. When there are multiple accounts within the contract, a transer is accounted as a withdrawel and booked as a deposit.";
      ...
      $intro = "Le tableau vouw indique les apports et retraits effectués sur votre portefeuille. 
  Ces apports/retraits sond pris en compte dan le calcul de vortre performance. Quand pluseurs comptes de liquidités sont repris dans le contrat, un transfert entre ces deux comptes est comptabilisé comme un retrait et affiché comme un dépôt.";
    } else {
...
De berekening van het beleggingsresultaat wordt gecorrigeerd voor alle stortingen en/of onttrekkingen die u gedurende de rapportageperiode heeft verricht. Stortingen dragen niet bij aan het resultaat, terwijl onttrekkingen niet ten koste van het resultaat gaan. Wanneer er sprake is van meerdere rekeningen binnen het contract wordt een overboeking tussen deze rekeningen getoond als een onttrekking en als storting geboekt.";
    ...
    $this->printLiqKop('Liquiditeiten',$regels['totaal']);
    ...
    if( $this->pdf->rapport_taal == 2 ) {
      ...
      $intro = "Le tableau vouw indique les revenus percus. Il s'agit, par exemple, des paiements de dividendes sur les actions et les fonds de placement, d'intérêts sur obligations individuelles (coupons) et des intérêts servis sur les liquidités. Ces revenus sont versés sur vortre compte et font partie du résultat des placements.";
    } else {
...
//                                           'Kosten'=>$regels['totaalBelcat'][$belcat]['KostenEUR']),true);
      foreach($belcatData as $boekDatum=>$fondsen)
      {
        ...
        {
          if ( ! in_array($regel['Transactietype'], array('A', 'V') ) ) {
