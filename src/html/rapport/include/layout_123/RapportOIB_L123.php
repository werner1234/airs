...
class RapportOIB_L123
{

	function RapportOIB_L123($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    while($categorien = $DB->NextRecord())
    {
      ...
      $categorieGrafiek[$categorien['Omschrijving']]['kleur']=array($this->pdf->triodosDarkBlue[0],$this->pdf->triodosDarkBlue[1],$this->pdf->triodosDarkBlue[2]);//array($allekleuren['OIB'][$categorien[$this->verdeling]]['R']['value'],$allekleuren['OIB'][$categorien[$this->verdeling]]['G']['value'],$allekleuren['OIB'][$categorien[$this->verdeling]]['B']['value']);
    ...
    $poly=array($this->pdf->marge,25,
      ...
      $this->pdf->marge+90,50,
      $this->pdf->marge+90,55,
      $this->pdf->marge,55);
    ...
    if( $this->pdf->rapport_taal == 2 ) {
      ...
      $this->pdf->Row(array(vertaalTekst("Ce tableau illistre l'allocation actuelle de votre portefeuilles par clases d'actifs.", $this->pdf->rapport_taal)));
    } else {
...
Onderaan geven we een grafische voorstelling van de vermogenscategorieën alsook de valutaverdeling van uw portefeuille.", $this->pdf->rapport_taal)));
    ...
    PieChart_L123($this->pdf,$hoogte,$hoogte,$dataRegelsValuta,'%l - %p',array_values($valutaKleuren),vertaalTekst('Valuta verdeling', $this->pdf->rapport_taal),array($this->pdf->getX()+$hoogte+5,$this->pdf->getY()+$yCorrectie+5),true);
    
   // DonutChart($this,$hoogte,$hoogte,$dataRegelsValuta,array_values($valutaKleuren),vertaalTekst('Valuta verdeling', $this->pdf->rapport_taal),array($this->pdf->getX()+$hoogte+5,$this->pdf->getY()+$yCorrectie+5));
  ...
  function VBarDiagram($w, $h, $data, $titel)
  {
    ...
    $minVal=min($minEnMax);
    $numBars = count($data);
    $color=array($this->pdf->triodosDarkBlue[0],$this->pdf->triodosDarkBlue[1],$this->pdf->triodosDarkBlue[2]);
