...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: RapportVOLKD_L51.php,v $
 		Revision 1.2  2018/08/18 12:40:15  rvv
 		php 5.6 & consolidatie
 		
 		Revision 1.1  2017/06/18 09:18:24  rvv
 		*** empty log message ***
 		
 		Revision 1.12  2017/03/29 15:56:14  rvv
 		*** empty log message ***
 		
 		Revision 1.11  2011/06/25 16:51:45  rvv
 		*** empty log message ***
 		
 		Revision 1.10  2009/11/04 16:13:50  rvv
 		*** empty log message ***

 		Revision 1.9  2009/07/12 09:31:17  rvv
 		*** empty log message ***
...
class RapportVOLKD_L51
{
	function RapportVOLKD_L51($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//echo $subquery."<br><br>";exit;
			while($subdata = $DB2->NextRecord())
			{
				...
				else
				{
				  if($this->pdf->rapport_layout == 8) //rvv
					...
					$this->pdf->row(array("",
					  ...
					  {
              $subFondsData = $this->verdiept->getFondsVerdeling($subdata['fonds'],10);
              ...
              foreach ($subFondsData as $fonds)
              {
                ...
                $this->pdf->setX($this->pdf->marge);
                $this->pdf->row(array('','',
                $this->formatAantal($fonds['aantal'],2),
                $fonds['valuta'],
                $this->formatGetal($fonds['fondskoers'],2),
                $this->formatGetal($fonds['valutaWaardeRegel'],$this->pdf->rapport_VOLK_decimaal),
                $this->formatGetal($fonds['waardeRegelEur'],$this->pdf->rapport_VOLK_decimaal),
                "",$this->formatGetal($fonds['portefeuilleDeel']*100,1)." %"));
              }
              $this->pdf->SetTextColor($this->pdf->rapport_fonds_fontcolor[r],$this->pdf->rapport_fonds_fontcolor[g],$this->pdf->rapport_fonds_fontcolor[b]);
  				  }
		...
		if($DB1->records() > 0)
		{
			...
			while($data = $DB1->NextRecord())
			{
				...
				{
					if($this->pdf->rapport_layout == 7 ||$this->pdf->rapport_layout == 14)
					{
						$this->pdf->row(array("",
												"",
												"",
		...
		{
			echo "<b>Fout : Fout in rapport ".$this->portefeuille.", totale waarde (".round($totaalWaarde,2).") komt niet overeen met afgedrukte totaal (".round($actueleWaardePortefeuille,2).") in rapport ".$this->pdf->rapport_type."</b>";
			exit;
		}
