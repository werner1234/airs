...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2019/10/11 17:41:02 $
File Versie					: $Revision: 1.12 $

$Log: RapportVKMS_L83.php,v $
Revision 1.12  2019/10/11 17:41:02  rvv
*** empty log message ***

Revision 1.11  2019/09/04 15:33:34  rvv
*** empty log message ***

Revision 1.10  2019/08/31 13:13:05  rvv
*** empty log message ***

Revision 1.9  2019/06/29 18:24:40  rvv
*** empty log message ***
...
class RapportVKMS_L83
{
	function RapportVKMS_L83($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    if($this->pdfVullen==true)
      ...
      $this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
      $this->pdf->row(array(vertaalTekst('Doorlopende kosten',$this->pdf->rapport_taal), $this->formatGetal($totaalDoorlopendekostenGesplitst['TotCostFund'], 0), $this->formatGetal($totaalDoorlopendekostenGesplitst['TotCostFund']/ $perfTotaal['gemWaarde']*100, 2).' %' ));
      $this->pdf->row(array(vertaalTekst('Transactie kosten',$this->pdf->rapport_taal), $this->formatGetal($totaalDoorlopendekostenGesplitst['FundTransCost'], 0), $this->formatGetal($totaalDoorlopendekostenGesplitst['FundTransCost']/ $perfTotaal['gemWaarde']*100, 2).' %' ));
      $this->pdf->row(array(vertaalTekst('Performance fee',$this->pdf->rapport_taal), $this->formatGetal($totaalDoorlopendekostenGesplitst['FundPerfFee'], 0), $this->formatGetal($totaalDoorlopendekostenGesplitst['FundPerfFee']/ $perfTotaal['gemWaarde']*100, 2).' %' ));
      $this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
	  	$this->pdf->row(array(vertaalTekst('Totaal indirecte fondskosten',$this->pdf->rapport_taal), $this->formatGetal($totaalDoorlopendekosten, 0) ,$this->formatGetal($doorlopendeKostenPercentage * 100, 2).' %'));
      $this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
			$this->pdf->excelData[]=array('','Indirecte fondskosten', '','',round($totaalDoorlopendekosten, 0), 'EUR');
	//		$this->pdf->row(array(vertaalTekst('Indirecte fondskosten ten opzichte van onderliggend vermogen',$this->pdf->rapport_taal), $this->formatGetal($doorlopendeKostenPercentage * 100, 2) . ' %'));
			$this->pdf->excelData[]=array('','Indirecte fondskosten ten opzichte van onderliggend vermogen','','', round($doorlopendeKostenPercentage * 100, 2) ,'%');
			$this->pdf->ln();
			$this->pdf->excelData[]=array();
     // $this->pdf->setWidths(array(108, 30, 30));
		...
		//echo "$herrekendeKosten=$doorlopendeKostenPercentage/$percentage;<br>\n";
		$aandeelIndirect=$perfTotaal['gemWaarde']/$gemiddelde;
		//echo "$aandeelIndirect=".$perfTotaal['gemWaarde']."/$gemiddelde <br>\n";
		$vkmPercentagePortefeuille=$herrekendeKosten*$aandeelIndirect*100;
		//echo "$vkmPercentagePortefeuille=$herrekendeKosten*$aandeelIndirect*100 <br>\n";
		if($this->pdfVullen==true)
		{
		...
		//	$this->pdf->row(array(vertaalTekst("Herrekende indirecte fondskosten",$this->pdf->rapport_taal),'', $this->formatGetal($herrekendeKosten * 100, 2) . ' %'));
		//	$this->pdf->excelData[]=array('',"Herrekende indirecte fondskosten", '','',round($herrekendeKosten * 100, 2),'%');
			$this->pdf->row(array(vertaalTekst('Gemiddeld vermogen in fondsen met beschikbare kosten',$this->pdf->rapport_taal),$this->formatGetal($perfTotaal['gemWaarde'], 0), $this->formatGetal($aandeelIndirect * 100, 2) . ' %'));
			$this->pdf->excelData[]=array('','Gemiddeld vermogen in fondsen','','',round($aandeelIndirect * 100, 2),'%');
			$this->pdf->ln();
			$this->pdf->excelData[]=array();
			$this->pdf->row(array(vertaalTekst('Totaal gemiddeld vermogen',$this->pdf->rapport_taal), $this->formatGetal($gemiddelde, 0),'100,00 %'));
			$this->pdf->excelData[]=array('','Totaal gemiddeld vermogen','','',round($gemiddelde,0),'EUR');
			$this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
			$this->pdf->row(array(vertaalTekst('Indirecte fondskosten factor van de portefeuille',$this->pdf->rapport_taal), '',$this->formatGetal($vkmPercentagePortefeuille, 2).' %'));
			$barData=array();
			$barData['Indirecte fondskosten']=$vkmPercentagePortefeuille;
			$this->pdf->excelData[]=array('','Indirecte fondskosten factor van de portefeuille','','',round($vkmPercentagePortefeuille, 2),'%');
			$this->pdf->ln();
			$this->pdf->excelData[]=array();
    ...
    $grootboekKosten=array();
		while($data = $DB->nextRecord())
		{
          ...
          $data['Omschrijving']=$data['Omschrijving'].' (incl. BTW)';
    ...
    $this->vkmWaarde=array('vkmPercentagePortefeuille'=>$vkmPercentagePortefeuille,'kostenPercentage'=>$kostenPercentage,'vkmWaarde'=>$vkmWaarde,'grootboekKosten'=>$grootboekKosten,
      ...
      'fondsGemiddeldeWaarde'=>$perfTotaal['gemWaarde']);
...
" $rekeningFondsenWhere ";
	...
	function writeRapport()
  {
    ...
    $gewensteKleuren['Indirecte fondskosten']=$gewensteKleuren['Indirecte (fonds)kosten'];
...
ORDER BY HoofdcategorienVolgorde.Afdrukvolgorde, HoofdBeleggingscategorien.Afdrukvolgorde,BeleggingscategorienVolgorde.Afdrukvolgorde, BeleggingscategorienVolgorde.Afdrukvolgorde, Beleggingscategorien.Afdrukvolgorde,fondsVolgorde,OptieBovenliggendFonds,FondsOmschrijving ";
    ...
    foreach ($perCategorie as $hoofdCategorie => $regioData)
    {
      ...
      {
        $perCategorie[$hoofdCategorie][$categorie]['perf'] = $this->fondsPerformance($categorieData);
      }
    } //[$regio]
    ...
    {
      $grootboekKleuren['Indirecte fondskosten'] = $mogelijkeKleuren[$n];
    }
    $grootboekKleuren['Beheervergoeding (incl. BTW)']=$grootboekKleuren['Beheervergoeding'];
    ...
    $gebruikteCrmVelden=array('OPALVerwachtRendement');
    $query = "DESC CRM_naw";
    $db->SQL($query);
    $db->Query();
    $crmVelden=array();
    ...
    foreach($gebruikteCrmVelden as $veld)
    {
      if(in_array(strtolower($veld),$crmVelden))
      {
		...
		$query="SELECT CRM_naw.doeldatum $nawSelect FROM CRM_naw WHERE portefeuille='".$this->portefeuille."'";
		$db->SQL($query);
		$db->query();
		$data=$db->nextRecord();
		...
		if($data['OPALVerwachtRendement']<>0)
    {
      $rendement=$data['OPALVerwachtRendement'];
    }
    else
    {
 ...
 WHERE Portefeuilles.portefeuille='".$this->portefeuille."'";
    ...
    for($i=$this->pdf->rapport_jaar; $i<=$doelJaar; $i++)
		...
		$waardeZonderKostenKleur=array(55,96,145);
		$waardeNaKostenKleur=array(125,160,205);
		$cumulatieveKostenKleur=array(160,185,220);
...
//		echo "$doelJaar $rendement $beginwaarde";
    ...
    $bereik=abs($maxVal - $minVal);
    
    $minVal = floor(($minVal-1) -$bereik*0.1);
    $maxVal = ceil(($maxVal+1) +$bereik*0.1);
    
	//	echo $maxVal;exit;
    $tekens=strlen(round($minVal));
		
		//if($minVal > 0)
		$minVal=floor($minVal/pow(10,$tekens-1))*pow(10,$tekens-1);
    ...
    $bereik=abs($maxVal - $minVal);
		$stapgrootte = ceil($bereik/$horDiv);
		$unith = $hDiag / (-1 * $bereik);

		$top = $YPage;
		$bodem = $YDiag+$hDiag;
		$absUnit =abs($unith);

		//$nulpunt = $YDiag;// + ($minVal * $stapgrootte);
		
		$n=0;
		for($i=$YDiag; $i<= $bodem; $i+= $absUnit*$stapgrootte)
		{

			$skipNull = true;
			$this->pdf->Line($XDiag, $i, $XPage+$w ,$i,array('dash' => 1,'color'=>array(0,0,0)));
			$this->pdf->setXY($XDiag-7, $i);
			$this->pdf->Cell(7 , 4 , $this->formatGetal( ($n*$stapgrootte*-1+$maxVal),0) , 0, 1, "R");
		...
		$n=0;
		for($i=$YDiag; $i >= $top; $i-= $absUnit*$stapgrootte)
		{
      if($i>$bodem)
        continue;
			...
			{
				$this->pdf->setXY($XDiag-7, $i);
				$this->pdf->Cell(7 , 4 , $this->formatGetal(($n * $stapgrootte) + $minVal,0) , 0, 1, "R");
    ...
    for ($i=0; $i<count($data); $i++)
    {
      ...
      $polly[]=$XDiag+($i+1)*$unit;
      $polly[]=$yval2;
      $yval = $yval2;
    }
    $pollyReverse=array();
    $yval=$YDiag + (($maxVal) * $waardeCorrectie) ;
    $aantalWaarden=count($data1);
    for ($i=0; $i<=count($data1); $i++)
    {
      ...
      if($i>-1)
      {
        ...
        {
          $pollyReverse[] = $yval2;
          $pollyReverse[] = $XDiag + ($i + 1) * $unit;
        }
        if($i>0)
        {
          $pollyReverse[] = $yval;
          $pollyReverse[] = $XDiag + $i * $unit;
        }
      }
      $yval = $yval2;
    }
    ...
    $this->pdf->Polygon($polly, 'F', null, $color2) ;
