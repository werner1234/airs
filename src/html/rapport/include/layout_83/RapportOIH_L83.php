...
/*
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2020/04/11 16:25:49 $
 		File Versie					: $Revision: 1.24 $

 		$Log: RapportOIH_L83.php,v $
 		Revision 1.24  2020/04/11 16:25:49  rvv
 		*** empty log message ***
 		
 		Revision 1.23  2020/04/09 12:38:24  rvv
 		*** empty log message ***
 		
 		Revision 1.22  2020/04/08 15:43:41  rvv
 		*** empty log message ***
 		
 		Revision 1.21  2020/04/05 18:27:28  rvv
 		*** empty log message ***
 		
 		Revision 1.20  2019/10/16 15:24:48  rvv
 		*** empty log message ***
 		
 		Revision 1.19  2019/10/11 17:41:02  rvv
 		*** empty log message ***
 		
 		Revision 1.18  2019/10/02 15:20:29  rvv
 		*** empty log message ***
 		
 		Revision 1.17  2019/09/28 17:22:14  rvv
 		*** empty log message ***
 		
 		Revision 1.16  2019/09/18 14:53:53  rvv
 		*** empty log message ***
 		
 		Revision 1.15  2019/09/14 17:10:27  rvv
 		*** empty log message ***
 		
 		Revision 1.14  2019/09/04 15:33:34  rvv
 		*** empty log message ***
 		
 		Revision 1.13  2019/09/01 12:04:35  rvv
 		*** empty log message ***
 		
 		Revision 1.12  2019/08/21 15:34:07  rvv
 		*** empty log message ***
 		
 		Revision 1.11  2019/08/17 18:20:11  rvv
 		*** empty log message ***
 		
 		Revision 1.10  2019/07/10 15:49:24  rvv
 		*** empty log message ***
 		
 		Revision 1.9  2019/07/10 11:25:06  rvv
 		*** empty log message ***
 		
 		Revision 1.8  2019/07/10 09:10:11  rvv
 		*** empty log message ***
 		
 		Revision 1.7  2019/07/10 07:11:30  rvv
 		*** empty log message ***
 		
 		Revision 1.6  2019/07/10 07:02:25  rvv
 		*** empty log message ***
...
include_once($__appvar["basedir"]."/html/rapport/include/layout_83/PDFRapport_headers_L83.php");

class RapportOIH_L83
{
	function RapportOIH_L83($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	...
	function writeRapport()
  {
    ...
    $fondsenPerPortefeuilleOpDatum=array();
    foreach ($this->portefeuilles as $portefeuille)
    {
      $mutatieLijstPerPortefeuille[$portefeuille]=$this->genereerMutatieLijst($portefeuille,$this->rapportageDatumVanaf,$this->rapportageDatum);
        ...
        {
          if($fondsRegel['fonds'] <> '')
            $fondsenPerPortefeuilleOpDatum[$portefeuille][$datum][$fondsRegel['fonds']] = $fondsRegel;
        }
      }
    }
  
    $veldenOnderstrukken=array('totaalAantal','historischeWaarde','actueleFonds','beginPortefeuilleWaardeInValuta','beginPortefeuilleWaardeEuro','actuelePortefeuilleWaardeInValuta','actuelePortefeuilleWaardeEuro');
    foreach($fondsenPerPortefeuilleOpDatum as $portefeuille=>$fondsenOpDatum)
    {
      foreach($fondsenOpDatum as $datum=>$fondsen)
      {
        if($datum==$this->rapportageDatumVanaf)
        {
          foreach($fondsen as $fonds=>$fondsDetailsStart)
          {
            if(!isset($fondsenPerPortefeuilleOpDatum[$portefeuille][$this->rapportageDatum][$fonds]))
            {
              $tmp=$fondsDetailsStart;
              foreach($veldenOnderstrukken as $veld)
                $tmp[$veld]=0;
              $fondsRegelsPerPortefeuille[$portefeuille][$this->rapportageDatum][]=$tmp;
            }
          }
        }
      }
          ...
          foreach($fondsRegels as $regel)
          {
          	if($regel['type']=='rekening')
            {
              ...
              if($datum==$this->rapportageDatum)
              {
                ...
                if($regel['valuta']<>'EUR')
                {
                  $st = getRekeningStortingenKruis($regel['rekening'], $this->rapportageDatumVanaf, $this->rapportageDatum, 'EUR', true,true);
                  $on = getRekeningOnttrekkingenKruis($regel['rekening'], $this->rapportageDatumVanaf, $this->rapportageDatum, 'EUR', true,true);
                  if ($_POST['debug'])
                  {
		  		...
		  		foreach($fondsRegels as $regel)
          {
            ...
            if($regel['beleggingscategorie']=='EFFECT' || $regel['beleggingscategorie']=='AAND')
            {
              ...
              else
              {
                ...
                if(checkVLC_Fondsomschrijving()==true && $regel['fonds']<>'')
                {
                  $query="SELECT FondsExtraInformatie.VLC_Fondsomschrijving FROM FondsExtraInformatie WHERE FondsExtraInformatie.Fonds='".mysql_real_escape_string($regel['fonds'])."'";
                  $db->SQL($query);
                  $tenaamstelling=$db->lookupRecord();
                  if($tenaamstelling['VLC_Fondsomschrijving']<>'')
                    $regel['fondsOmschrijving']=$tenaamstelling['VLC_Fondsomschrijving'];
                }
   ...
   //listarray($beleggingscategorieen);exit;
    $this->pdf->rapport_header1 = array("Categorie\n    Beleggingen\n ","Waarde\nin ".$this->pdf->rapportageValuta."\n".date("d-m-y",$this->pdf->rapport_datumvanaf),
      "Stortingen/\n onttrekkingen\n ","Resultaat\nverslag-\nperiode","Waarde\nin ".$this->pdf->rapportageValuta."\n".date("d-m-y",$this->pdf->rapport_datum),"in %\n\n ",
	     ...
	     foreach($waarden[$categorie] as $portefeuille=>$regels)
          ...
          //   " | ".$regelWaarden[$this->rapportageDatum]['actuelePortefeuilleWaardeEuro']." - ".$regelWaarden[$this->rapportageDatumVanaf]['actuelePortefeuilleWaardeEuro']." - ".$regelWaarden['stortingenOnttrekkingen']."<br>\n";
            ...
            $row=array($regelWaarden['fondsOmschrijving'],
              ...
              $this->formatGetal($regelWaarden['performance'],1),
