...
Revision 1.4  2019/05/25 16:22:55  rvv
*** empty log message ***

Revision 1.3  2019/05/18 16:28:56  rvv
*** empty log message ***

Revision 1.2  2019/05/15 18:06:34  rvv
*** empty log message ***

Revision 1.1  2019/05/15 15:31:34  rvv
*** empty log message ***

Revision 1.9  2018/08/18 12:40:14  rvv
php 5.6 & consolidatie

Revision 1.8  2018/02/21 17:15:09  rvv
*** empty log message ***

Revision 1.7  2018/01/05 11:57:31  rvv
*** empty log message ***

Revision 1.6  2017/12/27 18:29:09  rvv
*** empty log message ***

Revision 1.5  2016/11/30 16:48:42  rvv
*** empty log message ***

Revision 1.4  2015/05/10 13:37:43  rvv
*** empty log message ***

Revision 1.3  2015/05/10 08:02:25  rvv
*** empty log message ***

Revision 1.2  2015/05/06 15:35:03  rvv
*** empty log message ***
...
class RapportATT_L83
{

	function RapportATT_L83($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    $this->att=new ATTberekening_L83($this);
...
function addResultaat()
{
 ...
 // listarray($this->pdf->portefeuilles);
  ...
  $resultaatTotaal=array("","");//vertaalTekst("Subtotaal",$this->pdf->rapport_taal));
...
$rente=array("",vertaalTekst("Resultaat opgelopen rente",$this->pdf->rapport_taal));//
  $renteTotaal=array("","");//vertaalTekst("Subtotaal",$this->pdf->rapport_taal));//
...
foreach($categorien as $categorie=>$categorieOmschrijving)
{
  ...
  foreach($categorien as $categorie=>$categorieOmschrijving)
  {
    ...
    else
    {
      ...
      {
       // $effectenmutaties[]=$this->formatGetal(($perfWaarden['stort']*-1)-$perfWaarden['resultaat']*3-$perfWaarden['kosten'],0);//
        $effectenmutaties[]=$this->formatGetal(($perfWaarden['stort']+$perfWaarden['resultaat'])*-1,0);//
      }
      else
      ...
      if($categorie=='Liquiditeiten_uit')
      {
        $opbrengsten[]=$this->formatGetal($perfWaarden['resultaatBruto']-$perfWaarden['kosten']-$perfWaarden['extraKosten'],0);
      }
      else
      {
     ...
     // listarray($perfWaarden);
      $opbrengsten[]='';
      $kosten[]=$this->formatGetal(($perfWaarden['kosten']+$perfWaarden['extraKosten'])*-1,0);
      $kosten[]='';
    ...
    if($categorie == 'totaal')
    {
      ...
      $opbrengst=$perfWaarden['ongerealiseerdFondsResultaat']+$perfWaarden['ongerealiseerdValutaResultaat']+
        $perfWaarden['gerealiseerdFondsResultaat']+$perfWaarden['gerealiseerdValutaResultaat']+
        $perfWaarden['resultaatValuta'];
      $resultaatTotaal[]=$this->formatGetal($opbrengst,0);
  
      $totaalOpbrengstEUR-=$opbrengst;
      $totaalOpbrengstEUR-=$perfWaarden['opgelopenrente'];
      $resultaatTotaal[]='';
      
      $rente[]=$this->formatGetal($perfWaarden['opgelopenrente'],0);
      $rente[]='';
      $renteTotaal[]=$this->formatGetal($perfWaarden['opgelopenrente'],0);
      $renteTotaal[]='';
...
//    $this->pdf->SetTextColor(255,255,255);
    $yBeforeHeader=$this->pdf->getY();
  $this->pdf->SetTextColor(255,255,255);
  $this->pdf->row($header);
  $this->pdf->SetTextColor($this->pdf->rapport_fontcolor['r'],$this->pdf->rapport_fontcolor['g'],$this->pdf->rapport_fontcolor['b']);
  
  $yAfterHeader=$this->pdf->getY();

    if(round($yAfterHeader-$yBeforeHeader) < 8)
      $this->pdf->ln();
  
    $this->pdf->Line($this->pdf->marge,$this->pdf->GetY(),$this->pdf->marge + array_sum($this->pdf->widthB)-5.5,$this->pdf->GetY());
