...
/*
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2019/06/19 15:59:42 $
 		File Versie					: $Revision: 1.6 $

 		$Log: ATTberekening_L83.php,v $
 		Revision 1.6  2019/06/19 15:59:42  rvv
 		*** empty log message ***
 		
 		Revision 1.5  2019/06/15 20:53:56  rvv
 		*** empty log message ***
 		
 		Revision 1.4  2019/05/29 15:45:42  rvv
 		*** empty log message ***
 		
 		Revision 1.3  2019/05/25 16:22:55  rvv
 		*** empty log message ***
 		
 		Revision 1.2  2019/05/15 15:31:34  rvv
 		*** empty log message ***
...
class ATTberekening_L83
{
  
  function ATTberekening_L83($rapportData)
  {
  ...
  function bereken($van,$tot,$verdeling='attributie')
  {
    global $__appvar;
    $DB=new DB();
  ...
  function fondsPerformance($fondsData,$van,$tot,$stapeling=false,$categorie='')
  {
    ...
    foreach ($perioden as $periode)
    {
      foreach ($periode as $rapDatum)
      {
        ...
        {
          $fondswaarden =  berekenPortefeuilleWaarde($this->rapport->portefeuille, $rapDatum,$startJaar,'EUR',$rapDatum);
...
//      }
      ...
      if($categorie=='totaal')
      {
        ...
        $performanceBruto = ((($totaalEindwaarde - $totaalBeginwaarde) - $storting['totaal'] - $nietToegerekendeKosten['kostenTotaal']) / $this->totaalGemiddelde);
       // echo $categorie." |$performanceBruto| = ((($totaalEindwaarde - $totaalBeginwaarde) - ".$storting['totaal']." - ".$nietToegerekendeKosten['kostenTotaal'].") / ".$this->totaalGemiddelde.")<br>\n";
      ...
      //  echo $categorie." $datumEind  |$performance| = ((($eindwaarde - $beginwaarde) - ".$storting['totaal'].") / $this->totaalGemiddelde)<br>\n";ob_flush();
        ...
        $resultaatBruto=$resultaat - $nietToegerekendeKosten['kostenTotaal'];
     
        $bijdrage = ($resultaat - $nietToegerekendeKosten['kostenTotaal']) / $gemiddelde * $weging;
        //echo "$datumEind Bijdrage $categorie |$bijdrage| = $resultaat - ".$nietToegerekendeKosten['kostenTotaal']." / $gemiddelde * $weging; <br>\n";
      }
      else
      {
       ...
       SUM(if(Grootboekrekeningen.Grootboekrekening ='RENME', (ABS(Rekeningmutaties.Credit) * Rekeningmutaties.Valutakoers ) - (ABS(Rekeningmutaties.Debet) * Rekeningmutaties.Valutakoers ),0)) as RENMETotaal
        ...
        WHERE
         ...
         // echo "$categorie | $eindAandeel = ($eindwaarde/$totaalEindwaarde) | ".$nietToegerekendeKosten['kostenTotaal']."<br>\n";
          ...
          {
          //  echo "$eindAandeel ";
         //   listarray($nietToegerekendeKosten);exit;
            $AttributieStortingenOntrekkingen['totaal']+= $nietToegerekendeKosten['kostenTotaal']*(1-$eindAandeel);
            $FondsDirecteKostenOpbrengsten['opbrengstTotaal']-= $nietToegerekendeKosten['kostenTotaal'];
     ...
     // listarray($FondsDirecteKostenOpbrengsten);listarray($RekeningDirecteKostenOpbrengsten);listarray($nietToegerekendeKostenExtra);
        
        $gemiddelde = $beginwaarde - $AttributieStortingenOntrekkingen['gewogen'];
  
        $resultaaBruto=(($eindwaarde - $beginwaarde) - $stort + $opbr );
       
        $resultaat=(($eindwaarde - $beginwaarde) - $stort + $opbr + $kost);
       // echo "$categorie | $resultaat=(($eindwaarde - $beginwaarde) - $stort + $opbr + $kost); <br>\n";
        $performanceBruto = $resultaaBruto / $gemiddelde;
        $performance      = $resultaat / $gemiddelde;
  
        $weging = $gemiddelde / $this->totaalGemiddelde;//$this->totalen['gemiddeldeWaarde'];
        $aandeelOpTotaal = $eindwaarde / $totaalEindwaarde;
        $bijdrage = $resultaaBruto / $gemiddelde * $weging;
       // if($bijdrage<>0)
        //  echo "$datumEind | $categorie | $bijdrage | = $resultaaBruto / $gemiddelde * |$weging|  <br>\n";
    //   echo $categorie." $datumEind b: $performanceBruto = (($eindwaarde - $beginwaarde) +  $stort + $opbr ) / $gemiddelde)<br>\n";
     //  echo $categorie." $datumEind n: $performance = (($eindwaarde - $beginwaarde) +  $stort + $opbr + $kost ) / $gemiddelde)<br>\n";
        //listarray($AttributieStortingenOntrekkingenBruto);
// listarray($AttributieStortingenOntrekkingen);
      ...
      $aandeelEindwaarde=$this->totalen[$datumEind]['WaardeEur'][$categorie]/ $this->totalen[$datumEind]['totaalWaardeEur'];
      ...
      $waarden[$datumEind]=array('categorie'=>$categorie,
    ...
    $avgItems=array('weging','gemWaarde','wegingEind');
    ...
    foreach ($stapelItems as $item)
    {
      ...
      //echo $categorie." $item ".$perfData['totaal'][$item] ."<br>\n";
    }
    $perfData['totaal']['categorie']=$waarde['categorie'];
