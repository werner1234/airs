...
$Log: RapportMUT_L83.php,v $
Revision 1.6  2020/03/18 17:47:30  rvv
*** empty log message ***

Revision 1.5  2019/10/05 17:38:05  rvv
*** empty log message ***

Revision 1.4  2019/05/11 16:49:13  rvv
*** empty log message ***

Revision 1.3  2019/04/24 14:42:25  rvv
*** empty log message ***
...
class RapportMUT_L83
{
	function RapportMUT_L83($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
		$this->pdf->rapport_type = "MUT";
		$this->pdf->rapport_datum = db2jul($rapportageDatum);
		$this->pdf->rapport_datumvanaf = db2jul($rapportageDatumVanaf);
  		$this->pdf->rapport_titel = vertaalTekst("Mutatie-overzicht in",$this->pdf->rapport_taal).' '. vertaalTekst($this->pdf->valutaOmschrijvingen[$this->pdf->portefeuilledata['RapportageValuta']],$this->pdf->rapport_taal);
...
//		$this->pdf->Line($totaal2+2,$this->pdf->GetY(),$totaal2 + $this->pdf->widthA[7],$this->pdf->GetY());
	...
	function printKop($title, $type="default")
	{
		...
		if($this->pdf->getY()+8>$this->pdf->PageBreakTrigger)
    {
      $this->pdf->addPage();
    }
	...
	function writeRapport()
	{
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
		...
		$DB = new DB();
		// voor data
		$this->pdf->widthA = array(20,15,90,25,40,20,25,25,25);
    ...
    $this->pdf->templateVarsOmschrijving[$this->pdf->rapport_type.'Paginas']=$this->pdf->rapport_titel;
	   ...
	   $extraquery .= "AND Rekeningmutaties.Grootboekrekening IN('$grootboekSelectie')  ";
        ...
        $mutaties['gbOmschrijving']='Banking fee';
			...
			{
				$this->printKop(vertaalTekst($mutaties['gbOmschrijving'],$this->pdf->rapport_taal), 'b');
				$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
			}
