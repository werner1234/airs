...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2020/07/11 17:36:10 $
File Versie					: $Revision: 1.9 $

$Log: RapportTRANS_L83.php,v $
Revision 1.9  2020/07/11 17:36:10  rvv
*** empty log message ***

Revision 1.8  2019/10/09 15:04:12  rvv
*** empty log message ***

Revision 1.7  2019/07/20 16:30:18  rvv
*** empty log message ***

Revision 1.6  2019/06/29 18:24:40  rvv
*** empty log message ***

Revision 1.5  2019/06/08 16:05:23  rvv
*** empty log message ***

Revision 1.4  2019/05/15 15:31:34  rvv
*** empty log message ***

Revision 1.3  2019/05/11 16:49:13  rvv
*** empty log message ***

Revision 1.2  2019/05/04 18:23:53  rvv
*** empty log message ***

Revision 1.1  2019/04/27 18:27:04  rvv
*** empty log message ***

Revision 1.46  2018/08/18 12:40:14  rvv
...
class RapportTRANS_L83
{
	function RapportTRANS_L83($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	...
	function writeRapport()
	{
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
		...
		{
		// voor data
		$this->pdf->widthB = array(15,12,14,67,16,21,21,16,21,21,20,1,24,12);
    ...
    {
      $join="LEFT JOIN FondsExtraInformatie ON Rekeningmutaties.Fonds=FondsExtraInformatie.Fonds";
      $selectFondsOmschrijving="if(isnull(FondsExtraInformatie.VLC_Fondsomschrijving),Fondsen.Omschrijving,FondsExtraInformatie.VLC_Fondsomschrijving) as Omschrijving,";
    }
    else
    {
      $join='';
      $selectFondsOmschrijving='Fondsen.Omschrijving,';
    }
    
		// loopje over Grootboekrekeningen Opbrengsten = 1
		$query = "SELECT $selectFondsOmschrijving ".
...
//	echo "lopende -> ".$resultaatlopende." <-  voorgaande ".$resultaatvoorgaande. " -  <br>" ;
        ...
        $result_totaal = $this->formatGetal($resultaatvoorgaande+$resultaatlopende,0);
      ...
      //listarray($mutaties);    listarray($historischekostprijs);
			// % van totaal
			if($this->pdf->rapport_TRANS_procent == 1)
			{
				//$percentageTotaal = ABS(($t_verkoop_waarde-$historischekostprijs) / ( $historischekostprijs) *100);
        $percentageTotaal = ABS(($resultaatvoorgaande+$resultaatlopende) / ( $historischekostprijs) *100);
//echo "$t_verkoop_waarde $percentageTotaal = ABS(($resultaatlopende + $resultaatvoorgaande / ($resultaatvoorgaande + $historischekostprijs)) *100); <br>\n";ob_flush();
				...
				if($percentageTotaal <>0)
				{
					if($percentageTotaal > 1000 || $percentageTotaal < -1000)
						...
						$percentageTotaalTekst = $this->formatGetal($percentageTotaal,2);
...
*/
    ...
    $this->pdf->row(array("",
                      ...
                      vertaalTekst("Totalen",$this->pdf->rapport_taal),
								...
								"",
								"",
								$this->formatGetal($totaal_resultaat_waarde,$this->pdf->rapport_TRANS_decimaal)));
