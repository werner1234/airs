...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2019/02/07 07:19:42 $
File Versie					: $Revision: 1.6 $

$Log: RapportTRANSFEE_L56.php,v $
Revision 1.6  2019/02/07 07:19:42  rvv
*** empty log message ***
...
Revision 1.3  2018/02/07 17:17:21  rvv
*** empty log message ***

Revision 1.2  2018/02/04 15:47:35  rvv
*** empty log message ***
...
class RapportTRANSFEE_L56
{
	function RapportTRANSFEE_L56($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    if($this->pdfVullen==true)
		{
	  	...
	  	$this->pdf->row(array('Doorlopende kosten', $this->formatGetal($totaalDoorlopendekosten, 0) . ' '.$this->pdf->rapportageValuta ));
			$this->pdf->excelData[]=array('','Doorlopende kosten', '','',round($totaalDoorlopendekosten, 0), $this->pdf->rapportageValuta );
		...
		if($this->pdfVullen==true)
		{
			...
			$this->pdf->ln();
			$this->pdf->excelData[]=array();
			$this->pdf->row(array('Gemiddeld vermogen', $this->formatGetal($gemiddelde, 0) . ' '.$this->pdf->rapportageValuta ));
    ...
    if($spreadKostenEUR <> 0)
		...
		if($this->pdfVullen==true)
		{
			...
			$this->pdf->excelData[]=array('','Totaal directe kosten','', round($totaal/$this->pdf->ValutaKoersEind, 0), round($kostenPercentage, 2));
			$this->pdf->ln();
			$this->pdf->excelData[]=array();
