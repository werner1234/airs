...
/*
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2019/08/03 17:04:39 $
 		File Versie					: $Revision: 1.3 $

 		$Log: RapportCASHY_L81.php,v $
 		Revision 1.3  2019/08/03 17:04:39  rvv
 		*** empty log message ***
...
class RapportCASHY_L81
{
	function RapportCASHY_L81($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	 //
		$this->pdf = &$pdf;
...
$maanden=array(0,'jan','feb','mrt','apr','mei','jun','jul','aug','sep','okt','nov','dec');
    $cashflowHuidigjaar=array();
      ...
      $jaarXls=substr($regel['0'],6,4);
 		  if($jaar > ($rapJaar+13))
	      $jaar='Overig';
      if($jaarXls > ($rapJaar+30))
        $jaarXls='Overig';
      ...
      $cashflowJaarXls[$jaarXls]['lossing'] +=0;
      $cashflowJaarXls[$jaarXls]['rente'] +=0;
      $cashflowJaarXls[$jaarXls][$regel[2]] +=$regel[3];
    ...
    $header=array('',vertaalTekst('maand',$this->pdf->rapport_taal),vertaalTekst('lossing',$this->pdf->rapport_taal),vertaalTekst('coupon',$this->pdf->rapport_taal),vertaalTekst('totaal',$this->pdf->rapport_taal));
    $headerXls=array(vertaalTekst('maand',$this->pdf->rapport_taal),vertaalTekst('lossing',$this->pdf->rapport_taal),vertaalTekst('coupon',$this->pdf->rapport_taal),vertaalTekst('totaal',$this->pdf->rapport_taal));
    $this->pdf->Row($header);
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
    ...
    $totalen=array();
       ...
       $this->pdf->excelData[]=array(vertaalTekst($maand,$this->pdf->rapport_taal),round($waarden['lossing'],0),round($waarden['rente'],0),round($waarden['lossing']+$waarden['rente'],0));
    ...
    $this->pdf->excelData[]=array(vertaalTekst('Totaal',$this->pdf->rapport_taal),round($totalen['lossing'],0),round($totalen['rente'],0),round($totalen['lossing']+$totalen['rente'],0));
    ...
    $this->pdf->excelData[]=array(vertaalTekst('jaar',$this->pdf->rapport_taal),vertaalTekst('lossing',$this->pdf->rapport_taal),vertaalTekst('coupon',$this->pdf->rapport_taal),vertaalTekst('totaal',$this->pdf->rapport_taal));
		$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
    $totalen=array();
    ...
    foreach ($cashflowJaarXls as $jaar=>$waarden)
    {
      $this->pdf->excelData[]=array($jaar,round($waarden['lossing'],0),round($waarden['rente'],0),round($waarden['lossing']+$waarden['rente'],0));
    }
...
$this->pdf->CellBorders = array();
	...
	function VBarDiagram($w, $h, $data,$titel)
  {
      ...
      $colors=array('rente'=>array(167,122,76),
      'lossing'=>array($this->pdf->rapport_kop_bgcolor['r'],$this->pdf->rapport_kop_bgcolor['g'],$this->pdf->rapport_kop_bgcolor['b']));
