...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2020/05/30 15:31:41 $
File Versie					: $Revision: 1.5 $

$Log: RapportVOLK_L81.php,v $
Revision 1.5  2020/05/30 15:31:41  rvv
*** empty log message ***

Revision 1.4  2019/02/16 19:37:40  rvv
*** empty log message ***

Revision 1.3  2019/01/02 16:26:48  rvv
*** empty log message ***
...
class RapportVOLK_L81
{
	function RapportVOLK_L81($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    $this->pdf->excelData[]=array("Categorie",'Valuta','Fondsomschrijving','Aantal','Koers','Waarde','Waarde EUR','Koers','Waarde','Waarde EUR','in %','Fondsresultaat','Valutaresultaat','Direct resultaat','in %');
  ...
  function getDividend($fonds)
  {
	...
	function writeRapport()
	{
		...
		$this->pdf->widthB = array(5,55,18,18,20,22,1,17,21,22,18,18,18,18,12);
		$this->pdf->widthA = array(  60,18,18,20,22,1,17,21,22,18,18,18,18,12);
    ...
    $this->pdf->templateVars['VOLKPaginas']=$this->pdf->page;
		...
		while($categorien = $DB->NextRecord())
		{
           ...
           $procentResultaat = (($totaalactueel - $totaalbegin + $totaaldividendCorrected) / ($totaalbegin /100));
			...
			$subquery = "SELECT TijdelijkeRapportage.fondsOmschrijving, ".
				...
				.$__appvar['TijdelijkeRapportageMaakUniek'].
				" ORDER BY TijdelijkeRapportage.fondspaar, TijdelijkeRapportage.fondsOmschrijving asc";//exit;
      ...
      $kopPrinted=false;
        ...
        $this->pdf->excelData[]=array($categorien['Omschrijving'],$subdata['Valuta'],$subdata['fondsOmschrijving'],round($subdata['totaalAantal'],4),
...
//rvv
    ...
    " TijdelijkeRapportage.beleggingscategorie =  '".$categorien['beleggingscategorie']."' AND ".
      ...
      if($totaalRenteInValuta <> 0)
      {
        ...
        $this->pdf->excelData[]=array($categorien['Omschrijving'],$categorien['valuta'],'Opgelopen Rente',
          "",
          "",
          "",
          "",
          ...
          round($subtotaalPercentageVanTotaal,1));
...
//rvv
    ...
    if($totaalactueel<>0)
		...
		if($DB1->records() > 0)
		{
		  ...
		  if($DB1->records()==1 && ($DB1->records()+1)*$this->pdf->rowHeight+$this->pdf->GetY() > $this->pdf->PageBreakTrigger )
        $this->pdf->AddPage();
