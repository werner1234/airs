...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2019/02/06 07:34:23 $
File Versie					: $Revision: 1.8 $

$Log: RapportVKMS_L81.php,v $
...
class RapportVKMS_L81
{
	function RapportVKMS_L81($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    ...
    $kostenData=array('BEH'=>array('Grootboekrekening'=>'BEH','Omschrijving'=>'Beheer- of adviesvergoeding (incl. administratie- en toezichtskosten)'),
                      ...
                      'BEW'=>array('Grootboekrekening'=>'BEW','Omschrijving'=>'Kosten depotbank (administratie, bewaring, servicekosten, eventueel transactiekosten)'));
...
//				$this->pdf->excelData[]=array('',$data['Omschrijving'],'',round($data['totaal'],0),round($kostenProcent,2) );
    ...
    if($this->pdfVullen==true)
    {
      ...
      $this->pdf->row(array('Wij verstrekken u ten minste jaarlijks een overzicht van de totale directe en indirecte kosten waar u mee te maken heeft. De verschillende kosten die worden onderscheiden zijn:
- Directe kosten: de kosten van onze dienstverlening, inclusief de kosten van de depotbank voor het aanhouden van uw beleggingsrekening.
...
In het onderstaande overzicht zijn de werkelijke directe en indirecte kosten met betrekking tot uw beleggingsportefeuille opgenomen over de laatste 12 maanden. Door middel van twee illustraties is weergegeven wat de impact is van de kosten op het rendement van de afgelopen 12 maanden en wat de impact is van de verwachte kosten op het verwachte rendement van de portefeuille op lange termijn. Indien u meer informatie wenst over de specificatie van de te verwachten kosten, kunt u uiteraard contact met ons opnemen.'));
    ...
    $this->pdf->excelData[]=array();
      ...
      $this->pdf->row(array('Verwacht bruto rendement per jaar', $this->formatGetal($risicoklasse['verwachtRendement'], 2) . ' %' ));
      $this->pdf->row(array('Gemiddeld vermogen', $this->formatGetal($gemiddelde, 0)));
      $this->pdf->ln();
  
      $this->pdf->setWidths(array(130,5,15,15));
      $this->pdf->SetFont($this->pdf->rapport_font, 'B', $fontsize);
      $this->pdf->row(array('Direct: werkelijke kosten van onze dienstverlening', '', ''));
      ...
      $this->pdf->SetFont($this->pdf->rapport_font, 'B', $fontsize);
      $this->pdf->row(array('Indirect: werkelijke kosten van financiële instrumenten in portefeuille (o.a. beleggingsfondsen)'));
      $this->pdf->SetFont($this->pdf->rapport_font, '', $fontsize);
      $this->pdf->row(array('Eenmalig kosten (in- en uitstapkosten)', '€',$this->formatGetal(0, false,false,true) , $this->formatGetal(0, 2,true,true)));
      $kostenSom=array_sum($totaalDoorlopendekostenGesplitst);
      $aandeel=($totaalDoorlopendekostenGesplitst['TotCostFund'])/$kostenSom;
      $this->pdf->row(array('Doorlopend (lopende kosten factor)','€',$this->formatGetal($vkmPercentagePortefeuille*$aandeel*$gemiddelde/100, 0), $this->formatGetal($vkmPercentagePortefeuille*$aandeel, 2,true,true)));
      $aandeel=$totaalDoorlopendekostenGesplitst['FundTransCost']/$kostenSom;
      $this->pdf->row(array('Doorlopend (transactiekosten)','€',$this->formatGetal($vkmPercentagePortefeuille*$aandeel*$gemiddelde/100, 0), $this->formatGetal($vkmPercentagePortefeuille*$aandeel, 2,true,true)));
      $aandeel=$totaalDoorlopendekostenGesplitst['FundPerfFee']/$kostenSom;
      $this->pdf->row(array('Incidentele kosten (bijvoorbeeld performance fee)','€',$this->formatGetal($vkmPercentagePortefeuille*$aandeel*$gemiddelde/100, 0,false,true), $this->formatGetal($vkmPercentagePortefeuille*$aandeel, 2,true,true)));
      ...
      $this->pdf->row(array('Totale directe en indirecte kosten over de afgelopen 12 maanden', '€',$this->formatGetal($vkmWaarde*$gemiddelde/100, 0), $this->formatGetal($vkmWaarde, 2,true ,true)));
      $this->pdf->ln();
      ...
      $this->pdf->row(array('Verwacht netto rendement per jaar','', '',$this->formatGetal($risicoklasse['verwachtRendement']-$vkmWaarde, 2) . ' %' ));
...
foreach ($perCategorie as $hoofdcategorie=>$categorieData)
{
 ...
 WHERE Portefeuilles.portefeuille='".$this->portefeuille."'";
    ...
    $einddatum=substr($this->rapportageDatum,0,4);
    
    $grafiekWaardenVorigjaar[$beginDatum]['waardeZonderKosten']=$beginwaardeVorigJaar;
    $grafiekWaardenVorigjaar[$einddatum]['waardeZonderKosten']=$beginwaarde+$kosten;
    $grafiekWaardenVorigjaar[$beginDatum]['waardeNaKosten']=$beginwaardeVorigJaar;
    $grafiekWaardenVorigjaar[$einddatum]['waardeNaKosten']=$beginwaarde;
    $grafiekWaardenVorigjaar[$beginDatum]['cumulatieveKosten']=0;
    $grafiekWaardenVorigjaar[$einddatum]['cumulatieveKosten']=$kosten;
   ...
   // $grafiekWaardenVorigjaar['titel']="Impact van kosten op het rendement van de afgelopen 12 maanden";
    ...
    for($i=$this->pdf->rapport_jaar; $i<=$doelJaar; $i++)
      ...
      $grafiekWaarden[$i]['waardeZonderKosten']=$nieuweWaarde-$kosten;
			$grafiekWaarden[$i]['waardeNaKosten']=$nieuweWaarde;
...
//		echo "$doelJaar $rendement $beginwaarde";
  ...
  function VBarDiagram2($w, $h, $data,$nbDiv=4,$numBars=0)
  {
   ...
   // $colors=array('allocateEffect'=>array(108,31,128),'selectieEffect'=>array(234,105,11));//,'totaalEffect'=>array(0, 52, 121)); //
    ...
    $legenda=array('waardeZonderKosten'=>'Waardeontwikkeling zonder kosten','waardeNaKosten'=>'Waardeontwikkeling na kosten','cumulatieveKosten'=>'Cumulatieve kosten');
