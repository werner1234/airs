...
/*
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2020/05/31 16:12:00 $
 		File Versie					: $Revision: 1.2 $

 		$Log: PreProcessor_L81.php,v $
 		Revision 1.2  2020/05/31 16:12:00  rvv
 		*** empty log message ***
 		
 		Revision 1.1  2020/05/30 15:34:00  rvv
 		*** empty log message ***
 		
 		Revision 1.1  2020/05/30 15:30:39  rvv
 		*** empty log message ***
...
class PreProcessor_L81
{
	function PreProcessor_L81($portefeuille,$datum='',$pdf)
  {
    global $__appvar;
...
trim(substr(TijdelijkeRapportage.Fondsomschrijving,INSTR(TijdelijkeRapportage.Fondsomschrijving ,'% ' )+1)) as sortering FROM TijdelijkeRapportage
JOIN Fondsen on TijdelijkeRapportage.Fonds=Fondsen.Fonds WHERE Fondsen.fondssoort='OBL' AND  $vasteWhere ORDER BY sortering";
    $this->db->SQL($query);//echo $query;exit;
    $this->db->Query();
    $OBL=array();
    while($data=$this->db->nextRecord())
      $OBL[]=$data;
