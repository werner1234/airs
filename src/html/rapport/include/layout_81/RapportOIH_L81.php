...
$Log: RapportOIH_L81.php,v $
Revision 1.3  2019/09/21 16:32:39  rvv
*** empty log message ***

Revision 1.2  2019/09/18 14:54:35  rvv
*** empty log message ***

Revision 1.1  2019/09/15 09:00:00  rvv
*** empty log message ***
...
class RapportOIH_L81
{
	function RapportOIH_L81($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
    ...
    if(db2jul($this->pdf->PortefeuilleStartdatum) > db2jul("$RapStartJaar-01-01"))
      $this->kostprijsstart = $this->pdf->PortefeuilleStartdatum;
    else
      $this->kostprijsstart = "$RapStartJaar-01-01";
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
		...
		$db2 = new DB();
			...
			// voor data
		$this->pdf->widthB = array(15,16,17,76+9,16,1,27,16,1,27,27,1,23,11);
...
//			{
				...
				$historie = berekenHistorischKostprijs($this->portefeuille, $mutaties[Fonds], $mutaties[Boekdatum],$this->pdf->rapportageValuta,$this->kostprijsstart,$mutaties['id']);
...
//	echo "lopende -> ".$resultaatlopende." <-  voorgaande ".$resultaatvoorgaande. " -  <br>" ;
        ...
        $result_kostprijslopendejaar=$this->formatGetal($beginditjaar,$this->pdf->rapport_TRANS_decimaal);
        ...
        $percentageTotaal = ABS(($resultaatlopende) / $beginditjaar * 100);
          ...
          $datum=date("d-m",db2jul($mutaties['Boekdatum']));
				...
				$this->pdf->row(array($datum,
											...
											$verkoop_waarde,
                      $result_kostprijslopendejaar,'',
											$result_lopendejaar,//$result_voorgaandejaren,
											$percentageTotaalTekst));
					...
					$this->pdf->excelData[]=array(date("d-m",db2jul($mutaties['Boekdatum'])),
                      ...
                      $t_aankoop_waardeinValuta,
											...
											$resultaatlopende,						//											$resultaatvoorgaande,
											$percentageTotaal);
