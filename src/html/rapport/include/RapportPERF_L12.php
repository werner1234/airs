...
class RapportPERF_L12
{
	function RapportPERF_L12($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
//    $rendamentWaardenJaar = $index->getWaardenATT($this->perioden['jan'] ,$this->perioden['eind'] ,$this->portefeuille,'Totaal','maand',$this->pdf->rapportageValuta);
    ...
    $rendamentWaardenPeriode=$this->berekening->bereken($this->rapportageDatumVanaf,$this->rapportageDatum,'Totaal',$this->pdf->rapportageValuta);
    $rendamentWaardenJaar=$this->berekening->bereken($this->tweedePerformanceStart ,$this->rapportageDatum,'Totaal',$this->pdf->rapportageValuta);
...
$query = "SELECT TijdelijkeRapportage.fondsOmschrijving,
		           ...
		           TijdelijkeRapportage.beleggingscategorie,
				       ...
				       ORDER BY beleggingscategorieVolgorde asc, beleggingssectorVolgorde asc, TijdelijkeRapportage.Lossingsdatum, TijdelijkeRapportage.fondsOmschrijving asc";
$DB->SQL($query);
$DB->Query();
while($data=$DB->nextRecord())
{
  if($data['beleggingscategorie']=='AAND')
    $data['beleggingssector']='AAND';
	  ...
	  foreach ($this->perioden as $periode)
	  {
		...
		$query = "SELECT (TijdelijkeRapportage.fondsOmschrijving) as fondsOmschrijving,  TijdelijkeRapportage.rapportageDatum, ".
			...
			" TijdelijkeRapportage.rekening ,
	  		TijdelijkeRapportage.beleggingscategorie,TijdelijkeRapportage.AttributieCategorie,TijdelijkeRapportage.beleggingssector,".
			" sum(TijdelijkeRapportage.actuelePortefeuilleWaardeInValuta) as actuelePortefeuilleWaardeInValuta , ".
			" sum(TijdelijkeRapportage.actuelePortefeuilleWaardeEuro ) / ".$this->rapportageKoersen[$periode]." as actuelePortefeuilleWaardeEuro, ".
...
foreach ($this->waarden as $periode=>$data)
{
  foreach ($fondsMutaties as $fonds=>$waarden)
  {
     ...
     if(!isset($this->waarden[$periode][$waarden[7]][$waarden[6]][$waarden[5]][$fonds]))
     {
       ...
       $this->fondsOmschrijvingen[$fonds]=($fondsOmschrijving['Omschrijving']<>''?$fondsOmschrijving['Omschrijving']:$fonds);
...
//listarray($this->fondsenTree);exit;
...
foreach ($superCategorieKoppeling as $supercategorie=>$hoofdcategorien)
{
  ...
  //echo "$supercategorie $hoofdcategorie <br>\n ";ob_flush();listarray($hoofdcategorieKoppeling[$hoofdcategorie]);
       ...
       {
          $categorieOmschrijving['AAND']="Aandelen";
         $regels['overzicht'][] = array('kopHoofd',array('omschrijving'=>($categorieOmschrijving[$hoofdcategorie]<>''?$categorieOmschrijving[$hoofdcategorie]:$hoofdcategorie)));
         foreach ($hoofdcategorieKoppeling[$hoofdcategorie] as $categorie)
         {
             ...
             {
               $categorieOmschrijving['AAND']="Directe aandelen";
               ...
               foreach ($this->fondsenTree[$supercategorie][$hoofdcategorie][$categorie] as $fonds)//($this->fondsenPerCategorie[$categorie] as $fonds)
                 ...
                 //echo "$supercategorie $hoofdcategorie $categorie $fonds $totaalMutaties<br>\n";ob_flush();
  	             ...
  	             $directeKostenOpbrengstenJaar = $this->fondsKostenOpbrengsten($fonds,$this->perioden['jan'],$this->perioden['eind']);
		             ...
		             if(strtoupper($categorie) == 'LIQUIDITEITEN')
		             {
...
//supercategorie
                  ...
                  $this->toon['fonds'][$supercategorie][$hoofdcategorie][$categorie][$fonds]) && $categorie <> 'AAND' )
                  ...
                  if($categorie=='AAND')
                    $stort=$categorienStortingenontrekkingen[$hoofdcategorie][$categorie];
                  else
                    $stort=0;  
         ...
         case "overzicht":
           ...
           if(substr($data[1]['actuelePortefeuilleWaardeInValuta'],0,9)=='Subtotaal')
           {
		         $this->pdf->row(array($data[1]['omschrijving'],
                           ...
                           $this->formatGetal($data[1]['resultaat'],2,true,$limit),
                           $this->formatGetal($data[1]['resultaatAbsoluutJaar'],0),
                           $this->formatGetal($data[1]['resultaatJaar'],2,true,$limit)
                           ));
             $this->pdf->ln();
 ...
 */
    ...
    $totaal_verkoop_waarde=0;
      ...
      if($mutaties['beleggingscategorie']=='AAND')
        $mutaties['beleggingssector']='AAND';
