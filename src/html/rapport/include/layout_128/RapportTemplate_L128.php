...
class RapportTemplate_L128
{
	function RapportTemplate_L128($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	  global $__appvar;
		$this->pdf = &$pdf;
    ...
    $totPagina = ($lastpage-$startpagina+$paginaCorrectie);//-1
       ...
       $this->pdf->ln(10);
	     ...
	     $inhoudsItems=array('PERFPaginas'=>'Performance over de beleggingscategorieën',
                           ...
                           'VKMAPaginas'=>'Vergelijkende kostenmaatstaf ex-ante',
        ...
        foreach ($this->pdf->templateVars as $key=>$value)
        {
          if($inhoudsItems[$key] || $this->pdf->templateVarsOmschrijving[$key]) //
          {
            ...
            $dots=round((219-$stringWidth)/($this->pdf->CurrentFont['cw']['.']*$this->pdf->FontSize/1000));
