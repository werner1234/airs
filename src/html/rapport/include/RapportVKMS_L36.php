...
$Log: RapportVKMS_L36.php,v $
Revision 1.12  2019/02/03 15:50:24  rvv
*** empty log message ***

Revision 1.11  2019/01/23 09:44:48  rvv
*** empty log message ***

Revision 1.10  2019/01/23 08:49:58  rvv
*** empty log message ***

Revision 1.9  2019/01/19 13:54:10  rvv
*** empty log message ***
...
class RapportVKMS_L36
{
	function RapportVKMS_L36($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    ...
    $kostenRow=array();
    $kostenData=array('BEH'=>array('Grootboekrekening'=>'BEH','Omschrijving'=>'Adviesvergoeding'),
                      'KOST'=>array('Grootboekrekening'=>'KOST','Omschrijving'=>'Transactiekosten depotbank'),
                      'ALLI'=>array('Grootboekrekening'=>'ALLI','Omschrijving'=>'All-in fee'),
                      'VALK'=>array('Grootboekrekening'=>'VALK','Omschrijving'=>'Overige kosten depotbank'));
    
    while($data = $DB->nextRecord())
    {
      if(in_array($data['Grootboekrekening'],array('KOBU','KNBA','BEW')))
      {
        $data['Grootboekrekening'] = 'VALK';
      }
      ...
      $kostenData[$data['Grootboekrekening']]['totaal']+=$data['totaal'];
        ...
        if($data['Grootboekrekening']=='BEH')
        {
          ...
          $kostenRow[] = array(vertaalTekst('Beheervergoeding IVM Caring Capital', $this->pdf->rapport_taal), $this->formatGetal($bedrag, 0), $this->formatGetal($bedrag/$gemiddelde*100, 2) . ' %');
          if($risicoklasse['BeheerfeeBTW'] <> 0)
          {
            $kostenRow[] = array("BTW over ".vertaalTekst('beheervergoeding', $this->pdf->rapport_taal), $this->formatGetal($btw, 0), $this->formatGetal($btw/$gemiddelde*100, 2) . ' %');
          }
        }
        else
        {
...
//				$this->pdf->excelData[]=array('',$data['Omschrijving'],'',round($data['totaal'],0),round($kostenProcent,2) );
    ...
    if($this->pdfVullen==true)
    {
      ...
      $this->pdf->row(array('Werkelijk bruto jaarrendement', $this->formatGetal($rendementProcent+$vkmWaarde, 2) . ' %' ));
      ...
      $this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
      $this->pdf->row(array('Indirecte kosten vanaf '.date('d-m-Y',$this->pdf->rapport_datumvanaf)));
      $this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
      ...
      $this->pdf->row(array('Incidentele kosten (performance fee)',$this->formatGetal($vkmPercentagePortefeuille*$aandeel*$gemiddelde/100, 0,false,true), $this->formatGetal($vkmPercentagePortefeuille*$aandeel, 2,true,true)));
    ...
    //  $this->pdf->setWidths(array(110, 30, 30));
      ...
      {
        $this->pdf->row(array('De totale te verwachten directe en indirecte kosten', '', $this->formatGetal($percentage['AfwijkendeExAnte'], 2, true, true)));
      }
      else
      {
        $this->pdf->row(array('De totale te verwachten directe en indirecte kosten', '', $this->formatGetal($vkmWaarde, 2, true, true)));
      }
      $this->pdf->ln();
      $this->pdf->ln();
      $this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
...
foreach ($perCategorie as $hoofdcategorie=>$categorieData)
{
 ...
 $perfTotaal['percentageIndirectVermogenMetKostenfactor']=$percentageIndirectVermogenMetKostenfactor;
    ...
    {
      if($data['doeldatum'] > $this->pdf->rapport_jaar+10)
 ...
 WHERE Portefeuilles.portefeuille='".$this->portefeuille."'";
    ...
    $this->pdf->setXY(175,120);
		$this->LineDiagram(110, 55, $grafiekWaarden,array($waardeZonderKostenKleur,$waardeNaKostenKleur,$cumulatieveKostenKleur),0,0,4,4,false);//50
    ...
    if($vanafBegin==true)
      ...
      $offset=0;
		...
		foreach ($legendaItems as $index=>$item)
		{
			...
			$this->pdf->Rect($XPage-15+$step , $YPage+$h+10, 3, 3, 'DF','',$kleur);
			$this->pdf->SetXY($XPage+3-15+$step,$YPage+$h+10);
