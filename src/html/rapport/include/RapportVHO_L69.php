...
class RapportVHO_L69
{
	function RapportVHO_L69($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
      ...
      $this->pdf->Line($totaal6+$extra,$this->pdf->GetY(),$totaal6 + $this->pdf->widthB[13],$this->pdf->GetY());
		...
		if($grandtotaal)
		{
			if($this->pdf->rapport_VHO_volgorde_beginwaarde == 1)
			{
				...
				{
					$this->pdf->Line($totaal5+$extra,$this->pdf->GetY(),$totaal5 + $this->pdf->widthB[12],$this->pdf->GetY());
					$this->pdf->Line($totaal5+$extra,$this->pdf->GetY()+1,$totaal5 + $this->pdf->widthB[12],$this->pdf->GetY()+1);
				}
				if(!empty($totaalD))
				{
					$this->pdf->Line($totaal4+$extra,$this->pdf->GetY(),$totaal4 + $this->pdf->widthB[11],$this->pdf->GetY());
				...
				{
					$this->pdf->Line($totaal6+$extra,$this->pdf->GetY(),$totaal6 + $this->pdf->widthB[13],$this->pdf->GetY());
					$this->pdf->Line($totaal6+$extra,$this->pdf->GetY()+1,$totaal6 + $this->pdf->widthB[13],$this->pdf->GetY()+1);
				}
			}
			else
			{
				...
				if(!empty($totaalE))
				{
					...
					$this->pdf->Line($totaal5,$this->pdf->GetY()+1,$totaal5 + $this->pdf->widthB[12],$this->pdf->GetY()+1);
				}
				if(!empty($totaalD))
				{
...
//     " Rekeningmutaties.Boekdatum >= '".	$this->rapportageDatumVanaf."' AND ".
	...
	function writeRapport()
	{
		...
		$this->pdf->widthB = array(5,55,18,25,1,20,1,25,21,21,15,22,22,22,12);
		$this->pdf->alignB = array('R','L','R','R','R','R','R','R','R','R','R','R','R','R','R','R');

		// voor kopjes
		$this->pdf->widthA = array(   60,18,25,1,20,1,25,21,21,15,22,22,22,12);
...
//			$procentResultaat = (($categorien[subtotaalactueel] - $categorien[subtotaalhistorisch]) / ($categorien[subtotaalhistorisch] /100));
      ...
      $grandtotaaldividendCorrected  += $subtotaal['totaalDividendCorrected'];
		...
		// print grandtotaal
		$this->printTotaal(vertaalTekst("Totale actuele waarde portefeuille",$this->pdf->rapport_taal), "", $actueleWaardePortefeuille, $percentageVanTotaal,"","","",true,'');//$grandtotaaldividend
