...
/*
Author  						: $Author: rvv $
Laatste aanpassing	: $Date: 2019/10/19 16:45:25 $
File Versie					: $Revision: 1.1 $

$Log: RapportAFM_L54.php,v $
Revision 1.1  2019/10/19 16:45:25  rvv
*** empty log message ***
...
class RapportAFM_L54
{
	function RapportAFM_L54($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function formatGetalKoers($waarde, $dec , $start = false)
    ...
    $this->pdf->templateVars['AFMPaginas']=$this->pdf->page;
      ...
      $lastCat       = $categorien['afmCategorie'];
		...
		$percentageVanTotaal = $totaalactueel / ($totaalWaarde/100);
		$actueleWaardePortefeuille += $this->printTotaal("", $totaalactueel, $percentageVanTotaal,false);
		// voor Pie
		$this->pdf->pieData[vertaalTekst($lastCategorie,$this->pdf->rapport_taal)] = $percentageVanTotaal;
		$grafiekCategorien[$lastCat]=$percentageVanTotaal; //toevoeging voor kleuren.
	...
	//	listarray($kleurdata);
		...
		$chartsize=55;
		$vwh=$chartsize;//((210-$headerHeight-$this->pdf->marge)/2+$headerHeight)-$headerHeight;
		$ystart=50;
		$this->pdf->setXY(200 , $ystart);
		$legendaStart=array(200,$ystart+$chartsize+10);
		PieChart_L54($this->pdf, $chartsize, $vwh, $this->pdf->pieData, '%l', $kleurdata, vertaalTekst('Onderverdeling in AFM categorieën', $this->pdf->rapport_taal), $legendaStart);
