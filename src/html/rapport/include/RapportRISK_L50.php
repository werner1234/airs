...
class RapportRISK_L50
{
	function RapportRISK_L50($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
	 //
		$this->pdf = &$pdf;
...
//echo $q;listarray($kleuren);
    ...
    $this->pdf->Row(array('','kwaliteit','Absoluut','in %'));
    $this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
    $ratingGrafiekKleuren=array();
    foreach ($this->ratingData as $rating=>$data)
    {
      $this->pdf->Row(array('',$rating,$this->formatGetal($data['waarde'],0),$this->formatGetal($data['procent']*100,1)."%"));
    ...
    $this->pdf->setY(130);
		...
		$this->pdf->SetAligns(array('L','L','R','R','R','R','R'));
		$this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
		$this->pdf->Row(array('','Jaar','Absoluut',"in %"));
		$this->pdf->SetFont($this->pdf->rapport_font,'',$this->pdf->rapport_fontsize);
    ...
    $this->pdf->Cell(0,5,"Verdeling naar kwaliteit");
    $this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
    ...
    function VBarDiagram($w, $h, $data)
  {
      ...
      $this->pdf->Multicell($w,5,'Verdeling over jaren','','C');
