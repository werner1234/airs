...
$Log: RapportRISK_L72.php,v $
Revision 1.3  2019/01/26 19:33:28  rvv
*** empty log message ***
...
class RapportRISK_L72
{
	function RapportRISK_L72($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
  ...
  function formatGetalKoers($waarde, $dec , $start = false)
    ...
    $att->specifiekeIndex=$portefeuilledata['SpecifiekeIndex'];
    //$this->waarden['Historie']=$att->bereken(substr($this->pdf->PortefeuilleStartdatum,0,10),  $this->rapportageDatum,'EUR','hoofdcategorie');
    $this->waarden['Historie']=$att->bereken($rapportageStartJaar,  $this->rapportageDatum,'EUR','categorie','maanden');
    ...
    { 
       ...
       foreach ($categorieData['perfWaarden'] as $datum=>$waarden)
      {
        ...
        { 
         // echo $categorie."<br>\n";
         // listarray($waarden); 
          $attributieEffect=(($waarden['weging']*$waarden['procent'])-($waarden['indexPerf']*$waarden['indexBijdrageWaarde']))*100;
        //  echo " $attributieEffect=((".$waarden['weging']." * ".$waarden['procent'].")-(".$waarden['indexPerf']."*".$waarden['indexBijdrageWaarde']."))*100; $categorie <br>\n";
          ...
          $selectieEffect=($waarden['procent']-$waarden['indexPerf'])*$waarden['indexBijdrageWaarde']*100;
          $interactieEffect=($waarden['weging']-$waarden['indexBijdrageWaarde'])*($waarden['procent']-$waarden['indexPerf'])*100;
      ...
      //  echo " $attributieEffect=((".$waarden['weging']."*".$waarden['procentBruto'].")-(".$waarden['indexPerf']."*".$waarden['indexBijdrageWaarde']."))*100 <br>\n";
       ...
       //   $attributieEffect=$allocatieEffect+$selectieEffect+$interactieEffect;
        ...
        else
        {
          ...
          $benchmark=(1+$waarden['indexPerf']*1+$benchmark)-1;
...
*/
    ...
    $this->pdf->SetWidths(array(60,28,27,28,27,28,27,28,27));
...
while($data=$DB->nextRecord())
  $categorieLookup[$data['Beleggingscategorie']]=$data['Omschrijving'];
    $categorieLookup['totaal']='Totaal';
   //foreach ( as $categorie=>$jaarWaarden)
    $categorieenTonen=$att->alleCategorien;
    $categorieenTonen['totaal']='totaal';
    $normData=$att->normData;
    if(isset($normData['totaal']))
      unset($normData['totaal']);
   foreach($categorieenTonen as $categorie=>$omschrijving)
   {
     if(isset($this->jaarTotalen[$categorie]))
     {
       $jaarWaarden=$this->jaarTotalen[$categorie];
       $waarden = $jaarWaarden[$startJaar];
       if ($waarden['weging'] <> 0 || $att->normData[$categorie] <> 0 || $waarden['procent'] <> 0 || $waarden['indexPerf'] <> 0)
       {
         if ($categorie == 'totaal')
         {
           if ($benchmark <> 0)
           {
             $benchmarkTxt = $this->formatGetal($benchmark * 100, 2);
           }
           else
           {
             $benchmarkTxt = '';
           ...
           $this->pdf->row(array($categorieLookup[$categorie],
                             ...
                             $this->formatGetal(array_sum($normData), 1),
                             $this->formatGetal($waarden['procent'] * 100, 2),
                             $benchmarkTxt,//$this->formatGetal($waarden['indexPerf'] * 100, 2),
