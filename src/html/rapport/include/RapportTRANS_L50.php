...
$Log: RapportTRANS_L50.php,v $
Revision 1.8  2019/06/21 14:52:45  rm
7880

Revision 1.7  2015/12/19 08:29:17  rvv
*** empty log message ***

Revision 1.6  2014/07/16 16:01:16  rvv
*** empty log message ***

Revision 1.5  2014/07/14 10:03:47  rvv
*** empty log message ***

Revision 1.4  2014/07/12 15:30:53  rvv
*** empty log message ***
...
class RapportTRANS_L50
{
	function RapportTRANS_L50($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
	...
	function writeRapport()
	{
	  ...
	  if ($this->pdf->rapportageValuta != "EUR" || $this->pdf->rapportageValuta != '')
	   $koersQuery =	" / (SELECT Koers FROM Valutakoersen WHERE Valuta='".$this->pdf->rapportageValuta."' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) ";
		...
		{
		// voor data
		$this->pdf->widthB = array(15,10,20,38,16,22,22,16,22,22,22,23,23,12);
		$this->pdf->alignB = array('L','L','R','L','R','R','R','R','R','R','R','R','R','R');
		// voor kopjes
		$this->pdf->widthA = array(15,10,20,38,16,22,22,16,22,22,22,23,23,12);
...
Rekeningmutaties.Credit AS Credit,
Rekeningmutaties.Valutakoers,
1 / (SELECT Koers FROM Valutakoersen WHERE Valuta='EUR' AND Datum <= Rekeningmutaties.Boekdatum ORDER BY Datum DESC LIMIT 1 ) AS Rapportagekoers,
BeleggingscategoriePerFonds.Beleggingscategorie,
CategorienPerHoofdcategorie.Hoofdcategorie,
Beleggingscategorien.Omschrijving as hoofdcategorieOmschrijving
FROM
Rekeningmutaties
...
INNER JOIN BeleggingscategoriePerFonds ON Rekeningmutaties.Fonds = BeleggingscategoriePerFonds.Fonds AND BeleggingscategoriePerFonds.Vermogensbeheerder = '".$this->pdf->portefeuilledata['Vermogensbeheerder']."'
...
WHERE Rekeningen.Portefeuille = '".$this->portefeuille."' AND Rekeningmutaties.Verwerkt = '1' AND Rekeningmutaties.Transactietype <> 'B' AND Grootboekrekeningen.FondsAanVerkoop = '1' AND 
Rekeningmutaties.Boekdatum > '".$this->rapportageDatumVanaf."' AND Rekeningmutaties.Boekdatum <= '".$this->rapportageDatum."'
ORDER BY CategorienPerHoofdcategorie.Hoofdcategorie, Rekeningmutaties.Boekdatum, Rekeningmutaties.Fonds";
		$DB = new DB();
		$DB->SQL($query);
		$DB->Query();
    ...
    $resultaatlopendeTotaal=0;
    $totaal_aankoop_waarde=array();
    $totaal_verkoop_waarde=array();
      ...
      $t_aankoop_waardeinValuta=0;
			...
			switch($mutaties['Transactietype'])
			{
					case "A" :
						...
						$totaal_aankoop_waarde[$mutaties['hoofdcategorieOmschrijving']] += $t_aankoop_waarde;
					...
					case "L" :
						...
						$totaal_verkoop_waarde[$mutaties['hoofdcategorieOmschrijving']] += $t_verkoop_waarde;
...
//	echo "lopende -> ".$resultaatlopende." <-  voorgaande ".$resultaatvoorgaande. " -  <br>" ;
				...
				$totaal_resultaat_waarde[$mutaties['hoofdcategorieOmschrijving']] += $resultaatlopende;
    ...
    if($lastHcat!=$mutaties['hoofdcategorieOmschrijving'])
    {
      if($lastHcat <> '')
      {
        ...
        $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
        $this->pdf->Cell(100,4,vertaalTekst("Totaal",$this->pdf->rapport_taal).' '.$lastHcat);
        $this->pdf->setX($this->pdf->marge);
		    $this->pdf->row(array("","","",'',"","",$this->formatGetal($totaal_aankoop_waarde[$lastHcat],$this->pdf->rapport_TRANS_decimaal),"","",$this->formatGetal($totaal_verkoop_waarde[$lastHcat],$this->pdf->rapport_TRANS_decimaal),"","",$this->formatGetal($totaal_resultaat_waarde[$lastHcat],$this->pdf->rapport_TRANS_decimaal)));
        //$subtotalenTotaal=array('aankoop'=>$totaal_aankoop_waarde,'verkoop'=>$totaal_verkoop_waarde,'resultaat'=>$totaal_resultaat_waarde);
        $this->pdf->Ln();
      }
      $this->pdf->SetFont($this->pdf->rapport_font,'B',$this->pdf->rapport_fontsize);
      $this->pdf->setX($this->pdf->marge);
      $this->pdf->Cell(100,4,$mutaties['hoofdcategorieOmschrijving'],0,1);
      $this->pdf->SetFont($this->pdf->rapport_font,$this->pdf->rapport_fontstyle,$this->pdf->rapport_fontsize);
    }
    ...
    if($lastHcat <> '')
    {
      ...
      $this->pdf->row(array("","","",'',"","",$this->formatGetal($totaal_aankoop_waarde[$lastHcat],$this->pdf->rapport_TRANS_decimaal),"","",$this->formatGetal($totaal_verkoop_waarde[$lastHcat],$this->pdf->rapport_TRANS_decimaal),"","",$this->formatGetal($totaal_resultaat_waarde[$lastHcat],$this->pdf->rapport_TRANS_decimaal)));
		...
		$this->pdf->row(array("",
								...
								"",
								$this->formatGetal(array_sum($totaal_aankoop_waarde),$this->pdf->rapport_TRANS_decimaal),
								"",
								"",
								$this->formatGetal(array_sum($totaal_verkoop_waarde),$this->pdf->rapport_TRANS_decimaal),
								"",
								"",
