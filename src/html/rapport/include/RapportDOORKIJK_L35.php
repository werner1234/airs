...
$Log: RapportDOORKIJK_L35.php,v $
Revision 1.2  2020/02/29 16:23:08  rvv
*** empty log message ***

Revision 1.1  2019/08/03 17:03:12  rvv
*** empty log message ***
...
class RapportDOORKIJK_L35
{
	function RapportDOORKIJK_L35($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = $pdf;
...
WHERE bronKoppeling='$airsCategorie' AND doorkijkCategoriesoort='".mysql_real_escape_string($doorkijkSoort)."' AND systeem='AIRS' AND vermogensbeheerder='". $this->pdf->portefeuilledata['Vermogensbeheerder']."'";
    ...
    $teTonenRegels = array();
			if($titel=='Beleggingssector')
      {
        $query="SELECT
SectorenPerHoofdsector.Vermogensbeheerder,
SectorenPerHoofdsector.Hoofdsector,
SectorenPerHoofdsector.Beleggingssector,
hoofd.Omschrijving as Hoofd,
sect.Omschrijving as Sec
FROM
SectorenPerHoofdsector
INNER JOIN Beleggingssectoren as hoofd ON SectorenPerHoofdsector.Hoofdsector = hoofd.Beleggingssector
INNER JOIN Beleggingssectoren as sect ON SectorenPerHoofdsector.Beleggingssector = sect.Beleggingssector
WHERE SectorenPerHoofdsector.Vermogensbeheerder='".$this->pdf->portefeuilledata['Vermogensbeheerder']."'
ORDER BY hoofd.Afdrukvolgorde , sect.Afdrukvolgorde";
        $db=new DB();
        $db->SQL($query);
        $db->query();
        $lastHoofd='';
        ...
        {
          if(isset($regels[$data['Sec']]))
          {
            if($data['Hoofd']<> $lastHoofd)
            {
              $teTonenRegels[] = array('hoofdcat' => $data['Hoofd']);
            }
              $regels[$data['Sec']]['categorie'] = $data['Sec'];
              $teTonenRegels[] = $regels[$data['Sec']];
            $lastHoofd=$data['Hoofd'];
          }
        }
      }
      else
      {
        foreach ($kleuren as $categorie => $kleur)
        {
          ...
          {
            $regels[$categorie]['categorie'] = $categorie;
            $teTonenRegels[] = $regels[$categorie];
          }
        }
      }
    $grafiekdata=array();
		foreach($teTonenRegels as $key=>$data)
		{
		  if(isset($data['hoofdcat']))
      {
        $this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
        $this->pdf->row(array('', $data['hoofdcat']));
        $this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
      }
      else
      {
        $this->pdf->rect($this->pdf->getX() + $xOffset, $this->pdf->getY() + 1, 2, 2, 'DF', '', $kleuren[$data['categorie']]);
        $this->pdf->row(array('', $data['categorie'], $this->formatGetal($data['waardeEUR'], 0), $this->formatGetal($data['percentage'], 2)));
        $this->pdf->excelData[] = array($data['categorie'], round($data['waardeEUR'], 0), round($data['percentage'], 2));
        $totalen['waardeEUR'] += $data['waardeEUR'];
        $totalen['percentage'] += $data['percentage'];
        $grafiekdata[$data['categorie']]['kleur'] = $kleuren[$data['categorie']];//array('R' => array('value' => $kleuren[$categorie][0]),'G' => array('value' => $kleuren[$categorie][1]),'B' => array('value' => $kleuren[$categorie][2]));
        $grafiekdata[$data['categorie']]['percentage'] = $data['percentage'];
      }
		}
    $this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
...
*/
	...
	function vulPagina($belCategorien='')
	{
      ...
      $grafiekdata=$this->toonTabel($doorKijk['details'],$xOffset,$doorkijkTitels[$doorkijkCategorieSoort],$this->kleuren[$doorkijkCategorieSoort]);
			//$grafiekdata=array();
			$grafiekTonen=true;
