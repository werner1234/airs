...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: PortefeuilleParameters.php,v $
 		Revision 1.33  2019/11/16 17:36:34  rvv
 		*** empty log message ***
 		
 		Revision 1.32  2019/10/12 17:10:59  rvv
 		*** empty log message ***
 		
 		Revision 1.31  2018/09/01 16:52:29  rvv
 		*** empty log message ***
 		
 		Revision 1.30  2018/07/26 05:51:36  rvv
 		*** empty log message ***
 		
 		Revision 1.29  2018/07/25 16:09:04  rvv
 		*** empty log message ***
...
class PortefeuilleParameters
{
	...
	*/
	...
	function PortefeuilleParameters( $selectData )
	{
	  $this->pdf = new PDFRapport('L','mm');
	  $this->selectData = $selectData;
    if($this->selectData['filterFonds']=='alles')
      $this->selectData['filterFonds']='';
    ...
    $portefeuilles = $selectie->getSelectie();
		...
		{
	 	$pdata[$a]['Client'] = $portData['Client'];
	 	$pdata[$a]['Naam'] = $portData['Naam'];
		$pdata[$a]['Portefeuille'] = $portData['Portefeuille'];
		$pdata[$a]['Depotbank'] = $portData['Depotbank'];
  	$pdata[$a]['Risicoklasse'] = $portData['Risicoklasse'];
    $pdata[$a]['Accountmanager'] = $portData['Accountmanager'];
    $pdata[$a]['SoortOvereenkomst'] = $portData['SoortOvereenkomst'];
  	$pdata[$a]['Vermogensbeheerder'] = $portData['Vermogensbeheerder'];
...
$queries['duurzaam'] = "SELECT DuurzaamCategorien.DuurzaamCategorie as type ,
						                DuurzaamCategorien.Omschrijving
		 		                    FROM 	DuurzaamCategorien 
                            JOIN BeleggingssectorPerFonds ON DuurzaamCategorien.DuurzaamCategorie=BeleggingssectorPerFonds.DuurzaamCategorie
...
elseif ($this->selectData['typeInvoer'] == 'duurzaam')
{
  $invoerWhere   = " AND TijdelijkeRapportage.duurzaamCategorie = '".$this->selectData['invoer']."' ";
  $invoerTables  = '';
}
...
$percentageQuery['duurzaam']             = "SELECT TijdelijkeRapportage.duurzaamCategorie as type ,sum(ActuelePortefeuilleWaardeEuro) AS WaardeEuro
	                                       FROM TijdelijkeRapportage $invoerTables
	                                       WHERE Portefeuille = '".$portefeuille."'
	                                       AND rapportageDatum ='".$rapportageDatum."' AND TijdelijkeRapportage.Type = 'Fondsen' "
                                         .$__appvar['TijdelijkeRapportageMaakUniek']. $invoerWhere .
