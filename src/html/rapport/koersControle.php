...
/*
    ...
    Author  						: $Author: rvv $
 		...
 		$Log: koersControle.php,v $
 		Revision 1.2  2017/08/02 18:22:25  rvv
 		*** empty log message ***
...
class koersControle
{
	...
	*/
	...
	function koersControle( $selectData )
	{
	...
	function valutaCheck()
	{
		...
		if($this->progressbar)
		{
			...
			$this->progressbar->setLabelValue('txt1', vt('Valuta controle...'));
			$pro_step = 0;
		}
		$DB2 	= new DB();
		while($data = $DB->NextRecord())
		{
			...
			if($DB2->Records() <= 0)
			{
				...
				if($this->selectData['aanvullen'] == 1)
				{
					...
					{
						$this->onbekendekoers[] = vt("Valutakoers")." ".$data['Valuta']." : ".vt("geen laatst ingevoerde koerswaarde gevonden")."!";
					}
				}
				else
				{
					...
					$this->onbekendekoers[] = vt("Valutakoers")." ".$data['Valuta']." op ".$datum." ".vt("niet gevonden (laatste")." ".date('d-m-Y',db2jul($laatsteKoers['datum'])).")";
				}
			}
		}
    ...
    global $USR;
		...
		if($this->progressbar)
		{
			...
			$this->progressbar->moveStep(0);
			$this->progressbar->setLabelValue('txt1', vt('Koers controle...'));
		}
		$pro_step = 0;
      ...
      FROM Fondsen WHERE Fonds='".mysql_real_escape_string($fonds)."'  ".$this->FondsEinddatumFilter." ".$this->koersControleCheck;
			...
			{
				  $query = "SELECT id FROM Fondskoersen WHERE datum = '".$date."' AND Fonds = '".mysql_real_escape_string($fonds)."'";
					$DB2->SQL($query);
					$DB2->Query();
					if($DB2->Records() <= 0)
					{
						...
						if($this->selectData['aanvullen'] == 1)
						{
							...
							{
								$koers = $DB2->NextRecord();
								$query = "INSERT INTO Fondskoersen SET datum = '".$date."' , Fonds = '".mysql_real_escape_string($fonds)."', Koers = '".$koers['Koers']."', ".
									" add_user = '".$USR."', ".
									" add_date = NOW(), ".
									" change_user = '".$USR."',".
									" change_date = NOW()";
							...
							{
								$this->onbekendekoers[] = vt("Fondskoers")." ".$fonds." : ".vt("geen laatst ingevoerde koerswaarde gevonden")."!";
							}
						}
						else
						{
							$query = "SELECT datum FROM Fondskoersen WHERE Fonds = '".mysql_real_escape_string($fonds)."' AND datum <='".$date."' ORDER BY datum DESC LIMIT 1";
							$DB2->SQL($query);
							$DB2->Query();
							$laatsteKoers=$DB2->NextRecord();
							$this->onbekendekoers[] = vt("Fondskoers")." ".$fonds." op ".$datum." ".vt("niet gevonden (laatste")." ".date('d-m-Y',db2jul($laatsteKoers['datum'])).")";
						}
					}
			}
		}
...
ORDER BY Rekeningmutaties.Fonds";
    ...
    if($this->selectData['gebruikInactieveFondsen']==1)
      ...
      $this->FondsEinddatumFilter="AND (Einddatum > now() OR Einddatum < '1980-01-01') ";
		...
		$this->pdf->row(array("Controle (".date("d-m-Y",$this->selectData['datumTm']).")"));
		$this->pdf->excelData[]=array("Controle (".date("d-m-Y",$this->selectData['datumTm']).")");
		...
		if($this->selectData['aanvullen']==1)
		{
			$this->pdf->row(array("<br>".vat("fondsen")." ".$this->aantalfondsen." ".vat("en")." ".$this->valutas." ".vat("valuta aangevuld")."."));
			$this->pdf->excelData[]=array("<br>".vat("fondsen")." ".$this->aantalfondsen." ".vat("en")." ".$this->valutas." ".vat("valuta aangevuld").".");
		}

		for ($a=0; $a < count($this->onbekendekoers); $a++)
		{
			$this->pdf->row(array($this->onbekendekoers[$a]));
			$this->pdf->excelData[]=array($this->onbekendekoers[$a]);
		}
