...
class RapportVKM
{
	function RapportVKM($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum,$laatstejaar=true)
	{
		if(is_object($pdf))
		{
			$this->pdf = &$pdf;
    ...
    $db=new DB();
    $query = "SELECT Portefeuilles.portefeuille, Portefeuilles.Clientvermogensbeheerder, Portefeuilles.Startdatum, Portefeuilles.BeheerfeeBTW FROM Portefeuilles WHERE Portefeuilles.portefeuille='$portefeuille' limit 1";
    $db->SQL($query);
    $pdata = $db->lookupRecord();
    $this->BeheerfeeBTW=$pdata['BeheerfeeBTW']/100;
	...
	{
	  if($waarde==0 && $toonNul==false && $this->nullTonen==false)
	    return;
...
//echo "$doorlopendeKostenPercentage = $totaalDoorlopendekosten / ".$perfTotaal['gemWaarde']."<br>\n";exit;
    if($this->pdfVullen==true)
      ...
      $this->pdf->SetTextColor($this->pdf->rapport_default_fontcolor['r'], $this->pdf->rapport_default_fontcolor['g'], $this->pdf->rapport_default_fontcolor['b']);
	  	...
	  	$this->pdf->row(array(vertaalTekst('Indirecte (fonds)kosten',$this->pdf->rapport_taal), $this->formatGetal($totaalDoorlopendekosten, 0) . ' '.$this->pdf->rapportageValuta));
			$this->pdf->excelData[]=array('','Indirecte (fonds)kosten', '','',round($totaalDoorlopendekosten, 0), $this->pdf->rapportageValuta);
			$this->pdf->row(array(vertaalTekst('Indirecte (fonds)kosten ten opzichte van onderliggend vermogen',$this->pdf->rapport_taal), $this->formatGetal($doorlopendeKostenPercentage * 100, 2) . ' %'));
		...
		if($this->pdfVullen==true)
		{
			...
			$this->pdf->row(array(vertaalTekst("Herrekende indirecte (fonds)kosten",$this->pdf->rapport_taal), $this->formatGetal($herrekendeKosten * 100, 2) . ' %'));
			$this->pdf->excelData[]=array('',"Herrekende indirecte (fonds)kosten", '','',round($herrekendeKosten * 100, 2),'%');
			...
			$this->pdf->ln();
			$this->pdf->excelData[]=array();
			$this->pdf->row(array(vertaalTekst('Gemiddeld vermogen',$this->pdf->rapport_taal), $this->formatGetal($gemiddelde, 0) . ' '.$this->pdf->rapportageValuta));
			$this->pdf->excelData[]=array('','Gemiddeld vermogen','','',round($gemiddelde,0),$this->pdf->rapportageValuta);
			$this->pdf->SetFont($this->pdf->rapport_font, 'B', $this->pdf->rapport_fontsize);
			$this->pdf->row(array(vertaalTekst('Indirecte (fonds)kosten factor van de portefeuille',$this->pdf->rapport_taal), $this->formatGetal($vkmPercentagePortefeuille, 2) . ' %'));
			$barData=array();
      ...
      else
      {
        $kostenTxt=vertaalTekst('Directe kosten vanaf',$this->pdf->rapport_taal).' ' . date('d-m-Y', db2jul($this->vanafDatum));
			...
			$this->pdf->row(array($kostenTxt, $this->pdf->rapportageValuta, vertaalTekst('Percentage',$this->pdf->rapport_taal)));
    ...
    $gbDetails=array();
      ...
      if($data['Kosten']==1)
      {
        if ($this->pdfVullen == true)
        {
          ...
          {
            $bedrag=$data['totaal']/(1+$this->BeheerfeeBTW);
            $btw=$data['totaal']-$bedrag;
            $kostenProcent = $bedrag / $gemiddelde * 100;
            $this->pdf->row(array(vertaalTekst($data['Omschrijving'], $this->pdf->rapport_taal).'',$this->formatGetal($bedrag, 0), $this->formatGetal($kostenProcent, 2) . ' %'));
            $barData[$data['Omschrijving']] = $kostenProcent;
            $gbDetails[$data['Omschrijving']]=array($bedrag,$kostenProcent);
            if($this->BeheerfeeBTW <> 0)
            {
              $kostenProcent = $btw / $gemiddelde * 100;
              $omschrijving= vertaalTekst("BTW over ".$data['Omschrijving'], $this->pdf->rapport_taal);
              $barData[$omschrijving] = $kostenProcent;
              $this->pdf->row(array($omschrijving, $this->formatGetal($btw, 0), $this->formatGetal($kostenProcent, 2) . ' %'));
              $gbDetails[$omschrijving]=array($btw,$kostenProcent);
            }
          }
          else
          {
            ...
            $barData[$data['Omschrijving']] = $kostenProcent;
            $gbDetails[$data['Omschrijving']]=array($data['totaal'],$kostenProcent);
          }
        }
        else
        {
          if($data['Grootboekrekening']=='BEH')
          {
            ...
            {
              $kostenProcent = $btw / $gemiddelde * 100;
              $omschrijving= "BTW over ".$data['Omschrijving'];
              $gbDetails[$omschrijving]=array($btw,$kostenProcent);
            }
          }
          else
      ...
      $alleGrootboeken[$data['Grootboekrekening']] += $data['totaal'];
    ...
    //echo " $grootBoekKostenTotaal=$totaal;";exit;
      ...
      {
        $this->pdf->ln();
        $this->pdf->row(array(vertaalTekst('Fiscaliteit',$this->pdf->rapport_taal),
                          $this->formatGetal($alleGrootboeken['DIVBE'] + $alleGrootboeken['ROER'], 0),
                          $this->formatGetal(($alleGrootboeken['DIVBE'] + $alleGrootboeken['ROER']) / $gemiddelde * 100, 2).' %'));
      }
			$this->pdf->SetFont($this->pdf->rapport_font, '', $this->pdf->rapport_fontsize);
    ...
    $this->vkmWaarde=array('vkmPercentagePortefeuille'=>$vkmPercentagePortefeuille,'kostenPercentage'=>$kostenPercentage,'vkmWaarde'=>$vkmWaarde,'grootboekKosten'=>$grootboekKosten,
      ...
      'totaalDoorlopendekostenGesplitst'=>$totaalDoorlopendekostenGesplitst,'doorlopendeKostenPercentage'=>$doorlopendeKostenPercentage,
      'percentageIndirectVermogenMetKostenfactor'=>$perfTotaal['percentageIndirectVermogenMetKostenfactor'],'spreadKosten'=>$spreadKostenEUR,
      'fondsGemiddeldeWaarde'=>$perfTotaal['gemWaarde'],'gbDetails'=>$gbDetails);
...
ORDER BY HoofdcategorienVolgorde.Afdrukvolgorde, HoofdBeleggingscategorien.Afdrukvolgorde,BeleggingscategorienVolgorde.Afdrukvolgorde, BeleggingscategorienVolgorde.Afdrukvolgorde, Beleggingscategorien.Afdrukvolgorde,fondsVolgorde,OptieBovenliggendFonds,FondsOmschrijving ";
    ...
    foreach (array('Indirecte (fonds)kosten','BTW over Beheervergoeding') as $key)
    {
      ...
      {
        $grootboekKleuren[$key] = $mogelijkeKleuren[$n];
      }
    }
