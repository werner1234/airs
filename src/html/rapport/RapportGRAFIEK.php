...
class RapportGRAFIEK
{
	function RapportGRAFIEK($pdf, $portefeuille, $rapportageDatumVanaf, $rapportageDatum)
	{
		$this->pdf = &$pdf;
...
if ($this->pdf->rapport_GRAFIEK_sortering == 1)
  $order = 'TijdelijkeRapportage.hoofdsectorVolgorde ASC';
else
  $order = 'WaardeEuro desc';
	...
	$query = "SELECT SUM(TijdelijkeRapportage.actuelePortefeuilleWaardeEuro) as WaardeEuro,
			TijdelijkeRapportage.hoofdsector as Hoofdsector,
 			TijdelijkeRapportage.hoofdsectorOmschrijving as Omschrijving
			FROM
			TijdelijkeRapportage
			WHERE
			...
			.$__appvar['TijdelijkeRapportageMaakUniek']."
			GROUP BY TijdelijkeRapportage.hoofdsector ORDER BY $order;";
	debugSpecial($query,__FILE__,__LINE__);
	$DB->SQL($query);
	$DB->Query();
	...
	while($sec = $DB->nextRecord())
	{
	  if ($sec['Hoofdsector']== "")
	  {
	  	if (round($sec['WaardeEuro'] - $totaalLiquiditeiten,1) != 0)
	  	{
	  		if(round($totaalLiquiditeiten,2) != 0)
	  		{
			$data['hoofdsectoren']['Liquiditeiten']['waardeEur']=$totaalLiquiditeiten;
    ...
    {
	    $data['hoofdsectoren'][$sec['Hoofdsector']]['waardeEur']=$sec['WaardeEuro'];
	    $data['hoofdsectoren'][$sec['Hoofdsector']]['Omschrijving']=$sec['Omschrijving'];
    }
	}

		if ($this->pdf->rapport_GRAFIEK_sortering == 1 && round($liquididiteiten['waardeEur'],2) != 0 ) // liquiditeiten toevoegen
	{
	  $data['hoofdsectoren']['Liquiditeiten']['waardeEur']     = $liquididiteiten['waardeEur'];
