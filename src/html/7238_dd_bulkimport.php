...
/*
    ...
    Author              : $Author: cvs $
    Laatste aanpassing  : $Date: 2018/10/17 09:32:29 $
    File Versie         : $Revision: 1.1 $

    $Log: 7238_dd_bulkimport.php,v $
    Revision 1.1  2018/10/17 09:32:29  cvs
    call 7238
...
while ( ($file = readdir($handle)) !== false )
{
    ...
    $progressStep = 0;
    if (is_dir($fp) AND $file != ".." AND $file != ".")
    {
      if (checkForPortefeuille(substr($file,0,6)))
      {
        $portefeuilleArray[] = array(
          "folder" => $fp,
          "portefeuille" => checkForPortefeuille(substr($file,0,6)),
          "rel_id" => $rel_id
        );
      }
    }
...
foreach($portefeuilleArray as $dir)
{
  $map          = $dir["folder"];
  $portefeuille = $dir["portefeuille"];
  $rel_id       = $dir["rel_id"];
  $pFiles = getFilesFromDir($map);
  foreach ($pFiles as $theFile)
  {
    ...
    $dd->addDocumentToStore($rec);
    //debug($rec);
    echo "<li>document toegevoegd aan portefeuille $portefeuille / $rel_id ($nameOnly).<br>\n";flush(); ob_flush();
  }
  debug($pFiles, $portefeuille);
...
<div class="mailbox">
  <table>
    ...
    <?
    foreach($portefeuilleArray as $msg)
    {
...
*/
  /*  2018--> DeGiro jaaropgave */
    ...
    $portefeuileCheckFailed = $filenaam;
    $query = "
  SELECT
    portefeuille,
    id
