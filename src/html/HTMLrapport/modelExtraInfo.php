...
$query = "SELECT * FROM `_htmlRapport_MODEL` WHERE id = ".$_GET["id"];
...
switch ($_GET["tab"])
{
  case "1":
    ...
    $query = $templ->parseBlock("getTransacties",array(
      ...
      "ytd" => "1971-01-01",
      "boekDatum" => $_SESSION['modelHtmlRapporttempData']['datum']
    ));
  ...
  case "3":
    ...
    */
    ...
    if ($rec["fondssoort"] == "OBL" OR $rec["fondssoort"] == "OVERIGE")
    {
      ...
      $extraData = "
      <tr>
        <td>" . vt('Huidige rentepercentage') . ":</td>
        <td>$rente</td>
      </tr>
      ";
    }
    ...
    */
    ...
    <article class="extraInfoiArticle" >
      <table class="extraInfoTable" >
        <tr>
          <td>
            <button id="btnYtd">vanaf 1 januari</button> &nbsp;&nbsp;<button id="btn2Y">2 jaar</button> &nbsp;&nbsp;<button id="btn5Y"><?= vt('5 jaar'); ?></button>
    ...
    <script>
      $(document).ready(function ()
      {
        ...
        {
          $("#infoTab4").load("modelExtraInfo.php?tab=4&id=<?=$_GET["id"]?>");
        });
        $("#btn2Y").click(function ()
        {
          $("#infoTab4").load("modelExtraInfo.php?tab=4_2y&id=<?=$_GET["id"]?>");
        });
        $("#btn5Y").click(function ()
        {
          $("#infoTab4").load("modelExtraInfo.php?tab=4_5y&id=<?=$_GET["id"]?>");
        });
      })
    ...
    */
    ...
    <article class="extraInfoiArticle">
      <table class="extraInfoTable">
        ...
        while ($rec = $db->nextRecord())
        {
          $jDatum = __julDag($_SESSION['modelHtmlRapporttempData']['datum']);
      ...
      ";
          ...
          {
            $saldo += $rec["Bedrag"];
            $rec["rekeningOmschrijving"] = "Saldo per ".$fmt->format("@D {d}-{m}-{Y}", $_SESSION['modelHtmlRapporttempData']['datum']);
            $rec["Bedrag"] = 0;
          ...
          if ($firstRec )
          {
            ...
            if ( date('m-d', strtotime($_SESSION['modelHtmlRapporttempData']['datum'])) == '01-01' ) {
              $rec["Bedrag"] = 0;
    ...
    <script>
      $(document).ready(function()
      {
        ...
        {
          $("#infoTab1").load("modelExtraInfo.php?tab=1R&id=<?=$_GET["id"]?>&stop=<?=$_GET['stop'];?>");
        });
        $("#btnTR1Y").click(function ()
        {
          $("#infoTab1").load("modelExtraInfo.php?tab=1R_1&id=<?=$_GET["id"]?>&stop=<?=$_GET['stop'];?>");
        });
        $("#btnTRhis").click(function ()
        {
          $("#infoTab1").load("modelExtraInfo.php?tab=1R_H&id=<?=$_GET["id"]?>&stop=<?=$_GET['stop'];?>");
        });
      });
    </script>
    <?
...
//      Grootboekrekening";
    ...
    <h1>Mutaties van <?=$mainRec["rekening"]?>, <?=$gbArray[$_GET["GBgrootboek"]]?>, <?=$kopTxt?> </h1>
    <article class="extraInfoiArticle">
      <table class="extraInfoTable">
        <tr class="trHeader">
          <td><?= vt('Datum'); ?></td>
    ...
    <script>
      $(document).ready(function()
      {
        $("#GBgrootboek").change(function ()
        {
          ...
          $("#infoTab2").load("modelExtraInfo.php?tab=<?=$_GET["tab"]?>&id=<?=$_GET["id"]?>&stop=<?=$_GET['stop'];?>&GBgrootboek="+$(this).val());
...
//          $("#btnGBhis").prop("disabled",false);
        ...
        {
          $("#infoTab2").load("modelExtraInfo.php?tab=2R&id=<?=$_GET["id"]?>&stop=<?=$_GET['stop'];?>&GBgrootboek="+$("#GBgrootboek").val());
        });
        $("#btnGB1Y").click(function ()
        {
          $("#infoTab2").load("modelExtraInfo.php?tab=2R_1&id=<?=$_GET["id"]?>&stop=<?=$_GET['stop'];?>&GBgrootboek="+$("#GBgrootboek").val());
        });
        $("#btnGBhis").click(function ()
        {
          $("#infoTab2").load("modelExtraInfo.php?tab=2R_H&id=<?=$_GET["id"]?>&stop=<?=$_GET['stop'];?>&GBgrootboek="+$("#GBgrootboek").val());
        });
      });
    </script>
    <?
