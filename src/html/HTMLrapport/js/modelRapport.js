$(document).ready(function()
{
  ...
  $(document).on({
    mouseenter: function () {
      $(this).find('.toggle1').hide("slide", { direction: "left" }, 1000);
      $(this).find('.toggle2').show("slide", { direction: "left" }, 1000);
    },
    mouseleave: function () {
      $(this).find('.toggle1').show();
      $(this).find('.toggle2').hide();
    }
  }, ".buttonToggle");
  ...
  $(".iBtn").click(function (e)
  {
    ...
    $("#infoTab1").load("modelExtraInfo.php?tab=1&id=" + btnId + "&stop=" + stop);
    $("#infoTab2").load("modelExtraInfo.php?tab=2&id=" + btnId + "&stop=" + stop);
    $("#infoTab3").load("modelExtraInfo.php?tab=3&id=" + btnId + "&stop=" + stop);
    $("#infoTab4").load("modelExtraInfo.php?tab=4&id=" + btnId + "&stop=" + stop);
    $("#titleTab1").click();
  });
  ...
  {
    $('#modeltest').css('display', 'table');
    if ($(this).is(':checked'))
    {
      hideCol = $(this).data('colname');
      $(document).find('*[data-field=' + hideCol + ']').slideDown('slow');
      $('#modeltest').find('*[data-field=' + hideCol + ']').slideDown('slow');
    }
    else
    {
      hideCol = $(this).data('colname');
      $(document).find('*[data-field=' + hideCol + ']').slideUp('slow');
      $('#modeltest').find('*[data-field=' + hideCol + ']').slideUp('slow');
    }
    $('#modeltest').css('display', 'none');
  });


  $('#modelTable').dragtable({
...
//[data-field=totaalAantal]
  ...
  {console.log('test12');
    ...
    var numbodies = $('#modelTable').find('tbody').length;

    $('#modelTable').find('tbody').each(function (i, el)
    {
      var table = $(this);
