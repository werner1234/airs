...
/*
    ...
    Author              : $Author: rm $
    Laatste aanpassing  : $Date: 2019/07/05 10:55:21 $
...
$query = "SELECT * FROM `_htmlRapport_TRANS` WHERE id = ".$_GET["id"];
...
?>
    <script>
      $(document).ready(function(){
        $("#titleTab1").text("<?= vt('Transacties'); ?>");
        $("#titleTab2").text("<?= vt('Dividenden/Coupons'); ?>");
        $("#titleTab3").text("<?= vt('Fondsgegevens'); ?>");
        $("#titleTab4").text("<?= vt('Koersinformatie'); ?>");
        $("#titleTab3").show();
        $("#titleTab4").show();
      });
    </script>
    ...
    <article class="extraInfoiArticle">
      <table class="extraInfoTable">
        <tr class="trHeader">
          ...
          <td class="ar"><?= vt('Valutakoers'); ?></td>
          <td class="ar"><?= vt('Rekening'); ?><br/><?= vt('Valuta'); ?></td>
          <td class="ar"><?= vt('Afreken'); ?><br/><?= vt('Bedrag'); ?></td>
        </tr>
  ...
  case "3":
    ...
    */
    ...
    if (trim($mainRec["beleggingssectorOmschrijving"]) <> "")
    {
      ...
      <tr>
        <td>" . vt('Sector') . ":</td>
        <td>".trim($mainRec["beleggingssectorOmschrijving"])."</td>
      </tr>
      ";
    }
    if (trim($rec["rating"]) <> "")
...
<?
  ...
  case "4_5y":
    ...
    */
    ...
    if ($_GET['stop'] == "0000-00-00")
    {
      $thisYear = date("Y");
    }
    else
    {
      $thisYear = substr($_GET['stop'],0,4);
    }
    ...
    switch ($_GET["tab"])
    {
      ...
      default:
        ...
        $title = vt("Koersverloop vanaf 1 januari");
    }
    ...
    <table class="extraInfoTable" >
      <tr>
        <td>
          ...
          <?= vt('Laatste koers'); ?>: <?=$fmt->format("@N{.2}",$koersRec["Koers"])." ".$rec["Valuta"]." d.d.:".$fmt->format("@D{d}-{m}-{Y}",$koersRec["Datum"])?><br/>
          <br/>
<?
  if ($fondsRec["koersBron"] == 1 AND $fondsRec["identifierVWD"] <> "")
  {
    echo "" . vt('VWD-code') . ": ".$fondsRec["identifierVWD"]."<br/>";
  }
?>
...
?>
        ...
        {
          $("#infoTab4").load("transExtraInfo.php?tab=4&id=<?=$_GET["id"]?>");
        });
        $("#btn2Y").click(function ()
        {
          $("#infoTab4").load("transExtraInfo.php?tab=4_2y&id=<?=$_GET["id"]?>");
        });
        $("#btn5Y").click(function ()
        {
          $("#infoTab4").load("transExtraInfo.php?tab=4_5y&id=<?=$_GET["id"]?>");
        });
      })
...
<?
  ...
  case "1R_H":
    ...
    */
    ...
    <script>
      $(document).ready(function(){
        $("#titleTab1").text("<?= vt('Rekenmutaties'); ?>");
        $("#titleTab2").text("<?= vt('Grootboekselectie'); ?>");
        $("#titleTab3").hide();
        $("#titleTab4").hide();
      });
    </script>
    ...
    <table class="extraInfoTable" >
      <tr>
        <td>
          <button id="btnTRYtd"><?= vt('vanaf 1 januari'); ?></button> &nbsp;&nbsp;
          <button id="btnTR1Y"><?= vt('1 jaar'); ?></button> &nbsp;&nbsp;
          <button id="btnTRhis"><?= vt('hele historie'); ?></button>
          <br/><br/>
        </td>
      </tr>
    </table>
...
//                ));
      ...
      ";
          ...
          {
            $saldo += $rec["Bedrag"];
            $rec["rekeningOmschrijving"] = "Saldo per ".$fmt->format("@D {d}-{m}-{Y}", $rec["Boekdatum"]);
            $rec["Bedrag"] = 0;
  ...
  <script>
    $(document).ready(function()
    {
      ...
      {
        $("#infoTab1").load("transExtraInfo.php?tab=1R&id=<?=$_GET["id"]?>&stop=<?=$_GET['stop'];?>");
      });
      $("#btnTR1Y").click(function ()
      {
        $("#infoTab1").load("transExtraInfo.php?tab=1R_1&id=<?=$_GET["id"]?>&stop=<?=$_GET['stop'];?>");
      });
      $("#btnTRhis").click(function ()
      {
        $("#infoTab1").load("transExtraInfo.php?tab=1R_H&id=<?=$_GET["id"]?>&stop=<?=$_GET['stop'];?>");
      });
    });
  </script>
    <?
...
//      Grootboekrekening";
    ...
    switch($_GET["tab"])
    {
      case "2R_1":
        $startDisp = (date("Y", db2jul($_GET['stop'])) - 1) . '-'. (date("m-d", db2jul($_GET['stop'])));;
        $date = (date("Y", db2jul($_GET['stop'])) - 1) . '-01-01';
        $kopTxt = "" . vt('vanaf') . " ". date('d-m-Y', db2jul($startDisp));
        break;
      case "2R_H":
        $date = "1971-01-01";
        $kopTxt = vt("beschikbare historie");
        break;
      default:
        $date = (date("Y", db2jul($_GET['stop']))) . "-01-01";
        $kopTxt = "" . vt('vanaf') . " 01-01-".(date("Y", db2jul($_GET['stop'])));
    }
    $dateStop = $_GET['stop'];
    ...
    <table class="extraInfoTable" >
      <tr>
        <td>
          <?= vt('Grootboek'); ?>: <select id="GBgrootboek" name="GBgrootboek">
            <?=$options?>
          </select>&nbsp;&nbsp;&nbsp;
          <button id="btnGBYtd"><?= vt('vanaf 1 januari'); ?></button> &nbsp;&nbsp;
          <button id="btnGB1Y"><?= vt('1 jaar'); ?></button> &nbsp;&nbsp;
          <button id="btnGBhis"><?= vt('hele historie'); ?></button>
          <br/><br/>
        </td>
      </tr>
    </table>
    <h1><?= vt('Mutaties van'); ?> <?=$mainRec["rekening"]?>, <?=$gbArray[$_GET["GBgrootboek"]]?>, <?=$kopTxt?> </h1>
    <article class="extraInfoiArticle">
      <table class="extraInfoTable">
        <tr class="trHeader">
          <td><?= vt('Datum'); ?></td>
    ...
    <script>
      $(document).ready(function()
      {
        $("#GBgrootboek").change(function ()
        {
          ...
          $("#infoTab2").load("transExtraInfo.php?tab=<?=$_GET["tab"]?>&id=<?=$_GET["id"]?>&stop=<?=$_GET['stop'];?>&GBgrootboek="+$(this).val());
...
<?
      ...
      switch($_GET["tab"])
      {
        ...
        ?>
          ...
          {
            $("#infoTab2").load("transExtraInfo.php?tab=2R&id=<?=$_GET["id"]?>&stop=<?=$_GET['stop'];?>&GBgrootboek="+$("#GBgrootboek").val());
          });
          $("#btnGB1Y").click(function ()
          {
            $("#infoTab2").load("transExtraInfo.php?tab=2R_1&id=<?=$_GET["id"]?>&stop=<?=$_GET['stop'];?>&GBgrootboek="+$("#GBgrootboek").val());
          });
          $("#btnGBhis").click(function ()
          {
            $("#infoTab2").load("transExtraInfo.php?tab=2R_H&id=<?=$_GET["id"]?>&stop=<?=$_GET['stop'];?>&GBgrootboek="+$("#GBgrootboek").val());
          });
      });
    </script>
<?
  ...
  case '5':
    ...
    foreach ( $doorkijkArray as $msCategoriesoort => $doorkijkDatas )
    {
      ...
      $editObject->formVars['doorkijkDetails'] .= '<div class="formblock"><div class="formlinks" style="width: 400px;"> <b>' . $msCategoriesoort . '</b> </div> <div class="formrechts"><br>vanaf ' . date('d-m-Y', db2jul($vanaf)) . '</div></div>';
      $count = 1;
      $type = 'pi';
