...
//    $prb->moveNext();
    ...
    if ($data[1] == "SECURITYPOS")
    {
      ...
      if ($data[5] <> "")
      {
        $q = "SELECT * FROM Fondsen WHERE aabbeCode='$data[5]' ";
    ...
    else
    {
      ...
      // leenrekening nr aanpassen
      if ($data[99] == "0002")
      {
        $record["rekening"]  = "L-".$record["rekening"];
      }

      $idx = $record["rekening"].$record["valuta"].$data[99];
      if (isset($cashOutput[$idx]))
      {
        $bedrag = str_replace(",",".",$data[5]);
        $cashOutput[$idx]["bedrag"] += $bedrag;
      }
      else
      {
        $cashOutput[$idx] = $record;
    }
  ...
  foreach ($cashOutput as $key=>$record)
  {
    $output[] = $record;
    $recon->addRecord($record);
  }
...
{   
  ...
  while (!feof($handle))
  {
    ...
    if ($data[1] <> "SECURITYPOS" )
    {
        $error[] = "Bestand validatie mislukt, geen AABBE positie bestand";
        return false;
    }
  }  
  fclose($handle);
  ...
  while (!feof($handle))
  {
    ...
    $data = convertFixedLine($dataRaw);
    if ( $data[1] <> "000000000000000"   )
    {
        $error[] = "Bestand validatie mislukt, geen AABBE Cash bestand";
        return false;
    }
  }  
  fclose($handle);
...
function convertFixedLine($rawData,$debug=false)
{
  ...
  else
  {
    ...
    $data[99] = textPart($rawData,56,59);               // rek type
