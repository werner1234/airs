...
//  $prb->moveMin();
  ...
  while ($data = fgetcsv($handle, 4096, ","))
  {
    ...
    $record = array("depot" => "KBC", "batch" => $batch . "/" . $teller);  // reset $record per ingelezen regel
...
//    }
    ...
    if ($filetype == "GLD")
    {
      if ($data[4] != "AMOUNTS ON VALUE DATE")
      {
        continue;
      }
      $record["type"] = "cash";
      $record["rekening"] = $data[0];
    ...
    else
    {
      ...
      $record["datum2"] = $recon->testDate;
      $record["soort"] = "";
      $record["aantalRaw"] = $data[9];
      $record["aantal"] = $data[9];
      $record["ISIN"] = trim($data[2]);
      $record["valuta"] = $data[8];
...
//debug($data, $filetype);
    if (count($data) == 6 AND
      strlen($data[3]) == 3 )      // valuta kolom
    {

    }
    else
    {
      $err[] = "FOUT geen KBC geldbestand";
    }
    fclose($handle);
  }
  else
  {
    ...
    $data = fgetcsv($handle, 4096, ",");
//    debug($data, $filetype);
    if (count($data) == 11 AND
      strlen($data[8]) == 3 )      // valuta kolom
    {

    }
    else
    {
      $err[] = "FOUT geen KBC positiebestand ";
    }
    fclose($handle);
  }
