...
require_once('../html/rapport/PDFRapport.php');

class HTML2FPDF extends PDFRapport
{
//internal attributes
var $HREF; //! string
var $pgwidth; //! float
  ...
  $this->blockjustfinished=false;
    ...
    case 'ADDRESS': 
			...
			if(isset($attr['SRC']))
      {
        $srcpath = $attr['SRC'];
        global $__appvar;
        if(substr($attr['SRC'],0,10)=='data:image')
        {
          $type=substr($attr['SRC'],11,4);
          
          if($type=='png;')
          {
             $img=base64_decode(substr($attr['SRC'],22)); 
             $srcpath=$__appvar["tempdir"].'/'.md5($img).'.png';
             file_put_contents($srcpath,$img);
          }
          if($type=='jpeg')
          {
             $img=base64_decode(substr($attr['SRC'],23));
             $srcpath=$__appvar["tempdir"].'/'.md5($img).'.jpg';
             file_put_contents($srcpath,$img);
          }
        }
            ...
            if($bak_y+$attr['HEIGHT'] > $this->PageBreakTrigger)
              $this->AddPage();
              
            $sizesarray = $this->Image($srcpath, $this->GetX(), $this->GetY(), $attr['WIDTH'], $attr['HEIGHT'],'','',false);
            
            
            $this->y = $bak_y+$attr['HEIGHT'];
            $this->x = $bak_x;
            $xpos = '';
