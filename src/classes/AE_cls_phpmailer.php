...
class PHPMailer {
   ...
   * @var string
   */
  var $action_function = ''; //'callbackAction';
  ...
  var $STOP_MESSAGE  = 0; // message only, continue processing
  var $STOP_CONTINUE = 1; // message?, likely ok to continue processing
  var $STOP_CRITICAL = 2; // message, plus full stop, critical error reached
   ...
   * @return bool
   */
   function mail_passthru($to, $subject, $body, $header, $params) {
  ...
  function IsSMTP() {
        global $__appvar;
        $this->Mailer = "smtp";
        if(isset($__appvar["smtpServer"]))
          $this->Host = $__appvar["smtpServer"];
        else
        {
          $cfg=new AE_config();
          $this->Host =$cfg->getData('smtpServer');
          $smtpUser=$cfg->getData('smtpUser');
          $smtpPwd=$cfg->getData('smtpPwd');
          
          if($smtpUser <>'')
          {
            $this->SMTPAuth=true;
            $this->Username=$smtpUser;
            $this->Password=$smtpPwd;
            $smtpPort=$cfg->getData('smtpPort');
            $smtpSecure=$cfg->getData('smtpSecure');
            if($smtpSecure<>'')
  ...
  function AddAddress($address, $name = '') 
  {
    ...
    if(count($multipleEmail) > 1)
    {
      foreach ($multipleEmail as $email)
        $this->AddAnAddress('to',trim($email),'');
    }
    else
  ...
  function AddCC($address, $name = '') 
  {
    ...
    {
      foreach ($multipleEmail as $email)
        $this->AddAnAddress('cc',trim($email),'');
    }
    else
  ...
  function AddBCC($address, $name = '') 
  {
    ...
    {
      foreach ($multipleEmail as $email)
        $this->AddAnAddress('bcc',trim($email),'');
    }
    else
  ...
  function AddAnAddress($kind, $address, $name = '') 
  {
    if (!preg_match('/^(to|cc|bcc|Reply-To)$/', $kind)) {
      ...
      if ($this->exceptions) {
        new phpmailerException('Invalid recipient array: ' . $kind);
      }
      if ($this->SMTPDebug) {
    ...
    if (!$this->ValidateAddress($address)) {
      ...
      if ($this->exceptions) {
         new phpmailerException($this->Lang('invalid_address').': '.$address);
      }
      if ($this->SMTPDebug) {
  ...
  function Send() {
      global $__appvar;
      if(isset($__appvar['debugEmail']))
      {
        $this->From     = $__appvar['debugEmail'];
        $this->FromName = "Airs debug";
      }
      if(!$this->PreSend())
	    return false;
      $returncode=$this->PostSend();
      $this->logMail($returncode);
      return $returncode;
  }
  ...
  function PreSend() {
      ...
      if ((count($this->to) + count($this->cc) + count($this->bcc)) < 1) {
        new phpmailerException($this->Lang('provide_address'));
      }

      if(isset($__appvar['debugEmail']))
      {
        if($this->ValidateAddress($__appvar['debugEmail']))
        {
          $this->all_recipients=array();
          if(count($this->to) > 0)
          {
            $this->to=array();
            $this->AddAddress($__appvar['debugEmail'],'Debug to');
          }
          if(count($this->cc) > 0)
          {
            $this->cc=array();   
            $this->AddCC($__appvar['debugEmail'],'Debug cc');
          }
          if(count($this->bcc) > 0)
          {
            $this->bcc=array();
            $this->AddBCC($__appvar['debugEmail'],'Debug bcc');
          }
        }
        else
        {
          echo "Ongeldig debug emailadres (".$__appvar['debugEmail'].") opgegeven in __appvar['debugEmail']";
          exit;
        }                 
      }
    ...
    if ($this->SingleTo === true) {
      foreach ($this->SingleToArray as $val) {
        if(!@$mail = popen($sendmail, 'w')) {
          new phpmailerException($this->Lang('execute') . $this->Sendmail);
        }
        fputs($mail, "To: " . $val . "\n");
        fputs($mail, $header);
        fputs($mail, $body);
  ...
  function MailSend($header, $body) {
    ...
    if(!$rt) {
      new phpmailerException($this->Lang('instantiate'));
    }
    return true;
  }
  ...
  function SmtpSend($header, $body) {
    global $__appvar;
    require_once $this->PluginDir . 'AE_cls_smtp.php';
    $bad_rcpt = array();

    if(!$this->SmtpConnect()) {
      new phpmailerException($this->Lang('smtp_connect_failed'));
    }
    ...
    if (count($bad_rcpt) > 0 ) { //Create error message for any bad addresses
      ...
      new phpmailerException($this->Lang('recipients_failed') . $badaddresses);
    }
    
    $this->fullEmail=$header . $body;
    if(!$this->smtp->Data($header . $body)) {
      new phpmailerException($this->Lang('data_not_accepted'));
    }
      ...
      if ($this->smtp->Connect(($ssl ? 'ssl://':'').$host, $port, $this->Timeout, $options)) {
          ...
          if ($this->SMTPAuth) {
            if (!$this->smtp->Authenticate($this->Username, $this->Password, $this->AuthType, $this->Realm, $this->Workstation)) {
              new phpmailerException($this->Lang('authenticate'));
            }
          }
          return true;
  ...
  */
  ...
  function GetSentMIMEMessage() {
    return $this->MIMEHeader . $this->mailHeader . $this->CRLF . $this->MIMEBody;
  }
    ...
    if ($this->IsError()) {
      $body = '';
        ...
        if (@openssl_pkcs7_sign($file, $signed, 'file://'.realpath($this->sign_cert_file), array('file://'.realpath($this->sign_key_file), $this->sign_key_pass), null)) {
          ...
          @unlink($file);
          @unlink($signed);
          new phpmailerException($this->Lang('signing').openssl_error_string());
        }
  ...
  function AddAttachment($path, $name = '', $encoding = 'base64', $type = '', $disposition = 'attachment')
  {
      ...
      if ( !@is_file($path) ) {
         new phpmailerException($this->Lang('file_access') . $path);
      }

      //If a MIME type is not specified, try to work it out from the file name
      if ($type == '') 
	  {
        $type = $this->filenameToType($path);
      }
  ...
  */
  ...
  function EncodeFile($path, $encoding = 'base64')
  {
      ...
      if (!is_readable($path)) 
	  {
        new phpmailerException($this->Lang('file_open') . $path);
      }
  ...
  function EncodeString($str, $encoding = 'base64') {
    ...
    switch(strtolower($encoding)) {
      ...
      case '7bit':
      case '8bit':
        $encoded = $this->FixEOL(wordwrap($str,250));
   ...
   * @return string
   */
  function EncodeQPphp($string, $line_max = 76, $space_conv = false) {
   ...
   * @return void
   */
  function AddStringAttachment($string, $filename, $encoding = 'base64', $type = '', $disposition = 'attachment',$cid=0) {
    //If a MIME type is not specified, try to work it out from the file name
    if ($type == '') {
   ...
   * @return bool True on successfully adding an attachment
   */
  function AddStringEmbeddedImage($string, $cid, $name = '', $encoding = 'base64', $type = '', $disposition = 'inline') {
    ...
    if (isset($images[2])) {
      foreach ($images[2] as $i => $url) {
        ...
        if (!preg_match('#^[A-z]+://#', $url)) {
          ...
          if ($this->AddEmbeddedImage($basedir.$directory.$filename, $cid, $filename, 'base64', $this->_mime_types(self::mb_pathinfo($filename, PATHINFO_EXTENSION)))) {
  ...
  function filenameToType($filename) {
    ...
    return $this->_mime_types($pathinfo['extension']);
  }
  ...
  function set($name, $value = '') {
      ...
      if (isset($this->$name) ) {
        ...
        new phpmailerException($this->Lang('variable_set') . $name );
      }
   ...
   * @static
   */
  function NormalizeBreaks($text, $breaktype = "\r\n") {
    return preg_replace('/(\r\n|\r|\n)/ms', $breaktype, $text);
  }
   ...
   * @param string $from
   */
  function doCallback($isSent, $to, $cc, $bcc, $subject, $body, $from = null) {
    ...
    function encodeInlineImages($eMail)
    {
      
      preg_match_all('/<img[^>]*>/i',$eMail,$allImgs);
      foreach($allImgs[0] as $i=>$img)
      {
        $dataEncoding='base64';
        preg_match('/data:([^;]*);(.*),([^=]*)/i',$img,$image);
        if(count($image)>1)
        {
          $dataType=$image[1];
          ...
          if(count($parts)>1)
          {
            ...
            {
              if(strtolower(substr($part,0,8))=='filename')
                $filename=substr($part,9);
              elseif(strtolower(substr($part,0,6))=='base64')
                $dataEncoding='base64';
            }
          }
  
          $content['image'.$i]['type'] = $dataType;
          $content['image'.$i]['decoded'] = base64_decode($dataPart);
          $this->AddStringAttachment($content['image'.$i]['decoded'], $filename, $dataEncoding, $dataType,'inline','image'.$i);
          
          //$content['image'.$i]['type'] = $image[1];
          //$content['image'.$i]['decoded'] = base64_decode($image[2]);
          //$this->AddStringAttachment($content['image'.$i]['decoded'], 'image', "base64", $image[1],'inline','image'.$i);
         
          $img=preg_replace('/src="[^"]*"/i','src="cid:image'.$i.'"',$img);
          $img=preg_replace('/<img/i','<tmpimg',$img);
          $eMail=preg_replace('/<img[^>]*>/i',$img,$eMail,1);
         }
         else
         {
           preg_match('/src="([^"]*)"/i',$img,$image);
           $imgContent = file_get_contents($image[1]);
           if ($imgContent <> '')
           {
             $content['image' . $i]['decoded'] = $imgContent;
             $this->AddStringAttachment($imgContent, basename($image[1]), "base64", "application/octet-stream", 'inline', 'image' . $i);
             $img = preg_replace('/src="[^"]*"/i', 'src="cid:image' . $i . '"', $img);
             $img = preg_replace('/<img/i', '<tmpimg', $img);
             $eMail = preg_replace('/<img[^>]*>/i', $img, $eMail, 1);
           }
      ...
      $eMail=preg_replace('/<tmpimg/i','<img',$eMail);
      return $eMail;
      
    }
    
  function logMail($lastCode)
  {
    global $USR;
    $db=new DB();
    $zender=mysql_real_escape_string($this->From);
    $ontvangers='';
    $cc='';
    $bcc='';
    $onderwerp=mysql_real_escape_string($this->Subject);
    $script=mysql_real_escape_string(basename($_SERVER['PHP_SELF']));
    foreach($this->to as $to)
      $ontvangers.=$to[0].";";
    ...
    foreach($this->bcc as $to)
      $bcc.=$to[0].";";
    $ontvangers=mysql_real_escape_string($ontvangers);
    $cc=mysql_real_escape_string($cc);
    $bcc=mysql_real_escape_string($bcc);
    $error='';
    if($lastCode==0)
      $error=mysql_real_escape_string($this->ErrorInfo);
    $bijlagen='';  
    foreach($this->attachment as $attData)
    {
      $bijlagen.=$attData[1]."; ";
    }
    $bijlagen=mysql_real_escape_string(trim($bijlagen));  
    $body=mysql_real_escape_string($this->Body);

        
    $query="INSERT INTO emailLog 
    SET zender='$zender',ontvangers='$ontvangers',cc='$cc',bcc='$bcc',onderwerp='$onderwerp',verzonden='$lastCode',bijlagen='$bijlagen',body='$body',
        vanaf='$script',foutmelding='$error',add_date=now(),change_date=now(),add_user='$USR',change_user='$USR'";
    $db->SQL($query);
    $db->Query();  
    //ErrorInfo
  } 
...
class phpmailerException {
   ...
   * @return string
   */
  function phpmailerException($txt)
  {
    echo $txt."<br>\n";
  } 
