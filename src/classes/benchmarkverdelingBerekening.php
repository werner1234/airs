...
/*
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2019/09/25 15:28:07 $
 		File Versie					: $Revision: 1.22 $

 		$Log: benchmarkverdelingBerekening.php,v $
 		Revision 1.22  2019/09/25 15:28:07  rvv
 		*** empty log message ***
 		
 		Revision 1.21  2019/02/23 18:28:46  rvv
 		*** empty log message ***
 		
 		Revision 1.20  2018/05/24 05:58:53  rvv
 		*** empty log message ***
 		
 		Revision 1.19  2018/05/23 13:44:24  rvv
 		*** empty log message ***
 		
 		Revision 1.18  2017/08/12 12:00:33  rvv
 		*** empty log message ***
...
*/
class benchmarkverdelingBerekening
{
  function benchmarkverdelingBerekening($benchmark)
  {
    $this->benchmarks=array();
    $this->nietBijgewerkteRecords=array();
    $this->kostenPerFonds=array();
    if($benchmark)
      $this->benchmarks[]=$benchmark;
    else
    {
      $db=new DB();
      $query="SELECT benchmark FROM benchmarkverdeling GROUP BY benchmark";
      $db->SQL($query);
      $db->Query();
  ...
  function bereken($benchmark,$vanaf='')
  {
    ...
    $query="SELECT OptieUitoefenPrijs as kostenPercentage FROM Fondsen WHERE Fonds='".$benchmark."'";
    $db->SQL($query);
    $db->Query();
    while($data=$db->nextRecord())
      $this->kostenPerFonds[$benchmark]=$data['kostenPercentage'];
    
    
    $query="SELECT fonds,percentage FROM benchmarkverdeling WHERE benchmark='".$benchmark."'";
    $db->SQL($query);
    $db->Query();
    while($data=$db->nextRecord())
      $this->fondsPercentages[$benchmark][$data['fonds']]=$data['percentage'];
...
//echo $vanafFilter;exit;
    ...
    foreach ($this->fondsPercentages[$benchmark] as $fonds=>$percentage)
    {
      $query="SELECT Koers,Datum FROM Fondskoersen WHERE Fonds='$fonds' AND Datum >= '".$this->laatsteKoers[$benchmark]['Datum']."' ORDER BY Datum ASC";
      $db->SQL($query);
      $this->error[]=$query;
      $db->Query();
      $koersen=array();
      ...
      $this->fondsKoersen[$benchmark][$fonds]=$tmp;
      //echo "$benchmark  $fonds <br>\n";
    }
    
    $laatsteKoers='';
    foreach ($this->fondsKoersen[$benchmark] as $fonds=>$fondsWaarden)
    {
      $n=0;
      $laatsteDatum=$this->laatsteKoers[$benchmark]['Datum'];
      foreach ($fondsWaarden['koersen'] as $datum=>$koersen)
      {
        $datumOke=true;
        
        foreach ($this->fondsPercentages[$benchmark] as $fonds2=>$percentage)
        {
          if(!isset($this->fondsKoersen[$benchmark][$fonds2]['koersen'][$datum]))
          {
            $datumOke=false;
        ...
        if($datumOke)
        {
          if($n>0)
          {
            ...
            if(isset($this->kostenPerFonds[$benchmark]))
              $kostenPercentage=$this->kostenPerFonds[$benchmark];
            else
              $kostenPercentage=0;
  //listarray($this->kostenPerFonds);exit;
            $dagen=round((db2jul($datum)-db2jul($laatsteDatum))/86400);
            $laatsteDatum=$datum;
      
            //$kostenDeel = 1+(($kostenPercentage)/100);
            //$kostenDeel=pow($kostenDeel,($dagen/365))-1;
            $kostenDeel=($kostenPercentage)/100*($dagen/365);
            
            $dagRendement=($dagRendement-$kostenDeel);
            
             //echo "$dagen | $datum | $laatsteKoers -> ".$koersen['Koers']." | $kostenDeel | $dagRendement | $kostenPercentage <br>\n";
  
            
            $this->fondsKoersen[$benchmark][$fonds][$datum]['dagRendement']=$dagRendement;
  ...
  function calulateEuribor($benchmark='',$vanaf='',$debug=false)
  {
    global $USR;
    $cfg=new AE_config();
    $db=new DB();
    ...
    $indices=$this->getEuriborFondsen();
    if($benchmark<>'')
    {
      $tmpIndices=$indices;
      $indices=array();
      foreach($tmpIndices as $index)
      {
        if($benchmark==$index['indexFonds'])
          $indices[]=$index;
      }
    }
