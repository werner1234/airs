...
require_once realpath(dirname(__FILE__)) . '/BIFFwriter.php';

if(!class_exists('Spreadsheet_Excel_Writer_Worksheet'))
{
...
//            // Match formula
//            return $this->writeFormula($row, $col, $token, $format);
        } elseif (preg_match("/^[0-9]{2,4}-[0-9]{2}-[0-9]{2,4}/", $token) && (strlen($token)==10 || strlen($token)==19) ) {
            // Match date
            return $this->writeDate($row, $col, $token, $format);
        }
        elseif ($token == '') {
            // Match blank
    ...
    */
    ...
    function writeDate($row, $col, $waarde, $format = null)
    {

       $lengte=strlen($waarde);  
       $waarde=substr($waarde,0,10);
       
       if(substr($waarde,2,1)=='-' && substr($waarde,5,1)=='-' && (substr($waarde,6,1)=='1' || substr($waarde,6,1)=='2') && ($lengte==10 || $lengte=19))
       {
         $waarde=round((adodb_form2jul($waarde)+(86400 * 25569))/86400);
         $this->writeNumber($row, $col, $waarde, $this->_date_format);
       }  
       elseif(substr($waarde,4,1)=='-' && substr($waarde,7,1)=='-' && (substr($waarde,0,1)=='1' || substr($waarde,0,1)=='2') && ($lengte==10 || $lengte=19))
       {
         $waarde=round((adodb_db2jul($waarde)+(86400 * 25569))/86400); 
         $this->writeNumber($row, $col, $waarde, $this->_date_format);
       }  
    ...
    * c:\foo.xls#Sheet1!A1', $__appvar["basedir"].'/../foo.xls'. and $__appvar["basedir"].'/../foo.xls#Sheet1!A1'.
    *
