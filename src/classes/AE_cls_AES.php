...
/* 	
    ...
    Author  						: $Author: rvv $
 		Laatste aanpassing	: $Date: 2018/08/22 10:14:43 $
 		File Versie					: $Revision: 1.7 $
 		
 		$Log: AE_cls_AES.php,v $
 		Revision 1.7  2018/08/22 10:14:43  rvv
 		PHP 5.6
 		
 		Revision 1.6  2018/08/18 12:40:13  rvv
 		php 5.6 & consolidatie
 		
 		Revision 1.5  2017/09/24 16:28:27  rvv
 		*** empty log message ***
 		
 		Revision 1.4  2017/09/24 14:58:03  rvv
 		*** empty log message ***
 		
 		Revision 1.3  2017/09/24 14:53:17  rvv
 		*** empty log message ***
 		
 		Revision 1.2  2017/09/24 14:12:39  rvv
 		*** empty log message ***
 		
 		Revision 1.1  2017/09/24 14:05:07  rvv
 		*** empty log message ***
...
class AE_AES
{
  ...
  function AE_AES($pass)
  {
    global $USR;
	  $this->user = $USR;

    if($pass=='')
      $pass= "abcd3a32123d[}!4s&1dgd";
    $this->key=md5($pass.'-h5'.(ord(substr($pass,0,1))*3+7)).md5($pass.'-1a'.(ord(substr($pass,1,2))*1+3));
  ...
  function encrypt($plain_text)
  {
    ...
    {
      $iv=pack('H*',md5((time()/7).$this->key.time()));
      $aes = new AES($this->key, 'CBC', $iv);
      $ciphertext = $iv.$aes->encrypt($plain_text);
    }
    return $ciphertext;
  }
  ...
  function decrypt($enc_text)
  {
    if($this->useMcrypt==true)
    {
      ...
      $iv_dec = substr($enc_text, 0, $iv_size);
      # retrieves the cipher text (everything except the $iv_size in the front)
      $enc_text = substr($enc_text, $iv_size);
      # may remove 00h valued characters from end of plain text
      $plaintext_dec = mcrypt_decrypt(MCRYPT_RIJNDAEL_128, $this->key,
                                      $enc_text, MCRYPT_MODE_CBC, $iv_dec);
    }
    else
    {
      $iv= substr($enc_text, 0, 16);
      $aes = new AES($this->key, 'CBC', $iv);
      $plaintext_dec = $aes->decrypt($enc_text);
      $plaintext_dec=substr($plaintext_dec,16);
    }
    $plaintext_dec=base64_decode($plaintext_dec);
    return $plaintext_dec;

  }

  function encrypt_file($inFile,$outFile)
  {

    file_put_contents($outFile,$this->encrypt(file_get_contents($inFile)));
  }
  
  function decrypt_file($inFile,$outFile)
  {
    file_put_contents($outFile,$this->decrypt(file_get_contents($inFile)));
  }
}
...
class AES
{
  ...
  var $w = array(); // Nb*(Nr+1) 32-bit words
  ...
  function AES($z, $mode = 'ECB', $iv = null)
  {
    ...
    if ($this->mode != "ECB" && strlen($this->iv) != 16) {
      die('The initialization vector must be 128 bits (or 16 characters) long.'.strlen($this->iv));
    }
