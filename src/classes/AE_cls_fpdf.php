...
function AddPage($orientation='')
{
	...
	if($family)
		$this->SetFont($family,$style,$size);
	//Restore colors
	if($this->skipRestoreColor == false)
	{
	...
	if(!isset($this->fonts[$fontkey]))
	{
		...
		if(isset($this->CoreFonts[$fontkey]))
		{
			if(!isset($fpdf_charwidths[$fontkey]))
			{
				...
				include($file);
				if(!isset($fpdf_charwidths[$fontkey]))
					$this->Error('Could not include font metric file for '.$fontkey.'. (File charwidths not found in '.$file.')');
			}
			$i=count($this->fonts)+1;
...
function Cell($w,$h=0,$txt='',$border=0,$ln=0,$align='',$fill=0,$link='')
{
	...
	if($fill==1 or $border==1 or is_array($fill))
	{
    if(is_array($fill))
    {
...
function Output($name='',$dest='')
{
	...
	global $HTTP_SERVER_VARS,$USR;
  
  if($this->stopOutput==true)
  {
    $this->buffer='';
    return '';
  }
...
function _putpages()
{
	...
	if(!empty($this->AliasNbPages))
	{
		...
		if($this->AliasNbPages=='{customPageNo}')
		  $endPage=$this->customPageNo;
		else
		  $endPage=$nb;
		for($n=1;$n<=$nb;$n++)
			$this->pages[$n]=str_replace($this->AliasNbPages,$endPage,$this->pages[$n]);
	}
...
{
  $tmp="";
...
function _parsepng($file)
{
	...
	$info = array('w'=>$w, 'h'=>$h, 'cs'=>$colspace, 'bpc'=>$bpc, 'f'=>'FlateDecode', 'parms'=>$parms, 'pal'=>$pal, 'trns'=>$trns);
...
{
 if ($this->encrypted)
	 return str_replace("\r",'\\r',str_replace(')','\\)',str_replace('(','\\(',str_replace('\\','\\\\',$s))));
 else
...
function _putencryption()
{
    ...
    //echo 'U '.$this->strToHex($this->Ovalue)."<br>\n";
    //echo 'P '.$this->strToHex($this->Uvalue)."<br>\n";
...
function DisplayPreferences($preferences)
{
    ...
    function _parsemempng($var)
    {
        ...
        elseif($ct==3)
            ...
            $this->Error('Unknown color type: '.$ct);
